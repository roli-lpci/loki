...
Ledger path resolution (first wins):
  --ledger PATH
  $LOKI_CITATION_LEDGER
  $LOKI_HOME/cache/citations/ledger.json
"""
...
from _loki_home import get_loki_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("LOKI_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_loki_home() / "cache" / "citations" / "ledger.json"
