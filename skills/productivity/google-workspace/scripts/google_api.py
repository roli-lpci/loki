#!/usr/bin/env python3
"""Google Workspace API CLI for Loki Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Loki-facing JSON contract and falls back to the Python client
...
from _loki_home import get_loki_home
...
def _gws_binary() -> str | None:
    override = os.getenv("LOKI_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Loki Agent")
