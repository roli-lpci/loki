#!/usr/bin/env python3
"""Bridge between Loki OAuth token and gws CLI.
...
def get_token_path() -> Path:
    return get_loki_home() / "google_token.json"
...
def refresh_token(token_data: dict) -> dict:
    ...
    params = urllib.parse.urlencode({
        ...
        # The refresh token goes in BOTH the body and the ``x-wundercorp-refresh-token`` header. Portal's token
        ...
        # name must match Portal's ``REFRESH_TOKEN_HEADER`` exactly (``x-wundercorp-refresh- token``); any other
