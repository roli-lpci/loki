#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Loki Agent.
...
# Ensure sibling modules (_loki_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _loki_home import display_loki_home, get_loki_home

LOKI_HOME = get_loki_home()
TOKEN_PATH = LOKI_HOME / "google_token.json"
CLIENT_SECRET_PATH = LOKI_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = LOKI_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Loki profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Loki Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Loki Docker image's "
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Loki home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_loki_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Loki")
