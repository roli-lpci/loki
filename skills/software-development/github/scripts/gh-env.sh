#!/usr/bin/env bash
# GitHub environment detection helper for Loki Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _loki_env="${LOKI_HOME:-$HOME/.loki}/.env"; [ -f "$_loki_env" ] && grep -q "^GITHUB_TOKEN=" "$_loki_env" 2>/dev/null; then
