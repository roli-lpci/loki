...
Author widget apps for the Loki TUI (`loki --tui`): glanceable ambient
...
- The TUI must be in use (`loki --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.loki/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.loki/logs/tui_gateway_crash.log` if a widget never appears.
