...
`loki config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `loki tools` (interactive) or `loki tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_LOKI_CORE_TOOLS` is the default bundle most platforms inherit).
