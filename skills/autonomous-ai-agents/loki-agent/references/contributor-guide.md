...
Config: `~/.loki/config.yaml` (settings), `~/.loki/.env` (API keys) — both under `$LOKI_HOME` when it is set.
...
`_LOKI_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_loki_home()` for paths,
never hardcode `~/.loki`. For custom/local-only tools, write a plugin in
`~/.loki/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `loki_cli/commands.py`
...
- Tests auto-redirect `LOKI_HOME` to temp dirs — never touch real `~/.loki/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/loki_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_loki_home()` from `loki_constants` for all paths (profile-safe)
