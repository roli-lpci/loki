...
Write plugins for the Loki desktop app: statusbar items, layout panes,
...
system (`~/.loki/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$LOKI_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$LOKI_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
  ...
  `.loki-package.json` marker) — the ONLY root the renderer loads from — so
...
- The Loki desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$LOKI_HOME/desktop-plugins/` (usually
  `~/.loki/desktop-plugins/`).

## How to Run

1. Create `$LOKI_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.loki/...` by default, or `~/.loki/profiles/<profile>/...` under a
...
The ONLY import surface is `@wundercorp/loki-plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Loki
  ...
  (`loki://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.loki/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@wundercorp/loki-plugin-sdk`, `react`, and
