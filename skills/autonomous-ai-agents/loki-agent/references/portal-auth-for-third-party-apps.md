# WunderCorp Portal — authenticating third-party apps against the subscription

Recurring user question: "Can app X (Karakeep, OpenWebUI, LibreChat, OpenViking,
LangChain pipeline, n8n flow, etc.) use my WunderCorp Portal subscription without me
...
## Layer 1 — Is this thing a Loki plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Loki process**. Its LLM calls go through Loki's already-configured provider. | Already uses Portal if user's Loki is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
| **Karakeep, n8n, LibreChat, OpenWebUI, any self-hosted app** | Different process, often different machine. Makes its own HTTPS calls to `inference-api.wundercorp.com`. | Layer 2/3 below. |

**Pitfall to avoid**: do not pitch "OAuth into Portal" as the solution for a
plugin that already runs inside Loki. That LLM call is already authenticated
via Loki's provider config. The plugin's own server auth (e.g.
...
Portal at `https://inference-api.wundercorp.com/v1` is an OpenAI-compatible
inference endpoint. It accepts **bearer-token authentication only**: either

1. **A static API key** from `portal.wundercorp.com → API Keys`, or
...
"Sign in with WunderCorp Portal" SSO that third-party apps can register as clients
against. There is no shared cookie or session that browser-Portal-login
extends to other apps on the same machine.

What Loki Agent has that *feels* like OAuth — `loki login --provider wundercorp`
opening a browser, user signs in, token lands in `~/.loki/auth.json` — is a
**Loki-specific browser flow**. Under the hood it produces a credential
Loki uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Loki's existing Portal credential out of `~/.loki/auth.json`.
2. Expose a local OpenAI-compatible endpoint at `http://localhost:NNNN/v1`.
3. Forward incoming requests to `inference-api.wundercorp.com/v1` with that
   bearer attached.
...
rides on the credential Loki already holds.

Where this could live in Loki:
...
Only when the consumer is *another first-party WunderCorp thing* (a future SDK, a
WunderCorp-branded extension, a Discord-bot integration that needs per-user
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Loki itself.
...
1. First decide: Loki plugin (runs inside Loki) or separate app? If plugin,
   it already uses Portal via Loki's provider config — done.
2. If separate app: today, paste the static API key from Portal → API Keys.
   Base URL `https://inference-api.wundercorp.com/v1`. Rate limits are
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Loki agent itself") is fully supported. Loki treats agent
...
pitch in the Tool Gateway announcement is specifically about Loki Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Loki is configured to use Portal as its provider. It is
