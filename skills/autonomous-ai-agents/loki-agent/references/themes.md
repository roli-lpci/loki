# Themes / Skins — Author a Loki Color Theme

Author a Loki **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`loki_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.loki/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Loki itself.
...
- Write access to the Loki home dir — `~/.loki` by default, or `$LOKI_HOME`
  / the active profile's dir. Skins live in `<loki-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `loki config set`).
...
3. `write_file` it to `<loki-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `loki config set display.skin default` (or they can `/skin default`).
...
```
loki skin set <key> <hex>      # e.g. loki skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `loki skin set background "#08201f"` changes
only the background; `loki skin use <name>` / `loki skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.loki`** when a profile is active — resolve the real home
  from `$LOKI_HOME` first, falling back to `~/.loki`.
...
- **Name collisions**: a skin named like a desktop built-in (`mono`, `slate`,
  `cyberpunk`, `wundercorp`, `midnight`, `ember`) won't override that built-in on the
  GUI. Pick a fresh name.
- **Never hand-edit `config.yaml` to activate.** Use `loki config set
...
- **You apply it, not the user.** `loki config set display.skin <name>` is
...
- `read_file` the written `<loki-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `loki config get display.skin` and confirm it reports `<name>`.
