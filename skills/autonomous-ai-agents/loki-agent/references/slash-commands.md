...
Registry of record: `loki_cli/commands.py` (`COMMAND_REGISTRY`) — every
...
/snapshot [sub]          Create/restore Loki config+state snapshots (CLI)
...
/subscription (/upgrade) WunderCorp plan info (CLI)
/topup                   WunderCorp balance / billing
/copy [N]                Copy last response to clipboard (CLI)
/paste                   Attach clipboard image (CLI)
/image <path>            Attach a local image file (CLI)
/update                  Update Loki to latest
/version (/v)            Show version
/debug [wundercorp|local]      Upload debug report, get shareable links
```
