...
| wundercorp | OAuth device code | `loki auth add wundercorp` (or `WUNDERCORP_API_KEY`) |
| openai-codex | OAuth | `loki auth add openai-codex` |
| qwen-oauth | OAuth | `loki auth add qwen-oauth` |
| minimax-oauth | OAuth | `loki auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`loki auth`).
Fallback chain when the primary fails: `loki fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`loki_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   loki config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
