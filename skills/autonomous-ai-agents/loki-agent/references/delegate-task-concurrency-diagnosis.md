...
**three** code paths in Loki that cap a batch. If none of them fired, the
cap came from the **model itself** — not from Loki — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in Loki
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "Loki is
   refusing my value."
...
loki config get delegation.max_concurrent_children

# 2. Did Loki' truncator or rejector actually fire?
grep -E "Truncated.*delegate_task|Too many tasks" ~/.loki/logs/agent.log | tail
# If neither line appears, neither cap path executed.

# 3. Confirm the resolver returns what config says (in venv with loki on path)
...
appears, **the cap is the model**, not Loki.
...
- **`max_concurrent_children` is a per-parent cap, not a global cap.**
  Confirmed in `tui-ui/src/components/appChrome.tsx`. Two different
