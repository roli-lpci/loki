 ...
 * `loki-boot-background` / `loki-boot-color-scheme` localStorage keys.
 *
 * Theme resolution is asynchrowundercorp by nature (gateway skin arrives after
...
// Profile-aware: the Python launcher exports LOKI_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.loki matches get_loki_home()'s default.
const bootFilePath = () => join(process.env.LOKI_HOME ?? join(homedir(), '.loki'), 'tui-theme-boot.json')

// Never touch the user's real ~/.loki from test runs (the TS suite has no
// LOKI_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into LOKI_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.LOKI_TUI_THEME || env.LOKI_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.LOKI_TUI_BACKGROUND
  ) {
    env.LOKI_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.LOKI_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.LOKI_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into LOKI_TUI_THEME.
