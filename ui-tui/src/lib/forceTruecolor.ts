...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.LOKI_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Loki on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via LOKI_TUI_TRUECOLOR=1.
