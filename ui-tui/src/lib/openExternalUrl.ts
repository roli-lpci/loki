 ...
 * (`openCommand` returned null), or (c) `spawn()` threw synchrowundercorply
...
export function openExternalUrl(rawUrl: string, dependencies: OpenDependencies = {}): boolean {
  ...
  try {
    ...
    // Async failure path: spawn returns a ChildProcess synchrowundercorply even
    ...
    // consumer; we already returned `true` synchrowundercorply, so the user
    ...
    // spawn can also throw synchrowundercorply on argv-validation failures
