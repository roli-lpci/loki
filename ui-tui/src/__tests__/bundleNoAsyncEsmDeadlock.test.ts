 ...
 *
 * Root cause: re-exporting `ink-text-input` from `@wundercorp/loki-ink`'s
 * entry-exports drags the upstream `ink` package into the bundle. That
 * `ink` graph and our in-tree `@wundercorp/loki-ink` graph reference each other
 ...
 *     runs synchrowundercorply, `__esm`'s closure-capture quirk is irrelevant.
...
function bundleIsFresh(): boolean {
  ...
  try {
    ...
    const sourceMtime = statSync(resolve(uiTuiRoot, 'packages/loki-ink/src/entry-exports.ts')).mtimeMs
...
describe('TUI bundle (issue #31227)', () => {
  ...
  it('has the @wundercorp/loki-ink entry-exports module compiled to sync init', () => {
    // Sanity check that the alias swap to packages/loki-ink/src/entry-exports.ts
    // is still active and producing the expected synchrowundercorp init shape.
    expect(bundleSrc).toMatch(/var init_entry_exports = __esm\(\{\s*"packages\/loki-ink\/src\/entry-exports\.ts"\(\)/)
  })
})
