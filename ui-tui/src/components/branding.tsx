import { Box, Text, useStdout } from '@wundercorp/loki-ink'
import { useEffect, useState } from 'react'
import unicodeSpinners from 'unicode-animations'

import { artWidth, logo, LOGO_WIDTH, wordmark, WORDMARK_WIDTH } from '../banner.js'
...
const TAG_FULL = 'WunderCorp, Inc. · Loki Agent'
const TAG_MID = 'Loki Agent'
const TAG_TINY = 'WunderCorp, Inc.'
const HIDE_BELOW = 34
...
export function SessionPanel({ info, maxWidth, sid, t }: SessionPanelProps) {
  ...
  const heroMaxWidth = cols >= 132 ? WORDMARK_WIDTH : cols >= 40 ? 24 : 9
  const heroLines = wordmark(t.color, t.bannerHero || undefined, heroMaxWidth)
  const leftW = Math.min((artWidth(heroLines) || heroMaxWidth) + 4, Math.floor(cols * 0.4))
  const wide = cols >= 96 && leftW + 44 < cols
  ...
  // loki_cli/banner.py) and the "connected" label on the collapse toggle.
  ...
  const heroColumn = wide ? (
    <Box alignItems="center" flexDirection="column" width="100%">
      ...
      <Text color={t.color.muted}>by WunderCorp, Inc.</Text>
      <Text />
  ...
  const infoColumn = (
    <Box flexDirection="column" width="100%">
      {wide ? (
        ...
        // Narrow layout stacks the compact identity above session metadata.
        // This keeps Loki recognizable in ordinary 80-column terminals without
        // forcing the two-column panel to clip the art or tool list.
      ...
      {typeof info.update_behind === 'number' && info.update_behind > 0 && (
        <Text bold color={t.color.warn}>
          ...
          <Text bold color={t.color.warn}>
            {info.update_command || 'loki update'}
          </Text>
          <Text bold={false} color={t.color.warn} dimColor>
            {' '}
            to update
