import type { InputEvent, Key } from '@wundercorp/loki-ink'
import * as Ink from '@wundercorp/loki-ink'
 ...
 * but that accepts `key.meta` on macOS — and loki-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.LOKI_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which loki-ink decodes as meta+'d'; without this branch it
        ...
        if (text.length > 1 || text.includes('\n')) {
          ...
          // synchrowundercorply so the recomposed value reaches the parent atomically.
          commit(v, c)
