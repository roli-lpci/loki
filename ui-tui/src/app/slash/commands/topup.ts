import { driveChargeSettlement, type SettlementOutcome } from '@wundercorp/loki-shared/charge-settlement'
...
const renderBillingError = (
  ...
  switch (env.error) {
    ...
    case 'remote_spending_disabled':
      ...
      // turn it on from the portal's Loki Agent page; this is NOT a per-terminal stop.
      sys(
      ...
      ctx.gateway
        ...
        .then(
          ctx.guarded<BillingStateResponse>(s => {
            if (!s.logged_in) {
              sys('💳 Not logged into WunderCorp Portal — run /portal to log in, then /topup.')
