...
import { terminalBackgroundHex } from '@wundercorp/loki-ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $LOKI_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see LOKI_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['LOKI_TUI_BACKGROUND', process.env.LOKI_TUI_BACKGROUND ?? '(unset)'],
            ['LOKI_TUI_THEME', process.env.LOKI_TUI_THEME ?? '(unset)'],
