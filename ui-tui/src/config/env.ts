...
export const STARTUP_RESUME_ID = (process.env.LOKI_TUI_RESUME ?? '').trim()
export const STARTUP_QUERY = (process.env.LOKI_TUI_QUERY ?? '').trim()
export const STARTUP_IMAGE = (process.env.LOKI_TUI_IMAGE ?? '').trim()
...
//
// - LOKI_TUI_MOUSE_TRACKING (truthy/falsy) explicitly overrides everything.
//   This is the "force a value" knob and intentionally beats the legacy
//   kill-switch and the Termux default.
// - LOKI_TUI_DISABLE_MOUSE=1 forces mouse off — the legacy kill switch.
...
//   behavior.
const mouseTrackingOverride = parseToggle(process.env.LOKI_TUI_MOUSE_TRACKING)
const mouseTrackingDisabledLegacy = truthy(process.env.LOKI_TUI_DISABLE_MOUSE)
...
// LOKI_DEV_CREDITS — dev-only live-spend readout (Δ status segment + "(dev credits)"
...
const inlineOverride = parseToggle(process.env.LOKI_TUI_INLINE)
...
// less fragile. Override explicitly with LOKI_TUI_INLINE=0/1.
