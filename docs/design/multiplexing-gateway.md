...
_profile_runtime_scope(profile_home)           (gateway/run.py)
   ├── set_loki_home_override(home)          config / state.db / skills /
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`LOKI_HOME`, `LOKI_PROFILE`, proxy settings,
...
reload helper: `loki_cli.env_loader.load_loki_dotenv` skips the
...
## The LOKI_HOME override

`loki_constants.py` holds a context-local override consulted by
`get_loki_home()` before the `LOKI_HOME` env var. Everything that resolves
...
automatically. `get_process_loki_home()` exists for the few machine-level
assets that must not follow the override. `loki_home_key()` gives
...
`profiles_to_serve(multiplex, profile_allowlist)` in `loki_cli/profiles.py`
...
DB handles are resolved at call time through the active LOKI_HOME override —
...
LOKI_HOME override. Asset writes are atomic, type- and size-capped.
...
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `loki_home_key()`. |
...
- `agent/secret_scope.py`, `loki_constants.py`, `gateway/profile_routing.py`,
  `gateway/run.py` (`_profile_runtime_scope`), `loki_cli/profiles.py`
