...
Supersedes: PR #31781 (prompt_toolkit `loki profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `loki profile wizard` in the terminal.
...
needs, and a profile is just a LOKI_HOME directory — so the dashboard is the
right home for a full-featured builder, and it can reuse everything that
already exists.

A profile = a full `~/.loki/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — LOKI_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with LOKI_HOME override |
...
### Seam #1 — hub-skill install cannot use the LOKI_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = LOKI_HOME / "skills"` at **module
import time**. The context-local `set_loki_home_override()` swap (which makes
...
The correct mechanism is the existing subprocess path: `_spawn_loki_action`
runs `python -m loki_cli.main <subcommand>`, and `_apply_profile_override()`
re-reads `sys.argv` at import in the fresh child. Prepend `-p <profile>`:

```python
_spawn_loki_action(["-p", profile, "skills", "install", identifier], "skills-install")
```

A fresh subprocess re-imports `skills_hub` with the profile's LOKI_HOME bound
...
Built-in/optional skill enabling and MCP writes are **synchrowundercorp config ops**
and can be part of the create call. Hub installs are long-running git fetches
spawned detached (`_spawn_loki_action` returns a PID immediately). So the
create flow is:

1. `create_profile()` — make the dir (synchrowundercorp)
2. write model (synchrowundercorp, LOKI_HOME override)
3. write selected MCP servers (synchrowundercorp, LOKI_HOME override)
4. seed/enable selected built-in + optional skills (synchrowundercorp)
5. spawn `loki -p <profile> skills install <id>` per hub skill (async, returns PIDs)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchrowundercorp, LOKI_HOME override
    builtin_skills: List[str] = []               # synchrowundercorp enable/seed
...
page afterward. Mirror `_write_profile_model`'s LOKI_HOME-override helper for
...
- Backend E2E with isolated LOKI_HOME: POST a full create body
