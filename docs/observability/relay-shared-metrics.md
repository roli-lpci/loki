...
Loki includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Loki and does not require a Loki observability plugin. Loki remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Loki execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `loki-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Loki `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `LOKI_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `LOKI_NEMO_RELAY_ATOF_*` and `LOKI_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Loki does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Loki requires NeMo Relay 0.8.3 or later within the 0.8 release line. That
line provides the provider-codec and canonical tool-result contracts Loki
uses for managed provider and tool calls.

## Runtime Dependency and Data Boundary

Loki installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Loki process so configured interceptors can
...
Collection remains off unless Loki policy enables it:
...
opt-in. Set `LOKI_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Loki does not invoke Relay's plugin initializer, so Relay
...
selected file cannot be loaded, Loki reports the error and does not invoke
...
retain the same `session_id` and add `loki.session.segment` plus
`loki.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Loki
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Loki process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Loki logs:
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Loki
process and share the same policy boundary.

Run profiles in separate Loki processes when they require different trust
...
Loki core owns one Relay host and one isolated Relay session scope per Loki
...
marks do not require Loki plugin registration. Shared-metrics marks must
...
```text
Loki turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Loki shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Loki sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Loki used for the logical call, such as
...
New calls use `loki.model_route.count`. The previous
`loki.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `loki.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Loki
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Loki processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `loki.task_run`, parented to
the owning Loki session. The start counter contains only bounded execution
...
provider attempts for the same Loki API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Loki tool-call
...
cancellations. Active task ownership follows the task ID if Loki rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`loki.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Loki
...
tool or plugin names. Loki does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `loki.tool_approval` marks and
...
Successful skill mutations emit `loki.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `loki.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Loki derives reuse and patch-generation
...
```text
$LOKI_HOME/telemetry/shared_metrics/metrics.sqlite3
$LOKI_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Loki version,
...
schema field name, its current scope is one `LOKI_HOME`, so it is more
...
Deleting `$LOKI_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `LOKI_HOME`. To reset it, stop Loki
processes and remove `$LOKI_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Loki CLI turn against the deterministic local model server:
...
`$LOKI_HOME/telemetry/shared_metrics/outbox/` to the Loki telemetry ingest
...
telemetry:
  shared_metrics:
    ...
    send: false      # NEW: transmit to the WunderCorp telemetry service
```
...
Removing `$LOKI_HOME/telemetry/shared_metrics` still resets local identity,
