...
Loki observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Loki' planner, model providers, memory, tool registry,
...
Loki also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "loki.observer.v1"
```

Hook callbacks are fail-open. Loki catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Loki only builds
...
The native NeMo Relay SDK integration maps Loki session, turn, LLM, and tool
