...
This document is the formal interface between the **Loki gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`WunderCorp/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `loki gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`loki gateway enroll` → connector `/relay/enroll`, or
...
(`WunderCorp/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `loki` CLI.
