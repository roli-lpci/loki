...
When running Loki inside Docker, the default `network_mode: host` gives the
...
The Loki [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   LOKI_UID=$(id -u) LOKI_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=loki,loki-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://loki-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Loki trust model and vulnerability reporting
