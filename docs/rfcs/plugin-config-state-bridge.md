...
or change arbitrary Loki configuration.
...
small host fact requested by the RFC. Settings writes use Loki'
...
`LOKI_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<LOKI_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
The implementation is covered with real temporary-Loki-home tests for:
...
- [Issue #64227](https://github.com/wundercorp/loki/issues/64227)
- [RFC PR #58542](https://github.com/wundercorp/loki/pull/58542) by Topher Ross
