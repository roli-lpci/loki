...
`loki config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.loki/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `LOKI_HOME`, secret scope, and
