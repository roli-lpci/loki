...
## 2026-07-13: Scope plugin manager state by Loki home/profile (keyed cache)

Status: Accepted

Context:
Loki supports multiple profiles via different Loki home directories.
Homes are switched two ways in a running process: the `LOKI_HOME`
environment variable (single-profile CLI/gateway processes), and the
context-local `set_loki_home_override()` (`loki_constants.py`), which
...
`get_loki_home() / "plugins"`, and context-engine plugins (e.g.
`loki-lcm`) capture profile-scoped state — such as the LCM database
path — at registration time. A single-slot cache meant:

1. Switching homes via `set_loki_home_override()` was invisible to a
   naive "did `LOKI_HOME` change" check, so the singleton silently kept
...
2. Even when a fresh `PluginManager` *was* created for a new home, plugin
   modules are imported into `sys.modules` as `loki_plugins.<slug>` by
   ...
   `loki_plugins.<slug>.<submodule>`, and Python's import machinery
...
- Replace the single-slot singleton with a cache keyed on the *resolved*
  Loki home path (`_plugin_managers_by_home: Dict[Path, PluginManager]`).
  `get_plugin_manager()` resolves the current home via `get_loki_home()`
  (which itself already consults `get_loki_home_override()` before
...
- Per-profile LCM instances (and any other context-engine plugin) use
  ...
  through `LOKI_HOME` or `set_loki_home_override()`.
...
- Regression coverage exercises the real production path
  (`set_loki_home_override()`) rather than only the env-var path, and
