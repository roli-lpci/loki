...
Loki middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Loki'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Loki calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Loki retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Loki guardrails,
...
- `telemetry_schema_version`: currently `loki.observer.v1`
- `middleware_schema_version`: currently `loki.middleware.v1`
...
Loki stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Loki runs
them as a nested chain in registration order. Middleware failures are fail-open:
Loki logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Loki applies middleware in this order:
...
For each tool call, Loki applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Loki pre-execution path against the effective arguments:
...
For isolated local testing, use one `LOKI_HOME` for plugin enablement and the
agent run:

```bash
export LOKI_HOME=/tmp/loki-middleware-test
mkdir -p "$LOKI_HOME"
loki plugins enable <plugin-name>
loki chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run loki plugins enable <plugin-name>
uv run loki chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["loki_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/loki-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Loki expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Loki treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Loki preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Loki does not convert downstream
