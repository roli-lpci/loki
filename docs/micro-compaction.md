...
has to be thrown away or summarized. Loki has always done this in one batch:
...
Loki folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Loki's normal session flush is
...
synchrowundercorp `/models` probe and telemetry must never be what blocks a turn.
...
Defaults to `$LOKI_HOME/logs/agent.log`. It reports passes, outcome mix, net
