...
`loki doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `loki doctor --fix` while the gateway
is still up. Next steps:

```bash
loki gateway stop
LOKI_HOME="$HOME/.loki" loki sessions recover --source "$HOME/.loki/state.db" --inspect-only
# if recoverable:
LOKI_HOME="$HOME/.loki" loki sessions recover --source "$HOME/.loki/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
loki gateway stop
LOKI_HOME="$HOME/.loki" loki sessions repair --check-only
LOKI_HOME="$HOME/.loki" loki sessions repair
```
