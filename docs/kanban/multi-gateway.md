...
Loki supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.loki/config.yaml`:
...
Or set the env var: `LOKI_KANBAN_DISPATCH_IN_GATEWAY=false`
