...
The canonical transcript store is SQLite via `SessionDB` (from `loki_state`). The
...
Counts consecutive restarts via a JSON file (`{LOKI_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `loki update`, `loki gateway restart`,
or `/restart`.
