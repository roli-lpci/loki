...
from loki_constants import OPENROUTER_BASE_URL, get_loki_home
from agent.retry_utils import jittered_backoff
from loki_cli.env_loader import load_loki_dotenv

# Load .env from LOKI_HOME first, then project root as a dev fallback.
load_loki_dotenv(loki_home=get_loki_home(), project_env=Path(__file__).parent / ".env")
...
_PROVIDER_HOSTS: Tuple[Tuple[str, str], ...] = (
    ("openrouter.ai", "openrouter"), ("wundercorp.com", "wundercorp"), ("z.ai", "zai"), ("moonshot.ai", "kimi-coding"),
...
class TrajectoryCompressor:
    ...
    def _init_summarizer(self):
        ...
        if provider:
            ...
            if client is None:
                raise RuntimeError(f"Provider '{provider}' is not configured. Check your API key or run: loki setup")
