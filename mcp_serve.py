"""
Loki MCP Server — expose messaging conversations as MCP tools (`loki mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the Loki-specific
channels_list. Client config: {"mcpServers": {"loki": {"command": "loki", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("loki.mcp_serve")
...
def _loki_home() -> Path:
    try:
        ...
        return Path(os.environ.get("LOKI_HOME", Path.home() / ".loki"))


def _get_sessions_dir() -> Path:
    return _loki_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_loki_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_loki_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the Loki analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Loki MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(f"MCP server requires the 'mcp' package. Install with: {sys.executable} -m pip install 'mcp'")
    mcp = MCPServer("loki", instructions=(
        "Loki Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Loki MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
