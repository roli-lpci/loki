...
Loki runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.loki/plugins/`, registering a backend the user selects
...
| `loki setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `loki status` / `loki doctor` | `doctor_checks()` |
...
```python title="~/.loki/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.loki"    # where synced cache files land, or None
...
```yaml title="~/.loki/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
loki plugins enable acmebox
loki config set terminal.backend acmebox
```
