...
description: "How to add a new inference provider to Loki Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Loki can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `loki model` menu entries
...
1. `loki_cli/auth.py` decides how credentials are found.
2. `loki_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `loki_cli/models.py` and `loki_cli/main.py` make the provider show up in the CLI. (`loki_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `loki_cli/auth.py`
2. `loki_cli/models.py`
3. `loki_cli/runtime_provider.py`
4. `loki_cli/main.py`
...
:::tip
`loki_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `loki setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$LOKI_HOME/plugins/model-providers/` both get picked up.
...
7. `loki model` menu includes the provider
8. `loki setup` wizard delegates to `main.py` automatically
...
User plugins at `$LOKI_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- OAuth or token refresh (WunderCorp Portal, Codex, Qwen Portal, Copilot)
...
- Provider-specific `loki model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `loki_cli/auth.py`
- `_PROVIDER_LABELS` in `loki_cli/models_catalog_static.py` (re-exported by `loki_cli/models.py`)
- `_PROVIDER_ALIASES` in both `loki_cli/auth.py` and `loki_cli/models_catalog_static.py`
- CLI `--provider` choices in `loki_cli/main.py`
...
## Step 2: Add auth metadata in `loki_cli/auth.py`
...
- OAuth / auth-store path: WunderCorp, OpenAI Codex

Questions to answer here:

- What env vars should Loki check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `loki_cli/models.py`
...
## Step 4: Resolve runtime data in `loki_cli/runtime_provider.py`
...
Be careful with API-key precedence. Loki already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `loki_cli/main.py`

A provider is not discoverable until it shows up in the interactive `loki model` flow.

Update these in `loki_cli/main.py`:
...
:::tip
`loki_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `loki model` and `loki setup` automatically.
:::
...
When you add a native provider, double-check that Loki is only sending fields that provider actually understands.
...
- `tests/loki_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/loki_cli/test_model_switch_custom_providers.py` (and adjacent `tests/loki_cli/test_model_switch_*.py`)
- `tests/loki_cli/test_setup_model_provider.py`
...
python -m pytest tests/loki_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/loki_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -q
```
...
```bash
source venv/bin/activate
python -m loki_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `loki_cli/auth.py`
- [ ] aliases added in `loki_cli/auth.py` and `loki_cli/models.py`
- [ ] model catalog added in `loki_cli/models.py`
- [ ] runtime branch added in `loki_cli/runtime_provider.py`
- [ ] CLI wiring added in `loki_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `loki model` but not `loki setup`
