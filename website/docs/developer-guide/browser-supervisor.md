...
description: "How Loki detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---

# Browser CDP Supervisor

The CDP supervisor closes two long-standing gaps in Loki' browser tooling:
...
`window.alert`/`confirm`/`prompt` with a synchrowundercorp XHR to a magic host
(`loki-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs before
...
One `asyncio.Task` running in a background daemon thread per Loki `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `loki-acp`, `loki-api-server`, and core toolsets (gated on CDP reachability)
- `loki_cli/config.py` — `browser.dialog_policy` and `browser.dialog_timeout_s` defaults
