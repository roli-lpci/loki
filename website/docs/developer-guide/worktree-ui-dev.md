...
| Variable | Meaning |
|----------|---------|
| `LOKI_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `LOKI_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `LOKI_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Loki itself — they're private to these helpers. The variables Loki *does* read are covered in [Environment Variables](../reference/environment-variables.md).
...
htui() {
  local root
  root="$(_loki_root)" || { echo "htui: not in a Loki checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$LOKI_MAIN_CHECKOUT/.venv/bin/python" -m loki_cli.main --tui --dev "$@" )
}
```
...
hgui() {
  ...
  root="$(_loki_root)" || { echo "hgui: not in a Loki checkout" >&2; return 1; }
  deps="${LOKI_GUI_DEPS_CHECKOUT:-$LOKI_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _loki_link_deps "$desktop" "$deps/apps/desktop"
    _loki_link_deps "$root" "$deps"
  else
  ...
  trap '_loki_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    LOKI_DESKTOP_ROOT="$root" \
    LOKI_DESKTOP_PYTHON="$LOKI_MAIN_CHECKOUT/.venv/bin/python" \
    LOKI_DESKTOP_IGNORE_EXISTING=1 \
    LOKI_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `LOKI_DESKTOP_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `loki`. |
| `LOKI_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `LOKI_DESKTOP_IGNORE_EXISTING` | Ignores any `loki` on `PATH` so it can't shadow the worktree. |
| `LOKI_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Loki checkout.
_loki_root() {
  local root
...
_loki_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'loki_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `loki --tui --dev` and the `LOKI_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/wundercorp/loki/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `LOKI_*` variable Loki reads
