...
description: "How to build a memory provider plugin for Loki Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Loki Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `loki plugins`.
:::

## Installation Layouts

Loki discovers memory providers from four sources, in this precedence order:

| Source | Location | Notes |
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with Loki. Closed to new providers — see [CONTRIBUTING](https://github.com/wundercorp/loki/blob/main/CONTRIBUTING.md). |
| User | `$LOKI_HOME/plugins/<name>/` | Dropped in by the user, per profile. |
| Project | `./.loki/plugins/<name>/` | Opt-in via `LOKI_ENABLE_PROJECT_PLUGINS=1`. |
| Package | `loki_agent.memory_providers` entry point | `pip install`, nothing to copy. |
...
Loki, in `$LOKI_HOME/plugins/<name>/` when installed by a user, or in
`./.loki/plugins/<name>/` for a project-local one:
...
`loki_agent.memory_providers` group. The entry-point name is the provider
name users select in `memory.provider`; its value points to the provider's
`register(ctx)` function:

```toml title="pyproject.toml"
[project.entry-points."loki_agent.memory_providers"]
...
package layout. No copy under `$LOKI_HOME/plugins/` is required.

A package entry point gets everything a directory install does, including the
two files Loki reads from disk rather than importing — `config_schema.py`
(the dashboard config panel) and `cli.py` (your `loki <provider>`
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          loki_home (str): Active LOKI_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `loki memory setup` | **Yes** |
| `save_config(values, loki_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
The gate binds to every compaction authority, not just the Loki
...
required checkpoint cannot be guaranteed there. The checkpoint-aware Loki
...
`get_config_schema()` returns a list of field descriptors used by `loki memory setup`:
...
Every field in `get_config_schema()` is prompted during `loki memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$LOKI_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::

## Save Config

```python
def save_config(self, values: dict, loki_home: str) -> None:
    """Write non-secret config to your native location."""
    import json
    from pathlib import Path
    config_path = Path(loki_home) / "my-provider.json"
...
the `messages` parameter; Loki will continue calling them with the legacy
signature.
...
All storage paths **must** use the `loki_home` kwarg from `initialize()`, not hardcoded `~/.loki`:

```python
# CORRECT — profile-scoped
from loki_constants import get_loki_home
data_dir = get_loki_home() / "my-provider"

# WRONG — shared across all profiles
data_dir = Path("~/.loki/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `loki my-provider status`, `loki my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `loki <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `loki --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: loki my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the loki my-provider argparse tree.
