...
description: "How Loki stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `loki_cli/cron.py` | CLI `loki cron` subcommands |
...
Jobs are stored in `~/.loki/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
renderer (`loki cron list`/`doctor`, the `cronjob` tool, the web dashboard
...
In CLI mode, cron jobs only fire when `loki cron` commands are run or during active CLI sessions.
...
keep the process awake), it asks WunderCorp infrastructure to arm exactly **one
managed one-shot per job at that job's real next-fire time**. At fire time WunderCorp
...
The flow (the managed scheduler is provided by WunderCorp; the agent holds no
scheduler credentials):

```
create/update a cron job
  → Chronos asks WunderCorp to arm a one-shot at the job's next_run_at
      (authenticated with the agent's existing WunderCorp token)
  → at fire time WunderCorp calls the gateway: POST {callback_url}/api/cron/fire
      (authenticated with a short-lived, purpose-scoped WunderCorp-minted JWT)
...
Config (all non-secret; on hosted agents WunderCorp sets these at provision time):

| key | meaning |
|---|---|
| `cron.provider` | `chronos` to activate (empty = built-in ticker) |
| `cron.chronos.portal_url` | WunderCorp base URL (arming + the fire-token issuer) |
...
If Chronos is misconfigured or the agent isn't logged into WunderCorp,
...
agent↔WunderCorp wire contract lives in `docs/chronos-managed-cron-contract.md`.
...
```python
# ~/.loki/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.loki/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `loki cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `loki cron` CLI provides direct job management:

```bash
loki cron list                    # Show all jobs
loki cron create                  # Interactive job creation (alias: add)
loki cron edit <job_id>           # Edit job configuration
loki cron pause <job_id>          # Pause a running job
loki cron resume <job_id>         # Resume a paused job
loki cron run <job_id>            # Trigger immediate execution
loki cron remove <job_id>         # Delete a job
```
