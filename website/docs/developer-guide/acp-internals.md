...
The ACP adapter wraps Loki' synchrowundercorp `AIAgent` in an async JSON-RPC stdio server.
...
```text
loki acp / loki-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.loki/.env
  -> configure stderr logging
  -> construct LokiACPAgent
...
- `allow_once` -> Loki `once`
- `allow_always` -> Loki `always`
- reject options -> Loki `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Loki tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["loki-acp"])
...
Instead it reuses Loki' runtime resolver:

- `acp_adapter/auth.py`
- `loki_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Loki provider/credentials. It also always advertises a terminal setup auth method (`loki-setup`, args `--setup`) so first-run ACP clients can open Loki' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.loki/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `loki-acp` toolset definition
- `loki_cli/main.py` — `loki acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `loki-acp` script
