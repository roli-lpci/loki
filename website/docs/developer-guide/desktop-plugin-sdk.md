...
title: "Desktop Plugin SDK (@wundercorp/loki-plugin-sdk)"
description: "Extend the native Loki Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Loki Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `LokiPlugin`.
It imports one module — `@wundercorp/loki-plugin-sdk` — and gets everything: the app's
...
`$LOKI_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Loki. This page is the **native
desktop app** (`loki desktop`) SDK — the `@wundercorp/loki-plugin-sdk` module and
`$LOKI_HOME/desktop-plugins/`. The **web dashboard** (`loki dashboard`) has
its own, unrelated plugin system on `window.__LOKI_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Loki Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$LOKI_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$LOKI_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `LokiPlugin` contract, appear in **Capabilities → Plugins**,
...
[`loki-example-plugins`](https://github.com/WunderCorp/loki-example-plugins)
repo.

## Quick start — your first plugin

Create `$LOKI_HOME/desktop-plugins/hello/plugin.js` (that's `~/.loki/...`
...
```javascript
// ~/.loki/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@wundercorp/loki-plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Loki' }),
      jsx('div', {
...
The only importable specifiers are `@wundercorp/loki-plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `LokiPlugin`:
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `loki.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@wundercorp/loki-plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@wundercorp/loki-plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@wundercorp/loki-plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@wundercorp/loki-plugin-sdk'
...
```javascript
import { Button, useTheme } from '@wundercorp/loki-plugin-sdk'
...
```javascript
import { host, requestTheme } from '@wundercorp/loki-plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@wundercorp/loki-plugin-sdk'
...
`window.loki.send('get-price eth')` (or a declarative
`<button data-loki-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@wundercorp/loki-plugin-sdk'
...
for requests; `targetProfile` is the backend Loki profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Loki
...
ctx.os.notify({
  ...
  // Body click → focus Loki + navigate. Same vocabulary as OS deep links:
  activate: 'loki://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'loki://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `loki://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `loki://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@wundercorp/loki-plugin-sdk'
...
```javascript
import { queryClient } from '@wundercorp/loki-plugin-sdk'
...
copies that half into `$LOKI_HOME/desktop-plugins/<id>/` beside a
`.loki-package.json` marker, and the renderer loads it through the exact same
pipeline as the standalone disk door (hot reload included):

```
~/.loki/plugins/<id>/           # ONE installable folder
...
is refreshed when the source `plugin.js` changes (`loki plugins update`, or
...
a package into `~/.loki/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.loki/plugins` is not reachable as a filesystem —
...
the `loki://` scheme — a plain anchor on your website or README:
...
`dashboard/` subfolder of a regular Loki plugin and declare it in a
`manifest.json`:
...
loki-agent codebase directly (`loki_state`, `loki_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.loki/`)
...
(`loki.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `LokiPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@wundercorp/loki-plugin-sdk` alias.
2. It's still lint-fenced to `@wundercorp/loki-plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@wundercorp/loki-plugin-sdk`, `react`,
...
| Plugin contract | `LokiPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `LokiOpenTarget`, `Contribution` |
...
### Agents: the `loki-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`loki-desktop-plugins`** skill — it carries the same contract as this page in
...
`$LOKI_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `loki logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@wundercorp/loki-plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.loki/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.loki/logs/errors.log` for
