...
sidebar_position: 1
title: "Architecture"
description: "Loki Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Loki Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ loki_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # LokiCLI facade — mixins in loki_cli/cli_*_mixin.py
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── loki_state.py           # SQLite session/state database facade (+ loki_state_*.py siblings)
├── loki_constants.py       # LOKI_HOME, profile-aware paths
...
│
├── loki_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — `loki` subcommands (parsers in subcommands/, main_*.py)
...
│   ├── skills_config.py      # loki skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # loki tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # loki gateway start/stop
│
...
├── website/                  # Doku documentation site
...
```text
User input → LokiCLI.process_input()
...
Three discovery sources: `~/.loki/plugins/` (user), `.loki/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `loki plugins` or `config.yaml`.
...
Exposes Loki as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`loki -p <name>`) gets its own LOKI_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
