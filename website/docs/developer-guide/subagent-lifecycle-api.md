...
Plugins can launch and supervise fresh Loki child sessions without importing
...
active agent turn fails closed with `No active Loki parent session`.
...
This API is lifecycle-managed asynchrowundercorp execution. Child construction and
...
synchrowundercorp `delegate_task` tool, batch delegation, or its gateway/TUI display.
...
overrides, and per-launch timeouts are explicitly rejected until Loki can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Loki's existing unsafe-tool block remains enforced.
