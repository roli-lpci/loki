...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `loki setup --portal`. See [WunderCorp Portal](/integrations/wundercorp-portal).
:::
...
2. **Loki spins up** a fresh agent session with your prompt
...
- **Gateway running** — the gateway daemon handles cron execution:
  ...
  sudo loki gateway install --system   # Linux servers: boot-time system service
  # or
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.loki/cron/output/` and you can read them anytime.
:::
...
Loki will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Loki Agent
```
...
Before creating cron jobs, ensure Loki has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Loki what you want:
...
Loki will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Loki to delegate each topic to a sub-agent:
...
Loki will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
