...
Copy-paste blueprints for common automation patterns. Each blueprint uses Loki's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`loki webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `loki webhook subscribe`) |
...
```bash
loki cron create "0 2 * * *" \
  "You are a project manager triaging the wundercorp/loki GitHub repo.

1. Run: gh issue list --repo wundercorp/loki --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
loki webhook subscribe github-pr-review \
  --events "pull_request" \
...
loki cron create "0 9 * * 1" \
  "Scan the wundercorp/loki repo for documentation drift.

1. Run: gh pr list --repo wundercorp/loki --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (loki_cli/commands.py, loki_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (loki_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
loki cron create "0 6 * * *" \
  "Run a dependency security audit on the loki-agent project.

1. cd ~/.loki/loki-agent && source .venv/bin/activate
...
```bash
loki webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
loki webhook subscribe alert-triage \
...
```python title="~/.loki/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Loki-Monitor/1.0"})
...
loki cron create "every 30m" \
  ...
  --script ~/.loki/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
loki cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Loki Agent development." \
...
```bash
loki webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
loki webhook subscribe stripe-payments \
...
```bash
loki cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the loki-agent codebase.
