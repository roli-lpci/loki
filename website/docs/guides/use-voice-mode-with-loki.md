---
sidebar_position: 8
title: "Use Voice Mode with Loki"
description: "A practical guide to setting up and using Loki voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Loki
...
:::tip
[WunderCorp Portal](/integrations/wundercorp-portal) bundles both the LLM and TTS through one OAuth — voice mode works end-to-end with no extra credentials.
:::
...
- you want Loki sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Loki.
...
## Step 1: make sure normal Loki works first
...
Loki supports both local and cloud speech stacks.
...
### If you use `loki setup`

If you choose NeuTTS in the setup wizard, Loki checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Loki transcribes and responds
...
- asking Loki to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Loki loop.

## Tuning CLI behavior

### Silence threshold

If Loki starts/stops too aggressively, tune:
...
Loki stays a normal chat bot, but can speak replies.
...
- you want Loki to function like a portable research or ops assistant
...
Loki joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Loki detects speech boundaries
- transcripts are posted in the associated text channel
- Loki responds in text and audio
...
1. get text Loki working
2. run `loki setup tts` to enable voice support
