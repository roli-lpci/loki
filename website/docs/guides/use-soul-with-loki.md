---
sidebar_position: 7
title: "Use SOUL.md with Loki"
description: "How to use SOUL.md to shape Loki Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Loki

`SOUL.md` is the **primary identity** for your Loki instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Loki to feel like the same assistant every time you talk to it — or if you want to replace the Loki persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Loki should be
- what Loki should avoid stylistically
- how Loki should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Loki is and how Loki speaks
...
Loki now uses only the global SOUL file for the current instance:

```text
~/.loki/SOUL.md
```

If you run Loki with a custom home directory, it becomes:
...
Loki automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Loki does not overwrite it
- if the file exists but is empty, Loki adds nothing from it to the prompt

## How Loki uses it

When Loki starts a session, it reads `SOUL.md` from `LOKI_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Loki feels.
...
Loki already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Loki should sound.

# Avoid
What Loki should not do.

# Defaults
How Loki should behave when ambiguity appears.
```
...
Then restart Loki or start a new session.
...
### I edited SOUL.md but Loki still sounds the same

Check:
- you edited `~/.loki/SOUL.md` or `$LOKI_HOME/SOUL.md`
...
### Loki is ignoring parts of my SOUL.md
