...
```bash
loki teams-pipeline token-health
loki teams-pipeline token-health --force-refresh
```
...
```bash
loki teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
loki teams-pipeline maintain-subscriptions
loki teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Loki cron (recommended if you already run the Loki gateway)

Loki ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.loki/scripts/`. First create the script:

```bash
mkdir -p ~/.loki/scripts
cat > ~/.loki/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec loki teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.loki/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
loki cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/loki-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Loki Teams pipeline subscription maintenance
...
ExecStart=/usr/local/bin/loki teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/loki-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Loki Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now loki-teams-pipeline-maintain.timer
systemctl list-timers loki-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/loki teams-pipeline maintain-subscriptions >> /var/log/loki/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.loki/.env` at the top of a wrapper script that crontab calls.
...
```bash
loki teams-pipeline subscriptions   # should show expirationDateTime advanced
loki teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
loki teams-pipeline list
loki teams-pipeline list --status failed
loki teams-pipeline show <job-id>
```

### Replay a stored job

```bash
loki teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
loki teams-pipeline fetch --meeting-id <meeting-id>
loki teams-pipeline fetch --join-web-url "<join-url>"
loki teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `loki teams-pipeline maintain-subscriptions --dry-run`
- inspect `loki teams-pipeline list --status failed`
...
- run `loki teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `loki teams-pipeline run`
...
- [ ] `loki teams-pipeline validate` returns an OK snapshot
- [ ] `loki teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Loki cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
