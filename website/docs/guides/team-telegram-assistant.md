...
This tutorial walks you through setting up a Telegram bot powered by Loki Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Loki Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.loki/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Loki itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Loki Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_loki_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Loki Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u loki-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `loki gateway install` to update the plist.
:::
...
```bash
loki gateway stop && loki gateway start
```
...
loki pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.loki/.env`:
...
Control how much detail the bot shows when using tools. In `~/.loki/config.yaml`:
...
Customize how the bot communicates by editing `~/.loki/SOUL.md`:

For a full guide, see [Use SOUL.md with Loki](/guides/use-soul-with-loki).
...
```bash
# From the CLI
loki cron list          # View all scheduled jobs
loki cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u loki-gateway` (Linux) or `~/.loki/logs/gateway.log` (macOS) |
| Cron job output | `~/.loki/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.loki/cron/jobs.json` |
| Pairing data | `~/.loki/pairing/` |
| Session history | `~/.loki/sessions/` |
