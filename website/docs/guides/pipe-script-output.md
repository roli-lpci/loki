...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `loki send`."
---

# Pipe Script Output to Messaging Platforms

`loki send` is a small, scriptable CLI that pushes a message to any
messaging platform Loki is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | loki send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `loki
...
loki send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
loki send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
loki send --list

# Filter by platform
loki send --list telegram
```
...
Any platform Loki ships adapters for works as a target:
...
`loki send` resolves the message body in this order:

1. **Positional argument** — `loki send --to telegram "hi"`
2. **`--file PATH`** — `loki send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | loki send --to telegram`

When stdin is a TTY (no pipe), Loki does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  loki send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `loki send` reuses your Loki config, the same script works on
any host where Loki is installed — no need to export bot tokens into
...
`loki send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  loki send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | loki send \
...
```bash
./train.py --epochs 200 && \
  loki send --to telegram "training done" || \
  loki send --to telegram "training failed (exit $?)"
```
...
loki send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(loki send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `loki send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `loki send` calls
the platform's REST endpoint directly using credentials from
`~/.loki/.env` and `~/.loki/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `loki gateway start` and retry.
...
The listing is built from `~/.loki/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`loki gateway
...
| Approach | Multi-platform | Reuses Loki creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `loki send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`loki send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `loki send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `loki send` shares with cron delivery.
