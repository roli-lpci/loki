...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Loki Agent — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`loki claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Loki. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.

:::note
Coming from **Claude Code** or **OpenAI Codex CLI** instead? Use [`loki import-agent`](../user-guide/import-from-other-agents.md).
:::

:::tip
If your OpenClaw setup was multi-provider, `loki setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [WunderCorp Portal](/integrations/wundercorp-portal).
:::
...
loki claw migrate --preset full --migrate-secrets --yes
```
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.loki/` (by default a single restore-point archive is written before apply, under `~/.loki/backups/pre-migration-*.zip`; restorable with `loki import`). |
...
| What | OpenClaw source | Loki destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.loki/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.loki/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.loki/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.loki/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | Loki destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.loki/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.loki/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.loki/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.loki/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Loki skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Loki destination | Notes |
|------|---------------------|-------------------|-------|
| Default model | `agents.defaults.model` | `config.yaml` → `model` | Can be a string or `{primary, fallbacks}` object |
| Custom providers | `models.providers.*` | `config.yaml` → `custom_providers` (auto-migrated to the canonical `providers:` dict on the next `loki update` config migration) | Maps `baseUrl`, `apiType`/`api` — handles both short ("openai", "anthropic") and hyphenated ("openai-completions", "anthropic-messages", "google-generative-ai") values |
| Provider API keys | `models.providers.*.apiKey` | `~/.loki/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | Loki config path | Mapping |
|------|---------------------|-------------------|---------|
...
Idle and daily reset timers are not imported: Loki conversations persist until an explicit `/new` or `/reset`. Advanced session settings (identity links, thread bindings, maintenance, scope and send policy) remain archived for reference.

### MCP servers

| OpenClaw field | Loki field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.loki/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | Loki `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Loki path | Notes |
|------|-------------|-------------|-------|
...
| Gateway auth token | `gateway.auth.token` | `.env` → `LOKI_GATEWAY_TOKEN` | Requires `--migrate-secrets` |
...
### Archived (no direct Loki equivalent)

These are saved to `~/.loki/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Loki |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Loki has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `loki cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `loki webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `loki honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `loki skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use Loki profiles |
...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env` and the `openclaw.json` env sub-object. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — the migration warns about these, and those values must be added to Loki manually via `loki config set`.

## After migration

1. **Check the migration report** — printed on completion with counts of migrated, skipped, and conflicting items.

2. **Review archived files** — anything in `~/.loki/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Start a new session** — imported skills and memory entries take effect in new sessions, not the current one.

4. **Verify API keys** — run `loki status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart loki-gateway`

6. **Check session archives** — review archived advanced settings; idle and daily reset timers are intentionally not imported.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `loki whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `loki claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `loki config set`.

### Skills not appearing after migration

Imported skills land in `~/.loki/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `loki config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
