...
description: "How the Loki Desktop app signs in to a gated gateway using your system browser and PKCE — no embedded webview, no session cookies"
---

# Desktop Native Sign-In (RFC 8252)

When the Loki Desktop app connects to a **gated gateway** (a hosted or
...
For Loki specifically, native sign-in means:
...
```
Desktop app                Gateway (/auth/native/*)          WunderCorp Portal (IDP)
...
desktop app* and an OAuth client *to the upstream identity provider* (WunderCorp
...
dashboard-auth subsystem. OAuth providers (e.g. the bundled **WunderCorp** provider)
...
- [Run Loki with WunderCorp Portal](./run-loki-with-wundercorp-portal.md)
