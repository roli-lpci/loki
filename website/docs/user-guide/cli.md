...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Loki Agent terminal interface — commands, keybindings, personalities, and more"
---
...
:::tip
Loki also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `loki --tui` — see the [TUI](tui.md) guide.
:::
...
loki chat --query-file prompt.txt
loki chat --query-file - < prompt.txt
...
loki -s loki-agent-dev,github-auth
loki chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
loki --continue             # Resume the most recent CLI session (-c)
loki --resume <session_id>  # Resume a specific session by ID (-r)
loki --resume latest        # Resume the most recent session (same as -c)
loki --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
loki chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
loki -w                         # Interactive mode in worktree
loki -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`loki -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
loki worktree list              # audit: age, size, verdict, reason per tree
loki worktree prune             # remove safe trees + delete merged branches
loki worktree prune --dry-run   # show the plan without changing anything
loki worktree prune --trees-only     # leave local branches alone
loki worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running loki session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.loki/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `loki -w` for days — no longer accumulate merged scratch trees
...
The `loki plugins` commands manage native Loki plugins and portable Agent
...
loki plugins disable <plugin-name>
loki plugins update <plugin-name>
loki plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Loki currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Loki CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Loki CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `LOKI_YOLO_MODE` is on (either `loki --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
On the `openai-codex` provider, `/usage` also shows any banked usage-limit resets on your ChatGPT account ("You have N resets banked - use /usage reset to activate"). `/usage reset` redeems one banked reset, fully restoring your 5-hour and weekly limits. Loki refuses to redeem while your limits aren't exhausted (a banked reset restores the full allowance, so spending it early wastes it) — pass `/usage reset --force` to redeem anyway.

### Session Resume Display

When resuming a previous session (`loki -c` or `loki --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Loki to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.loki/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Loki supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Loki replies |
...
```bash
loki -s loki-agent-dev,github-auth
loki chat -s github-pr-workflow -s github-auth
```

Loki loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.loki/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.loki/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Loki..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Loki also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Loki — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Loki is working, Loki prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Loki to the background — just like any terminal process. The shell prints a confirmation:

```
Loki Agent has been suspended. Run `fg` to bring Loki Agent back.
```
...
Resume this session with:
  loki --resume 20260225_143052_a1b2c3
...
```bash
loki --continue                          # Resume the most recent CLI session
loki -c                                  # Short form
loki -c "my project"                     # Resume a named session (latest in lineage)
loki --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
loki --resume "refactoring auth"         # Resume by title
loki --resume latest                     # Resume the most recent session (same as -c)
loki --resume latest --in ./my-project   # Latest session for ./my-project's workspace
loki -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `loki sessions rename <id> <title>` from the command line. Use `loki sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Loki's SQLite state database under `~/.loki/state.db`. The database keeps:
...
Loki immediately confirms the task and gives you back the prompt:
