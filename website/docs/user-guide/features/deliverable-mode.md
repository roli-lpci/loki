...
When Loki Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.loki/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.loki/config.yaml` (switchable per session
via `/personality`).
...
If you use Loki' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.loki/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Loki Agent's deliverable
