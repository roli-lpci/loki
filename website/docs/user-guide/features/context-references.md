...
Type `@` followed by a reference to inject content directly into your message. Loki expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Loki env: `$LOKI_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$LOKI_HOME/skills/.hub/`
