...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Loki's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Loki instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
loki memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `loki` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Loki syncs them back at session init.
...
The `loki honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `loki memory setup honcho` (or run `loki memory setup` and pick it from the list); the `loki honcho` subcommand then appears on the next invocation.

```bash
loki memory setup honcho    # Configure Honcho directly (works before activation)
loki honcho status          # Connection status, config, and key settings
loki honcho setup           # Redirects to `loki memory setup` (post-activation alias)
loki honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
loki honcho peer            # Show or update peer names + dialectic reasoning level
loki honcho mode            # Show or set recall mode (hybrid/context/tools)
loki honcho tokens          # Show or set token budget for context and dialectic
loki honcho identity        # Seed or show the AI peer's Honcho identity
loki honcho sync            # Sync Honcho config to all existing profiles
loki honcho peers           # Show peer identities across all profiles
loki honcho sessions        # List known Honcho session mappings
loki honcho map             # Map current directory to a Honcho session name
loki honcho enable          # Enable Honcho for the active profile
loki honcho disable         # Disable Honcho for the active profile
loki honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `loki honcho`

If you previously used the standalone `loki honcho setup`:
...
No re-login or re-setup needed. Run `loki memory setup` and select "honcho" — the wizard detects your existing config.
