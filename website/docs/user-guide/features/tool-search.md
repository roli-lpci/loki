...
**Tool Search** is Loki' opt-in progressive-disclosure layer for that
...
:::info Built-in Loki tools never defer
The tools that make up Loki' core capability set (`terminal`,
`read_file`, `write_file`, `patch`, `search_files`, `todo`, `memory`,
`browser_*`, `web_search`, `web_extract`, `clarify`, `execute_code`,
`delegate_task`, `session_search`, and the rest of
`_LOKI_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, Loki **unwraps the bridge** and
...
When you are signed in to the WunderCorp Portal, the bridge additionally reaches
...
- **No provider-native validation for deferred schemas.** `tool_describe`
  ...
  only the generic `tool_call.arguments` object. Loki therefore coerces and
  validates the underlying arguments locally before dispatch; the concrete
  tool or MCP server remains responsible for schemas Loki cannot safely
...
- **No JS sandbox.** Loki uses the simpler "structured tools" mode
