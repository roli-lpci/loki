...
Loki Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Loki integration. It
...
**Fresh installs already have the driver.** The Loki installer
...
- **`loki tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
loki computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `loki computer-use
status` to verify the install.

Already have cua-driver? Loki reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `loki update`, and the first
`computer_use` call of a session, Loki checks the local version and
...
selected with `LOKI_CUA_DRIVER_CMD` stays
under your control, so Loki reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Loki autodetection is a
planned cua-driver follow-up, so currently point Loki at that directory or
...
interface. The built-in toolset provides Loki actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `loki computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Loki host identity. |
...
```
loki -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.loki/config.yaml`.

## Permission modes and logged-in browser profiles

Loki maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| Loki session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Loki approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Loki risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.loki/cua-manifest.yaml
```
...
for the format). Loki launches a private runtime with
...
identity instead of resetting with every Loki build), and Loki verifies
...
resetting or closing the Loki session, cancellation cleanup, or process exit
closes that transport session. Loki also stops private runtimes that it
launched for bounded or unrestricted access. One Loki
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the Loki host identity.
...
## `loki computer-use doctor` — your first triage stop

`loki computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ loki computer-use doctor
...
Loki run declares a public cua-driver **session name** (something like
`loki-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
without it. Loki disables it automatically where it is a known failure
...
Loki keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Loki-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Loki
autodetection is a planned cua-driver follow-up, so currently point Loki at
...
These are **platform deep dives, not duplicates of the Loki skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Loki
...
image capture also saves a bounded, shareable copy under Loki' image cache
...
Loki applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.loki/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Loki applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Loki agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Loki agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Loki warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Loki from inside the RDP / console
...
On Linux, native Wayland support remains an explicit opt-in. Loki passes the
...
```
LOKI_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
LOKI_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Loki disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Loki sets
...
When it's on, `loki computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Loki at a binary you built from source instead
of the published release. Loki resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`LOKI_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Loki testing (see notes).
...
### Option B — point Loki straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`LOKI_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
LOKI_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
LOKI_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Loki is using your build

- `loki computer-use status` prints the resolved binary path and
  version.
- `loki computer-use doctor` confirms the binary is reachable and
...
- **Loki spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Loki YOLO, Loki instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Loki
  change needed either way.
- **Local builds skip the version check.** Loki warns when the
...
**First action when anything's off: run `loki computer-use doctor`.**
...
Run `loki computer-use install` to fetch the cua-driver binary, or
run `loki tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `loki computer-use doctor` will
...
- **Loki-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Loki `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Loki autodetection is a
  planned follow-up; currently point Loki at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Loki among them)
