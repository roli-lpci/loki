...
fills the password through Loki. The tool result it sees is
...
- **Authenticator key saved with the login** (the "setup key" or `otpauth://`
  ...
  hold a TOTP seed count too): Loki generates the current code and enters
  it. Nobody is asked. Add the key in **Settings → Passwords & Logins → Add**
  or `loki vault add`; the item shows a *2FA auto* badge.
- **Code sent to your phone or email**: a small prompt appears in your
  surface ("Verification code for github.com"), you type the code, Loki
...
in, Loki picks it up automatically and its website logins become fillable
...
per session, 30 minutes idle). Loki hands the master password to the manager's
...
Prefer not to use a detected manager? `loki vault sources --disable bitwarden`,
...
Passwords & Logins → Add**, or `loki vault add`), bound to the checkout site,
...
- **CLI**: `loki vault list`, `loki vault add`, `loki vault rm <handle>`,
  `loki vault sources`.

Items live encrypted under `~/.loki/vault/` (Fernet key + vault file, both
...
Cron jobs, webhooks, the API server and `loki chat -q` have nobody to answer a
...
**Does:** the password never enters the model's context through Loki: not in
