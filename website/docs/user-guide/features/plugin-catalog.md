...
description: "Browse and install reviewed, SHA-pinned Loki plugins from the curated catalog"
---

# Plugin Catalog

The plugin catalog is a curated, human-reviewed directory of Loki plugins you
can install by name with a single command:

```bash
loki plugins install <name>
```
...
[`plugin-catalog/`](https://github.com/wundercorp/loki/tree/main/plugin-catalog)
directory of the loki-agent repository, declaring:

| Field | Meaning |
|---|---|
| `name` | The catalog key you pass to `loki plugins install` |
...
| `tier` | `official` (maintained by WunderCorp) or `community` |
| `maintainer` | Who owns the plugin |
| `capabilities` | Declared tools, hooks, middleware, and required env vars |
| `requires_loki` | Minimum Loki version, e.g. `>=0.19` (optional) |
...
`loki plugins update <name>` never runs `git pull` for catalog installs —
...
enabled/disabled state is preserved. `loki plugins list` shows catalog
...
(`https://loki.computer/api/plugin-catalog.json`).
`search`/`install`/`update` fetch it at most every six hours and cache it under
`~/.loki/cache/`, so new entries and removals reach installed clients without
updating Loki. Offline, the copy shipped with your checkout is used. Removals
from the in-tree list and the live list are always both enforced.

### Custom git URLs are different

`loki plugins install <git-url>` still works for any repository, but it
...
[plugin-catalog README](https://github.com/wundercorp/loki/tree/main/plugin-catalog);
...
- [Plugins](plugins.md) — the plugin system itself: manifest format, enabling,
  configuration
- [Built-in Plugins](built-in-plugins.md) — plugins that ship with Loki
- [Build a Loki Plugin](/developer-guide/plugins) — write your own
