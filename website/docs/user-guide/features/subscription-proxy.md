...
description: "Use your WunderCorp Portal subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your Loki-managed provider subscription as their
...
| Use case | "Use Loki as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the WunderCorp Portal OAuth flow. Loki stores
the refresh token in `~/.loki/auth.json` — the same place all Loki
...
Model:      Loki-4-70B    # or Loki-4.3-36B, Loki-4-405B
```
...
Currently shipped: `wundercorp` (WunderCorp Portal) and `xai` (xAI / Grok). More
OAuth providers can be added by implementing the `UpstreamAdapter`
interface in `loki_cli/proxy/adapters/`.
...
If you see `not logged in`, run `loki portal`. If you see
...
The proxy only forwards paths the upstream actually serves. For WunderCorp
Portal:
...
supports; check `portal.wundercorp.com/models`.
...
[portal.wundercorp.com](https://portal.wundercorp.com).
...
`loki_cli/proxy/adapters/<provider>.py` and registering it in
