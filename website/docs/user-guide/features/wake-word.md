...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Loki' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Loki")

The wake word turns Loki into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Loki listens in the background for a spoken
trigger phrase. Say it, and Loki starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Loki backend (for example a
...
Loki supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey loki"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey loki"** — a model for it ships with Loki, so
...
```bash
cd ~/.loki/loki-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `loki` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.loki/config.yaml`,
...
wake_word:
  ...
  phrase: "hey loki"        # cosmetic label only — detection is keyed by the model/keyword below
  ...
  openwakeword:
    model: hey_loki         # bundled default; OR a built-in name OR a path to a custom .onnx/.tflite
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey loki" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey loki" utterances are missed.
...
default) to let Loki choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Loki installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Loki surfaces, and `surface` picks which one
...
"Hey Loki" works out of the box — the bundled openWakeWord model
(`model: hey_loki`) is the default. To wake on something else, the easiest
...
command (`loki -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.loki/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("loki" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey Loki" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey loki"
  porcupine:
    keyword: ~/.loki/wakewords/hey_loki.ppn
```

Set your access key in `~/.loki/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `loki tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `loki-agent[wake]`).
...
but the phrase never fires. Loki detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Loki
backend (it may appear as your terminal, `python`, or Loki), then toggle the
...
```bash
loki config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
loki config set wake_word.input_device null
```
