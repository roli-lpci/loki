...
Top-level model calls run in the background automatically. Loki returns a handle immediately so the conversation can continue, then posts the result back as a new message. An orchestrator subagent waits for its own workers so it can synthesize their results before returning.
...
One exception: when the parent has a resolved workspace directory, every subagent's system prompt embeds that workspace's **project context files** (`.loki.md` > AGENTS.md chain > CLAUDE.md > `.cursorrules` — the same discovery, priority, and size caps as the main agent's system prompt; SOUL.md is excluded). Subagents working in a repo operate under the repo's own conventions without having to rediscover them.
...
When a top-level agent provides a `tasks` array, Loki returns one background handle and runs the subagents in parallel. By default the call returns **one** consolidated message once every task has finished. Results are delivered only between the parent's turns: the parent should finish anything that does not depend on the children, then end its turn rather than polling transcripts, artifacts, or CI while it waits.
...
- **Cancellation:** Follow-up messages do not cancel a top-level background batch. `/stop` or closing/resetting the owning session cancels its active children. Synchrowundercorp orchestrator children still follow their parent's interrupt state

Synchrowundercorp single-task delegation from an orchestrator runs directly without thread pool overhead.

### Durable background completions

When a background delegation finishes, Loki stores its completion event in
the active profile's `state.db` before publishing it to the normal fresh-turn
queue. If Loki restarts after completion but before delivery, the pending
...
Loki cannot prove whether its external side effects happened. Pending and
...
auxiliary:
  review:
    provider: openrouter               # or wundercorp, anthropic, a direct base_url, ...
...
Control actions run synchrowundercorply in-turn (never backgrounded), are scoped to the caller's own spawn tree — a conversation can never see or control another session's children — and never consume the per-turn subagent spawn cap, so `stop` keeps working even after the cap is hit.
...
```
<loki_home>/cache/delegation/live/<delegation_id>/task-<n>.log
```
...
```bash
tail -f ~/.loki/cache/delegation/live/deleg_ab12cd34/task-0.log
```
...
Top-level model-facing `delegate_task` calls run in the background automatically where the session supports later delivery. Loki returns a handle immediately, and the result re-enters the conversation after the child or batch finishes. Orchestrator subagents wait for their workers in the current turn because they must synthesize those results before returning. Stateless request/response endpoints fall back to synchrowundercorp execution when they cannot deliver a detached result later.
...
- A Loki process restart does **not** resume a running child. Its attempt becomes `unknown` because Loki cannot prove which side effects happened.
...
- **Cancellation follows ownership** — `/stop` or closing/resetting the owning session cancels its background children; synchrowundercorp descendants under orchestrators follow their parent's interrupt state
...
- Each child starts its terminal in `<repo>/.worktrees/subagent-<id>` on its
  own branch `loki-subagent/subagent-<id>`, and its goal message tells it to
