...
Loki runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Loki
...
1. Loki captures a baseline of current diagnostics for the file.
...
…). Loki auto-detects the binary on PATH or in
`<LOKI_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Loki at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<LOKI_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`loki lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Loki installs both packages together when you
run `loki lsp install typescript` or auto-install fires on first
use.

## CLI

```
loki lsp status          # service state + per-server install status
loki lsp list            # registry, optionally --installed-only
loki lsp install <id>    # eagerly install one server
loki lsp install-all     # try every server with a known recipe
loki lsp restart         # tear down running clients
loki lsp which <id>      # print resolved binary path
```

`loki lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <LOKI_HOME>/lsp/bin
...
When `install_strategy: auto`, Loki installs binaries into
`<LOKI_HOME>/lsp/bin/`. NPM packages land in
`<LOKI_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Loki-owned and is
...
**single process** per Loki process: the first Python project spawns
...
**`loki lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<LOKI_HOME>/lsp/bin/`. Run
`loki lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `loki lsp status`**
...
When `loki lsp status` shows a `Backend warnings` section, install
...
Check `~/.loki/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `loki lsp restart` to clear the set;
