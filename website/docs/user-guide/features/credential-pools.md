...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Loki automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
:::tip
Credential pools are mainly for API-key providers (OpenRouter, Anthropic). A single [WunderCorp Portal](/integrations/wundercorp-portal) OAuth covers 300+ models, so most users don't need a pool when on Portal.
:::
...
If you already have an API key set in `.env`, Loki auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
loki auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
loki auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
loki auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  loki_pkce          oauth   id=ab12cd34 priority=0 loki_pkce ←
...
`loki auth remove <provider> <target>` when a label is ambiguous, and `priority=` is
the order the pool tries credentials in under the `fill_first` strategy.

## Interactive Management

Run `loki auth` with no subcommand for an interactive wizard:
...
For providers that support both API keys and OAuth (Anthropic, WunderCorp, Codex), the add flow asks which type:
...
| Command | Description |
|---------|-------------|
| `loki auth` | Interactive pool management wizard |
| `loki auth list` | Show all pools and credentials |
| `loki auth list <provider>` | Show a specific provider's pool |
| `loki auth add <provider>` | Add a credential (prompts for type and key) |
| `loki auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `loki auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `loki auth add <provider> --priority 0` | Add a credential and place it first in the `fill_first` order |
| `loki auth priority <provider> <target> <n>` | Move a credential to priority `n` (0 = tried first); the rest are renumbered |
| `loki auth remove <provider> <index>` | Remove credential by 1-based index |
| `loki auth reset <provider>` | Clear all cooldowns/exhaustion status |
| `loki auth reset <provider> <target>` | Clear the cooldown on one credential by index, id, or label |
| `loki auth refresh <provider> [target]` | Refresh one OAuth credential's tokens and return it to rotation (proves the grant is alive; the next request re-checks quota) |

For WunderCorp, `auth refresh` supports only the login's `device_code` singleton.
Independent WunderCorp pool accounts are rejected before refresh; their tokens and
cooldowns are preserved. Reauthenticate with `loki auth add wundercorp --type oauth`
...
Configure via `loki auth` → "Set rotation strategy" or in `config.yaml`:
...
| Strategy | Behavior |
|----------|----------|
| `fill_first` (default) | Use the first healthy key until it's exhausted, then move to the next; order is each credential's `priority` (`loki auth priority` changes it) |
...
When you set up a custom endpoint via `loki model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via loki model:
loki auth list
# Shows:
...
loki auth add Together.ai --api-key sk-together-second-key
```
...
Loki automatically discovers credentials from multiple sources and seeds the pool on startup:

| Source | Example | Auto-seeded? |
|--------|---------|-------------|
| Environment variables | `OPENROUTER_API_KEY`, `ANTHROPIC_API_KEY` | Yes |
| OAuth tokens (auth.json) | Codex device code, WunderCorp device code | Yes |
| Claude Code credentials | `~/.claude/.credentials.json` | Yes (Anthropic) |
| Loki PKCE OAuth | `~/.loki/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `loki auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `loki auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Loki can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Loki-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
2. **`loki_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`loki_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`agent/turn_api_error.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.loki/auth.json` under the `credential_pool` key:
...
The OpenRouter entry above was borrowed from an external source, so the raw key is not stored in `auth.json`. The manual Anthropic entry was intentionally added to Loki' credential store, so its token remains persistable.
