...
Schedule tasks to run automatically with natural language or cron expressions. Loki exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Loki itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
**Which model does a cron job run on?** Resolution at fire time is: per-job pin → `cron.model` in `config.yaml` → the global default from `loki model`.

- **Per-job pin** — set by *you* via the dashboard, `loki cron create/edit --model … --provider …`, or by editing `~/.loki/cron/jobs.json`. Once set, it sticks until you change it. The agent's `cronjob` tool cannot set or change per-job models — inference pins are user-owned.
- **`cron.model` / `cron.model_provider`** — a cron-fleet default: every unpinned job runs on this model, independent of your chat model. Set it once (`loki config set cron.model <name>`) and switching your chat model with `loki model` or `/model` never touches your cron fleet.
- **Global default** — only when neither of the above is set does a job follow `loki model`. Loki **snapshots** the provider and model at creation, and that snapshot is the job's effective pin: if you later switch the global default (`loki model`, `/model`, `loki config set model.default …`), the job **keeps running on the model and provider it was created under** and logs one INFO line per run noting the difference. A global model change never stops a scheduled job, and an unattended job never silently inherits a switch to a paid provider/model (#44585). To move a job to the new default, pin it (`loki cron edit <job_id> --provider <provider> --model <model>`) or set `cron.model` to move the whole fleet at once. Jobs created before snapshots existed keep following the live global default.
...
`loki setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [WunderCorp Portal](/integrations/wundercorp-portal).
:::

:::tip
**Per-job reasoning effort.** A job can pin its own thinking level, independent of the model pin: one of `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`. When set, it overrides both the global `agent.reasoning_effort` and per-model `agent.reasoning_overrides` for that job's runs (`none` disables thinking). Set it via `loki cron create/edit --reasoning-effort high`; pass an empty string on edit to clear the pin and follow config again. (It is deliberately not exposed on the agent's `cronjob` tool — model configuration stays a user decision.) Levels a model doesn't support are clamped or omitted by the provider at request time — pinning `xhigh` on a model that caps at `high` runs at `high`. The pin has no effect on `no_agent` jobs (there is no LLM call to tune). Use it to run heavy scheduled analyses at `high` while cheap recurring jobs run at `minimal`, without touching your global default.
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. Loki disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
loki cron create "every 2h" "Check server status"
loki cron create "every 1h" "Summarize new feed items" --skill blogwatcher
loki cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Loki will use the unified `cronjob` tool internally.
...
Or: `loki config set cron.preflight false`
...
```bash
loki cron edit <job_id> --provider <provider> --model <model>   # one job
loki config set cron.model <model>                               # every unpinned job
```

`loki config set model.default …` and the Desktop model picker list the unpinned jobs that will
...
loki cron create "every 1d at 09:00" \
...
```bash
loki cron edit <job_id> --schedule "every 4h"
loki cron edit <job_id> --prompt "Use the revised task"
loki cron edit <job_id> --skill blogwatcher --skill maps
loki cron edit <job_id> --add-skill maps
loki cron edit <job_id> --remove-skill blogwatcher
loki cron edit <job_id> --clear-skills
```
...
```bash
loki cron list
loki cron pause <job_id_or_name>
loki cron resume <job_id_or_name>
loki cron run <job_id_or_name>
loki cron remove <job_id_or_name>
loki cron edit <job_id_or_name> [...flags]
loki cron status
loki cron tick
```
...
```bash
loki cron create "every 1h" "Post the digest" --paused --paused-reason "Awaiting review"
loki cron resume <job_id>
```
...
```bash
loki gateway install     # Install as a user service
sudo loki gateway install --system   # Linux: boot-time system service for servers
loki gateway             # Or run in foreground
...
1. loads jobs from `~/.loki/cron/jobs.json`
...
A file lock at `~/.loki/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.

### Execution history

Loki records each claimed cron attempt in the profile-local
`~/.loki/cron/executions.db` before executor or provider dispatch. Attempts
move through `claimed`, `running`, and one immutable terminal state:
`completed`, `failed`, or `unknown`. After restart, Loki marks an abandoned
...
Inspect recent attempts with `loki cron runs [job-id] --limit 20` (alias:
...
that the retained ledger records as completed, Loki skips that replay and
...
in a row and suggesting you fix, pause (`loki cron pause <job>`), or remove
it. Any successful run resets the streak, and `loki cron list` shows the
...
```bash
loki cron incidents                 # list incidents (newest activity first)
loki cron incidents --state alerted # filter: detected | alerted | closed
loki cron incidents ack <id>        # acknowledge — stop re-pinging
```
...
### Fleet health check: `loki cron doctor`

`loki cron doctor` is a read-only health check over every active job. It
...
- script missing, not a file, or resolving outside `LOKI_HOME/scripts`,
...
`loki cron incidents` (durable failure records) and `loki cron runs`
...
| `"local"` | Save to local files only (`~/.loki/cron/output/`) | Default on CLI |
...
`last_delivery_error`. `loki cron list` shows it in yellow as
`delivery_failed: <reason>`, `loki cron doctor` reports it as a delivery
...
- `bot-chat:<profile>` targets another profile **on the same machine**. Names are validated against `loki profile list` when the job is created; profiles on other gateways or machines can never be targeted, so same-named profiles across machines are unambiguous.
...
and in `loki cron doctor` as `last delivery unverified (...)`. The marker is
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.loki/cron/output/`), but no message is sent to the delivery target.
...
Or set the `LOKI_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 3600s default.
...
Or set the `LOKI_CRON_MEDIA_SEND_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 300s default. A timed-out attachment is recorded in the job's run status as a partial delivery failure (the text still delivers).
...
`.sh` / `.bash` files run under `bash` from `PATH` when available, otherwise `/bin/bash` (important on Windows Git Bash). Anything else runs under the current Python interpreter (`sys.executable`). Scripts must resolve inside `$LOKI_HOME/scripts/` — relative names, absolute paths, and `~`-prefixed paths are accepted when the resolved target stays in that directory; paths that escape it are rejected. Subprocess env is sanitized (`_sanitize_subprocess_env`): provider API credentials and other Loki-managed secrets are **not** inherited by cron scripts.

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to Loki directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Loki will write the check script to `~/.loki/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.loki/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.loki/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.loki/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.loki/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Loki reads Job 1's most recent output from `~/.loki/cron/output/{job1_id}/*.md`
...
From the CLI: `loki cron create "every 6h" "Scan for news" --continuity`, and `loki cron edit <job_id> --continuity` / `--no-continuity` to toggle it on an existing job. The same toggle appears in the dashboard's cron editor and the desktop Bot Mode routine dialog.
...
Loki home. Its `## Error` section includes the chained traceback, with credential patterns
...
- `loki cron list` — a red `⚠ Missed scheduled fire:` line under the job
- The dashboard job view

The stamp always reflects **current** auto-fire health: it is overwritten by newer misses and cleared automatically by the next successful run. If you see it, the job and its schedule are fine — the gateway side of the fire path needs attention (most commonly, restart the gateway through its supervisor so it loads the full profile environment: `loki gateway restart`).
...
### Manual runs are asynchrowundercorp
...
Runtimes that can't receive detached results (one-shot `loki -z`, `loki
cron run` from the CLI, cron child sessions, Kanban workers) fall back to
synchrowundercorp execution automatically.

## Toolsets available to cron jobs

Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `loki tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `loki tools` cron-platform config wins; otherwise Loki falls back to the built-in defaults. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Loki should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.loki/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.loki/scripts/.feed-changed.last"
...
:::tip
Loki's own `~/.loki/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/wundercorp/loki/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.loki/cron/jobs.json`. Output from job runs is saved to `~/.loki/cron/output/{job_id}/{timestamp}.md`.

Job definitions are plain JSON on disk: they survive `loki update`, gateway restarts, and machine reboots. A job that was mid-run during a restart is marked `unknown` in the execution ledger — it is not automatically retried, but the job's next scheduled tick fires normally. See [Execution history](#execution-history) for details.

:::tip
Ask the agent to manage jobs through the `cronjob` tool, `loki cron edit`, or `/cron` — not by patching `jobs.json` directly. Direct edits can fail silently when [file write safety](../security.md#file-write-safety) blocks the path (for example when `LOKI_WRITE_SAFE_ROOT` is set), and the [file-mutation verifier](../configuration.md#file-mutation-verifier) footer is the authoritative signal that nothing was saved.
:::

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Loki resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
