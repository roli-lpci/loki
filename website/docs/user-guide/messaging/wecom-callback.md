...
Connect Loki to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Loki supports two WeCom integration modes:
...
> Run `loki gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Loki decrypts the message, queues it for the agent
...
(Use `loki gateway start` only after `loki gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Loki and the token the
...
in `~/.loki/.env` values around `=` will also break signature checks. After
fixing, restart `loki gateway run`.
...
Check `loki gateway run` logs for the bound host/port. If the adapter bound to
