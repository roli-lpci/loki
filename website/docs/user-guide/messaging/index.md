...
description: "Chat with Loki from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Loki from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Loki](/guides/use-voice-mode-with-loki).

:::tip
Bots need both a model provider and tool providers (TTS, web). A [WunderCorp Portal](/integrations/wundercorp-portal) subscription bundles all of them.
:::
...
saved by `loki gateway setup` can enable a credential-based platform without a
...
For group chats, hooks, and automation flows, Loki supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Loki keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Loki does not hide failures just because the text resembles a silence token.
...
```bash
loki gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
loki gateway              # Run in foreground
loki gateway setup        # Configure messaging platforms interactively
loki gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo loki gateway install --system   # Linux only: install a boot-time system service
loki gateway start        # Start the default service
loki gateway stop         # Stop the default service
loki gateway status       # Check default service status
loki gateway status --system         # Linux only: inspect the system service explicitly
```
...
```bash
loki gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Loki sends heartbeats
...
| `/update` | Update Loki Agent to the latest version |
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.loki/config.yaml`:
...
loki pairing approve telegram XKGH5N7P

# Other pairing commands:
loki pairing list          # View pending + approved users
loki pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Loki appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.loki/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.loki/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `LOKI_HOME`:

- `~/.loki/status_phrases.yaml` or any `*.yaml` in `~/.loki/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to LOKI_HOME
...
Off by default. When enabled, Loki prepends a human-readable timestamp
...
Each `/bg` prompt spawns a **separate agent instance** that runs asynchrowundercorply:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.loki/config.yaml`:
...
```bash
LOKI_BACKGROUND_NOTIFICATIONS=result
```
...
loki gateway start                 # Start the service
loki gateway stop                  # Stop the service
loki gateway status                # Check status
journalctl --user -u loki-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo loki gateway install --system
sudo loki gateway start --system
sudo loki gateway status --system
journalctl -u loki-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Loki installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit loki-gateway` (or `sudo systemctl edit loki-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `loki update`. When `loki update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart loki-gateway` command (it never blocks on an interactive password prompt).
...
```bash
loki gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `loki update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo loki update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/loki-gateway`:

```
loki ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed loki-gateway*, /usr/bin/systemctl --no-ask-password start loki-gateway*, /usr/bin/systemctl --no-ask-password restart loki-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Loki will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Loki installations on the same machine (with different `LOKI_HOME` directories), each gets its own systemd service name. The default `~/.loki` uses `loki-gateway`; other installations use `loki-gateway-<hash>`. The `loki gateway` commands automatically target the correct service for your current `LOKI_HOME`.
:::

### macOS (launchd)

```bash
loki gateway install               # Install as launchd agent
loki gateway start                 # Start the service
loki gateway stop                  # Stop the service
loki gateway status                # Check status
tail -f ~/.loki/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.loki.gateway.plist`. It includes three environment variables:
...
- **LOKI_HOME** — scopes the gateway to your Loki installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `loki gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `LOKI_HOME` directory gets its own launchd label. The default `~/.loki` uses `ai.loki.gateway`; other installations use `ai.loki.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `loki-cli` | Full access |
| Telegram | `loki-telegram` | Full tools including terminal |
| Discord | `loki-discord` | Full tools including terminal |
| WhatsApp | `loki-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `loki-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `loki-slack` | Full tools including terminal |
| Google Chat | `loki-google_chat` | Full tools including terminal |
| Signal | `loki-signal` | Full tools including terminal |
| SMS | `loki-sms` | Full tools including terminal |
| Email | `loki-email` | Full tools including terminal |
| Home Assistant | `loki-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `loki-mattermost` | Full tools including terminal |
| Matrix | `loki-matrix` | Full tools including terminal |
| DingTalk | `loki-dingtalk` | Full tools including terminal |
| Feishu/Lark | `loki-feishu` | Full tools including terminal |
| WeCom | `loki-wecom` | Full tools including terminal |
| WeCom Callback | `loki-wecom-callback` | Full tools including terminal |
| Weixin | `loki-weixin` | Full tools including terminal |
| BlueBubbles | `loki-bluebubbles` | Full tools including terminal |
| QQBot | `loki-qqbot` | Full tools including terminal |
| Yuanbao | `loki-yuanbao` | Full tools including terminal |
| Microsoft Teams | `loki-teams` | Full tools including terminal |
| API Server | `loki-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `loki-webhook` | Full tools including terminal |
| Raft | `loki-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
`platforms.<name>.enabled: false` in `~/.loki/config.yaml` is authoritative.
...
1. **Gateway log** (`~/.loki/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
