...
sidebar_position: 30
title: "Loki Relay"
description: "Connect Loki to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Loki Relay (Connector)
...
Loki Relay is not a chat platform itself — it is a **connector system** that
...
secret. `loki gateway enroll` redeems a **single-use enrollment token**
...
1. Resolves a fresh WunderCorp Portal access token from your existing login
   (`~/.loki/auth.json`) — this proves which WunderCorp org (tenant) you own. If
   `gateway.idp.token_url` is configured, your own IdP is used instead (the
   air-gapped / self-hosted-IdP path, no WunderCorp Portal involved): with
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.loki/.env`:
...
`loki gateway enroll` refuses to run in managed/hosted installs — there the
...
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.loki/.env`) | Connector relay WebSocket URL. Presence enables the relay platform. |
...
| `gateway.idp.token_url` | `config.yaml` | When set, enrollment/provisioning authenticates against your own IdP instead of WunderCorp Portal: OAuth2 client-credentials when `gateway.idp.client_id`/`client_secret` are also set; otherwise an ambient token endpoint (plain GET returning the token, raw or `{"access_token": …}`). |
...
token. Re-login with `loki auth add wundercorp` (or `loki setup`) and retry.
...
startup. Restart it (`loki gateway restart`).
