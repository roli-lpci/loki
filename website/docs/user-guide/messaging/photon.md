...
Connect Loki to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Loki — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Loki runs it in a
...
loki photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Loki Agent` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.loki/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.loki/.env`
...
(device token, dashboard project id) lives in `~/.loki/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Loki
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Loki replies with a pairing code. Approve it with:

```bash
loki pairing approve photon <CODE>
```

Use `loki pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.loki/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.loki/.env`):
...
By default Loki responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Loki` and
`@Loki agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Loki will reply.
...
assigned iMessage line Loki uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run loki photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `loki photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `loki photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `loki photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Loki sends images, voice
  ...
  media.
- **Native polls are supported.** Loki sends poll content through
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage
  read after forwarding it to Loki, so the sender sees `Read` without
  waiting for a model/tool turn. Inbound receipts for Loki-sent messages
...
- **Message effects are supported.** Loki sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `loki send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<loki-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Loki is set
...
| `PHOTON_MENTION_PATTERNS` | Loki wake words  | JSON list / comma / newline regex patterns for group mentions |
