...
sidebar_position: 12
title: "Google Chat"
description: "Set up Loki Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Loki Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Loki
...
> Run `loki gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Loki can read (e.g.,
`~/.loki/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a loki restart)
...
- Principal: `loki-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Loki calls
...
- **App name**: whatever you want users to see ("Loki" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/loki-chat-events`.
...
`ADDED_TO_SPACE` event that Loki uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Loki

Add the Google Chat section to `~/.loki/.env`:
...
Send "hola" in the test DM. The bot posts a "Loki is thinking…" marker, then
...
`~/.loki/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Loki is thinking…").
...
Thread support: when a user replies inside a thread, Loki detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Loki session.
...
3. Download the JSON. Move it onto the host that runs Loki.
4. Register the client with Loki (run under the profile you want it scoped to):
...
loki -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Loki home (e.g.
`~/.loki/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.loki/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.loki/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Loki isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `loki gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Loki will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.loki/google_chat_user_tokens/<sanitized_email>.json` (filesystem
