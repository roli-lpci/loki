...
Loki Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.loki/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Loki stores session history so it can resume conversations, but it does not
...
Loki explicitly injects for that turn.
...
For example, if a user sends an image and asks Loki to make a meme from it,
Loki may inspect that image once with vision and run an image-processing
...
`loki sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `loki sessions optimize` merges FTS5 index segments and
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`loki` or `loki chat`) |
...
A bare `-c` is terminal-aware: each CLI session drops a small breadcrumb file under `~/.loki/terminal-sessions/` keyed by the terminal it runs in (tty device, tmux pane, kitty window, wezterm pane, Zellij pane, Windows Terminal session, ...). When you run `loki -c` again in the *same* terminal, Loki resumes that terminal's own session — so two panes side by side each continue their own conversation instead of both grabbing the globally most-recent one. If there's no breadcrumb for the terminal (first use, deleted session, or a stale breadcrumb older than 30 days), `-c` falls back to the most-recent-session behavior. `-c "name"` and `--resume` are unaffected. Disable with `session.terminal_continue: false` in `config.yaml`.
...
loki -c "my project"   # → resumes "my project #3"
```
...
loki --resume 20250305_091523_a1b2c3d4
loki -r 20250305_091523_a1b2c3d4

# Resume by title
loki --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
loki --resume latest

# Or with the chat subcommand
loki chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `loki sessions list`.
...
loki --resume latest --in ./my-project

# Works with the TUI too
loki --tui --resume latest --in ./my-project
```
...
```bash
loki --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`loki sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
loki sessions list --workspace my-project
loki sessions list --workspace ~/code/loki-agent
```

### Conversation Recap on Resume

When you resume a session, Loki displays a compact recap of the previous conversation in a styled panel before the input prompt:

![Stylized preview of the Previous Conversation recap panel shown when resuming an Loki session.](/img/docs/session-recap.svg)
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.loki/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `loki -r "<title>"` from the shell) and pick up where the platform left off.
...
Loki automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `loki sessions list` or `loki sessions browse`.
...
```bash
loki sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Loki creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`loki -c "my project"`), it automatically picks the most recent session in the lineage.
...
Loki provides a full set of session management commands via `loki sessions`:
...
# Filter by platform
loki sessions list --source telegram

# Show more sessions
loki sessions list --limit 50
```
...
`loki sessions export` is one surface for every export format, selected with `--format`:
...
loki sessions export backup.jsonl

# Export sessions from a specific platform
loki sessions export telegram-history.jsonl --source telegram

# Export a single session
loki sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
loki sessions export backup.jsonl --redact
```
...
loki sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
loki sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
loki sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
loki sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `loki-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
loki sessions export --format trace

# One session to a local trace file
loki sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
loki sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.loki/session-exports`).

```bash
# Export one session to Markdown
loki sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
loki sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
loki sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
loki sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
loki sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
loki sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
loki sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
loki sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
loki sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
loki sessions pin 20250305_091523_a1b2c3d4
loki sessions pin 20250305 20250306

# Remove the pin
loki sessions unpin 20250305_091523_a1b2c3d4

# List pinned sessions
loki sessions pinned

# Machine-readable output, e.g. for a nightly backup of your pin set
loki sessions pinned --json > pinned-sessions.json
```
...
loki sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
loki sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
loki sessions prune --newer-than 5h

# Explicit window with absolute timestamps
loki sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
loki sessions prune --source telegram
loki sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
loki sessions prune --newer-than 5h --title "smoke test"   # title substring
loki sessions prune --older-than 30 --max-messages 3        # tiny sessions
loki sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
loki sessions prune --model gpt-5 --older-than 1w           # by model (substring)
loki sessions prune --provider openrouter --older-than 60   # by billing provider
loki sessions prune --branch feature/old-experiment         # by git branch
loki sessions prune --user 12345678 --chat-type group       # by messaging origin
loki sessions prune --max-tokens 500 --older-than 7         # by token usage
loki sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
loki sessions prune --newer-than 5h --dry-run

# Skip confirmation
loki sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `loki sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `loki sessions prune` keeps the 90-day cutoff. Every
...
`loki sessions archive` takes the same filters as `prune` but soft-hides
...
loki sessions archive --newer-than 5h

# Archive by title substring, preview first
loki sessions archive --title "dry run" --dry-run
loki sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `loki sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`loki sessions list` and `/resume` but remain in the database and can be
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`loki insights`](/reference/cli-commands#loki-insights).
...
`loki sessions repair-routing` finds message-bearing session rows with no
...
loki sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
loki sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.loki/state.db ~/.loki/state.db.bak`).


## Importing Sessions from Claude Code and Codex CLI

Started a conversation in another agent CLI? You can pull it into Loki and
continue it here. Loki reads Claude Code's session logs
...
loki sessions import

# Limit to one tool, or point at a specific file
loki sessions import --from claude
loki sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl
...
When Loki cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Loki uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Loki in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.loki/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.loki/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.loki/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.loki/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.loki/sessions/sessions.json` is a **legacy mirror** of it,
...
`loki sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.loki/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `loki sessions list`, the cause is
`state.db` not receiving them — run `loki sessions repair` and watch for a
...
`*.jsonl` files in `~/.loki/sessions/`. They are no longer written or
read by Loki. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Loki process in the same `LOKI_HOME`

Without pruning, `state.db` grows without bound — multi-GB files within weeks were reported on gateway + cron installs. If you would rather keep every ended session forever (the pre-#54189 behavior), turn it off in `~/.loki/config.yaml`:
...
`in its tip segment`). `loki sessions export` still works for such sessions.
...
loki sessions delete <session_id>
...
:::tip
Auto-prune is **on by default**: ended sessions that have been inactive for `sessions.retention_days` (default 90) are removed at startup, and active sessions are never touched (see [Automatic Cleanup](#automatic-cleanup) above). Session history powers `session_search` recall across past conversations, so if you want to keep every ended session forever, set `sessions.auto_prune: false` in `config.yaml`, or raise `retention_days`. With auto-prune off, `loki sessions prune` remains available for one-off cleanup (observed failure mode without any pruning: a 384 MB `state.db` with ~1000 sessions slowing down FTS5 inserts and `/resume` listing).
:::
