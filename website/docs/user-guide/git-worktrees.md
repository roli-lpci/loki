...
title: "Git Worktrees"
description: "Run multiple Loki agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Loki Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Loki so each session has a clean, isolated working directory.

## Why Use Worktrees with Loki?

Loki treats the **current working directory** as the project root:

- CLI: the directory where you run `loki` or `loki chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.loki/config.yaml`
...
Loki creates `.worktrees/my-experiment/` inside the repo (branch
`loki/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `loki-<id>`
...
git worktree add ../repo-feature feature/loki-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/loki-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Loki there:

```bash
cd ../repo-feature

# Start Loki in the worktree
loki
```
...
git worktree add ../repo-experiment-a feature/loki-a
git worktree add ../repo-experiment-b feature/loki-b
```
...
- Works on its own branch (`feature/loki-a` vs `feature/loki-b`).
...
- Loki checkpoint data under `~/.loki/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Loki experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/loki-checkpoints-docs`, `feature/loki-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Loki from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `loki -w` (Automatic Worktree Mode)

Loki has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `loki/loki-<hash>`).
...
```bash
loki -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `loki -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Loki session its own clean checkout.
