...
# Connecting Desktop to Many Loki Instances

Register every Loki backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Loki Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Loki gateway…"**) deep-links straight to
...
list of Loki gateways. Its intro says it plainly: *"Manage this device and
every Loki gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Loki runtime managed by this app." | automatic |
| **Remote gateway** | "A Loki gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Loki install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Loki Cloud** | "A hosted instance discovered through your Loki Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Loki Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Loki Cloud** kind in the add-connection
...
The Loki Cloud panel also lists **Saved Cloud gateways** when portal discovery
   ...
   - *Remote gateway only:*
     ...
     - **Authentication** — choose **Session token** or **OAuth**:
       - **Session token** — paste the dashboard session token from the
         ...
         token."*
       - **OAuth** — sign in through the WunderCorp Portal browser flow; no token
         to paste.
   - *SSH only:*
...
Gateways are machines or hosted backends; profiles are isolated Loki agents
...
connection is registered) dispatches `loki update` to every eligible
...
- **Loki Cloud** instances are skipped with a *"Managed by Loki Cloud"*
...
panel, ⌘K **Update Loki**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `loki serve` is running on the remote host, the
...
- **"Update Loki Desktop to chat with agents on other connections"** — the
