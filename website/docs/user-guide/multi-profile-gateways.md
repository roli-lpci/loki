...
If you only run one Loki agent, you don't need this page — see
...
[Connecting Desktop to Many Loki Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Loki agents that should all
...
(`ai.loki.gateway-<name>.plist`) or systemd user service
(`loki-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
loki profile create coder
loki profile create personal-bot
loki profile create research
...
```bash
loki config set gateway.multiplex_profiles true
loki gateway restart
```

Equivalently, in the default profile's `~/.loki/config.yaml`:
...
You do **not** run `loki gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `loki gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.loki/profiles/coder/.env`; the default listener key is rejected.
...
home). `loki status` reports the multiplexer and the profiles it serves;
`loki status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/loki-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: loki-gateways {start|stop|restart|status|list}"
}
...
```bash
loki-gateways start      # start every configured profile
loki-gateways stop       # stop every configured profile
loki-gateways restart    # restart all
loki-gateways status     # status across all
loki-gateways list       # delegates to `loki gateway list`
```

:::tip
The `default` profile is targeted with `loki gateway <action>` (no `-p`),
not `loki -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `loki -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.loki.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/loki-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.loki.gateway.plist` /
...
```bash
# Default profile
tail -f ~/.loki/logs/gateway.log
tail -f ~/.loki/logs/gateway.error.log

# Named profile
tail -f ~/.loki/profiles/<name>/logs/gateway.log
tail -f ~/.loki/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.loki/logs/gateway.log ~/.loki/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
loki logs -f                  # follow default profile
loki -p coder logs -f         # follow one profile
loki logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
loki profile list             # profiles + model + gateway state
loki-gateways status          # full status across every profile
launchctl list | grep loki    # macOS — PIDs and labels
systemctl --user list-units 'loki-gateway-*'   # Linux — units
```
...
The default profile uses `~/.loki/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
loki config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.loki/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=loki --why="gateways running" \
  sleep infinity &
...
`loki-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.loki/.env ~/.loki/profiles/*/.env
```

## Updating the code

`loki update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `loki gateway start` after a previous `loki gateway stop`. The
...
```bash
ps -ef | grep "loki_cli.*-p <profile>"
cat ~/.loki/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.loki.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.loki.gateway-<profile>.plist

# Linux
systemctl --user restart loki-gateway-<profile>.service
```

### Health check

```bash
loki doctor                  # default profile
loki -p <profile> doctor     # one profile
```
