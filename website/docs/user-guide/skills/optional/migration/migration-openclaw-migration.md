---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Loki"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Loki"
---
...
Import an OpenClaw setup (memories, skills) into Loki.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `loki skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration\openclaw-migration` |
| Version | `1.0.0` |
| Author | Loki Agent (WunderCorp, Inc.) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Loki`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Loki Migration

Use this skill when a user wants to move their OpenClaw setup into Loki Agent with minimal manual cleanup.
...
```bash
loki claw migrate              # Full interactive migration
...
loki claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `loki setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_loki.py` to:

- import `SOUL.md` into the Loki home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Loki memory entries
- merge OpenClaw command approval patterns into Loki `command_allowlist`
- migrate Loki-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Loki working-directory configuration
- copy OpenClaw skills into `~/.loki/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Loki workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.loki/tts/`
- archive non-secret docs that do not have a direct Loki destination
...
- `scripts/openclaw_to_loki.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.loki/skills/migration/openclaw-migration/scripts/openclaw_to_loki.py`

Do not guess a shorter path like `~/.loki/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.loki/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Loki-compatible secrets, currently:
...
Loki CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Loki. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Loki workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Loki skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.loki/skills/migration/openclaw-migration/scripts/openclaw_to_loki.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.loki/skills/migration/openclaw-migration/scripts/openclaw_to_loki.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.loki/skills/migration/openclaw-migration/scripts/openclaw_to_loki.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.loki/skills/migration/openclaw-migration/scripts/openclaw_to_loki.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.loki/skills/migration/openclaw-migration/scripts/openclaw_to_loki.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.loki/skills/migration/openclaw-migration/scripts/openclaw_to_loki.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Loki unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Loki targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Loki destination. Unsupported auth blobs must still be reported as skipped.
...
- Loki persona state imported
- Loki memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.loki/skills/openclaw-imports/`
