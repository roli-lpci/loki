...
| | |
|---|---|
| Source | Optional — install with `loki skills install official/security/1password` |
| Path | `optional-skills/security\1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Loki Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Loki terminal calls (desktop app flow only)
...
### Service Account (recommended for Loki)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${LOKI_HOME:-~/.loki}/.env` (the skill will prompt for this on first load).
...
## Loki Execution Pattern (desktop app flow)

Loki terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/loki-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/loki-op.sock"
