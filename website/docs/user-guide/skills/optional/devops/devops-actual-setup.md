---
title: "Actual Setup — Set up Actual Computer (actual.inc) inference in Loki"
sidebar_label: "Actual Setup"
description: "Set up Actual Computer (actual.inc) inference in Loki"
---
...
Set up Actual Computer (actual.inc) inference in Loki.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `loki skills install official/devops/actual-setup` |
| Path | `optional-skills/devops\actual-setup` |
| Version | `2.0.0` |
| Author | shl0ms + Loki Agent |
| License | MIT |
...
Sets up [actual.inc](https://actual.inc) (Actual Computer) as a Loki inference
...
- User has an `ac_` key and wants Loki routed through their Actual cluster.
...
- Loki has **first-class `actual` provider support** (provider id `actual`,
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point Loki at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
   built-in provider into local no-auth mode automatically — no key needed:
   append `ACTUAL_BASE_URL=http://127.0.0.1:8080` to `~/.loki/.env`, then:
   ```bash
   loki config set model.provider actual
...
1. **reasoning_effort trap (handled by Loki since the first-class provider).**
...
2. **Context-window overflow on small local models.** Loki' default toolset
   ...
   servers emit a bare `data: [DONE]` — Loki reports
