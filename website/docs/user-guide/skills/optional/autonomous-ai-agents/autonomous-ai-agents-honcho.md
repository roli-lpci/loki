---
title: "Honcho — Configure and troubleshoot Honcho memory for Loki"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Loki"
---
...
Configure and troubleshoot Honcho memory for Loki.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `loki skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Loki

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Loki profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/loki#running-honcho-locally-with-loki

### Verify

```bash
loki honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Loki profile's AI peer
...
Honcho models conversations as interactions between **peers**. Loki creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Loki instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Loki run |
| `global` | Single session across all directories |

Manual override: `loki honcho map my-project-name`
...
Each Loki profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block loki.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `loki.coder` host block in `honcho.json`
...
```bash
loki honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Loki when Honcho memory is active.
...
Config file: `$LOKI_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `loki honcho setup`. Ensure `memory.provider: honcho` is in `~/.loki/config.yaml`.

### Memory not persisting across sessions
Check `loki honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `loki profile create <name> --clone`. For existing profiles: `loki honcho sync`.
...
| Command | Description |
|---------|-------------|
| `loki honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `loki honcho status` | Show resolved config, connection test, peer info for active profile |
| `loki honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `loki honcho disable` | Disable Honcho for the active profile |
| `loki honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `loki honcho peers` | Show peer identities across all profiles |
| `loki honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `loki honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `loki honcho sessions` | List known directory-to-session-name mappings |
| `loki honcho map <name>` | Map current working directory to a Honcho session name |
| `loki honcho identity` | Seed AI peer identity or show both peer representations |
| `loki honcho sync` | Create host blocks for all Loki profiles that don't have one yet |
| `loki honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Loki + Honcho |
| `loki memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `loki memory status` | Show active memory provider and config |
| `loki memory off` | Disable external memory provider |
