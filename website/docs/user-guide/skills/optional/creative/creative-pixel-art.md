...
| | |
|---|---|
| Source | Optional — install with `loki skills install official/creative/pixel-art` |
...
import sys
import os
sys.path.insert(0, os.path.expanduser("~/.loki/skills/creative/pixel-art/scripts"))
...
```bash
cd ~/.loki/skills/creative/pixel-art/scripts
...
- ffmpeg on PATH (only needed for video — Loki installs package this)
