...
| | |
|---|---|
| Source | Optional — install with `loki skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Loki MCP catalog. The
...
Two halves, in this order: the editor side must be up before Loki connects.
...
### One-time, Loki side
    ...
    loki mcp install unreal-engine
...
`~/.loki/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Loki — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Loki connects
...
2. Start the Loki session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Loki
they appear as:
...
then happens at `loki mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Loki runs batched calls
...
- **Start order matters.** Editor + server up first, then the Loki
...
- **Timeouts on long operations.** Loki' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.loki/config.yaml` for
