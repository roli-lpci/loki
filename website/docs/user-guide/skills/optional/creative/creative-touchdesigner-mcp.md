...
| | |
|---|---|
| Source | Optional — install with `loki skills install official/creative/touchdesigner-mcp` |
...
```
Loki Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${LOKI_HOME:-$HOME/.loki}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Loki config (if missing)
...
3. **Restart Loki session** to pick up the new MCP server
