...
| | |
|---|---|
| Source | Optional — install with `loki skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research\darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Loki Agent |
| License | MIT |
...
import upstream classes into Loki itself.
...
```bash
mkdir -p ~/.loki/cache/darwinian-evolver && cd ~/.loki/cache/darwinian-evolver
...
```bash
cd ~/.loki/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.loki/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.loki/skills/research/darwinian-evolver
DE_DIR=~/.loki/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Loki core.
   Custom driver scripts under `~/.loki/skills/...` are user-side and fine.
