...
| | |
|---|---|
| Source | Optional — install with `loki skills install official/mcp/mcp-oauth-remote-gateway` |
| Path | `optional-skills/mcp\mcp-oauth-remote-gateway` |
| Version | `1.0.0` |
| Author | Ben Barclay (benbarclay), Loki Agent |
| License | MIT |
| Platforms | linux, macos |
| Tags | `MCP`, `OAuth`, `PKCE`, `Remote-Deployment` |
| Related skills | [`loki-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-loki-agent), [`mcporter`](/user-guide/skills/optional/mcp/mcp-mcporter), [`fastmcp`](/user-guide/skills/optional/mcp/mcp-fastmcp) |
...
# MCP OAuth on a Remote Loki Gateway

## Overview

Loki' built-in MCP OAuth client runs a one-shot HTTP listener on `127.0.0.1:<port>`
inside the Loki process and registers that loopback address as the OAuth
`redirect_uri`. That works perfectly for a local CLI on the user's own machine.
It breaks completely when Loki runs as a remote gateway (container, VPS,
messaging bot), because the user's browser resolves `127.0.0.1` to the user's own
laptop, not the remote container — so the authorization code never reaches Loki.

This skill does the OAuth dance by hand and writes the resulting tokens into the
exact files Loki' token storage expects, so a subsequent `/reload-mcp` finds
...
2. Loki is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI Loki** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
laptop and fails to connect. The callback never reaches the Loki process, the
flow times out, and `/reload-mcp` returns "No MCP tools available" with no detail.

Symptoms to recognize: `[xdg-open] <defunct>` processes under the loki user, an
empty or missing tokens directory (`$LOKI_HOME/mcp-tokens/`), and a reload that
...
already cover the deployment. When Loki detects a remote session it prints two
...
Both require an interactive terminal to the Loki host. The rest of this skill
is for when there is NO interactive TTY — Loki running purely as a messaging
gateway/bot where `/reload-mcp` triggers the flow with nobody at a prompt.

## Preferred Front Door: the Loki Dashboard (try this BEFORE manual token surgery)

A remote Loki gateway often also runs the **dashboard** web UI as a SEPARATE
process (e.g. `loki dashboard --host 0.0.0.0 --port <port>`; check with
`ps aux | grep 'loki dashboard'`). It exposes a connector/MCP console —
...
```bash
env | grep -iE "LOKI_DASHBOARD_PUBLIC_URL|RAILWAY_PUBLIC_DOMAIN|RAILWAY_STATIC_URL|RAILWAY_SERVICE_.*_URL|PUBLIC_URL|BASE_URL|DOMAIN" \
  | sed -E 's/(TOKEN|SECRET|KEY|PASSWORD)=.*/\1=***REDACTED***/I'
```

`LOKI_DASHBOARD_PUBLIC_URL` is authoritative when present. On Railway also check
...
across restarts) and anything reading credentials from `$LOKI_HOME/.env`. Those
...
Loki' `LokiTokenStorage` would have written, so on `/reload-mcp` Loki finds
cached tokens and skips the browser flow entirely.
...
```bash
env | grep -iE "LOKI|RAILWAY|CONTAINER"
echo "$DISPLAY $WAYLAND_DISPLAY $SSH_CLIENT"
```

No display + a remote indicator = remote gateway. `tools/mcp_oauth.py::_can_open_browser()`
uses these same env vars, so if Loki' own auto-detect says "headless", the
built-in flow won't work.

### 2. Find LOKI_HOME and the config path

```bash
LOKI_HOME=$(python3 -c 'from loki_constants import get_loki_home; print(get_loki_home())')
echo "config: $LOKI_HOME/config.yaml"
echo "tokens: $LOKI_HOME/mcp-tokens/"
```
...
```json
{
  "client_name": "Loki Agent (manual OAuth)",
...
### 8. Write tokens in Loki' exact schema

`tools/mcp_oauth.py::LokiTokenStorage` expects two files under
`$LOKI_HOME/mcp-tokens/` (create dir with `0o700`, files with `0o600`):
...
body = json.dumps({
    ...
    "params": {
        ...
        "capabilities": {},
        "clientInfo": {"name": "loki-debug", "version": "1.0"},
    },
}).encode()
...
UA** — Cloudflare will 403 you even though Loki (which uses httpx) will succeed.
`scripts/diagnose-oauth-mcp.py` automates this smoke test.

### 11. Tell the user to run `/reload-mcp`

On reload, Loki sees `auth: oauth`, calls `LokiTokenStorage.get_tokens()`,
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the Loki process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** Loki needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (Loki does natively).
...
- **Don't claim Loki doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
...
- `native-mcp` — general guide to configuring MCP in Loki. Authoritative config reference lives there.
- `mcporter` — the external CLI bridge, for ad-hoc MCP calls outside of Loki' config.
