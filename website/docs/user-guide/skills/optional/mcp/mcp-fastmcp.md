...
| | |
|---|---|
| Source | Optional — install with `loki skills install official/mcp/fastmcp` |
...
| Related skills | [`loki-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-loki-agent), [`mcporter`](/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Loki or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Loki. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.loki/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.loki/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Loki integration, either:

- configure the server in `~/.loki/config.yaml` using the `native-mcp` skill, or
...
### Loki cannot see the deployed server

The server-building part may be correct while the Loki config is not. Load the `native-mcp` skill and configure the server in `~/.loki/config.yaml`, then restart Loki.
