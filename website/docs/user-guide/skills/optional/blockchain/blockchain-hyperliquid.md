...
| | |
|---|---|
| Source | Optional — install with `loki skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain\hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Loki Agent |
| License | MIT |
...
The script reads `${LOKI_HOME:-~/.loki}/.env` for two optional defaults:
...
Helper script: `~/.loki/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python ~/.loki/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${LOKI_HOME:-~/.loki}/.env`.
...
```bash
python ~/.loki/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python ~/.loki/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
