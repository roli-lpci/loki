...
| | |
|---|---|
| Source | Optional — install with `loki skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Loki Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${LOKI_HOME:-~/.loki}/.env`:
...
```bash
CANVAS="python $LOKI_HOME/skills/productivity/canvas/scripts/canvas_api.py"
