...
| | |
|---|---|
| Source | Optional — install with `loki skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${LOKI_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${LOKI_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${LOKI_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client loki
```
...
```bash
PUBLISH="${LOKI_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client loki
```
...
```bash
DRIVE="${LOKI_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `loki`)    |
