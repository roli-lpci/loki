...
| Related skills | [`blogwatcher`](/user-guide/skills/optional/research/research-blogwatcher), [`rss-feeds`](/user-guide/skills/optional/research/research-rss-feeds), [`reddit-reading`](/user-guide/skills/optional/social-media/social-media-reddit-reading) |
...
Use `rss-feeds` (optional) or `blogwatcher` (optional, stateful) for feeds, `reddit-reading` for community discussion, and `web_search`/`web_extract` for pages. Write the watch contract (watchlist, categories, materiality threshold, last cutoff) to a state file under `~/.loki/competitor-watches/<watch-slug>.json`, then create the job:

```
cronjob(action="create",
        schedule="every monday 9am",
        prompt="Load the competitor-news-monitor skill and run the tick for the watch contract at ~/.loki/competitor-watches/<watch-slug>.json.",
