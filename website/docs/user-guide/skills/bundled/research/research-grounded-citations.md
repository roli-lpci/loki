...
Ledger location: `$LOKI_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `LOKI_CITATION_LEDGER`.

## How to Run

```bash
S=~/.loki/skills/research/grounded-citations/scripts/sources.py
...
`loki skills install official/social-media/reddit-reading` or
`loki skills install official/research/rss-feeds` before using them.
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `LOKI_CITATION_LEDGER`) if their
