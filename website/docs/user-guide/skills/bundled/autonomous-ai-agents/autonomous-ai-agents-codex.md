...
| Related skills | [`claude-code`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`loki-agent`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-loki-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Loki terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Loki itself, `model.provider: openai-codex` uses Loki-managed Codex
OAuth from `~/.loki/auth.json` after `loki auth add openai-codex`. For the
...
When invoking the Codex CLI from a Loki gateway/service context (for example,
