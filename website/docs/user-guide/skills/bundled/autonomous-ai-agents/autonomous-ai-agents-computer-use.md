...
| Version | `2.0.0` |
| Author | Francesco Bonacci (f-trycua), Loki Agent |
| License | MIT |
...
Loki drives [cua-driver](https://github.com/trycua/cua) under the hood.
This wrapper skill teaches the Loki `computer_use` workflow and action
...
skill installed by `cua-driver skills install`. Loki autodetection is a
planned cua-driver follow-up, so currently point Loki at the resulting
...
|---|---|
| `cua-driver not installed` | Run `loki computer-use install`, or `loki tools` and enable Computer Use |
| Captures consistently return empty / "no on-screen window" | On Linux: DISPLAY may not be set (X11) or you're on pure Wayland — ask the user to run `loki computer-use doctor`. On Windows: you may be in Session 0 (SSH session) instead of the interactive desktop — see the cua-driver `WINDOWS.md` deep-dive |
...
| Type text disappears into a terminal emulator | cua-driver detects terminals (Ghostty, iTerm2, Terminal.app, Windows Terminal, mintty, etc.) and routes through key-event synthesis — should "just work" on a recent cua-driver. If it doesn't, ask the user to run `loki computer-use doctor` |
...
| Anything else weird | **First action: ask the user to run `loki computer-use doctor`.** It runs the cua-driver `health_report` MCP tool and prints a structured per-check matrix. Their output tells you (and them) exactly what's wrong |
...
Loki intentionally keeps THIS skill focused on the Loki-side
...
Loki autodetection is a planned follow-up in trycua/cua. For now, the command
installs the pack under `~/.cua-driver/skills/cua-driver`; point Loki at that
