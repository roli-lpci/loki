...
| List/filter by metadata (age, source, cost, tokens, workspace) | `loki sessions list` / `stats` via terminal |
| Rename | `loki sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `loki sessions archive <filters>` |
| Delete (destructive) | `loki sessions delete` / `loki sessions prune <filters>` |
| Export before deleting anything valuable | `loki sessions export --session-id <id> --format md` |
...
`loki sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `loki sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `loki sessions list`
