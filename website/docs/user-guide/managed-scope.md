...
`~/.loki/config.yaml`, `~/.loki/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/loki`:

```text
/etc/loki/
├── config.yaml     # managed config layer (wins over ~/.loki/config.yaml)
└── .env            # managed env layer (wins over ~/.loki/.env + shell)
```
...
The location can be relocated with the `LOKI_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `LOKI_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Loki.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export LOKI_MANAGED_DIR=/opt/org/loki-policy
```

:::warning
A user who can set `LOKI_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `loki doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/loki/config.yaml` (managed) | `/etc/loki/.env` (managed) |
| 2 | `~/.loki/config.yaml` (user) | `~/.loki/.env` (user) |
...
```bash
loki config        # shows a header naming the managed source + the pinned keys
loki doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Loki refuses and names the source:

```bash
$ loki config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/loki/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `loki config set` / setup will not write
...
sudo tee /etc/loki/config.yaml >/dev/null <<'YAML'
model:
  provider: wundercorp
security:
  redact_secrets: true
...
sudo tee /etc/loki/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/loki
sudo chmod 0644 /etc/loki/config.yaml /etc/loki/.env
```

Changes take effect on the next Loki start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`loki doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Loki as `root`), managed scope is advisory.
