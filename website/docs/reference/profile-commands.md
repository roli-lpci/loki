...
This page covers all commands related to [Loki profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `loki profile` without a subcommand shows help.
...
```bash
loki profile use <name>
```

Sets `<name>` as the active profile. All subsequent `loki` commands (without `-p`) will use this profile.
...
loki profile use default
```

## `loki profile create`

```bash
loki profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `loki profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `loki update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.loki`), use `loki skills opt-out` / `loki skills opt-in`. |
...
loki profile create work2 --clone-from work

# Clone everything from a specific profile
loki profile create work2-backup --clone-from work --clone-all
```

## `loki profile describe`

```bash
loki profile describe [<name>] [options]
```
...
loki profile describe researcher

# Set it explicitly
loki profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
loki profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
loki profile describe --all --auto
```

## `loki profile delete`

```bash
loki profile delete <name> [options]
```
...
```bash
loki profile delete mybot
loki profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.loki`) cannot be deleted — use `loki uninstall` to remove everything.
:::

## `loki profile show`

```bash
loki profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Loki home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ loki profile show work
Profile: work
Path:    ~/.loki/profiles/work
...
```bash
loki profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Loki installation.
...
```bash
loki profile alias work
# Creates/updates ~/.local/bin/work

loki profile alias work --name mywork
# Creates ~/.local/bin/mywork

loki profile alias work --remove
...
```bash
loki profile rename <old-name> <new-name>
```
...
```bash
loki profile rename mybot assistant
# ~/.loki/profiles/mybot → ~/.loki/profiles/assistant
...
```bash
loki profile export <name> [options]
```
...
```bash
loki profile export work
# Creates work.tar.gz in the current directory

loki profile export work -o ./work-2026-03-29.tar.gz
```
...
```bash
loki profile import <archive> [options]
```
...
```bash
loki profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

loki profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `loki -p telemetry`). |
...
loki profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
loki profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
loki profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
loki profile install ./telemetry/
```

### `loki profile update`

```bash
loki profile update <name> [--force-config] [--yes]
```
...
```bash
loki profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Loki version, author, env var requirements, the source URL/path, and
...
`loki profile list` also shows the distribution name and version in a
`Distribution` column, and `loki profile show <name>` / `delete <name>`
...
loki profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
loki profile install https://github.com/your-org/internal-assistant.git
```
...
`loki_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Loki version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Loki can clone from.
3. Tell recipients to run `loki profile install <your-repo-url>`.
...
```bash
loki -p <name> <command> [options]
loki --profile <name> <command> [options]
```

Global flag to run any Loki command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
loki -p work chat -q "Check the server status"
loki --profile dev gateway start
loki -p personal skills list
loki -p work config edit
```

## `loki completion`

```bash
loki completion <shell>
```
...
- `loki profile <TAB>` — subcommands (list, use, create, etc.)
- `loki profile use <TAB>` — profile names
- `loki -p <TAB>` — profile names
