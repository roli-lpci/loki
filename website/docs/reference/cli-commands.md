...
| `loki egress` | Outbound credential-injection firewall for remote terminal sandboxes (iron-proxy). Disabled by default. See [Egress proxy](../user-guide/egress/iron-proxy.md). |
...
| `loki auth` | Manage credentials — add, list, remove, reset, status, logout. Handles OAuth flows for Codex/WunderCorp/Anthropic. |
...
| `loki peer` | Register peer Loki gateways on other machines and DM their agents' canonical Bot Chats (`loki peer dm <peer>[/<agent>] "…"`). The transport behind cross-machine bot-to-bot messaging. |
...
| `loki approvals` | Approval-prompt tools — mine approval history into allowlist proposals. |
...
| `loki skin` | List, switch, and tweak display skins. |
| `loki console` | Open the safe Loki command console. |
...
| `loki journey` (aliases `learning`, `memory-graph`) | Timeline of learned skills + memories over time. |
...
| `loki import-agent` | Import a Claude Code (`~/.claude`) or Codex CLI (`~/.codex`) setup. |
| `loki dashboard` | Launch the web dashboard for managing config, API keys, and sessions. |
| `loki serve` | Start the Loki backend server (headless; powers the desktop app and remote backends). |
...
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, shell hooks, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from Loki itself. |
...
```bash
loki
loki chat -q "Summarize the latest PRs"          # seeds an interactive session
loki chat --oneshot -q "Summarize the latest PRs"  # answer and exit
...
Same agent, same tools, same skills — just strips every interactive / cosmetic layer. If you need tool output in the transcript too, use `loki chat --oneshot -q` instead; `-z` is explicitly for "I only want the final answer".

#### `--usage-file` — JSON usage report for pipelines

`loki -z "…" --usage-file /path/report.json` writes a machine-readable usage report after the run: `estimated_cost_usd`, `input_tokens` / `output_tokens` / `cache_read_tokens` / `cache_write_tokens` / `reasoning_tokens` / `total_tokens`, `api_calls`, `model`, `provider`, `session_id`, `service_tier`, and `completed` / `failed` flags. The report is written **even when the run fails**, so batch pipelines can always account for spend. It has no effect outside `-z`/`--oneshot`, and a broken usage write never masks the run's own outcome.

```bash
loki -z "summarize this repo" --usage-file /tmp/usage.json
...
| `enroll` | Experimental: enroll this gateway with a relay connector and save relay credentials for connector-backed platforms. See [Loki Relay](/user-guide/messaging/relay). |
...
loki slack manifest --long-description-file AGENTS.md --write
...
```bash
loki peer add <name> --url http://host:port --key <API_SERVER_KEY>
loki peer list
loki peer dm <peer>[/<agent>] "message"
loki peer run <peer>[/<agent>] --idempotency-key <key> "message"
loki peer status <peer>[/<agent>] <run_id>
loki peer stop <peer>[/<agent>] <run_id>
loki peer remove <name>
```

Bot-to-bot DMs across machines. Register another Loki gateway (any machine
running the `api_server` platform) as a *peer*, then message its agents:
`loki peer dm` resolves the remote agent's canonical **Bot Chat** session
over the peer's API server, runs one agent turn there, and prints the reply
on stdout — the cross-machine twin of the local
`loki -p <bot> chat --in ~ -c "Bot Chat" …` bot-messaging command.
...
| Subcommand | Description |
|--------|-------------|
| `add <name> --url <URL> [--key <KEY>] [--note TEXT]` | Register or update a peer. The URL goes to `config.yaml` (`bot_peers`); the key is stored as `LOKI_PEER_<NAME>_KEY` in `~/.loki/.env`. |
...
| `run <peer>[/<agent>] [message]` | Start a long canonical Bot Chat turn asynchrowundercorply and return its `run_id`, session ID, and idempotency key (`--json` supported). Reuse `--idempotency-key` when retrying the same request. |
| `status <peer>[/<agent>] <run_id>` | Poll an asynchrowundercorp peer run and print its final output when complete (`--json` supported). |
| `stop <peer>[/<agent>] <run_id>` | Stop the exact asynchrowundercorp peer run without targeting another turn (`--json` supported). |
...
includes the peer roster and the `loki peer dm` pattern, so agents discover
...
| Subcommand | Description |
|------------|-------------|
| `start` | Run the proxy in the foreground. Flags: `--provider <wundercorp\|xai>` (default `wundercorp`), `--host <addr>` (default `127.0.0.1`; use `0.0.0.0` to expose on LAN), `--port <int>` (default `8645`). |
...
loki auth add openai-codex --type oauth --priority 0   # Add an account and try it first
loki auth remove openrouter 2                          # Remove by index
loki auth priority openrouter backup-key 0             # Move a credential to the front of fill_first order
loki auth reset openrouter                             # Clear cooldowns
loki auth reset openrouter 2                           # Clear the cooldown on one credential
loki auth refresh openai-codex work                    # Refresh one OAuth credential and clear its cooldown
...
```bash
loki cron <list|create|edit|pause|resume|run|remove|status|runs|incidents|doctor|tick>
```
...
loki egress install --force          # re-download even if already installed

loki egress setup                    # interactive wizard: CA, mappings, config
loki egress setup --tunnel-port N    # override the tunnel listener port (default 9090)
loki egress setup --from-bitwarden   # use Bitwarden Secrets Manager as credential source
loki egress setup --no-bitwarden     # explicitly switch back to env-based credentials
loki egress setup --rotate-tokens    # mint fresh proxy tokens (default preserves existing)

loki egress start                    # spawn the managed proxy daemon
loki egress stop                     # SIGTERM (then SIGKILL after 5s grace)
loki egress restart                  # stop (if running) then start — needed for secret changes
loki egress reload                   # hot-reload the ruleset in-place (no restart, no dropped
                                       #   connections) via the loopback management API

loki egress status                   # binary + config + pid + listening + mappings
loki egress status --show-tokens     # print proxy tokens in full (default: redacted)

loki egress disable                  # flip proxy.enabled = false (does not stop a running proxy)
loki egress config                   # print the path to proxy.yaml for inspection
```
...
loki egress setup && loki egress start
loki config set terminal.backend docker   # if not already

# Switching credential source after the fact
loki egress setup --from-bitwarden       # env → bitwarden
loki egress setup --no-bitwarden         # bitwarden → env
# (just `setup` without either flag preserves the existing mode)

# Rotating all tokens (e.g. after a suspected token leak)
loki egress setup --rotate-tokens    # setup offers to restart the running daemon for you
# (running sandboxes still hold old tokens; restart them too)

# Adding a new upstream
# Edit ~/.loki/config.yaml proxy.extra_allowed_hosts: [api.example.com]
loki egress setup
loki egress restart                  # one-command apply (stop + start)
```

### Diagnostic shortcuts

```bash
loki egress status                     # current state in one view
cat ~/.loki/proxy/proxy.yaml           # the rendered iron-proxy config
tail -20 ~/.loki/proxy/iron-proxy.log  # daemon-level diagnostics
tail -f ~/.loki/proxy/iron-proxy.log | jq  # daemon + per-request log (line-delimited JSON; v0.39 combines both streams)
```
...
| `--script` | Filter/transform script under `~/.loki/scripts/`. The webhook payload is passed as JSON on stdin; JSON stdout replaces the payload, and empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. See [Script Filters and Transforms](../user-guide/messaging/webhooks.md#script-filters-and-transforms). |
...
| `--wundercorp` | Upload to WunderCorp-internal diagnostics storage instead of a public paste service. Use this when WunderCorp support asks for a private diagnostic bundle. |
...
Default uploads use public paste services tried in order: paste.rs, dpaste.com. `--wundercorp` uploads the same debug bundle to private WunderCorp diagnostics storage instead; the returned viewer link is for the WunderCorp team and auto-deletes after 14 days.
...
loki debug share --wundercorp       # Upload a private diagnostics bundle for WunderCorp support
...
Create a zip archive of your Loki configuration, skills, sessions, and data. The backup excludes the loki-agent codebase itself, and it does not nest earlier backup artifacts (`backups/`, `state-snapshots/`) — each of those already contains its own copy of `state.db`.
...
| `-k`, `--keep <N>` | After a full backup, delete older `loki-backup-*.zip` files in the output directory beyond the newest N (default 3; `0` keeps everything). Custom-named zips are never touched. |
...
```
  ⚠ Session data replaced by older backup contents:
    ...
    Recover from a newer backup or snapshot: loki snapshot list
```
...
| `get <key> [--json]` | Print a single config value by dotted key (e.g. `loki config get model.default`). `--json` emits machine-readable output. |
...
`loki config set/get/unset` use `.` as the nesting separator, but many real
...
- **Existing keys just work.** When navigating an existing mapping, an
  ...
  splitting. `loki config set providers.p.models.grok-4.6.supports_vision true`
...
- **Creating a new dotted key requires escaping.** Escape literal dots with a
  backslash: `loki config set 'providers.p.models.grok-4\.7.context_length' 128000`
...
| Subcommand | Description |
|------------|-------------|
| *(none)* or `picker` | Interactive catalog picker — browse WunderCorp-approved MCPs and install/enable/disable. |
| `catalog` | List WunderCorp-approved MCPs (plain text, scriptable). |
...
| `install <identifier> [--force] [--ref COMMIT_SHA] [--allow-removed]` | Install a plugin from the Loki plugin catalog (bare entry name), a Git URL, or `owner/repo` shorthand. Catalog names resolve to the entry's repo at its pinned 40-hex commit SHA, show the declared capability summary, and record catalog provenance in a `.loki-catalog.json` sidecar. Raw URLs are flagged as custom (unreviewed) sources; `--ref` (full 40-character commit SHA) pins them. `--allow-removed` (DANGEROUS) bypasses the removed-plugin blocklist. |
| `search [term] [--json]` | Search the Loki plugin catalog (matches entry names, descriptions, and declared tools; omit `term` to list everything). The catalog is curated in-repo (`plugin-catalog/`), refreshed from the live repo with a 6-hour cache, and falls back to the in-tree copy offline. Cataloged ≠ audited — admission reviews the entry, not the code. |
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Loki Plugin](../developer-guide/plugins/index.md).
...
If cua-driver is already present, Loki checks its version and runtime
...
installer. Loki never replaces a custom binary selected through
`LOKI_CUA_DRIVER_CMD`; update that binary directly or remove the override.
`loki computer-use status` reports when repair is required.

The built-in `computer_use` toolset is the recommended Loki integration.
Registering raw Cua MCP tools is an alternative when you need Cua's low-level
tool vocabulary. `cua-driver skills install` detects Loki and links Cua's
skill pack into the Loki skills directory automatically.

Permission mode and capability-manifest approval
belong to runtime launch. In bounded mode Loki passes Cua's canonical
...
```bash
loki import-agent [claude-code|codex] [options]
```

Import a **Claude Code** (`~/.claude`) or **OpenAI Codex CLI** (`~/.codex`) setup into Loki. Maps `CLAUDE.md`/`AGENTS.md` instructions to memory entries, `Bash(...)` permission allow/deny rules to `command_allowlist`/`approvals.deny`, MCP servers to `mcp_servers` in `config.yaml`, and skill directories into `~/.loki/skills/`. Always previews before applying; API keys and credentials are never imported.
...
Register this install as a self-hosted dashboard with your WunderCorp Portal account. Creates an OAuth client, writes `LOKI_DASHBOARD_OAUTH_CLIENT_ID` into `~/.loki/.env`, and prints how to engage the login gate. Requires being logged in (`loki setup`).
...
| `--portal-url` | Override the WunderCorp Portal base URL for registration (default: the portal you logged into). Also settable via `LOKI_DASHBOARD_PORTAL_URL`. |
...
```bash
loki update [--gateway] [--check] [--plan] [--no-backup] [--backup] [--yes]
```
...
| `--backup` | Force a **full** pre-update backup for this run: the quick state snapshot plus a complete zip of `LOKI_HOME` (config, auth, sessions, skills, pairing data). The default mode is `quick` — a lightweight state snapshot only. Set the permanent mode via `updates.pre_update_backup: quick | full | off` in `config.yaml`. |
...
- **Update receipts + fleet version check.** Every run writes a machine-readable receipt to `~/.loki/logs/update_receipts/` (pre-update fleet plan, steps, skips with reasons, restart outcome; `latest.json` points at the newest). After the restart phase the updater verifies each live gateway's running code against the updated checkout and prints a per-profile version matrix; a gateway still on pre-update code fails the update (exit 1) with the exact restart command.
...
| `loki uninstall [--full] [--gui] [--dry-run] [--yes]` | Remove Loki, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--dry-run` prints what would be removed without changing anything; `--yes` skips prompts. |
