...
of fields, and Loki schedules it as a cron job — no cron syntax required.
...
- **CLI, TUI, and messengers** — type `/blueprint <name>` (e.g.
  `/blueprint morning-brief`) and Loki asks you for what it needs, one
...
The generated machine-readable blueprint catalog is published at [`/api/automation-blueprints-index.json`](/api/automation-blueprints-index.json).

## Writing your own

A blueprint is just a skill with a `metadata.loki.blueprint` block in its
