...
loki config get     # Inspect individual config values
...
You don't need to rebuild your setup from scratch. Restore a full backup with `loki import` (see [Exporting Loki to another machine](/reference/faq#exporting-loki-to-another-machine)), or bring over a single agent with `loki profile import` (see [Moving a single profile to another machine](/reference/faq#moving-a-single-profile-to-another-machine)). Note that a profile export excludes credentials by design, so an export alone is not a full backup — [`loki backup` vs `loki profile export`](/reference/faq#loki-backup-vs-loki-profile-export) explains which to use.
:::
...
Loki supports a symlinked `LOKI_HOME` and symlinked home subdirectories,
...
stops with a storage error naming the path and link target. Loki does **not**
...
`loki doctor` reports these failures as storage problems, not invalid YAML.
Keep your existing `config.yaml`; running `loki setup` is not the repair for an
...
Loki auto-detects whether it was installed via the git installer, Docker, or NixOS, and `loki update` prints the matching update command for that path. There's no env var to set — the detection is based on the install layout (`~/.loki/loki-agent/` checkout, Docker image stamp, or Nix store path). `loki doctor` also surfaces the detected method under its environment summary.
