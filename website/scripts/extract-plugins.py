...
Feeds the generated plugin catalog at /api/plugins.json.
...
Outputs (both under website/static/api/, CDN-served at /api/):
...
- ``plugin-catalog.json`` — ``{"entries": [raw YAML mappings], "removed": [...]}`` in the loader's own
  schema; installed Loki clients fetch this for live catalog refresh
  (``loki_cli.plugin_catalog.LIVE_CATALOG_URL``) so new entries and removals reach them without
...
def load_catalog_entries(catalog_dir: Path) -> list[dict]:
    ...
    for path in sorted(catalog_dir.glob("*.yaml")):
        ...
        entries.append({
            ...
            "requiresLoki": str(raw.get("requires_loki") or "").strip(),
            ...
            "installCommand": f"loki plugins install {name}",
        })
