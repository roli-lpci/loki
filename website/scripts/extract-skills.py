...
2. The unified Loki Skills Index at ``website/static/api/skills-index.json``,
    ...
    for base_dir, source_label in LOCAL_SKILL_DIRS:
        ...
        for root, _dirs, files in os.walk(base_path):
            ...
            if isinstance(metadata, dict):
                loki_meta = metadata.get("loki", {})
                if isinstance(loki_meta, dict):
                    tags = loki_meta.get("tags", [])
            if not tags:
                tags = fm.get("tags", [])
