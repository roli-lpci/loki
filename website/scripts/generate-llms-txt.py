#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Loki docs site.
...
LLM reads to learn what Loki does.

Both publish at:
  https://loki.computer/llms.txt
  https://loki.computer/llms-full.txt
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Loki as a Python Library", None),
        ("guides/use-mcp-with-loki", "Use MCP with Loki", None),
        ("guides/use-voice-mode-with-loki", "Use Voice Mode with Loki", None),
        ("guides/use-soul-with-loki", "Use SOUL.md with Loki", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Loki"),
...
def emit_llms_index() -> str:
    ...
    lines.append(
        ...
        "backends. Works with WunderCorp Portal, OpenRouter, OpenAI, Anthropic, Google, or any "
        "OpenAI-compatible endpoint."
    )
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/WunderCorp/"
        "loki-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/wundercorp/loki")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Loki Agent — Full Documentation\n",
        (
            "This file is the entire Loki Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Loki, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://loki.computer\n",
        "Short index: https://loki.computer/llms.txt\n",
        "\n---\n\n",
    ]
