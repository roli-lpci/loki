#!/usr/bin/env python3
"""Generate per-skill Doku pages from skills/ and optional-skills/ SKILL.md files.
...
def mdx_escape_body(body: str) -> str:
    ...
    other markup-safe tags because Doku Markdown accepts them as HTML.
    """
...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/wundercorp/loki/blob/main/{source_dir}/{meta['rel_path']}"
...
def sidebar_doc_id(meta: dict[str, Any]) -> str:
    """documentation navigation id, relative to docs/."""
...
def render_skill_page(
    ...
    loki_meta = (fm.get("metadata") or {}).get("loki") or {}
    tags = loki_meta.get("tags") or []
    related = loki_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `loki skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
    ...
    # duplicate the page title -- Doku handles this fine because the
            ...
            result.append(
                {
                    "type": "category",
                    "label": category,
                    # The documentation generator derives a translation key from the label by
