...
# changing PATH, installing software, or touching the user's Loki home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("loki-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$LokiHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -LokiHome $LokiHome -InstallDir $InstallDir
