...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[loki] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[loki`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[loki`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        LokiHome = $(if ($paths) { $paths.loki_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\loki\loki-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-loki'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-loki') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
