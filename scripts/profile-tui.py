#!/usr/bin/env python3
"""Drive the Loki TUI under LOKI_DEV_PERF and summarize the pipeline.
...
try:
    ...
    def get_loki_home() -> Path:  # type: ignore[misc]
        val = (os.environ.get("LOKI_HOME") or "").strip()
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — LOKI_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["LOKI_DEV_PERF"] = "1"
    env["LOKI_DEV_PERF_MS"] = str(args.threshold_ms)
    env["LOKI_DEV_PERF_LOG"] = str(log)
    env["LOKI_TUI_RESUME"] = sid
    ...
    node = os.environ.get("LOKI_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="LOKI_DEV_PERF_MS (0 = capture all)")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "loki-ink" / "src"
