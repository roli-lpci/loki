...
When Loki runs in TUI mode, the gateway child process communicates with
...
# ``get_loki_home() / "plugins"`` (user) and ``./.loki/plugins/`` (project,
# gated behind ``LOKI_ENABLE_PROJECT_PLUGINS``) — see
# ``loki_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``loki_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import loki_constants (this script
    ...
    # plugins in get_loki_home()/plugins/ and project plugins in
    # ./.loki/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_loki_home() / "plugins"]
    if os.environ.get("LOKI_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".loki" / "plugins")
