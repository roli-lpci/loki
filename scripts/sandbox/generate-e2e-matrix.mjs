 ...
 *   The artifact published on the website right now -- Loki-Setup.exe has
 ...
 * @typedef {InstallMethod | 'loki-update' | 'open-app-update' | 'loki-desktop-app-update'} UpdateMethod
 ...
 *   desktop-installer install; loki-desktop-app-update starts the app
 *   via `loki desktop`, which every install method provides on every
...
export const SPEC = {
  windows: {
    install: [
      // irm https://loki.wundercorp.com/install.ps1 | iex
      { method: 'installer-script' },
      // The same one-liner with -IncludeDesktop: builds Loki.exe AND
      ...
      // Website Loki-Setup.exe, clicked through the GUI.
    ...
    update: [
      ...
      // Same button, app launched via `loki desktop`.
      { method: 'loki-desktop-app-update' },
    ],
  },
  macos: {
    install: [
      { method: 'installer-script' },
      { method: 'installer-script+desktop' },
      // The published Loki-Setup.dmg from the website, mounted and run.
      { method: 'desktop-installer', versions: ['latest'] },
    ],
    update: [
      { method: 'installer-script' },
  ...
  linux: {
    ...
    update: [
      ...
      // linux, so there is no open-app-update; `loki desktop` is always
...
export function methodNeedsDesktop(m) {
  return m.startsWith('desktop-installer') || m === 'installer-script+desktop' ||
    m === 'open-app-update' || m === 'loki-desktop-app-update';
}
