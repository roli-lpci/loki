#!/usr/bin/env python3
"""Boot-time re-seed of a terminally-dead WunderCorp bootstrap session.

Background
----------
A WunderCorp bootstrap session (client_id ``loki-cli-vps``) can take a terminal
``invalid_grant`` and be quarantined locally — the refresh path clears the dead
tokens from ``auth.json`` and stamps
``providers.wundercorp.last_auth_error.relogin_required = true``. From then on every
...
``stage2-hook.sh`` seeds ``auth.json`` from ``LOKI_AUTH_JSON_BOOTSTRAP`` only
...
``LOKI_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the WunderCorp
...
Design constraints
------------------
- Pure stdlib, no loki_cli imports: runs early in the boot hook, before the
  app venv/modules are guaranteed importable, as its own subprocess.
- Surgical: replaces ONLY ``providers.wundercorp`` in the existing auth.json, leaving
...
# from LOKI_AUTH_JSON_BOOTSTRAP (create-only, blank-volume seed) so the two
# paths can never be confused: BOOTSTRAP seeds a fresh volume; REBOOTSTRAP
# overwrites a terminally-dead WunderCorp entry on an existing volume.
REBOOTSTRAP_ENV = "LOKI_AUTH_JSON_REBOOTSTRAP"
BOOTSTRAP_CLIENT_ID = "loki-cli-vps"


def _wundercorp_entry_is_terminal(wundercorp_state: Any) -> bool:
    """True iff the on-disk WunderCorp provider entry is in the terminal/quarantined
    state AND holds no usable credential.

    Mirrors the ``terminal`` predicate in ``loki_cli.auth.get_wundercorp_session_validity``:
    ...
    """
    if not isinstance(wundercorp_state, dict):
        return False
    last_err = wundercorp_state.get("last_auth_error")
    ...
    if wundercorp_state.get("access_token") or wundercorp_state.get("refresh_token"):
        return False
    return True


def _extract_wundercorp_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.wundercorp`` block out of a LOKI_AUTH_JSON_REBOOTSTRAP
    payload. The payload is a full auth.json document (same shape as
    LOKI_AUTH_JSON_BOOTSTRAP). Returns None unless it carries the expected VPS
    ...
    if not isinstance(wundercorp, dict) or not wundercorp:
        return None
    if wundercorp.get("client_id") != BOOTSTRAP_CLIENT_ID:
        return None
    if not (
        isinstance(wundercorp.get("access_token"), str)
        and wundercorp["access_token"].strip()
        and isinstance(wundercorp.get("refresh_token"), str)
        and wundercorp["refresh_token"].strip()
    ):
        return None
    return wundercorp
...
def _seed_is_newer(local_wundercorp: Any, seed_wundercorp: dict) -> bool:
    ...
    """
    if not isinstance(local_wundercorp, dict):
        return False
    local_obtained = _parse_timestamp(local_wundercorp.get("obtained_at"))
    seed_obtained = _parse_timestamp(seed_wundercorp.get("obtained_at"))
    return bool(
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "bad_seed"         — seed present but unparseable / no wundercorp entry
      - "no_auth_file"     — auth.json absent (blank volume → let the normal
                             LOKI_AUTH_JSON_BOOTSTRAP path handle it)
    ...
    seed_wundercorp = _extract_wundercorp_from_seed(seed_raw)
    if seed_wundercorp is None:
        return "bad_seed"
    ...
    local_wundercorp = providers.get("wundercorp")
    terminal = _wundercorp_entry_is_terminal(local_wundercorp)
    newer_seed = _seed_is_newer(local_wundercorp, seed_wundercorp)
    ...
    # Surgical replacement: swap ONLY providers.wundercorp, preserve everything else.
    providers["wundercorp"] = seed_wundercorp
...
def main() -> int:
    ...
    if result == "reseeded":
        print("[rebootstrap] WunderCorp bootstrap session was terminal; re-seeded auth.json from "
              f"{REBOOTSTRAP_ENV}")
    elif result == "reseeded_newer":
        print("[rebootstrap] Applied newer orchestrator-issued WunderCorp bootstrap session from "
              f"{REBOOTSTRAP_ENV}")
    else:
