...
* ``site``        — Doku + generated skill docs.
...
# tree generates llms.txt — the index every LLM (Loki included, via the
# loki-agent skill) reads to learn what Loki can do — and
...
_MCP_CATALOG_FILES = {"loki_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Loki-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``loki serve`` backend, the Docker image)
