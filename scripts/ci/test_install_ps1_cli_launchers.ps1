# Behavioral test for install.ps1's loki launcher staging (PR #92092,
...
#
# The test lifts the real Install-LokiCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $LokiHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-LokiCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-LokiCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'loki-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'loki-agent'
    ...
    $missingThrew = $false
    try {
        Install-LokiCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing loki.exe fails the launcher stage'
    ...
    $lokiV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'loki.exe'), $lokiV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-LokiCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $lokiV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'loki.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'loki-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'loki.exe'), $lokiV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'loki-acp.exe'), $acp)
    Install-LokiCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $lokiV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'loki.exe'))) `
        'installer refreshes an existing Loki launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'loki-acp.exe'))) `
    ...
    Install-LokiCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'loki.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'loki.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'loki.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'loki.exe')) -and $cmdBody.Contains('%*')) `
