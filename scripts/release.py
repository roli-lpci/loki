...
LEGACY_AUTHOR_MAP = {
    ...
    "pi@loki.local": "Elektrofussel",  # PR #61675 salvage (honcho: defaultHost + private-range local URL detection)
    ...
    "75556242+webtecnica@users.noreply.github.com": "webtecnica",  # PR #63360 salvage (wundercorp: restore inference-api base_url)
    ...
    "kavi@local.loki": "kavioavio",  # Issue #46544 / PR #47705 evolution (smart DENY exact-operation owner override)
    ...
    "sonxi@wundercorp.local": "17324393074",  # PR #53196 salvage (tools_config: known_plugin_toolsets null guard; commit under unlinked local identity)
    ...
    "sunsky.lau@gmail.com": "liuhao1024",  # PR #56993 salvage (gateway: process-level LOKI_HOME for pid/lock/status identity files; #56986)
    ...
    "wyuebei@gmail.com": "wyuebei-cloud",  # PR #56640 salvage (loki journey: replace GNU-only %-d strftime with dt.day for Windows)
    ...
    "loki.wanderer@yahoo.com": "trismegistus-wanderer",  # PR #31856 salvage (gateway: defer idle-TTL agent-cache eviction until the session store says the session actually expired, so the expiry watcher can still fire MemoryProvider.on_session_end with the live transcript; #11205)
    ...
    "me@keslerm.com": "keslerm",  # PR #3459 salvage (gateway: 'log' tool_progress mode — silent in chat, tool calls appended to ~/.loki/logs/tool_calls.log via rotating handler; duplicate of #3458 by @dlkakbs who submitted 4 min earlier — both credited)
    ...
    "hodlclone@gmail.com": "HODLCLONE",  # PR #49351 salvage (WunderCorp Portal token resilience: rotate refresh tokens write-through to the source auth store in profile mode, skip WunderCorp fallback when no local token, sync gateway session model after fallback)
    ...
    "moonsong@wundercorp.local": "Tranquil-Flow",  # PR #52623 salvage (auxiliary Anthropic base_url host validation; #52608)
    ...
    "121278003+Cossackx@users.noreply.github.com": "Cossackx",  # PR #52528 salvage (Windows loki-shim resolution + prefer --update on recovery; #52378)
    ...
    "annguyenWunderCorp@users.noreply.github.com": "annguyenWunderCorp",
    "32201324+simpolism@users.noreply.github.com": "simpolism",
    "simpolism@gmail.com": "simpolism",
    "jake@wundercorp.com": "simpolism",
    ...
    "annguyenWunderCorp@users.noreply.github.com": "annguyenWunderCorp",
    "285874597+annguyenWunderCorp@users.noreply.github.com": "annguyenWunderCorp",
    "perkintahmaz50@gmail.com": "devatnull",
    ...
    "teknium@wundercorp.com": "teknium1",
    ...
    "274096618+loki-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "mind-dragon@wundercorp.research": "Mind-Dragon",
    ...
    "shannon@wundercorp.com": "shannonsands",
    ...
    "loki@wundercorp.com": "WunderCorp",
    ...
    "loki@wundercorphq.ai": "benbarclay",
    ...
    "emozilla@wundercorp.com": "emozilla",
    ...
    "jonny@wundercorp.com": "yoniebans",
    "jake@wundercorp.com": "simpolism",
    ...
    "sberan@gmail.com": "sberan",  # PR #54494 salvage (--connect-timeout flag on loki mcp add)
    ...
    "loki@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    # v0.16.0 additions
    "teknium@wundercorp.dev": "teknium1",
    ...
    "github@nadyaloki.anonaddy.com": "ruangraung",  # PR #42308
    ...
    "wundercorpeman666@gmail.com": "wundercorpeman666",  # PR #19088
    ...
    "sylw3st3rr@gmail.com": "Sylw3ster",  # salvage of #23252 (LOKI_KANBAN_BOARD restore)
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (LOKI_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: loki-eval community link)
    ...
    "lokiagent26@gmail.com": "lokiagent26",  # PR #26438 (kimi model-name reasoning pad)
    "276067471+lokiagent26@users.noreply.github.com": "lokiagent26",
    ...
    "clement@wundercorp.com": "lemassykoi",  # PR #27042 (model-switch probe keyless providers)
    ...
    "vigo@loki": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "ethie@wundercorp": "ethernet8023",  # PR #29342 (TUI clipboard copy on linux/wayland)
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local Loki runtime files)
    ...
    "2663402852@qq.com": "x1am1",  # PR #35098 (chown root-owned top-level LOKI_HOME state files)
    ...
    "annguyen@wundercorp.com": "annguyenWunderCorp",  # PR #52844 co-author
    ...
    "zhchl@loki-agent.local": "8294",  # PR #50572 salvage (honor config context_length on banner)
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "loki@wundercorp.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/wundercorp/loki",
    ...
    lines.append(f"# Loki Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Loki Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Loki Agent Release Tool")
    ...
    print(f"{'='*60}")
    print("  Loki Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Loki Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Loki Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Loki Agent v{new_version} ({calver_date})' "
