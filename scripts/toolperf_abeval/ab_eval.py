...
measured in the production session DB) through `loki chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    LOKI_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Loki home with credentials for the
...
""".strip(),
            ...
            env.update({
                ...
                "LOKI_HOME": str(HOME),
                "LOKI_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "loki_cli.main", "chat", "--query", q,
