...
batch (tracker: [#77056](https://github.com/wundercorp/loki/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Loki home, same model, same tasks, same reps.
...
1. Create a dedicated Loki home with credentials for the models under test:
   ...
   integration at it; no Loki observability plugin needs to be enabled.
