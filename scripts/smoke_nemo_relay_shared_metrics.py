"""Run a real Loki CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_loki_executable(loki_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "loki",
        Path(".venv") / "Scripts" / "loki.exe",
    ):
        candidate = loki_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("loki")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Loki executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Loki source checkout containing .venv/bin/loki",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated LOKI_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["loki.client.active"] != [
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['loki.client.active']}"
        )
    [model] = by_name["loki.model_route.count"]
    expected_model = {
        "name": "loki.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['loki.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["loki.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['loki.task_run.started']}"
        )
    [terminal] = by_name["loki.task_run.finished"]
    ...
    [tool] = by_name["loki.tool_call.count"]
    ...
    lifecycle = by_name["loki.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["loki.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Loki development environment requires jsonschema"
        ) from exc
    ...
    if metrics["loki.client.active"] != [
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['loki.client.active']}"
        )
    [model] = metrics["loki.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['loki.model_route.count']}"
        )
    [terminal] = metrics["loki.task_run.finished"]
    ...
    [tool] = metrics["loki.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["loki.skill.lifecycle.count"]
    ...
    loads = metrics["loki.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    loki_repo = args.loki_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    loki = _resolve_loki_executable(loki_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="loki-relay-shared-metrics-"))
    home = root / "loki-home"
    ...
    (root / "loki.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "loki.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Loki exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Loki model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Loki did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Loki did not print the mock model response")
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "loki.shared_metrics.v2.schema.json",
    )

    print("Loki -> NeMo Relay shared-metrics smoke test passed")
