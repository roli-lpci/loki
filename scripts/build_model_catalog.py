#!/usr/bin/env python3
"""Build the Loki Model Catalog — a centralized JSON manifest of curated models.

This script reads the in-repo hardcoded curated lists (``OPENROUTER_MODELS``,
``_PROVIDER_MODELS["wundercorp"]``) and writes them to a JSON manifest that the
Loki CLI fetches at runtime. Publishing the catalog through the docs site
lets maintainers update model lists without shipping a Loki release.
...
# Ensure LOKI_HOME is set for imports that touch it at module level.
os.environ.setdefault("LOKI_HOME", os.path.join(os.path.expanduser("~"), ".loki"))

from loki_cli.models import (  # noqa: E402
    OPENROUTER_MODELS,
...
def _wundercorp_entry(mid: str) -> dict:
    entry: dict = {"id": mid}
    if mid == PREFERRED_SILENT_DEFAULT_MODEL:
        entry["default"] = True
    return entry
...
def build_catalog() -> dict:
    return {
        ...
        "providers": {
            "openrouter": {
                "metadata": {
                    ...
                    "note": (
                        ...
                        'The entry labeled "default": true is the model Loki '
            ...
            "wundercorp": {
                "metadata": {
                    "display_name": "WunderCorp Portal",
                    "note": (
                        "Free-tier gating is determined live via Portal pricing "
                        "(partition_wundercorp_models_by_tier), not this manifest. "
                        'The entry labeled "default": true is the model Loki '
                        "silently lands on when the user never picked one."
                    ),
                },
                "models": [
                    _wundercorp_entry(mid)
                    for mid in _PROVIDER_MODELS.get("wundercorp", [])
                ],
            },
        },
    }
