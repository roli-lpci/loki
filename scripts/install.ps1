# ============================================================================
# Loki Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$LokiHome = $(if ($env:LOKI_HOME) { $env:LOKI_HOME } else { "$env:LOCALAPPDATA\loki" }),
    [string]$InstallDir = $(if ($env:LOKI_HOME) { "$env:LOKI_HOME\loki-agent" } else { "$env:LOCALAPPDATA\loki\loki-agent" }),
    ...
    # builds apps/desktop into a launchable Loki.exe.
    #
    # Why opt-in:
    #   * Loki-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Loki.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Loki.exe
    ...
    #     `loki desktop` already builds on demand.
...
# 5.1's progress UI repaints synchrowundercorply on every received byte, which
...
# LOKI_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[loki] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'LokiInstall.LongPath').Type) {
            Add-Type -Namespace 'LokiInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [LokiInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -LokiHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('LokiHome')) {
    $LokiHome = ConvertTo-LongPath $LokiHome
} else {
    $LokiHome = ConvertTo-LongPath $(
        if ($env:LOKI_HOME) { $env:LOKI_HOME } else { "$env:LOCALAPPDATA\loki" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:LOKI_HOME) { "$env:LOKI_HOME\loki-agent" } else { "$env:LOCALAPPDATA\loki\loki-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: LokiHome=$LokiHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:wundercorp/loki.git"
$RepoUrlHttps = "https://github.com/wundercorp/loki.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Loki Agent Installer                      |" -ForegroundColor Cyan
    Write-Host "+---------------------------------------------------------+" -ForegroundColor DarkCyan
    Write-Host "|  An open source AI agent by WunderCorp, Inc.            |" -ForegroundColor Cyan
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Blue
    Write-Host ""
}
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `loki update`'s npx
    ...
    $prefixDir = Join-Path $LokiHome "node"
...
function Install-Uv {
    # Loki owns its own uv at $LokiHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (loki_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `loki update` stay in sync.
    $managedUv = Join-Path $LokiHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $LokiHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $LokiHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $LokiHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $LokiHome "bin"
        ...
        # (loki_cli/managed_uv.py looks only at $LokiHome\bin\uv.exe).
...
# Put the Loki-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone loki-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Loki-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Loki-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("loki-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and LOKI_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by Loki belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # loki_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".loki-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first Loki-managed interpreter
    ...
    #
    # Under Loki-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve Loki-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Loki setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Loki uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\loki\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Loki needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Loki copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\loki\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``LOKI_GIT_BASH_PATH`` (User scope) so Loki can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Loki-managed PortableGit install instead..."
    }

    # Download PortableGit into $LokiHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $LokiHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Loki features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Loki needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``LOKI_GIT_BASH_PATH`` (User env scope) so Loki can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $LokiHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $LokiHome\git\usr\bin\bash.exe
    $candidates += "$LokiHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$LokiHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("LOKI_GIT_BASH_PATH", $candidate, "User")
            $env:LOKI_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set LOKI_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Loki may not find Git Bash."
    Write-Info "If needed, set LOKI_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the Loki-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (Loki requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Loki requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Loki-managed Node.js installation instead..."

    # Prefer a Loki-managed Node from a previous run over a too-old system one.
    $managedNode = "$LokiHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$LokiHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$LokiHome\node"
        Write-Success "Node.js $version found (Loki-managed)"
        ...
        Update-ManagedNpm "$LokiHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Loki-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $LokiHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $LokiHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\loki-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$LokiHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$LokiHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$LokiHome\node") {
                    try {
                        Rename-Item "$LokiHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Loki-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$LokiHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $LokiHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\loki-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `loki
                    # update` path (#4735).
                    ...
                    $stashName = "loki-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. loki-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``loki update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Loki behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running loki processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/wundercorp/loki/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/wundercorp/loki/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/wundercorp/loki/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\loki-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\loki-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `loki update` (see the notes at the shared clone-path
    ...
    # next `loki update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Loki-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "Loki-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running loki process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running loki processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Loki_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (loki.exe) plus its child tree.
            & taskkill /F /T /IM loki.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM loki.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m loki_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not loki.exe.
            ...
            # Loki children re-exec through .loki-runtime, so killing only
            ...
            # user (see loki_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Loki " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `loki update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Loki (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `loki.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install loki-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and loki.exe broken with
    ...
    # here, not 30 seconds later when the user runs `loki`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Loki processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Loki processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Loki and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register loki.exe in dist-info/RECORD but fail to
        ...
        # `loki` command while loki-agent.exe / loki-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m loki_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `loki dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\loki_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `loki dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `loki dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: loki_cli/web_server.py"
        }
    }
...
function Install-LokiCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the loki launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring loki.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "loki.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the loki command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # loki_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("loki", "loki", "loki-acp")) {
        $src = Join-Path $scriptsDir "$launcher.exe"
        if ($launcher -eq "loki" -and -not (Test-Path -LiteralPath $src -PathType Leaf)) {
            $src = Join-Path $scriptsDir "loki.exe"
        }
    ...
    $requiredExe = Join-Path $Destination "loki.exe"
    $requiredCmd = Join-Path $Destination "loki.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the loki command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up loki / loki commands..."
    
    if ($NoVenv) {
        $lokiBin = "$InstallDir"
    } else {
        # $LokiHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `loki update`'s autostash
        ...
        # installer staged under loki-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-LokiCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $lokiBin = "$LokiHome\bin"
        Install-LokiCommandLaunchers -Root $InstallDir -Destination $lokiBin | Out-Null
    }
    ...
    #   loki-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The loki-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept loki via $lokiBin)"
        }
    }
    
    if ($currentPath -notlike "*$lokiBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$lokiBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $lokiBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set LOKI_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\loki instead
    # of the Unix default ~/.loki
    $currentLokiHome = [Environment]::GetEnvironmentVariable("LOKI_HOME", "User")
    if (-not $currentLokiHome -or $currentLokiHome -ne $LokiHome) {
        [Environment]::SetEnvironmentVariable("LOKI_HOME", $LokiHome, "User")
        Write-Success "Set LOKI_HOME=$LokiHome"
    }
    $env:LOKI_HOME = $LokiHome
    
    # Update current session
    $env:Path = "$lokiBin;$env:Path"
    
    Write-Success "loki and loki commands ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.loki-bootstrap-complete which tells the Loki
    ...
    $markerPath = Join-Path $InstallDir ".loki-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the LOKI_HOME directory structure ($LokiHome, default %LOCALAPPDATA%\loki)
    New-Item -ItemType Directory -Force -Path "$LokiHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$LokiHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$LokiHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$LokiHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$LokiHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$LokiHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$LokiHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$LokiHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$LokiHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$LokiHome\.env"
    ...
    # Create config.yaml
    $configPath = "$LokiHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Loki's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$LokiHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in loki_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $LokiHome"
    
    # Seed bundled skills into $LokiHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $LokiHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $LokiHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$LokiHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $LokiHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Loki-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'loki setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\loki-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\loki-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
...
# failure is non-fatal (browser_exec can still run via uvx, and `loki tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $LokiHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Loki' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Loki' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'loki tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `loki
...
# paths still lazy-install via install_cua_driver() (loki_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `loki computer-use install` runs. Bounded
        ...
        # Loki' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'loki tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: loki computer-use install"
            }
        } else {
...
# the final `electron` -> `Loki` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Loki.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_loki_desktop command
    # resolves apps/desktop/release/win-unpacked/Loki.exe directly,
    ...
    # enforces the supported Node lines and prepends the Loki-managed Node to
    ...
    # The --dir mode produces an unpacked Loki.exe in
    ...
    # are NOT trying to work around). The Loki icon + product name are
    # stamped onto Loki.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\loki-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Loki.exe",
        "$desktopDir\release\win-arm64-unpacked\Loki.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Loki.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Loki icon + identity are stamped onto Loki.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, loki-agent#38216).
    ...
    #    Loki.exe. We deliberately do NOT point them at `loki desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Loki.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Loki.lnk')
        )
        ...
        # with the Loki icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.loki/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'loki setup'."
        return
    }
    ...
    # Run loki setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m loki_cli.main setup
    } else {
        python -m loki_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $lokiCmd = "$InstallDir\venv\Scripts\loki.exe"
    if (-not (Test-Path $lokiCmd)) {
        $lokiCmd = "loki"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$LokiHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'loki whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: loki gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$LokiHome\logs\gateway.log"
            Start-Process -FilePath $lokiCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$LokiHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: loki gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: loki gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$LokiHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$LokiHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$LokiHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$LokiHome\loki-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   loki / loki       " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   loki setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   loki config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   loki config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   loki gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   loki update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Loki repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    # Managed Python lives under $InstallDir\.loki-runtime, so the checkout
...
if ($IncludeDesktop) {
    ...
    # (Loki-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Loki to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://loki.computer/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
