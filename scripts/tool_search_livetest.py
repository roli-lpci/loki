#!/usr/bin/env python3
"""Live test harness for Loki Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any loki imports.
ORIGINAL_HOME = os.environ.get("LOKI_HOME")
ORIGINAL_AUTH = Path.home() / ".loki" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.loki/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.loki/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="loki_ts_live_"))
    loki_home = home_dir / ".loki"
    loki_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, loki_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".loki" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, loki_home / ".env")
        ...
        load_loki_dotenv(loki_home=str(Path.home() / ".loki"))
    ...
    (loki_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return loki_home
...
def reset_module_state():
    """Drop cached modules so the new LOKI_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "loki_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original LOKI_HOME
    if ORIGINAL_HOME is not None:
        os.environ["LOKI_HOME"] = ORIGINAL_HOME
    else:
