...
an unlinked SQLite DB/WAL/SHM generation.  ``loki_state`` supplies only the
...
_LOKI_EXECUTABLES = frozenset({"loki", "loki-agent", "loki-acp"})
_LOKI_PYTHON_MODULES = frozenset({"acp_adapter", "loki_cli.main"})
_LOKI_PYTHON_SCRIPTS = frozenset({"loki_cli/main.py", "run_agent.py"})
...
def _looks_like_loki(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported Loki execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _LOKI_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _LOKI_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _LOKI_PYTHON_SCRIPTS
    )
        ...
        try:
            ...
            for pid_str in os.listdir("/proc"):
                ...
                try:
                    ...
                    if argv is not None and _looks_like_loki(argv):
                        cmdline = " ".join(argv)
