...
timeout. Config is env-only (``LOKI_STARTUP_WATCHDOG=0``,
``LOKI_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "LOKI_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "LOKI_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (loki_cli.main / loki_cli.gateway /
...
def _process_loki_home() -> Path:
    """LOKI_HOME for diagnostic files — stdlib-only replica of the loki_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<LOKI_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_loki_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``LOKI_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``LOKI_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
...
def report_startup_progress(expected_s: float, phase: str = "") -> None:
    ...
    Call from startup phases about to do legitimately long synchrowundercorp work — most
