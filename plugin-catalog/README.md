...
Curated, WunderCorp-approved Loki plugins. Each YAML file in this directory
(except `removed.yaml`) is one catalog entry, discoverable via
`loki plugins catalog` / `loki plugins search` and installable with
...
1. **Human-merged gate.** Entries are added *only* via a PR to the
   `loki-agent` repository, reviewed and merged by a maintainer. There is
...
3. **Pin maturity.** The pinned release should be **at least 2 weeks old**
   ...
   compromised release before Loki ships a pointer to it.
