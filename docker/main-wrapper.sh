#!/bin/sh
# shellcheck shell=sh
# /opt/loki/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching LOKI_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `loki` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `loki <args>` (subcommand passthrough)
#
# Drop to loki via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${LOKI_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${LOKI_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export LOKI_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset LOKI_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid loki "$@"; exec "$@"; }
...
# an arbitrary non-root, non-loki UID, the bootstrap was skipped and the
# baked image dirs (owned by the loki build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u loki)" ]; then
    cat >&2 <<EOF
[loki] ERROR: container started with --user $cur_uid (an arbitrary, non-loki UID) — not supported.
...
# to the loki user's home before dropping privileges so libraries that
...
_loki_orig_cwd="${LOKI_ORIG_CWD:-$PWD}"
...
# the user's command so `loki chat` starts in the Docker -w
