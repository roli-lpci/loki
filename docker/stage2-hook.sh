...
#
# Wired into the image as /etc/cont-init.d/01-loki-setup by the
...
LOKI_HOME="${LOKI_HOME:-/opt/data}"
INSTALL_DIR="/opt/loki"

# Drop to loki via s6-setuidgid, but skip it when already non-root.
as_loki() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid loki "$@"; }
...
# starts non-root. The baked install tree under /opt/loki is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $LOKI_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass LOKI_UID/LOKI_GID — or the PUID/PGID aliases — which the
...
# `id -u loki` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# loki UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u loki)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-loki UID).
...
and the baked /opt/loki install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e LOKI_UID=\$(id -u) -e LOKI_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the loki user to that UID/GID at boot and chowns the data
...
# --- Bootstrap LOKI_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid loki mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom LOKI_HOME paths whose parents only
# root can create (e.g. `LOKI_HOME=/home/loki/.loki` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for LOKI_UID/LOKI_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  LOKI_UID/
# LOKI_GID still win when both are set.  See #15290, salvages #25872.
LOKI_UID="${LOKI_UID:-${PUID:-}}"
LOKI_GID="${LOKI_GID:-${PGID:-}}"

if [ -n "${LOKI_UID:-}" ] && validate_uid_gid "$LOKI_UID" && [ "$LOKI_UID" != "$(id -u loki)" ]; then
    echo "[stage2] Changing loki UID to $LOKI_UID"
    usermod -u "$LOKI_UID" loki
fi
if [ -n "${LOKI_GID:-}" ] && validate_uid_gid "$LOKI_GID" && [ "$LOKI_GID" != "$(id -g loki)" ]; then
    echo "[stage2] Changing loki GID to $LOKI_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$LOKI_GID" loki 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised loki user (UID 10000)
...
# loki process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes loki. Idempotent across container
...
#   - loki is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G loki 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] loki already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" loki 2>/dev/null; then
        echo "[stage2] Added loki to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group loki failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When LOKI_UID is remapped or the top-level $LOKI_HOME isn't owned by
# the runtime loki UID, restore ownership to loki — but ONLY for the
# directories loki actually writes to. The full $LOKI_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of loki-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_loki_uid=$(id -u loki)
...
chown_loki_tree() {
    ...
    chown -R loki:loki "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_loki_owner() {
    target="$1"
    find "$target" \( ! -user loki -o ! -group loki \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$LOKI_HOME" 2>/dev/null)" != "$actual_loki_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $LOKI_HOME (targeted) to loki ($actual_loki_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $LOKI_HOME: chown the directory itself (not its contents)
    # so loki can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$LOKI_HOME"; then
        :
    else
        chown loki:loki "$LOKI_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $LOKI_HOME failed (rootless container?) — continuing"
    fi
    # Loki-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by loki (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$LOKI_HOME/$sub" ] && tree_has_non_loki_owner "$LOKI_HOME/$sub"; then
            chown_loki_tree "$LOKI_HOME/$sub"
        fi
    done
fi
...
# loki user. Hosted/container instances keep mutable state under
# $LOKI_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# LOKI_DISABLE_LAZY_INSTALLS=1. Keeping /opt/loki root-owned and
...
# $LOKI_HOME/lazy-packages dir on the data volume (Dockerfile sets
# LOKI_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to loki
...
# Always reset ownership of $LOKI_HOME/profiles to loki on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> loki …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as loki and walks
...
if [ -d "$LOKI_HOME/profiles" ] && tree_has_non_loki_owner "$LOKI_HOME/profiles"; then
    chown_loki_tree "$LOKI_HOME/profiles"
fi

# Always reset ownership of $LOKI_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged loki runtime even
...
# gate as profiles/).
if [ -d "$LOKI_HOME/cron" ] && tree_has_non_loki_owner "$LOKI_HOME/cron"; then
    chown_loki_tree "$LOKI_HOME/cron"
fi

# Always ensure logs/gateways is loki-owned (#45258). Formerly healed by
...
# top-level $LOKI_HOME is mis-owned, so a warm volume with loki-owned
# LOKI_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid loki mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as loki.
if [ -d "$LOKI_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$LOKI_HOME/logs/gateways"; then
        :
    else
        chown loki:loki "$LOKI_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# loki pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged loki gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $LOKI_HOME is
...
# and cron/.
if [ -d "$LOKI_HOME/platforms/pairing" ] && tree_has_non_loki_owner "$LOKI_HOME/platforms/pairing"; then
    chown_loki_tree "$LOKI_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$LOKI_HOME/pairing" ] && tree_has_non_loki_owner "$LOKI_HOME/pairing"; then
    chown_loki_tree "$LOKI_HOME/pairing"
fi

# Reset ownership of loki-owned top-level state files on every boot.
# The targeted data-volume chown above only covers loki-owned
# *subdirectories*; loose state files living directly under $LOKI_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> loki …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged loki runtime then hits
...
# sweep so host-owned files in a bind-mounted $LOKI_HOME are never
...
# entries of loki_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$LOKI_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$LOKI_HOME/$f"; then
            :
        else
            chown loki:loki "$LOKI_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the loki runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$LOKI_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$LOKI_HOME/config.yaml"; then
        :
    else
        chown loki:loki "$LOKI_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$LOKI_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as loki user ---
# Run as loki via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $LOKI_HOME values
...
as_loki mkdir -p \
    ...
    "$LOKI_HOME/home" \
    "$LOKI_HOME/pairing" \
    "$LOKI_HOME/platforms/pairing" \
    "$LOKI_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/loki/.install_method (see Dockerfile), NOT written here into
# $LOKI_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $LOKI_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.loki:/opt/data) and sometimes shared with a
...
# 'loki update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $LOKI_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/loki stamp, so deleting the data-dir copy is safe).
if [ -f "$LOKI_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$LOKI_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$LOKI_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$LOKI_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$LOKI_HOME/$dest"; then
            :
        else
            as_loki cp "$INSTALL_DIR/$src" "$LOKI_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/loki/.env.example being present
...
# not generate one. Loki loads $LOKI_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$LOKI_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$LOKI_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $LOKI_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$LOKI_HOME/.env" ] && ! refuse_symlinked_path "clean" "$LOKI_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$LOKI_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$LOKI_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$LOKI_HOME/.env"; then
        :
    else
        if [ ! -f "$LOKI_HOME/.env" ]; then
            ...
            (umask 077 && as_loki touch "$LOKI_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$LOKI_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$LOKI_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$LOKI_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $LOKI_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $LOKI_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$LOKI_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$LOKI_HOME/.env"; then
        :
    else
        chown loki:loki "$LOKI_HOME/.env" 2>/dev/null || true
        chmod 600 "$LOKI_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `loki` user — without this it gets EACCES on every
...
# loki group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp loki /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted loki group access to the Fly Machines API socket"
    else
...
# $LOKI_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `loki update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set LOKI_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$LOKI_HOME/config.yaml" ]; then
    s6-setuidgid loki "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$LOKI_HOME/auth.json" ] && [ -n "${LOKI_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$LOKI_HOME/auth.json"; then
        :
    else
        printf '%s' "$LOKI_AUTH_JSON_BOOTSTRAP" > "$LOKI_HOME/auth.json"
        chown loki:loki "$LOKI_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$LOKI_HOME/auth.json"
    fi
fi

# auth.json: re-seed a TERMINALLY-DEAD WunderCorp bootstrap session (self-heal).
#
# The [ ! -f ] guard above deliberately refuses to clobber an existing
# auth.json, so a container whose WunderCorp bootstrap session took a terminal
# invalid_grant (tokens cleared, providers.wundercorp.last_auth_error.relogin_required
...
# supply a freshly-issued session via LOKI_AUTH_JSON_REBOOTSTRAP (distinct
# from the create-only *_BOOTSTRAP var); this helper swaps ONLY the
# providers.wundercorp entry when the on-disk entry is provably terminal OR the
...
if [ -f "$LOKI_HOME/auth.json" ] && [ -n "${LOKI_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$LOKI_HOME/auth.json"; then
        :
    else
        s6-setuidgid loki "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_wundercorp_session.py" \
            "$LOKI_HOME/auth.json" \
            || echo "[stage2] Warning: docker_rebootstrap_wundercorp_session.py failed; continuing"
    fi
fi
...
# can set LOKI_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# LOKI_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$LOKI_HOME/gateway_state.json" ] && \
        [ "${LOKI_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$LOKI_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$LOKI_HOME/gateway_state.json"
        chown loki:loki "$LOKI_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$LOKI_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_loki "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/loki/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Loki spawns for the
...
# main-wrapper.sh propagates it into the supervised ``loki`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-loki, dashboard, etc.).
