...
# Loki WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: LOKI_TINI_SHIM_TARGET overrides the `/init` path (and
# LOKI_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${LOKI_TINI_SHIM_TARGET:-/init}"
WRAPPER="${LOKI_TINI_SHIM_WRAPPER:-/opt/loki/docker/main-wrapper.sh}"
