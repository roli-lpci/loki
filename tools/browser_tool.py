...
from loki_cli.config import DEFAULT_CONFIG, cfg_get
...
# dependency could read every Loki secret from process.env.
...
def _build_browser_env() -> dict:
    ...
    env = loki_subprocess_env(inherit_credentials=False)
...
# than a concrete path (also compared in loki_cli/tools_config.py and doctor.py).
...
_REAL_PROFILE_SESSION = "loki-real-profile"
...
def _socket_safe_tmpdir() -> str:
    """Short temp dir for Unix sockets: macOS ``TMPDIR`` + ``agent-browser-loki_…``
...
# Owner Loki home per session: the janitor is one process-global thread, so each
...
def _url_policy_error(url: str, *, auto_local: bool = False) -> Optional[dict]:
    ...
    URL protects nothing. Loki' own secrets leaking into a URL are caught by ``_secret_url_error``."""
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        if not cfg_get(read_raw_config(), "browser", "record_sessions", default=False):
            return
        recordings_dir = loki_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    screenshots_dir = get_loki_dir("cache/screenshots", "browser_screenshots")
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_loki_home_override': ('loki_constants', 'get_loki_home_override'),
    'loki_home_key': ('loki_constants', 'loki_home_key'),
    ...
    'node_tool_runnable': ('loki_constants', 'node_tool_runnable'),
