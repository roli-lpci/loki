...
from loki_constants import get_loki_home_override, loki_home_key
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active Loki profile."""
    _bt = _origin()
    scope = loki_home_key()
    ...
    try:
        ...
        if isinstance(browser_cfg, dict) and "cloud_provider" in browser_cfg:
            ...
            if provider_key in ("local", "camofox"):
                ...
                _bt._cloud_provider_resolved = True
                return None
            if provider_key == "wundercorp":
                # Managed "WunderCorp Subscription" is serviced by the Browser Use provider.
