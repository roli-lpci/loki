...
_FOREGROUND_GUIDANCE = (
    (
        ...
        "notify_on_complete=true) so Loki tracks the process, then run readiness "
...
def gateway_lifecycle_block(
    *,
    ...
    ``systemctl``/``launchctl``/``loki gateway restart|stop|uninstall``
    targeting loki-gateway would SIGTERM the gateway — and this very
    ...
    SUPERVISED-gateway probe, not the raw ``_LOKI_GATEWAY`` marker: that
    marker leaks into every process that merely imports gateway.run (loki
    serve, CLI, web server), which must still be able to restart the gateway;
    an unsupervised foreground ``loki gateway run`` has no KeepAlive to turn
    ...
    if lifecycle_scan_root_within_budget(command) and contains_launchctl_submit_command(command):
        return _blocked_json(
            ...
            "or determine whether it is independent of Loki. Perform authorized "
    ...
    if contains_gateway_lifecycle_command_or_referenced_script(
        ...
        return _blocked_json(
            ...
            "to child processes). Run `loki gateway restart` from a "
