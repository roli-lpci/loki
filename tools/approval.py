...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("LOKI_YOLO_MODE", ""))
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode derives from Loki YOLO. Lazy import so approval-only
...
def _unattended_contexts() -> list[_Unattended]:
    """Active unattended contexts in evaluation order: single-query first (``loki chat -q``
    exports LOKI_INTERACTIVE=1 but nobody answers); cron beats a platform marker because
...
def _human_decision(spec: _GateSpec, *, command: str, description: str,
    ...
    # Gateway/async approval: block the agent thread until /approve or /deny, mirroring the CLI's synchrowundercorp input()
...
def _presence(approval_callback=None) -> tuple:
    ...
    (-q) exports LOKI_INTERACTIVE=1 but nobody answers prompts, and LOKI_EXEC_ASK has no
    ...
    is_ask = env_var_enabled("LOKI_EXEC_ASK")
    if _is_single_query_approval_context():
        ...
        for ctx in _unattended_contexts():
            ...
            if fail_closed_when_no_human:
                logger.warning("%s (pattern: %s): %s — no interactive user/gateway present; "
                               "BLOCKED (fail-closed). Set LOKI_INTERACTIVE or "
                               "LOKI_GATEWAY_SESSION to answer the prompt.", *log_args)
                return _blocked(no_human_block_message or (
                    ...
                    pattern_key=pattern_key, description=description)
        logger.warning("%s (pattern: %s): %s — set LOKI_INTERACTIVE or "
                       "LOKI_GATEWAY_SESSION to require approval.", *log_args)
        return _approved()
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'reset_loki_interactive_context': ('tools.approval_context', 'reset_loki_interactive_context'),
    ...
    'set_loki_interactive_context': ('tools.approval_context', 'set_loki_interactive_context'),
}
