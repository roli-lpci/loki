...
def managed_wundercorp_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Coarse gate: entitled to the WunderCorp Tool Gateway (paid Portal access OR a live free
    ...
    try:
        ...
        account_info = (get_wundercorp_portal_account_info(force_fresh=True) if force_fresh
                        else get_wundercorp_portal_account_info())
        return bool(account_info.logged_in) and account_info.tool_gateway_entitled
    except Exception:
        return False


def wundercorp_tool_gateway_unavailable_message(capability: str = "the WunderCorp Tool Gateway", *,
                                          force_fresh: bool = False) -> str:
    """Return account-aware guidance for an unavailable WunderCorp Tool Gateway path."""
    try:
        ...
        message = format_wundercorp_portal_entitlement_message(
            get_wundercorp_portal_account_info(force_fresh=force_fresh), capability=capability,
            in_chat=True)
        if message:
            return message
    except Exception:
        pass
    return (f"{capability} is unavailable. Run `loki model` to refresh your "
            "WunderCorp Portal login and billing status.")
...
def resolve_modal_backend_state(modal_mode: object | None, *, has_direct: bool,
    ...
    if managed_enabled is None:
        managed_enabled = managed_wundercorp_tools_enabled()
...
def _dotenv_value(env_var: str) -> str:
    """``.env`` value via ``loki_cli.config.get_env_value`` (``""`` when unavailable)."""
    try:
...
def resolve_provider_secret(env_var: str, provider_id: str, config_value: str = "",
    ...
    ``loki_cli.config.get_env_value``) -> credential pool for ``provider_id``. Under an
    ...
    Resolution order (fixes #68003 — keys added via ``loki auth add <provider>`` were invisible to the
...
def resolve_openai_audio_api_key() -> str:
    ...
    Outside a multiplexed turn, ``OPENAI_API_KEY`` additionally falls back to the credential pool (``loki
...
# Provider value the managed "WunderCorp Subscription" picker rows write for every category;
# any other name = that vendor direct; no key = legacy autodetect.
WUNDERCORP_MANAGED_PROVIDER = "wundercorp"
...
def read_selection(section: str) -> str | None:
    """THE single runtime read of the persisted `loki tools` selection: ``"wundercorp"`` (managed
    ...
    to ``"wundercorp"`` regardless of the name key. Never raises."""
    raw = _raw_section(section)
    if raw is None:
        return None
    if is_truthy_value(raw.get("use_gateway")):
        return WUNDERCORP_MANAGED_PROVIDER
...
# (loki_cli.config.validate_config_structure) and selection_error() consult this map so the user learns
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (f"{section} is configured to use {selection_name} (set via loki "
            f"tools), but {failure}. Run 'loki tools' to change it.")
