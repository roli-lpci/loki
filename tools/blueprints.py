...
``metadata.loki.blueprint`` (``schedule`` required; optional ``deliver`` [default "origin"],
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.loki.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.loki.blueprint`` is a mapping with a
    ...
    loki = meta.get("loki") if isinstance(meta, dict) else None
    blueprint = loki.get("blueprint") if isinstance(loki, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.loki.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    ``metadata.loki.blueprint`` block) ready for ``loki skills publish``.
    ...
    frontmatter = {
        ...
        "metadata": {"loki": {"tags": ["blueprint", "automation"], "blueprint": block}},
    }
