...
# Hard-blocked regardless of approval level (e.g. logout kills the session Loki runs in). Alt is
...
# Per-Loki-session cached backends (own cua-driver session, native target, refs, grant namespace).
...
# -> set of (action, delivery_mode) scope keys See wundercorp/loki#67052 gap 4.
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Loki's approval bypass onto Cua's immutable mode; fails closed. Both identity namespaces are consulted
...
def _new_backend(permission_mode: str) -> ComputerUseBackend:
    backend_name = os.environ.get("LOKI_COMPUTER_USE_BACKEND", "cua").lower()
    if backend_name in {"cua", "cua-driver", ""}:
        from tools.computer_use.cua_backend import CuaDriverBackend
        return CuaDriverBackend(permission_mode=permission_mode)
    if backend_name != "noop":
        raise RuntimeError(f"Unknown LOKI_COMPUTER_USE_BACKEND={backend_name!r}")
...
def _shutdown_backend_atexit() -> None:
    ...
    the Loki process that spawned it (#28152 item 3). #69903 kept the orphan from burning a core by
...
class _NoopBackend(ComputerUseBackend):  # pragma: no cover
    """Test/CI stub (LOKI_COMPUTER_USE_BACKEND=noop). Records ``(name, kwargs)`` calls; returns trivial results."""
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({"error": f"computer_use backend unavailable: {e}",
                           "hint": "If the cua-driver binary is missing, run `loki computer-use install`. "
...
def _cache_file(subdir: str, legacy: str, name: str, pattern: str = "", cap: int = 0):
    """Path for a new file under ``$LOKI_HOME/<subdir>`` (dir created). With ``pattern``/``cap``, first unlinks the
    oldest matching files so at most ``cap - 1`` remain (best-effort)."""
    from loki_constants import get_loki_dir  # lazy so tests can patch get_loki_dir
    cache_dir = get_loki_dir(subdir, legacy)
...
def _persist_capture_image(cap: CaptureResult) -> Optional[str]:
    """Copy of the capture in Loki' media cache so attachment surfaces can deliver it (None without an image)."""
...
def _route_capture_through_aux_vision(cap: CaptureResult, summary: str, *, visible_elements: Optional[List[UIElement]] = None,
    ...
    """Pre-analyse the capture via ``vision_analyze_tool`` (temp file under ``$LOKI_HOME/cache/vision/``) and merge
...
def check_computer_use_requirements() -> bool:
    """macOS/Windows/Linux + cua-driver binary (or env override). `loki computer-use doctor` names blocked checks."""
