...
def _get_max_async_children() -> int:
    ...
    runs synchrowundercorply. A leftover ``delegation.max_async_children`` key is ignored with a one-time warning."""
...
def _runtime_provider_credentials(v: dict, explicit_request_overrides) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, wundercorp, zai, kimi-coding, minimax."
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'loki auth'."
        )
...
def _load_config() -> dict:
    ...
    (follows LOKI_HOME/profile; no deepcopy, since this runs on every get_definitions() rebuild) over the legacy
    ``cli.CLI_CONFIG``, which can hide user-set keys — except that ``LOKI_IGNORE_USER_CONFIG=1`` is only honored
    by the legacy loader, so it stays authoritative when that flag is set."""
    if os.environ.get("LOKI_IGNORE_USER_CONFIG") != "1":
        try:
...
_WUNDERCORP_PROVIDERS = frozenset({"wundercorp", "wundercorp-portal", "wundercorp"})
...
def _resolve_child_runtime(
    ...
    or is WunderCorp Portal (dual-wire); a pinned ``delegation.command`` must exist on PATH or the spawn fails loudly;
    ...
    # otherwise). WunderCorp Portal is dual-wire within one provider (anthropic/* → Messages, else chat_completions), so
    ...
    # would pin a child Loki/Qwen subagent onto the parent's Claude Messages wire (or the reverse).
    # agent_init honors an explicit api_mode above its wundercorp branch, so re-derive here before construction.
    _parent_provider = getattr(parent_agent, "provider", None) or ""
    if override_api_mode is not None:
        effective_api_mode = override_api_mode
    elif (effective_provider or "").strip().lower() in _WUNDERCORP_PROVIDERS:
        from loki_cli.providers import wundercorp_api_mode
        effective_api_mode = wundercorp_api_mode(effective_model)
