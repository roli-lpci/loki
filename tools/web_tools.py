...
Backend is selected during ``loki tools`` (``web.backend`` in config.yaml; per
...
from tools.tool_backend_helpers import WUNDERCORP_MANAGED_PROVIDER, selection_exists
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via the config-aware env layer (``loki config set`` values), then process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set through Loki' config/.env layer
    (``loki config set``, ``loki tools``) are honored here too — not just raw process-env exports.
...
def _get_backend() -> str:
    ...
    if configured:
        # "wundercorp" (managed subscription) is serviced by firecrawl, routed through the managed Tool Gateway.
        return "firecrawl" if configured == WUNDERCORP_MANAGED_PROVIDER else configured
    if selection_exists("web"):
        # Selection exists (use_gateway / per-capability keys) but no shared name: firecrawl, no ladder.
        return "firecrawl"

    # Never-configured install. Explicit user credentials beat the managed-gateway probe (a WunderCorp OAuth
...
def _web_requires_env() -> list[str]:
    ...
    on ``managed_wundercorp_tools_enabled()`` cost a synchrowundercorp portal HTTP refresh at every CLI startup.
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        if provider is None:
            fallback = "No web search provider configured. Run `loki tools` to set one up."
...
def _provider_is_ready(provider) -> bool:
    ...
    False (so dispatch can emit a precise error), so readiness gates (tool check_fn, ``loki doctor``)
