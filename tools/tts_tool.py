...
def get_env_value(name, default=None):
    ...
    try:
        from loki_cli.config import get_env_value as _get_env_value
    except ImportError:
...
from tools.tool_backend_helpers import WUNDERCORP_MANAGED_PROVIDER
...
def _get_default_output_dir() -> str:
    ...
    return str(get_loki_dir("cache/audio", "audio_cache"))
...
def _default_output_dir() -> str:
    ...
    LOKI_HOME and later scope requests to a different profile via
    ``loki_constants.set_loki_home_override()`` — a frozen module constant keeps writing synthesized
    ...
    re-resolve from the live profile-scoped LOKI_HOME on every call.
    """
...
def _load_tts_config() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug("loki_cli.config not available, using default TTS config")
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    speech); ``wundercorp`` is serviced by the OpenAI path through the managed openai-audio gateway."""
    provider = (tts_config.get("provider") or DEFAULT_PROVIDER).lower().strip()
    return "openai" if provider == WUNDERCORP_MANAGED_PROVIDER else provider
...
_BUILTIN_DISPATCH: Dict[str, tuple] = {
    ...
    "mistral": (lambda: _importable(_import_mistral_client), "Mistral Voxtral TTS", "_generate_mistral_tts",
                ...
                "Run `loki setup` to install Mistral support."),
    ...
    "neutts": (lambda: _check_neutts_available(), "NeuTTS (local)", "_generate_neutts",
               ...
               "Run loki setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."),
    "kittentts": (lambda: _importable(_import_kittentts), "KittenTTS (local, ~25MB)", "_generate_kittentts",
                  ...
                  "Run 'loki setup tts' and choose KittenTTS, or install manually: "
    ...
    "piper": (lambda: _importable(_import_piper), "Piper (local)", "_generate_piper_tts",
              ...
              "Run 'loki tools' and select Piper under TTS, or install manually: "
...
def _session_platform() -> tuple:
    ...
    platform = get_session_env("LOKI_SESSION_PLATFORM", "").lower()
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_loki_home()}/audio_cache/<timestamp>.mp3"
            },
            "speed": {
                "type": "number",
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'loki_xai_user_agent': ('tools.xai_http', 'loki_xai_user_agent'),
    'managed_wundercorp_tools_enabled': ('tools.tool_backend_helpers', 'managed_wundercorp_tools_enabled'),
    'wundercorp_tool_gateway_unavailable_message': ('tools.tool_backend_helpers', 'wundercorp_tool_gateway_unavailable_message'),
