"""Skills Hub official sources: repo-shipped optional skills and the centralized Loki index."""
...
from tools.skills_hub_models import (
    SkillBundle, SkillMeta, SkillSource, _loki_tags, _matches_query, _memo_json, _parse_frontmatter, hub,
)
...
class OptionalSkillSource(SkillSource):
    """Skills from the repo's ``optional-skills/`` directory: official (WunderCorp-maintained) but not
    activated by default — absent from the system prompt and not copied to ~/.loki/skills/ at
    setup. Discoverable via the Skills Hub as source "official" with "builtin" trust."""

    SOURCE_ID = "official"
    TRUST_LEVEL = "builtin"
    OFFICIAL_REPO = "wundercorp/loki"
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # (metadata.loki.upstream); install pulls the live content from there.
        try:
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``main``; rather than demanding ``loki update`` first, resolve against the live repo.
    ...
    def _upstream_pointer_from_content(self, content: Union[str, bytes]) -> Optional[Dict[str, str]]:
        """Parse ``metadata.loki.upstream: {repo: owner/name, path: ...}`` out of SKILL.md content
        ...
        loki_meta = meta_block.get("loki") if isinstance(meta_block, dict) else None
        upstream = loki_meta.get("upstream") if isinstance(loki_meta, dict) else None
...
class LokiIndexSource(SkillSource):
    """Skill source backed by the centralized Loki Skills Index: a JSON catalog on the docs site,
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            from tools.skills_hub_search import _load_loki_index
            self._index, self._loaded = _load_loki_index(), True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        for github_id in filter(None, candidates):
            ...
            if bundle:
                bundle.source = entry.get("source", "loki-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            ...
            source=entry.get("source", "loki-index"), identifier=entry.get("identifier", ""),
