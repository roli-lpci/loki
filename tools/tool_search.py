...
tool_search / tool_describe / tool_call. Invariants: core tools (``toolsets._LOKI_CORE_TOOLS``)
...
def _config_from_loader(loader_name: str) -> ToolSearchConfig:
    """Tool-search config via ``loki_cli.config.<loader_name>`` (defaults on any failure)."""
    try:
...
def _core_tool_names() -> frozenset[str]:
    ...
    try:
        ...
        return frozenset(_LOKI_CORE_TOOLS)
    except Exception:
        return frozenset()


# Session-gated GUI toolsets: off ``_LOKI_CORE_TOOLS`` so non-GUI clients never pay
# their schema; once enabled they stay direct unless the deferral list names them.
_DIRECT_SURFACE_TOOLSETS = frozenset({"desktop_ui", "project", "browser-use", "webmcp", "link-wallet"})
