...
local server > env > managed WunderCorp gateway). Facade-owned state and helpers
...
def _transcribe_openai(
    ...
    def _run(client):
        ...
        with tempfile.TemporaryDirectory(prefix="loki-stt-") as work_dir:
            try:
...
def _transcribe_xai(
    ...
    if not api_key:
        return _error_result("No xAI credentials found. Configure xAI OAuth in `loki model` or set XAI_API_KEY")
    ...
    def _post() -> Any:
        from tools.xai_http import loki_xai_user_agent
        ...
        def _post_transcription(bearer: str, endpoint_base_url: str):
            headers = {"Authorization": f"Bearer {bearer}", "User-Agent": loki_xai_user_agent()}
...
def _transcribe_deepinfra(
    ...
    """Resolve DeepInfra credentials/model (shared ``loki_cli.models`` helpers), then delegate to :func:`_transcribe_openai`."""
    ...
    from loki_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _resolve_openai_audio_client_config() -> tuple[str, str]:
    ...
    ``"wundercorp"`` -> managed gateway ONLY (a direct OPENAI_API_KEY must NOT override it); any other
    ...
    from tools.tool_backend_helpers import (
        WUNDERCORP_MANAGED_PROVIDER, managed_wundercorp_tools_enabled, wundercorp_tool_gateway_unavailable_message,
    ...
    def _managed() -> Optional[tuple[str, str]]:
        ...
        if gateway is None:
            return None
        return gateway.wundercorp_user_token, urljoin(f"{gateway.gateway_origin.rstrip('/')}/", "v1")

    if selected == WUNDERCORP_MANAGED_PROVIDER:
        managed = _managed()
        if managed is None:
            raise ValueError(selection_error("stt", WUNDERCORP_MANAGED_PROVIDER,
    ...
    if managed is None:
        ...
        if managed_wundercorp_tools_enabled():
            message += ". " + wundercorp_tool_gateway_unavailable_message("managed OpenAI audio for transcription")
