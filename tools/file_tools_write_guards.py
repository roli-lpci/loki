...
_loki_config_resolved: str | None = None
_loki_config_resolved_loaded = False
_real_loki_home_cached: str | None = None
_real_loki_home_loaded = False
...
def _loki_home_real() -> str:
    from loki_constants import get_loki_home
    return os.path.realpath(str(get_loki_home()))


def _get_loki_config_resolved() -> str | None:
    """Return the resolved absolute path of the Loki config file (cached)."""
    return _cached_lookup("_loki_config_resolved", "_loki_config_resolved_loaded", _config_path_resolved,
                          lambda: str(Path(_expand_tilde("~/.loki/config.yaml")).resolve()))


def _get_real_loki_home() -> str | None:
    """Return the realpath of the authoritative Loki home (cached)."""
    return _cached_lookup("_real_loki_home_cached", "_real_loki_home_loaded", _loki_home_real,
                          lambda: os.path.realpath(_expand_tilde("~/.loki")))
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    loki_config = _get_loki_config_resolved()
    if loki_config and loki_config in candidates:
        return (
            f"Refusing to write to Loki config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.loki/config.yaml directly or use 'loki config' instead.")
    return None
...
# vector (AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project .loki tree).
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # ~/.loki itself is governed by its own guards (config.yaml hard-block,
    # mirror guard, write_approval); this gate targets PROJECT-LOCAL files only.
    # Must run before the ``.loki`` component rule, which would match the home.
    real_home = _get_real_loki_home()
    ...
    for candidate in (normalized, resolved):
        ...
        if base_lower in _PROTECTED_INSTRUCTION_BASENAMES or any(
            ...
            return base
        # Project-local .loki config dirs (<repo>/.loki/config.yaml) steer
        # behavior too. Only the IMMEDIATE parent counts — matching any ancestor
        # would gate every write inside a checkout living under ~/.loki.
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".loki":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Loki mirror prefix for persistent Docker file tools."""
    try:
        ...
        if env is not None:
            ...
            return "/root/.loki" if persistent_docker else None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    Loki state (a write the host never reads). Not profile isolation — that guard was
