...
def _check_feishu():
    ...
    # every ``loki`` startup; the handlers still do the real import when invoked.
...
def lark_call(client, method, uri, paths=None, queries=None, body=None):
    ...
    Tool handlers run synchrowundercorply in a worker thread (no running event loop), so the
