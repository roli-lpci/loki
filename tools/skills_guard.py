...
_LOKI_CONFIG_FILES = r'\.loki/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.loki/\.env|\~/\.loki/\.env',
     "loki_env_access", "critical", "exfiltration", "directly references Loki secrets file"),
    ...
    # quarantine only acts on "dangerous") but high for Loki/other config (setup docs routinely say
    ...
    (_prose_modify_re(_LOKI_CONFIG_FILES),
     "loki_config_mod", "high", "persistence", "modification language aimed at Loki configuration files (verify intent)"),
    (_shell_write_re(_LOKI_CONFIG_FILES),
     "loki_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting Loki configuration files"),
    (r'\.loki/config\.yaml|\.loki/SOUL\.md',
     "loki_config_ref", "low", "persistence", "references Loki configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is Loki-native; `.clawhubignore` is honored for skills published through ClawHub.
