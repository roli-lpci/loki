...
is ALWAYS host-side ``$LOKI_HOME/cache/spillover/{id}.txt`` (works for sessions that never
...
def get_spillover_dir():
    """Return $LOKI_HOME/cache/spillover as a Path (not created)."""
    from loki_constants import get_loki_home
    return get_loki_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write host-side to $LOKI_HOME/cache/spillover; returns path str or None."""
    try:
...
def _resolve_storage_dir(env) -> str:
    ...
    return f"{temp_dir.rstrip('/') or '/'}/loki-results" if temp_dir else STORAGE_DIR
...
HEREDOC_MARKER = "LOKI_PERSIST_EOF"
# ---- END PLUGIN-COMPAT ----
