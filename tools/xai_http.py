...
def has_xai_credentials() -> bool:
    ...
    try:
        ...
        auth_path = get_loki_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.loki/.env`` first, then ``os.environ``.

    Wraps :func:`loki_cli.config.get_env_value` so tests can patch ``tools.xai_http.get_env_value``.
    """
    try:
        from loki_cli.config import get_env_value as _loki_get_env_value
    except ImportError:
        return os.environ.get(name, default)
    value = _loki_get_env_value(name)
    return value if value is not None else default


def loki_xai_user_agent() -> str:
    """Return a stable Loki-specific User-Agent for xAI HTTP calls."""
    try:
        from loki_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Loki-Agent/{__version__}"


def loki_xai_default_headers() -> Dict[str, str]:
    """Default headers for OpenAI-SDK and raw HTTP clients talking to xAI (replaces the SDK User-Agent)."""
    return {"User-Agent": loki_xai_user_agent()}
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Loki home, then mark it seen."""
    notice = xai_storage_notice_text(section_name)
    if not notice:
        return None
    try:
        from loki_constants import get_loki_home
        marker_dir = get_loki_home() / "state"
...
def _xai_base_url_override() -> str:
    """``LOKI_XAI_BASE_URL`` then ``XAI_BASE_URL``, stripped; '' when unset."""
    return str(get_env_value("LOKI_XAI_BASE_URL") or get_env_value("XAI_BASE_URL") or "").strip().rstrip("/")
...
def resolve_xai_http_credentials(
    *, force_refresh: bool = False, api_key_hint: Optional[str] = None, prefer_api_key: bool = False,
    ...
    Default order: Loki-managed xAI OAuth, then ``XAI_API_KEY`` (via ``get_env_value`` so
    ``~/.loki/.env`` keys count). ``prefer_api_key=True`` inverts that for API-metered
    endpoints where the subscription OAuth bearer authorizes but misbehaves (x_search answers
    without citations, TTS 403s). Both branches honor ``LOKI_XAI_BASE_URL``/``XAI_BASE_URL``
    ...
    Prefers Loki-managed xAI OAuth credentials when available, then falls back to ``XAI_API_KEY`` resolved
    via ``loki_cli.config.get_env_value`` so keys stored in ``~/.loki/.env`` (the standard Loki
    ...
    scoping is identical to the fallback branch, and the base URL honors ``LOKI_XAI_BASE_URL`` /
