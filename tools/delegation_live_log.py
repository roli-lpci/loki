...
One append-only log per child under ``<loki_home>/cache/delegation/live/
...
def live_transcript_root() -> Path:
    """Root directory for live transcripts (profile-safe, never ~/.loki)."""
    from loki_constants import get_loki_dir
    return get_loki_dir("cache/delegation", "delegation_cache") / "live"
