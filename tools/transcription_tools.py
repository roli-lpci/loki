...
(also serves the managed ``wundercorp`` selection), mistral, xai, elevenlabs, deepinfra; plus
...
def _resolve_stt_language(
    ...
    (plus *extra_keys* aliases, e.g. ``language_code``) > ``stt.language`` > ``LOKI_LOCAL_STT_LANGUAGE``
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    """Whether *provider* is exempt from Loki's remote upload cap."""
...
def _resolve_explicit_local() -> str:
    ...
    if not backend:
        logger.warning("STT provider 'local' configured but unavailable "
                       "(install faster-whisper or set LOKI_LOCAL_STT_COMMAND)")
...
def _get_provider(stt_config: dict) -> str:
    ...
    # The managed "WunderCorp Subscription" selection is the OpenAI backend routed via the managed gateway.
    if isinstance(provider, str) and provider.strip().lower() == "wundercorp":
        provider = "openai"
...
def _start_idle_unload_watcher(timeout_seconds: int) -> None:
    ...
    with _idle_unload_mgmt_lock:
        ...
        _idle_unload_thread = threading.Thread(target=_watch, name="loki-stt-idle-unload", daemon=True)
...
def _no_provider_error(provider: str, stt_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Surface it — with its `loki tools` remediation — instead of the all-provider setup hint (#93045).
