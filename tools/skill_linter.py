...
def _check_frontmatter(frontmatter: Dict[str, Any], skill_dir: Optional[Path]) -> Iterator[LintFinding]:
    ...
    loki_meta = meta.get("loki") if isinstance(meta, dict) else None
    if not isinstance(loki_meta, dict):
        yield _warn("missing-metadata", "frontmatter is missing metadata.loki.{tags, related_skills}.")
    elif "tags" not in loki_meta:
        yield _warn("missing-metadata", "metadata.loki.tags is missing.")
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("loki", "agent", "loki agent") and (
        author != "Loki Agent"):
        yield _warn("author-caps", f"author '{author}' should be 'Loki Agent' (proper caps) "
