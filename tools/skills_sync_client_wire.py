...
Independent of local skill state (no ``~/.loki`` reads); ``tools/skills_sync_client.py``
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if ver.split(".", 1)[0] != WIRE_VERSION:
        raise SyncError(f"this server speaks sync version {ver!r}, but this Loki speaks "
                        f"{WIRE_VERSION} — update Loki to sync with it")
...
class SyncClient:
    """Sync client bound to a base URL + WunderCorp bearer. Org refs/objects live behind SEPARATE ``org/``
