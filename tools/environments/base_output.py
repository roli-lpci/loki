...
def _new_output_collector(proc, bounded_capture: bool) -> _BoundedOutputCollector:
    ...
    spill file under ``$LOKI_HOME/cache/terminal-output`` (created only on actual overflow;
    ...
    spill_path = None
    try:
        spill_dir = get_loki_home() / "cache" / "terminal-output"
...
def _pipe_stdin(proc: subprocess.Popen, data: str) -> None:
    ...
    ``proc._loki_stdin_errors`` (surfaced by ``_wait_for_process`` as ``stdin_error``) and
    stdin is still closed in ``finally`` so the child sees EOF instead of hanging.
    """
    errors: list[BaseException] = []
    proc._loki_stdin_errors = errors
    ...
    proc._loki_stdin_thread = thread
    thread.start()
...
def _popen_bash(cmd: list[str], stdin_data: str | None = None, **kwargs) -> subprocess.Popen:
    ...
    asynchrowundercorply via :func:`_pipe_stdin`. Backends with special Popen needs (e.g. local's
