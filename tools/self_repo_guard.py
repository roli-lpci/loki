...
def _block_message(operation: str, root: Path) -> str:
    ...
    scratch = (Path(loki_home).expanduser() if loki_home else Path.home() / ".loki") / "scratch"
    return (
        f"Blocked: `{operation}` would rewrite Loki's live source checkout "
        ...
        "is pushed. To change this checkout, stop Loki, run the command externally, then restart "
        "Loki.")
