...
# `cronjob(action="run")` executes in-process (comfortably below LOKI_AGENT_TIMEOUT).
# Mirrors the 10s cadence of tools/environments/base.py::touch_activity_if_due (delegate_task's heartbeat
# uses 30s) — comfortably below the 1800s default LOKI_AGENT_TIMEOUT. See #76502.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0
# Hard ceiling: with LOKI_CRON_TIMEOUT=0 a truly hung run would otherwise mask the
# gateway watchdog forever; past this the heartbeat stops and the watchdog regains authority.
# The child cron run has its own inactivity watchdog (LOKI_CRON_TIMEOUT, default 600s) that bounds a
# wedged job, but with LOKI_CRON_TIMEOUT=0 (explicit "unlimited") a truly hung run_one_job would otherwise
...
def _run_heartbeat(job_name: str):
    ...
    synchrowundercorply on the caller's thread — without tool activity the gateway inactivity
    ...
    try:
        ...
        # executes the job synchrowundercorply on the caller's thread, and a cron job is itself a full agent run
...
def _run_claimed_job(job: Dict[str, Any], extra_prompt: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``_execute_job_now`` so the background path can claim synchrowundercorply and hand the run
...
def _reap_stale_executions(job_name: str) -> None:
    ...
    one-shot `loki cron run` that died mid-run). The ticker does this at startup; one-shot
    ...
    try:
        ...
        # process -- e.g. a PRIOR one-shot `loki cron run` invocation whose dispatched runner died with
...
def _try_dispatch_background_run(
    ...
    """Claim ``job`` now (SYNCHROWUNDERCORPLY, so unrunnable jobs report immediately), then fire it
    ...
    # completion, so don't claim-and-dispatch (direct callers like `loki cron run` exit right after).
    ...
    try:
        ...
        origin_ui_session_id = get_session_env("LOKI_UI_SESSION_ID", "") or ""
    except Exception:
        pass
...
Jobs run in a fresh session with no current-chat context, so prompts must be self-contained, and the agent's FINAL RESPONSE is what gets delivered — cron runs are autonomous and cannot ask questions. Prefer updating an existing job over creating near-duplicates.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional script run each tick; stdout is injected into the agent's prompt as context (with no_agent=True the script IS the job). Relative paths resolve under {display_loki_home()}/scripts/; .sh/.bash via bash, else Python. On update, '' clears."
            },
            "monitor": {
                "type": "string",
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("LOKI_INTERACTIVE")
        or env_var_enabled("LOKI_GATEWAY_SESSION")
        or env_var_enabled("LOKI_EXEC_ASK")
    )


# Agent-facing arguments forwarded verbatim to cronjob(). model / provider / base_url are
# intentionally NOT here: per-job inference pins are user-owned (dashboard, `loki cron
