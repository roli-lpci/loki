...
DIALOG_BRIDGE_HOST = "loki-dialog-bridge.invalid"
...
# prompted synchrowundercorply without racing navigation); the native path still records it.
_DIALOG_BRIDGE_SCRIPT = r"""
(() => {
  if (window.__lokiDialogBridgeInstalled) return;
  window.__lokiDialogBridgeInstalled = true;
  const ENDPOINT = "http://loki-dialog-bridge.invalid/";
