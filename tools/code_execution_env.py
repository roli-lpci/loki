...
# prefix, the exact-name LOKI_ allowlist, or (Windows) an OS-essential name.
# The broad "LOKI_" prefix is deliberately NOT safe — it leaked config vars
# without a secret substring (LOKI_BASE_URL, LOKI_KANBAN_DB, *_WEBHOOK).
# LOKI_RPC_SOCKET / LOKI_RPC_DIR / TZ / HOME are injected after scrubbing.
...
# imports may read at import time. LOKI_DELEGATED_CHILD_CONTEXT must ride
# along or a child that imports Loki code loses the Kanban mutation guard
# while still inheriting LOKI_HOME.
_LOKI_CHILD_ALLOWED = frozenset({
    "LOKI_HOME", "LOKI_PROFILE", "LOKI_CONFIG", "LOKI_ENV", "LOKI_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    (4) operational LOKI_* pass by exact name; (5) on Windows the
    ...
    scrubbed = {}
    # Non-secret LOKI_* vars no allowlist admits are dropped on purpose; a script importing a
    ...
    for k, v in source_env.items():
        ...
        if (any(k.startswith(p) for p in _SAFE_ENV_PREFIXES)
                or k in _LOKI_CHILD_ALLOWED
                or (is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS)):
            scrubbed[k] = v
        elif k.startswith("LOKI_"):
            _dropped_loki.append(k)
    if _dropped_loki:
        logger.debug(
            "execute_code: dropped %d non-allowlisted LOKI_* var(s) from the "
            ...
            len(_dropped_loki), ", ".join(sorted(_dropped_loki)),
        )
    ...
    # non-allowlisted LOKI_* values in otherwise ordinary execution environments.
    if scoped.get(DELEGATED_CHILD_ENV_MARKER):
        for key in (DELEGATED_CHILD_ENV_MARKER, "LOKI_KANBAN_DB", "LOKI_KANBAN_BOARD"):
            if key in scoped:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    from loki_constants import apply_subprocess_home_env
    child_env = _scrub_child_env(os.environ)
    child_env["LOKI_RPC_SOCKET"] = rpc_endpoint
    child_env["LOKI_RPC_TOKEN"] = rpc_token
    ...
    # Only TZ reaches the child; LOKI_TIMEZONE is an internal setting.
    _tz_name = os.getenv("LOKI_TIMEZONE", "").strip()
    if _tz_name:
        child_env["TZ"] = _tz_name
    child_env.pop("LOKI_TIMEZONE", None)
    apply_subprocess_home_env(child_env)
    # PYTHONPATH: the staging dir (loki_tools.py) must always be importable even when project
    # mode changes CWD. Loki's root is added ONLY when the child runs in Loki's Python env —
    # exposing Loki's site-packages to an external interpreter can mix incompatible compiled
    # extensions (3.12 NumPy under a 3.9 venv). Inherited Loki-owned entries are stripped first.
    # Before re-injecting PYTHONPATH, strip Loki-owned entries that leaked through _scrub_child_env
    # (PYTHONPATH is in _SAFE_ENV_PREFIXES so it passes the scrub). They are redundant for same-Loki-
    ...
    _strip_loki_owned_pythonpath(child_env)
    _existing_pp = child_env.get("PYTHONPATH", "")
    _pp_parts = [tmpdir]
    if _uses_loki_python_environment(child_python):
        _pp_parts.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    elif child_python not in _external_env_logged:
        # Surface once per interpreter so "import loki_constants fails" is diagnosable.
        _external_env_logged.add(child_python)
        logger.info("execute_code: child interpreter %s is outside the Loki "
                    "environment; loki root omitted from PYTHONPATH", child_python)
    if _existing_pp:
...
# Interpreter paths already reported as outside the Loki environment.
...
def _uses_loki_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Loki's active Python environment. Short-circuits when
    it IS the running interpreter (by path or realpath — covers ``uv run`` venvs) so no probe
    runs on the default strict path and a flaky probe can never drop the loki root."""
