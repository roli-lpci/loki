...
class MCPServerHealthMixin:
    ...
    def _make_logging_callback(self):
        """``logging_callback`` forwarding server ``notifications/message`` into Loki logging (SDK default drops them).

        Routes MCP ``notifications/message`` log notifications from the server into Loki' logging
        (agent.log via loki_logging), tagged with the server name. Without this, the SDK's default
        ...
        async def _handler(message):
            try:
                ...
                if isinstance(payload, _core.ToolListChangedNotification):
                    ...
                    # Separate task: refreshing synchrowundercorply inside the handler can wedge the stdio
    ...
    async def _refresh_tools(self):
        ...
        notifications; after the list_tools ``await`` all mutations are synchrowundercorp — atomic on the event loop."""
