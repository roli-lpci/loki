...
def _deepinfra_model(section: Dict[str, Any], kind: str) -> Optional[str]:
    ...
    from loki_cli.models import deepinfra_model_ids
...
def _resolve_stt_client_config() -> Dict[str, Any]:
    ...
    if provider == "openai":
        # Covers the WunderCorp-managed selection too: the resolver returns the user's
...
def _resolve_tts_client_config() -> Dict[str, Any]:
    ...
    if provider == "openai":
        # Covers the direct-key, custom-base_url, and WunderCorp-managed selections.
        try:
...
def resolve_client_voice_config() -> Dict[str, Any]:
    ...
    Callers scope the profile via ``loki_constants.set_loki_home_override``
