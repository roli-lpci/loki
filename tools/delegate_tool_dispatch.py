...
(children + origin identity) and hands it to ``_run_batch``, which runs it synchrowundercorply or as
...
def _capture_origin() -> tuple[str, str, Any, Any, bool]:
    ...
    clobbers the LOKI_SESSION_ID ContextVar/os.environ with the subagent's id.  The wake-
    ...
    with _quiet(None):
        ...
        _origin_ui_session_id = get_session_env("LOKI_UI_SESSION_ID", "")
...
_SYNC_FALLBACK_NOTES = {
    "no_async": (
        ...
        "finite chat using -Q, --oneshot, or non-TTY stdio, `loki -z`, a cron job, a Kanban "
        "worker, or a stateless HTTP endpoint). The subagent(s) ran SYNCHROWUNDERCORPLY and the result is included above."
    ),
    "at_capacity": (
        "The background delegation pool was at capacity (delegation.max_concurrent_children), so the subagent(s) ran "
        "SYNCHROWUNDERCORPLY and the result is included above. Raise "
...
def _resolve_async_wake_sid(origin_wake_sid: str, origin_session_history_delivery: bool = False) -> Optional[str]:
    ...
    if get_session_env("LOKI_SINGLE_QUERY_SESSION") == "1":
        return None
    ...
    try:
        ...
        # back to SYNCHROWUNDERCORP execution so the result returns in this same turn instead of handing out a
    ...
    if origin_wake_sid:
        logger.info(
            ...
            "synchrowundercorply so the result returns in this turn.", origin_wake_sid,
        )
    return None
...
def _resolve_async_session_key(parent_agent: Any, origin_ui_session_id: str) -> tuple[str, str]:
    ...
    platform conversation key (agent:main:...). The CLI has no bound approval contextvar and no LOKI_SESSION_KEY, so
    ...
    with _quiet(None):
        ...
        origin_ui_session_id = get_session_env("LOKI_UI_SESSION_ID", "") or origin_ui_session_id
...
def _dispatch_background(batch: _Batch) -> str:
    ...
    running synchrowundercorply (with an explanatory ``note``) when the session cannot receive detached completions or the
    ...
    if wake_sid is None:
        logger.info("delegate_task: async delivery unsupported on this session runtime; running the batch synchrowundercorply instead.")
    ...
    for k, unit in enumerate(units):
        ...
        if not dispatched:
            logger.info(
                "delegate_task: async pool at capacity (%s); running the whole batch synchrowundercorply instead.",
