...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    for key in ("LOKI_KANBAN_DB", "LOKI_KANBAN_BOARD"):
...
def _node_fallback(command: str) -> str:
    ...
    loki_home = os.path.expanduser(os.getenv("LOKI_HOME", os.path.join(home, ".loki")))
    # /usr/local/bin: canonical Node location (from-source Linux, Loki Docker image, Intel Homebrew),
    # needed when a hand-authored env.PATH omits it — npx's shebang re-execs /usr/bin/env node.
    candidates = (os.path.join(loki_home, "node", "bin", command), os.path.join(home, ".local", "bin", command),
...
def _npx_bin_candidates(bin_dir: str, name: str, *, windows: Optional[bool] = None) -> list:
    ...
    extension (same precedence as ``loki_constants._candidate_node_command_names``). ``windows``
...
def _npx_cached_bin(args: list) -> Optional[tuple]:
    ...
    for nothing (~48 MB private memory per MCP server, measured); Loki already supervises the
