#!/usr/bin/env python3
"""Image generation via FAL.ai (model picked in ``loki tools``, persisted to ``image_gen.model``).
...
from tools.tool_backend_helpers import (
    WUNDERCORP_MANAGED_PROVIDER, fal_key_is_configured, managed_wundercorp_tools_enabled,
    wundercorp_tool_gateway_unavailable_message, read_selection, selection_error)
...
# --- Managed FAL gateway (WunderCorp Subscription) ---
def _resolve_managed_fal_gateway():
    """Managed gateway config for the stored `loki tools` selection, or ``None`` for direct FAL.

    ``"wundercorp"`` (or legacy ``use_gateway: true``) → managed ONLY (unreachable = selection-naming
    ...
    if selected == WUNDERCORP_MANAGED_PROVIDER:
        gateway = resolve_managed_tool_gateway("fal-queue")
        if gateway is None:
            raise ValueError(selection_error(
                "image_gen", WUNDERCORP_MANAGED_PROVIDER,
                "the WunderCorp Tool Gateway is not available (not entitled or unreachable)"))
        return gateway
...
def _get_managed_fal_client(managed_gateway):
    ...
    client_config = (managed_gateway.gateway_origin.rstrip("/"), managed_gateway.wundercorp_user_token)
    with _managed_fal_client_lock:
        if _managed_fal_client is None or _managed_fal_client_config != client_config:
            ...
            _managed_fal_client = _ManagedFalSyncClient(
                _load_fal_client(), key=managed_gateway.wundercorp_user_token,
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            if billing is not None:
                raise ValueError(
                    f"WunderCorp Subscription gateway rejected model '{model}' (HTTP {status}): {billing}") from exc
            gateway_message = ""
            if status in {401, 402, 403}:
                gateway_message = "\n\n" + wundercorp_tool_gateway_unavailable_message(
                    "managed FAL image generation", force_fresh=True)
            raise ValueError(
                f"WunderCorp Subscription gateway rejected model '{model}' (HTTP {status}). This model "
                f"may not yet be enabled on the WunderCorp Portal's FAL proxy. Either:\n"
                f"  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                f"  • Pick a different model via `loki tools` → Image Generation."
...
def _plugin_provider_name() -> Optional[str]:
    """Configured provider that must go through the plugin registry; None for unset/fal/wundercorp."""
    configured = _read_configured_image_provider()
    if not configured or configured in ("fal", WUNDERCORP_MANAGED_PROVIDER):
        return None
    return configured
...
_CACHE_BASE_BY_BACKEND = {"docker": "/root/.loki", "singularity": "/root/.loki",
                          "modal": "/root/.loki", "ssh": "~/.loki"}
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.loki"
...
def _prepare_fal_request(model_id, meta, prompt, aspect_ratio, seed, overrides, source_images):
    ...
    if source_images and not edit_endpoint:
        raise ValueError(
            ...
            f"via `loki tools` → Image Generation.")
...
def _build_no_backend_setup_message() -> str:
    ...
    managed = managed_wundercorp_tools_enabled()
    ...
    if managed:
        ...
        if gateway_message := wundercorp_tool_gateway_unavailable_message("managed FAL image generation"):
    ...
    if managed:
        lines.append("  2. Sign in to a WunderCorp account that has the managed FAL gateway enabled "
                     "(`loki setup`)")
    lines.append("  3. Configure a different image_gen provider via `loki tools` → Image Generation "
                 "(run `loki plugins list` to see installed backends)")
    return "\n".join(lines)
...
def _dispatch_to_plugin_provider(
    ...
    (provider unset / ``"fal"`` / ``"wundercorp"``). Providers without ``upscale`` ignore it via ``**kwargs``."""
    configured = _plugin_provider_name()
    if configured is None:
        return None
    try:
    ...
    if provider is None:
        return _provider_error(
            ...
            f"Run `loki plugins list` to see available image gen backends.", "provider_not_registered")
    ...
    try:
        ...
        if is_type_error and ("image_url" in kwargs or "reference_image_urls" in kwargs):
            ...
            return _provider_error(
                ...
                f"text-to-image, or pick a backend that supports editing via `loki tools` → "
...
def _maybe_route_managed_krea(
    ...
    ``"wundercorp"`` stored (a picker choice dispatches normally) and a resolvable Krea gateway.
    """
    configured_provider = _read_configured_image_provider()
    if configured_provider is not None and configured_provider != WUNDERCORP_MANAGED_PROVIDER:
        return None
