...
def _loki_tags(fm: dict) -> Any:
    """``metadata.loki.tags`` from parsed frontmatter, or ``[]`` (unvalidated type)."""
    metadata = fm.get("metadata", {})
    loki_meta = metadata.get("loki", {}) if isinstance(metadata, dict) else None
    return loki_meta.get("tags", []) if isinstance(loki_meta, dict) else []
