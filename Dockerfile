...
# node:26 image instead (Loki pins its toolchain to Node 26 everywhere).
...
# Do not write .pyc files at runtime: /opt/loki is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/loki/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when loki
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# loki process, the dashboard, and per-profile gateways.
RUN apt-get -o Acquire::Retries=3 update && \
    apt-get -o Acquire::Retries=3 install -y --no-install-recommends \
...
db.execute(\"INSERT INTO docs VALUES ('loki')\"); \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main loki process, the dashboard,
...
# 'Loki WebUI' catalog, NAS compose projects that preserve an old
...
# Non-root user for runtime; UID can be overridden via LOKI_UID at runtime
RUN useradd -u 10000 -m -d /opt/data loki
...
# lock on the @wundercorp/loki-ink entry, tripped the TUI launcher's
...
#
# The [otlp] extra contains the SDK/exporter imported by Loki when Gateway
# Health export is enabled. Collector and observability-backend dependencies
# remain external and are not part of the Loki production image.
#
# The hindsight memory provider's client (hindsight-client) is baked in
# for the same reason: it lazy-installs into /opt/loki/.venv at first
...
# gives the non-root loki user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link loki-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/loki are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the loki user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/loki/bin && \
    cp /opt/loki/docker/loki-exec-shim.sh /opt/loki/bin/loki && \
    chmod 0755 /opt/loki /opt/loki/bin/loki && \
    printf 'docker\n' > /opt/loki/.install_method
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $LOKI_HOME. $LOKI_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``loki update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the loki user via
# `s6-setuidgid loki` in its run script. If LOKI_UID is unset, services
# run as the default loki user (UID 10000).
...
# lives outside both /opt/loki (which operators sometimes bind-mount as a
# checkout) and /opt/data (the mutable LOKI_HOME volume).
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `loki dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the LOKI_GIT_SHA build-arg to
# /opt/loki/.loki_build_sha at build time, and have
# loki_cli/build_info.py read it at runtime.  Both `loki dump` and
...
ARG LOKI_GIT_SHA=
RUN set -eu; \
    if [ -n "${LOKI_GIT_SHA}" ]; then \
        printf '%s\n' "${LOKI_GIT_SHA}" > /opt/loki/.loki_build_sha; \
    fi; \
    mkdir -p /etc/loki; \
    LOKI_GIT_SHA="${LOKI_GIT_SHA}" python3 -c 'import json, os, pathlib, tomllib; project = tomllib.loads(pathlib.Path("/opt/loki/pyproject.toml").read_text(encoding="utf-8"))["project"]; marker = pathlib.Path("/etc/loki/image-provenance.json"); marker.write_text(json.dumps({"schema": 1, "deployment_kind": "image", "manager": "docker", "image": "wundercorp/loki-agent", "version": project["version"], "revision": os.environ.get("LOKI_GIT_SHA") or None}, sort_keys=True, separators=(",", ":")) + "\n", encoding="utf-8"); marker.chmod(0o444)'

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-loki + dashboard.
...
# `exec loki`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $LOKI_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/loki/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-loki-setup && \
    chmod +x /etc/cont-init.d/01-loki-setup
...
# The published image seals /opt/loki (root-owned, read-only) so a runtime
...
# is seeded + chowned to the loki user by docker/stage2-hook.sh and lives
...
ENV LOKI_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> loki ...` they default to root, and any file the
# command writes under $LOKI_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/loki/bin/loki, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid loki`
# when invoked as root. Non-root callers (supervised processes,
# `--user loki`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/loki/.venv/bin/loki). See the shim source for
# the opt-out env var (LOKI_DOCKER_EXEC_AS_ROOT=1).
COPY --chmod=0755 docker/loki-exec-shim.sh /opt/loki/bin/loki
COPY --chmod=0755 docker/entrypoint-dispatch.sh /opt/loki/docker/entrypoint-dispatch.sh

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> loki ...` and any
# subprocess that doesn't activate the venv first still find loki.
#
# /opt/loki/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/loki/bin:/opt/loki/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. loki
# subcommand vs. no-args), drops to the loki user via s6-setuidgid,
...
ENTRYPOINT [ "/opt/loki/docker/entrypoint-dispatch.sh" ]
CMD [ ]
