"""ACP tool-call helpers for mapping loki tools to ACP ToolKind and building content."""
...
# Loki tool name -> ACP ToolKind (anything unlisted is "other").
...
def get_tool_kind(tool_name: str) -> ToolKind:
    """Return the ACP ToolKind for a loki tool, defaulting to 'other'."""
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    Some Loki tools append a human hint after the payload (``{...}\\n\\n[Hint: ...]``),
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    """Return True when a structured Loki tool result clearly failed.
...
def build_tool_start(tool_call_id: str, tool_name: str, arguments: Args, *, edit_diff: Any = None) -> ToolCallStart:
    """Create a ToolCallStart event for the given loki tool invocation.
