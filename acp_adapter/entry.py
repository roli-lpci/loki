"""CLI entry point for the loki-agent ACP adapter.

Loads ``~/.loki/.env``, routes logging to stderr (stdout is reserved for ACP
JSON-RPC), and starts the ACP agent server.

Usage::

    python -m acp_adapter.entry   # or: loki acp / loki-acp
"""

# IMPORTANT: loki_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See loki_bootstrap.py for full rationale.
try:
    import loki_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Partial ``loki update`` (git-reset landed, ``uv pip install -e .`` did not):
    # UTF-8 stdio setup is skipped on Windows; POSIX is unaffected.
    pass
else:
    # Stop a ``utils/``/``proxy/``/``ui/`` package in the launch cwd from shadowing Loki modules.
...
def _load_env() -> None:
    """Load .env from LOKI_HOME (default ``~/.loki``)."""
    from loki_cli.env_loader import load_loki_dotenv

    loki_home = get_loki_home()
    loaded = load_loki_dotenv(loki_home=loki_home)
    log = logging.getLogger(__name__)
    for env_file in loaded or ():
        log.info("Loaded env from %s", env_file)
    if not loaded:
        log.info("No .env found at %s, using system env", loki_home / ".env")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="loki-acp", description="Run Loki Agent as an ACP stdio server.")
    parser.add_argument("--version", action="store_true", help="Print Loki version and exit")
    parser.add_argument("--check", action="store_true", help="Verify ACP dependencies and adapter imports, then exit")
    parser.add_argument("--setup", action="store_true",
                        help="Run interactive Loki provider/model setup for ACP terminal auth")
    parser.add_argument("--setup-browser", action="store_true",
                        help="Install agent-browser + Playwright Chromium into ~/.loki/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import LokiACPAgent  # noqa: F401

    print("Loki ACP check OK")


def _run_setup() -> None:
    from loki_cli.main import main as loki_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "loki", "model"]
        loki_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting loki-agent ACP adapter")
    ...
    import acp
    from .server import LokiACPAgent
    ...
    if os.environ.get("LOKI_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
