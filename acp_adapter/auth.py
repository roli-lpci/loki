"""ACP auth helpers — detect and advertise Loki authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "loki-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Loki runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Loki.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no Loki credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate Loki using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure Loki provider", type="terminal", args=["--setup"],
        description=("Open Loki' interactive model/provider setup in a terminal. "
                     "Use this when Loki has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if Loki can resolve any runtime provider credentials."""
