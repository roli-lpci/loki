...
import { Box, Text } from '@wundercorp/loki-ink'
...
/**
 * User widget apps — Loki authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$LOKI_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.loki/plugins/`: files under LOKI_HOME execute
...
const widgetsDir = () => join(process.env.LOKI_HOME?.trim() || join(homedir(), '.loki'), 'tui-widgets')
 ...
 *  every change, so a widget Loki writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Loki) ever writes must hot-load too — a 10s poll here read
