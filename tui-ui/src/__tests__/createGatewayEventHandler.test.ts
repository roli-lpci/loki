...
describe('createGatewayEventHandler', () => {
  ...
  it('opens a billing confirm dialog routing WunderCorp to /topup', () => {
    const appended: Msg[] = []
    ...
    onEvent({
      payload: {
        billing: {
          ...
          model: 'm',
          provider: 'wundercorp',
          provider_label: 'WunderCorp Portal'
        },
    ...
    expect(confirm?.title).toContain('WunderCorp')
  ...
  it('surfaces self-improvement review summaries as a persistent system line', () => {
    ...
    onEvent({
      payload: { text: "💾 Self-improvement review: Skill 'loki-release' patched" },
      type: 'review.summary'
    } as any)

    expect(ctx.system.sys).toHaveBeenCalledWith("💾 Self-improvement review: Skill 'loki-release' patched")
  })
  ...
  it('annotates gateway.start_timeout with stderr tail lines so users can diagnose without /logs', () => {
    ...
    onEvent({
      payload: {
        ...
        stderr_tail:
          '[startup] timed out\nModuleNotFoundError: No module named openai\nFileNotFoundError: ~/.loki/config.yaml'
      },
  ...
  it('prefers raw text over Rich-rendered ANSI on message.complete (#16391)', () => {
    ...
    const rendered = '\u001b[33mLoki here.\u001b[0m\n\n\u001b[2mLine two.\u001b[0m'
  ...
  it('shows setup panel for missing provider startup error', () => {
    ...
    onEvent({
      payload: {
        message:
          'agent init failed: No LLM provider configured. Run `loki model` to select a provider, or run `loki setup` for first-time configuration.'
      },
      type: 'error'
    } as any)
  ...
  it('picks the polarity-matching paired palette from gateway.ready skins', async () => {
    ...
    vi.stubEnv('LOKI_TUI_BACKGROUND', '')
    ...
    vi.stubEnv('LOKI_TUI_BACKGROUND', '#ffffff')
    createGatewayEventHandler(buildCtx(appended))({ payload: skin, type: 'skin.changed' } as any)
  ...
  it('opens a fresh session before starting voice after wake detection', async () => {
    ...
    createGatewayEventHandler(ctx)({
      payload: { phrase: 'hey loki', start_new_session: true },
      type: 'wake.detected'
    } as any)
  ...
  it('keeps the current session when wake detection disables session creation', async () => {
    ...
    createGatewayEventHandler(ctx)({
      payload: { phrase: 'hey loki', start_new_session: false },
      type: 'wake.detected'
    } as any)
  ...
  it('on gateway.ready after a crash, resumes the recovered session once and skips forge', async () => {
    ...
    ctx.session.newSession = newSession
    // Mimic resumeById's synchrowundercorp status write so the test proves the
