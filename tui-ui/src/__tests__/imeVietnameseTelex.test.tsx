...
import { renderSync } from '@wundercorp/loki-ink'
...
//   2. textInput commits multi-character (IME/paste) inserts synchrowundercorply
...
// wait) so a passing assertion proves the commit was synchrowundercorp, not deferred.
async function drive(
  ...
  try {
    ...
    // synchrowundercorply; the old deferred path (scheduleKeyBurstCommit, 16ms)
...
describe('Vietnamese Telex IME recomposition', () => {
  ...
  it('commits a multi-character recompose synchrowundercorply (no dropped tail)', async () => {
...
describe('Fast-echo suppression reset (60ms window)', () => {
  ...
  it('suppresses fast-echo backspace for one keystroke after an Ink repaint (IME recompose)', async () => {
    ...
    try {
      ...
      // The recomposed value must be committed synchrowundercorply (no dropped tail).
