...
function OverviewScreen({ onClose, onPatch, overlay, t }: ScreenProps) {
  ...
  const doManage = () => {
    if (s.portal_url) {
      ...
      ctx.sys('🔴 No portal URL available — manage your subscription on the WunderCorp portal.')
    }
...
function ConfirmScreen({ onClose, onPatch, overlay, t }: ScreenProps) {
  ...
  // Synchrowundercorp guard: two key events can both see submitting===false before
