...
import { Box, Text, useInput } from '@wundercorp/loki-ink'
...
function OverviewScreen({ ctx, onClose, onPatch, s, t }: ScreenProps) {
  ...
  const note = !s.is_admin
    ...
    : !s.cli_billing_enabled
      ? "Remote spending is off for this org — a billing admin can turn it on from the portal's Loki Agent page."
      : null
...
function BuyScreen({ ctx, onPatch, s, t }: ScreenProps) {
  ...
  // Synchrowundercorp guard: double-Enter on "check again" must not stack re-fetches.
...
function ConfirmScreen({
  ...
  // Synchrowundercorp guard: two key events can both observe `submitting === false`
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text color={t.color.muted}>By confirming, you allow WunderCorp, Inc. to charge your card.</Text>
      <Text />
...
function AutoReloadScreen({ ctx, onClose, onPatch, s, t }: ScreenProps) {
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text />
      <Text color={t.color.muted}>
        By confirming, you authorize WunderCorp, Inc. to charge {chargeCardName} whenever your balance falls below the
