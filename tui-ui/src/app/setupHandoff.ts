import type { RunExternalProcess } from '@wundercorp/loki-ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`loki ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching loki: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`loki ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
