import type { SkinBranding, SkinColors } from '@wundercorp/loki-shared/skin'
...
//     (very pale custom text still gets rescued).
...
// bright brand tones, not a WCAG-darkened mustard. So the light
// floor is a near-invisible rescue only and leaves readable brand tones untouched. Pixel-sampled target: #F5C242 (L61 S90),
 ...
 *  answer when known (cached in LOKI_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.LOKI_TUI_BACKGROUND ?? '').trim()
 ...
 *   dark label follows desaturate(mix(accent, bg, .13), .16)
...
// Terminal. Explicit LOKI_TUI_THEME / COLORFGBG signals above still win,
...
// `LOKI_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `LOKI_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `LOKI_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `LOKI_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Loki palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.LOKI_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.LOKI_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.LOKI_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into LOKI_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into LOKI_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Loki navy
  ...
  // the derived value already carries the identity seeds (e.g. Loki navy).
  const activeRow =
