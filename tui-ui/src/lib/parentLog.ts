...
const logDir = join(process.env.LOKI_HOME?.trim() || join(homedir(), '.loki'), 'logs')
...
// a real ~/.loki (tests must stay hermetic — see AGENTS.md).
...
// or add noticeable blocking on the synchrowundercorp append. Mirrors the spirit of
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('loki-tui: parent lifecycle log unavailable\n')
    }
  }
}
