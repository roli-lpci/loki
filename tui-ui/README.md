...
React + Ink terminal UI for Loki. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `LOKI_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `loki setup`, suspends Ink while it runs, opens a new session on success
...
- `commands/billing.ts` — `/billing`: manage WunderCorp remote spending — buy credits, auto-reload, limits
...
- Input history is stored in `~/.loki/.loki_history` or under `LOKI_HOME`.
...
### Billing (`billing.ts`)
`/billing` — manage WunderCorp remote spending — buy credits, auto-reload, limits
...
### Credits (`credits.ts`)
`/credits` — WunderCorp credit balance and browser top-up

### Setup (`setup.ts`)
`/setup` — launches external `loki setup` wizard, suspends Ink while it runs
...
tui-ui/
  ...
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external loki setup, suspends Ink while it runs
      ...
      slash/
        ...
        commands/
          billing.ts                /billing — manage WunderCorp remote spending
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.loki/tui-parent.log
    ...
    types/
      loki-ink.d.ts            type declarations for @wundercorp/loki-ink
...
tui_gateway/
  ...
  slash_worker.py        persistent LokiCLI subprocess for slash commands
```
