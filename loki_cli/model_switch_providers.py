...
Split out of ``loki_cli/model_switch.py``; every moved name is re-imported there so
``loki_cli.model_switch.<name>`` keeps resolving (and monkeypatching) as before."""
...
from loki_cli.providers import custom_provider_aliases, custom_provider_slug, get_label
from utils import base_url_host_matches

# Log-record parity with the origin module.
logger = logging.getLogger("loki_cli.model_switch")
...
def _save_discovered_models_to_config(
    ...
    from loki_cli.model_switch import _extra_headers_from_config
...
def _discovered_catalog_stale(entry: dict, model_ids: list[str]) -> bool:
    ...
    A mapping Loki itself discovered (entry flag or legacy in-mapping sentinel) is ours to
...
def _fetch_picker_live_models(
    ...
    from loki_cli.models import _get_ollama_native_headers, cached_fetch_api_models, fetch_api_models
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    from loki_cli.model_switch import list_authenticated_providers
    ...
    def _warm() -> None:
        try:
            from loki_cli.inventory import load_picker_context
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from loki_cli.models import update_provider_cache_entry
...
def _iter_builtin_candidates(models_dev_data: dict, excluded: set, seen: set):
    """Yield ``(loki_id, mdev_id, pconfig, env_vars)`` for section-1 rows.
    ...
    from loki_cli.auth import PROVIDER_REGISTRY, is_runtime_provider_routable
    from loki_cli.models import _AGGREGATOR_PROVIDERS
    from loki_cli.providers import ALIASES
    for loki_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        alias_target = ALIASES.get(loki_id)
        if alias_target and alias_target != loki_id and alias_target in _AGGREGATOR_PROVIDERS:
            continue
        try:
            from providers import get_provider_profile
            prof = get_provider_profile(loki_id)
            if prof is not None and prof.name != loki_id:
                continue
        except Exception:
            pass
        if loki_id.lower() in seen:
            continue
        if loki_id.lower() in excluded or mdev_id.lower() in excluded:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(loki_id)
        if (pconfig and pconfig.auth_type != "api_key") or not is_runtime_provider_routable(loki_id):
            continue
        env_vars = list(pconfig.api_key_env_vars) if pconfig and pconfig.api_key_env_vars else pdata.get("env", [])
        if isinstance(env_vars, list):
            yield loki_id, mdev_id, pconfig, env_vars


def _auth_store_has_provider(*keys: str) -> bool:
    """True when ``auth.json`` has a ``providers`` entry under any of *keys*."""
    try:
        from loki_cli.auth import _load_auth_store
...
def _raw_pool_usable(loki_id: str) -> bool:
    """Section-1 pool check: only consult the pool when auth.json lists a raw entry."""
    try:
        from loki_cli.auth import _load_auth_store
        store = _load_auth_store()
        if store and store.get("credential_pool", {}).get(loki_id):
            return _credential_pool_is_usable(loki_id, raw_pool_present=True)
    except Exception:
        pass
    return False
...
def _overlay_has_env_creds(pid: str, loki_slug: str, overlay, read_env) -> bool:
...
def _live_or_curated_ids(slug: str, curated: dict, *fallback_keys: str, merge_models_dev: bool = True) -> list:
    """``cached_provider_model_ids`` (the SAME disk-cached list ``loki model`` builds), falling
    back to the curated list (merged with models.dev for preferred providers) when live is empty."""
    from loki_cli.models import _MODELS_DEV_PREFERRED, _merge_with_models_dev, cached_provider_model_ids
...
def _wundercorp_picker_model_ids(curated: dict, force_fresh_wundercorp_tier: bool) -> list:
    """WunderCorp serves a huge live catalog; the picker shows ONLY the curated agentic list, augmented
    with the Portal's free/paid recommendations (new models surface without a CLI release) and
    narrowed by org policy. Mirrors ``_model_flow_wundercorp`` so GUI pickers match the CLI. A failed
    recommendation fetch still yields a policy-filtered curated list."""
    model_ids = curated.get("wundercorp", [])
    try:
        ...
        pricing = get_pricing_for_provider("wundercorp", cached_only=True) or {}
        try:
            portal = (get_provider_auth_state("wundercorp") or {}).get("portal_base_url", "") or ""
        except Exception:
            portal = ""
        if check_wundercorp_free_tier(force_fresh=force_fresh_wundercorp_tier):
    ...
    try:
        ...
        model_ids = restrict_to_wundercorp_policy(model_ids, wundercorp_policy_allowed_ids(), rescue_empty=True)
    except Exception:
        pass
    return model_ids


def _free_tier_wundercorp_row(row: dict) -> dict | None:
    """The one free-tier rule for a WunderCorp picker row, shared by every row builder.

    ``row`` carries at least ``name`` and ``models``. A guest identity carrying inference turns
    it into "WunderCorp · free tier" with the single model ``wundercorp/welcome`` (the welcome host serves
    nothing else). A guest that ``wundercorp.guest: false`` has switched off yields ``None``: no WunderCorp
    row at all, since there is nothing selectable. A real account (or no WunderCorp state) passes the
...
def _absorb_entry_models(grp: dict, entry: dict, active_model: Any) -> None:
    ...
    from loki_cli.model_switch import _declared_model_ids, _entry_models_discovered, _models_config_is_allowlist
...
def _display_prefix(name: str) -> str:
    """Text before the per-model separator Loki's own writer uses ("—" / " - ")."""
...
def _discover_endpoint_models(
    ...
    if probe_live:
        ...
        try:
            from loki_cli.models import cached_fetch_api_models
...
def _collect_authed_provider_slugs(
    ...
    (heavier detection)."""
    from loki_cli.model_switch import _scoped_key_env
    from agent.models_dev import PROVIDER_TO_MODELS_DEV
    from loki_cli.auth import PROVIDER_REGISTRY
    from loki_cli.providers import LOKI_OVERLAYS
    from loki_cli.models import CANONICAL_PROVIDERS
    ...
    for loki_id, _mdev_id, _pconfig, env_vars in _iter_builtin_candidates(models_dev_data, excluded_set, seen):
        if _any_env(env_vars, _scoped_key_env) or _raw_pool_usable(loki_id):
            _emit(loki_id, loki_id)

    mdev_to_loki = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in LOKI_OVERLAYS.items():
        loki_slug = mdev_to_loki.get(pid, pid)
        if _skip(seen, excluded_set, pid, loki_slug) or overlay.auth_type == "aws_sdk":
            continue
        if (
            _overlay_has_env_creds(pid, loki_slug, overlay, _scoped_key_env)
            or _auth_store_has_provider(pid, loki_slug) or _pool_usable(loki_slug)):
            _emit(loki_slug, pid, loki_slug)
    ...
    # WunderCorp excluded: its picker branch builds from the curated list and never reads the
    # api_key-only cache entry a prefetch would write.
    return [s for s in slugs if s != "wundercorp"]


@dataclass
class _PickerBuild:
    """State threaded through the ``list_authenticated_providers`` sections: 1 built-ins mapped to
    models.dev, 2 Loki-only overlays, 2b canonical providers missed by 1/2, 3 ``providers:``
    entries + 3b the bare active custom endpoint, 4 ``custom_providers:`` entries. Row-builder
    imports of ``loki_cli.auth/models`` stay lazy so tests can patch those modules."""
    current_provider: str
    current_base_url: str
    current_model: str
    max_models: int | None
    for_picker: bool
    force_fresh_wundercorp_tier: bool
    ...
    def add_builtin_row(
        ...
        if slug == "wundercorp":
            # Free-tier identity: one row "WunderCorp · free tier" / wundercorp/welcome, or no row when
            # wundercorp.guest is off. Still marks the slug seen so a later lap cannot re-emit it.
            row = _free_tier_wundercorp_row(row)
        if row is not None:
...
def _lap_builtin_rows(b: _PickerBuild, data: dict, user_providers: dict) -> None:
    ...
    from loki_cli.model_switch import _declared_model_ids
    from agent.models_dev import get_provider_info
    for loki_id, mdev_id, pconfig, env_vars in _iter_builtin_candidates(data, b.excluded, b.seen_slugs):
        if not (_any_env(env_vars) or _raw_pool_usable(loki_id)):
            continue
        model_ids = _live_or_curated_ids(loki_id, b.curated)
        ...
        configured = user_providers.get(loki_id) if isinstance(user_providers, dict) else None
        ...
        b.add_builtin_row(
            loki_id, display_name, b.current_provider in (loki_id, mdev_id), model_ids, "built-in")


def _overlay_has_creds(b: _PickerBuild, pid: str, loki_slug: str, overlay) -> bool:
    ...
    if overlay.auth_type == "aws_sdk":
        has_creds = _has_aws_sdk_creds_for_listing(loki_slug, b.current_provider)
    else:
        has_creds = _overlay_has_env_creds(pid, loki_slug, overlay, os.environ.get)
    ...
    if not has_creds and overlay.auth_type == "external_process":
        try:
            ...
            _ext_status = get_auth_status(loki_slug) or {}
    ...
    has_creds = has_creds or _auth_store_has_provider(pid, loki_slug)
    if not has_creds:
        # Full auto-seeding pool check catches external stores (Codex CLI ~/.codex/auth.json)
        # not yet in auth.json.
        try:
            if _credential_pool_is_usable(loki_slug):
                has_creds = True
            elif b.for_picker:
                ...
                try:
                    from agent.credential_pool import load_pool
                    has_creds = load_pool(loki_slug).has_credentials()
                except Exception:
                    pass
        except Exception as exc:
            logger.debug("Credential pool check failed for %s: %s", loki_slug, exc)
    if not has_creds and loki_slug == "anthropic":
        ...
        try:
            from agent.anthropic_credentials import read_claude_code_credentials, read_loki_oauth_credentials
            loki_creds = read_loki_oauth_credentials()
            cc_creds = read_claude_code_credentials()
            if (loki_creds and loki_creds.get("accessToken")) or (cc_creds and cc_creds.get("accessToken")):
                has_creds = True
        except Exception as exc:
...
def _lap_overlay_rows(b: _PickerBuild, data: dict) -> None:
    """Section 2: Loki-only providers (wundercorp, openai-codex, copilot, opencode-go, ...)."""
    from agent.models_dev import PROVIDER_TO_MODELS_DEV
    from loki_cli.providers import LOKI_OVERLAYS

    # LOKI_OVERLAYS keys may be models.dev IDs ("github-copilot") while config.yaml uses
    # Loki IDs ("copilot").
    mdev_to_loki = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in LOKI_OVERLAYS.items():
        loki_slug = mdev_to_loki.get(pid, pid)
        if _skip(b.seen_slugs, b.excluded, pid, loki_slug):
            continue
        if not _overlay_has_creds(b, pid, loki_slug, overlay):
            continue
        if loki_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(loki_slug)
        elif overlay.auth_type == "aws_sdk":
            model_ids = _aws_live_or_curated_ids(loki_slug, b.curated, loki_slug, pid)
        elif loki_slug == "wundercorp":
            # A guest identity never needs the Portal catalog: add_builtin_row pins wundercorp/welcome
            # (or drops the row when wundercorp.guest is off), so only a real account fetches.
            tier_row = _free_tier_wundercorp_row({"name": get_label(loki_slug), "models": []})
            real_account = tier_row is not None and not tier_row["models"]
            model_ids = _wundercorp_picker_model_ids(b.curated, b.force_fresh_wundercorp_tier) if real_account else []
        else:
            model_ids = _live_or_curated_ids(loki_slug, b.curated, loki_slug, pid)
        b.add_builtin_row(
            loki_slug, get_label(loki_slug), b.current_provider in (loki_slug, pid), model_ids, "loki")
...
def _lap_user_provider_rows(b: _PickerBuild, user_providers: dict) -> None:
    ...
    from loki_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from loki_cli.config import coerce_provider_id, is_provider_enabled
...
def _lap_custom_provider_rows(b: _PickerBuild, custom_providers: list) -> None:
    ...
    their own rows."""
    from loki_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from loki_cli.config import coerce_provider_id
...
def _build_curated_lists(current_provider: str, current_base_url: str, current_model: str) -> dict[str, list[str]]:
    """Curated model lists keyed by loki provider id, plus the dynamic ones (wundercorp manifest,
    Ollama Cloud, LM Studio live probe)."""
    from loki_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS, get_curated_wundercorp_model_ids
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # Remote manifest so new Portal models surface without a release; in-repo snapshot fallback.
    curated["wundercorp"] = get_curated_wundercorp_model_ids()
...
def list_authenticated_providers(
    ...
    custom_providers: list | None = None, *, force_fresh_wundercorp_tier: bool = False,
    ...
    ("built-in", "loki", "canonical", "user-config", "model-config").
    ``force_fresh_wundercorp_tier`` bypasses the short WunderCorp tier cache (account-sensitive flows only);
    ...
    from loki_cli.config import coerce_provider_id, stringify_provider_map
    ...
    # as (loki_id / mdev_id / canonical slug).
    b = _PickerBuild(
        current_provider=current_provider, current_base_url=current_base_url, current_model=current_model,
        max_models=max_models, for_picker=for_picker, force_fresh_wundercorp_tier=force_fresh_wundercorp_tier,
...
def _finalize_picker_rows(results: list, user_providers, current_model: str) -> list:
    ...
    try:
        from loki_cli.config import is_provider_enabled
    ...
    if current_model:
        for row in results:
            ...
            if current_model not in models:
                from loki_cli.models import _model_requires_account_discovery
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    try:
        from loki_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    OpenRouter's list is replaced with :func:`loki_cli.models.fetch_openrouter_models` (curated
    ...
    from loki_cli.model_switch import list_authenticated_providers
    from loki_cli.models import fetch_openrouter_models
