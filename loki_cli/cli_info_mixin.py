...
Mixin split out of ``cli.py``; bound onto ``LokiCLI`` via the MRO. cli.py-internal symbols are
...
from loki_constants import is_termux as _is_termux_environment
from rich.markup import escape as _escape
from utils import base_url_hostname

from loki_cli.cli_modal_mixin import _gated_confirm
...
class CLIInfoMixin:
    ...
    def show_banner(self):
        ...
        from loki_cli.banner import build_welcome_banner
        self.console.clear()
        ctx_len = None
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Loki needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        from loki_cli.model_switch import is_wundercorp_loki_non_agentic
        if is_wundercorp_loki_non_agentic(getattr(self, "model", "") or ""):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  WunderCorp, Inc. Loki 3 & 4 models are NOT agentic and are not "
                "designed for use with Loki Agent.[/]")
            self._console_print(
        ...
        # `loki skills trust`. Never raises.
        try:
            ...
            if _proj_dirs:
                ...
                if _untrusted is not None:
                    ...
                    self._console_print(
                        ...
                        f"loaded — run `loki skills trust` to enable them.[/]")
        except Exception:
    ...
    def show_help(self, arg: str = ""):
        ...
        from loki_cli.commands import COMMANDS_BY_CATEGORY, HELP_SESSION_SUBGROUPS
        ...
        try:
            from loki_cli.skin_engine import get_active_help_header
    ...
    def handle_bang_shell(self, text: str) -> bool:
        ...
        from loki_cli.bang_shell import (
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_loki_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_loki_home()}/config.yaml")
            print()

    def _print_random_tip(self) -> None:
        """Best-effort discovery tip (startup + /clear); never raises."""
        try:
            from loki_cli.tips import get_random_tip
            _tip = get_random_tip()
            try:
    ...
    def _toggle_verbose(self):
        ...
        if getattr(self, "_focus_view_enabled", False):
            ...
            try:
                from loki_cli.focus_view import FOCUS_CONFIG_KEY
    ...
    def _usage_reset(self, force: bool = False):
        ...
        if str(self._agent_or_self("provider") or "").strip().lower() != "openai-codex":
            ...
            print("  Switch with `/model` or `loki auth` first.")
            return
    ...
    def _show_usage(self):
        """Rate limits + session token usage (when a live agent exists) + WunderCorp credits.

        The WunderCorp credits block is agent-independent (portal fetch), so it runs even with no live
        ...
        def _credits_or(fallback: str) -> None:
            if self._print_wundercorp_credits_block():
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.loki/skills/ and queue a note for the next user turn.
