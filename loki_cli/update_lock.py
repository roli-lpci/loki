"""Cross-process mutual exclusion for in-flight Loki updates.
...
MARKER_NAME = ".loki-update-in-progress"

# Set by an orchestrating updater (Tauri `loki-setup --update`) to its own pid before
# spawning `loki update` as a child stage; the parent holds the marker for its whole run,
...
HANDOFF_PID_ENV = "LOKI_UPDATE_HANDOFF_PID"
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "Loki is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Loki home (never the context-local profile override): the Rust
    updater resolves ``$LOKI_HOME`` or the platform default and the desktop pins that same
    ...
    return get_process_loki_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``loki update`` as a (grand)child, so a live marker
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Loki update is already running (PID {holder.pid}, "
