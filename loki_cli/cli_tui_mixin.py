...
interactive CLI. Mixin bound onto ``LokiCLI`` via the MRO; cli.py-internal symbols are imported
...
from loki_cli.commands_completion import SlashCommandAutoSuggest, SlashCommandCompleter
...
class CLITuiMixin:
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from loki_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from loki_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def _render_scroll_list_panel(self, state, title, hint, labels, *, min_width, max_width, indent):
        ...
        """
        from cli import LokiCLI, _panel_box_width, _wrap_panel_text
        ...
        scroll_offset, visible = LokiCLI._compute_model_picker_viewport(
        ...
        if code := self._sudo_state.get("vault_code"):
            return self._render_sudo_style_panel(
                ...
                [f'{code["site"]} is asking for a one-time code (text message, email or authenticator app).',
                 'Type the code and press Enter; Loki enters it into the page for you.',
    ...
    def _tui_handle_ignored_terminal_sequence(self, event):
        ...
        loki_cli.pt_input_extras registers focus reports (CSI I / CSI O) as Keys.Ignore at the
    ...
    def _tui_handle_ctrl_z(self, event):
        ...
        agent_name = get_active_skin().get_branding("agent_name", "Loki Agent")
    ...
    def _tui_enter_while_busy(self, text: str, images: list, payload) -> None:
        ...
        """
        from cli import CLI_CONFIG, _ACCENT, _DIM, _RST, _cprint, _loki_home
        ...
        if _effective_mode == "queue":
            ...
            if redirected:
                ...
                try:
                    with open(_loki_home / "interrupt_debug.log", "a", encoding="utf-8") as _f:
                        _f.write(
        ...
        try:
            ...
            if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                ...
                mark_seen(_loki_home / "config.yaml", BUSY_INPUT_FLAG)
    ...
    def _tui_enter_overlay(self, event) -> bool:
        ...
        if self._model_picker_state:
            try:
                ...
                from loki_cli.model_switch import resolve_persist_behavior
    ...
    def _tui_collapse_paste(self, text: str, line_count: int, *, fallback: bool) -> str:
        """Save a large paste under ~/.loki/pastes and return the placeholder for the buffer."""
        from cli import _loki_home, datetime, logger
        self._tui_paste_counter += 1
        paste_dir = _loki_home / "pastes"
    ...
    def _tui_handle_prompt_stash(self, event):
        ...
        """
        from loki_cli.prompt_stash import ACTION_RESTORED, ACTION_STASHED, resolve_ctrl_s
        buf = event.app.current_buffer
    ...
    def _tui_init_run_state(self):
        ...
        from loki_cli.config import get_config_path as _get_config_path
    ...
    def _tui_build_key_bindings(self):
        ...
        from loki_cli.cli_subagent_monitor import modal_prompt_active, open_monitor, toggle_dock
        for key in ('c-t', 'f6'):
    ...
    def _tui_build_layout(self, kb):
        ...
        cli_ref = self
        from loki_cli.cli_subagent_monitor import install_dock
        install_dock(self)
    ...
    def _tui_set_base_style(self):
        ...
        self._tui_style_base = {
            ...
            'hint': '#888888 italic',
            'status-bar': 'bg:#071A0F #CBD5E1',
            'status-bar-strong': 'bg:#071A0F #4ADE80 bold',
            'status-bar-dim': 'bg:#071A0F #64748B',
            'status-bar-good': 'bg:#071A0F #4ADE80 bold',
            'status-bar-warn': 'bg:#071A0F #FBBF24 bold',
            'status-bar-bad': 'bg:#071A0F #FB923C bold',
            'status-bar-critical': 'bg:#071A0F #F87171 bold',
            'status-bar-yolo': 'bg:#071A0F #EF4444 bold',
            'status-bar-session-title': 'bg:#4ADE80 #071A0F bold',
            'input-rule': '#16A34A',
            'image-badge': '#87CEEB bold',
            'completion-menu': 'bg:#071A0F #ECFDF5',
            'completion-menu.completion': 'bg:#071A0F #ECFDF5',
            'completion-menu.completion.current': 'bg:#052E16 #4ADE80',
            'completion-menu.meta.completion': 'bg:#071A0F #64748B',
            'completion-menu.meta.completion.current': 'bg:#052E16 #34D399',
            'clarify-border': '#16A34A',
            'clarify-title': '#4ADE80 bold',
            'clarify-question': '#ECFDF5 bold',
            'clarify-choice': '#AAAAAA',
            'clarify-selected': '#4ADE80 bold',
            'clarify-active-other': '#4ADE80 italic',
            'clarify-answer': '#98FB98',
            'clarify-countdown': '#16A34A',
            'sudo-prompt': '#FF6B6B bold',
            'sudo-border': '#16A34A',
            'sudo-title': '#FF6B6B bold',
            'sudo-text': '#ECFDF5',
            'approval-border': '#16A34A',
            'approval-title': '#FF8C00 bold',
            'approval-desc': '#ECFDF5 bold',
            'approval-cmd': '#AAAAAA italic',
            'approval-choice': '#AAAAAA',
            'approval-selected': '#4ADE80 bold',
            'voice-prompt': '#87CEEB',
            'voice-recording': '#FF4444 bold',
            'voice-processing': '#34D399 italic',
            'voice-status': 'bg:#071A0F #34D399',
            'voice-status-recording': 'bg:#071A0F #EF4444 bold'}
