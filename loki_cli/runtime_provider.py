...
Custom-provider lookup lives in :mod:`loki_cli.runtime_provider_custom`; Azure Foundry,
OpenRouter/bare-custom, Bedrock and external-process builders in
:mod:`loki_cli.runtime_provider_backends` — both re-exported here so
``loki_cli.runtime_provider.<name>`` imports and test patches keep working."""
...
from loki_cli.auth import (  # resolve_external_process_provider_credentials is read via origin by runtime_provider_backends
    ACTUAL_LOCAL_NOAUTH_PLACEHOLDER, AuthError, DEFAULT_CODEX_BASE_URL, DEFAULT_QWEN_BASE_URL, DEFAULT_XAI_OAUTH_BASE_URL,
    PROVIDER_REGISTRY, _agent_key_is_usable, _wundercorp_inference_env_override, format_auth_error, resolve_provider,
    resolve_wundercorp_runtime_credentials, resolve_codex_runtime_credentials, resolve_xai_oauth_runtime_credentials,
...
from loki_cli import config as _config_mod
from loki_cli import models as _models  # attribute access keeps ``loki_cli.models.<name>`` patches effective
from loki_constants import OPENROUTER_BASE_URL
from loki_cli.providers import determine_api_mode, is_actual_route, is_official_openai_host, wundercorp_api_mode
from utils import base_url_host_matches, base_url_hostname, env_int


# Late-bound delegates, deliberately NOT module-level from-imports: this module is often imported
# lazily, so its first import can happen while a test has ``loki_cli.config.load_config`` patched
...
# Hosts that only speak one wire protocol. Mirrors host_mandated_api_mode in loki_cli/providers.py
...
def _api_key_provider_api_mode(provider: str, model_cfg: Dict[str, Any], api_key: str, base_url: str, effective_model: Any, *,
    ...
    if provider == "xai":
        ...
        # the host_mandated_api_mode clause in loki_cli/providers.py so the runtime resolver stays in
...
def _wundercorp_entry_key_usable(entry: Any, min_ttl: int) -> bool:
    return _agent_key_is_usable({k: getattr(entry, k, None) for k in ("agent_key", "agent_key_expires_at", "scope")}, min_ttl)


def _wundercorp_min_key_ttl() -> int:
    return max(60, env_int("LOKI_WUNDERCORP_MIN_KEY_TTL_SECONDS", 1800))


def _resolve_wundercorp_creds() -> Dict[str, Any]:
    return resolve_wundercorp_runtime_credentials(timeout_seconds=float(_getenv("LOKI_WUNDERCORP_TIMEOUT_SECONDS", "15")))
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    """Provider request from explicit arg, then config, then ``LOKI_INFERENCE_PROVIDER``, else
    ...
    return _getenv("LOKI_INFERENCE_PROVIDER", "").strip().lower() or "auto"


# ── extracted collaborators (re-exported; see module docstring) ────────────────────────────

from loki_cli.runtime_provider_custom import (  # noqa: E402,F401
...
from loki_cli.runtime_provider_backends import (  # noqa: E402,F401
...
def _pool_entry_mode_and_url(provider, entry, model_cfg, effective_model, base_url) -> tuple:
    ...
    if provider == "wundercorp":
        return wundercorp_api_mode(effective_model), (_wundercorp_inference_env_override() or "") or base_url
    if provider == "copilot":
...
def _refresh_wundercorp_pool_entry(pool: CredentialPool, entry: Any, pool_api_key: str):
    """WunderCorp pool entries carry the agent_key (an invoke JWT) which the pool does not refresh on
    selection (avoids network calls in `loki auth list`); refresh here before falling back to
    singleton auth resolution. Returns (entry, pool_api_key) — key "" when still unusable."""
    min_ttl = _wundercorp_min_key_ttl()
    if _wundercorp_entry_key_usable(entry, min_ttl):
        return entry, pool_api_key
    logger.debug("WunderCorp pool entry agent_key expired/missing, refreshing selected pool entry")
    try:
        refreshed = pool.try_refresh_current()
    except Exception as exc:
        logger.debug("WunderCorp pool entry refresh failed: %s", exc)
        refreshed = None
    if refreshed is not None:
        entry, pool_api_key = refreshed, _pool_entry_api_key(refreshed)
    if not pool_api_key or not _wundercorp_entry_key_usable(entry, min_ttl):
        logger.debug("WunderCorp pool entry agent_key still unavailable, falling through to runtime resolution")
        pool_api_key = ""
    return entry, pool_api_key
...
def _resolve_from_pool(provider: str, requested_provider: str, model_cfg: Dict[str, Any], explicit_api_key, explicit_base_url,
    ...
    if provider == "wundercorp":
        entry, pool_api_key = _refresh_wundercorp_pool_entry(pool, entry, pool_api_key)
...
def _explicit_wundercorp(requested_provider, model_cfg, api_key, explicit_base_url, target_model):
    state = auth_mod.get_provider_auth_state("wundercorp") or {}
    base_url = (explicit_base_url or _wundercorp_inference_env_override()
                or str(state.get("inference_base_url") or auth_mod.DEFAULT_WUNDERCORP_INFERENCE_URL).strip().rstrip("/"))
    # The agent_key compatibility field is used for inference only when it holds a NAS invoke JWT;
    # raw OAuth access_token fallback is handled by resolve_wundercorp_runtime_credentials().
    api_key = api_key or (str(state.get("agent_key") or "").strip() if _agent_key_is_usable(state, _wundercorp_min_key_ttl()) else "")
    api_key, base_url, expires_at = _creds_fallback(api_key, explicit_base_url, base_url,
                                                    ...
                                                    _resolve_wundercorp_creds)
    return _runtime("wundercorp", wundercorp_api_mode(_effective_model(model_cfg, target_model)), base_url, api_key, source="explicit",
...
_EXPLICIT_RESOLVERS: Dict[str, Callable[..., Dict[str, Any]]] = {
    "anthropic": _explicit_anthropic, "openai-codex": _explicit_codex, "wundercorp": _explicit_wundercorp,
...
_OAUTH_RUNTIME_PROVIDERS: Dict[str, _OAuthRuntimeSpec] = {
    "wundercorp": _OAuthRuntimeSpec(_resolve_wundercorp_creds, wundercorp_api_mode, "portal", "expires_at",
                              "Auto-detected WunderCorp provider but credentials failed"),
    "openai-codex": _OAuthRuntimeSpec(lambda: resolve_codex_runtime_credentials(), "codex_responses", "loki-auth-store",
                                      "last_refresh", "Auto-detected Codex provider but credentials failed"),
    "xai-oauth": _OAuthRuntimeSpec(lambda: resolve_xai_oauth_runtime_credentials(), "codex_responses", "loki-auth-store",
...
def _resolve_vertex_runtime(requested_provider: str) -> Dict[str, Any]:
    ...
    if not token or not base_url:
        raise AuthError("Vertex AI credentials could not be resolved. Vertex uses OAuth2 (not a static API key): provide a "
                        "service-account JSON via GOOGLE_APPLICATION_CREDENTIALS (or VERTEX_CREDENTIALS_PATH) in ~/.loki/.env, "
                        "or run 'gcloud auth application-default login' for ADC. Set the GCP project/region under vertex: in "
                        "config.yaml if they aren't embedded in the credentials. Run `loki setup` to install Vertex support.")
...
def resolve_runtime_provider(*, requested: Optional[str] = None, explicit_api_key: Optional[str] = None,
    ...
    rung returns or raises, else falls to the next):
      ...
      7. OAuth specs (wundercorp/codex/xai/qwen; "auto" swallows AuthError and logs) → minimax-oauth
...
_PLUGIN_COMPAT_LAZY = {
    'custom_provider_aliases': ('loki_cli.providers', 'custom_provider_aliases'),
