...
resolved through the module object so test patches on ``loki_cli.setup.<name>`` take effect."""
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'loki setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'loki setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'loki tools' → Speech-to-Text")
...
_EDIT_CONFIG_ROWS = (
    ("loki config", "         View current settings"), ("loki config edit", "    Open config in your editor"),
    ("loki config set <key> <value>", ""))
_READY_ROWS = (
    ("loki", "              Start chatting"), ("loki gateway", "      Start messaging gateway"),
    ("loki doctor", "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'loki setup' to configure")


def _managed_or_provider_row(feature, name: str, managed_label: str, missing_hint: str):
    """Row for a WunderCorp-manageable feature: managed > available (with provider) > missing hint."""
    if feature.managed_by_wundercorp:
...
def _web_row(config, feats):
    ...
    return _managed_or_provider_row(feats.web, "Web Search & Extract", "WunderCorp subscription", _WEB_MISSING)
...
def _browser_row(config, feats):
    ...
    return _managed_or_provider_row(feats.browser, "Browser Automation", "WunderCorp Browser Use", hint)


def _image_gen_row(config, feats):
    # FAL (direct or via WunderCorp), or any plugin-registered provider (OpenAI, etc.)
    if feats.image_gen.managed_by_wundercorp:
        return ("Image Generation (WunderCorp subscription)", True, None)
...
def _video_gen_row(config, feats):
    # Opt-in via `loki tools` → Video Generation. Only show the row when a plugin reports
    # available so we don't badger users who don't care about video gen with a "missing" line.
    if feats.video_gen.managed_by_wundercorp:
        return ("Video Generation (FAL via WunderCorp subscription)", True, None)
...
def _tts_row(config, feats):
    ...
    if feats.tts.managed_by_wundercorp:
        return ("Text-to-Speech (OpenAI via WunderCorp subscription)", True, None)
...
def _stt_row(config, feats):
    ...
    if stt_feature is not None and stt_feature.managed_by_wundercorp:
        return ("Speech-to-Text (OpenAI via WunderCorp subscription)", True, None)
...
def _modal_row(config, feats):
    if feats.modal.managed_by_wundercorp:
        return ("Modal Execution (WunderCorp subscription)", True, None)
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'loki setup terminal'")
    if tool_backend_helpers.managed_wundercorp_tools_enabled() and feats.wundercorp_auth_present:
        return ("Modal Execution (optional via WunderCorp subscription)", True, None)
    return None
...
def _spotify_row(config, feats):
    # OAuth via loki auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, loki_home):
    ...
    try:
        from loki_cli.auth import resolve_provider
        resolve_provider()
    except Exception:
        print()
    ...
    tool_status = []
    subscription_features = wundercorp_subscription.get_wundercorp_subscription_features(config)
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'loki setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {loki_home}/cron/, sessions/, logs/")):
...
import loki_cli.setup as _setup  # noqa: E402  (bottom: loki_cli.setup imports this module)
