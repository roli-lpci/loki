"""Configuration management for Loki Agent: config.yaml / .env loading, saving,
validation, migration, and the ``loki config`` command."""
...
from loki_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
from loki_cli.secret_prompt import masked_secret_prompt
# Re-export from loki_constants — canonical definition lives there.
from loki_constants import get_loki_home, get_process_loki_home  # noqa: F401
...
_PARSE_FAILURE_FALLBACK_MSG = {
    ...
    "refuse-write": (
        ...
        "Fix the YAML (loki config edit) and retry.")}
...
def _warn_config_parse_failure(
    ...
    from loki_cli.config_backups import backup_config
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  loki config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
# interpreter init (PYTHON*, NODE_*: Loki restarts through them), PATH (fix tool lookup
...
# and Loki runtime-location / security-policy flags (config.yaml is the supported surface).
#
# ``LOKI_*`` overall is NOT blocked — many integration credentials use that prefix
# (LOKI_LANGFUSE_PUBLIC_KEY, LOKI_SPOTIFY_CLIENT_ID, ...). The denylist is name-by-name so
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "PYTHONEXECUTABLE", "PYTHONWUNDERCORPERSITE", "NODE_OPTIONS", "NODE_PATH",
    # General / git
    "PATH", "SHELL", "BROWSER", "EDITOR", "VISUAL", "PAGER",
    "GIT_SSH_COMMAND", "GIT_EXEC_PATH", "GIT_SHELL",
    # Loki runtime location
    "LOKI_HOME", "LOKI_PROFILE", "LOKI_CONFIG", "LOKI_ENV",
    "LOKI_CONFIG_PATH", "LOKI_ENV_PATH",
    ...
    "LOKI_COPILOT_ACP_COMMAND", "LOKI_COPILOT_ACP_ARGS",
    # Security policy / approval-routing context — set via their dedicated controls only.
    "LOKI_YOLO_MODE", "LOKI_ACCEPT_HOOKS", "LOKI_REDACT_SECRETS",
    "LOKI_INTERACTIVE", "LOKI_EXEC_ASK", "LOKI_GATEWAY_SESSION",
    "LOKI_CRON_SESSION", "LOKI_SINGLE_QUERY_SESSION",
    "LOKI_SESSION_KEY", "LOKI_SESSION_PLATFORM"})
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "or Loki runtime location and security policy (LOKI_HOME, LOKI_YOLO_MODE, ...) "
            "cannot be persisted via the env writer. If you really need this, edit ~/.loki/.env "
            "directly.")
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # back-compat fallback. The boolean LOKI_TOOL_PROGRESS variant is unsupported (its only
    # consumer, the v3->4 migration, is below the v12 support floor); doctor flags it as ignored.
    "LOKI_TOOL_PROGRESS_MODE",
    ...
    "LOKI_LANGFUSE_ENV", "LOKI_LANGFUSE_RELEASE", "LOKI_LANGFUSE_SAMPLE_RATE",
    "LOKI_LANGFUSE_MAX_CHARS", "LOKI_LANGFUSE_CAPTURE", "LOKI_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "LOKI_ACP_AUTH_METHOD", "LOKI_ACP_AUTO_APPROVE", "LOKI_COPILOT_ACP_COMMAND",
    "LOKI_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
# LOKI_MANAGED). Module-level so tests can patch it without touching /nix/store.
...
def get_managed_system() -> Optional[str]:
    ...
    Signals: LOKI_MANAGED env var (systemd service) or a ``.managed`` marker file in
    LOKI_HOME (NixOS activation script — interactive shells don't see the service env)."""
    marker = os.getenv("LOKI_MANAGED", "").strip().lower() or None
    managed_marker = get_loki_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Loki is running in package-manager-managed mode."""
...
_NIX_UPDATE_MSG = (
    "Update Loki through the Nix source that installed it "
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Loki was installed: apt/docker/nix/nixos/home-manager/git/unknown.
    Order: code-scoped ``<install tree>/.install_method`` stamp (authoritative) -> legacy
    ``$LOKI_HOME/.install_method`` -> managed marker -> /nix/store path -> .git dir -> unknown.
    The stamp lives next to the code because LOKI_HOME is shared data: a container and a host
    install can bind-mount the same home, so a home-scoped ``docker`` stamp would make the host
    ``loki update`` refuse to run. A legacy ``docker`` value is therefore ignored unless we are
    ...
    the code; - the published ``wundercorp/loki-agent`` image bakes a ``docker`` stamp into
    ``/opt/loki`` at build time. An unsupported manual install dropped into a container (no stamp) falls
    through to the ``.git`` checks and behaves like any off-path install. See issue #34397.
    """
    # The stamp is a property of the running code tree (parent of loki_cli/), NOT of $LOKI_HOME,
    ...
    method = _install_method_stamp(get_loki_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``loki_constants.is_container``."""
    try:
...
_UPDATE_COMMAND_BY_METHOD = {
    "docker": "docker pull wundercorp/loki-agent:latest",
    "apt": "pkg upgrade loki-agent",  # "apt" == Termux APT by contract; uses Termux's `pkg`.
}
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    return _UPDATE_COMMAND_BY_METHOD.get(method, "loki update")
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (loki_cli/main.py) so the wording never
# forks. The published image excludes ``.git``, so the git update path can never succeed there
# and the generic "reinstall via install.sh" fallback would install a NEW host-side Loki.
_DOCKER_UPDATE_MESSAGE = """\
✗ ``loki update`` doesn't apply inside the Docker container.

Loki Agent runs as a published image (wundercorp/loki-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull wundercorp/loki-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate loki-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm wundercorp/loki-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/wundercorp/loki-agent/tags
  • Your config and session history live under ``$LOKI_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``loki update`` inside Docker."""
    return _DOCKER_UPDATE_MESSAGE


def format_managed_message(action: str = "modify this Loki installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Loki installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Loki.")
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from LOKI_HOME/.container-mode.
    ...
    already inside the container, or when LOKI_DEV=1 is set. Only FileNotFoundError is
    swallowed; other errors (permissions, malformed data) propagate."""
    if os.environ.get("LOKI_DEV") == "1":
        return None
    ...
    try:
        info = {}
        with open(get_loki_home() / ".container-mode", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
    ...
    return {
        ...
        "container_name": info.get("container_name", "loki-agent"),
        "exec_user": info.get("exec_user", "loki"),
        "loki_bin": info.get("loki_bin", "/data/current-package/bin/loki")}


# ---- Config paths / LOKI_HOME skeleton ----

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_loki_home() / "config.yaml"
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    states; ``--ignore-user-config`` / LOKI_IGNORE_USER_CONFIG=1 remain authoritative."""
    if ignore_user_config or os.environ.get("LOKI_IGNORE_USER_CONFIG") == "1":
        return
...
def _resolve_loki_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read LOKI_UID / LOKI_GID (set by Docker deployments); (None, None) if unset/invalid/Windows.
    The entrypoint chowns LOKI_HOME once, but subdirs created at runtime (``profiles/<name>/``)
    need the same chown or they land root:root and block later uid-mapped workers.

    Docker containers running Loki commonly set these to map the in-container user to a host user so
    ...
    return _env_int("LOKI_UID"), _env_int("LOKI_GID")


def _chown_to_loki_uid(path) -> None:
    """Chown ``path`` to ``LOKI_UID:LOKI_GID`` when set; EPERM/ENOENT are non-fatal (the
    ...
    :func:`ensure_loki_home` on Docker deployments. See #34107.
    """
    uid, gid = _resolve_loki_uid_gid()
...
def _secure_dir(path):
    """chmod a directory owner-only (0700) and apply LOKI_UID/GID ownership. No-op when managed.
    LOKI_HOME_MODE (e.g. 0701) overrides the mode so a web server can traverse LOKI_HOME to
    a served subdirectory without directory listings.

    Also applies ``LOKI_UID``/``LOKI_GID``-based ownership when those env vars are set (#34107 — Docker
    ...
    if is_managed():
        return
    try:
        mode = int(os.environ.get("LOKI_HOME_MODE", "").strip() or "700", 8)
    except ValueError:
        mode = 0o700
    try:
        os.chmod(path, mode)
        ...
        pass
    _chown_to_loki_uid(path)


def _is_container() -> bool:
    """Detect Docker/Podman/LXC (or LOKI_CONTAINER / LOKI_SKIP_CHMOD opt-out).
    ...
    if (os.environ.get("LOKI_CONTAINER") or os.environ.get("LOKI_SKIP_CHMOD")
...
_LOKI_HOME_ENSURED: set = set()
_LOKI_HOME_SUBDIRS = (
    "cron", "sessions", "logs", "logs/curator", "memories",
    "pairing", "hooks", "image_cache", "audio_cache", "skills")


def ensure_loki_home():
    """Ensure the ~/.loki directory skeleton exists with secure permissions.
    ...
    from loki_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _LOKI_HOME_ENSURED and home.is_dir():
        return
    from loki_cli.config_home import initialize_home
    initialize_home(home, _LOKI_HOME_SUBDIRS, _LOKI_HOME_ENSURED)


# ---- Config loading/saving ----

from loki_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from loki_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use loki_cli.config.<name>)
...
# Back-compat re-exports — :mod:`loki_cli.personality` owns personality/overlay semantics.
from loki_cli.personality import (  # noqa: E402,F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``loki config set`` uses ``.`` as the nesting separator, so a key that itself contains a literal dot
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `loki config set` routes this key to .env."""
    if "." in key:
        return False
    key_upper = key.upper()
    return (
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # A malformed SKILL.md must never break `loki update`; this prompting is a nicety.
    ...
    for _key in ("backend", "search_backend", "extract_backend"):
        ...
        if note:
            _issue(issues, "warning",
                   ...
                   "Run 'loki tools' and pick a different Web Search & Extract provider")
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            from loki_cli.config_home import config_load_issue
    ...
    if cp and not config.get("model"):
        _issue(issues, "warning",
               "custom_providers defined but no 'model' section — Loki won't know which provider to use",
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'loki doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars() -> None:
    ...
    if lines:
        from loki_constants import display_loki_home

        hint_path = display_loki_home()
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from loki_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ loki config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
...
def _disable_suspicious_mcp_servers(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if not isinstance(raw_mcp_servers, dict):
        return
    try:
        from loki_cli.mcp_security import validate_mcp_server_entry
    except Exception:
        return
    mcp_touched = False
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from loki_cli.toolset_validation import validate_platform_toolsets
        from loki_cli.toolset_scope import toolset_allowed_for_platform
...
def _offer_list(heading: str, items: List[str], question: str) -> bool:
    ...
    if not _ask_yes_no(question):
        print("  Set later with: loki config set <key> <value>")
        return False
    print()
    return True
...
def _env_expand_match(m: re.Match) -> str:
    ...
    if inner.startswith("env:"):
        logger.warning(
            "Config ref %r: %s is not set (check ~/.loki/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    made against a different environment (load before ``load_loki_dotenv()``, in-process
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    LiteLLM users reach for, and ``loki config set`` blindly accepts any dotted key — so
    ...
    from OpenAI-compatible backends) — while display paths (``loki status``/``dump``) read ``name`` and
...
# the LOKI_MAX_ITERATIONS env bridge, works in every ``<``/``>=``/``max - used`` comparison in
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # load_loki_dotenv() pins unexpanded literals for the process lifetime.
            # Without this, a load_config() that ran before load_loki_dotenv() pins unexpanded literals
...
#   openai-codex (OAuth — loki auth) — OpenAI Codex
#   wundercorp         (OAuth — loki auth) — WunderCorp Portal
...
def save_config(
    ...
    """Save configuration to ~/.loki/config.yaml.
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Loki writes itself (bare, 'single', or "double" with
    ``\\"`` / ``\\\\`` escapes)."""
...
def load_env() -> Dict[str, str]:
    """Load ~/.loki/.env as a dict (memoised; ``get_env_value()`` runs hundreds of times per
...
def sanitize_env_file() -> int:
    """Rewrite ~/.loki/.env with normalized line formatting; returns the number of changed lines."""
    env_path = get_env_path()
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.loki/.env (also matching ``export KEY=`` lines, so a save
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    so two endpoints on one host get separate slots. The fixed ``LOKI_CUSTOM_`` prefix keeps the
    ...
    return f"LOKI_CUSTOM_{slug}_API_KEY" if slug else "LOKI_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.loki/.env and os.environ; True if it was found and removed."""
...
def save_env_value_secure(key: str, value: str) -> Dict[str, Any]:
    ...
    from loki_cli.credential_lifecycle import save_provider_env_credential
...
def reload_env() -> int:
    """Re-read ~/.loki/.env into os.environ; returns count of vars changed.
    Removes deleted vars only when known to Loki (OPTIONAL_ENV_VARS and _EXTRA_ENV_KEYS) so
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` (scope-aware) or ``~/.loki/.env``.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a Loki-managed credential preferring ``~/.loki/.env`` over ``os.environ``, so a
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    # Read the .env FILE directly so a stale LOKI_MAX_ITERATIONS ghost is caught even when the
    # gateway bridge already overrode os.environ.
    try:
        env_ghost = load_env().get("LOKI_MAX_ITERATIONS")
    except Exception:
        env_ghost = None
    if env_ghost is not None and str(env_ghost).strip() != str(cfg_max_turns).strip():
        print(color(f"                ⚠ .env has stale LOKI_MAX_ITERATIONS={env_ghost} "
                    f"(run 'loki doctor --fix' to remove)", Colors.YELLOW))
...
def _show_display_section(config: Dict[str, Any]) -> None:
    ...
    try:
        from loki_cli.personality import active_personality_name
...
def show_config():
    ...
    print(color("│              𖤍 Loki Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from loki_cli.auth import get_anthropic_key
    ...
    print(color("  loki config edit     # Edit config file", Colors.DIM))
    print(color("  loki config set <key> <value>", Colors.DIM))
    print(color("  loki setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    Mirrors the scheduler's precedence: a truthy configured model wins over ``LOKI_MODEL``; the
    ...
    return provider, configured_model or _model_assignment_text(env.get("LOKI_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "`loki cron edit <job_id> --provider <provider> --model <model>` or set a fleet default "
        "with `loki config set cron.model <model>`.")
...
def _coerce_config_set_value(key: str, value: str) -> Any:
    """Auto-coerce a ``loki config set`` string to bool/None/int/float/list/dict.
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    Before #71047 a write such as ``loki config set platforms.telegram.streaming false`` landed on a key
...
def _guard_section_overwrite(key: str, value: Any, user_config: Dict[str, Any], force: bool) -> str:
    ...
    err += [
        ...
        f"    loki config set {key}.<sub-key> <value>",
        "  Or use --force to replace the entire section:",
        f"    loki config set --force {key} {value!r}"]
...
def _touch_skin_file(key: str, value: Any) -> None:
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_loki_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
            pass
...
def _print_unknown_key_notice(key: str, suggestion: Optional[str]) -> None:
    print(color(
        ...
        "but Loki may not read it.", Colors.YELLOW))
    if suggestion:
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if key.strip().lower() in ("model.api_base", "api_base"):
        # Normalize the api_base → base_url alias at set-time too (issue #8919), so a fresh `loki config
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # See #51071.
        from loki_cli.credential_lifecycle import remove_provider_env_credential
...
_USAGE_GET = ("Usage: loki config get <key> [--json]", [
    "loki config get model", "loki config get terminal.backend",
    "loki config get skills.config --json"], None)
_USAGE_SET = ("Usage: loki config set [--force] <key> <value>", [
    "loki config set model anthropic/claude-sonnet-4", "loki config set terminal.backend docker",
    "loki config set OPENROUTER_API_KEY sk-or-..."], [
    "", "  --force: skip the unknown-key notice for unrecognized keys,",
    "           and allow a scalar to replace a whole mapping section"])
_USAGE_UNSET = ("Usage: loki config unset <key>", [
    "loki config unset model", "loki config unset terminal.backend",
    "loki config unset OPENROUTER_API_KEY"], None)
...
def _cmd_config_check(args):
    ...
    if missing_config:
        ...
        print("    Run 'loki config migrate' to add them")
...
_CONFIG_USAGE = """Available commands:
  loki config           Show current configuration
  loki config edit      Open config in editor
  loki config get <key>          Print a resolved config value
  loki config set <key> <value>   Set a config value
  loki config unset <key>        Remove a config value
  loki config check     Check for missing/outdated config
  loki config migrate   Update config with new options
  loki config path      Show config file path
  loki config env-path  Show .env file path"""
...
def _inject_platform_plugin_env_vars() -> None:
    ...
    Chat etc. are configurable in ``loki config`` UI without the core knowing they exist.
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``loki_cli/`` — i.e. the git checkout for source
    installs, ``/opt/loki`` inside the published image. It is a property of
    the running interpreter, NOT of ``$LOKI_HOME``, which is why a
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$LOKI_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/loki`` in the published image, which instead bakes the stamp at
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('loki_cli.route_identity', 'normalize_route_base_url'),
}
