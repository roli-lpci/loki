...
Split out of ``loki_cli.models``. OpenRouter and
WunderCorp Portal share one implementation parametrized by :class:`_CapsSource`; the per-source module
globals (``_openrouter_reasoning_caps_cache``, ``_wundercorp_caps_disk_checked``, ...) stay defined on
``loki_cli.models`` — tests reset them there — and are read/written by attribute name.
...
logger = logging.getLogger("loki_cli.models")

Caps = dict[str, Optional[dict[str, Any]]]


def _origin():
    from loki_cli import models
    return models
...
# that must never block on HTTP — so without a disk copy, `loki -p`, a cron job, or a freshly
...
def _reasoning_caps_disk_path() -> Path:
    ...
    return get_loki_home() / "cache" / "reasoning_caps.json"


def _read_reasoning_caps_disk() -> dict[str, Any]:
    from loki_cli.models import _read_json_cache
...
def _save_reasoning_caps_disk(url: str, caps: Caps) -> None:
    ...
    from loki_cli.models import _write_json_cache
    try:
...
def _fetch_reasoning_caps_catalog(url: str, timeout: float) -> Optional[Caps]:
    ...
    m = _origin()
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": m._LOKI_USER_AGENT})
...
# ── Per-source cache (OpenRouter, WunderCorp Portal) ─────────────────────────

@dataclass(frozen=True)
class _CapsSource:
    """One catalog's cache slots on ``loki_cli.models`` plus how to name its URL.
...
def refresh_reasoning_caps_async(provider: Optional[str]) -> None:
    ...
    """
    src = {"wundercorp": _WUNDERCORP_CAPS, "wundercorp-portal": _WUNDERCORP_CAPS, "wundercorp": _WUNDERCORP_CAPS,
...
# WunderCorp Portal serves OpenRouter's catalog schema, so the same parser and contract apply. Its own
# cache because the two catalogs list different models (and different capabilities for shared ids).
_WUNDERCORP_CAPS = _CapsSource(
    "_wundercorp_reasoning_caps_cache", "_wundercorp_reasoning_caps_failed_at",
    "_wundercorp_caps_disk_checked", "_wundercorp_caps_warm_started",
    lambda: wundercorp_catalog_url(),
)


def wundercorp_catalog_url() -> str:
    """The Portal ``/v1/models`` URL for the endpoint we actually talk to (``WUNDERCORP_INFERENCE_BASE_URL``
    ...
    return f"{_resolve_wundercorp_pricing_credentials()[1]}/v1/models"
...
def wundercorp_model_reasoning_capabilities(
    model_id: Optional[str], *, timeout: float = 6.0, allow_fetch: bool = False,
) -> Optional[dict[str, Any]]:
    """WunderCorp Portal counterpart of :func:`openrouter_model_reasoning_capabilities`; warm the cache
    with :func:`warm_wundercorp_reasoning_caps_async` from hot paths."""
    return _model_caps(_WUNDERCORP_CAPS, model_id, timeout=timeout, allow_fetch=allow_fetch)
...
def warm_wundercorp_reasoning_caps_async() -> None:
    """WunderCorp Portal counterpart of :func:`warm_openrouter_reasoning_caps_async`."""
    _warm_caps_async(_WUNDERCORP_CAPS)
