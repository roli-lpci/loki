...
The provider list shown by ``loki model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`loki_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `loki model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``loki model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("loki_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("loki_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    LOKI_OVERLAYS = _safe_import("loki_cli.providers", "LOKI_OVERLAYS", {})
    try:
