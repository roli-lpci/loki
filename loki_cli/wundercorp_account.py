"""Normalized WunderCorp Portal account entitlement helpers."""
...
WunderCorpAccountInfoSource = Literal["jwt", "account_api", "inference_key", "none", "error"]
...
_account_info_cache: tuple[str, float, "WunderCorpPortalAccountInfo"] | None = None
_ACCOUNT_INFO_CACHE_LOCK = threading.Lock()


@dataclass(frozen=True)
class WunderCorpPortalSubscriptionInfo:
...
@dataclass(frozen=True)
class WunderCorpPaidServiceAccessInfo:
...
@dataclass(frozen=True)
class WunderCorpToolAccessInfo:
...
FREE_TIER_NEEDS_ACCOUNT = "This needs a WunderCorp account. Run `loki auth upgrade`."
FREE_TIER_NEEDS_ACCOUNT_CHAT = "This needs a WunderCorp account. Use /login to sign in."


def _is_anonymous_tier(account_info: Optional["WunderCorpPortalAccountInfo"]) -> bool:
    return account_info is not None and account_info.account_tier == _ANON_ACCOUNT_TIER


@dataclass(frozen=True)
class WunderCorpPortalAccountInfo:
    logged_in: bool
    source: WunderCorpAccountInfoSource
    fresh: bool
    ...
    wundercorp_client: Optional[str] = None
    ...
    subscription: Optional[WunderCorpPortalSubscriptionInfo] = None
    paid_service_access: Optional[bool] = None
    paid_service_access_info: Optional[WunderCorpPaidServiceAccessInfo] = None
    tool_access: Optional[WunderCorpToolAccessInfo] = None
    raw_claims: Optional[dict[str, Any]] = None
    raw_account: Optional[dict[str, Any]] = None
    error: Optional[str] = None
    # NAS account tier claim; ``"anonymous"`` is the free tier (no WunderCorp account behind it).
    ...
    def is_anonymous_tier(self) -> bool:
        """The free tier: no WunderCorp account, so no billing, credits, or entitlement to speak of."""
...
def wundercorp_portal_billing_url(account_info: Optional[WunderCorpPortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized WunderCorp account snapshot."""
    try:
        from loki_cli.auth import DEFAULT_WUNDERCORP_PORTAL_URL
    except Exception:
        DEFAULT_WUNDERCORP_PORTAL_URL = "https://portal.wundercorp.com"

    base = account_info.portal_base_url if account_info is not None else None
    if not _nonblank(base):
        base = DEFAULT_WUNDERCORP_PORTAL_URL
    return f"{base.rstrip('/')}/billing"


def wundercorp_portal_topup_url(account_info: Optional[WunderCorpPortalAccountInfo] = None) -> str:
    ...
    ``/orgs/None/billing``.
    """
    base = wundercorp_portal_billing_url(account_info)[: -len("/billing")]
...
def format_wundercorp_portal_entitlement_message(
    account_info: Optional[WunderCorpPortalAccountInfo], *, capability: str = "this feature",
    include_refresh_hint: bool = True, coverage_category: Optional[str] = None,
    in_chat: bool = False,
) -> Optional[str]:
    """User-facing guidance for a missing WunderCorp tool-gateway entitlement; ``None`` when entitled.
    ...
    billing_url = wundercorp_portal_billing_url(account_info)
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                return (
                    f"{capability} isn't included with your current WunderCorp Portal access. "
    ...
    if account_info is None:
        return (
            f"Loki could not verify your WunderCorp Portal entitlement, so {capability} is unavailable. "
            f"Run `loki model` to refresh your login, or check billing at {billing_url}."
        )
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"WunderCorp inference credentials are configured, but Loki cannot verify your WunderCorp Portal "
                f"paid access for {capability}. Log in with `loki model` to enable Portal-managed "
                f"features. Billing and credits are managed at {billing_url}."
            )
        return (
            f"Log in to WunderCorp Portal to use {capability}: run `loki model`. "
            f"Billing and credits are managed at {billing_url}."
        )
    if account_info.paid_service_access is None:
        detail = f"Loki could not verify your WunderCorp Portal paid access, so {capability} is unavailable."
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `loki model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Loki could not find a WunderCorp Portal account or organisation for this login, so {capability} "
            f"is unavailable. Run `loki model` to authenticate again; if the problem persists, contact WunderCorp support."
        )
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `loki model` to refresh Loki."
        return message
    return (
        f"Your WunderCorp Portal account does not currently have paid service access, "
        f"so {capability} is unavailable. Add credits or update billing at {billing_url}."
    )


def _no_paid_access_message(
    account_info: WunderCorpPortalAccountInfo, capability: str, billing_url: str, *, in_chat: bool = False,
) -> str:
    if _is_anonymous_tier(account_info):
        return FREE_TIER_NEEDS_ACCOUNT_CHAT if in_chat else FREE_TIER_NEEDS_ACCOUNT
    access = account_info.paid_service_access_info or WunderCorpPaidServiceAccessInfo()
    ...
    if access.member_spend_cap_exceeded:
        ...
        return (
            f"Your WunderCorp Portal access is paused because you've exceeded the per-member spend cap set by "
            f"your organisation.{cap_detail}{credit_detail} Ask your organisation admin to raise the "
            f"member spend cap at {billing_url}, then run `loki model` to refresh."
        )
    if active and paid:
        return (
            f"Your WunderCorp Portal credits are exhausted{credit_detail}, so {capability} is unavailable. "
            f"Top up or renew credits at {billing_url}."
        )
    if active and paid is False:
        return (
            f"Your current WunderCorp Portal plan does not include paid service access, "
            f"so {capability} is unavailable. Upgrade or add credits at {billing_url}."
        )
    if active is False:
        return (
            f"Your WunderCorp Portal account has no active subscription or usable credits{credit_detail}, "
            f"so {capability} is unavailable. Subscribe or add credits at {billing_url}."
        )
    return (
        f"Your WunderCorp Portal account has no usable paid credits{credit_detail}, so "
        f"{capability} is unavailable. Add credits or update billing at {billing_url}."
    )


def reset_wundercorp_portal_account_info_cache() -> None:
    """Clear the short-lived account-info cache used by tests."""
    global _account_info_cache
    _account_info_cache = None


def get_wundercorp_portal_account_info(*, force_fresh: bool = False, min_jwt_ttl_seconds: int = 60) -> WunderCorpPortalAccountInfo:
    """Normalized WunderCorp Portal account entitlement.
    ...
    if not _nonblank(access_token):
        return (
            ...
            or WunderCorpPortalAccountInfo(logged_in=False, source="none", fresh=False, portal_base_url=portal_base_url)
        )
    if not force_fresh:
...
def wundercorp_policy_present() -> Optional[bool]:
    ...
    """
    try:
        from loki_cli.auth import get_provider_auth_state, _decode_jwt_claims

        access_token = (get_provider_auth_state("wundercorp") or {}).get("access_token")
        if not _nonblank(access_token):
            return None
...
def wundercorp_policy_notice(*, removed: bool) -> str:
    ...
    """
    if not removed or wundercorp_policy_present() is not True:
        return ""
    return (
...
def _fresh_account_info(state: dict[str, Any], force_fresh: bool, portal_base_url: Optional[str]) -> WunderCorpPortalAccountInfo:
    global _account_info_cache

    try:
        from loki_cli.auth import get_provider_auth_state, resolve_wundercorp_access_token

        access_token = resolve_wundercorp_access_token()
...
def _info_from_inference_key_pool(portal_base_url: Optional[str]) -> Optional[WunderCorpPortalAccountInfo]:
    """Return an explicit unknown-entitlement snapshot for opaque WunderCorp keys."""
    try:
        entry = _select_wundercorp_pool_entry()
        if entry is None:
            return None
...
) -> Optional[WunderCorpPortalAccountInfo]:
    try:
        entry = _select_wundercorp_pool_entry()
    except Exception:
        return None
...
def _info_from_fetched_account(
    ...
    payload = _fetch_wundercorp_account_info(access_token, portal_base_url)
    if not payload:
...
def _select_wundercorp_pool_entry() -> Optional[Any]:
...
def _fetch_wundercorp_account_info(access_token: str, portal_base_url: Optional[str] = None) -> dict[str, Any]:
...
def _info_from_valid_jwt(
    ...
    try:
        from loki_cli.auth import _decode_jwt_claims
    except Exception:
        return None
    ...
    access_info = WunderCorpPaidServiceAccessInfo(
    ...
    return WunderCorpPortalAccountInfo(
        ...
        wundercorp_client=_coerce_str(claims.get("wundercorp_client")),
...
def _info_from_account_payload(
    ...
    access = _coerced_dataclass(WunderCorpPaidServiceAccessInfo, payload.get("paid_service_access"))
    paid_access = None
...
def _tool_access_from_value(value: Any) -> Optional[WunderCorpToolAccessInfo]:
    ...
    return WunderCorpToolAccessInfo(enabled=value.get("enabled") is True, coverage=coverage)
...
def _subscription_from_payload(value: Any) -> Optional[WunderCorpPortalSubscriptionInfo]:
    return _coerced_dataclass(WunderCorpPortalSubscriptionInfo, value)
