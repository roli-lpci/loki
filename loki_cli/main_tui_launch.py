...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    from loki_cli.main import _resolve_last_session
    target = (
    ...
    print(f"\nResume this session with:\n  loki --tui --resume {target}")
    if title:
        print(f'  loki --tui -c "{title}"')
    print(f"\nSession:        {target}")
    if title:
        print(f"Title:          {title}")
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @wundercorp/loki-ink is missing or node_modules is behind package-lock.json.
    ...
    if not (ws_root / "node_modules" / "@loki" / "ink" / "package.json").is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_DIRS = ("src", "packages/loki-ink/src")
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/loki-ink/package.json",
    "packages/loki-ink/index.js",
    "packages/loki-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    ``LOKI_TUI_FORCE_BUILD=1`` forces a rebuild."""
    force = (os.environ.get("LOKI_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    the resolved node dir to PATH. ``LOKI_SKIP_NODE_BOOTSTRAP=1`` disables auto-install."""
    from loki_cli.main import PROJECT_ROOT
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("LOKI_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "LOKI_HOME": loki_home},
    ...
    extras += [Path(loki_home) / "node" / "bin", Path.home() / ".local" / "bin"]
    for extra in extras:
        s = str(extra)
...
def _find_bundled_tui(loki_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if loki_cli_dir is None:
        loki_cli_dir = Path(__file__).parent
    bundled = loki_cli_dir / "tui_dist" / "entry.js"
...
def _npm_lifecycle_env(env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    # the rebuild fails). ``$LOKI_HOME`` lives outside the git tree and the
    # update hand-off already carries ``LOKI_HOME`` down to every npm child.
    # An explicit ``NPM_CONFIG_USERCONFIG`` wins (#106373).
    from loki_constants import get_loki_home
    npmrc = get_loki_home() / "npmrc"
    if npmrc.is_file():
...
def _tui_node_bin(bin: str) -> str:
    """Resolve ``node``/``npm`` for the TUI launch, or exit with a hint. ``LOKI_NODE`` wins for node;
    ``find_node_executable()`` sees the managed ``$LOKI_HOME/node`` tree a bare which() misses."""
    if bin == "node":
        env_node = os.environ.get("LOKI_NODE")
    ...
    if not path and bin == "node":
        with contextlib.suppress(Exception):
            from loki_cli.dep_ensure import ensure_dependency
...
def _install_tui_dependencies(tui_dir: Path, *, termux_startup: bool) -> None:
    ...
    def _run_tui_install() -> subprocess.CompletedProcess:
        ...
        return subprocess.run(
            ...
            env=_npm_lifecycle_env(with_loki_node_path()))
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (LOKI_TUI_DIR prebuilt or esbuild)."""
    from loki_cli.main import _is_termux_startup_environment
    _ensure_tui_node()

    # Footgun: --dev against a prebuilt bundle that has no source/node_modules.
    ext_dir = os.environ.get("LOKI_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with LOKI_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset LOKI_TUI_DIR (e.g. `unset LOKI_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr)
        sys.exit(1)
    ...
    if tui_dev:
        # --dev runs src/entry.tsx directly, but @wundercorp/loki-ink resolves through
        # packages/loki-ink/dist/entry-exports.js; a stale dist after a pull
        # leaves newer hooks/components missing at runtime. Prebuild it here.
        npm = _tui_node_bin("npm")
        _run_tui_npm_build(npm, tui_dir / "packages" / "loki-ink", "TUI dev prebuild failed.")
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from loki_cli.oneshot import _normalize_toolsets
...
def _apply_tui_python_env(env: dict) -> None:
    ...
    from loki_cli.main import PROJECT_ROOT
    src_root = str(env.get("LOKI_PYTHON_SRC_ROOT") or "").strip()
    if not src_root or not Path(src_root).is_dir():
        env["LOKI_PYTHON_SRC_ROOT"] = str(PROJECT_ROOT)

    cwd = str(env.get("LOKI_CWD") or "").strip()
    if not cwd or not Path(cwd).is_dir():
        env["LOKI_CWD"] = _safe_tui_cwd(env)

    python = str(env.get("LOKI_PYTHON") or "").strip()
    if os.path.dirname(python):
        python_path = Path(python)
        if not python_path.is_absolute():
            python_path = Path(env["LOKI_CWD"]) / python_path
    ...
    if not python_is_executable:
        env["LOKI_PYTHON"] = sys.executable
...
def _launch_tui(
    ...
    from loki_cli.shared_session_attach import configure_tui_attachment
    try:
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="loki-tui-active-session-", suffix=".json")
    os.close(active_session_fd)
    env["LOKI_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env.setdefault("NODE_ENV", "development" if tui_dev else "production")

    wt_info = None
    if worktree:
        wt_info = _setup_tui_worktree()
        env["LOKI_CWD"] = wt_info["path"]
    ...
    for key, value in (
        ("LOKI_MODEL", model), ("LOKI_INFERENCE_MODEL", model),
        ("LOKI_TUI_PROVIDER", provider), ("LOKI_INFERENCE_PROVIDER", provider),
        ("LOKI_TUI_TOOLSETS", ",".join(_normalize_tui_toolsets(toolsets))),
        ("LOKI_TUI_SKILLS", skills_value),
        ("LOKI_TUI_QUERY", query), ("LOKI_TUI_IMAGE", image),
        ("LOKI_TUI_CHECKPOINTS", "1" if checkpoints else None),
        ("LOKI_TUI_PASS_SESSION_ID", "1" if pass_session_id else None),
        ("LOKI_TUI_MAX_TURNS", str(max_turns) if max_turns is not None else None),
        ("LOKI_TUI_TOOL_PROGRESS", "verbose" if verbose else "off" if quiet else None),
        ("LOKI_ACCEPT_HOOKS", "1" if accept_hooks else None)):
        if value:
            env[key] = value
    ...
    # LOKI_TUI_RESUME is an internal hand-off to the Ink app. We start from a
    # full os.environ snapshot, so a stale exported value would make a plain
    # `loki --tui` try to resume a non-existent session; only forward the id
    # argparse resolved for this invocation.
    env.pop("LOKI_TUI_RESUME", None)
    if resume_session_id:
        env["LOKI_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `loki update`;
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``LOKI_KANBAN_BOARD`` so in-process tools and shelled-out
    ``loki kanban`` calls agree even if a concurrent ``boards switch`` flips the file mid-turn.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls (``loki kanban …``) resolve
    the board on different paths: the env-pin if set, otherwise the global ``<root>/kanban/current`` file. A
    concurrent ``loki kanban boards switch`` from another session can flip the file mid-turn, so the same
    ...
    """
    if os.environ.get("LOKI_KANBAN_BOARD"):
        return
    with contextlib.suppress(Exception):
        from loki_cli.kanban_db import get_current_board
        os.environ["LOKI_KANBAN_BOARD"] = get_current_board()


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.loki/skills/`` with the bundled library (idempotent, milliseconds when synced).
...
def _resolve_use_tui(args) -> bool:
    ...
    ``LOKI_TUI=1`` → TUI; ``display.interface`` config; default classic.

    The TTY gate is load-bearing: ambient preferences must never hijack a piped
    ``loki chat -q`` (kanban workers, cron) — the Ink no-TTY bail-out exits 0 and
