"""Bounded product contract for the first Loki shared-metrics slice."""
...
SCHEMA_KEY = "loki.metrics.schema_version"
SCHEMA_VERSION = "loki.metrics.event.v2"
MODEL_CALL_SCOPE = "loki.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "loki.task_run"
TOOL_CALL_SCOPE = "loki.tool_call"
CLIENT_ACTIVE_MARK = "loki.client.active"
TOOL_APPROVAL_MARK = "loki.tool_approval"
SKILL_LIFECYCLE_MARK = "loki.skill.lifecycle"
SKILL_LOAD_MARK = "loki.skill.load"
SUBSCRIBER_NAME = "loki.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "loki.client.active"
LEGACY_MODEL_CALL_METRIC = "loki.model_call.count"
MODEL_ROUTE_METRIC = "loki.model_route.count"
TASK_STARTED_METRIC = "loki.task_run.started"
TASK_FINISHED_METRIC = "loki.task_run.finished"
TOOL_CALL_METRIC = "loki.tool_call.count"
TOOL_APPROVAL_METRIC = "loki.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "loki.skill.lifecycle.count"
SKILL_LOAD_METRIC = "loki.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "loki_version", "install_method", "os_family"})
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Loki installation method."""
    normalized = _norm(value)
    return _allowlisted("nixos" if normalized == "nix" else normalized, CLIENT_INSTALL_METHODS)


def client_resource(
    loki_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(loki_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "loki_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("loki_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Loki logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "loki.call_role"
    )
    call_role = (metadata or {}).get("loki.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from loki_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Loki terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "loki-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Loki registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Loki tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Loki."""
