"""MCP Server Management CLI — ``loki mcp`` subcommand."""
...
from loki_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_loki_home,  # noqa: F401 — used by test mocks
)
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    Mirrors ``_load_mcp_config()`` in ``tools/mcp_tool.py``: load ``~/.loki/.env`` into ``os.environ`` and
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    try:
        ...
        return LokiTokenStorage(name).has_cached_tokens()
    ...
    if needs_auth and (auth_type == "header" or not auth_type):
        ...
        if get_env_value(env_key):
            ...
            if api_key:
                ...
                _success(f"Saved to {display_loki_home()}/.env as {env_key}")
    return True
...
def cmd_mcp_add(args):
    ...
    # --command uses dest="mcp_command" (see loki_cli/main.py for why the dest is renamed).
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  loki mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  loki mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  loki mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: loki mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(
            f"Saved '{name}' to {display_loki_home()}/config.yaml ({tool_count}/{len(tools)} tools enabled)"
        )
...
def cmd_mcp_remove(args):
    ...
    # an earlier `loki mcp test`) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  loki mcp add <name> --url <endpoint>')
        _info('  loki mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict, *, flow: str | None = None) -> bool:
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `loki mcp remove` + `loki mcp add` to reconfigure auth.")
        return False
    ...
    # re-auth path in web_server.py. force_interactive_oauth: `loki mcp login` is explicitly
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `loki mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    if not name:
        ...
        _info("Usage: loki mcp reauth <name>   |   loki mcp reauth --all")
        return
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'loki mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    cfg = _lookup_server(name, _get_mcp_servers(), "Available")
    if cfg is None:
...
_MCP_USAGE = (
    ...
    "loki mcp add <name> --preset <preset>       Add from a known preset",
    "loki mcp remove <name>                      Remove a server",
    "loki mcp list                               List configured servers",
    "loki mcp test <name>                        Test connection",
    "loki mcp configure <name>                   Toggle tools",
    "loki mcp login <name>                       Re-authenticate OAuth",
    "loki mcp reauth <name> | --all              Re-auth one or all OAuth servers",
)


def mcp_command(args):
    """Main dispatcher for ``loki mcp`` subcommands."""
    ...
    if action in ("picker", "catalog", "install"):
        ...
        from loki_cli import mcp_picker
    ...
    if handler:
        handler(args)
        return
    # No subcommand — drop the user into the catalog picker (same UX as `loki plugin`).
    from loki_cli.mcp_picker import run_picker
    run_picker()
