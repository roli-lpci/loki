"""Interactive setup wizard for Loki Agent (config lives in ~/.loki/).
...
``loki_cli.setup.<name>`` keep working.
"""
...
from loki_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
# Config helpers are re-exported (tests patch them on this module). display_loki_home is
# imported lazily at call sites (stale-module safety during loki update).
from loki_cli.config import (
    cfg_get, DEFAULT_CONFIG, get_loki_home, get_config_path, get_env_path, load_config, save_config,
    save_env_value, remove_env_value, get_env_value, ensure_loki_home,
)
from loki_cli.colors import Colors, color
from loki_cli.cli_output import print_error, print_info, print_success, print_warning
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    _info("The interactive wizard cannot be used here.", None,
          "Configure Loki using environment variables or config commands:",
          "  loki config set model.provider custom",
          "  loki config set model.base_url http://localhost:8080/v1",
          "  loki config set model.default your-model-name", None,
          "Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.",
          "Run 'loki setup' in an interactive terminal to use the full wizard.", None)
...
class _SetupNavigationState:
    """Per-invocation navigation state for the synchrowundercorp setup wizard."""
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar("loki_setup_navigation", default=None)
...
def _setup_navigation_scope():
    ...
    from loki_cli.curses_ui import reset_menu_navigation_handler, set_menu_navigation_handler
...
def run_setup_action_with_navigation(
    ...
    ``loki model`` that use the wizard's pickers outside ``run_setup_wizard``."""
...
def is_noninteractive() -> bool:
    ...
    with ``stdin=DEVNULL`` and ``LOKI_NONINTERACTIVE=1`` (``loki_cli/web_server.py``), where a
    prompt that aborts on EOF would kill the spawned action — callers fall back to their default."""
    return os.environ.get("LOKI_NONINTERACTIVE", "").strip().lower() in {"1", "true", "yes", "on"}


def prompt_yes_no(question: str, default: bool = True) -> bool:
    """Prompt for yes/no. Ctrl+C exits; empty input, ``LOKI_NONINTERACTIVE=1`` or a
...
def _prompt_api_key(var: dict):
    ...
    print()
    _prompt_and_save_env_var(var, "  ✓ Saved", "  Skipped (configure later with 'loki setup')")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    """Configure the inference provider and default model via the ``loki model`` flow (one code
    ...
    # keep safe defaults (configure via `loki auth add` / `loki setup tts`).
    config.clear()
...
def _apply_default_agent_settings(config: dict):
    ...
    # config.yaml is authoritative for max_turns (the gateway bridges it into LOKI_MAX_ITERATIONS);
    # a stale .env entry silently shadowing it caused the 60-vs-500 bug, so drop it.
    remove_env_value("LOKI_MAX_ITERATIONS")
    ...
    _info("  Max iterations: 150", "  Tool progress: all", "  Compression threshold: 0.50",
          "  Run `loki setup agent` later to customize.")
...
_TOOL_PROGRESS_HELP = (
    ...
    "  log     — Silent in chat; write every tool call to ~/.loki/logs/tool_calls.log (gateway only)",
)
def setup_agent_settings(config: dict):
    ...
    if max_iter is None:
        ...
        # config.yaml only; gateway/run.py derives LOKI_MAX_ITERATIONS from agent.max_turns.
...
def setup_tools(config: dict, first_install: bool = False):
    """`loki setup tools` == `loki tools`: platform selection → toolset toggles → provider keys.
    ``first_install`` selects the simplified flow (no platform menu, prompts for all missing keys)."""
    from loki_cli.tools_config import tools_command
...
_SEND_CONSENT_EXPLAINER = (
    "", "Sending uploads each daily package to the WunderCorp telemetry",
...
def setup_telemetry(config: dict):
    ...
    _info("Shared metrics contain only bounded counters and histograms.",
          "Collection is local. Sending them to WunderCorp is a separate opt-in.")
    ...
    shared_metrics["send"] = prompt_yes_no("Send shared metrics to WunderCorp?", default=shared_metrics.get("send") is True)
...
def _record_send_consent_change(*, enabled: bool) -> None:
    ...
    try:
        from loki_cli.observability.shared_metrics import SharedMetricsStore
        from loki_cli.observability.shared_metrics_sender import reconcile_send_consent
        from loki_cli.sqlite_util import write_txn
...
def _run_setup_section(config: dict, section: str) -> None:
    """``loki setup <section>``: run one SETUP_SECTIONS entry under the banner."""
...
def _run_full_setup(config: dict, loki_home, *, is_existing: bool, migration_ran: bool) -> None:
    ...
    _info(f"Config file:  {get_config_path()}", f"Secrets file: {get_env_path()}",
          f"Data folder:  {loki_home}", f"Install dir:  {PROJECT_ROOT}", None,
          "You can edit these files directly or use 'loki config edit'")
    if migration_ran:
...
def _run_setup_wizard_impl(args):
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps([("Quick Setup", lambda: _run_quick_setup(config, loki_home))])
            return
        ...
        migration_ran = _offer_openclaw_migration(loki_home)  # before configuration begins
        if migration_ran:
            config = load_config()
        setup_mode = prompt_choice("How would you like to set up Loki?", [label for label, _ in _FIRST_TIME_MODES], 0)
        label, runner = _FIRST_TIME_MODES[setup_mode]
        if runner is not None:
            from loki_cli import setup_quick
            _run_setup_steps([(label, lambda: getattr(setup_quick, runner)(config, loki_home, is_existing))])
            return
    _run_full_setup(config, loki_home, is_existing=is_existing, migration_ran=migration_ran)
...
_PLUGIN_COMPAT_LAZY = {
    'get_wundercorp_subscription_features': ('loki_cli.wundercorp_subscription', 'get_wundercorp_subscription_features'),
    'get_optional_skills_dir': ('loki_constants', 'get_optional_skills_dir'),
