...
Mixed into :class:`loki_cli.plugins.PluginManager`. ``_resolve_hook_callback_timeout`` stays on
...
from loki_cli.middleware import OBSERVER_SCHEMA_VERSION
...
# abandon without join — joining reintroduced a shutdown hang). Unlisted hooks run synchrowundercorply.
...
# synchrowundercorply to completion. (on_session_start/end stay bounded — they sit on the common session-boundary
...
PLUGIN_SECTIONS_START = "<!-- loki-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- loki-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- loki-plugin-section-chars:{len(content)} -->\n\n{content}")
...
# Reserved event namespace prefix — only core may publish ``loki:<event>``.
LOKI_EVENT_NAMESPACE = "loki"
...
class PluginDispatchMixin:
    ...
    def invoke_hook(self, hook_name: str, **kwargs: Any) -> List[Any]:
        ...
        """
        from loki_cli.plugins import _resolve_hook_callback_timeout
    ...
    def _run_hook_callback_bounded(
        ...
        thread = threading.Thread(target=_runner, name=f"loki-hook-{callback_name}"[:40], daemon=True)
        try:
            thread.start()
    ...
    def _ensure_event_worker_locked(self) -> None:
        ...
        worker = threading.Thread(
            target=self._event_worker_loop, args=(self._event_queue,), name="loki-plugin-events",
            daemon=True,
        )
    ...
    def _deliver_event(self, item: _QueuedPluginEvent) -> None:
        ...
        from loki_cli.plugins import resolve_plugin_command_result
        with self._event_lock:
