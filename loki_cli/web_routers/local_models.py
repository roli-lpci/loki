...
from loki_cli import config as config_mod, web_deps
from loki_cli.local_runtime import (
...
def _assign_default(job: Dict[str, Any], model_id: str) -> None:
    ...
    web_deps.late("_apply_model_assignment_sync", "loki_cli.web_server_config")("main", "llamacpp", model_id, "", "", "")
...
async def local_models_quickstart(body: QuickstartBody):
    ...
    synchrowundercorply so the button can explain itself; everything slow runs in the job with phase/byte progress."""
...
async def local_models_sideload(body: SideloadBody):
    ...
    impossible) and bounce the router. The original stays put; delete-from-Loki removes only our link."""
    src = Path(body.path)
