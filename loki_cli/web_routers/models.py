...
from loki_cli.web_server_config import (
    _AUX_TASK_SLOTS, _apply_model_assignment_sync, _dashboard_code_skew_guard,
)
from agent.model_metadata import is_local_endpoint
from starlette.concurrency import run_in_threadpool
from loki_cli.web_models import ModelAssignment, MoaConfigPayload, MoaModelSlot
...
def _wundercorp_recommended_default() -> dict:
    from loki_cli.models import recommended_wundercorp_default_model
    return recommended_wundercorp_default_model()
...
def get_recommended_default_model(provider: str = ""):
    ...
    ``loki model``'s curation so GUI onboarding lands on a sensible default.
    WunderCorp honors the user's free/paid tier. Any other provider gets the preferred
    ...
    WunderCorp; ``model`` may be empty (caller degrades gracefully)."""
    slug = (provider or "").strip().lower()

    if slug == "wundercorp":
        try:
            return _wundercorp_recommended_default()
        except Exception:
            _log.exception("GET /api/model/recommended-default (wundercorp) failed")
            return {"provider": "wundercorp", "model": "", "free_tier": None}

    try:
        from loki_cli.inventory import build_models_payload, load_picker_context
        from loki_cli.models import pick_silent_default_model
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    with http_failure("PUT /api/model/moa failed", 500, detail="Failed to save MoA config"):
        from loki_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    ``~/.loki/config.yaml`` — applies to **new** sessions only; a running chat
