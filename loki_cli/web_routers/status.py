...
Extracted from ``loki_cli.web_server``; app state and helpers are late-bound through
:mod:`loki_cli.web_deps` (cycle-safe, monkeypatch-friendly).
"""
...
from loki_cli.web_deps import LateState, late
from loki_cli.web_server_gateway import _display_system_platform
...
from loki_cli.config import get_config_path, get_env_path
from loki_cli.web_models import CuratorPause, LearningNodeRef, LearningNodeEdit, DebugShareRequest
from loki_cli.web_routers._common import scoped_to_thread
...
_collect_profile_gateway_topology_cached = late("_collect_profile_gateway_topology_cached", "loki_cli.web_server_gateway")
_config_profile_scope = late("_config_profile_scope", "loki_cli.web_server_profiles")
_dashboard_local_update_managed_externally = late("_dashboard_local_update_managed_externally", "loki_cli.web_server_files")
_load_configured_gateway_platforms = late("_load_configured_gateway_platforms", "loki_cli.web_server_gateway")
_probe_gateway_health = late("_probe_gateway_health", "loki_cli.web_server_gateway")
_require_token = late("_require_token")
_resolve_profile_dir = late("_resolve_profile_dir", "loki_cli.web_server_profiles")
_resolve_restart_drain_timeout = late("_resolve_restart_drain_timeout", "loki_cli.web_server_lifecycle")
_spawn_loki_action = late("_spawn_loki_action", "loki_cli.web_server_gateway")
_ssh_runtime_intact = late("_ssh_runtime_intact")
app = LateState("app")  # the FastAPI instance (app.state.*)
check_config_version = late("check_config_version", "loki_cli.config")
get_loki_home = late("get_loki_home", "loki_cli.config")
...
load_config = late("load_config", "loki_cli.config")
read_runtime_status = late("read_runtime_status", "gateway.status")
_open_session_db_for_profile = late("_open_session_db_for_profile", "loki_cli.web_server_sessions")
...
def _count_status_active_sessions() -> int:
    ...
    /api/status never routinely writes to state.db while another Loki process uses it."""
    from loki_state import _default_db_path
...
async def get_ssh_ownership(request: Request):
    from loki_cli.web_server import _SSH_OWNER_NONCE
...
# Profile segment mirrors loki_cli.profiles._PROFILE_ID_RE. Platform segment mirrors the
...
# status) are written to the per-profile home, not the process-level LOKI_HOME (see issue #69143). Plain
...
def _auth_gate_status() -> Dict[str, Any]:
    ...
    auth_flows: list[str] = []
    try:
        from loki_cli.dashboard_auth import (
...
def _wundercorp_session_validity() -> str:
    """WunderCorp bootstrap-session validity for the NAS health sweep: a hosted agent whose WunderCorp
    ...
    try:
        from loki_cli.auth import get_wundercorp_session_validity
        return get_wundercorp_session_validity()
    except Exception:
        return "unknown"
...
async def _component_health(gateway: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from loki_cli.web_server import DASHBOARD_HEALTH
    ...
    try:
        ...
        storage_check = await run_in_threadpool(_probe_state_db, get_loki_home())
...
async def _advisory_pressure(status: Dict[str, Any], home: Path) -> None:
    ...
    try:
        from loki_state import SessionDB as _SDB
...
async def get_status(profile: Optional[str] = None):
    ...
    """
    from loki_cli.web_server import _GATEWAY_HEALTH_URL
    status_scope = None
    ...
    try:
        ...
        # Off-loop: on a cold Windows install the first import of loki_cli.gateway blocks
        ...
        status = {
            ...
            "can_update_loki": not _dashboard_local_update_managed_externally(),
            ...
            **auth, "wundercorp_session_valid": _wundercorp_session_validity()}
        ...
        await _advisory_pressure(status, profile_dir if profile_dir else get_loki_home())

        # Profile NAMES and ``gateway_mode`` are low-sensitivity product surface (Loki Cloud
        ...
        if not auth["auth_required"]:
            status.update({
                "loki_home": str(get_loki_home()), "config_path": str(get_config_path()),
...
async def get_system_stats():
    ...
    disk/uptime when available). Non-sensitive: no env values, no paths beyond loki home."""
    ...
    info: Dict[str, Any] = {
        **_display_system_platform(
        ...
        "loki_version": __version__, "cpu_count": os.cpu_count()}

    def _disk():
        du = psutil.disk_usage(str(get_loki_home()))
...
def _spawn_action(argv: list, name: str, prefix: str) -> dict:
    """Spawn a background ``loki <argv>`` action; a spawn failure is ``500 "<prefix>: <exc>"``."""
    try:
        proc = _spawn_loki_action(argv, name)
    except Exception as exc:
...
# Portal — WunderCorp Portal auth + Tool Gateway routing status (read-only).
...
def _feature_state(feat) -> str:
    if getattr(feat, "managed_by_wundercorp", False):
        return "via WunderCorp Portal"
...
def _get_portal_status_sync():
    ...
    try:
        ...
        auth = get_wundercorp_auth_status_local() or {}
    except Exception:
        auth = {}
    ...
    try:
        ...
        feats = get_wundercorp_subscription_features(cfg)
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    """Upload a redacted debug report + full logs and return the paste URLs. Synchrowundercorp,
...
async def get_logs(
    ...
    from loki_cli.logs import _read_tail, LOG_FILES
    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_loki_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from loki_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
