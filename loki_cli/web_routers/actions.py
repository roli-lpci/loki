"""Gateway restart/drain, Loki update and background-action status dashboard routes.
...
from loki_cli.config import format_docker_update_message, recommended_update_command_for_method
from loki_cli.web_deps import LateState, late
from loki_cli.web_server_gateway import _ACTION_LOG_FILES
from loki_cli.web_routers._common import http_failure
...
detect_install_method = late("detect_install_method", "loki_cli.config")
get_loki_home = late("get_loki_home", "loki_cli.config")
_ACTION_COMMANDS = LateState("_ACTION_COMMANDS", "loki_cli.web_server_gateway")
_ACTION_IDS = LateState("_ACTION_IDS", "loki_cli.web_server_gateway")
_ACTION_PROCS = LateState("_ACTION_PROCS", "loki_cli.web_server_gateway")
_ACTION_RESULTS = LateState("_ACTION_RESULTS", "loki_cli.web_server_gateway")
...
def _server_path(name: str) -> Path:
    ...
    import loki_cli.web_server_gateway as gw
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(r"^=== loki-update completed ([0-9a-f]{32}) ===$")

_MANAGED_EXTERNALLY_MESSAGE = "Loki updates are managed outside this dashboard in containerized environments."
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== loki update started "):
            last_start = index
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``loki gateway restart`` in the background."""
...
def _update_refused(error: str, message: str, update_command: str) -> Dict[str, Any]:
    _record_completed_action("loki-update", message, exit_code=1)
    return {
        "ok": False, "pid": None, "name": "loki-update", "error": error, "message": message,
        "update_command": update_command,
    }


@router.post("/api/loki/update")
async def update_loki():
    """Kick off ``loki update`` in the background."""
    ...
    existing = _ACTION_PROCS.get("loki-update")
    if existing is not None and existing.poll() is None:
        response = {"ok": True, "pid": existing.pid, "name": "loki-update", "already_running": True}
        action_id = _ACTION_IDS.get("loki-update")
        if action_id:
            response["action_id"] = action_id
        return response

    action_id = secrets.token_hex(16)
    with http_failure("Failed to spawn loki update", 500, "Failed to start update"):
        proc = _spawn_loki_action(["update"], "loki-update", env_overrides={"LOKI_ACTION_ID": action_id})
    return {"ok": True, "pid": proc.pid, "name": "loki-update", "action_id": action_id}
...
_NON_APPLYABLE_MESSAGES = {
    ...
    "apt": lambda: "Loki is managed by Termux APT; run `pkg upgrade loki-agent`.",
}


@router.get("/api/loki/update/check")
async def check_loki_update(force: bool = False):
    """Report whether a Loki update is available, without applying it.
    ...
    try:
        from loki_cli.banner import check_for_updates

        if force:
            with contextlib.suppress(OSError):
                (get_loki_home() / ".update_check").unlink()
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "loki-update":
        ...
        if durable_update_action_id:
            marker = f"=== loki-update completed {durable_update_action_id} ==="
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    """Compact summary of the latest receipt (written by EVERY ``loki update`` run,
    incl. refused/failed), or None; never raises. Steps/skips stay in the full endpoint.

    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``loki update`` run since #91283, including
...
@status_router.get("/api/loki/update/receipt")
async def get_update_receipt():
    ...
    receipt = _read_latest_receipt()
    if not receipt:
        raise HTTPException(status_code=404, detail="No update receipt found (no `loki update` run recorded).")
