...
Shared helpers are reached via the late-binding seam in :mod:`loki_cli.web_deps`
...
from loki_cli.web_server_config import _apply_main_model_assignment, _normalize_main_model_assignment
from loki_cli.web_server_gateway import _strip_session_list_rows
from loki_cli.web_server_profiles import (
    _fallback_profile_dicts, _hub_action_name, _write_profile_mcp_servers,
)
from loki_cli.web_server_sessions import _open_session_db_at_path
...
from loki_cli.web_models import (
    ...
    SessionPrScanBody)
from loki_cli.web_server_profiles import _loki_home_scope
...
_cron_profile_home = late("_cron_profile_home", "loki_cli.web_server_cron")
...
def _profile_setup_command(name: str) -> str:
    ...
    return "loki setup" if name == "default" else f"{name} setup"


def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    """Write the main model assignment into ``profile_dir``'s config.yaml (LOKI_HOME-scoped);
    clears stale ``base_url`` / ``context_length`` like ``POST /api/model/set`` does."""
    from loki_cli.config import load_config, save_config
    with _loki_home_scope(profile_dir):
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    from loki_cli.skills_config import get_disabled_skills, save_disabled_skills
...
def _profile_errors(log_msg: str, *args, not_found=(FileNotFoundError,),
    ...
    """Map loki_cli.profiles exceptions to HTTP: ``not_found`` -> 404, ``bad_request`` -> 400
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    def _spawn_install(ident: str):
        return _spawn_loki_action(["-p", body.name, "skills", "install", ident, "--yes"],
...
async def get_active_profile_endpoint():
    """``active`` is the sticky default written by ``loki profile use`` (what new CLI
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``loki profile use``); does not retarget the
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    own profile — ``POST /api/model/set`` (main scope) via the LOKI_HOME override."""
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    """Auto-generate a profile's description via the auxiliary LLM (mirrors ``loki profile
    ...
    with _profile_errors("POST /api/profiles/%s/describe-auto failed", name,
                         not_found=(), bad_request=()):
        # A synchrowundercorp LLM round-trip with a 60 s ceiling; on the loop it stalls everything.
...
# ── Export / Import ── wraps loki_cli.profiles.export_profile / import_profile. Paths are
