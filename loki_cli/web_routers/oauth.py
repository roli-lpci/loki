...
Extracted from ``loki_cli.web_server``; helpers/state that tests monkeypatch on
...
from loki_cli.web_server_oauth import (
    _external_process_cli_command, _minimax_poller, _wundercorp_plain_poller, _wundercorp_promotion_poller, _oauth_profile_name, _oauth_sessions, _oauth_sessions_lock, _truncate_token, _xai_device_poller,
)
from loki_cli.web_models import OAuthSubmitBody
...
_profile_scope = late("_profile_scope", "loki_cli.web_server_profiles")
_require_token = late("_require_token")
_resolve_profile_dir = late("_resolve_profile_dir", "loki_cli.web_server_profiles")
_OAUTH_PROVIDER_CATALOG = LateState("_OAUTH_PROVIDER_CATALOG", "loki_cli.web_server_oauth")
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Loki "
...
def _codex_request_user_code(httpx) -> Dict[str, Any]:
    ...
    from loki_cli.auth import CODEX_OAUTH_CLIENT_ID
...
def _codex_exchange_tokens(httpx, code_resp: Dict[str, Any]) -> Dict[str, str]:
    ...
    from loki_cli.auth import CODEX_OAUTH_CLIENT_ID, CODEX_OAUTH_TOKEN_URL
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from loki_cli.auth import _save_codex_tokens
...
# wundercorp: refresh-free local snapshot so listing providers never performs an OAuth
...
_PROVIDER_STATUS: Dict[str, tuple[str, Callable[[dict], dict]]] = {
    "wundercorp": ("get_wundercorp_auth_status_local", lambda r: {**_status_card(
        r, "wundercorp_portal", r.get("portal_base_url") or "WunderCorp Portal",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        if status_fn is not None:
            return status_fn()
        from loki_cli import auth as hauth
...
async def _start_wundercorp_device_code(profile: Optional[str]) -> Dict[str, Any]:
    """Start a WunderCorp sign-in. Over a free-tier identity (``wundercorp.guest`` on) the whole sign-in is the
    ...
    from loki_cli.auth import PROVIDER_REGISTRY, _request_device_code
    from loki_cli.web_server_profiles import _config_profile_scope, _profile_scope
    pconfig = PROVIDER_REGISTRY["wundercorp"]
    portal_base_url = (
        os.getenv("LOKI_PORTAL_BASE_URL") or os.getenv("WUNDERCORP_PORTAL_BASE_URL") or pconfig.portal_base_url
    ).rstrip("/")
    with _profile_scope(_oauth_profile_name(profile)):
        guest = anon_auth.current_wundercorp_state() if anon_auth.guest_enabled() else None
    ...
    if not anon_auth.is_guest_state(guest):
        ...
        return _device_session_started(
            "wundercorp", profile, _wundercorp_plain_poller, fields, str(device_data["user_code"]),
    ...
    sid, sess = _new_oauth_session("wundercorp", "device_code", profile=profile)
    ...
    _start_poller(_wundercorp_promotion_poller, sid)   # last: nothing observes `sess` before it is complete
    return {
...
async def _start_xai_device_code(profile: Optional[str]) -> Dict[str, Any]:
    from loki_cli.auth import _xai_oauth_request_device_code
...
_DEVICE_CODE_STARTERS = {
    "wundercorp": _start_wundercorp_device_code, "openai-codex": _start_codex_device_code,
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    # "anthropic" is flow == "external" (no in-dashboard login) but Loki still
    # OWNS its credential (the PKCE file ~/.loki/.anthropic_oauth.json and its
    # credential-pool entry, written by `loki auth add anthropic`), so it is
    ...
    if provider.get("flow") == "external" and provider.get("id") != "anthropic":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Loki — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    in ``loki model`` order, so plugin-added OAuth/external providers appear automatically."""
    ...
    try:
        ...
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "cli_command": f"loki auth add {d.slug}", "docs_url": d.signup_url or "", "status_fn": None,
            })
    except Exception:
        pass
    return rows
...
def _clear_anthropic_auth() -> bool:
    """Clear only the Loki-managed PKCE file and auth-store entry (never ~/.claude/*)."""
    cleared = False
    try:
        from agent.anthropic_credentials import _get_loki_oauth_file
        oauth_file = _get_loki_oauth_file()
...
async def disconnect_oauth_provider(provider_id: str, request: Request, profile: Optional[str] = None):
    ...
    def _run():
        ...
        try:
            from loki_cli.auth import clear_provider_auth, invalidate_wundercorp_auth_status_cache
            cleared = clear_provider_auth(provider_id)
            if provider_id == "wundercorp":
...
async def poll_oauth_session(provider_id: str, session_id: str, profile: Optional[str] = None):
    ...
    return {
        ...
        # WunderCorp over a free-tier identity: why a transfer ended, who signed in, and the default model
