"""Implementation of the ``loki bundles`` CLI subcommand."""
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  loki bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``loki bundles`` argparse tree (called from loki_cli/main.py, which owns the
    top-level subparser)."""
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `loki bundles list`")
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``loki bundles <subcommand>`` to the right handler."""
