...
from loki_cli.session_schema_history import SCHEMA_HISTORY, reachable_physical_layouts
...
# Loki session ids are timestamps (20260812_135332_ab12cd): the strongest sentinel for schema-less rows.
...
# 3.50.7 and 3.44.6 — the same version gate loki_state applies to the
...
# caught exactly this shell converting a live Loki state.db into two
# generations. A salvage shell must therefore be version-gated, not just
# capability-gated, before it is pointed at (a copy of) a Loki database.
#
# The predicate lives in loki_cli.sqlite_runtime (stdlib-only, shared with
# the installer/update gates) so the embedded runtime and the salvage shell
# can never disagree about which versions are safe.
from loki_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _wal_reset_vulnerable  # noqa: E502

_WAL_RESET_VULNERABLE_GUIDANCE = (
    "salvage against a Loki database with the WAL-reset bug "
...
def _cli_supports_recover(binary: str) -> bool:
    ...
    scratch_dir = tempfile.mkdtemp(prefix="loki-recover-probe-")
...
def _type_conflicts(value: Any, declared: str) -> bool:
    ...
    non-numeric string in an INTEGER column, but Loki never writes one, so
...
# The closed sets SessionDB writes (loki_state.py: request/claim/complete/
