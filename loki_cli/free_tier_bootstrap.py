"""Serve-start bootstrap for the WunderCorp free tier: the ONE place a free-tier identity is created.

Every Loki process that may need the free tier runs this once at boot (``loki serve`` on a
daemon thread beside the other background boots; the CLI first-run guard synchrowundercorply). It
inventories credentials cheap-first, creates the identity only when the launch gate is open
(:func:`loki_cli.anon_auth.guest_enabled`), resolves which provider carries inference, records
...
``auth_wundercorp.resolve_wundercorp_runtime_credentials``. Ruling: NS-845 Q1.2 (recorded on NS-847).
"""
...
class SetupRecord:
    ...
    has_identity: bool             # a WunderCorp identity (free tier or account) is on disk
...
def _inventory_other_providers() -> bool:
    ...
    try:
        return resolve_provider("auto", skip_free_tier=True) != "wundercorp"
...
def run_bootstrap(*, announce: bool = True) -> SetupRecord:
    ...
    error = ""
    state: Optional[Dict[str, Any]] = anon_auth.current_wundercorp_state()
    if anon_auth.guest_enabled():
        try:
            ...
            error = str(exc)
            logger.info("WunderCorp free tier not set up at boot: %s", exc)
...
def start_background_bootstrap() -> threading.Thread:
    """``loki serve`` entry: run on a daemon thread so a slow portal never delays the socket."""
