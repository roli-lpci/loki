"""loki fallback — manage the fallback provider chain (tried in order when the primary fails).

Subcommands: ``list`` (default), ``add`` (same picker as `loki model`), ``remove``, ``clear``.
"""
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if chain is None:
        print("  Add one with:  loki fallback add\n")
        return
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `loki model`, then append the selection to the chain."""
    from loki_cli.main import _require_tty, select_provider_and_model
    from loki_cli.config import load_config, save_config
    ...
    print("\n  Adding a fallback provider.  The picker below is the same one used by\n"
          "  `loki model` — select the provider + model you want as a fallback.\n")
    ...
    print("  Run `loki fallback list` to view, or `loki fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``loki fallback [subcommand]``."""
