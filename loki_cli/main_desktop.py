"""Desktop (Electron) app: build/stamp, stage-and-swap pack, exe integrity gate, macOS signing/TCC, Linux sandbox, launch (loki gui/desktop).
...
from loki_cli.main_tui_launch import _npm_lifecycle_env
...
def _desktop_stamp_path() -> Path:
    """Path of the desktop build stamp under $LOKI_HOME."""
    from loki_constants import get_loki_home
    return get_loki_home() / "desktop-build-stamp.json"
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Loki.app/Contents/MacOS/Loki → …/Contents/Resources
    resources = (
...
def _desktop_packaged_executable_in(release_dir: Path) -> Optional[Path]:
    ...
    *release_dir* is electron-builder's ``directories.output`` — the live ``apps/desktop/release`` or a
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Loki.app/Contents/MacOS/Loki"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / d / "Loki.exe" for d in ("win-unpacked", "win-ia32-unpacked", "win-arm64-unpacked")
        ]
    else:
        candidates = [
            release_dir / d / n for d in ("linux-unpacked", "linux-arm64-unpacked") for n in ("loki", "Loki")
        ]
    ...
    if sys.platform == "win32" and len(existing) > 1:
        ...
        # mtime can hand a wrong-architecture Loki.exe to the launcher. Prefer
        ...
        # wrong-architecture Loki.exe to the launcher, which Windows rejects with "This app can't run on
        # your computer" (#69179).
...
# PLACE: before-pack.mjs wipes ``release/<platform>- unpacked`` (or the mac ``Loki.app``) and the Electron
# unpack + asar + rename then rebuild it. Any failure after that wipe — corrupt cached zip, blocked
# download, missing dep, disk full — leaves the user with NO app, and ``loki update`` used to report
...
# (Desktop → loki-setup --update → `loki update` → `loki desktop --build-only` → relaunch) rebuilds
# Loki.exe on the end user's machine and used to verify only that the file EXISTS before declaring
...
# Loki.exe is caught BEFORE the updater replaces the working app, and the previous build can be restored
...
def _windows_native_machine_from_iswow64() -> Optional[str]:
    ...
    emulated process arch) and rejected a correctly-built ARM64 ``Loki.exe``. Binding
...
def _windows_native_machine() -> str:
    ...
    chain runs an x64 loki-setup.exe (and thus x64 Python) on Windows-on-ARM devices, where
...
def _ensure_desktop_exe_launchable(desktop_dir: Path, packaged_executable: Optional[Path]) -> tuple:
    ...
    print(f"✗ The built Loki.exe failed its integrity check: {error}\n    at: {packaged_executable}")
    ...
    if restored is not None:
        print("  ↩ Update aborted — restored the previous working Loki.exe from backup.")
        print("    Your existing version was kept and still works. Run `loki desktop`")
        print("    (or the in-app update) again to retry with a fresh Electron download.")
        return restored, True

    print("  ✗ No usable backup was found to restore.")
    print("    Run `loki desktop --force-build` to rebuild, or re-run the Loki")
...
def _redownload_electron_dist(project_root: Path, env: dict, *, mirror: Optional[str] = None) -> bool:
    ...
    if not installer.is_file():
        return False
    from loki_constants import find_node_executable, with_loki_node_path
    ...
    dl_env = with_loki_node_path(env)
    if mirror:
...
def _desktop_macos_relaunchable_fixup(
    ...
    "Loki is damaged" and loses every grant. Clear quarantine xattrs, then sign
    ...
    if exe is None:
        return True
    # exe = .../Loki.app/Contents/MacOS/Loki  ->  app bundle = .../Loki.app
    app = exe.parents[2]
...
def _macos_create_signing_identity(
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="loki-tcc-"))
    try:
        ...
        def _export_p12(extra_args: list) -> None:
            subprocess.run(
                [
                    ...
                    "-out", str(p12), "-passout", "pass:lokilocal",
                ],
                capture_output=True, check=True)
...
def _desktop_macos_setup_tcc_identity(identity: str = "Loki Local Signing") -> bool:
    ...
    print(
        ...
        "stuck, reset it with:  tccutil reset All com.wundercorp.lokiagent"
    )
    return True
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Loki Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Loki Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _register_linux_desktop_entry() -> None:
    """Install the XDG desktop entry for Loki Desktop (Linux only, best-effort).

    ``Exec`` and ``Icon`` are absolute so the entry works outside a login shell.
    ``loki uninstall --gui`` removes it.
    """
    from loki_cli.main import PROJECT_ROOT
    try:
        from loki_cli.linux_desktop_entry import install_desktop_entry, is_supported
        if not is_supported():
            return
...
def _install_desktop_workspace_deps(npm: str, env: dict) -> None:
    ...
    from loki_cli.main import PROJECT_ROOT
    from loki_cli.main_web_build import _run_npm_install_deterministic
    ...
    nixos_env = with_loki_node_path(_nixos_build_env())
...
def _run_desktop_pack_with_recovery(
    ...
    if build_result.returncode != 0 and staging_dir is not None and _staged_exe() is None:
        ...
        if restored:
            ...
            for p in purged:
                print(f"    - {p}")
            # The purge can't remove a win-unpacked tree whose Loki.exe is
...
def _promote_staged_desktop_app(desktop_dir: Path, staging_dir: Path) -> Path:
    ...
    # Loki.exe Windows cannot load. Verified on the STAGED exe, so a failure
...
def _build_desktop_app(desktop_dir: Path, *, source_mode: bool, npm: str, env: dict) -> Optional[Path]:
    ...
    if not source_mode:
        ...
        # A running desktop instance holds Loki.exe locked on Windows, so the
    ...
    if build_result.returncode != 0:
        ...
        if sys.platform == "win32":
            print("  If this says \"Access is denied\" on Loki.exe, close any")
            print("  running Loki desktop window and retry.")
        print("  If the log shows Electron download retries, rebuild via a mirror:")
        print("    ELECTRON_MIRROR=<mirror-base-url> loki desktop --force-build")
...
def _desktop_launch_env(args: argparse.Namespace) -> tuple[dict, list[str]]:
    ...
    from loki_constants import with_loki_node_path
    # with_loki_node_path() copies os.environ when called with no arg.
    env = with_loki_node_path()
    _prefer_wsl_d3d12(env)
    for attr, key in (
        ("fake_boot", "LOKI_DESKTOP_BOOT_FAKE"), ("ignore_existing", "LOKI_DESKTOP_IGNORE_EXISTING")):
        if getattr(args, attr, False):
            env[key] = "1"
    if getattr(args, "loki_root", None):
        env["LOKI_DESKTOP_ROOT"] = str(Path(args.loki_root).expanduser().resolve())
    cwd = getattr(args, "cwd", None)
    env["LOKI_DESKTOP_CWD"] = str(Path(cwd).expanduser().resolve()) if cwd else os.getcwd()

    config_electron_flags, config_disable_gpu, config_password_store, config_ozone_hint = (
        _desktop_launch_options())
    if config_disable_gpu != "auto" and "LOKI_DESKTOP_DISABLE_GPU" not in os.environ:
        env["LOKI_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if sys.platform == "linux" and "LOKI_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
            config_password_store if config_password_store != "auto" else _detect_linux_password_store()
        )
        if password_store:
            env["LOKI_DESKTOP_PASSWORD_STORE"] = password_store
...
def cmd_gui(args: argparse.Namespace):
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "Loki Local Signing"
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  loki gui")
            sys.exit(1)
    ...
    if source_mode:
        print("→ Launching Loki Desktop from source build...")
    ...
    if not source_mode:
        print(f"→ Launching packaged Loki Desktop: {' '.join(launch_command)}")
