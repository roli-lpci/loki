...
from loki_cli.web_server_memory import _normalize_memory_provider_name
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `loki egress setup` "
            "and `loki egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
    ...
    "updates.non_interactive_local_changes": _select(
        "When the chat app / gateway updates Loki (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "boolean",
        "description": (
            "Refresh an already-installed cua-driver during loki update. "
...
# wundercorp, telegram) currently surface a single schema field each.
...
def _schema_with_dynamic_provider_options() -> Dict[str, Dict[str, Any]]:
    ...
    """
    from loki_cli.web_server_profiles import _plugin_terminal_backend_rows
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    1. Vendor-name → Loki-provider: when the provider is not a known provider/alias but the
    ...
    from loki_cli.models import _AGGREGATOR_PROVIDERS, _KNOWN_PROVIDER_NAMES, normalize_provider
    from loki_cli.model_normalize import normalize_model_for_provider
    from loki_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and not is_custom_provider_slug and "/" in model_in:
        ...
        if cur_provider and normalize_provider(cur_provider) in _AGGREGATOR_PROVIDERS:
            ...
            from loki_cli.models_detect import provider_has_credentials
...
# in loki_cli/config.py — listed here for deterministic ordering in the UI.
...
def _dashboard_code_skew_guard() -> Optional[str]:
    ...
    Long-lived dashboard / Desktop-owned ``loki serve`` processes freeze ``sys.modules``
    at boot; after ``loki update`` replaces the checkout, a first-time lazy import can
...
def _dashboard_skew_restart_hint() -> str:
    ...
    dashboard and Desktop-owned ``loki serve``; naming a systemd unit would mislead
    ...
    if os.environ.get("LOKI_SERVE_HEADLESS") == "1":
        return (
            ...
            "(use Restart backend in Loki Desktop, or quit and reopen the app)"
        )
    return (
        "restart this Loki process to load the new code "
        "(loki dashboard --port <port>, or the equivalent service restart for this install)"
    )
...
def _apply_wundercorp_gateway_defaults(cfg: dict) -> list:
    """Mirror the CLI's post-model-selection behaviour when switching main to WunderCorp: route
    *unconfigured* tools through the WunderCorp Tool Gateway. Purely additive — tools with a direct
    key or explicit backend are skipped. Failures never block saving the assignment."""
    try:
        from loki_cli.wundercorp_subscription import apply_wundercorp_managed_defaults
        ...
        return sorted(apply_wundercorp_managed_defaults(cfg, enabled_toolsets=enabled, force_fresh=True))
    except Exception:
        _log.debug("apply_wundercorp_managed_defaults skipped", exc_info=True)
        return []
...
def _register_custom_endpoint(base_url: str, api_key: str, model: str) -> None:
    ...
    ``loki model`` custom flow) so the picker gets a proper ready row instead of a "needs
    setup" dead-end. Dedups by base_url; never blocks the already-persisted assignment."""
    try:
        from loki_cli.main_provider_setup import _auto_provider_name, _save_custom_provider
...
def _apply_main_assignment_sync(cfg: dict, provider: str, model: str, base_url: str, api_key: str) -> dict:
    ...
    new_provider = provider.strip().lower()
    gateway_tools = _apply_wundercorp_gateway_defaults(cfg) if new_provider == "wundercorp" else []
    save_config(cfg)
...
def _apply_model_assignment_sync(
    ...
    """Synchrowundercorp body of POST /api/model/set.
...
def _infer_provider_on_model_change(model_val: str, prev_provider: str) -> tuple[str, str]:
    ...
    if not name:
        return "", name
    try:
        from loki_cli.models import _AGGREGATOR_PROVIDERS, detect_provider_for_model, normalize_provider
    except Exception:
        return "", name
