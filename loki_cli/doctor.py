"""``loki doctor`` — diagnose (and with --fix, repair) a Loki install.
...
from loki_cli.config import get_env_path, get_loki_home, get_project_root
from loki_cli.env_loader import load_loki_dotenv
from loki_constants import display_loki_home

PROJECT_ROOT = get_project_root()
LOKI_HOME = get_loki_home()
_DHH = display_loki_home()  # user-facing display path (e.g. ~/.loki or ~/.loki/profiles/coder)

# Load environment variables from ~/.loki/.env so API key checks work
_env_path = get_env_path()
load_loki_dotenv(loki_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from loki_cli.colors import Colors, color
from loki_cli.doctor_report import Finding, _section, check_bool, check_info, doctor_check, warn_on_error
from loki_cli.doctor_connectivity import _has_healthy_oauth_fallback_for_apikey_provider, build_probes, run_probes
from loki_cli.doctor_tools import _safe_which

from loki_cli.doctor_config import (
    _check_config_drift,
    _check_config_file,
    _check_env_file,
    _check_mcp_security,
...
from loki_cli.doctor_platform import (
    _check_certificates,
...
from loki_cli.doctor_tools import (
    _check_git_and_rg,
...
from loki_cli.doctor_state import (
...
_PROVIDER_ENV_HINTS = (
    ...
    "OPENAI_BASE_URL", "WUNDERCORP_API_KEY", "GLM_API_KEY", "ZAI_API_KEY", "Z_AI_API_KEY", "KIMI_API_KEY",
...
def _check_auth_providers(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("Auth provider status", "(could not check: {e})"):
        from loki_cli.auth import get_wundercorp_auth_status_local, get_codex_auth_status, get_minimax_oauth_auth_status
        _login_row("WunderCorp Portal auth", get_wundercorp_auth_status_local())
        # Native OAuth is Loki' own device-code flow; the Codex CLI only imports existing ~/.codex/auth.json
    ...
    with warn_on_error(""):  # xAI OAuth separately, so an import failure cannot disrupt the rows already printed
        from loki_cli.auth import get_xai_oauth_auth_status
...
def _ack_advisory(ack_target: str) -> None:
    """`loki doctor --ack <id>`: persist the ack and return without running diagnostics."""
    from loki_cli.security_advisories import ADVISORIES, ack_advisory
    ...
    if ack_advisory(ack_target):
        ...
        print(color(f"  ✗ Failed to persist ack for {ack_target}. Check ~/.loki/config.yaml is writable.", Colors.RED))
        sys.exit(1)
...
def _print_summary(should_fix: bool, total: Finding) -> None:
    ...
    if should_fix and total.fixed > 0:
        ...
        print(numbered)
        if not should_fix:
            print(color("  Tip: run 'loki doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
...
def run_doctor(args):
    ...
    os.environ.setdefault("LOKI_INTERACTIVE", "1")
    ...
    with warn_on_error(""):
        from loki_cli.doctor_live import maybe_run_live_checks
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'OPENROUTER_MODELS_URL': ('loki_constants', 'OPENROUTER_MODELS_URL'),
    'STATE_DB_SIZE_WARN_BYTES': ('loki_cli.doctor_state', 'STATE_DB_SIZE_WARN_BYTES'),
    'agent_browser_runnable': ('loki_constants', 'agent_browser_runnable'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'check_certificates': ('loki_cli.doctor_platform', 'check_certificates'),
    'check_macos_full_disk_access': ('loki_cli.doctor_platform', 'check_macos_full_disk_access'),
    'check_macos_tcc_anchor': ('loki_cli.doctor_platform', 'check_macos_tcc_anchor'),
    'check_macos_tcc_grants': ('loki_cli.doctor_platform', 'check_macos_tcc_grants'),
    'collect_deprecated_config_keys': ('loki_cli.doctor_config', 'collect_deprecated_config_keys'),
    'collect_deprecated_env_vars': ('loki_cli.doctor_config', 'collect_deprecated_env_vars'),
    'collect_relay_plugin_cutover_findings': ('loki_cli.doctor_config', 'collect_relay_plugin_cutover_findings'),
    'describe_vercel_auth': ('loki_cli.vercel_auth', 'describe_vercel_auth'),
    'detect_install_method': ('loki_cli.config', 'detect_install_method'),
    'is_nix_install_method': ('loki_cli.config', 'is_nix_install_method'),
    'managed_scope_check': ('loki_cli.doctor_config', 'managed_scope_check'),
    'recommended_update_command_for_method': ('loki_cli.config', 'recommended_update_command_for_method'),
    'report_deprecated_config_and_env': ('loki_cli.doctor_config', 'report_deprecated_config_and_env'),
}
