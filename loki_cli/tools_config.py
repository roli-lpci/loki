"""Unified tool configuration for Loki Agent."""
...
from loki_cli.cli_output import print_info as _print_info
from loki_cli.colors import Colors, color
from loki_cli.config import cfg_get, load_config, save_config, get_env_value
from loki_cli.wundercorp_subscription import (
    WunderCorpSubscriptionFeatures, apply_wundercorp_managed_defaults, get_wundercorp_subscription_features)
from loki_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
from loki_cli.toolset_scope import (
    _TOOLSET_PLATFORM_RESTRICTIONS, toolset_allowed_for_platform as _toolset_allowed_for_platform)
# Re-exports: keep ``loki_cli.tools_config.X`` callers and test patch targets resolving.
from loki_cli.tools_config_cua import (  # noqa: F401
...
from loki_cli.tools_config_post_setup import (  # noqa: F401
...
from loki_cli.tools_config_providers import (  # noqa: F401
...
from loki_cli.tools_config_mcp import (  # noqa: F401
...
CONFIGURABLE_TOOLSETS = [
    ...
    ("typesafe",        "⚡ TypeSafe Jev decision tool", "typed decisions: Choice, Score, Noul; router: /jev"),
...
def _xai_credentials_present() -> bool:
    ...
    try:
        from loki_cli.auth import _read_xai_oauth_tokens
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from loki_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        ...
        from loki_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Toolset keys the ``loki tools`` checklist offers for ``platform`` (mirrors ``_prompt_toolset_checklist``);
...
def _platform_default_toolset(platform: str) -> str:
    """Composite toolset a platform falls back to (plugin platforms derive ``loki-<platform>``)."""
    return PLATFORMS[platform]["default_toolset"] if platform in PLATFORMS else f"loki-{platform}"
...
def _row(name: str, badge: str = "", tag: str = "", env_vars: list = (), **markers) -> dict:
    """One TOOL_CATEGORIES provider row; ``markers`` are the ``*_provider`` / ``post_setup`` / WunderCorp keys."""
    row = {"name": name}
    if badge:
        row["badge"] = badge
    if tag:
...
_WUNDERCORP = {"requires_wundercorp_auth": True}
...
_LANGFUSE_PUBLIC = ("LOKI_LANGFUSE_PUBLIC_KEY", "Langfuse public key (pk-lf-...)")
_LANGFUSE_SECRET = ("LOKI_LANGFUSE_SECRET_KEY", "Langfuse secret key (sk-lf-...)")
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            ...
            _row("WunderCorp Subscription", "subscription", "Managed OpenAI TTS billed to your subscription", tts_provider="openai",
                 **_WUNDERCORP, managed_wundercorp_feature="tts", override_env_vars=["VOICE_TOOLS_OPENAI_KEY", "OPENAI_API_KEY"]),
    ...
    "stt": {
        ...
        "providers": [
            ...
            _row("WunderCorp Subscription", "subscription", "Managed OpenAI transcription billed to your subscription",
                 stt_provider="openai", **_WUNDERCORP, managed_wundercorp_feature="stt",
    ...
    "web": {
        ...
        # non-provider firecrawl setup-flow rows live here: managed via WunderCorp subscription, and self-hosted.
        "providers": [
            {"name": "WunderCorp Subscription", "badge": "subscription", "tag": "Managed Firecrawl billed to your subscription",
             "web_backend": "firecrawl", "env_vars": [], **_WUNDERCORP, "managed_wundercorp_feature": "web",
    ...
    "image_gen": {
        ...
        # _plugin_image_gen_providers(). Only the managed "WunderCorp Subscription" row lives here — fal backend, distinct UX.
        "providers": [
            _row("WunderCorp Subscription", "subscription", "Managed FAL image generation billed to your subscription", **_WUNDERCORP,
                 managed_wundercorp_feature="image_gen", override_env_vars=["FAL_KEY"], imagegen_backend="fal"),
        ],
    },
    "video_gen": {
        "name": "Video Generation", "icon": "🎬",
        # Mirrors image_gen: managed FAL video billed via the WunderCorp Portal. Plugin-backed rows (FAL BYOK, xAI, …)
        ...
        "providers": [
            _row("WunderCorp Subscription", "subscription", "Managed FAL video generation billed to your subscription", **_WUNDERCORP,
                 managed_wundercorp_feature="video_gen", override_env_vars=["FAL_KEY"], video_gen_plugin_name="fal"),
        ],
    },
    "x_search": {
        "name": "X (Twitter) Search", "setup_title": "Select xAI Credential Source",
        "setup_note": (
            "Loki routes X searches through xAI's built-in x_search Responses tool for read-only public X "
    ...
    "browser": {
        ...
        # first so a fresh install's Enter lands on the free local backend (index 0), never on the paid WunderCorp row.
        ...
        "providers": [
            ...
            _row("Lightpanda", "free · local · no Chromium", "Zig headless browser spawned by Loki, text-only (no screenshots)",
            ...
            _row("WunderCorp Subscription (Browser Use cloud)", "subscription", "Managed Browser Use billed to your subscription",
                 browser_provider="browser-use", **_WUNDERCORP, managed_wundercorp_feature="browser",
    ...
    "computer_use": {
        ...
        # cua-driver reads HOME/TMPDIR from the process env; LOKI_CUA_DRIVER_CMD selects a specific
        ...
        "providers": [
            ...
            _row("Langfuse Self-Hosted", tag="Self-hosted Langfuse instance", post_setup="langfuse",
                 env_vars=[_key(*_LANGFUSE_PUBLIC), _key(*_LANGFUSE_SECRET),
                           _key("LOKI_LANGFUSE_BASE_URL", "Langfuse server URL (e.g. http://localhost:3000)", default="http://localhost:3000")]),
        ],
    },
}
...
def enabled_mcp_server_names(config: dict) -> Set[str]:
    ...
    try:
        from loki_cli.plugins import get_portable_mcp_server_names_nowait
...
#: "declined": saving ``loki tools`` freezes a platform's composite into an explicit list nothing adds to, so
#: a later toolset stays off forever for picker users while ``[loki-cli]`` users inherit it.
...
_RECENTLY_SHIPPED_TOOLSETS: frozenset = frozenset({"typesafe"})
...
def _enable_recently_shipped_toolsets(enabled_toolsets: Set[str], config: dict, platform: str) -> None:
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # composites (loki-acp, loki-webhook) stay narrow.
...
def _explicit_toolsets(
    ...
    ``loki-cli`` would re-enable disabled toolsets). A mixed list (``[loki-cli, spotify]``) still expands the
...
def _get_platform_tools(config: dict, platform: str, *, include_default_mcp_servers: bool = True) -> Set[str]:
    ...
    # An explicitly saved list (even a composite like ``loki-discord``) is an opt-in to the platform's
    ...
    # Plugin toolsets are first-class on a saved list: ``[loki-cli, a2a]`` must survive filtering.
    # Plugin-provided toolsets are first-class on a platform-toolsets list — explicit config like
    # ``[loki-cli, a2a]`` must survive filtering just like a built-in configurable toolset would. See
    # issue #81163.
    ...
    # above. It may arrive as a JSON-array string ("['memory']") from `loki config set` or a JSON-mode editor.
...
def _prune_toolsets_stripped_by_disabled(enabled_toolsets: Set[str], disabled_names: List[str]) -> Set[str]:
    ...
    never appear in the list. A name-only subtraction here left inspection surfaces (``loki tools
...
def _recover_platform_native_toolsets(enabled_toolsets: Set[str], platform: str, *, skip: Set[str]) -> None:
    ...
    skip = skip | {k for k in TOOLSETS if k.startswith("loki-")} | (set(_DEFAULT_OFF_TOOLSETS) - {platform})
...
def _warn_all_invalid_platform_toolsets(platform: str, explicit: list) -> None:
    """Warn once when an explicit platform list has only invalid names (``loki`` for ``loki-cli`` → no
    ...
    if named and not any(validate_toolset(t) for t in named) and platform not in _warned_invalid_platform_toolsets:
        ...
        logger.warning(
            ...
            "name(s): %s) - tools will be unavailable. Run `loki tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # names): platform defaults (loki-cli, ...) resolve to ALL tools and would silently override the user's
...
def _toolset_has_keys(
    ts_key: str, config: dict = None, *, force_fresh: bool = False, features: Optional[WunderCorpSubscriptionFeatures] = None,
) -> bool:
    ...
    if ts_key in {"web", "image_gen", "video_gen", "tts", "stt", "browser"}:
        ...
        if feature and (feature.available or feature.managed_by_wundercorp):
            return True
...
def _prompt_choice(question: str, choices: list, default: int = 0) -> int:
    ...
    from loki_cli.curses_ui import curses_radiolist
...
def _prompt_toolset_checklist(platform_label: str, enabled: Set[str], platform: str = "cli", *, force_fresh: bool = True) -> Set[str]:
    ...
    from loki_cli.curses_ui import curses_checklist
    from toolsets import resolve_toolset
...
def _shared_metrics_menu_label(config: dict) -> str:
    ...
    state = "off" if not enabled else ("collecting + sending to WunderCorp" if send else "collecting locally")
...
def _configure_shared_metrics_interactive(config: dict) -> None:
    ...
    from loki_cli.setup import setup_telemetry
...
def _print_tools_summary(config: dict, enabled_platforms: List[str]) -> None:
    """``loki tools --summary``: enabled toolsets per platform, non-interactive."""
    total = len(_get_effective_configurable_toolsets())
...
def _first_install_flow(config: dict, enabled_platforms: List[str]) -> None:
    ...
    for pkey in enabled_platforms:
        ...
        auto_configured = apply_wundercorp_managed_defaults(config, enabled_toolsets=new_enabled, force_fresh=True)
        for ts_key in sorted(auto_configured):
            label = next((l for k, l, _ in CONFIGURABLE_TOOLSETS if k == ts_key), ts_key)
            print(color(f"  ✓ {label}: using your WunderCorp subscription defaults", Colors.GREEN))
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `loki tools` / `loki setup tools`. ``first_install`` skips the menu (checklist + key
    ...
    print(color(f"  Tool configuration saved to {display_loki_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'loki' or gateway restart.", Colors.DIM))
    print()
...
_PLUGIN_COMPAT_LAZY = {
    'MANAGED_FEATURE_COVERAGE_CATEGORY': ('loki_cli.wundercorp_subscription', 'MANAGED_FEATURE_COVERAGE_CATEGORY'),
    'WUNDERCORP_MANAGED_PROVIDER': ('tools.tool_backend_helpers', 'WUNDERCORP_MANAGED_PROVIDER'),
    'base_url_hostname': ('utils', 'base_url_hostname'),
    'fal_key_is_configured': ('tools.tool_backend_helpers', 'fal_key_is_configured'),
    'format_wundercorp_portal_entitlement_message': ('loki_cli.wundercorp_account', 'format_wundercorp_portal_entitlement_message'),
