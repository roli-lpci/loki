...
from loki_cli.model_setup_flows_common import _ask, _ensure_dict_section, _print_numbered, _radiolist, _say
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.loki/.env unless the active provider is 'custom' — a
    leftover value routes provider:auto auxiliary clients to the old custom endpoint."""
    from loki_cli.config import get_env_value, save_env_value, load_config
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    (:meth:`loki_cli.plugins.PluginContext.register_auxiliary_task`)."""
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        _say("", "  Auxiliary models — side-task routing", "",
             ...
             "  Loki only falls back to a lightweight backend (OpenRouter,",
             "  WunderCorp Portal) if the main model is unavailable.  Override a",
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    with contextlib.suppress(Exception):
        from loki_cli.setup import _curses_prompt_choice
...
_CUSTOM_API_MODES = (
    ("", "Auto-detect", "Use Loki URL heuristics; best for standard OpenAI-compatible endpoints.",
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    from loki_cli.runtime_provider import _detect_api_mode_for_url
...
def _prompt_api_key(
    ...
    if choice.startswith("c") and not pool_backed:
        ...
        print(f"  API key cleared.  Re-run `loki setup` to configure {pconfig.name} again.")
        return "", True
...
def _stepfun_base_url_for_region(region: str) -> str:
    from loki_cli.auth import STEPFUN_STEP_PLAN_CN_BASE_URL, STEPFUN_STEP_PLAN_INTL_BASE_URL
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    from loki_cli.config import save_anthropic_oauth_token, use_anthropic_claude_code_credentials
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (is_claude_code_token_valid(creds) or bool(creds.get("refreshToken"))):
            ...
            from loki_constants import display_loki_home as _dhh_fn
            print(f"    Loki will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
    ...
    from loki_cli.config import get_compatible_custom_providers, read_raw_config
...
def _build_provider_picker_rows(config: dict, active: str, provider_labels: dict[str, str],
    ...
    """Rows for the ``loki model`` provider picker plus the pre-selected index. Canonical providers
    ...
    from loki_cli.models import CANONICAL_PROVIDERS, _PROVIDER_ALIASES
    from loki_cli.models_catalog_static import group_providers, provider_group_for_slug
