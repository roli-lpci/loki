"""Shell completion script generation for loki CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_loki_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;")
            ...
            cases.append(
                ...
                f"            ;;")
    cases_str = "\n".join(cases)
    return f"""# Loki Agent bash completion
# Add to ~/.bashrc:
#   eval "$(loki completion bash)"

_loki_profiles() {{
    local profiles_dir="$HOME/.loki/profiles"
...
_loki_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_loki_profiles)" -- "$cur"))
        return
    fi
...
complete -F _loki_completion loki
"""
    ...
    for cmd, info in subcommands:
        ...
        if cmd == "profile":
            ...
            sub_cases.append(
                ...
                f"                    ;;")
    sub_cases_str = "\n".join(sub_cases)
    return f"""#compdef loki
# Loki Agent zsh completion
# Add to ~/.zshrc:
...
_loki_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.loki/profiles" ]]; then
        profiles+=($HOME/.loki/profiles/*(N/:t))
    fi
...
_loki() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_loki_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'loki command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Loki Agent fish completion",
        "# Add to your config:",
        "#   loki completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __loki_profiles",
        "    echo default",
        "    if test -d $HOME/.loki/profiles",
        "        for d in $HOME/.loki/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c loki -f -s p -l profile"
        " -d 'Profile name' -xa '(__loki_profiles)'",
        "",
        ...
        if cmd == "profile":  # profile names for the actions that take one
            for action in sorted(_PROFILE_NAME_ACTIONS):
                lines.append(
                    ...
                    f"-a '(__loki_profiles)' -d 'Profile name'")
    lines.append("")
    return "\n".join(lines)
