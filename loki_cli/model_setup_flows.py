"""Per-provider model-selection wizard flows for ``loki setup`` / ``loki model``.

main / config / auth / models helpers are imported lazily inside bodies: avoids the main.py import
cycle and lets tests patch ``loki_cli.config.load_config`` etc. at call time. The shared skeleton
lives in :mod:`loki_cli.model_setup_flows_common`; the custom / Azure / Bedrock flows live in their
...
from loki_cli.model_setup_flows_common import (
...
from loki_cli.model_setup_flows_custom import _model_flow_custom, _model_flow_named_custom
from loki_cli.model_setup_flows_azure import _model_flow_azure_foundry
from loki_cli.model_setup_flows_bedrock import _model_flow_bedrock
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from loki_cli.auth import ProviderConfig, _prompt_model_selection
    ...
    from loki_cli.models import model_ids
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from loki_cli.main_provider_setup import _prompt_api_key
    from loki_cli.auth import PROVIDER_REGISTRY, _prompt_model_selection
    from loki_cli.config import get_env_value
    ...
    from loki_cli.models import ai_gateway_model_ids
    from loki_cli.models_pricing import get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    from loki_cli.auth import _save_model_choice
    from loki_cli.moa_config import normalize_moa_config
    moa = normalize_moa_config(config.get("moa") if isinstance(config, dict) else {})
    presets = moa.get("presets") or {}
    if not presets:
        print("No MoA presets configured. Run `loki moa configure <name>` first.")
        return
...
def _wundercorp_login_args(args) -> argparse.Namespace:
...
def _wundercorp_model_catalog(free_tier: bool, portal_url: str, model_ids: list, pricing: dict):
    """Free/paid-tier catalog for the WunderCorp picker: ``(model_ids, pricing, unavailable_models,
    ...
    if free_tier:
        try:
            from loki_cli.wundercorp_account import format_wundercorp_portal_entitlement_message, get_wundercorp_portal_account_info
            _account_info = get_wundercorp_portal_account_info(force_fresh=True)
            unavailable_message = format_wundercorp_portal_entitlement_message(_account_info, capability="paid WunderCorp models") or ""
        except Exception:
            unavailable_message = ""
    ...
    if free_tier:
        model_ids, unavailable_models = partition_wundercorp_models_by_tier(model_ids, pricing, free_tier=True)

    if not model_ids and not unavailable_models:
        print("No models available for WunderCorp Portal after filtering.")
        return None
    if free_tier and not model_ids:
        print("No free models currently available.")
        if unavailable_models:
            from loki_cli.auth import DEFAULT_WUNDERCORP_PORTAL_URL
            _url = (portal_url or DEFAULT_WUNDERCORP_PORTAL_URL).rstrip("/")
            print(unavailable_message or f"Upgrade at {_url} to access paid models.")
        return None
    return model_ids, pricing, unavailable_models, unavailable_message, _policy_narrowed


def _wundercorp_verified_credentials(creds_or_none=None):
    """Resolve WunderCorp runtime credentials; on failure print the diagnosis (re-login when the
    session expired) and return None."""
    from loki_cli.auth import (
        AuthError, PROVIDER_REGISTRY, _login_wundercorp, format_auth_error, resolve_wundercorp_runtime_credentials)

    try:
        return resolve_wundercorp_runtime_credentials()
    except Exception as exc:
        relogin = isinstance(exc, AuthError) and exc.relogin_required
        msg = format_auth_error(exc) if isinstance(exc, AuthError) else str(exc)
        if relogin:
            _say(f"Session expired: {msg}", "Re-authenticating with WunderCorp Portal...\n")
            try:
                _login_wundercorp(_wundercorp_login_args(None), PROVIDER_REGISTRY["wundercorp"])
...
def _wundercorp_persist_selection(selected: str, creds: dict) -> dict:
    """WunderCorp persist step: model choice + provider state, then rewrite ``model`` on a fresh
    ...
    from loki_cli.auth import _save_model_choice, _update_config_for_provider
    from loki_cli.config import get_env_value, load_config, save_config, save_env_value
    _save_model_choice(selected)
    inference_url = creds.get("base_url", "")
    _update_config_for_provider("wundercorp", inference_url)
    config = load_config()
...
def _model_flow_wundercorp(config, current_model="", args=None):
    """WunderCorp Portal provider: ensure logged in, then pick model."""
    from loki_cli.auth import get_provider_auth_state, _prompt_model_selection, _login_wundercorp, PROVIDER_REGISTRY
    from loki_cli.config import load_config
    from loki_cli.wundercorp_subscription import prompt_enable_tool_gateway
    state = get_provider_auth_state("wundercorp")
    if not state or not state.get("access_token"):
        _say("Not logged into WunderCorp Portal. Starting login...", "")

        def _login_then_offer_gateway(login_args, pconfig):
            _login_wundercorp(login_args, pconfig)
            # Offer Tool Gateway enablement for paid subscribers
            with contextlib.suppress(Exception):
                prompt_enable_tool_gateway(load_config() or {})

        # login_wundercorp already handles model selection + config update
        _run_login(_login_then_offer_gateway, _wundercorp_login_args(args), PROVIDER_REGISTRY["wundercorp"])
        return
    ...
    from loki_cli.models import check_wundercorp_free_tier, get_curated_wundercorp_model_ids
    from loki_cli.models_pricing import get_pricing_for_provider
    from loki_cli.model_switch_providers import _free_tier_wundercorp_row
    tier_row = _free_tier_wundercorp_row({"name": "WunderCorp Portal", "models": []})
    if tier_row is None:
        print("The WunderCorp free tier is off for this install; sign in with `loki auth upgrade` to use WunderCorp models.")
        return
    if tier_row["models"]:
        # Free-tier identity: the welcome host serves the single pinned model; no Portal catalog,
        # pricing, or account lookups apply.
        creds = _wundercorp_verified_credentials()
        if creds is None:
            return
        selected = tier_row["models"][0]
        _wundercorp_persist_selection(selected, creds)
    ...
    if not free_tier:
        ...
        try:
            creds = resolve_wundercorp_runtime_credentials(force_refresh=True) or creds
        except Exception:
    ...
    _wundercorp_portal_url = ""
    with contextlib.suppress(Exception):
        _wundercorp_portal_url = (get_provider_auth_state("wundercorp") or {}).get("portal_base_url", "")

    catalog = _wundercorp_model_catalog(free_tier, _wundercorp_portal_url, model_ids, pricing)
    if catalog is None:
        return
    ...
    selected = _prompt_model_selection(
        ...
        portal_url=_wundercorp_portal_url, unavailable_message=unavailable_message, confirm_provider="wundercorp",
        confirm_base_url=creds.get("base_url", ""), confirm_api_key=creds.get("api_key", ""))
    if not selected:
        print("No change.")
        return
    config = _wundercorp_persist_selection(selected, creds)
    print(f"Default model set to: {selected} (via WunderCorp Portal)")
    # Offer Tool Gateway enablement for paid subscribers
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    # Prefer the credential pool (where `loki auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from loki_cli.models import provider_model_ids
    ...
    # credentials may live only in the pool (``loki auth add xai-oauth``) — fall back to
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from loki_cli.main_provider_setup import _DEFAULT_QWEN_PORTAL_MODELS
    from loki_cli.auth import (
        get_qwen_auth_status, resolve_qwen_runtime_credentials, _prompt_model_selection, DEFAULT_QWEN_BASE_URL)
    from loki_cli.models import fetch_api_models
...
def _model_flow_minimax_oauth(config, current_model="", args=None):
    ...
    from loki_cli.models import _PROVIDER_MODELS
...
def _copilot_catalog(api_key: str):
    ...
    from loki_cli.models import fetch_github_model_catalog, normalize_copilot_model_id
...
def _copilot_obtain_token() -> bool:
    ...
    if choice == "1":
        try:
            from loki_cli.copilot_auth import copilot_device_code_login
    ...
    if choice == "2":
        ...
        with contextlib.suppress(ImportError):
            from loki_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot(config, current_model=""):
    ...
    from loki_cli.main_provider_setup import _prompt_reasoning_effort_selection
    from loki_cli.setup import _current_reasoning_effort, _set_reasoning_effort
    from loki_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    from loki_cli.config import load_config
    from loki_cli.models import fetch_api_models, github_model_reasoning_efforts, copilot_model_api_mode
    provider_id = "copilot"
    ...
    if not api_key:
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from loki_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    _say("  GitHub Copilot ACP delegates Loki turns to `copilot --acp`.",
         "  Loki currently starts its own ACP subprocess for each request.",
         "  Loki uses your selected model as a hint for the Copilot ACP session.",
         f"  Command: {resolved_command}", f"  Backend marker: {effective_base}", "")
    try:
        creds = resolve_external_process_provider_credentials(provider_id)
    except Exception as exc:
        _say(f"  ⚠ {exc}", "  Set LOKI_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from loki_cli.auth import PROVIDER_REGISTRY, KIMI_CODE_BASE_URL
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from loki_cli.main_provider_setup import _infer_stepfun_region, _prompt_provider_choice, _stepfun_base_url_for_region
    from loki_cli.auth import PROVIDER_REGISTRY
    from loki_cli.config import save_env_value
    from loki_cli.models import _PROVIDER_MODELS, fetch_api_models
    provider_id = "stepfun"
    pconfig = PROVIDER_REGISTRY[provider_id]
    base_url_env = pconfig.base_url_env_var or ""
...
def _model_flow_vertex(config, current_model=""):
    ...
    from loki_cli.config import load_config, get_env_value
    from loki_cli.models import _PROVIDER_MODELS
    ...
    if sa_path:
        ...
        _say("  Vertex credentials: Application Default Credentials (ADC)",
             ...
             "      • set VERTEX_CREDENTIALS_PATH in ~/.loki/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    """Picker for the official Z.AI endpoints (``ZAI_ENDPOINTS`` in ``loki_cli.auth``, kept in
    sync with the probe list) plus a custom-proxy option. Returns the selected base URL;
    *current_base* on cancel/error."""
    from loki_cli.main_provider_setup import _prompt_provider_choice
    from loki_cli.auth import ZAI_ENDPOINTS
...
_GEMINI_FREE_TIER_NOTICE = (
    ...
    "   Loki typically makes 3-10 API calls per user turn (tool iterations + auxiliary tasks),",
    "   so the free tier is exhausted after a handful of messages and cannot sustain",
    "   an agent session.", "",
    "   To use Gemini with Loki, enable billing on your Google Cloud project and regenerate",
    "   the key in a billing-enabled project: https://aistudio.google.com/apikey", "",
    "   Alternatives with workable free usage: DeepSeek, OpenRouter (free models), Groq, WunderCorp.", "",
...
def _lmstudio_models(pconfig, curated, api_key, base_url):
    ...
    from loki_cli.models_local import fetch_lmstudio_models
    try:
...
def _ollama_cloud_models(pconfig, curated, api_key, base_url):
    ...
    from loki_cli.models import fetch_ollama_cloud_models
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from loki_cli.auth import PROVIDER_REGISTRY
    from loki_cli.config import save_env_value, load_config
    from loki_cli.models import opencode_model_api_mode, normalize_opencode_model_id
    pconfig = PROVIDER_REGISTRY[provider_id]
...
def _anthropic_authenticate() -> bool:
    ...
    from loki_cli.main_provider_setup import _run_anthropic_oauth_flow
    from loki_cli.config import save_env_value, save_anthropic_api_key
...
_PLUGIN_COMPAT_LAZY = {
    'BEDROCK_GEO_PREFIXES': ('loki_cli.model_setup_flows_bedrock', 'BEDROCK_GEO_PREFIXES'),
    'bedrock_model_routable_from_region': ('loki_cli.model_setup_flows_bedrock', 'bedrock_model_routable_from_region'),
    'bedrock_region_geo_prefix': ('loki_cli.model_setup_flows_bedrock', 'bedrock_region_geo_prefix'),
    'custom_provider_slug': ('loki_cli.providers', 'custom_provider_slug'),
