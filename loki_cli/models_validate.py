...
Split out of ``loki_cli.models``. Catalog fetchers defined in ``loki_cli.models`` are looked up
there at call time (``_m.<name>``) so ``patch("loki_cli.models.<name>")`` mocks keep intercepting;
local-server probes are looked up on ``loki_cli.models_local`` (``_ml.<name>``) the same way.
...
def _validate_moa(req: _Request) -> dict[str, Any]:
    try:
        ...
        if req.requested in cfg["presets"]:
            return _accept()
        return _reject(f"MoA preset `{req.requested}` was not found. Run `loki moa list`.")
...
def _validate_lmstudio(req: _Request) -> dict[str, Any]:
    from loki_cli import models_local as _ml
...
def _ollama_probe_headers(req: _Request) -> dict[str, str]:
    ...
    from loki_cli import models as _m
    from loki_cli import models_local as _ml
    from loki_cli.models_local import _configured_ollama_base_url, _drop_authorization
...
def _validate_ollama_native(req: _Request) -> Optional[dict[str, Any]]:
    ...
    if models is None:
        return _soft_accept(
            ...
            "Loki will save the model name, but local Ollama model discovery could not verify it."
        )
...
def _validate_custom(req: _Request) -> dict[str, Any]:
    ...
    message = (
        ...
        f"Loki will still save `{req.requested}`, but the endpoint should expose `/models` for verification."
    )
    if anthropic_style:
...
def _validate_static_catalog(req: _Request) -> Optional[dict[str, Any]]:
    ...
    if req.normalized == "openai-codex":
        ...
        # the suffix is a Loki picker convention, so an unknown `*-900k` can never be a real
...
def _validate_minimax(req: _Request) -> Optional[dict[str, Any]]:
    ...
    return match.verdict(req) or _soft_accept(
        ...
        f"{match.suggestion_text}"
        "\n  MiniMax does not expose a /models endpoint, so Loki cannot verify the model name."
...
def _wundercorp_portal_recommended_names() -> set[str]:
    """Lower-cased ids from the Portal's live recommended-models feed (empty on any failure)."""
    from loki_cli import models as _m

    try:
        payload = _m.fetch_wundercorp_recommended_models(_m._resolve_wundercorp_portal_url())
        return {
            name.lower()
...
def _validate_live_listing(req: _Request) -> Optional[dict[str, Any]]:
    ...
    # WunderCorp: the Portal's recommended-models feed can list a model before the curated list or the
    # docs-hosted manifest catches up; `loki chat` already accepts those at model-list build
    # time, so mirror that source of truth for per-message /model validation.
    if req.normalized == "wundercorp" and req.lookup.lower() in _wundercorp_portal_recommended_names():
        return _accept_with_note(f"Note: `{req.requested}` was not found in the live /v1/models listing "
                                 "but is a current WunderCorp Portal recommendation — accepted.")
