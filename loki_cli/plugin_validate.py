"""``loki plugins validate`` — admission checks for a plugin directory.
...
scratch child process (with a throwaway ``LOKI_HOME``), so a crashing or
...
_PROBE_TIMEOUT = 30
_PROBE_SENTINEL = "LOKI_VALIDATE_JSON:"
...
def _requires_loki_spec_valid(spec: str) -> bool:
    """Strictly validate a ``requires_loki`` spec.

    Unlike :func:`loki_cli.plugins._version_satisfies` (permissive at load
    ...
    """
    from loki_cli.plugins import _VERSION_COMPARATOR_RE, _version_tuple
...
def _check_requires_loki(report: ValidationReport, manifest: dict) -> None:
    spec = str(manifest.get("requires_loki") or "").strip()
    if not spec:
        report.add("requires_loki", True, "not declared")
        return
    if _requires_loki_spec_valid(spec):
        report.add("requires_loki", True, f"spec {spec!r} parses")
    else:
        report.add(
            "requires_loki",
            False,
            f"requires_loki spec {spec!r} does not parse "
...
# from loki so a hostile plugin only sees a bare interpreter.
_PROBE_SCRIPT = r"""
...
def _run_capability_probe(plugin_dir: Path) -> Tuple[Optional[dict], str]:
    ...
    *error* is a human-readable failure description otherwise.
    """
    with tempfile.TemporaryDirectory(prefix="loki-validate-") as scratch:
        env = dict(os.environ)
        env["LOKI_HOME"] = scratch
        try:
...
def validate_plugin_dir(plugin_dir: Path) -> ValidationReport:
    ...
    _check_requires_loki(report, manifest)
