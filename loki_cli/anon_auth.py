"""WunderCorp free-tier identity: the ``anonymous`` auth method of the ``wundercorp`` provider.

The identity is created in exactly one place, at boot (``loki_cli.free_tier_bootstrap``), and only
while ``LOKI_GUEST_ONBOARDING=1`` (see ``guest_enabled``). The bootstrap mints an anonymous WunderCorp
account (``POST /api/anonymous/create``); its ``anon_`` credential is later exchanged for short-lived
JWTs (``POST /api/anonymous/token``). The result is persisted as the singleton ``providers.wundercorp``; it
...
(the welcome inference host, single model ``wundercorp/welcome``).

Users are never shown the words guest / anonymous / account for this state: surfaces say
"WunderCorp · free tier". Two user-facing verbs reach the same flow, both keeping the identity's
connectors: ``loki auth upgrade`` in a terminal and ``/login`` inside a chat.
...
from loki_cli.auth_constants import (
    AuthError, DEFAULT_WUNDERCORP_PORTAL_URL, DEFAULT_WUNDERCORP_WELCOME_URL, _decode_jwt_claims, httpx)
...
GUEST_MODEL = "wundercorp/welcome"
...
ANON_SECRET_ENV = "LOKI_ANON_API_SECRET"
...
GUEST_ONBOARDING_ENV = "LOKI_GUEST_ONBOARDING"
...
FREE_TIER_LABEL = "WunderCorp · free tier"
UPGRADE_HINT = "Run `loki auth upgrade` to sign in with a WunderCorp account, or /login inside a chat."
FREE_TIER_NOT_SIGNED_IN = (
    "You're not signed in. Free inference and connectors are always on. "
    "Run `loki auth` to sign in with a WunderCorp account.")
...
def guest_enabled() -> bool:
    """The free tier is on for this process: the launch gate is set AND ``wundercorp.guest`` (default
    ...
    try:
        ...
        wundercorp_cfg = load_config_readonly().get("wundercorp")
    except Exception as exc:  # config unreadable: keep today's behaviour (no guest) rather than mint
        logger.debug("guest: config unreadable, treating wundercorp.guest as false: %s", exc)
        return False
    if not isinstance(wundercorp_cfg, dict):
        return True
    return bool(wundercorp_cfg.get("guest", True))
...
def current_wundercorp_state() -> Optional[Dict[str, Any]]:
    """The profile's ``providers.wundercorp`` state without locking or network (status/picker reads)."""
    from loki_cli.auth import _load_auth_store, _load_provider_state
    try:
        return _load_provider_state(_load_auth_store(), "wundercorp")
...
def has_guest() -> bool:
    return is_guest_state(current_wundercorp_state())


def guest_carries_inference() -> bool:
    """True when the profile's WunderCorp identity is the free tier and the free tier is on.
    ...
    credential-pool entry can pick a paid WunderCorp key while the profile singleton is still a guest.
    """
    return guest_enabled() and has_guest()


WELCOME_HOSTS = frozenset({"welcome-api.wundercorp.com"})


def pin_model_for_route(provider: Any, base_url: Any, model: Any) -> Any:
    """Model policy at agent START: on the WunderCorp welcome host the model is ``wundercorp/welcome``; anywhere
    ...
    """
    if provider == "wundercorp" and route_is_welcome_host(base_url):
        if model and model != GUEST_MODEL:
            logger.info("WunderCorp free tier: using %s instead of configured model %s", GUEST_MODEL, model)
        return GUEST_MODEL
    return model


def route_can_serve_model(provider: Any, base_url: Any, model: Any) -> bool:
    """Eligibility for a credential ROTATION: the welcome host serves only ``wundercorp/welcome``, so a
    conversation on any other model must not be rotated onto it (and a ``wundercorp/welcome`` conversation
    may move to the portal host, which serves it too). Non-WunderCorp routes are always eligible."""
    if provider != "wundercorp" or not route_is_welcome_host(base_url):
        return True
    return not model or model == GUEST_MODEL


def route_is_welcome_host(base_url: Any) -> bool:
    """The routing predicate for the free tier: the welcome host serves exactly ``wundercorp/welcome``.
...
def _raise_for_anon_status(response: httpx.Response, *, action: str) -> Dict[str, Any]:
    ...
    if response.status_code == 404 and error == "unknown_token":
        raise AnonCredentialDead("WunderCorp free-tier credential is no longer valid.", code="anon_credential_dead")
    if response.status_code == 401 and error == "invalid_shared_secret":
        raise _anon_err("WunderCorp free tier is not open on this portal.", "anon_gate_closed")
    if response.status_code == 401:
        raise AnonCredentialDead("WunderCorp free-tier credential was revoked.", code="anon_credential_dead")
    if response.status_code == 429:
        raise _anon_err("WunderCorp free tier is rate limited; try again shortly.", "anon_rate_limited")
    if response.status_code == 403 and error in {"anonymous_accounts_disabled", "circuit_open"}:
        raise _anon_err("WunderCorp free tier is currently disabled.", "anon_gate_closed")
    raise _anon_err(
        f"WunderCorp free tier {action} failed ({response.status_code}{': ' + error if error else ''}).",
        "anon_server_error")
...
def mint_guest(client: httpx.Client, portal_base_url: str) -> Dict[str, Any]:
    ...
    if not isinstance(token, str) or not token.startswith("anon_"):
        raise _anon_err("WunderCorp free tier sign-up returned no credential.", "anon_server_error")
    return payload
...
def exchange_anon_jwt(client: httpx.Client, portal_base_url: str, anon_token: str) -> Dict[str, Any]:
    ...
    if not isinstance(payload.get("access_token"), str) or not payload["access_token"]:
        raise _anon_err("WunderCorp free tier token exchange returned no token.", "anon_server_error")
    return payload
...
def apply_exchange_to_state(state: Dict[str, Any], exchanged: Dict[str, Any]) -> None:
    ...
    from loki_cli.auth_wundercorp import _validate_wundercorp_inference_url_from_network
    ...
    # (a staging host without WUNDERCORP_INFERENCE_BASE_URL set), the literal stands in. Never the paid
    # host: the gateway cross-refuses an anonymous JWT there.
    inference_url = (_validate_wundercorp_inference_url_from_network(exchanged.get("inference_base_url"))
                     or DEFAULT_WUNDERCORP_WELCOME_URL)
...
def _portal_base_url() -> str:
    from loki_cli.auth_wundercorp import _wundercorp_portal_env_override
    return (_wundercorp_portal_env_override() or DEFAULT_WUNDERCORP_PORTAL_URL).rstrip("/")


def _shared_identity_key(state: Any) -> Optional[str]:
    """Stable identity of a WunderCorp credential: the anon_ token for a guest, the refresh token for an
...
def _mint_locked(
    ...
    from loki_cli.auth import _store_provider_state, _save_auth_store
    from loki_cli.auth_wundercorp import _write_shared_wundercorp_state
    ...
    _store_provider_state(auth_store, "wundercorp", state, set_active=carries_inference)
    _save_auth_store(auth_store)
    _write_shared_wundercorp_state(state)
    logger.info("WunderCorp free tier ready (identity minted)")
    return state
...
def _reconcile_and_provision(*, timeout_seconds: float, carries_inference: bool = True) -> Optional[Dict[str, Any]]:
    ...
    1. The shared store is the identity of record for this Loki root. If it holds an identity
    ...
    from loki_cli.auth_wundercorp import (
        _wundercorp_http_client, _wundercorp_shared_store_lock, _read_shared_wundercorp_state, _write_shared_wundercorp_state)
    portal = _portal_base_url()
    with _auth_store_lock():
        auth_store = _load_auth_store()
        profile_state = _load_provider_state(auth_store, "wundercorp")
        with _wundercorp_shared_store_lock(timeout_seconds=max(timeout_seconds, 5.0)):
            shared = _read_shared_wundercorp_state()
            if shared and _shared_identity_key(shared) != _shared_identity_key(profile_state):
                state = dict(shared)
                _store_provider_state(
                    auth_store, "wundercorp", state,
                    set_active=carries_inference or not is_guest_state(state))
                _save_auth_store(auth_store)
                logger.debug("WunderCorp identity adopted from the shared store")
                return state
            if profile_state:
                if not shared:
                    _write_shared_wundercorp_state(profile_state)
...
def ensure_portal_identity(
    *, explicit: bool, timeout_seconds: float = GUEST_MINT_TIMEOUT_SECONDS,
    ...
    """Make sure this profile has a WunderCorp identity (guest or account); mint a guest only if the shared
    store has none. Returns the ``providers.wundercorp`` state, or None (disabled / failed once already).
    ...
    dead-credential replacements (``auth_wundercorp.resolve_wundercorp_runtime_credentials``,
    ...
    first, then shared, matching every other WunderCorp path. ``carries_inference=False`` leaves
    ...
    if not guest_enabled():
        return None
    if _mint_failed and not current_wundercorp_state():
...
def refresh_guest_state(state: Dict[str, Any], client: httpx.Client) -> None:
    ...
    if not isinstance(anon_token, str) or not anon_token:
        raise AnonCredentialDead("WunderCorp free-tier credential is missing.", code="anon_credential_dead")
    from loki_cli.auth import _wundercorp_portal_base_url
    apply_exchange_to_state(state, exchange_anon_jwt(client, _wundercorp_portal_base_url(state), anon_token))
...
def clear_dead_guest(reason: str, *, dead_token: Optional[str] = None) -> None:
    ...
    *dead_token* is None the profile's current guest is treated as the failed one.
    ...
    from loki_cli.auth_wundercorp import _clear_shared_wundercorp_state, _wundercorp_shared_store_lock, _read_shared_wundercorp_state
    with _auth_store_lock():
        auth_store = _load_auth_store()
        state = _load_provider_state(auth_store, "wundercorp")
        if is_guest_state(state):
            token = dead_token or state.get("anon_token")
            if state.get("anon_token") == token:
                _store_section(auth_store, "providers").pop("wundercorp", None)
                _store_section(auth_store, "credential_pool").pop("wundercorp", None)
                if auth_store.get("active_provider") == "wundercorp":
    ...
    global _mint_failed
    _mint_failed = False
    logger.info("WunderCorp free-tier identity retired (%s); a new one is set up on next use", reason)
...
# account that still asks for ``wundercorp/welcome`` is served the id's backing model and told what to
# switch to in the ``x-wundercorp-model-switch`` response header. Every rule for reading those lives here;
# the error classifier and the turn loop only call in.

MODEL_SWITCH_HEADER = "x-wundercorp-model-switch"
...
_WELCOME_ROUTE_REFUSALS = (
    ...
    ("serves anonymous loki agent accounts only", "named_on_welcome_host"),
    ("anonymous accounts are not accepted", "tier_disabled"),
)
_WELCOME_ROUTE_COPY = {
    "anon_on_paid_host": "The WunderCorp free tier must use its own inference host ({host}); "
                         "Loki is pointed at the paid one. Restart Loki to re-read the route, "
                         "or unset WUNDERCORP_INFERENCE_BASE_URL if you set it.",
    "named_on_welcome_host": "This WunderCorp account must use the WunderCorp Portal inference host, "
                             "not the free tier's. Run /model and pick the WunderCorp row again.",
    "tier_disabled": "The WunderCorp free tier is switched off right now. {signin}",
}
_SIGNIN_CHAT = "Sign in with a WunderCorp account for the full catalog: /login."
_SIGNIN_TERMINAL = "Sign in with a WunderCorp account for the full catalog: `loki auth upgrade`."
...
def welcome_refusal_copy(refusal: Dict[str, Any], *, model: str = "", in_chat: bool = True) -> str:
    ...
    if reason == "model_not_free":
        what = f"{model} isn't on the WunderCorp free tier" if model else "That model isn't on the WunderCorp free tier"
        return f"{what}; it serves {serves} only. {signin}"
    if reason == "feature_not_free":
        return f"This feature isn't on the WunderCorp free tier. {signin}"
    if reason == "at_capacity":
        return f"The WunderCorp free tier is at capacity and briefly paused. {wait} {signin}"
    if reason == "admission_closed":
        return f"The WunderCorp free tier isn't admitting new sessions right now. {wait} {signin}"
    if reason == "rate_limited":
        return f"WunderCorp free tier rate limit active \u2014 resets in {retry}s. {signin}"
    return f"The WunderCorp free tier refused this request ({reason}). {signin}"
...
def welcome_route_refusal_copy(kind: str, *, in_chat: bool = True) -> str:
    template = _WELCOME_ROUTE_COPY.get(kind) or "The WunderCorp inference gateway refused this route."
    return template.format(
        host=DEFAULT_WUNDERCORP_WELCOME_URL, signin=_SIGNIN_CHAT if in_chat else _SIGNIN_TERMINAL)


def note_model_switch(agent: Any, headers: Any) -> Optional[str]:
    """Record the gateway's ``x-wundercorp-model-switch`` header on *agent* for the next call, if present.

    The header arrives on a NAMED account's response that asked for ``wundercorp/welcome`` (the gateway
    ...
    if backing == requested:
        return None
    try:
        agent._wundercorp_pending_model_switch = (requested, backing)
    except Exception:
        return None
    return backing
...
def apply_model_switch(agent: Any) -> Optional[str]:
    ...
    map. The config write is the same one a sign-in completion uses, so ``loki model`` and the
    gateway's config re-read agree with the live session.
    """
    pending = getattr(agent, "_wundercorp_pending_model_switch", None)
    if not pending:
        return None
    agent._wundercorp_pending_model_switch = None
    ...
    agent._wundercorp_model_switch = (requested, backing)
    logger.info("WunderCorp gateway asked to switch %s -> %s; applied for this session", requested, backing)
    try:
        from loki_cli.config import load_config_readonly
        ...
        if str(model_cfg.get("default") or "").strip() == requested:
            from loki_cli.auth import _update_config_for_provider
            _update_config_for_provider(
                "wundercorp", str(getattr(agent, "base_url", "") or ""), default_model=backing)
...
FREE_TIER_AVAILABLE_NOTICE = (
    "Free WunderCorp inference and connectors are now available. "
...
def guest_notice_pending() -> bool:
    ...
    state = current_wundercorp_state()
    return is_guest_state(state) and not bool(state.get(GUEST_NOTICE_FLAG))


def mark_guest_notice_shown() -> bool:
    """Persist ``guest_notice_shown`` on the guest's ``providers.wundercorp`` state (whichever store holds it).
    ...
    from loki_cli.auth import (
        ...
        _store_section)
    with _provider_state_transaction("wundercorp") as (auth_store, state, source_path):
        ...
        if _same_path(source_path, _auth_file_path()):
            _store_section(auth_store, "providers")["wundercorp"] = state
            _save_auth_store(auth_store)
        else:
            source_store = _load_auth_store(source_path)
            _store_section(source_store, "providers")["wundercorp"] = state
            _save_auth_store(source_store, target_path=source_path)
    return True


# --- ``loki auth upgrade``: sign the guest into a real WunderCorp account, keeping its connectors ---------
#
...
# ``completed`` promotion is followed by the token grant, which ``persist_wundercorp_credentials`` writes
...
def register_promotion_intent(
    ...
    if not isinstance(payload.get("claim_code"), str) or not payload["claim_code"]:
        raise _anon_err("WunderCorp free tier sign-in returned no transfer code.", "anon_server_error")
    return payload
...
def _account_state_from_token(
    ...
    """The ``providers.wundercorp`` shape for the signed-in account (same fields the device-code login writes)."""
    from loki_cli.auth import PROVIDER_REGISTRY, _coerce_ttl_seconds, _optional_base_url, _tls_state_from_verify
    from loki_cli.auth_wundercorp import _WUNDERCORP_EMPTY_AGENT_KEY_FIELDS, _iso_after, refresh_wundercorp_oauth_from_state
    now = datetime.now(timezone.utc)
    ttl = _coerce_ttl_seconds(token_data.get("expires_in", 0))
    inference_url = (
        _optional_base_url(token_data.get("inference_base_url"))
        or PROVIDER_REGISTRY["wundercorp"].inference_base_url.rstrip("/"))
    state = {
        ...
        "tls": _tls_state_from_verify(verify), **_WUNDERCORP_EMPTY_AGENT_KEY_FIELDS}
    state = refresh_wundercorp_oauth_from_state(state, timeout_seconds=timeout_seconds, force_refresh=False)
...
def settle_after_upgrade(account_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Picking the free-tier row may have written ``model.default: wundercorp/welcome`` and ``model.base_url``
    = welcome host. An account cannot keep either: the welcome host refuses account tokens, and the
    portal host serves ``wundercorp/welcome`` as a paid model. When the config is on the free tier's route,
    ``model.base_url`` becomes the account's inference host and ``model.default`` the recommended
    default for the account's tier (:func:`loki_cli.models.recommended_wundercorp_default_model`, the
    same pick as ``GET /api/model/recommended-default``), through the same config write a plain WunderCorp
    login uses. A config on the user's own model and host is left alone.

    Every sign-in completion (CLI ``loki auth upgrade``, the desktop poller) calls this once, after
    ``persist_wundercorp_credentials``. Returns ``{"model": str, "changed": bool}``: ``model`` is the default
    ...
    model = current
    if on_welcome_model:
        from loki_cli.models import recommended_wundercorp_default_model
        try:
            model = str(recommended_wundercorp_default_model().get("model") or "")
        except Exception as exc:
    ...
    try:
        ...
        # same write; the runtime's silent default applies until the user picks one with `loki model`.
        _update_config_for_provider(
            "wundercorp", str(account_state.get("inference_base_url") or ""),
...
def _poll_for_token(*args, **kwargs) -> Dict[str, Any]:
    ...
    from loki_cli.auth_device_flow import _poll_for_token as poll
    return poll(*args, **kwargs)


def persist_wundercorp_credentials(*args, **kwargs):
    """Keep the existing auth_wundercorp persistence seam behind the sign-in entry point."""
    from loki_cli.auth_wundercorp import persist_wundercorp_credentials as persist
...
from loki_cli.anon_sign_in import (  # noqa: E402
...
from loki_cli.anon_sign_in_cli import (  # noqa: E402
