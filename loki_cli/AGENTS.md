# loki_cli/ + cli.py — CLI, slash commands, config, skins, updater, profiles

Applies on top of the root `AGENTS.md`. Long-form: `website/docs/developer-guide/cli-internals.md`.

## CLI architecture

`cli.py` holds `LokiCLI` (REPL loop, config, slash dispatch); behaviour lives in mixins
`loki_cli/cli_commands_mixin.py`, `cli_stream_mixin.py`, `cli_status_bar_mixin.py`,
...
`LokiCLI._SLASH_DISPATCH` (`canonical -> (method name, pass_arg)`), falling back to a
`_handle_<name>_command` method by naming convention. **There is no `elif` ladder — do not add one.**
Skill slash commands (`agent/skill_commands.py`) scan `~/.loki/skills/` and inject as a **user
message**, never into the system prompt (prompt caching).

Rules: all interactive menu-pickers use curses (`loki_cli/curses_ui.py`; example
`loki_cli/tools_config.py`). Never emit `\033[K` (ANSI erase-to-EOL) in spinner/display code —
...
## Slash command registry (`loki_cli/commands.py`)
...
`loki_cli/goal_command.py::dispatch_goal_command` owns `/goal` parsing and manager
...
## Config system (`loki_cli/config.py`)
...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`loki tools/setup`, most subcommands, `loki_cli/config.py`, merges `DEFAULT_CONFIG`); raw
...
## Skin engine (`loki_cli/skin_engine.py`)
...
`load_skin(name)` (user `~/.loki/skins/*.yaml` → built-ins → default; missing values inherit
...
## Update pipeline (`loki update`) — transactional; every stage guards a real field failure
...
- **Plan** (`update_inventory.py`, `loki update --plan`): read-only inventory — install kind, all
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE (every `loki-gateway*` unit /
  `ai.loki.gateway*` LaunchAgent), drain-first (SIGUSR1), with per-unit/per-label failure
...
- **Report**: every run writes a machine-readable receipt to `~/.loki/logs/update_receipts/`
...
`_apply_profile_override()` in `loki_cli/main.py` sets `LOKI_HOME` before any module import, so
every `get_loki_home()` scopes to the active profile (rules in root). Profiles are independent
...
## WunderCorp free tier (`loki_cli/anon_auth.py`)
...
(`models.recommended_wundercorp_default_model`, shared with `GET /api/model/recommended-default`).
...
which never contains a raw exception, a URL or a `loki` verb) and `.copy_terminal`, so no caller
...
contextvars. `upgrade_guest` (`loki auth upgrade`), the CLI `/login` handler and the desktop
promotion poller are renderers over it; a surface that needs the cancel check and the save to be
atomic passes `persist_guard`. The desktop's plain "connect another WunderCorp account" device-code login
is a separate path (`_wundercorp_plain_poller`) and must stay one.
