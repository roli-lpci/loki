"""Multi-provider authentication system for Loki Agent.

- ``ProviderConfig`` / ``PROVIDER_REGISTRY`` describe every known inference provider.
- The auth store (``~/.loki/auth.json``) holds per-provider state, the credential pool and
...
- ``OAUTH_PROVIDER_FLOWS`` maps each OAuth provider to its resolver/status builder; the flows live in
  ``auth_wundercorp``/``auth_codex``/``auth_xai``/``auth_qwen``/``auth_minimax``/``auth_spotify`` and are
  re-imported here so ``loki_cli.auth.<name>`` stays the public/patchable surface."""
...
from loki_cli.config import (
    get_loki_home, get_config_path, read_raw_config, require_readable_config_before_write)
from loki_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
from loki_cli.auth_zai_kimi import (  # noqa: F401  re-exported
...
from loki_cli.auth_model_picker import (  # noqa: F401  re-exported
    _prompt_model_selection, _save_model_choice)
from loki_cli.auth_device_flow import (  # noqa: F401  re-exported
    _can_open_graphical_browser, _default_verify, _is_remote_session,
    _wundercorp_device_auth_timeout_message, _offer_existing_oauth_credentials,
...
from loki_cli.auth_oauth_grants import (  # noqa: F401  re-exported
...
from loki_cli.auth_wundercorp import (  # noqa: F401  re-exported
    WUNDERCORP_SESSION_TERMINAL, WUNDERCORP_SESSION_UNKNOWN, WUNDERCORP_SESSION_VALID, _ALLOWED_WUNDERCORP_INFERENCE_HOSTS,
    _agent_key_is_usable, _apply_wundercorp_refreshed_tokens, _assert_wundercorp_inference_jwt_usable,
    _compute_wundercorp_auth_status, _format_wundercorp_entitlement_auth_error, _healed_wundercorp_inference_url,
    _login_wundercorp, _merge_shared_wundercorp_oauth_state, _migrate_stale_wundercorp_portal_url,
    _wundercorp_device_code_login, _wundercorp_inference_env_override, _wundercorp_invoke_jwt_is_usable,
    _wundercorp_invoke_jwt_status, _wundercorp_portal_env_override, _wundercorp_shared_store_lock,
    _wundercorp_shared_store_path, _pool_first_oauth_status, _quarantine_wundercorp_oauth_state,
    _quarantine_wundercorp_pool_entries, _read_shared_wundercorp_state, _refresh_access_token,
    _refresh_wundercorp_or_quarantine, _select_wundercorp_invoke_jwt, _sync_wundercorp_pool_from_auth_store,
    _token_fingerprint, _try_import_shared_wundercorp_state, _validate_wundercorp_inference_url_from_network,
    _write_shared_wundercorp_state, fetch_wundercorp_models, get_wundercorp_auth_status_local,
    get_wundercorp_session_validity, persist_wundercorp_credentials, refresh_wundercorp_oauth_from_state,
    resolve_wundercorp_runtime_credentials, step_up_wundercorp_billing_scope)
from loki_cli.auth_minimax import (  # noqa: F401  re-exported
...
from loki_cli.auth_xai import (  # noqa: F401  re-exported
...
from loki_cli.auth_codex import (  # noqa: F401  re-exported
...
from loki_cli.auth_spotify import (  # noqa: F401  re-exported
    _refresh_spotify_oauth_state, get_spotify_auth_status, login_spotify_command,
    resolve_spotify_runtime_credentials)
from loki_cli.auth_qwen import (  # noqa: F401  re-exported
...
from loki_cli.auth_constants import (  # noqa: F401  re-exported
    _decode_jwt_claims, AUTH_STORE_VERSION, AUTH_LOCK_TIMEOUT_SECONDS, DEFAULT_WUNDERCORP_PORTAL_URL,
    DEFAULT_WUNDERCORP_INFERENCE_URL, DEFAULT_WUNDERCORP_CLIENT_ID, WUNDERCORP_BILLING_MANAGE_SCOPE,
    DEFAULT_WUNDERCORP_SCOPE, WUNDERCORP_DEVICE_CODE_SOURCE, WUNDERCORP_AUTH_PATH_INVOKE_JWT,
    ACCESS_TOKEN_REFRESH_SKEW_SECONDS, WUNDERCORP_INVOKE_JWT_MIN_TTL_SECONDS, DEFAULT_CODEX_BASE_URL,
    ...
    ACTUAL_LOCAL_NOAUTH_PLACEHOLDER, CODEX_RATE_LIMITED_CODE, AuthError, _wundercorp_err, httpx)
...
_REGISTRY_ROWS: Tuple[Any, ...] = (
    ...
    # is a 401), so there is no secret to configure. Select via `loki model` / `/model free`.
...
def get_anthropic_key() -> str:
    ...
    Checks both the ``.env`` file and the process environment, preferring ``~/.loki/.env`` so a deliberate
    key rotation isn't shadowed by a stale shell export (matches the api-key resolution path — see #20591).
    """
    from loki_cli.config import get_env_value_prefer_dotenv
...
def _model_level_key_env(provider_id: str) -> str:
    ...
    (``model.key_env`` → ``$LOKI_HOME/.env``) instead of the registry's canonical env var,
...
def _resolve_api_key_provider_secret(provider_id: str, pconfig: ProviderConfig) -> tuple[str, str]:
    ...
    if provider_id == "copilot":
        ...
        try:
            from loki_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    # Prefer ~/.loki/.env over os.environ so a deliberate key rotation in .env isn't shadowed by
    ...
    # model.key_env → $LOKI_HOME/.env (e.g. LOKI_CUSTOM_LMSTUDIO_API_KEY) while keeping
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    re-authenticating cannot fix it, so callers should say "retry later", not ``loki auth``."""
    return (isinstance(error, AuthError) and not error.relogin_required
            and error.code == CODEX_RATE_LIMITED_CODE)


# Entitlement failures: WunderCorp gets a Portal-aware message; other providers a fixed generic one (or
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `loki model` to re-authenticate."
    if error.code in _ENTITLEMENT_ERROR_CODES:
        if error.provider == "wundercorp":
            return _format_wundercorp_entitlement_auth_error(error)
...
# ── Auth Store — persistence layer for ~/.loki/auth.json ──────────────────────────────────────────

def _auth_file_path() -> Path:
    path = get_loki_home() / "auth.json"
    # Seat belt: under pytest, refuse to touch the real user's auth store (tests that forgot to
    # monkeypatch LOKI_HOME or escaped the hermetic conftest). In production: one dict lookup.
    if (os.environ.get("PYTEST_CURRENT_TEST")
            and _same_path(path, Path.home() / ".loki" / "auth.json")):
        raise RuntimeError(
            f"Refusing to touch real user auth store during test run: {path}. "
            "Set LOKI_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    try:
        ...
        global_root = get_default_loki_root()
    except Exception:
        return None
    return None if _same_path(get_loki_home(), global_root) else global_root / "auth.json"
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST") and os.environ.get("HOME"):
        real_root = Path(os.environ["HOME"]) / ".loki" / "auth.json"
        try:
...
def _file_lock(
    ...
    shared WunderCorp store) track reentrancy separately."""
...
def _auth_store_lock(
    ...
    ``_wundercorp_shared_store_lock`` SECOND (inner), else deadlock against a concurrent shared import."""
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and (
        ...
        if isinstance(raw.get("providers"), dict):
            _migrate_stale_wundercorp_portal_url(raw["providers"])
        return raw

    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):  # legacy "systems" format
        systems = raw["systems"]
        providers = {"wundercorp": systems["wundercorp_portal"]} if "wundercorp_portal" in systems else {}
        return {**_empty_auth_store(), "providers": providers,
                "active_provider": "wundercorp" if providers else None}
    return _empty_auth_store()
...
def _save_auth_store(auth_store: Dict[str, Any], target_path: Optional[Path] = None) -> Path:
    ...
    # enforced); ignore failures. secure_parent_dir refuses to chmod /, top-level dirs, or the loki-agent
...
def _save_provider_state_to_source(
    ...
    left as it is (a WunderCorp free-tier identity refreshed for a connector call must not become the
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    """Set ``active_provider`` only when none is set yet: the first ``loki auth add`` credential must
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    when the profile has zero entries for it (``loki auth add`` in the profile shadows global)."""
...
def write_credential_pool(
    ...
    *status_cleared_ids* were cleared deliberately (``loki auth reset``) and skip the
...
def wundercorp_token_has_billing_scope() -> bool:
    """Return True if the currently-held WunderCorp token carries ``billing:manage``.
    ...
    if not isinstance(scope, str):
        return False
    return WUNDERCORP_BILLING_MANAGE_SCOPE in scope.split()
...
def _explicit_pool_entry_present(normalized: str) -> bool:
    """Pool rows from EXPLICIT Loki flows (manual add / device-code / PKCE) or live env keys;
...
# Set by Claude Code itself, not by the user explicitly configuring anthropic in Loki.
_IMPLICIT_ENV_VARS = frozenset({"CLAUDE_CODE_OAUTH_TOKEN"})
_EXPLICIT_POOL_SOURCES = frozenset({"device_code", "loopback_pkce", "loki_pkce", "manual"})
...
def _pool_entry_is_explicit(entry: Any) -> bool:
    """True for pool rows the user created via an explicit Loki flow (or a still-live env key)."""
...
def _keyless_provider_has_explicit_config(normalized: str) -> bool:
    """Vertex / Bedrock count as explicit when Loki-scoped routing config is present.
    ...
    Loki-scoped signals (VERTEX_PROJECT_ID / vertex.project_id / VERTEX_CREDENTIALS_PATH) count
    here."""
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    Loki-initiated flow, or Loki-scoped routing config for keyless cloud-SDK providers. Ambient
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    ...
    ``loki logout``. Returns True if something was cleared."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        from loki_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""
        lines = ["Config issue detected — run 'loki doctor' for full diagnostics:"]
        for ci in issues:
...
def _refuse_env_adoption_if_config_corrupt() -> None:
    ...
    try:
        from loki_cli.config import get_active_config_parse_failure
    ...
    raise AuthError(
        ...
        f"was saved next to it) or run loki setup.",
        code="corrupt_config")
...
def _openrouter_auto_detected(scoped_key_env: Callable[[str], str]) -> bool:
    ...
    `loki auth add openrouter` has no env var; without the pool check it is invisible to
    ...
    if any(has_usable_secret(scoped_key_env(v)) for v in ("OPENAI_API_KEY", "OPENROUTER_API_KEY")):
        return True
    try:
        # Auto-detect an OpenRouter credential added via `loki auth add openrouter` (manual pool entry, no
        # env var). Without this, a key that only lives in the credential pool is invisible to
        # auto-detection — the user sees `loki auth list` showing the credential while requests go out
...
def _env_key_auto_detected(
    ...
    preempts a logged-in OAuth provider so a stale key in ~/.loki/.env never switches silently."""
            ...
            if has_usable_secret(scoped_key_env(env_var)):
                if oauth_active and oauth_active != pid:
                    logger.warning(
                        ...
                        # ~/.loki/.env isn't silently switched without knowing why.
...
def resolve_provider(
    ...
    *,
    ...
    7. WunderCorp free tier when it is on and its identity exists (never created here);
    ...
    if normalized != "auto":
        ...
        tail = (f"\n\n{hint}" if hint else " Check 'loki model' for available providers, "
                "or run 'loki doctor' to diagnose config issues.")
    ...
    # WunderCorp free tier, when it is on and its identity already exists. This rung sits ABOVE the Bedrock
    ...
    raise AuthError(
        "No inference provider configured. Run 'loki model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.loki/.env.",
...
def _quarantine_flat_oauth_state(state: Dict[str, Any], provider: str, exc: "AuthError") -> None:
    ...
    subsequent calls fail fast without a network retry (mirrors the WunderCorp / xAI / Codex pattern)."""
...
# Valid WunderCorp Portal hosts; a stored portal_base_url outside this set is a misconfiguration and falls
# back to the default. localhost / 127.0.0.1 are for local development and testing.
_WUNDERCORP_PORTAL_ALLOWED_HOSTS: FrozenSet[str] = frozenset({
    "portal.wundercorp.com", "localhost", "127.0.0.1"})

# Per-process memo for resolve_wundercorp_access_token: startup runs one check_fn per managed tool and
# each would trigger its own ~15s blocking refresh of an expired token; a short-TTL memo collapses
# the burst into one round-trip. Callers needing freshness use force_fresh/refresh_wundercorp_oauth_pure.
_RESOLVE_TOKEN_CACHE_LOCK = threading.Lock()
_RESOLVE_TOKEN_CACHE: "tuple[float, str] | None" = None
_RESOLVE_TOKEN_CACHE_TTL_S = 5.0


def _wundercorp_portal_base_url(state: Dict[str, Any]) -> str:
    """LOKI_PORTAL_BASE_URL / WUNDERCORP_PORTAL_BASE_URL is the trusted operator override and wins
    ...
    portal_base_url = _optional_base_url(state.get("portal_base_url")) or DEFAULT_WUNDERCORP_PORTAL_URL
    portal_base_url = portal_base_url.rstrip("/")
    host = urlparse(portal_base_url).hostname
    if host and host not in _WUNDERCORP_PORTAL_ALLOWED_HOSTS:
        logger.warning(
            "auth: ignoring invalid portal_base_url %r (host %r not in allowlist), using default",
            portal_base_url, host)
        return DEFAULT_WUNDERCORP_PORTAL_URL
    return portal_base_url


def resolve_wundercorp_access_token(
    *,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None,
    refresh_skew_seconds: int = ACCESS_TOKEN_REFRESH_SKEW_SECONDS) -> str:
    """Resolve a refresh-aware WunderCorp Portal access token for managed tool gateways."""
    ...
    with _provider_state_transaction("wundercorp") as (auth_store, state, state_source_path):
        ...
        portal_base_url = _wundercorp_portal_base_url(state)
        client_id = str(state.get("client_id") or DEFAULT_WUNDERCORP_CLIENT_ID)
        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)
        persist = lambda: _save_provider_state_to_source(  # noqa: E731
            auth_store, "wundercorp", state, state_source_path)

        lock_timeout = max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)
        with _wundercorp_shared_store_lock(timeout_seconds=lock_timeout):
            ...
            merged_shared = _merge_shared_wundercorp_oauth_state(state)
            access_token = state.get("access_token")
            ...
            if not isinstance(refresh_token, str) or not refresh_token:
                raise _wundercorp_err("Session expired and no refresh token is available.", relogin=True)
            ...
            _apply_wundercorp_refreshed_tokens(state, refreshed, refresh_token)
...
# Process-level memo for get_wundercorp_auth_status(): it validates via a synchrowundercorp refresh POST
...
_WUNDERCORP_AUTH_STATUS_CACHE_TTL = 15.0  # seconds
_wundercorp_auth_status_cache: Optional[Tuple[float, str, Optional[float], Dict[str, Any]]] = None
...
def invalidate_wundercorp_auth_status_cache() -> None:
    """Clear the get_wundercorp_auth_status() memo (for code paths that mutate WunderCorp auth state without
    touching auth.json, e.g. tests; login/logout invalidate via the mtime check automatically)."""
    global _wundercorp_auth_status_cache
    _wundercorp_auth_status_cache = None


def get_wundercorp_auth_status() -> Dict[str, Any]:
    """Status snapshot for WunderCorp auth, memoised ~15s keyed on the auth.json mtime.
    ...
    cached = _wundercorp_auth_status_cache
    if (cached is not None and cached[1:3] == (auth_file_key, mtime)
            and (now - cached[0]) < _WUNDERCORP_AUTH_STATUS_CACHE_TTL):
        return dict(cached[3])
    status = _compute_wundercorp_auth_status()
    _wundercorp_auth_status_cache = (now, auth_file_key, mtime, dict(status))
    return status
...
class OAuthProviderFlow:
    ...
    ``monkeypatch.setattr("loki_cli.auth.resolve_codex_runtime_credentials", ...)`` applies."""
    provider_id: str
    resolve_fn: str
    status_fn: str
    terminal_refresh_codes: FrozenSet[str] = frozenset()  # retrying the same refresh token cannot succeed
    # ``loki logout`` with no active provider falls back to config.yaml ``model.provider`` only
...
OAUTH_PROVIDER_FLOWS: Dict[str, OAuthProviderFlow] = {
    "wundercorp": OAuthProviderFlow(
        "wundercorp", "resolve_wundercorp_runtime_credentials", "get_wundercorp_auth_status",
...
_is_terminal_wundercorp_refresh_error = partial(_is_terminal_refresh_error, provider="wundercorp")
...
def _provider_env_base_url(pconfig: ProviderConfig) -> str:
    if pconfig.id == "actual":
        from loki_cli.providers import normalize_provider
...
def _provider_is_keyless(provider_id: str) -> bool:
    """LokiOverlay keyless flag — the same source the provider catalog and GUI contract tests use."""
    try:
        from loki_cli.providers import LOKI_OVERLAYS
        return bool(getattr(LOKI_OVERLAYS.get(provider_id), "keyless", False))
    except Exception:
        return False
...
def _external_process_auth_evidence(provider_id: str) -> tuple[bool, Optional[str]]:
    ...
    Loki can't read). Deliberately subprocess-free: spawning ``gh auth token`` from status
    ...
    try:
        from loki_cli.copilot_auth import COPILOT_ENV_VARS, validate_copilot_token
...
def _external_process_spec(
    ...
    Launch details come from the provider's own profile (copilot-acp: LOKI_COPILOT_ACP_COMMAND /
    COPILOT_CLI_PATH / LOKI_COPILOT_ACP_ARGS), so out-of-tree providers describe their binary."""
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    tests that patch ``loki_cli.auth.get_*_auth_status`` still apply."""
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    CLI startup flat; ``loki doctor`` runs the live probe). ``api_key`` (default): usable
    ``AZURE_FOUNDRY_API_KEY``."""
    info: Dict[str, Any] = {"provider": "azure-foundry"}
    try:
        from loki_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            info.update(
                ...
                hint=(
                    ...
                    "is skipped here. Run `loki doctor` to verify token acquisition."
                ) if installed else (
                    "azure-identity not installed. Install with: "
                    "pip install azure-identity  (or rely on Loki' "
...
def _update_config_for_provider(
    ...
    *default_model*, when given, is written as ``model.default`` in the same step so the gateway
    ...
    from loki_cli.config import clear_model_endpoint_credentials
...
def login_command(args) -> None:
    """Deprecated: use 'loki model' or 'loki setup' instead."""
    print("The 'loki login' command has been removed.\nUse 'loki auth' to manage credentials,\n"
          "'loki model' to select a provider, or 'loki setup' for full setup.")
    raise SystemExit(0)
...
def logout_command(args) -> None:
    ...
    if not target:
        ...
        return
    if target == "wundercorp":
        from loki_cli.anon_auth import FREE_TIER_NOT_SIGNED_IN, is_guest_state
    ...
    if target == "wundercorp":
        ...
        from loki_cli.auth_wundercorp import _clear_shared_wundercorp_state
        _clear_shared_wundercorp_state("logout")
    ...
    if not should_reset_config:
        ...
        print("Loki will use OpenRouter for inference.")
    else:
        print("Run `loki model` or configure an API key to use Loki.")
...
_PLUGIN_COMPAT_LAZY = {
    'CODEX_OAUTH_USER_AGENT': ('loki_cli.auth_constants', 'CODEX_OAUTH_USER_AGENT'),
    'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS': ('loki_cli.auth_codex', 'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS'),
    'DEFAULT_SPOTIFY_REDIRECT_URI': ('loki_cli.auth_constants', 'DEFAULT_SPOTIFY_REDIRECT_URI'),
    'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS': ('loki_cli.auth_constants', 'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS'),
    'MINIMAX_OAUTH_GRANT_TYPE': ('loki_cli.auth_constants', 'MINIMAX_OAUTH_GRANT_TYPE'),
    'WUNDERCORP_INFERENCE_INVOKE_SCOPE': ('loki_cli.auth_constants', 'WUNDERCORP_INFERENCE_INVOKE_SCOPE'),
    'WUNDERCORP_SHARED_STORE_FILENAME': ('loki_cli.auth_wundercorp', 'WUNDERCORP_SHARED_STORE_FILENAME'),
    'OAUTH_OVER_SSH_DOCS_URL': ('loki_cli.auth_constants', 'OAUTH_OVER_SSH_DOCS_URL'),
    'QWEN_OAUTH_CLIENT_ID': ('loki_cli.auth_constants', 'QWEN_OAUTH_CLIENT_ID'),
    'QWEN_OAUTH_TOKEN_URL': ('loki_cli.auth_constants', 'QWEN_OAUTH_TOKEN_URL'),
    'SINGLE_USE_OAUTH_SINGLETON_FILES': ('loki_cli.auth_oauth_grants', 'SINGLE_USE_OAUTH_SINGLETON_FILES'),
    'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS': ('loki_cli.auth_constants', 'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS'),
    'SPOTIFY_DASHBOARD_URL': ('loki_cli.auth_constants', 'SPOTIFY_DASHBOARD_URL'),
    'XAI_OAUTH_DEVICE_CODE_URL': ('loki_cli.auth_constants', 'XAI_OAUTH_DEVICE_CODE_URL'),
    'XAI_OAUTH_DISCOVERY_URL': ('loki_cli.auth_constants', 'XAI_OAUTH_DISCOVERY_URL'),
    'XAI_OAUTH_ISSUER': ('loki_cli.auth_constants', 'XAI_OAUTH_ISSUER'),
    'refresh_wundercorp_oauth_pure': ('loki_cli.auth_wundercorp', 'refresh_wundercorp_oauth_pure'),
}
