...
def _anthropic_oauth_status() -> Dict[str, Any]:
    """Status for the "Anthropic API Key" card: Loki-managed PKCE file first, then the
    ...
    """
    try:
        from agent.anthropic_credentials import read_loki_oauth_credentials, _get_loki_oauth_file
        loki_creds = read_loki_oauth_credentials()
    except Exception:
        loki_creds = None
    if loki_creds and loki_creds.get("accessToken"):
        return _token_status("loki_pkce", f"Loki PKCE ({_get_loki_oauth_file()})", loki_creds)
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    store); the CLI may hold its session in an OS keychain Loki can't read, so the unverified
    state reads "managed by the Copilot CLI" — never signed out."""
    try:
        from loki_cli.auth import get_external_process_provider_status
...
def _external_process_cli_command(provider_id: str, default: str) -> str:
    ...
    (``LOKI_COPILOT_ACP_COMMAND`` / ``COPILOT_CLI_PATH``); others get ``default`` untouched."""
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    {"id": "wundercorp", "name": "WunderCorp Portal", "flow": "device_code", "cli_command": "loki auth add wundercorp",
     "docs_url": "https://portal.wundercorp.com", "status_fn": None},
    {"id": "openai-codex", "name": "ChatGPT or Codex Subscription", "flow": "device_code",
     "cli_command": "loki auth add openai-codex", "docs_url": "https://platform.openai.com/docs",
     "status_fn": None},
    {"id": "qwen-oauth", "name": "Qwen (via Qwen CLI)", "flow": "external",
     "cli_command": "loki auth add qwen-oauth", "docs_url": "https://github.com/QwenLM/qwen-code",
     "status_fn": None},
    ...
    {"id": "minimax-oauth", "name": "MiniMax (OAuth)", "flow": "device_code",
     "cli_command": "loki auth add minimax-oauth", "docs_url": "https://www.minimax.io", "status_fn": None},
    ...
    {"id": "xai-oauth", "name": "xAI Grok OAuth (SuperGrok / Premium+)", "flow": "device_code",
     "cli_command": "loki auth add xai-oauth",
     "docs_url": "https://doku.sh/#/i/cbe1e051e4be2bb725-26f1d908878243", "status_fn": None},
    ...
    # Login works via the terminal (`loki auth add anthropic`) or a plain API key.
    {"id": "anthropic", "name": "Anthropic API Key", "flow": "external", "cli_command": "loki auth add anthropic",
...
@_oauth_poller("wundercorp")
def _wundercorp_promotion_poller(session_id: str, sess: Dict[str, Any]) -> None:
    """Drain the sign-in the start route began: one shared flow, rendered onto the session.

    The generator was created and advanced to its ``Code`` state by ``_start_wundercorp_device_code``, so
...
@_oauth_poller("wundercorp")
def _wundercorp_plain_poller(session_id: str, sess: Dict[str, Any]) -> None:
    """Background poller for a plain WunderCorp device-code login (no free-tier identity to transfer).

    A sign-in that carries the free tier's connectors runs through ``anon_auth.run_sign_in`` and
    ``_wundercorp_promotion_poller`` instead; this is the "connect another WunderCorp account" path.
    """
    from loki_cli.web_server_profiles import _profile_scope
    from loki_cli.auth import _poll_for_token, persist_wundercorp_credentials, refresh_wundercorp_oauth_from_state
    ...
    if _cancelled():
        return
    # Same post-processing as _wundercorp_device_code_login (validate/refresh JWT)
    ...
    with _profile_scope(_oauth_session_profile(session_id)):
        full_state = refresh_wundercorp_oauth_from_state(auth_state, timeout_seconds=15.0, force_refresh=False)
        ...
        with _oauth_sessions_lock:
            if sess.get("cancelled"):
                sess["status"] = "cancelled"
                return
            persist_wundercorp_credentials(full_state)
...
def _minimax_poller(session_id: str, sess: Dict[str, Any]) -> None:
    """MiniMax poller: PKCE-style ``code_verifier`` + ``user_code`` instead of WunderCorp's
    ``device_code``. Builds the same auth_state as the CLI's ``_minimax_oauth_login`` and persists
    via ``_minimax_save_auth_state`` so the system ends up as after ``loki auth add minimax-oauth``.
...
def _xai_device_poller(session_id: str, sess: Dict[str, Any]) -> None:
    ...
    with _profile_scope(_oauth_session_profile(session_id)):
        ...
        # Mirror `loki auth add xai-oauth`: first credential may become active; never overwrite.
