"""Gateway fleet restart + post-update verification for ``loki update``.

Split out of ``loki_cli/update_cmd.py``; every name is re-imported there so
``loki_cli.update_cmd.<name>`` keeps resolving/monkeypatching. Origin helpers are
...
from loki_cli.update_inventory import _gateway_service_matches_profile

# Log-record parity with the origin module.
logger = logging.getLogger("loki_cli.update_cmd")

# Under LOKI_HOME (not next to the venv): records the fleet-restart obligation
...
_LIST_GATEWAY_UNITS = ["list-units", "loki-gateway*", "loki-serve*", "--plain", "--no-legend", "--no-pager"]


def _write_gateway_update_exit_code(ok: bool) -> None:
    from loki_cli.update_cmd import get_loki_home
    path = get_loki_home() / ".update_exit_code"
    with suppress(OSError):
        path.write_text("0" if ok else "1", encoding="utf-8")


def _fleet_restart_pending_marker_path() -> Path:
    """LOKI_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    from loki_cli.update_cmd import get_loki_home
    return get_loki_home() / _FLEET_RESTART_PENDING_NAME
...
def _current_checkout_sha() -> str | None:
    ...
    from loki_cli.update_cmd import _capture_head_sha, _m
    try:
        from loki_cli.build_info import get_code_identity
...
def _receipt_looks_unfinished(receipt: dict) -> bool:
    ...
    successful receipt look unfinished, or the next ``loki update`` retriggers
...
def _receipt_reports_stale_runtime(expected_sha: str | None = None) -> bool:
    ...
    *before* the pull, so a finished update's plan always looks stale and must not
    ...
    See #95294.
    """
    from loki_cli.update_cmd import _current_checkout_sha
    try:
        from loki_cli.update_receipt import read_latest_receipt
...
def _live_fleet_covers_receipt(expected_sha: str | None) -> bool:
    ...
    if not expected_sha:
        return False
    from loki_cli.update_receipt import collect_fleet_versions, read_latest_receipt
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print("⚠ A previous `loki update` pulled new code but did not restart running gateways.", file=stream)
    print("  Gateways may still be serving pre-update modules (mixed sys.modules).", file=stream)
    if startup:
        print("  Run `loki update` or `loki gateway restart`.", file=stream)
...
def _restart_systemd_gateway_units_best_effort(failed: list, listings) -> None:
    """Best-effort ``systemctl restart`` of every loki-gateway/serve unit."""
    answered = set()
...
def _run_pending_fleet_restart() -> bool:
    ...
    with suppress(Exception):
        _m()._purge_stale_loki_modules()
    # Warn if legacy Loki gateway unit files are still installed. When both loki.service (from a
    # pre-rename install) and the current loki-gateway.service are enabled, they SIGTERM-fight for the
    # same bot token (see PR #11909). Flagging here means every `loki update` surfaces the issue until the
    # user migrates.
    try:
    ...
    try:
        ...
        # --- Systemd services (Linux) --- Discover all loki-gateway* units (default + profiles) plus
        # loki-serve* units (the Desktop app's backend, #83438).
        if systemd_listings is not None:
            _restart_systemd_gateway_units_best_effort(failed, systemd_listings)
        # --- Launchd services (macOS) --- Restart EVERY ai.loki.gateway* LaunchAgent, not only the
...
def _apply_pending_fleet_restart_catchup() -> None:
    """On an already-up-to-date ``loki update``, finish a skipped restart.

    No-op when nothing is pending; exits 1 on incomplete catch-up so automation
    does not treat the fleet as healthy.
    """
    from loki_cli.update_cmd import _run_pending_fleet_restart
    ...
    if _run_pending_fleet_restart():
        ...
        return
    print("  ⚠ Fleet restart incomplete. Recover with: loki gateway restart")
    sys.exit(1)
...
def _is_loki_gateway_unit(unit: str) -> bool:
    """Exact base unit or hyphenated profile family only: ``startswith("loki-serve")``
    would accept ``loki-server.service``."""
    return (
        ...
        unit == "loki-gateway.service"
        or unit.startswith("loki-gateway-")
        or unit == "loki-serve.service"
        or unit.startswith("loki-serve-")
    )


def _for_each_systemd_gateway_unit(list_units_stdout: str, *, process_unit, on_unit_timeout) -> None:
    """Process each loki-gateway*/loki-serve* unit from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not parts:
            continue
        unit = parts[0]
        if not unit.endswith(".service") or not _is_loki_gateway_unit(unit):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``loki-gateway*`` runs ``gateway/run.py`` (the handler); SIGUSR1 would just
    kill ``loki-serve*`` and burn the drain budget, so those go straight to the blunt
    restart. Same exact/hyphenated shape as ``_for_each_systemd_gateway_unit`` so a
    near-prefix unit like ``loki-gatewayd`` is never signalled.

    See #83438.
    """
    return svc_name == "loki-gateway" or svc_name.startswith("loki-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from loki_cli.gateway import is_macos
    if not failed_units:
        return
    ...
    print("    loki gateway status")
    if any(not name.startswith("ai.loki.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.loki.") for name in ordered):
...
def _restart_launchd_gateway_after_update(*, supervision_verify: bool = True) -> tuple[list, list]:
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "Recover manually: loki gateway restart"
            )
    ...
    # running gateway, a plist reload to a detached helper; both asynchrowundercorp. See #88848.
    if wait_for_launchd_gateway_supervision(label=current_label):
        return [current_label], []
    print(
        f"  ✗ {current_label} restarted but launchd is not supervising it.\n"
        "    Check logs, then: loki gateway restart"
    )
    return [], [current_label]
...
def _restart_macos_launchd_gateways(
    ...
    The pull is shared across profiles, so every ``ai.loki.gateway*`` LaunchAgent
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    ``None`` when undeterminable (notably ``loki_cli.gateway`` no longer importing
    ...
    """
    try:
        from loki_cli.gateway import find_gateway_pids
...
# NOT a claim that no supervisor exists: a systemd-launched serve sets neither LOKI_SPAWN
# nor LOKI_PARENT_PID ("manual-serve"). Unit-backed serves are recovered by the fresh
...
_SERVE_SKIP_REASON = (
    ...
    " systemd unit pass when it owns a loki-serve* unit, else left running for explicit"
    " operator restart"
)
...
def _gateway_recovery_partition(plan, *, skip_profiles: set[str] | None = None) -> tuple[dict[str, str], list[dict]]:
    ...
    Uses only the pre-checkout inventory: re-importing ``loki_cli.gateway`` in the
    ...
    child's ``loki-serve*`` systemd pass enumerates units from systemd; leftovers are
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    logged at debug level, so an early failure (e.g. importing ``loki_cli.gateway`` from the freshly
...
def _resolve_manage_cmd(cache: dict, scope_: str, scope_cmd_: list, svc_name_: str):
    ...
    so a least-privilege sudoers entry scoped to loki-gateway* qualifies (idempotent
...
def _restart_one_systemd_gateway_unit(
    ...
    # → drain → exit, Restart=always respawns. loki-serve has no handler → blunt restart below.
    _main_pid = 0
    ...
    if _manage_cmd is None:
        ...
        print(
            ...
            f"    To let `loki update` restart it automatically, allow\n"
    ...
    # budget, restart-policy mismatch). Mirrors `loki gateway restart` (`systemd_restart()`).
...
def _restart_systemd_gateway_units(restarted_services, failed_or_stale_units, restarted_scoped_units, drain_budget):
    """Restart every active loki-gateway*/loki-serve* systemd unit (user + system).
    ...
    """
    from loki_cli.gateway import supports_systemd_services, _ensure_user_systemd_env
    ...
    def _on_list_timeout(scope: str, exc: subprocess.TimeoutExpired) -> None:
        ...
        print(
            ...
            f"Check the gateway with: loki gateway status"
        )
...
class _GatewayRestartOutcome:
    ...
    def record_receipt(self, **extra) -> None:
        ...
        with suppress(Exception):
            from loki_cli.update_receipt import record_gateway_restart
...
def _restart_manual_gateways(out: _GatewayRestartOutcome, _drain_budget) -> None:
    ...
    if out.restarted_services or out.killed_pids:
        ...
        if unmapped_count:
            ...
            print("    Restart manually: loki gateway run")
            if unmapped_count > 1:
                print("    (or: loki -p <profile> gateway run  for each profile)")
...
def _force_kill_stuck_gateways(killed_pids) -> None:
    ...
    with _best_effort('Post-restart survivor sweep failed: %s'):
        from loki_cli.gateway import find_gateway_pids, _get_service_pids
...
def _restart_gateway_fleet_after_update(_pre_update_plan, gateway_mode: bool):
    ...
    """
    from loki_cli.update_cmd import _m, _write_gateway_update_exit_code
    ...
    # Scope-qualified twin (``user/loki-serve`` vs ``system/loki-serve`` are different
    ...
    # Purge stale cached Loki modules FIRST: the import below loads new gateway
    ...
    try:
        ...
        from loki_cli.gateway import (  # noqa: F401
            is_macos,
            find_gateway_pids,
        ...
        try:
            from loki_cli.gateway import _get_restart_exit_wait_budget
        ...
        # macOS: EVERY ai.loki.gateway* LaunchAgent (systemd parity).
        if is_macos():
...
def _print_legacy_units_warning() -> None:
    """Legacy loki.service fights loki-gateway.service over the bot token; warn on
    every update until migrated."""
    from loki_cli.gateway import (has_legacy_loki_units, _find_legacy_loki_units, supports_systemd_services)
    if not (supports_systemd_services() and has_legacy_loki_units()):
        return
    print()
    print("⚠ Legacy Loki gateway unit(s) detected:")
    for name, path, is_sys in _find_legacy_loki_units():
        scope = "system" if is_sys else "user"
        print(f"    {path}  ({scope} scope)")
    print()
    print("  These pre-rename units (loki.service) fight the current")
    print("  loki-gateway.service for the bot token and cause SIGTERM")
    print("  flap loops. Remove them with:")
    print()
    print("    loki gateway migrate-legacy")
    print()
...
def _collect_fleet_snapshot(restart, rows_expected: bool) -> list:
    ...
    """
    from loki_cli.update_receipt import collect_fleet_versions
...
def _verify_fleet_after_update(restart, *, _pre_update_plan, _windows_gateway_resume, node_failures, update_complete):
    ...
    # units, so a unit-less `loki serve` keeps stale sys.modules. Runs AFTER
    ...
    # units, so an sshd-spawned `serve --isolated` or a manual `loki serve` (no unit) is left running its
    ...
    print("  loki model              # Select provider and model")
    ...
    with _best_effort('Fleet version verification failed: %s'):
        from loki_cli.update_receipt import print_fleet_version_matrix
    ...
    with _best_effort('Runtime-outcome reconciliation failed: %s'):
        ...
        if _pre_update_plan is not None and _pre_update_plan.runtimes:
            from loki_cli.update_inventory import (match_runtime_outcomes, report_unaccounted_runtimes)
            ...
            if report_unaccounted_runtimes(_runtime_outcomes):
                restart.incomplete = True
            with suppress(Exception):
                import loki_cli.update_receipt as _ur
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state (typically the freshly-pulled
    ``loki_cli.gateway`` no longer imports, one of the ways the phase aborts). That is proof-of-safety
