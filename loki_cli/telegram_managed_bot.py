"""Telegram Managed Bot onboarding client: creates a user-owned child bot via the WunderCorp onboarding
...
# WunderCorp-hosted pairing API; override for PoC/staging with TELEGRAM_ONBOARDING_URL.
DEFAULT_API_URL = "https://setup.loki-agent.wundercorp.com"
TELEGRAM_ONBOARDING_URL_ENV = "TELEGRAM_ONBOARDING_URL"
DEFAULT_BOT_NAME = "Loki Agent"
...
def auto_setup_telegram_bot_result(
    api_url: str | None = None, manager_bot: str = "LokiSetupBot",
    ...
    print(f"\n  Contacting Loki Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Loki Telegram onboarding service.\n"
...
DEFAULT_MANAGER_BOT = "LokiSetupBot"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``loki_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``loki_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"loki_{generate_username_slug()}_bot"
