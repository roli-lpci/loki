...
Split out of :mod:`loki_cli.plugins`; validation warns and never fails a load.
"""
...
from loki_cli.plugin_capabilities import parse_declared_capabilities as _parse_declared_capabilities
...
# and continue loading. ``capabilities``/``emits``/``listens``/``loki``/``depends`` are reserved.
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "license", "homepage", "tags", "capabilities", "emits", "listens", "loki", "depends",
    "requires_loki",
}

# Highest manifest schema version this Loki understands.
...
def _plugins_debug() -> bool:
    from loki_cli import plugins as _origin
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Loki "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `loki plugins enable %s` if it is installed.",
                    k, dep_id, dep_id,
                )
            elif resolved == k:
...
class PluginManifest:
    ...
    kind: str = "standalone"
    # Path-derived registry key used by plugins.enabled/disabled and `loki plugins list`: ``disk-cleanup``
    # for a flat plugin, ``image_gen/openai`` for a category plugin. Empty -> name.
    key: str = ""
    # Loki version requirement (``">=0.19"``, comma-separated clauses allowed). Unsatisfied plugins are
    # recorded with an error and skipped before import — see ``requires_loki_error``.
    requires_loki: str = ""
    portable: bool = False
    skill_namespace: str = ""
    ...
    # VALIDATED AND SURFACED ONLY — Loki never auto-installs these (isolation design for the install seam
...
# ── requires_loki version gate ─────────────────────────────────────────────
_VERSION_COMPARATOR_RE = re.compile(r"^\s*(>=|<=|==|!=|>|<)\s*(.+?)\s*$")


def running_loki_version() -> str:
    """Installed ``loki-agent`` distribution version, else ``loki_cli.__version__`` (source checkout)."""
    try:
        return importlib.metadata.version("loki-agent")
    except Exception:
...
def requires_loki_error(manifest: "PluginManifest") -> Optional[str]:
    """Load-blocking reason when the manifest's ``requires_loki`` rejects the running version."""
    if not manifest.requires_loki:
        return None
    current = running_loki_version()
    if version_satisfies(manifest.requires_loki, current):
        return None
    return f"requires loki {manifest.requires_loki}, running {current}"
...
def parse_manifest_file(
    ...
    try:
        ...
        return PluginManifest(
            ...
            kind=kind, key=key, requires_loki=str(data.get("requires_loki") or "").strip(),
