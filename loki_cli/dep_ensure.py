...
from loki_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import loki_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $LOKI_HOME/node is not on PATH, so
    ...
    "browser": lambda: (agent_browser_runnable(shutil.which("agent-browser")) or _has_system_browser()
                        or _has_loki_agent_browser() or _has_npx_agent_browser()),
...
def _has_loki_agent_browser() -> bool:
    from loki_constants import get_loki_home  # late import: tests patch loki_constants.get_loki_home
    home = get_loki_home()
    if _IS_WINDOWS:  # npm -g --prefix puts .cmd shims directly in the prefix dir
        return (home / "node" / "agent-browser.cmd").is_file()
    # install.sh installs into $LOKI_HOME/node/bin/ via npm -g --prefix; legacy git clones used node_modules/.bin/.
...
def ensure_dependency(dep: str, interactive: bool = True) -> bool:
    ...
    if shell == "powershell":
        ...
        if not ps_bin:
            ...
            return False
        cmd = [ps_bin, "-ExecutionPolicy", "Bypass", "-File", str(script), "-Ensure", dep, "-LokiHome", str(get_loki_home())]
    else:
        cmd = ["bash", str(script), "--ensure", dep]
    run_env = {**loki_subprocess_env(inherit_credentials=False), "IS_INTERACTIVE": "false"}
