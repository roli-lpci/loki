...
maps. Split out of ``loki_cli.models``.
"""
...
# OpenRouter entries the WunderCorp Portal does not carry (routing/fast variants, free tier).
_OPENROUTER_ONLY = {
...
def _codex_curated_models() -> list[str]:
    ...
    synthesis) so the gateway /model picker and the CLI ``loki model`` flow share one source."""
    from loki_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
# Mirrors the xAI-direct IDs from $LOKI_HOME/models_dev_cache.json; the cache overrides it on the
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Loki-curated xAI models missing from models.dev, right after the pinned headline."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    """Offline curated floor for xAI / xAI OAuth pickers: $LOKI_HOME/models_dev_cache.json
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    "moa": ["default"],
    "wundercorp": [mid for mid, _ in OPENROUTER_MODELS if mid not in _OPENROUTER_ONLY and not mid.endswith(":free")],
    ...
    # (see loki_cli/model_setup_flows.py); validated live against a GCP project (global region).
    "vertex": [
...
# lists, displays, or iterates providers (loki model, /model, list_authenticated_providers)
# derives from it. slug = internal ID (config.yaml, --provider); label = short display name;
# tui_desc = longer description for the `loki model` picker.
# ---------------------------------------------------------------------------
...
# ...) fold under one top-level row in the INTERACTIVE PICKERS (``loki model``, setup wizard,
...
# Offline/fresh-install fallback for the model Loki silently lands on when the user never picked
...
# (GUI onboarding / ``loki model``) uses the tier-aware ``get_recommended_default_model`` in
# loki_cli/web_server.py + ``partition_wundercorp_models_by_tier``, which may hit the Portal.
_SILENT_DEFAULT_PROVIDERS: frozenset[str] = frozenset({"wundercorp", "openrouter"})
...
_AGGREGATOR_PROVIDERS = frozenset({"wundercorp", "openrouter", "ai-gateway", "copilot", "kilocode"})
...
# agentic subset of 400+ models — merging would dump everything), "wundercorp" (curated list + Portal
...
# OpenRouter-style ids -> Copilot ids. Dash-notation Claude ids are accepted too: Loki' default
