"""Default configuration data for Loki Agent: DEFAULT_CONFIG and OPTIONAL_ENV_VARS.

Pure-data leaf module — must not import from loki_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    "session": {
        # Per-terminal `loki -c`: each CLI session writes a breadcrumb under
        # $LOKI_HOME/terminal-sessions/<terminal-id>, so bare -c/--continue resumes THIS
    ...
    "agent": {
        ...
        # Default 30 min is a safety valve for wedged agents, not a target latency — an interactive `loki
        ...
        # Loki-level retry attempts for API errors (connection drops, timeouts, 5xx) wrapping the
        ...
        # wrapping Loki (sandbox runner, managed platform) can describe proxy/credential/ mount
        # layout without editing SOUL.md. Env LOKI_ENVIRONMENT_HINT overrides it.
        "environment_hint": "",
        ...
        # share or `loki doctor` (status channel only; prompt cache untouched). 0 = no escalation
        ...
        # gateway status (`loki status` / fleet monitoring). Retries never stop — a signal, not a
        ...
        # slot is claimed synchrowundercorply first. 0 = wait forever.
        ...
        # LOKI_LOCAL_STREAM_STALE_TIMEOUT overrides.
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else LOKI_HOME/cache/terminal (auto-pruned
        ...
        # LOKI_HOME/home; "real" = force real HOME; "profile" = force LOKI_HOME/home when it
        ...
        # ["/home/user/.loki/cache/documents:/output"]. For gateway MEDIA delivery, write to
        ...
        # default for images whose entrypoints must start as root (e.g. the bundled Loki image,
        # which drops to `loki` via s6-setuidgid). When on, SETUID/SETGID caps are omitted.
    ...
    "web": {
        ...
        # parallel, firecrawl, keenable; tavily is opt-in keyless via `loki tools`, not a ring
        # member). Set by the `loki tools` picker. "free" = always anonymous endpoint even with a
    ...
    "browser": {
        ...
        # Consent to browse with the user's REAL logins locally: runs on a Loki-managed SNAPSHOT
        # of the ACTIVE default-Chromium profile (Local State -> profile.last_used; cookies, logins,
        # prefs copied and re-synced per fresh session) driven by Loki' packaged Chromium. The
        # snapshot dir sidesteps Chrome 136+'s default-profile debugging block and never contends
        # with the running browser. Turning off deletes ~/.loki/browser-profile/ so credentials
        ...
        # runs `loki browser close-profile` (kills that profile's browser tree, unsaved tabs lost)
    ...
    # write_file/patch call); restore with /rollback. Opt-in via `loki chat --checkpoints` or
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ~/.loki/checkpoints/ size (MB); the oldest checkpoint per project
        ...
        # must not guess. Orphans: `loki checkpoints prune` (`--keep-orphans` to skip).
        "auto_prune": True,
        "retention_days": 7,
        "min_interval_hours": 24,
    },
    # Hard cap (chars) for one auto-loaded context file (SOUL.md, AGENTS.md, CLAUDE.md, .loki.md,
    ...
    # Same bound for single-query mode (``loki -q/-z``). With only ONE turn there is no
    ...
    "compression": {
        ...
        # Loki' summarizer cannot shrink it. native = codex decides; loki = Loki' threshold
    ...
    "auxiliary": {
        ...
        # of skills); route cheaper via `loki model` → auxiliary → Curator.
        "curator": _aux(600),
    ...
    "display": {
        ...
        # Interface bare `loki`/`loki chat` launches: "cli" (prompt_toolkit REPL) | "tui" (Ink).
        # Flags win: `--cli` forces the REPL, `--tui` / LOKI_TUI=1 forces the TUI.
        "interface": "cli",
        # `loki --tui` auto-resumes the most recent human-facing session (like `loki -c`).
        # LOKI_TUI_RESUME=<id> always wins.
        ...
        # WunderCorp credits status-bar notices (usage bands, grant-spent, depleted/restored). False mutes
        ...
        # `path`/`paths` = LOKI_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        # Gateway runtime footer on the FINAL message, e.g. `model · 68% · ~/projects/loki`.
        ...
        # CLI/TUI/desktop, managed with `loki pets`. No effect on prompt caching.
        "pet": {
            "enabled": False,
            "slug": "",   # active pet slug in get_loki_home()/pets/; empty → first installed
    ...
    "dashboard": {
        ...
        # client disconnects. 0 = park forever. Env: LOKI_TUI_WS_ORPHAN_REAP_GRACE_S.
        ...
        # newest message older than LOKI_TUI_SESSION_TTL_S, default 6h) with
        ...
        # OAuth gate (engaged when --host is set and --insecure is not), read by the WunderCorp Portal
        # plugin. Env LOKI_DASHBOARD_OAUTH_CLIENT_ID / LOKI_DASHBOARD_PORTAL_URL win when
        ...
        # no-op. Env LOKI_DASHBOARD_BASIC_AUTH_USERNAME / _PASSWORD_HASH / _PASSWORD / _SECRET /
        ...
        # LOKI_DASHBOARD_DRAIN_SECRET; no-op unless >=256-bit, weak secrets rejected
        # (fail-closed). scope = capability label; min_secret_chars in url-safe-b64 chars.
        "drain_auth": {"scope": "drain", "min_secret_chars": 43},
        # Public URL (env LOKI_DASHBOARD_PUBLIC_URL): full authority (scheme + host + optional
        # prefix, e.g. https://example.com/loki) for the OAuth redirect_uri; its hostname is
    ...
    "tts": {
        ...
        "piper": {
            ...
            # (~/.loki/cache/piper-voices/), use_cuda, length_scale (2.0 = twice as slow),
    ...
    # "Hey Loki" hands-free wake word: always-on, on-device hotword detection that starts a fresh
    ...
    "wake_word": {
        ...
        "openwakeword": {
            # "hey_loki" | built-in openWakeWord name ("hey_jarvis", "alexa", ...) | path to a
    ...
    # plugins/context_engine/<name>/ or ~/.loki/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "moa": {
        ...
        # <loki_home>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    # and resolved; read-only — creation goes to ~/.loki/skills/ unless create_dir redirects it.
    "skills": {
        ...
        # LOKI_HOME, scanned alongside the local dir.
        "create_dir": "",
        # In a git checkout, <root>/.loki/skills/ and <root>/.agents/skills/ load as the
        # highest-precedence tier — ONLY if the root is in trusted_project_dirs. false = no scan, no
        # untrusted-skills notice.
        "project_discovery": True,
        # Trusted project roots; managed by `loki skills trust` / `untrust`.
        "trusted_project_dirs": [],
        # Substitute ${LOKI_SKILL_DIR} / ${LOKI_SESSION_ID} in SKILL.md content.
        "template_vars": True,
        ...
        # Advisory NVIDIA SkillEvaluator Tier 1 scan on `loki skills install` (alongside the
        ...
        "write_approval": False,
        # Audit ledger: every skill mutation appends to ~/.loki/skills/.curator_ledger.jsonl with
        # before/after hashes (blobs under ~/.loki/.curator_backups/blobs/); powers `loki
    ...
    # daemon. `loki curator status` shows the last run.
    "curator": {
        "enabled": True,
        ...
        # aux-model cost. `loki curator run --consolidate` overrides once.
        "consolidate": False,
        # Also prune bundled built-ins (a suppression list stops `loki update` restoring them);
        ...
        "prune_builtins": True,
        # TTL purge of skills/.archive/: 0 = never; > 0 lets the explicit `loki curator purge`
        # delete older archived skills (never automatic; logged in the ledger).
        "archive_ttl_days": 0,
        # Before every real (non-dry-run) pass, snapshot ~/.loki/skills/ to
        # ~/.loki/skills/.curator_backups/<utc-iso>/skills.tar.gz (`loki curator rollback`).
        "backup": {
            "enabled": True,
    ...
    # peerName, sessions, enabled); loki-specific overrides only here.
    "honcho": {},
    ...
    #   cron job, a -q session (LOKI_INTERACTIVE=1 but nobody to answer), or an unattended
    ...
    "approvals": {
        ...
        # export LOKI_INTERACTIVE=1 (for interactive sudo prompts) but have NO user waiting to answer
        ...
        # Approve" → false. LOKI_TUI_NO_CONFIRM=1 skips the TUI modal.
    ...
    # are given.
    "platform_hints": {},
    # Plugin system. `enabled`/`disabled` lists are written by `loki plugins enable|disable` and
    ...
    "plugins": {
        ...
        # 2026-09-14 removal date (see COMPAT_MANIFEST.md, `loki plugins compat`). Stopgap only: the
    ...
    # approvals persist in ~/.loki/shell-hooks-allowlist.json. Schema + examples:
    # website/docs/user-guide/features/hooks.md.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt (also --accept-hooks or
    # LOKI_ACCEPT_HOOKS=1). Gateway/cron/non-interactive runs need one of these to pick up
    # newly-added hooks.
    ...
    "security": {  # Security: pre-exec scanning via tirith plus related guards.
        ...
        # .loki config) always need human approval, even under yolo. Extra patterns are fnmatch
        ...
        # banner. Add via `loki doctor --ack <id>`; remove by editing the list. Catalog:
        # loki_cli/security_advisories.py.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $LOKI_HOME/plugins/ <name>/),
        ...
        # holds NO scheduler credentials (provision reuses the WunderCorp Portal token).
        "chronos": {
            # NAS/portal base URL that arms/cancels one-shots and mints the inbound fire JWT (used
            # as the expected issuer).
            "portal_url": "https://portal.wundercorp.com",
        ...
        # Env override: LOKI_CRON_MAX_PARALLEL.
        ...
        "output_retention": 50,
        # Timeout (seconds) for a no-agent cron script. Env: LOKI_CRON_SCRIPT_TIMEOUT. Keep in
        ...
        # job's dispatch guard forever. Env: LOKI_CRON_SESSION_DB_TIMEOUT. 0 = unlimited.
        "session_db_timeout_seconds": 10,
        # Timeout (seconds) per media attachment send during gateway delivery; large attachments
        # (long TTS audio, big exports) need more than 30s. Env: LOKI_CRON_MEDIA_SEND_TIMEOUT.
    ...
    # promotes dependency-satisfied todos to ready, and fires `loki -p <assignee> chat -q ...` per
    ...
    "kanban": {
        ...
        # Auto-run the decomposer on Triage tasks every tick. False = manual via `loki kanban
    ...
    "code_execution": {  # execute_code settings (programmatic tool calls).
        ...
        # relative paths resolve. strict = isolated temp dir with loki-agent's own python
    ...
    # by tool_search / tool_describe / tool_call bridges and surfaced on demand. Core Loki tools
    ...
    "tools": {
        ...
        # Remote connector discovery/lifecycle through the WunderCorp tool gateway.
        # The flag is the user's off switch; availability additionally requires
        # the portal sign-in every managed tool gates on.
        "connectors": {"enabled": True},
    },
    "logging": {  # File logging to ~/.loki/logs/: agent.log captures INFO+, errors.log WARNING+.
        "level": "INFO",       # minimum level for agent.log: DEBUG, INFO, WARNING
        "max_size_mb": 5,      # max size per log file before rotation
        "backup_count": 3,     # rotated backups to keep
    },
    # Remote model-catalog manifest: curated OpenRouter / WunderCorp Portal model lists fetched from this
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://loki.computer/docs/api/model-catalog.json",
        # Disk cache TTL in minutes. The gateway refreshes in the background on this cadence; the
        # CLI refetches on the next /model or `loki model` once the cache is older. Network
    ...
    # defaults (200K context, tools on, vision/reasoning off) and get patched. Provider keys: Loki
    ...
    "monitoring": {
        ...
        "gateway_health_export": {  # Gateway health & diagnostics export.
            ...
            "resource_attributes": {
                "service.name": "loki-gateway", "deployment.environment.name": "production"
            },
        },
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # to LOKI_GATEWAY_PLATFORM_CONNECT_TIMEOUT, which wins if set explicitly.
        ...
        # run_gateway() bridges these to LOKI_STARTUP_WATCHDOG / LOKI_STARTUP_WATCHDOG_TIMEOUT_S
        # and re-arms the live handle; explicit env wins.
        "startup_watchdog": True,
        "startup_watchdog_timeout_seconds": 300,
        # Keep writing the legacy ~/.loki/sessions/sessions.json mirror of the routing index
        ...
        # (LOKI_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent AND a wakeUrl is
        ...
        # LOKI_GATEWAY_MAX_STARTS / LOKI_GATEWAY_START_WINDOW_S.
        ...
        # credential/system denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.loki/.env, auth.json). True:
        # files must be under the Loki cache, media_delivery_allow_dirs, or fresher than
        # trust_recent_files_seconds — recommended for public-facing gateways so prompt injection
        # can't exfiltrate host secrets. Bridged to LOKI_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra roots (project/scratch dirs, mounted shares) from which bare file paths may be
        # uploaded; the Loki cache is always trusted. List of absolute paths or one
        # os.pathsep-separated string; tildes expanded. Bridged to LOKI_MEDIA_ALLOW_DIRS. Honored
        # in both modes.
        ...
        # pure-allowlist mode. Bridged to LOKI_MEDIA_TRUST_RECENT_FILES. Only consulted when
        # strict is true.
        "trust_recent_files": True,
        # Recency window in seconds; 600 covers a multi-tool turn. Bridged to
        # LOKI_MEDIA_TRUST_RECENT_SECONDS. Only consulted when strict is true.
    ...
    # Automatic cleanup of ~/.loki/state.db, which otherwise grows without bound and slows FTS5
    ...
    "sessions": {
        ...
        "auto_prune": True,
        # Inactive days of ended-session history to keep (= `loki sessions prune`).
        ...
        # stay until `loki sessions optimize-storage` runs, since the rebuild is disk-heavy on
        # large DBs. advise = `loki update` prints a one-line notice with reclaimable size when a
        ...
        # (native/fts5_cjk/build.sh → ~/.loki/lib/libfts5_cjk.so), 1-2 char CJK terms get exact
        # index matches instead of LIKE scans. True = use when present (inert otherwise); False =
        # never load/serve it. Bridged to LOKI_CJK_FTS.
        "cjk_fts": True,
        ...
        # Max active messages per session for in-memory export (`loki sessions export`); checked
    ...
    # and transmission to WunderCorp (`send`) are SEPARATE opt-ins; see
    ...
    "telemetry": {
        "shared_metrics": {
            ...
            "endpoint": "https://telemetry.wundercorp.com/v1/telemetry",
        },
    },

    "doctor": {
        # Per-probe timeout (seconds) for `loki doctor --live` real-call probes.
        "live_probe_timeout": 10,
    },

    "updates": {
        # Passive version/banner checks only; explicit `loki update --check` remains enabled.
        "check": True,
        # Pre-update backup. quick = snapshot small critical state (pairing JSONs, cron jobs,
        # config.yaml, .env, auth.json, profile DBs) into <LOKI_HOME>/state-snapshots/, skipping
        # files >1 GiB; restore via ``/snapshot``. full = quick PLUS a ``loki backup`` zip in
        # <LOKI_HOME>/backups/ (``loki import`` restores; slow on large homes; ``--backup``
        ...
        # update cleanly. `loki update --switch-branch` overrides to switch for one run.
        "parked_branch_strategy": "switch",
        # Refresh an installed cua-driver during `loki update` (best-effort, macOS only). Turn off
    ...
    "lsp": {
        ...
        "wait_mode": "document",
        "wait_timeout": 5.0,
        # Missing server binaries: auto = install via npm/go/pip into <LOKI_HOME>/lsp/bin/ on
    ...
    # (SuperGrok OAuth or XAI_API_KEY) AND the toolset is enabled in `loki tools`.
    "x_search": {
    ...
    # encrypted vault (`loki vault add`, Desktop → Settings → Credential Vault) is always on.
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # ~/.loki/.env (or the shell), never in config.yaml.
            ...
            # Last-good fallback for NETWORK/TIMEOUT outages: AES-GCM cache under ~/.loki/cache/,
            ...
            # Auto-download bws into ~/.loki/bin/ on first use; False = bws must be on PATH.
            "auto_install": True,
    ...
    "computer_use": {
        # cua-driver's upstream PostHog telemetry defaults ON; Loki sets
        ...
        # #47032; Linux/WSL2 idle spin #28152). cua-driver ≥ 0.6.x supports --no-overlay; Loki also calls
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned binary into ~/.loki/bin/; False = iron-proxy on PATH.
        "auto_install": True,
    ...
    "desktop": {  # Loki Desktop (Electron) launch options; only affect `loki desktop`.
        ...
        # Bridged to LOKI_DESKTOP_DISABLE_GPU: auto = disable GPU only on remote displays
        ...
        # to LOKI_DESKTOP_PASSWORD_STORE; ignored off-Linux.
        "password_store": "auto",
    ...
    "wundercorp": {
        # Upper bound (seconds) on the WunderCorp auth keepalive tick, which derives from the
        ...
        # WunderCorp/api#227), so chat is the default until that is fixed.
        "anthropic_wire": "chat",
        # WunderCorp free tier: with no other provider configured, Loki sets up a free WunderCorp identity on
        # first use (inference on wundercorp/welcome + connectors) and offers `/login` (terminal:
        # `loki auth upgrade`) to sign in. false turns the free tier off entirely: nothing is set
    ...
    "local_runtime": {
        # Off = detection-only (Loki still finds an external llama-server you run).
        "enabled": False,
        # Pinned llama.cpp release tag; bumped by Loki releases after validation.
        "tag": "b10679",
...
OPTIONAL_ENV_VARS = {
    ...
    "WUNDERCORP_BASE_URL": _base_url("WunderCorp Portal"),
    "LOKI_ANON_API_SECRET": _env(
        "Shared secret for the WunderCorp free-tier sign-up endpoints while they are in their gated "
        "integration phase (not needed once the gate is removed)",
        "WunderCorp free-tier shared secret (leave empty unless given one)", password=True,
    ...
    "VERTEX_CREDENTIALS_PATH": _prov(
        ...
        "OAuth2, not a static API key — this points at the credentials Loki mints short-lived "
    ...
    "FIRECRAWL_GATEWAY_URL": _tool(
        "Exact Firecrawl tool-gateway origin override for WunderCorp Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": _tool(
        "Shared tool-gateway domain suffix for WunderCorp Subscribers only, used to derive vendor "
        "hosts, e.g. wundercorp.com -> firecrawl-gateway.wundercorp.com",
        "Tool-gateway domain suffix", None, password=False, advanced=True),
    "TOOL_GATEWAY_SCHEME": _tool(
        "Shared tool-gateway URL scheme for WunderCorp Subscribers only, used to derive vendor hosts "
    ...
    "TOOL_GATEWAY_USER_TOKEN": _tool(
        "Explicit WunderCorp Subscriber access token for tool-gateway requests (optional; otherwise "
        "read from the Loki auth store)", "Tool-gateway user token", None, advanced=True),
    ...
    "PORCUPINE_ACCESS_KEY": _tool(
        "Picovoice access key for the Porcupine 'Hey Loki' wake word engine (optional; "
    ...
    "LOKI_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "LOKI_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "LOKI_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
    ...
    "SLACK_ALLOWED_USERS": _msg(
        "Comma-separated Slack member IDs allowed to use Loki, e.g. U01ABC2DEF3. Without "
    ...
    "MATRIX_USER_ID": _msg("Matrix user ID (e.g. @loki:example.org)",
    ...
    "MATRIX_DEVICE_ID": _msg(
        "Stable Matrix device ID for E2EE persistence across restarts (e.g. LOKI_BOT)",
    ...
    "IRC_CHANNEL": _msg("IRC channel to join (e.g. #loki)", "IRC channel", None),
    "IRC_NICKNAME": _msg("Bot nickname on IRC (default: loki-bot)", "IRC nickname", None),
    ...
    "API_SERVER_MODEL_NAME": _msg(
        "Model name advertised on /v1/models. Defaults to the profile name (or 'loki-agent' "
    ...
    "GATEWAY_PROXY_URL": _msg(
        "URL of a remote Loki API server to forward messages to (proxy mode). When set, the "
        ...
        "Remote Loki API server URL (e.g. http://192.168.1.100:8642)", None, advanced=True),
    "GATEWAY_PROXY_KEY": _msg(
        "Bearer token for authenticating with the remote Loki API server (proxy mode). Must "
    ...
    # LOKI_TOOL_PROGRESS_MODE (deprecated; use display.tool_progress) is intentionally NOT listed:
    # this dict feeds user-facing surfaces (dashboard keys page, setup checklists), so deprecated
    # knobs stay in config._EXTRA_ENV_KEYS only. LOKI_TOOL_PROGRESS is unsupported.
    "LOKI_PREFILL_MESSAGES_FILE": _setting(
        "Path to JSON file with ephemeral prefill messages for few-shot priming",
        "Prefill messages file path", None),
    "LOKI_EPHEMERAL_SYSTEM_PROMPT": _setting(
