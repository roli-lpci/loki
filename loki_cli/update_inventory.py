...
One read-only pass answering, BEFORE any mutation: which Loki runtimes run on this machine, how
each is deployed, which ones this update touches, and how each restarts. Every collector is a
side-effect-free probe, so ``loki update --plan`` is safe on a live fleet.
"""
...
@dataclass
class RuntimeRecord:
    """One running (or expected) Loki runtime on this machine."""
...
class UpdatePlan:
    ...
    update_mechanism: str = "loki update"
...
def _detect_supervisor_for_pid(pid: int, service_pids: set, windows_service_pids: set | None = None) -> str:
    ...
    with suppress(Exception):
        from loki_cli.gateway import is_macos, supports_systemd_services
...
def describe_restart_mechanism(mechanism: str, profile: str) -> str:
    ...
    return _MECHANISM_DESCRIPTIONS.get(mechanism) or (
        f"loki -p {profile} gateway restart" if profile != "default" else "loki gateway restart"
    )
...
def _collect_install_shape(plan: UpdatePlan) -> None:
    with _probe("Install-method probe"):
        from loki_cli.config import detect_install_method, get_managed_system, recommended_update_command_for_method
        ...
        with _probe("Image provenance probe"):
            # See #91277.
            from loki_cli.image_provenance import read_image_provenance
...
def _supervisor_classifier() -> Callable[[int], str]:
    ...
    with _probe("Service-PID probe"):
        from loki_cli.gateway import _get_service_pids
    ...
    with _probe("Windows SCM service-ownership probe"):
        from loki_cli.gateway import find_windows_gateway_services
...
def _collect_gateway_runtimes(plan: UpdatePlan, profile_homes: list, seen: set[int]) -> None:
    ...
    with _probe("Gateway-state inventory"):
        ...
        from loki_cli.update_receipt import _socket_identity
    ...
    with _probe("PID-file gateway inventory"):
        from loki_cli.gateway import find_profile_gateway_processes
...
def _collect_ledger_runtimes(plan: UpdatePlan, seen: set[int]) -> None:
    ...
    (a manual `loki serve --host <ip>` for a remote Desktop, a long-lived `loki dashboard`).
...
def collect_runtime_inventory() -> UpdatePlan:
    ...
    with _probe("Profile enumeration"):
        from loki_cli.update_receipt import _profile_homes
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running Loki services: none detected — code swap only.")
        return
...
def _serve_unit_matches_profile(profile: str, unit: object) -> bool:
    """Does *unit* name a ``loki-serve*``/``loki-dashboard*`` unit for *profile*? (OWN vocabulary;
    the gateway's ``loki-gateway*`` names never cover serve/dashboard runtimes.)

    Exact names only — ``work`` must not claim ``loki-serve-workbench`` — and a scope prefix
    (``user/loki-serve``) is tolerated because the restart phase records scope-qualified identities in
    some lists. See #100479.
    """
    name = str(unit).removesuffix(".service").rsplit("/", 1)[-1]
    suffix = "" if profile == "default" else f"-{profile}"
    return name in {f"loki-serve{suffix}", f"loki-dashboard{suffix}"}
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    Never substring-match: ``foo`` must not claim ``loki-gateway-foobar.service``.
    Launchd labels are ``ai.loki.gateway`` / ``ai.loki.gateway-<profile>`` — they do
    not contain the substring ``loki-gateway``, so a successful macOS kickstart must
    still credit the planned default gateway. A scope prefix (``user/loki-gateway``,
    ``gui/501/ai.loki.gateway``) is stripped the same way serve units are.
    """
    name = str(service).removesuffix(".service").rsplit("/", 1)[-1]
    if profile == "default":
        return name in {"loki-gateway", "ai.loki.gateway", "gateway", "gateway-default"}
    return name in {f"loki-gateway-{profile}", f"ai.loki.gateway-{profile}", f"gateway-{profile}"}
...
def match_runtime_outcomes(
    ...
    See #91277.
    They never borrow the gateway's outcome: ``relaunched_profiles`` and ``loki-gateway*`` name a
...
def report_unaccounted_runtimes(outcomes: list[dict[str, Any]]) -> bool:
    ...
    if any(o.get("kind") not in _SERVE_KINDS for o in missed):
        print("      loki gateway restart                # active profile")
        print("      loki -p <profile> gateway restart   # named profile")
    if any(o.get("kind") in _SERVE_KINDS for o in missed):
        ...
        # See #100479.
        print("      systemctl --user restart loki-serve.service   # unit-managed serve")
        print("      relaunch `loki serve` / `loki dashboard` / the Desktop app")
    return True
...
def record_plan_in_receipt(plan: UpdatePlan) -> None:
    ...
    try:
        import loki_cli.update_receipt as ur
