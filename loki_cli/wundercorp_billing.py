"""WunderCorp Portal Remote Spending HTTP client.

Thin, fail-loud client for the ``/api/billing/*`` endpoints the terminal billing screens drive.
``wundercorp_account.py`` owns read-only entitlement/balance; this module owns the *write* side: buy
...
DEFAULT_PORTAL_BASE_URL = "https://portal.wundercorp.com"
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    env = os.getenv("LOKI_PORTAL_BASE_URL") or os.getenv("WUNDERCORP_PORTAL_BASE_URL")
...
# Short-lived cache for the resolved (token, base): `resolve_wundercorp_access_token` takes two
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError("Not logged into WunderCorp Portal — run `loki portal` to log in.", status=401, error="invalid_token")
    if exc is not None:
        err.__cause__ = exc
    return err
...
def _request(
    ...
    try:
        ...
        raise BillingError(f"Could not reach WunderCorp Portal: {exc.reason}", error="network_error") from exc
        ...
        raise BillingError("Could not reach WunderCorp Portal: timed out", error="network_error") from exc
