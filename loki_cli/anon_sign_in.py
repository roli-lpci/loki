...
from loki_cli.auth_constants import httpx


UPGRADE_START = "Sign in with a WunderCorp account to unlock more models and tools."
...
UPGRADE_UNAVAILABLE = "The free tier is not available right now; run `loki auth add wundercorp` to sign in."
UPGRADE_REASON_COPY = {
...
UPGRADE_NO_DEFAULT_TERMINAL = "No default model is set yet; run `loki model` to pick one."
...
LOGIN_DM_ONLY = "Sign in from a direct message with Loki."
LOGIN_BUSY_ELSEWHERE = "Another sign-in is already running on this Loki. Try again in a few minutes."
LOGIN_NOT_ALLOWED = "Only an operator of this Loki can sign it in."
FREE_TIER_RATE_LIMIT_CHAT = (
    "WunderCorp free tier rate limit active \u2014 resets in {reset}. "
    "Sign in with a WunderCorp account for higher limits: /login.")
...
# the in-chat form (never a raw exception, a URL or a ``loki`` verb) and ``.copy_terminal`` the
...
def _outcome_state(outcome: Dict[str, Any], anon_token: str) -> SignInState:
    ...
    """
    from loki_cli import anon_auth as _core
...
def run_sign_in(
    *,
    ...
    """Sign the free tier into a WunderCorp account, keeping its connectors. Yields :class:`SignInState`s.
    ...
    *client_factory* is the HTTP client seam, ``client_factory(timeout_seconds, verify)``.
    """
    from loki_cli import anon_auth as _core
    from loki_cli.auth import PROVIDER_REGISTRY, _resolve_verify
    from loki_cli.auth_device_flow import _request_device_code
    from loki_cli.auth_wundercorp import _wundercorp_http_client
    ...
    try:
        with open_scope():
            state = _core.current_wundercorp_state()
    ...
    try:
        ...
        open_client = client_factory or _wundercorp_http_client
    ...
    try:
        ...
        with open_scope():
            with guard() as may_persist:
                if may_persist:
                    _core.persist_wundercorp_credentials(account_state)
            if may_persist:
                settled = _core.settle_after_upgrade(account_state)
    except Exception as exc:
        # persist_wundercorp_credentials takes the auth-store lock, writes auth.json, takes the shared
