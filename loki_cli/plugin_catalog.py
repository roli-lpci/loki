"""Plugin catalog — curated, WunderCorp-approved Loki plugins shipped with the repo.
...
an installed Loki sees new entries and removals without updating. Any fetch failure falls back to the
...
LIVE_CATALOG_URL = "https://loki.computer/docs/api/plugin-catalog.json"
...
class PluginCatalogEntry:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "maintainer": self.maintainer, "tier": self.tier, "requires_loki": self.requires_loki,
...
def entry_from_mapping(data: Any, label: str) -> Optional[PluginCatalogEntry]:
    ...
    return PluginCatalogEntry(
        ...
        requires_loki=str(data.get("requires_loki") or "").strip(),
...
def _live_cache_path() -> Path:
    ...
    return get_loki_home() / "cache" / "plugin-catalog.json"
...
def fetch_live_catalog(*, force: bool = False) -> Optional[Dict[str, Any]]:
    ...
    ``LOKI_HOME/cache`` for :data:`LIVE_CATALOG_TTL_SECONDS`. ``None`` on ANY failure — callers fall
...
def entry_capability_summary(entry: PluginCatalogEntry) -> str:
    ...
    if entry.requires_loki:
        bits.append(f"Requires Loki {entry.requires_loki}.")
    return " ".join(bits)
