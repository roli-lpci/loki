...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "wundercorp", "openai-codex", "xai-oauth", "qwen-oauth", "minimax-oauth"}
...
def _anthropic_oauth_login(args) -> dict:
    ...
    creds = anthropic_mod.run_loki_oauth_login_pure()
    if not creds:
...
def _qwen_oauth_login(args) -> dict:
    from loki_cli.auth_qwen import _mark_qwen_oauth_active
...
class _OAuthAddSpec:
    """Per-provider parameters for the generic ``loki auth add <provider> --type oauth`` path."""
...
_OAUTH_ADD_SPECS: dict[str, _OAuthAddSpec] = {
    "anthropic": _OAuthAddSpec(
        ...
        source=f"{SOURCE_MANUAL}:loki_pkce",
...
def _add_wundercorp_oauth_credential(args, provider: str) -> PooledCredential:
    """``loki auth add wundercorp --type oauth``: shared-credential import, else device-code login."""
    custom_label = (getattr(args, "label", None) or "").strip() or None
    timeout = getattr(args, "timeout", None) or 15.0

    def _persist(creds: dict, what: str) -> PooledCredential:
        # `--label` is embedded into providers.wundercorp so label_from_token doesn't overwrite it on every
        # subsequent load_pool("wundercorp").
        entry = auth_mod.persist_wundercorp_credentials(creds, label=custom_label)
    ...
    # Codex-style auto-import: a shared WunderCorp credential at <loki-root>/shared/wundercorp_auth.json
    # (written by any previous login) makes `loki --profile <name> auth add wundercorp --type oauth`
    # a one-tap operation for multi-profile users.
    if auth_mod._read_shared_wundercorp_state():
        try:
            found = f"Found existing WunderCorp OAuth credentials at {auth_mod._wundercorp_shared_store_path()}"
        except RuntimeError:
            found = "Found existing shared WunderCorp OAuth credentials"
        print()
        print(found)
        ...
        if do_import is None or do_import.lower() in {"", "y", "yes"}:
            ...
            rehydrated = auth_mod._try_import_shared_wundercorp_state(timeout_seconds=timeout)
            if rehydrated is not None:
                return _persist(rehydrated, "Imported")
            # Expired refresh_token, portal down, etc. — fall through to device-code.
            print("Could not refresh shared credentials — falling back to device-code login.")

    creds = auth_mod._wundercorp_device_code_login(
...
def _add_credential(args, provider: str, pool, requested_type: str) -> PooledCredential:
    ...
    if provider == "wundercorp":
        return _add_wundercorp_oauth_credential(args, provider)

    spec = _OAUTH_ADD_SPECS.get(provider)
    if spec is None:
        raise SystemExit(f"`loki auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def _report_priority(provider: str, pool, moved, requested: int, verb: str, prep: str) -> None:
    ...
    print(f'{verb} {provider} credential "{moved.label}" {prep} priority {moved.priority} '
          f"(#{moved.priority + 1} in `loki auth list {provider}`)")
...
def auth_priority_command(args) -> None:
    """`loki auth priority <provider> <target> <priority>`: reorder one pooled credential."""
    provider = _normalize_provider(getattr(args, "provider", ""))
    pool = load_pool(provider)
...
def auth_list_command(args) -> None:
    ...
    for provider in providers:
        ...
        if not entries:
            continue
        current = pool.peek()
        if provider == "wundercorp" and all(_is_free_tier_entry(e) for e in entries):
...
def auth_remove_command(args) -> None:
    ...
    # Every credential source Loki reads from (env vars, external OAuth files, auth.json blocks,
...
def auth_refresh_command(args) -> None:
    """`loki auth refresh <provider> [target]`: force one pooled OAuth entry to refresh.
    ...
    if target is None or not str(target).strip():
        if len(entries) != 1:
            raise SystemExit(
                ...
                f"label (see `loki auth list {provider}`).")
    ...
    if (provider not in REFRESHABLE_OAUTH_PROVIDERS or matched.auth_type != AUTH_TYPE_OAUTH
        ...
        raise SystemExit(
            ...
            f"credential.")
    # WunderCorp's resolver is singleton-bound, not an independent-account refresher.
    if provider == "wundercorp" and matched.source != "device_code":
        raise SystemExit(
            f"wundercorp credential #{index} ({matched.label}) is not a refreshable OAuth "
            "credential: only the device_code singleton supports refresh. "
            "Reauthenticate with `loki auth add wundercorp --type oauth`.")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `loki auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `loki auth` is called bare."""
...
def auth_upgrade_command(args) -> None:
    """``loki auth upgrade``: sign the free tier into a WunderCorp account, keeping its connectors."""
    from loki_cli.anon_auth import upgrade_guest
