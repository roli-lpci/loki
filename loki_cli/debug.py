"""``loki debug`` debug tools for Loki Agent."""
...
_REDACTION_BANNER = (
    "[loki debug share: log content redacted at upload time. "
...
_USER_AGENT = "loki-agent/debug-share"
...
# ``loki debug`` sweeps on entry (CLI-only users). Replaced a fork-and-sleep subprocess that
# leaked ~20 MB per share.


def _pending_file() -> Path:
    return get_loki_home() / "pastes" / "pending.json"
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        pass  # non-fatal — worst case the user runs ``loki debug delete`` manually


def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    """Synchrowundercorply DELETE pending pastes whose ``expire_at`` has passed → (deleted, remaining).
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `loki debug share` "
...
def _upload_dpaste_com(content: str, expiry_days: int = 1) -> str:
    boundary = "----LokiDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from loki_cli.logs import LOG_FILES
    filename = LOG_FILES.get(log_name)
    return (get_loki_home() / "logs" / filename) if filename else None
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Loki Desktop on the machine running the app, not by this "
...
def _capture_dump() -> str:
    """Run ``loki dump`` and return its stdout as a string."""
    from loki_cli.dump import run_dump
    capture = io.StringIO()
...
def collect_debug_report(
    *, log_lines: int = 200, dump_text: str = "",
    ...
    ``dump_text`` is pre-captured dump output; when empty, ``loki dump`` is run internally.
    """
    buf = io.StringIO()
...
# WunderCorp-S3 envelope format id; the discord-support viewer keys off it.
_WUNDERCORP_BUNDLE_FORMAT = "loki-debug-share/1"
...
def build_wundercorp_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    """Gzip a :func:`collect_share_bundle` mapping into the WunderCorp envelope (shape parsed by the
    discord-support viewer — keep it stable)."""
    envelope = {"format": _WUNDERCORP_BUNDLE_FORMAT, "redacted": bool(redact),
...
def build_debug_share(
    ...
    Shared by ``loki debug share`` and the dashboard ``POST /api/ops/debug-share``. Blocking
    ...
    if redact:
        logger.info(
            "loki debug share: applied force-mode redaction to log snapshots before upload")
...
def run_debug_share(args):
    ...
    if getattr(args, "wundercorp", False):
        return _run_debug_share_wundercorp(args, log_lines=log_lines, redact=redact)
    ...
    try:
        ...
        print("\nRun `loki debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    if dpaste_urls:
        ...
        print(f"⚠️  {len(dpaste_urls)} of {len(result.urls)} upload(s) fell back to "
              ...
              "deleted with `loki debug delete`.\n"
              "\nShare these links with the Loki team for support.")
    else:
        print(f"\n⏱  Pastes will auto-delete in {result.auto_delete_seconds // 3600} hours.\n"
              "To delete now:  loki debug delete <url>\n"
              "\nShare these links with the Loki team for support.")


_WUNDERCORP_PRIVACY_NOTICE = """\
⚠️  --wundercorp: This uploads your debug bundle to WunderCorp-INTERNAL storage (AWS S3),
    NOT a public paste service. The following is included:
  • System info (OS, Python/Loki version, provider, which API keys are
  ...
  • The bundle is viewable only by WunderCorp staff (and allowlisted Discord mods)
...
def _run_debug_share_wundercorp(args, *, log_lines: int, redact: bool) -> None:
    """``loki debug share --wundercorp``: gzip the same bundle into the WunderCorp envelope → WunderCorp-S3."""
    from loki_cli.diagnostics_upload import share_to_wundercorp
    print(_WUNDERCORP_PRIVACY_NOTICE)
    if not _confirm_upload(args):
        return
    if not redact:
    ...
    bundle = collect_share_bundle(log_lines=log_lines, redact=redact)
    if redact:
        logger.info("loki debug share --wundercorp: applied force-mode redaction before upload")
    print("Uploading to WunderCorp diagnostics storage...")
    try:
        res = share_to_wundercorp(build_wundercorp_bundle(bundle, redact=redact))
    except Exception as exc:
        print(f"\nWunderCorp upload failed: {exc}\n"
              "\nThe WunderCorp diagnostics service may be unavailable or not yet provisioned.\n"
              "Run `loki debug share --local` to print the report instead, "
              "or `loki debug share` to upload to a public paste service.\n", file=sys.stderr)
        sys.exit(1)
    view_url = res.get("viewUrl") or res.get("view_url")
    expires_at = res.get("expiresAt") or res.get("expires_at")
    print("\nDebug bundle uploaded to WunderCorp (private):")
    ...
    print("\nShare this private link with the WunderCorp team — only WunderCorp staff "
          "(via Google login) can open it.\n"
          "\nPick up the discussion in:\n"
          "  GitHub Issues        https://github.com/wundercorp/loki/issues\n"
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: loki debug delete <url> [<url> ...]\n"
              "  Deletes paste.rs pastes uploaded by 'loki debug share'.")
        return
    for url in urls:
        try:
            if delete_paste(url):
...
Options (share):
  ...
  --wundercorp       Upload to WunderCorp-internal storage (private, staff-only,
