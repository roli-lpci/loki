...
WunderCorp Portal sale chrome and org-policy filtering, and the Vercel AI Gateway / Novita / Fireworks /
DeepInfra pricing adapters. Split out of ``loki_cli.models``; helpers still defined there are
looked up on ``loki_cli.models`` at call time so ``patch("loki_cli.models.<name>")`` mocks keep
intercepting.
"""
...
from loki_cli.models_reasoning_caps import _seed_reasoning_caps
...
def compute_sale_discount(prompt: str, completion: str, original: Any) -> tuple[int, str, str] | None:
    """Sale chrome from gateway ``pricing.original`` (WunderCorp Portal only; callers gate on the provider
...
def _get_json(url: str, headers: dict[str, str], timeout: float, opener=None) -> Optional[dict]:
    ...
    from loki_cli.models import _urlopen_model_catalog_request
...
def _pricing_entry(pricing: dict, prompt_key: str = "prompt", completion_key: str = "completion") -> dict[str, Any]:
    ...
    ``pricing`` block whose cache fields already use the loki names."""
    entry: dict[str, Any] = {
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    another's read. *include_sale_original* (WunderCorp Portal only) copies the gateway's pre-discount
    ``pricing.original`` rates through as a nested ``original`` dict for sale chrome."""
    from loki_cli.models import _LOKI_USER_AGENT
    ...
    headers = {"Accept": "application/json", "User-Agent": _LOKI_USER_AGENT}
    if api_key:
    ...
    for item in payload.get("data", []):
        ...
        if mid and isinstance(pricing, dict):
            ...
            # Sale chrome is WunderCorp Portal-only; never copy pricing.original for other catalogs.
...
_DEFAULT_WUNDERCORP_INFERENCE_BASE = "https://inference-api.wundercorp.com"


def _resolve_wundercorp_pricing_credentials() -> tuple[str, str]:
    """``(api_key, base_url)`` for WunderCorp Portal pricing; base_url is the bare origin (no ``/v1``).
    Precedence mirrors runtime credential resolution: ``WUNDERCORP_INFERENCE_BASE_URL`` (staging /
    ...
    try:
        ...
        env_base = _wundercorp_inference_env_override()
    except Exception:
        env_base = None
    api_key = creds_base = ""
    try:
        ...
        pass
    base_url = (env_base or creds_base or _DEFAULT_WUNDERCORP_INFERENCE_BASE).rstrip("/")
    if base_url.endswith("/v1"):
        base_url = base_url[:-3]
    return (api_key, base_url)


# How long a WunderCorp catalog stays trusted. Its contents depend on the org's policy, which an admin
...
_WUNDERCORP_CATALOG_TTL_SECONDS = 300.0


def _fetch_wundercorp_pricing(api_key: str, base_url: str, *, force_refresh: bool) -> dict[str, dict[str, Any]]:
    """Shared by pricing and policy lookups so both read one cache entry."""
    return fetch_models_with_pricing(
        api_key=api_key,
        base_url=base_url,
        force_refresh=force_refresh,
        include_sale_original=True,  # Sale chrome (pricing.original) is WunderCorp Portal-only.
        cache_ttl_seconds=_WUNDERCORP_CATALOG_TTL_SECONDS,
    )


def wundercorp_policy_allowed_ids(*, force_refresh: bool = False) -> Optional[set[str]]:
    """The WunderCorp model ids the caller's org may reach (keys of an authenticated ``GET /v1/models``,
    ...
    try:
        from loki_cli.wundercorp_account import wundercorp_policy_present

        if wundercorp_policy_present() is not True:
            return None
    except Exception:
        return None
    ...
    if not api_key or not base_url:
        return None
    return set(_fetch_wundercorp_pricing(api_key, base_url, force_refresh=force_refresh)) or None
...
_WUNDERCORP_POLICY_APPEND_MAX = 64


def restrict_to_wundercorp_policy(
    model_ids: list[str],
    ...
    *rescue_empty*: an allowlist naming only models the curated manifest lacks would leave an empty
    ...
    if rescue_empty and not kept and len(allowed) <= _WUNDERCORP_POLICY_APPEND_MAX:
        return sorted(allowed)
    return kept


def _remember_provider_cache_key(provider: str, cache_key: str) -> None:
    from loki_cli.models import _pricing_profile_key
...
def _fetch_wundercorp_pricing_for_provider(*, force_refresh: bool = False) -> dict[str, dict[str, Any]]:
    api_key, base_url = _resolve_wundercorp_pricing_credentials()
    if not base_url:
        return {}
    _remember_provider_cache_key("wundercorp", base_url.rstrip("/"))
    return _fetch_wundercorp_pricing(api_key, base_url, force_refresh=force_refresh)
...
def get_cached_wundercorp_inference_base_url() -> str:
    """The profile's persisted WunderCorp endpoint (bare origin, no ``/v1``) without refreshing auth."""
    try:
        from loki_cli.auth import (
            _load_auth_store, _load_provider_state, _optional_base_url, _validate_wundercorp_inference_url_from_network,
        )

        state = _load_provider_state(_load_auth_store(), "wundercorp") or {}
        url = _validate_wundercorp_inference_url_from_network(_optional_base_url(state.get("inference_base_url"))) or ""
        return url.rstrip("/").removesuffix("/v1")
    except Exception:
        return ""


# Static endpoint identity per provider; dynamic ones (deepinfra, wundercorp) are resolved in pricing_cache_scope.
...
def pricing_cache_scope(provider: str, *, current_provider: str = "", current_base_url: str = "") -> str:
    ...
    from loki_cli.models import _deepinfra_catalog_url, _pricing_profile_key, normalize_provider
    ...
    if normalized == "wundercorp":
        ...
        if normalize_provider(current_provider) == "wundercorp" and current_base_url:
            return current_base_url.rstrip("/").removesuffix("/v1")
        persisted_base = get_cached_wundercorp_inference_base_url()
        if persisted_base:
            return persisted_base
        return _pricing_provider_cache_keys.get((_pricing_profile_key(), normalized), _DEFAULT_WUNDERCORP_INFERENCE_BASE)
    return ""
...
def _cached_only_pricing(normalized: str) -> dict[str, dict[str, str]]:
    ...
    from loki_cli.models import _deepinfra_catalog_cache, _deepinfra_catalog_url, _pricing_profile_key
    if normalized == "deepinfra":
...
def get_pricing_for_provider(
    ...
    """Return live pricing for providers that support it (openrouter, wundercorp, ai-gateway, novita,
...
def _fetch_novita_pricing(timeout: float = 8.0, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "Accept": "application/json", "User-Agent": _LOKI_USER_AGENT}
...
def _fetch_deepinfra_pricing(timeout: float = 5.0, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    helper)."""
    from loki_cli.models import _fetch_deepinfra_models_by_tag
...
_PRICING_FETCHERS = {
    ...
    "wundercorp": _fetch_wundercorp_pricing_for_provider,
}
