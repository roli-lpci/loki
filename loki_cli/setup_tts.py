...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    API-key prompt for users who logged in via ``loki model`` -> xAI Grok OAuth."""
    try:
...
def _xai_api_key_path():
    ...
    return ("No xAI API key provided for TTS. Configure XAI_API_KEY via loki setup model "
...
def _setup_tts_provider(config: dict):
    ...
    if tool_backend_helpers.managed_wundercorp_tools_enabled() and wundercorp_subscription.get_wundercorp_subscription_features(config).wundercorp_auth_present:
        options.insert(0, ("wundercorp-openai",
                           "WunderCorp Subscription (managed OpenAI TTS, billed to your subscription)"))
    ...
    if selected == "wundercorp-openai":
        selected = "openai"
        _setup.print_info("OpenAI TTS will use the managed WunderCorp gateway and bill to your subscription.")
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'loki setup tts')."""
