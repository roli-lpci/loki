...
def _strip_loki_path_lines(content: str) -> str:
    """Drop the ``# Loki Agent`` marker (+ its PATH line) and any loki PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
    for line in content.split('\n'):
        if '# Loki Agent' in line or '# loki-agent' in line:
            skip_next = True
            continue
        if skip_next and ('loki' in line.lower() and 'PATH' in line):
            skip_next = False
            continue
        skip_next = False
        if 'loki' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
            continue
...
def remove_path_from_shell_configs():
    """Remove Loki PATH entries from shell configuration files."""
    removed_from = []
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            content = config_path.read_text(encoding="utf-8")
            new_content = _strip_loki_path_lines(content)
...
def remove_wrapper_script():
    """Remove the loki wrapper script if it exists."""
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a loki_cli / loki-agent reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('loki_cli' in content or 'loki-agent' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("loki", "loki", "loki-acp", "loki-agent"))
...
def remove_node_symlinks(loki_home: Path) -> list:
    ...
    node_dir = (loki_home / "node").resolve()
...
def uninstall_gateway_service():
    ...
    try:
        from loki_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_launchd_gateway() -> bool:
    ...
    from loki_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from loki_cli import gateway_windows as gw
...
# LOKI_HOME / LOKI_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\loki\git\{cmd,bin,usr\bin}, ...\loki\node), PortableGit + Node copies
...
def _loki_path_markers(loki_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    r"""Prefixes identifying Loki-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    root = str(loki_home).rstrip("\\/")
    subs = ("loki-agent", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(loki_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Loki-owned entries from User-scope PATH in the registry (see ``_loki_path_markers``)."""
    markers = tuple(m.lower() for m in _loki_path_markers(loki_home, include_managed_bin=include_managed_bin))
...
def remove_loki_env_vars_windows() -> list[str]:
    """Delete LOKI_HOME and LOKI_GIT_BASH_PATH from User-scope env vars."""
    def edit(winreg, key, removed):
        for name in ("LOKI_HOME", "LOKI_GIT_BASH_PATH"):
            try:
...
def remove_portable_tooling_windows(loki_home: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``loki_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (loki_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``loki`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
    ...
    try:
        ...
        from loki_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from loki_constants import get_default_loki_root
        bin_dir = get_default_loki_root() / "bin"
    except Exception as e:
...
def _is_default_loki_home(loki_home: Path) -> bool:
    """Return True when ``loki_home`` points at the default (non-profile) root."""
    try:
        from loki_constants import get_default_loki_root
        return loki_home.resolve() == get_default_loki_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``loki -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current LOKI_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m loki_cli.main` (the `loki` wrapper may be half-gone).
    loki_invocation = [sys.executable, "-m", "loki_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                loki_invocation + ["gateway", subcmd], capture_output=True, text=True,
    ...
    # 3. Wipe the profile's LOKI_HOME directory.
    _rmtree_step(profile.path, indent="  ", fully=False)


def run_gui_uninstall(args):
    """``loki uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from loki_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    loki_home = get_loki_home()
    summary = gui_install_summary(loki_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    _print_box("│         𖤍 Loki Chat GUI Uninstaller                  │", Colors.MAGENTA)
    print()

    if not summary["gui_installed"]:
        print("No Loki Chat GUI installation was found.")
        print(f"  Checked: {loki_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Loki agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(loki_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Loki agent at {loki_home / 'loki-agent'}")
        print(f"  • Your config, sessions, and secrets under {loki_home}")
        print()
    ...
    print()
    uninstall_gui(loki_home)

    print()
    _print_box("│            ✓ Chat GUI Uninstalled!                      │", Colors.GREEN)
    print()
    print("The Loki agent is still installed. Run 'loki' to use the CLI,")
    print("or 'loki uninstall' to remove the agent too.")
    print()


def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.loki/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, loki_home=loki_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie LOKI_HOMEs and systemd units behind.
    is_default_profile = _is_default_loki_home(loki_home)
    ...
    print()
    _print_box("│            𖤍 Loki Agent Uninstaller                  │", Colors.MAGENTA)
    print()

    # Show what will be affected
    print(color("Current Installation:", Colors.CYAN, Colors.BOLD))
    print(f"  Code:    {project_root}")
    print(f"  Config:  {loki_home / 'config.yaml'}")
    print(f"  Secrets: {loki_home / '.env'}")
    print(f"  Data:    {loki_home / 'cron/'}, {loki_home / 'sessions/'}, {loki_home / 'logs/'}")
    print()
    ...
    # services, alias wrappers, LOKI_HOME dirs) — otherwise they leave zombie services behind.
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Loki data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
        if remove_profiles:
            print(color(f"   Plus {n_profiles} profile(s): {profile_names}", Colors.RED))
    else:
        print("This will remove the Loki code but keep your configuration and data.")
    ...
    _perform_uninstall(
        project_root=project_root, loki_home=loki_home, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, loki_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Loki PATH entries from shell configs / Windows User PATH")
    print("  • Loki wrapper scripts and Loki-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if not full_uninstall:
        print(f"  • Keep Loki config/data: {loki_home}")
    else:
        print(f"  • Loki config/data: {loki_home}")
        profiles = _discover_named_profiles() if _is_default_loki_home(loki_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$LOKI_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: loki_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; loki\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_loki_home(loki_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(loki_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No Loki-owned PATH entries in User environment"),
        (windows, "Removing LOKI_HOME / LOKI_GIT_BASH_PATH User env vars...",
         remove_loki_env_vars_windows, "Removed User env var: {}", "No Loki-set User env vars to remove"),
        (True, "Removing loki command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows loki launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows loki launchers found"),
        (True, "Removing Loki-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(loki_home), "Removed {}", "No Loki-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE LOKI_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from loki_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(loki_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(loki_home), "Removed {}",
            "No Windows installer artifacts to remove")

    # 5. Optionally remove ~/.loki/ data directory (and named profiles)
    if full_uninstall:
        ...
        _rmtree_step(loki_home)
    else:
        log_info(f"Keeping configuration and data in {loki_home}")
    ...
    print()
    print("Thank you for using Loki Agent! 𖤍")
    print()


_REINSTALL_HINT = {
    True: "  iex (irm https://loki.computer/install.ps1)",
    False: "  curl -fsSL https://loki.computer/install.sh | bash"}
...
def main(argv=None) -> int:
    """``python -m loki_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``loki_constants`` + ``loki_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m loki_cli.uninstall")
    parser.add_argument(
