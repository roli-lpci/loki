"""Loki Agent — Web UI server: FastAPI app assembly, auth/host middleware, ``start_server``.
...
Usage: ``python -m loki_cli.main web [--port 8080]``.
"""
...
from loki_cli.install_identity import get_install_id as _shared_get_install_id
from loki_cli.pty_session import run_reaper
...
try:
    ...
    # running `loki dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["LOKI_WEB_DIST"]) if "LOKI_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)


from loki_cli.web_server_lifecycle import (  # noqa: E402
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The desktop spawns a ``loki dashboard`` backend, not a gateway, so without
    ...
    if isinstance(provider, InProcessCronScheduler):
        try:
            from loki_cli.profiles import profiles_to_serve
            ...
            if len(profile_homes) > 1:
                ...
                from loki_cli.profiles import _check_gateway_running

                start_kwargs["profile_gate"] = lambda _name, home: not _check_gateway_running(Path(home))
                from loki_logging import enable_profile_log_routing
...
async def _lifespan(app: "FastAPI"):
    ...
    # (#79531/#80037): a store left behind by `loki update` otherwise 500s
    ...
    # Import loki_cli.gateway *before* the yield: on Windows + 3.11 the
    ...
    # refuse with "restart required" after `loki update` replaced the code
    ...
    # Desktop-spawned backends (LOKI_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `loki
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("LOKI_DESKTOP") == "1":
        ...
        try:
            from loki_cli.gateway import _reap_unsupervised_gateway_orphans
    ...
    def _boot_local_runtime():
        try:
            from loki_cli.config import load_config
            from loki_cli.local_runtime.bootstrap import ensure_local_runtime
    ...
    # WunderCorp free tier: the ONE place its identity is created. Inventories credentials, mints only
    # when LOKI_GUEST_ONBOARDING=1, records the answer for setup.status / free_tier.status and
    ...
    from loki_cli.free_tier_bootstrap import start_background_bootstrap
    ...
    try:
        ...
        try:
            from loki_cli.local_runtime.bootstrap import shutdown_local_runtime
...
app = FastAPI(title="Loki Agent", version=__version__, lifespan=_lifespan)


# Memory-provider OAuth connect routes live in the memory layer, not here.
from loki_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402

app.include_router(_memory_oauth_router)

# Session token for sensitive endpoints. The desktop shell mints it via
# LOKI_DASHBOARD_SESSION_TOKEN; otherwise fresh per server start. It dies with
# the process and is injected into the SPA HTML so only the web UI can use it.
def _resolve_session_token() -> str:
    return os.environ.get("LOKI_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
_SESSION_HEADER_NAME = "X-Loki-Session-Token"
...
def _apply_ssh_owner_nonce(nonce: Optional[str]) -> None:
    ...
    if nonce:
        ...
        try:
            marker = os.path.join(purelib, f".loki-ssh-runtime-{nonce}")
...
from loki_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS as _PUBLIC_API_PATHS
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    """
    from loki_cli.dashboard_auth.prefix import resolve_public_url
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    old launch scripts but IGNORED since the June 2026 loki-0day campaign.
    """
...
def _desktop_loopback_auth_exempt(
    ...
    ``LOKI_DESKTOP=1``, and an operator-minted credential (env token, SSH
    session token, or owner nonce).
    """
    return (
        host in _LOOPBACK_HOST_VALUES
        and os.environ.get("LOKI_DESKTOP") == "1"
        and bool(os.environ.get("LOKI_DASHBOARD_SESSION_TOKEN") or ssh_session_token or ssh_owner_nonce)
    )
...
async def _plugin_api_runtime_gate(request: Request, call_next):
    ...
    if plugin_name and (
        ...
        try:
            ...
            # (#46435)
            from loki_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
...
async def _dashboard_auth_gate(request: Request, call_next):
    ...
    """
    from loki_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    """
    from loki_cli.dashboard_auth.token_auth import token_auth_middleware
...
from loki_cli import web_server_gateway as _gateway_mod  # noqa: E402
from loki_cli.web_server_gateway import _ACTION_LOG_FILES, _terminate_desktop_managed_gateway  # noqa: E402
from loki_cli.web_server_sessions import _auto_archive_ticker_loop  # noqa: E402
from loki_cli.web_server_chat import PTY_REGISTRY  # noqa: E402
from loki_cli.web_server_dashboard import (  # noqa: E402
...
# persisted under the ROOT Loki home (not the profile LOKI_HOME) so every
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``loki gateway restart``, reusing an in-flight or recent restart.
    ...
    proc = _gateway_mod._spawn_loki_action(subcommand, "gateway-restart")
...
from loki_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _no_auth_provider_message(host: str) -> str:
    ...
    try:
        from plugins.dashboard_auth import wundercorp as _wundercorp_plugin

        if _wundercorp_plugin.LAST_SKIP_REASON:
            skip_reasons.append(f"  • wundercorp: {_wundercorp_plugin.LAST_SKIP_REASON}")
    except Exception:
        pass
    ...
    if host in _LOOPBACK_HOST_VALUES:
        ...
        fix_hint = (
            ...
            "(and unset LOKI_DASHBOARD_PUBLIC_URL) to restore the "
    ...
    fix_hint += (
        ...
        "  • OAuth: run `loki dashboard register` (WunderCorp Portal) or "
    ...
    try:
        from loki_cli.config import load_config as _load_cfg
        from loki_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
        ...
        if has_creds and (set(disabled) & _BASIC_AUTH_PLUGIN_KEYS):
            fix_hint = (
                ...
                "`loki plugins enable basic`), then restart the "
                "dashboard.\n\n"
            ) + fix_hint
    except Exception:
        pass
...
def _configure_auth_gate(
    ...
    # ``--insecure`` no longer disables the gate (June 2026 loki-0day
    ...
    if app.state.auth_required:
        ...
        from loki_cli.dashboard_auth import list_providers
...
def _build_uvicorn_server(host: str, port: int, *, ssh_isolated: bool = False):
    ...
    if ssh_isolated:
        from loki_cli.web_server_idle_exit import (
...
def _on_server_started(
    ...
    *,
    ...
    def _reap_desktop_serves() -> None:
        from loki_cli.dashboard_procs import _reap_orphaned_desktop_local_serves

        _reap_orphaned_desktop_local_serves()

    def _reap_mcp_helpers() -> None:
        from loki_cli.process_identity import reap_orphaned_mcp_helpers
    ...
    # No-op for standalone `loki serve` (no LOKI_PARENT_PID).
    _start_parent_death_watchdog()
    ...
    if getattr(app.state, "ssh_isolated_clients", None) is not None:
        from loki_cli.web_server_idle_exit import DEFAULT_IDLE_GRACE_S, start_idle_watchdog
        try:
    ...
    # ACTUAL port — what lets `loki update` relaunch a manually-started serve
    # on its real endpoint (#63206).
    def _register_identity() -> None:
        from loki_cli.process_identity import attach_self_to_kill_on_close_job, register_self
    ...
    ready_token = "LOKI_BACKEND_READY" if headless else "LOKI_DASHBOARD_READY"
    ...
    if headless:
        ...
        print(f"  Loki backend listening on {host}:{actual_port}", flush=True)
    else:
        print(f"  Loki Web UI → http://{host}:{actual_port}")
    ...
    if start_mcp_discovery_after_bind:
        ...
        try:
            from loki_cli.mcp_startup import defer_background_mcp_discovery
...
def start_server(
    ...
    JSON-RPC/WS backend, no UI build, no SPA mount (``LOKI_SERVE_HEADLESS``).
    ...
    from loki_cli.resource_limits import apply_nofile_soft_limit
    ...
    try:
        from loki_cli.wundercorp_auth_keepalive import start_wundercorp_auth_keepalive

        start_wundercorp_auth_keepalive()
    except Exception as exc:
        _log.debug("WunderCorp auth keepalive did not start: %s", exc)
...
_PLUGIN_COMPAT_LAZY = {
    'AudioTranscriptionRequest': ('loki_cli.web_models', 'AudioTranscriptionRequest'),
    'AutomationBlueprintInstantiate': ('loki_cli.web_models', 'AutomationBlueprintInstantiate'),
    'BackupRequest': ('loki_cli.web_models', 'BackupRequest'),
    'BulkDeleteSessions': ('loki_cli.web_models', 'BulkDeleteSessions'),
    'CONFIG_SCHEMA': ('loki_cli.web_server_config', 'CONFIG_SCHEMA'),
    'ChatImageUpload': ('loki_cli.web_models', 'ChatImageUpload'),
    'ConfigUpdate': ('loki_cli.web_models', 'ConfigUpdate'),
    'CredentialPoolAdd': ('loki_cli.web_models', 'CredentialPoolAdd'),
    'CronJobCreate': ('loki_cli.web_models', 'CronJobCreate'),
    'CronJobUpdate': ('loki_cli.web_models', 'CronJobUpdate'),
    'CuratorPause': ('loki_cli.web_models', 'CuratorPause'),
    'CustomEndpointUpdate': ('loki_cli.web_models', 'CustomEndpointUpdate'),
    'DEFAULT_CONFIG': ('loki_cli.config', 'DEFAULT_CONFIG'),
    'DebugShareRequest': ('loki_cli.web_models', 'DebugShareRequest'),
    'EnvVarDelete': ('loki_cli.web_models', 'EnvVarDelete'),
    'EnvVarReveal': ('loki_cli.web_models', 'EnvVarReveal'),
    'EnvVarUpdate': ('loki_cli.web_models', 'EnvVarUpdate'),
    'FontSetBody': ('loki_cli.web_models', 'FontSetBody'),
    'FsWriteText': ('loki_cli.web_models', 'FsWriteText'),
    'GitBranchSwitchBody': ('loki_cli.web_models', 'GitBranchSwitchBody'),
    'GitCommitBody': ('loki_cli.web_models', 'GitCommitBody'),
    'GitFileBody': ('loki_cli.web_models', 'GitFileBody'),
    'GitPathBody': ('loki_cli.web_models', 'GitPathBody'),
    'GitWorktreeAddBody': ('loki_cli.web_models', 'GitWorktreeAddBody'),
    'GitWorktreeRemoveBody': ('loki_cli.web_models', 'GitWorktreeRemoveBody'),
    'HookCreate': ('loki_cli.web_models', 'HookCreate'),
    'HookDelete': ('loki_cli.web_models', 'HookDelete'),
    'ImportRequest': ('loki_cli.web_models', 'ImportRequest'),
    'LearningNodeEdit': ('loki_cli.web_models', 'LearningNodeEdit'),
    'LearningNodeRef': ('loki_cli.web_models', 'LearningNodeRef'),
    'MCPCatalogInstall': ('loki_cli.web_models', 'MCPCatalogInstall'),
    'MCPEnabledToggle': ('loki_cli.web_models', 'MCPEnabledToggle'),
    'MCPServerCreate': ('loki_cli.web_models', 'MCPServerCreate'),
    'MCPServersReplace': ('loki_cli.web_models', 'MCPServersReplace'),
    'ManagedDirectoryCreate': ('loki_cli.web_models', 'ManagedDirectoryCreate'),
    'ManagedFileDelete': ('loki_cli.web_models', 'ManagedFileDelete'),
    'ManagedFileUpload': ('loki_cli.web_models', 'ManagedFileUpload'),
    'ManagedFilesPolicy': ('loki_cli.web_server_files', 'ManagedFilesPolicy'),
    'MemoryProviderConfigUpdate': ('loki_cli.web_models', 'MemoryProviderConfigUpdate'),
    'MemoryProviderSelect': ('loki_cli.web_models', 'MemoryProviderSelect'),
    'MemoryProviderSetupRequest': ('loki_cli.web_models', 'MemoryProviderSetupRequest'),
    'MemoryReset': ('loki_cli.web_models', 'MemoryReset'),
    'MessagingPlatformUpdate': ('loki_cli.web_models', 'MessagingPlatformUpdate'),
    'MoaConfigPayload': ('loki_cli.web_models', 'MoaConfigPayload'),
    'MoaModelSlot': ('loki_cli.web_models', 'MoaModelSlot'),
    'MoaPresetPayload': ('loki_cli.web_models', 'MoaPresetPayload'),
    'ModelAssignment': ('loki_cli.web_models', 'ModelAssignment'),
    'OAuthSubmitBody': ('loki_cli.web_models', 'OAuthSubmitBody'),
    'OPTIONAL_ENV_VARS': ('loki_cli.config', 'OPTIONAL_ENV_VARS'),
    'PairingApprove': ('loki_cli.web_models', 'PairingApprove'),
    'PairingRevoke': ('loki_cli.web_models', 'PairingRevoke'),
    'ProfileActiveUpdate': ('loki_cli.web_models', 'ProfileActiveUpdate'),
    'ProfileCreate': ('loki_cli.web_models', 'ProfileCreate'),
    'ProfileDescribeAuto': ('loki_cli.web_models', 'ProfileDescribeAuto'),
    'ProfileDescriptionUpdate': ('loki_cli.web_models', 'ProfileDescriptionUpdate'),
    'ProfileModelUpdate': ('loki_cli.web_models', 'ProfileModelUpdate'),
    'ProfileRename': ('loki_cli.web_models', 'ProfileRename'),
    'ProfileSoulUpdate': ('loki_cli.web_models', 'ProfileSoulUpdate'),
    ...
    'PtyBridge': ('loki_cli.pty_bridge', 'PtyBridge'),
    'PtySessionRegistry': ('loki_cli.pty_session', 'PtySessionRegistry'),
    'PtyUnavailableError': ('loki_cli.pty_bridge', 'PtyUnavailableError'),
    'RawConfigUpdate': ('loki_cli.web_models', 'RawConfigUpdate'),
    'RegistryFull': ('loki_cli.pty_session', 'RegistryFull'),
    'STORAGE_HONCHO_HOST_BLOCK': ('plugins.memory.config_schema', 'STORAGE_HONCHO_HOST_BLOCK'),
    'SessionImport': ('loki_cli.web_models', 'SessionImport'),
    'SessionPrune': ('loki_cli.web_models', 'SessionPrune'),
    'SessionRename': ('loki_cli.web_models', 'SessionRename'),
    'SkillContentUpdate': ('loki_cli.web_models', 'SkillContentUpdate'),
    'SkillCreate': ('loki_cli.web_models', 'SkillCreate'),
    'SkillInstallRequest': ('loki_cli.web_models', 'SkillInstallRequest'),
    'SkillToggle': ('loki_cli.web_models', 'SkillToggle'),
    'SkillUninstallRequest': ('loki_cli.web_models', 'SkillUninstallRequest'),
    'SkillsUpdateRequest': ('loki_cli.web_models', 'SkillsUpdateRequest'),
    'TTSLeaseRequest': ('loki_cli.web_models', 'TTSLeaseRequest'),
    'TTSSpeakRequest': ('loki_cli.web_models', 'TTSSpeakRequest'),
    'TelegramOnboardingApply': ('loki_cli.web_models', 'TelegramOnboardingApply'),
    'TelegramOnboardingStart': ('loki_cli.web_models', 'TelegramOnboardingStart'),
    'TerminalBackendSelect': ('loki_cli.web_models', 'TerminalBackendSelect'),
    'ThemeSetBody': ('loki_cli.web_models', 'ThemeSetBody'),
    'ToolsetEnvUpdate': ('loki_cli.web_models', 'ToolsetEnvUpdate'),
    'ToolsetModelSelect': ('loki_cli.web_models', 'ToolsetModelSelect'),
    'ToolsetPostSetup': ('loki_cli.web_models', 'ToolsetPostSetup'),
    'ToolsetProviderSelect': ('loki_cli.web_models', 'ToolsetProviderSelect'),
    'ToolsetToggle': ('loki_cli.web_models', 'ToolsetToggle'),
    'WebhookCreate': ('loki_cli.web_models', 'WebhookCreate'),
    'WebhookEnabledToggle': ('loki_cli.web_models', 'WebhookEnabledToggle'),
    'WhatsAppOnboardingApply': ('loki_cli.web_models', 'WhatsAppOnboardingApply'),
    'WhatsAppOnboardingStart': ('loki_cli.web_models', 'WhatsAppOnboardingStart'),
    'activate_custom_endpoint': ('loki_cli.web_routers.config_env', 'activate_custom_endpoint'),
    'add_credential_pool_entry': ('loki_cli.web_routers.ops', 'add_credential_pool_entry'),
    'add_mcp_server': ('loki_cli.web_routers.mcp', 'add_mcp_server'),
    'apply_telegram_onboarding': ('loki_cli.web_routers.messaging', 'apply_telegram_onboarding'),
    'apply_whatsapp_onboarding': ('loki_cli.web_routers.messaging', 'apply_whatsapp_onboarding'),
    'approve_pairing': ('loki_cli.web_routers.ops', 'approve_pairing'),
    'auth_mcp_server': ('loki_cli.web_routers.mcp', 'auth_mcp_server'),
    'build_cron_model_impact': ('loki_cli.config', 'build_cron_model_impact'),
    'bulk_delete_sessions_endpoint': ('loki_cli.web_routers.sessions', 'bulk_delete_sessions_endpoint'),
    'cancel_oauth_session': ('loki_cli.web_routers.oauth', 'cancel_oauth_session'),
    'cancel_telegram_onboarding': ('loki_cli.web_routers.messaging', 'cancel_telegram_onboarding'),
    'cancel_whatsapp_onboarding': ('loki_cli.web_routers.messaging', 'cancel_whatsapp_onboarding'),
    'cfg_get': ('loki_cli.config', 'cfg_get'),
    'check_config_version': ('loki_cli.config', 'check_config_version'),
    'check_loki_update': ('loki_cli.web_routers.actions', 'check_loki_update'),
    'clear_model_endpoint_credentials': ('loki_cli.config', 'clear_model_endpoint_credentials'),
    'clear_pending_pairing': ('loki_cli.web_routers.ops', 'clear_pending_pairing'),
    'coerce_provider_id': ('loki_cli.config', 'coerce_provider_id'),
    'console_ws': ('loki_cli.web_routers.chat_ws', 'console_ws'),
    'count_empty_sessions_endpoint': ('loki_cli.web_routers.sessions', 'count_empty_sessions_endpoint'),
    'create_cron_job': ('loki_cli.web_routers.cron', 'create_cron_job'),
    'create_hook': ('loki_cli.web_routers.ops', 'create_hook'),
    'create_managed_directory': ('loki_cli.web_routers.files', 'create_managed_directory'),
    'create_profile_endpoint': ('loki_cli.web_routers.profiles', 'create_profile_endpoint'),
    'create_skill': ('loki_cli.web_routers.skills', 'create_skill'),
    'create_webhook': ('loki_cli.web_routers.ops', 'create_webhook'),
    'cron_fire_webhook': ('loki_cli.web_routers.cron', 'cron_fire_webhook'),
    'custom_endpoint_key_env': ('loki_cli.config', 'custom_endpoint_key_env'),
    'delete_agent_plugin': ('loki_cli.web_routers.dashboard_ui', 'delete_agent_plugin'),
    'delete_cron_job': ('loki_cli.web_routers.cron', 'delete_cron_job'),
    'delete_custom_endpoint': ('loki_cli.web_routers.config_env', 'delete_custom_endpoint'),
    'delete_empty_sessions_endpoint': ('loki_cli.web_routers.sessions', 'delete_empty_sessions_endpoint'),
    'delete_hook': ('loki_cli.web_routers.ops', 'delete_hook'),
    'delete_learning_node': ('loki_cli.web_routers.status', 'delete_learning_node'),
    'delete_managed_file': ('loki_cli.web_routers.files', 'delete_managed_file'),
    'delete_profile_endpoint': ('loki_cli.web_routers.profiles', 'delete_profile_endpoint'),
    'delete_session_endpoint': ('loki_cli.web_routers.sessions', 'delete_session_endpoint'),
    'delete_webhook': ('loki_cli.web_routers.ops', 'delete_webhook'),
    'derive_gateway_busy': ('gateway.status', 'derive_gateway_busy'),
    'derive_gateway_drainable': ('gateway.status', 'derive_gateway_drainable'),
    'describe_profile_auto_endpoint': ('loki_cli.web_routers.profiles', 'describe_profile_auto_endpoint'),
    'detect_install_method': ('loki_cli.config', 'detect_install_method'),
    'disconnect_oauth_provider': ('loki_cli.web_routers.oauth', 'disconnect_oauth_provider'),
    'download_dashboard_backup': ('loki_cli.web_routers.ops', 'download_dashboard_backup'),
    'download_managed_file': ('loki_cli.web_routers.files', 'download_managed_file'),
    'enable_webhooks': ('loki_cli.web_routers.ops', 'enable_webhooks'),
    'env_var_enabled': ('utils', 'env_var_enabled'),
    'events_ws': ('loki_cli.web_routers.chat_ws', 'events_ws'),
    'export_session_endpoint': ('loki_cli.web_routers.sessions', 'export_session_endpoint'),
    'find_provider_entry': ('loki_cli.config', 'find_provider_entry'),
    'format_docker_update_message': ('loki_cli.config', 'format_docker_update_message'),
    'fs_default_cwd': ('loki_cli.web_routers.files', 'fs_default_cwd'),
    'fs_download': ('loki_cli.web_routers.files', 'fs_download'),
    'fs_git_root': ('loki_cli.web_routers.files', 'fs_git_root'),
    'fs_list': ('loki_cli.web_routers.files', 'fs_list'),
    'fs_read_data_url': ('loki_cli.web_routers.files', 'fs_read_data_url'),
    'fs_read_text': ('loki_cli.web_routers.files', 'fs_read_text'),
    'fs_write_text': ('loki_cli.web_routers.files', 'fs_write_text'),
    'gateway_drain': ('loki_cli.web_routers.actions', 'gateway_drain'),
    'gateway_ws': ('loki_cli.web_routers.chat_ws', 'gateway_ws'),
    'get_action_status': ('loki_cli.web_routers.actions', 'get_action_status'),
    'get_active_profile_endpoint': ('loki_cli.web_routers.profiles', 'get_active_profile_endpoint'),
    'get_auxiliary_models': ('loki_cli.web_routers.models', 'get_auxiliary_models'),
    'get_client_voice_config': ('loki_cli.web_routers.audio', 'get_client_voice_config'),
    'get_computer_use_status': ('loki_cli.web_routers.tools', 'get_computer_use_status'),
    'get_config': ('loki_cli.web_routers.config_env', 'get_config'),
    'get_config_path': ('loki_cli.config', 'get_config_path'),
    'get_config_raw': ('loki_cli.web_routers.analytics', 'get_config_raw'),
    'get_cron_delivery_targets': ('loki_cli.web_routers.cron', 'get_cron_delivery_targets'),
    'get_cron_job': ('loki_cli.web_routers.cron', 'get_cron_job'),
    'get_curator_status': ('loki_cli.web_routers.status', 'get_curator_status'),
    'get_dashboard_font': ('loki_cli.web_routers.dashboard_ui', 'get_dashboard_font'),
    'get_dashboard_plugins': ('loki_cli.web_routers.dashboard_ui', 'get_dashboard_plugins'),
    'get_dashboard_themes': ('loki_cli.web_routers.dashboard_ui', 'get_dashboard_themes'),
    'get_defaults': ('loki_cli.web_routers.config_env', 'get_defaults'),
    'get_egress_status': ('loki_cli.web_routers.config_env', 'get_egress_status'),
    'get_elevenlabs_voices': ('loki_cli.web_routers.audio', 'get_elevenlabs_voices'),
    'get_env_path': ('loki_cli.config', 'get_env_path'),
    'get_env_vars': ('loki_cli.web_routers.config_env', 'get_env_vars'),
    'get_health': ('loki_cli.web_routers.status', 'get_health'),
    'get_loki_home': ('loki_cli.config', 'get_loki_home'),
    'get_learning_graph': ('loki_cli.web_routers.status', 'get_learning_graph'),
    'get_learning_node': ('loki_cli.web_routers.status', 'get_learning_node'),
    'get_logs': ('loki_cli.web_routers.status', 'get_logs'),
    'get_media': ('loki_cli.web_routers.files', 'get_media'),
    'get_memory_provider_config': ('loki_cli.web_routers.memory_providers', 'get_memory_provider_config'),
    'get_memory_status': ('loki_cli.web_routers.ops', 'get_memory_status'),
    'get_messaging_platforms': ('loki_cli.web_routers.messaging', 'get_messaging_platforms'),
    'get_moa_models': ('loki_cli.web_routers.models', 'get_moa_models'),
    'get_model_info': ('loki_cli.web_routers.models', 'get_model_info'),
    'get_model_options': ('loki_cli.web_routers.models', 'get_model_options'),
    'get_models_analytics': ('loki_cli.web_routers.analytics', 'get_models_analytics'),
    'get_plugins_hub': ('loki_cli.web_routers.dashboard_ui', 'get_plugins_hub'),
    'get_portal_status': ('loki_cli.web_routers.status', 'get_portal_status'),
    'get_process_loki_home': ('loki_cli.config', 'get_process_loki_home'),
    'get_profile_setup_command': ('loki_cli.web_routers.profiles', 'get_profile_setup_command'),
    'get_profile_soul': ('loki_cli.web_routers.profiles', 'get_profile_soul'),
    'get_profiles_sessions': ('loki_cli.web_routers.profiles', 'get_profiles_sessions'),
    'get_profiles_sessions_sidebar': ('loki_cli.web_routers.profiles', 'get_profiles_sessions_sidebar'),
    'get_provider_config_schema': ('plugins.memory.config_schema', 'get_provider_config_schema'),
    'get_recommended_default_model': ('loki_cli.web_routers.models', 'get_recommended_default_model'),
    ...
    'get_schema': ('loki_cli.web_routers.config_env', 'get_schema'),
    'get_session_detail': ('loki_cli.web_routers.sessions', 'get_session_detail'),
    'get_session_latest_descendant': ('loki_cli.web_routers.sessions', 'get_session_latest_descendant'),
    'get_session_messages': ('loki_cli.web_routers.sessions', 'get_session_messages'),
    'get_session_stats': ('loki_cli.web_routers.sessions', 'get_session_stats'),
    'get_sessions': ('loki_cli.web_routers.sessions', 'get_sessions'),
    'get_skill_content': ('loki_cli.web_routers.skills', 'get_skill_content'),
    'get_skills': ('loki_cli.web_routers.skills', 'get_skills'),
    'get_ssh_ownership': ('loki_cli.web_routers.status', 'get_ssh_ownership'),
    'get_status': ('loki_cli.web_routers.status', 'get_status'),
    'get_system_stats': ('loki_cli.web_routers.status', 'get_system_stats'),
    'get_telegram_onboarding_status': ('loki_cli.web_routers.messaging', 'get_telegram_onboarding_status'),
    'get_terminal_backends': ('loki_cli.web_routers.tools', 'get_terminal_backends'),
    'get_toolset_config': ('loki_cli.web_routers.tools', 'get_toolset_config'),
    'get_toolset_models': ('loki_cli.web_routers.tools', 'get_toolset_models'),
    'get_toolsets': ('loki_cli.web_routers.tools', 'get_toolsets'),
    'get_update_receipt': ('loki_cli.web_routers.actions', 'get_update_receipt'),
    'get_usage_analytics': ('loki_cli.web_routers.analytics', 'get_usage_analytics'),
    'get_whatsapp_onboarding_status': ('loki_cli.web_routers.messaging', 'get_whatsapp_onboarding_status'),
    'git_base_branches_route': ('loki_cli.web_routers.git', 'git_base_branches_route'),
    'git_branch_switch_route': ('loki_cli.web_routers.git', 'git_branch_switch_route'),
    'git_branches_route': ('loki_cli.web_routers.git', 'git_branches_route'),
    'git_commit_context_route': ('loki_cli.web_routers.git', 'git_commit_context_route'),
    'git_commit_route': ('loki_cli.web_routers.git', 'git_commit_route'),
    'git_create_pr_route': ('loki_cli.web_routers.git', 'git_create_pr_route'),
    'git_file_diff_route': ('loki_cli.web_routers.git', 'git_file_diff_route'),
    'git_push_route': ('loki_cli.web_routers.git', 'git_push_route'),
    'git_rev_parse_route': ('loki_cli.web_routers.git', 'git_rev_parse_route'),
    'git_revert_route': ('loki_cli.web_routers.git', 'git_revert_route'),
    'git_review_diff_route': ('loki_cli.web_routers.git', 'git_review_diff_route'),
    'git_review_list_route': ('loki_cli.web_routers.git', 'git_review_list_route'),
    'git_ship_info_route': ('loki_cli.web_routers.git', 'git_ship_info_route'),
    'git_stage_route': ('loki_cli.web_routers.git', 'git_stage_route'),
    'git_status_route': ('loki_cli.web_routers.git', 'git_status_route'),
    'git_unstage_route': ('loki_cli.web_routers.git', 'git_unstage_route'),
    'git_worktree_add_route': ('loki_cli.web_routers.git', 'git_worktree_add_route'),
    'git_worktree_remove_route': ('loki_cli.web_routers.git', 'git_worktree_remove_route'),
    'git_worktrees_route': ('loki_cli.web_routers.git', 'git_worktrees_route'),
    'grant_computer_use_permissions': ('loki_cli.web_routers.tools', 'grant_computer_use_permissions'),
    'import_sessions_endpoint': ('loki_cli.web_routers.sessions', 'import_sessions_endpoint'),
    'install_mcp_catalog_entry': ('loki_cli.web_routers.mcp', 'install_mcp_catalog_entry'),
    'install_skill_hub': ('loki_cli.web_routers.skills', 'install_skill_hub'),
    'instantiate_blueprint': ('loki_cli.web_routers.cron', 'instantiate_blueprint'),
    'is_nix_install_method': ('loki_cli.config', 'is_nix_install_method'),
    'list_checkpoints': ('loki_cli.web_routers.ops', 'list_checkpoints'),
    'list_credential_pool': ('loki_cli.web_routers.ops', 'list_credential_pool'),
    'list_cron_blueprints': ('loki_cli.web_routers.cron', 'list_cron_blueprints'),
    'list_cron_job_runs': ('loki_cli.web_routers.cron', 'list_cron_job_runs'),
    'list_cron_jobs': ('loki_cli.web_routers.cron', 'list_cron_jobs'),
    'list_custom_endpoints': ('loki_cli.web_routers.config_env', 'list_custom_endpoints'),
    'list_hooks': ('loki_cli.web_routers.ops', 'list_hooks'),
    'list_managed_files': ('loki_cli.web_routers.files', 'list_managed_files'),
    'list_mcp_catalog': ('loki_cli.web_routers.mcp', 'list_mcp_catalog'),
    'list_mcp_servers': ('loki_cli.web_routers.mcp', 'list_mcp_servers'),
    'list_oauth_providers': ('loki_cli.web_routers.oauth', 'list_oauth_providers'),
    'list_pairing': ('loki_cli.web_routers.ops', 'list_pairing'),
    'list_profiles_endpoint': ('loki_cli.web_routers.profiles', 'list_profiles_endpoint'),
    'list_skills_hub_sources': ('loki_cli.web_routers.skills', 'list_skills_hub_sources'),
    'list_webhooks': ('loki_cli.web_routers.ops', 'list_webhooks'),
    'load_env': ('loki_cli.config', 'load_env'),
    'mcp_oauth_callback': ('loki_cli.web_routers.mcp', 'mcp_oauth_callback'),
    'mcp_oauth_flow_status': ('loki_cli.web_routers.mcp', 'mcp_oauth_flow_status'),
    'normalize_updated_at': ('gateway.status', 'normalize_updated_at'),
    'open_profile_terminal_endpoint': ('loki_cli.web_routers.profiles', 'open_profile_terminal_endpoint'),
    'parse_active_agents': ('gateway.status', 'parse_active_agents'),
    'pause_cron_job': ('loki_cli.web_routers.cron', 'pause_cron_job'),
    'poll_oauth_session': ('loki_cli.web_routers.oauth', 'poll_oauth_session'),
    'post_agent_plugin_disable': ('loki_cli.web_routers.dashboard_ui', 'post_agent_plugin_disable'),
    'post_agent_plugin_enable': ('loki_cli.web_routers.dashboard_ui', 'post_agent_plugin_enable'),
    'post_agent_plugin_install': ('loki_cli.web_routers.dashboard_ui', 'post_agent_plugin_install'),
    'post_agent_plugin_update': ('loki_cli.web_routers.dashboard_ui', 'post_agent_plugin_update'),
    'post_plugin_visibility': ('loki_cli.web_routers.dashboard_ui', 'post_plugin_visibility'),
    'preview_skill_hub': ('loki_cli.web_routers.skills', 'preview_skill_hub'),
    'prune_checkpoints': ('loki_cli.web_routers.ops', 'prune_checkpoints'),
    'prune_sessions_endpoint': ('loki_cli.web_routers.sessions', 'prune_sessions_endpoint'),
    'pty_ws': ('loki_cli.web_routers.chat_ws', 'pty_ws'),
    'pub_ws': ('loki_cli.web_routers.chat_ws', 'pub_ws'),
    'put_plugin_providers': ('loki_cli.web_routers.dashboard_ui', 'put_plugin_providers'),
    'read_managed_file': ('loki_cli.web_routers.files', 'read_managed_file'),
    'read_raw_config': ('loki_cli.config', 'read_raw_config'),
    'read_runtime_status': ('gateway.status', 'read_runtime_status'),
    'recommended_update_command_for_method': ('loki_cli.config', 'recommended_update_command_for_method'),
    'redact_key': ('loki_cli.config', 'redact_key'),
    'remove_credential_pool_entry': ('loki_cli.web_routers.ops', 'remove_credential_pool_entry'),
    'remove_env_value': ('loki_cli.config', 'remove_env_value'),
    'remove_env_var': ('loki_cli.web_routers.config_env', 'remove_env_var'),
    'remove_mcp_server': ('loki_cli.web_routers.mcp', 'remove_mcp_server'),
    'rename_profile_endpoint': ('loki_cli.web_routers.profiles', 'rename_profile_endpoint'),
    'rename_session_endpoint': ('loki_cli.web_routers.sessions', 'rename_session_endpoint'),
    'replace_mcp_servers': ('loki_cli.web_routers.mcp', 'replace_mcp_servers'),
    'rescan_dashboard_plugins': ('loki_cli.web_routers.dashboard_ui', 'rescan_dashboard_plugins'),
    'reset_memory': ('loki_cli.web_routers.ops', 'reset_memory'),
    'resolve_cron_model_drift_defaults': ('loki_cli.config', 'resolve_cron_model_drift_defaults'),
    'resolve_gateway_liveness': ('gateway.status', 'resolve_gateway_liveness'),
    'restart_gateway': ('loki_cli.web_routers.actions', 'restart_gateway'),
    'resume_cron_job': ('loki_cli.web_routers.cron', 'resume_cron_job'),
    'reveal_env_var': ('loki_cli.web_routers.config_env', 'reveal_env_var'),
    'revoke_pairing': ('loki_cli.web_routers.ops', 'revoke_pairing'),
    'run_backup': ('loki_cli.web_routers.ops', 'run_backup'),
    'run_config_migrate': ('loki_cli.web_routers.status', 'run_config_migrate'),
    'run_curator': ('loki_cli.web_routers.status', 'run_curator'),
    'run_debug_share_endpoint': ('loki_cli.web_routers.status', 'run_debug_share_endpoint'),
    'run_doctor': ('loki_cli.doctor', 'run_doctor'),
    'run_dump': ('loki_cli.dump', 'run_dump'),
    'run_import': ('loki_cli.web_routers.ops', 'run_import'),
    'run_import_upload': ('loki_cli.web_routers.ops', 'run_import_upload'),
    'run_prompt_size': ('loki_cli.web_routers.status', 'run_prompt_size'),
    'run_security_audit': ('loki_cli.web_routers.ops', 'run_security_audit'),
    'run_toolset_post_setup': ('loki_cli.web_routers.tools', 'run_toolset_post_setup'),
    'save_config': ('loki_cli.config', 'save_config'),
    'save_env_value': ('loki_cli.config', 'save_env_value'),
    'save_toolset_env': ('loki_cli.web_routers.tools', 'save_toolset_env'),
    'scan_skill_hub': ('loki_cli.web_routers.skills', 'scan_skill_hub'),
    'search_sessions': ('loki_cli.web_routers.sessions', 'search_sessions'),
    'search_skills_hub': ('loki_cli.web_routers.skills', 'search_skills_hub'),
    'select_terminal_backend': ('loki_cli.web_routers.tools', 'select_terminal_backend'),
    'select_toolset_model': ('loki_cli.web_routers.tools', 'select_toolset_model'),
    'select_toolset_provider': ('loki_cli.web_routers.tools', 'select_toolset_provider'),
    'serve_plugin_asset': ('loki_cli.web_routers.dashboard_ui', 'serve_plugin_asset'),
    'set_active_profile_endpoint': ('loki_cli.web_routers.profiles', 'set_active_profile_endpoint'),
    'set_curator_paused': ('loki_cli.web_routers.status', 'set_curator_paused'),
    'set_dashboard_font': ('loki_cli.web_routers.dashboard_ui', 'set_dashboard_font'),
    'set_dashboard_theme': ('loki_cli.web_routers.dashboard_ui', 'set_dashboard_theme'),
    'set_env_var': ('loki_cli.web_routers.config_env', 'set_env_var'),
    'set_mcp_server_enabled': ('loki_cli.web_routers.mcp', 'set_mcp_server_enabled'),
    'set_memory_provider': ('loki_cli.web_routers.ops', 'set_memory_provider'),
    'set_moa_models': ('loki_cli.web_routers.models', 'set_moa_models'),
    'set_model_assignment': ('loki_cli.web_routers.models', 'set_model_assignment'),
    'set_webhook_enabled': ('loki_cli.web_routers.ops', 'set_webhook_enabled'),
    'setup_memory_provider': ('loki_cli.web_routers.memory_providers', 'setup_memory_provider'),
    'speak_stream_ws': ('loki_cli.web_routers.audio', 'speak_stream_ws'),
    'speak_text': ('loki_cli.web_routers.audio', 'speak_text'),
    'start_gateway': ('loki_cli.web_routers.ops', 'start_gateway'),
    'start_oauth_login': ('loki_cli.web_routers.oauth', 'start_oauth_login'),
    'start_telegram_onboarding': ('loki_cli.web_routers.messaging', 'start_telegram_onboarding'),
    'start_whatsapp_onboarding': ('loki_cli.web_routers.messaging', 'start_whatsapp_onboarding'),
    'stop_gateway': ('loki_cli.web_routers.ops', 'stop_gateway'),
    'stream_managed_file': ('loki_cli.web_routers.files', 'stream_managed_file'),
    'submit_oauth_code': ('loki_cli.web_routers.oauth', 'submit_oauth_code'),
    'test_mcp_server': ('loki_cli.web_routers.mcp', 'test_mcp_server'),
    'test_messaging_platform': ('loki_cli.web_routers.messaging', 'test_messaging_platform'),
    'toggle_skill': ('loki_cli.web_routers.skills', 'toggle_skill'),
    'toggle_toolset': ('loki_cli.web_routers.tools', 'toggle_toolset'),
    'transcribe_audio_upload': ('loki_cli.web_routers.audio', 'transcribe_audio_upload'),
    'trigger_cron_job': ('loki_cli.web_routers.cron', 'trigger_cron_job'),
    'tts_lease': ('loki_cli.web_routers.audio', 'tts_lease'),
    'uninstall_skill_hub': ('loki_cli.web_routers.skills', 'uninstall_skill_hub'),
    'update_config': ('loki_cli.web_routers.config_env', 'update_config'),
    'update_config_raw': ('loki_cli.web_routers.analytics', 'update_config_raw'),
    'update_cron_job': ('loki_cli.web_routers.cron', 'update_cron_job'),
    'update_loki': ('loki_cli.web_routers.actions', 'update_loki'),
    'update_learning_node': ('loki_cli.web_routers.status', 'update_learning_node'),
    'update_memory_provider_config': ('loki_cli.web_routers.memory_providers', 'update_memory_provider_config'),
    'update_messaging_platform': ('loki_cli.web_routers.messaging', 'update_messaging_platform'),
    'update_profile_description_endpoint': ('loki_cli.web_routers.profiles', 'update_profile_description_endpoint'),
    'update_profile_model_endpoint': ('loki_cli.web_routers.profiles', 'update_profile_model_endpoint'),
    'update_profile_soul': ('loki_cli.web_routers.profiles', 'update_profile_soul'),
    'update_skill_content': ('loki_cli.web_routers.skills', 'update_skill_content'),
    'update_skills_hub': ('loki_cli.web_routers.skills', 'update_skills_hub'),
    'upload_chat_image': ('loki_cli.web_routers.files', 'upload_chat_image'),
    'upload_managed_file': ('loki_cli.web_routers.files', 'upload_managed_file'),
    'upload_managed_file_stream': ('loki_cli.web_routers.files', 'upload_managed_file_stream'),
    'upsert_custom_endpoint': ('loki_cli.web_routers.config_env', 'upsert_custom_endpoint'),
    'validate_custom_endpoint': ('loki_cli.web_routers.config_env', 'validate_custom_endpoint'),
    'validate_provider_credential': ('loki_cli.web_routers.config_env', 'validate_provider_credential'),
    'windows_detach_flags': ('loki_cli._subprocess_compat', 'windows_detach_flags'),
    'windows_hide_flags': ('loki_cli._subprocess_compat', 'windows_hide_flags'),
    'write_platform_config_field': ('loki_cli.config', 'write_platform_config_field'),
}
...
def __getattr__(name):  # PEP 562 — lazy so no import cycles
    ...
    import importlib
    from loki_cli.plugin_compat import warn_once
