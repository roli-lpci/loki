"""Helpers for WunderCorp subscription managed-tool capabilities."""
...
from loki_cli.config import get_env_value, load_config
from loki_cli.wundercorp_account import (
    WunderCorpPortalAccountInfo, format_wundercorp_portal_entitlement_message, get_wundercorp_portal_account_info,
)
...
_DEFAULT_PLATFORM_TOOLSETS = {"cli": "loki-cli"}
...
class _FeatureSpec:
    ...
    # Tool-pool coverage category (wundercorp_account.TOOL_COVERAGE_CATEGORIES) gating per backend:
    ...
    offer_label: str = ""
    direct_label: str = ""
    # Direct-credential env vars that stop apply_wundercorp_managed_defaults switching the category to
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    """Stored provider for a section (``read_selection`` semantics): ``"wundercorp"`` for the managed
    selection (stored ``wundercorp`` or legacy ``use_gateway: true``), a vendor name for BYOK, else None."""
...
@dataclass(frozen=True)
class WunderCorpFeatureState:
    key: str
    label: str
    included_by_default: bool
    available: bool
    active: bool
    managed_by_wundercorp: bool
...
@dataclass(frozen=True)
class WunderCorpSubscriptionFeatures:
    subscribed: bool
    wundercorp_auth_present: bool
    provider_is_wundercorp: bool
    features: Dict[str, WunderCorpFeatureState]
    account_info: Optional[WunderCorpPortalAccountInfo] = None

    def __getattr__(self, name: str) -> WunderCorpFeatureState:  # ``features.web`` -> per-key state
        if name in _FEATURE_ORDER:
            return self.features[name]
        raise AttributeError(name)

    def items(self) -> Iterable[WunderCorpFeatureState]:
...
def _select_wundercorp(config: Dict[str, object], key: str) -> None:
    """Store the managed ``wundercorp`` selection in the ``key`` section (field per _GATEWAY_SECTION_FIELDS)."""
    section_key, field = _GATEWAY_SECTION_FIELDS[key]
    section = _ensure_section(config, section_key)
    section[field] = "wundercorp"
...
def _provider_is_wundercorp(config: Dict[str, object]) -> bool:
    return _norm(_section(config, "model").get("provider")) == "wundercorp"
...
def _has_agent_browser() -> bool:
    ...
    from loki_constants import agent_browser_runnable
    ...
    try:
        ...
        # Loki-managed Node dirs ($LOKI_HOME/node, prepended to PATH at runtime but usually absent
        ...
        search_paths = [None, with_loki_node_path().get("PATH", ""), str(local_bin_dir) if local_bin_dir.is_dir() else ""]
        return any(
...
def _local_stt_backend_available() -> bool:
    ...
    if get_env_value("LOKI_LOCAL_STT_COMMAND"):
        return True
    try:
...
def _account_info_or_none(**kwargs) -> Optional[WunderCorpPortalAccountInfo]:
    """``get_wundercorp_portal_account_info(**kwargs)``, failing closed to ``None`` on any error."""
    try:
        return get_wundercorp_portal_account_info(**kwargs)
    except Exception:
        return None


def _state(key: str, **fields) -> WunderCorpFeatureState:
    spec = _FEATURES[key]
    fields.setdefault("direct_override", fields["active"] and not fields["managed_by_wundercorp"])
    return WunderCorpFeatureState(key, spec.label, spec.included_by_default, **fields)


def _web_feature(web_cfg: Dict[str, object], tool_enabled: bool, managed: bool, web_gw: bool, direct_firecrawl: bool) -> WunderCorpFeatureState:
    ...
    # The "wundercorp" selection is serviced by Firecrawl — normalize so downstream vendor checks hold.
    if backend == "wundercorp" or web_gw:
        backend = "firecrawl"
    ...
    return _state(
        ...
        managed_by_wundercorp=web_managed, toolset_enabled=tool_enabled,
...
def _fal_feature(key: str, tool_enabled: bool, direct: bool, managed: bool, selected: Optional[str]) -> WunderCorpFeatureState:
    ...
    if selected not in (None, "wundercorp") or (selected is None and direct):
        label = "FAL"
    else:
        label = "WunderCorp Subscription" if (fal_managed or selected == "wundercorp") else ""
    return _state(
        key, available=bool(managed or direct), active=bool(tool_enabled and (fal_managed or direct)),
        managed_by_wundercorp=fal_managed, toolset_enabled=tool_enabled, current_provider=label,
...
def _audio_provider(cfg: Dict[str, object], default: str, gw: bool) -> str:
    ...
    return "openai" if (provider == "wundercorp" or gw) else (provider or default)
...
def _audio_features(
...
) -> tuple[WunderCorpFeatureState, WunderCorpFeatureState]:
    tts_gw, stt_gw = use_gateway["tts"], use_gateway["stt"]
    # STT default is "local" (faster-whisper, needs a pip install); WunderCorp subscribers are routed to the
    # managed audio gateway by apply_wundercorp_managed_defaults. The "wundercorp" selection is serviced by OpenAI
    ...
    tts = _state(
        ...
        managed_by_wundercorp=tts_tool_enabled and tts_current == "openai" and managed["tts"] and not direct_openai_tts,
    ...
    stt = _state(
        ...
        managed_by_wundercorp=stt_current == "openai" and managed["stt"] and not direct_openai_stt,
...
def _browser_feature(
    ...
    if provider == "wundercorp" or browser_gw:
    ...
    return _state(
        "browser", available=available, active=bool(tool_enabled and available), managed_by_wundercorp=managed_now,
        toolset_enabled=tool_enabled, current_provider=_provider_label("browser", current), explicit_configured=explicit,
    )


def _modal_feature(terminal_cfg: Dict[str, object], tool_enabled: bool, managed: bool, managed_tools_flag: bool) -> WunderCorpFeatureState:
    ...
    return _state(
        "modal", available=available, active=active, managed_by_wundercorp=managed_now, direct_override=direct_override,
...
def get_wundercorp_subscription_features(config: Optional[Dict[str, object]] = None, *, force_fresh: bool = False) -> WunderCorpSubscriptionFeatures:
    if config is None:
        config = load_config() or {}
    provider_is_wundercorp = _provider_is_wundercorp(config)
    ...
    wundercorp_auth_present = bool(account_info and account_info.logged_in)
    managed_tools_flag = wundercorp_auth_present and account_info.tool_gateway_entitled
    enabled = {key: _toolset_enabled(config, key) for key in ("web", "image_gen", "video_gen", "tts", "browser", "terminal")}
    # Stored selections (strict model): "wundercorp" = managed gateway; vendor name = that vendor direct;
    ...
    use_gateway = {key: value == "wundercorp" for key, value in selected.items()}
    ...
    def _fal(key: str) -> WunderCorpFeatureState:
    ...
    return WunderCorpSubscriptionFeatures(
        subscribed=provider_is_wundercorp or wundercorp_auth_present, wundercorp_auth_present=wundercorp_auth_present,
        provider_is_wundercorp=provider_is_wundercorp, features=features, account_info=account_info,
    )
...
def apply_wundercorp_managed_defaults(config: Dict[str, object], *, enabled_toolsets: Optional[Iterable[str]] = None, force_fresh: bool = False) -> set[str]:
    features = get_wundercorp_subscription_features(config, force_fresh=force_fresh)
    account_info = features.account_info
    if not (account_info and account_info.logged_in and account_info.tool_gateway_entitled and features.provider_is_wundercorp):
        return set()
...
def get_gateway_eligible_tools(config: Optional[Dict[str, object]] = None, *, force_fresh: bool = False) -> tuple[list[str], list[str], list[str], list[str]]:
    ...
    and no explicit non-wundercorp selection (safe to pre-check) / own API keys / explicit non-wundercorp selection
    ...
    if config is None:
        config = load_config() or {}
    if not _provider_is_wundercorp(config):
        return [], [], [], []
    ...
    for key in _ALL_GATEWAY_KEYS:
        ...
        elif selected is not None and selected != "wundercorp":
...
def prompt_enable_tool_gateway(config: Dict[str, object], *, force_fresh: bool = True) -> set[str]:
    ...
    if not unconfigured and not has_direct:
        return set()
    try:
        from loki_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    source_label = "free tool pool" if pool_only else "WunderCorp subscription"
    ...
    # every WunderCorp model swap. See #92647.
    ...
    title = (
        "Your free WunderCorp tool pool — pick the tools to enable:" if pool_only
        else "Your WunderCorp subscription includes the Tool Gateway — pick the tools to enable:"
    )
    try:
...
# Inline WunderCorp Portal login for the Tool Gateway picker (`loki tools`)


def ensure_wundercorp_portal_access(*, capability: str = "the WunderCorp Tool Gateway", coverage_category: Optional[str] = None) -> bool:
    """Make sure the user is entitled to the WunderCorp Tool Gateway, logging in if needed.
    ...
    if not _entitled(info) and (info is None or not info.logged_in):
        if not _run_wundercorp_portal_login_only(capability=capability):
            return False
        info = _account_info_or_none(force_fresh=True)
    if _entitled(info):
        return True
    # Logged in but not entitled for this capability — neutral billing guidance, do not enable.
    message = format_wundercorp_portal_entitlement_message(info, capability=capability, coverage_category=coverage_category)
...
def _run_wundercorp_portal_login_only(*, capability: str) -> bool:
    """Run the WunderCorp Portal device-code OAuth and persist credentials only (no model selection, no
    ...
    try:
        ...
        print(f"  Could not start WunderCorp Portal login: {exc}")
        return False
    print()
    print(f"  {capability} requires a WunderCorp Portal login.")
    proceed = _confirm("  Log in to WunderCorp Portal now? [Y/n]: ")
    if proceed is None:
        print()
        return False
    if not proceed:
        print("  Skipped WunderCorp Portal login.")
        return False
    try:
        # Snapshot active_provider so a tool-config login never silently switches inference to WunderCorp.
        ...
        if auth._read_shared_wundercorp_state() and _confirm("  Found existing WunderCorp OAuth credentials. Import them? [Y/n]: ") is not False:
            auth_state = auth._try_import_shared_wundercorp_state(timeout_seconds=15.0)
        if auth_state is None:
            auth_state = auth._wundercorp_device_code_login()
        with auth._auth_store_lock():
            auth_store = auth._load_auth_store()
            auth._save_provider_state(auth_store, "wundercorp", auth_state)
            if prior_active_provider:
                auth_store["active_provider"] = prior_active_provider
            else:
                auth_store.pop("active_provider", None)
            auth._save_auth_store(auth_store)
        auth._write_shared_wundercorp_state(auth_state)
        auth._sync_wundercorp_pool_from_auth_store()
        print("  WunderCorp Portal login successful.")
        return True
    except KeyboardInterrupt:
        print("\n  Login cancelled.")
        return False
    except SystemExit:
        # _wundercorp_device_code_login raises SystemExit on subscription_required (guidance already printed).
        return False
    except Exception as exc:
        print(f"  WunderCorp Portal login failed: {exc}")
        return False
...
_PLUGIN_COMPAT_LAZY = {
    'managed_wundercorp_tools_enabled': ('tools.tool_backend_helpers', 'managed_wundercorp_tools_enabled'),
}
