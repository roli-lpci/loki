"""Provider rows, active-provider detection, model pickers and provider-selection persistence for `loki tools`."""
...
from loki_cli.cli_output import (
...
from loki_cli.config import cfg_get, get_env_value, load_config, save_config, save_env_value
from loki_cli.wundercorp_account import format_wundercorp_portal_entitlement_message
from loki_cli.wundercorp_subscription import MANAGED_FEATURE_COVERAGE_CATEGORY, WunderCorpSubscriptionFeatures
from tools.tool_backend_helpers import WUNDERCORP_MANAGED_PROVIDER, fal_key_is_configured
from utils import base_url_hostname, is_truthy_value

logger = logging.getLogger("loki_cli.tools_config")
...
def _plugin_registry(module: str):
    ...
    try:
        ...
        from loki_cli.plugins import _ensure_plugins_discovered
...
def _visible_providers(
    cat: dict, config: dict, *, force_fresh: bool = False, features: Optional[WunderCorpSubscriptionFeatures] = None,
) -> list[dict]:
    """Provider entries visible for the current auth/config state.
    WunderCorp-managed rows (``managed_wundercorp_feature``) are always shown, even logged-out/unentitled, to
    advertise the capability."""
    from loki_cli.tools_config import get_wundercorp_subscription_features
    ...
    for provider in cat.get("providers", []):
        managed = provider.get("managed_wundercorp_feature")
        # Managed rows stay visible regardless of auth (selecting one drives an inline Portal login); a
        # `requires_wundercorp_auth` row without a managed feature hides until logged in.
        if provider.get("requires_wundercorp_auth") and not managed and not features.wundercorp_auth_present:
            continue
...
def provider_readiness_status(provider: dict, config: dict, *, features=None, is_active: Optional[bool] = None) -> str:
    ...
    from loki_cli.tools_config import (
        _POST_SETUP_READY, _provider_env_ready, _xai_credentials_present, get_wundercorp_subscription_features,
    )

    if provider.get("env_vars", []):
        return "ready" if _provider_env_ready(provider) else "needs_keys"

    managed_feature = provider.get("managed_wundercorp_feature")
    if provider.get("requires_wundercorp_auth") or managed_feature:
        if features is None:
...
def _toolset_needs_configuration_prompt(ts_key: str, config: dict, *, force_fresh: bool = False) -> bool:
    ...
    from loki_cli.tools_config import TOOL_CATEGORIES, _post_setup_already_installed, _toolset_has_keys
...
def _configure_tool_category(ts_key: str, cat: dict, config: dict, *, force_fresh: bool = True, reconfigure: bool = False):
    ...
    ``reconfigure`` ("Reconfigure an existing tool"): no setup note / skip row / WunderCorp marker, and the
    chosen provider goes through the key-update prompts instead of the new-enable prompts."""
    from loki_cli.tools_config import _prompt_choice, _provider_env_ready, get_wundercorp_subscription_features
    ...
    # Logged-in WunderCorp users get a marker on rows included in their subscription (cost-extra vs. included).
    _wundercorp_logged_in = False
    if not reconfigure:
        try:
            _wundercorp_logged_in = bool(get_wundercorp_subscription_features(config, force_fresh=force_fresh).wundercorp_auth_present)
        except Exception:
            pass
    ...
    for p in providers:
        ...
        sub_marker = ""
        if not reconfigure and p.get("managed_wundercorp_feature"):
            sub_marker = "  ★ Included with your WunderCorp subscription" if _wundercorp_logged_in else "  ★ via WunderCorp Portal (login on select)"
...
def _managed_provider_active(provider: dict, config: dict, managed_feature: str, force_fresh: bool) -> bool:
    """Active check for a WunderCorp-managed row: the feature must be managed AND the category's selected
    provider must be the row's vendor or ``wundercorp``."""
    from loki_cli.tools_config import get_wundercorp_subscription_features

    feature = get_wundercorp_subscription_features(config, force_fresh=force_fresh).features.get(managed_feature)
    if feature is None:
        return False
    if managed_feature in ("image_gen", "video_gen"):
        ...
        if isinstance(gen_cfg, dict):
            ...
            if configured_provider not in {None, "", "fal", WUNDERCORP_MANAGED_PROVIDER}:
                return False
            if (
                configured_provider != WUNDERCORP_MANAGED_PROVIDER
                and gen_cfg.get("use_gateway") is not None
                and not is_truthy_value(gen_cfg.get("use_gateway"), default=False)):
                return False
        return feature.managed_by_wundercorp
    ...
    for marker, section, key in _MANAGED_SELECTION_KEYS:
        if _has_marker(provider, marker):
            ...
            selected = current in {provider[marker], WUNDERCORP_MANAGED_PROVIDER}
            if marker == "web_backend":
                selected = selected and _web_tier_matches(provider, config)
            return feature.managed_by_wundercorp and selected
    return feature.managed_by_wundercorp
...
def _detect_active_provider_index(providers: list, config: dict, *, force_fresh: bool = False) -> int:
    ...
    from loki_cli.tools_config import _provider_env_ready
...
def _pick_model_from_catalog(
    ...
    selection."""
    from loki_cli.tools_config import _cfg_section, _prompt_choice
...
def _select_into(config: dict, section: str, key: str, vendor, managed) -> dict:
    """Write ``config[section][key] = vendor`` (``wundercorp`` for a managed pick) and drop any legacy ``use_gateway``
    key so the old read-time shim cannot override the new choice. Returns the section dict."""
    from loki_cli.tools_config import _cfg_section

    cfg = _cfg_section(config, section)
    cfg[key] = WUNDERCORP_MANAGED_PROVIDER if managed else vendor
...
def _select_plugin_gen_provider(section: str, plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    """Persist a plugin-backed image/video gen provider selection (``wundercorp`` for a WunderCorp-managed pick, else the
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    Pure, non-interactive core of :func:`_configure_provider` (no env prompts, post-setup hooks, WunderCorp
    auth gating or model pickers) shared by the CLI and the GUI ``PUT .../provider`` endpoint. Each pick
    writes exactly ONE provider string per category (``wundercorp`` for managed rows) and removes any legacy
    ``use_gateway`` key so the read-time shim cannot override the new choice."""
    from loki_cli.tools_config import TOOL_CATEGORIES
    ...
    if managed_feature and managed_feature not in {"web", "tts", "stt", "browser"}:
        # Managed rows without a marker above (image_gen/video_gen "WunderCorp Subscription" rows carry only
        # managed_wundercorp_feature) still persist the "wundercorp" selection.
        section = config.setdefault(managed_feature, {})
        if isinstance(section, dict):
            section["provider"] = WUNDERCORP_MANAGED_PROVIDER
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    # GUI flow); managed picks store "wundercorp". The in-tree FAL BYOK row always persists an explicit
...
def _wundercorp_provider_gate(provider: dict, config: dict, managed_feature, *, force_fresh: bool) -> bool:
    """Return False (after printing why) when a WunderCorp-gated row cannot be selected.
    Managed Tool Gateway rows are always listed but only *activate* with paid WunderCorp Portal access —
    selecting one runs an inline Portal login (auth + entitlement only, no inference-provider switch).
    Pure pre-auth UX rows (``requires_wundercorp_auth`` without a managed feature) keep the older logged-in +
    entitled gate."""
    from loki_cli.tools_config import get_wundercorp_subscription_features

    if managed_feature:
        from loki_cli.wundercorp_subscription import ensure_wundercorp_portal_access

        if not ensure_wundercorp_portal_access(
            capability=f"{provider.get('name', 'the WunderCorp Tool Gateway')}",
            coverage_category=MANAGED_FEATURE_COVERAGE_CATEGORY.get(managed_feature)):
            _print_warning("  Not enabled — WunderCorp Portal access is required for this backend.")
            return False
        return True

    if provider.get("requires_wundercorp_auth"):
        features = get_wundercorp_subscription_features(config, force_fresh=force_fresh)
        entitled = bool(features.account_info and features.account_info.paid_service_access is True)
        if not features.wundercorp_auth_present or not entitled:
            message = format_wundercorp_portal_entitlement_message(
                features.account_info, capability=f"{provider.get('name', 'WunderCorp Subscription')}")
            _print_warning(f"  {message or 'WunderCorp Subscription is only available after logging into WunderCorp Portal.'}")
            return False
    return True
...
def _finish_provider_selection(provider: dict, config: dict, managed_feature) -> None:
    ...
    if backend:
        ...
        # In-tree FAL is the only non-plugin backend: "wundercorp" for a managed row, "fal" for BYOK, drop legacy
...
def _print_provider_selection(provider: dict, managed_feature, *, reconfigure: bool) -> None:
    ...
    if "browser_provider" in provider:
        ...
        if reconfigure and managed_feature:
            _print_success(f"  Browser cloud provider set to: {bp or 'wundercorp'}")
        elif bp == "local":
...
def _show_portal_hint(provider: dict, config: dict, managed_feature, force_fresh: bool) -> bool:
    """True when a BYOK row shares its category with a WunderCorp-managed sibling and the user is not authed to
    WunderCorp — a single dim hint tells them the key is avoidable via a Portal subscription."""
    from loki_cli.tools_config import TOOL_CATEGORIES, get_wundercorp_subscription_features

    if managed_feature or provider.get("requires_wundercorp_auth"):
        return False
    try:
        for _cat in TOOL_CATEGORIES.values():
            _providers = _cat.get("providers", [])
            if provider in _providers and any(sib.get("managed_wundercorp_feature") for sib in _providers):
                return not get_wundercorp_subscription_features(config, force_fresh=force_fresh).wundercorp_auth_present
    except Exception:
        pass
    return False
...
def _configure_provider(provider: dict, config: dict, *, force_fresh: bool = True, reconfigure: bool = False):
    ...
    hook."""
    from loki_cli.tools_config import _run_post_setup

    env_vars = provider.get("env_vars", [])
    managed_feature = provider.get("managed_wundercorp_feature")

    if not _wundercorp_provider_gate(provider, config, managed_feature, force_fresh=force_fresh):
        return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your WunderCorp subscription.")
    ...
    if not reconfigure and _show_portal_hint(provider, config, managed_feature, force_fresh):
        _print_info("  Available through WunderCorp Portal subscription.")
...
def _configure_vision_backend() -> None:
    ...
    Offers any authenticated provider + model (same surface as ``loki model``) or a custom endpoint
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    Rows come from ``build_aux_picker_rows()`` so this lists exactly what the ``loki model`` aux-task
    picker lists, including user-defined ``providers:`` / ``custom_providers:`` endpoints. Persists
    ``auxiliary.vision.provider`` + ``.model``."""
    from loki_cli.tools_config import _prompt_choice

    try:
        from loki_cli.inventory import build_aux_picker_rows, format_aux_picker_entries
    ...
    if not providers:
        _print_warning("  No authenticated providers found. Configure a provider first "
                       "with `loki model`, then re-run this.")
        return
...
def _configure_simple_requirements(ts_key: str, *, reconfigure: bool = False):
    ...
    from loki_cli.tools_config import TOOLSET_ENV_REQUIREMENTS, _toolset_has_keys, _toolset_label
