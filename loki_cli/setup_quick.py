...
logger = logging.getLogger("loki_cli.setup")
...
def _blank_slate_done(config: dict, loki_home, tools_line: str, *extra: str, intro: str | None = None) -> None:
    """Shared Blank Slate epilogue: success banner, the "enable later" hints, then the summary."""
    from loki_cli.setup import _info, _print_setup_summary, print_success
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    _info(*([intro] if intro else []), tools_line, "  Seed skills:         loki skills opt-in --sync",
          "  Add MCP servers:     loki mcp add", *extra, "  Tune agent settings: loki setup agent", None)
...
def _reload_config_into(config: dict, *, dict_only: bool = False) -> None:
    ...
    from loki_cli.setup import load_config
...
def _run_wundercorp_flow(config: dict, *, context: str, cancel_exc: tuple, cancel_lines: tuple, print_error) -> bool:
    """Run ``_model_flow_wundercorp`` (login, model pick, provider switch, Tool Gateway opt-in) — the
    single source of truth shared with ``loki model``. False when cancelled or failed (the
    message is already printed)."""
    from loki_cli.setup import _info
    try:
        from loki_cli.model_setup_flows import _model_flow_wundercorp
        _model_flow_wundercorp(config)
        return True
    except cancel_exc:
        # _login_wundercorp raises SystemExit(130)/(1) on cancel/failure; the expired-session re-login
        # path inside _model_flow_wundercorp only catches Exception, so SystemExit would kill the CLI.
        _info(*cancel_lines)
    except Exception as exc:
        logger.debug("_model_flow_wundercorp error during %s: %s", context, exc)
        print_error(exc)
    return False
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    def _on_error(exc: Exception) -> None:
        print()
        print_error(f"  WunderCorp Portal setup encountered an error: {exc}")
        print_info("  You can retry later with `loki portal`.")

    if not _run_wundercorp_flow(config, context="`loki portal`", cancel_exc=(KeyboardInterrupt, EOFError, SystemExit),
                          cancel_lines=(None, "  Setup cancelled.", "  You can retry later with `loki portal`."),
...
def _run_first_time_quick_setup(config: dict, loki_home, is_existing: bool):
    ...
    _info(None, "  Configure all settings:    loki setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  loki setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    _info(None, "  macOS tip: silence ALL folder permission prompts with one switch —",
          ...
          "  your terminal (and Loki.app if you use Desktop), or run:",
          "    open \"x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles\"",
          "  The grant is permanent — it survives every Loki update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``vision`` (``read_file`` can't read images) and ``skills`` (the seeded ``loki-agent`` skill
    ...
    try:
        ...
        from loki_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
        all_keys = {k for k, _, _ in CONFIGURABLE_TOOLSETS}
        all_keys.update(_get_plugin_toolset_keys())
        # Plain TOOLSETS entries catch recovered toolsets like ``kanban``. Skip "loki-*" platform
        ...
        for k, tdef in TOOLSETS.items():
            if k.startswith("loki-") or (isinstance(tdef, dict) and (tdef.get("includes") or tdef.get("posture"))):
                continue
...
def _blank_slate_minimize_config(config: dict):
    """Turn OFF every optional config feature; all opt back in via ``loki setup agent``."""
...
def _run_blank_slate_setup(config: dict, loki_home, is_existing: bool):
    ...
    _info("Everything starts OFF. First we force-enable only what's required",
          ...
          "essential `loki-agent` skill is always kept so the agent",
          "can help you drive and configure Loki itself.", None)
    ...
    if path != 0:
        _blank_slate_walkthrough(config, loki_home)
        return
    save_config(config)
    # Blank Slate means no bundled skills; record the opt-out so future `loki update` runs
    # don't re-inject them.
    _set_bundled_skills_opt_out(True, "skill opt-out")
    _blank_slate_done(config, loki_home, "  Enable tools:        loki tools", "  Enable plugins:      loki plugins",
                      intro="Enable anything later, on demand:")


def _blank_slate_walkthrough(config: dict, loki_home):
    ...
    def _opted_out(_result) -> None:
        _info("No skills seeded (except the essential `loki-agent`",
              "skill). A .no-bundled-skills marker keeps future",
              "`loki update` runs from re-injecting them. Opt back in any",
              "time with `loki skills opt-in --sync`.")
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `loki tools`.")
    ...
    for header, question, yes_msg, no_msg in (
        ("Plugins", "Review and enable built-in plugins now?",
         "Manage plugins with `loki plugins list` / `loki plugins install`.",
         "No plugins enabled. Add later with `loki plugins`."),
        ("MCP Servers", "Add an MCP server now?",
         "Add servers with `loki mcp add <name> --url ... | --command ...`.",
         "No MCP servers configured. Add later with `loki mcp add`."),
    ):
    ...
    if prompt_yes_no("Connect a messaging platform (Telegram, Discord, …)?", default=False):
        setup_gateway(config)
    save_config(config)
    _blank_slate_done(config, loki_home, "  Enable more tools:   loki tools")


def _run_quick_setup(config: dict, loki_home):
    ...
    from loki_cli.config import (get_missing_env_vars, get_missing_config_fields, check_config_version)
    ...
    if not (missing_required or missing_optional or missing_config or current_ver < latest_ver):
        ...
        _info(None, "Run 'loki setup' and choose 'Full Setup' to reconfigure,",
    ...
    if missing_messaging:  # checklist, then prompt for each selected platform's vars
        ...
        _info("Connect Loki to messaging apps to chat from anywhere.",
              "You can configure these later with 'loki setup gateway'.")
