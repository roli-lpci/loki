...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_LOKI_GATEWAY_BREAKAWAY"
...
def windows_hide_flags() -> int:
    ...
    For short-lived synchrowundercorp helpers (``taskkill``, ``where``, version probes): no flash, but the
...
def noninteractive_git_env(base: "Mapping[str, str] | None" = None) -> dict[str, str]:
    ...
    Loki's plumbing calls. ``core.sshCommand`` is pinned to ``ssh -o BatchMode=yes`` so the ssh
    ...
    Loki shells out to git from many non-interactive contexts — MCP catalog installs, plugin
...
def _text_names_loki(text: str) -> bool:
    r"""True when *text* names Loki at a path-segment / token boundary.

    A bare ``"loki" in text`` substring test would also match unrelated processes whose paths
    merely contain the letters (``...\slokia\...``) — the false-positive class this prevents.
    """
    return any(token.startswith(("loki", ".loki"))
               for token in re.split(r"[\\/\s=,;\"']+", text.lower()))


def _process_command_is_loki(pid: int) -> bool:
    """Best-effort check that *pid* currently runs Loki code."""
    try:
        import psutil
        ...
        return _text_names_loki(f"{command} {executable}")
    except Exception:
        return False


def pid_is_loki(pid: int, *, expected_start_time: int | None = None) -> bool:
    """Whether it is safe to use ``taskkill`` for *pid*.

    The PID must be valid, currently exist, and identify a Loki process. When the caller captured
    ...
    if expected_start_time is not None and current_start_time != expected_start_time:
        return False
    try:
        return _process_command_is_loki(pid)
    except Exception:
        return False
...
def _legacy_kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    if IS_WINDOWS:
        ...
        # be recycled while we hold it. The fail-closed ``pid_is_loki`` guard is for BARE pids.
        try:
...
def bounded_probe_run(
    ...
    ``Get-CimInstance Win32_Process`` gateway scan hit exactly this during ``loki update`` on slow-WMI
