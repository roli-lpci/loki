"""``loki logs`` — view and filter Loki log files.

``loki logs [name] [-n N] [-f] [--level L] [--session S] [--component C] [--since 1h]``;
``loki logs list`` shows the available files.
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_loki_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Loki runs — try 'loki chat' first)")
        sys.exit(1)
    ...
    print(f"--- {display_loki_home()}/logs/{filename}{filter_desc} ({mode}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_loki_home()}/logs/")
        return

    print(f"Log files in {display_loki_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'loki chat' to generate logs)")
