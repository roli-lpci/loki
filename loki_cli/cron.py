...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: loki gateway install\n"
                "                    sudo loki gateway install --system  # Linux servers\n"
                "     Check status:  loki cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        print(color("No scheduled jobs.\nCreate one with 'loki cron create ...' "
...
def cron_tick():
    ...
    try:
        ...
        print("  Check `loki cron status` and the gateway log for details.")
        return 1
    return 0
...
def cron_incidents(args) -> int:
    ...
    if action == "ack":
        ...
        if not incident_id:
            print(color("✗ Incident ID required: loki cron incidents ack <incident_id>", Colors.RED))
            return 1
    ...
    for inc in incidents:
        ...
        print()
    print(color(f"  {len(incidents)} incident(s)  |  ack one with: loki cron incidents ack <id>",
                Colors.DIM))
    return 0


_PERMISSION_HINT = ("  Hint: jobs.json may be owned by another user (e.g. rewritten by a root "
                    "`docker exec loki loki cron ...`). Fix ownership to match the gateway "
...
def _print_ticker_health(pids: list) -> None:
    ...
    if hb_age is None:
        ...
        print("  Cron jobs will NOT fire until the ticker writes its first heartbeat.\n"
              "  If the gateway just started, wait ~60s and re-run `loki cron status`.\n"
              "  If heartbeat never appears, restart: loki gateway restart")
        ...
        print("  Cron jobs may NOT be firing. Restart: loki gateway restart")
...
def _script_health_issue(script: str) -> Optional[str]:
    ...
    try:
        ...
        return f"script resolves outside LOKI_HOME/scripts: {script!r}"
    if not path.exists():
...
def cron_doctor() -> int:
    ...
    for job, issues in findings:
        ...
        for issue in issues:
            print(f"    - {issue}")
    print()
    print(color("Next: fix the listed job config, then run `loki cron doctor` again.", Colors.DIM))
    return 1
...
def _job_action(action: str, job_id: str, success_verb: str) -> int:
    ...
    if action == "run":
        ...
        # inherits LOKI_SESSION_KEY) would be orphaned mid-LLM-call, leaving the execution row
        # stuck 'claimed'. Declaring the channel stateless forces a synchrowundercorp run; scoped to
        ...
        with contextlib.suppress(Exception):
            ...
            # gateway/desktop session env (LOKI_SESSION_KEY); declare the channel stateless so
            # ``async_delivery_supported()`` gates it off and the run executes synchrowundercorply to completion
            # instead. See #86721.
...
def cron_resume(args) -> int:
    ...
    from cron.jobs import AmbiguousJobReference, _loki_now, rearm_oneshot
    try:
        job = rearm_oneshot(args.job_id, _loki_now().isoformat() if run_now else run_at)
...
def cron_notepad(args) -> int:
    """Handle ``loki cron notepad <job_id> [get|set|delete|list]`` (per-job durable KV).
    ...
    try:
        ...
        if key is None or (action == "set" and value is None):
            print(color(f"Usage: loki cron notepad <job_id> {usage_args}", Colors.RED))
            return 1
        if action == "set":
...
def cron_command(args):
    ...
    print(f"Unknown cron command: {subcmd}\n"
          "Usage: loki cron [list|create|edit|pause|resume|run|remove|status|runs|doctor|tick]")
    sys.exit(1)
