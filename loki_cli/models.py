...
``loki_cli.models.<name>`` stays the stable import/monkeypatch surface:
...
from loki_cli.urllib_security import open_credentialed_url
from loki_cli.models_catalog_static import (
    CANONICAL_PROVIDERS,
    OPENROUTER_MODELS,
...
from loki_cli.models_reasoning_caps import (
    _OPENROUTER_CATALOG_URL,
    _seed_reasoning_caps)
from loki_cli.models_local import (
...
_LOKI_USER_AGENT = f"loki-cli/{_LOKI_VERSION}"
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from loki_cli.config import get_custom_provider_tls_settings
...
def _openrouter_catalog_disk_ttl() -> float:
    ...
    from loki_cli.model_catalog import refresh_interval_seconds
...
def _openrouter_catalog_disk_path() -> Path:
    ...
    return get_loki_home() / "cache" / "openrouter_curated_catalog.json"
...
# ---------------------------------------------------------------------------
# WunderCorp Portal free-model helpers — the Portal models endpoint is the source of truth for what is
...
def partition_wundercorp_models_by_tier(
    model_ids: list[str], pricing: dict[str, dict[str, str]], free_tier: bool
) -> tuple[list[str], list[str]]:
    """Split WunderCorp models into (selectable, unavailable): free-tier users may only select free models
...
def _union_with_portal_recommendations(
    ...
    *, force_refresh: bool, synthesize_free_pricing: bool,
    ...
    """
    try:
        payload = fetch_wundercorp_recommended_models(portal_base_url, force_refresh=force_refresh)
    except Exception:
        payload = None
...
def _pricing_profile_key() -> str:
    ...
    from loki_constants import loki_home_key

    return loki_home_key()


def get_cached_wundercorp_free_tier() -> Optional[bool]:
...
def check_wundercorp_free_tier(*, force_fresh: bool = False, cached_only: bool = False) -> bool:
    """True only when the WunderCorp Portal user is KNOWN to be free-tier (unknown/error → False so this
    ...
    if not force_fresh:
        cached_result = get_cached_wundercorp_free_tier()
    ...
    try:
        ...
        result = get_wundercorp_portal_account_info(force_fresh=force_fresh).is_free_tier
    except Exception:
...
# ---------------------------------------------------------------------------
# WunderCorp Portal recommended models — curated paid/free suggestions plus dedicated compaction (aux)
...
WUNDERCORP_RECOMMENDED_MODELS_PATH = "/api/wundercorp/recommended-models"
_WUNDERCORP_RECOMMENDED_CACHE_TTL: int = 600  # seconds (10 minutes)
# (result_dict, timestamp) keyed by portal_base_url so staging vs prod don't collide.
_wundercorp_recommended_cache: dict[str, tuple[dict[str, Any], float]] = {}


def _wundercorp_recommended_disk_path() -> "Path":
    from loki_constants import get_loki_home
    return get_loki_home() / "cache" / "wundercorp_recommended_cache.json"


def _read_wundercorp_recommended_disk(base: str) -> dict[str, Any] | None:
    """Last-known-good payload for ``base`` from the per-base disk map
    ``{"<base>": {"data": {...}, "ts": <epoch>}}`` (staging and prod don't collide), or None."""
    blob = _read_json_cache(_wundercorp_recommended_disk_path(), errors=(OSError, json.JSONDecodeError))
    entry = (blob or {}).get(base)
    data = entry.get("data") if isinstance(entry, dict) else None
    return data if isinstance(data, dict) and data else None


def _write_wundercorp_recommended_disk(base: str, data: dict[str, Any]) -> None:
    """Merge ``data`` into the per-base disk map atomically; failures are debug-logged (the in-process
    cache still works)."""
    if not data:
        return
    path = _wundercorp_recommended_disk_path()
    try:
        ...
        logger.debug("wundercorp recommended-models disk cache write failed: %s", exc)


def fetch_wundercorp_recommended_models(
    portal_base_url: str = "", timeout: float = 5.0, *, force_refresh: bool = False
) -> dict[str, Any]:
    """Fetch the Portal's public ``/api/wundercorp/recommended-models`` payload (no auth).

    Cached per portal URL for ``_WUNDERCORP_RECOMMENDED_CACHE_TTL`` seconds in process (``force_refresh``
    ...
    """
    base = (portal_base_url or "https://portal.wundercorp.com").rstrip("/")
    now = time.monotonic()
    cached = _wundercorp_recommended_cache.get(base)
    if not force_refresh and cached is not None and now - cached[1] < _WUNDERCORP_RECOMMENDED_CACHE_TTL:
        return cached[0]
    try:
        data = _get_json(
            f"{base}{WUNDERCORP_RECOMMENDED_MODELS_PATH}", timeout=timeout, headers={"Accept": "application/json"}
        )
        ...
        data = {}
    if data:
        _write_wundercorp_recommended_disk(base, data)
    else:
        data = _read_wundercorp_recommended_disk(base) or data
    _wundercorp_recommended_cache[base] = (data, now)
    return data


def _resolve_wundercorp_portal_url() -> str:
    """Best-effort lookup of the Portal base URL the user is authed against."""
    try:
        from loki_cli.auth import DEFAULT_WUNDERCORP_PORTAL_URL, get_provider_auth_state

        state = get_provider_auth_state("wundercorp") or {}
        portal = str(state.get("portal_base_url") or "").strip()
        return (portal or str(DEFAULT_WUNDERCORP_PORTAL_URL)).rstrip("/")
    except Exception:
        return "https://portal.wundercorp.com"
...
def get_wundercorp_recommended_aux_model(
    *, vision: bool = False, free_tier: Optional[bool] = None, portal_base_url: str = "",
    ...
    base = portal_base_url or _resolve_wundercorp_portal_url()
    payload = fetch_wundercorp_recommended_models(base, force_refresh=force_refresh)
    if not payload:
        return None
    if free_tier is None:
        try:
            free_tier = check_wundercorp_free_tier()
        except Exception:
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    try:
        from loki_cli.model_catalog import get_default_model_from_cache
...
def recommended_wundercorp_default_model() -> dict[str, Any]:
    """The model a WunderCorp account lands on without choosing one, honouring the account's tier.
    ...
    ``{"provider": "wundercorp", "model": str, "free_tier": bool}``; ``model`` may be ``""`` when nothing
    is selectable (callers degrade). Shared by ``GET /api/model/recommended-default`` and the
    sign-in completion in ``loki_cli.anon_auth`` so both land on the same model.
    """
    from loki_cli import models_pricing as mp
    from loki_cli.auth import get_provider_auth_state

    model_ids = get_curated_wundercorp_model_ids()
    pricing = mp.get_pricing_for_provider("wundercorp") or {}
    free_tier = check_wundercorp_free_tier(force_fresh=True)
    try:
        portal_url = (get_provider_auth_state("wundercorp") or {}).get("portal_base_url", "") or ""
    except Exception:
        portal_url = ""
    # Narrow to policy BEFORE the tier split, so a rescued id still has to pass the free/paid predicate.
    policy_allowed = mp.wundercorp_policy_allowed_ids()
    ...
    model_ids = mp.restrict_to_wundercorp_policy(model_ids, policy_allowed, rescue_empty=True)
    if free_tier:
        model_ids, _unavailable = partition_wundercorp_models_by_tier(model_ids, pricing, free_tier=True)
    return {"provider": "wundercorp", "model": pick_silent_default_model(model_ids, provider="wundercorp"),
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    """True when ``supported_parameters`` advertises ``tools`` (loki-agent is tool-calling-first).
    Permissive when the field is absent/malformed: some OpenRouter-compatible gateways (WunderCorp Portal,
...
# Reasoning-capability cache slots, one set per catalog (OpenRouter, WunderCorp Portal). The logic
...
_wundercorp_reasoning_caps_cache: dict[str, Optional[dict[str, Any]]] | None = None
_wundercorp_reasoning_caps_failed_at: float | None = None
_wundercorp_caps_disk_checked = False
_wundercorp_caps_warm_started = False
...
def fetch_openrouter_models(
    ...
    try:
        from loki_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id, _ in fallback:
        ...
        if live_item is None or not _openrouter_model_supports_tools(live_item):
            continue
        # Hide models that don't advertise tool-calling support — loki-agent requires it and surfacing
...
def get_curated_wundercorp_model_ids() -> list[str]:
    """Curated WunderCorp Portal model ids: the remote catalog manifest, else the in-repo
    ``_PROVIDER_MODELS["wundercorp"]`` snapshot. Always a list."""
    try:
        from loki_cli.model_catalog import get_curated_wundercorp_models
        remote = get_curated_wundercorp_models()
    except Exception:
        remote = None
    return list(remote or _PROVIDER_MODELS.get("wundercorp", []))
...
def fetch_ai_gateway_models(
    ...
    from loki_constants import AI_GATEWAY_BASE_URL
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from loki_cli.providers import custom_provider_slug
...
def _provider_has_credentials(pid: str) -> bool:
    try:
        from loki_cli.auth import get_auth_status, has_usable_secret
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    derived from :data:`CANONICAL_PROVIDERS` (shared with ``loki model`` and ``/model``)."""
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, WunderCorp, etc. all support /models)
...
def _resolve_static_model_alias(
    ...
    try:
        from loki_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # Step 0: a bare provider name typed as the model (`/model wundercorp`) is a provider switch to that
    ...
    if resolved_provider not in {"custom", "openrouter"}:
        ...
        if resolved_provider in _PROVIDER_LABELS and default_models and resolved_provider not in current_keys:
            ...
            # so [0] would silently escalate `/model wundercorp` to the priciest flagship.
...
def _resolve_provider_prefix(model_name: str) -> Optional[tuple[str, str]]:
    """Route an explicit ``vendor/model`` prefix (``wundercorp/deepseek-v4-pro``, ``ollama/qwen3.5:4b``) to
    a provider the user defined in ``providers:`` (by raw name or alias) instead of the default.

    ``wundercorp/deepseek-v4-pro`` or ``ollama/qwen3.5:4b`` should route to the named provider instead of falling
...
def detect_provider_for_model(
    ...
    NAMED the provider (``/model wundercorp``), or there is no current provider yet (``auto``) — then the
    first guess is returned so the credential step fails loudly instead of silently ignoring input."""
    from loki_cli.models_detect import (
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``loki_cli.auth.resolve_provider()`` to resolve it from credentials."""
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Loki should expose the /fast toggle for this model."""
...
def _first_exchangeable_copilot_token(raw_tokens) -> str:
    ...
    from loki_cli.copilot_auth import exchange_copilot_token, validate_copilot_token
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    def _pool_token() -> str:
        from loki_cli.auth import read_credential_pool
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from loki_cli.model_search import model_alias_canonical
...
def _codex_catalog(normalized: str, force_refresh: bool) -> list[str]:
    from loki_cli.codex_models import get_codex_model_ids
...
def _wundercorp_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    try:
        from loki_cli.auth import fetch_wundercorp_models, resolve_wundercorp_runtime_credentials

        creds = resolve_wundercorp_runtime_credentials()
        if creds:
            live = fetch_wundercorp_models(api_key=creds.get("api_key", ""), inference_base_url=creds.get("base_url", ""))
            if live:
                return live
    except Exception:
        pass
    # Live failed / no creds: the docs-hosted manifest — NOT the in-repo snapshot — so newly added
    # Portal models still surface without a Loki release.
    return get_curated_wundercorp_model_ids() or None
...
def _openai_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    ...
    # entries, so intersect with the curated agentic catalog so ``/model`` matches ``loki model``.
    ...
    # pure-catalog helper (no extra live fetch) so we only accept models Loki actually ships. (#46850)
    ...
    from loki_cli.providers import is_official_openai_host
...
_PROVIDER_CATALOG_FETCHERS: dict[str, Any] = {
    ...
    "wundercorp": _wundercorp_catalog,
...
# every authed provider's /v1/models). One JSON file at $LOKI_HOME/provider_models_cache.json;
...
def _provider_models_cache_path() -> Path:
    from loki_constants import get_loki_home
    return get_loki_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    try:
        ...
        for rel in ("auth.json", "credentials.json"):
            _mtime_part(rel, get_loki_home() / rel)
    except Exception:
        pass
...
def clear_provider_models_cache(provider: Optional[str] = None) -> None:
    ...
    ``/model --refresh`` and ``loki model --refresh``."""
    try:
...
def opencode_zen_free_headers() -> dict:
    ...
    try:
        from loki_cli import __version__ as _v
    except Exception:
        _v = "0"
    return {
        "Authorization": "",
...
def probe_api_models(
    ...
    headers: dict[str, str] = {"User-Agent": _LOKI_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"loki-agent/{_LOKI_VERSION}"
    ...
    if isinstance(request_headers, dict):
        ...
        from loki_cli.config import normalize_extra_headers
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=loki"
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "User-Agent": _LOKI_USER_AGENT}
    try:
...
def cached_fetch_api_models(
    ...
    from loki_cli.model_switch_providers import _NativePickerModelList
...
def is_wundercorp_free_tier(account_info: dict[str, Any]) -> bool:
...
_PLUGIN_COMPAT_LAZY = {
    'LMStudioLoadResult': ('loki_cli.models_local', 'LMStudioLoadResult'),
    'PROVIDER_GROUPS': ('loki_cli.models_catalog_static', 'PROVIDER_GROUPS'),
    'ProviderEntry': ('loki_cli.models_catalog_static', 'ProviderEntry'),
    'atomic_json_write': ('utils', 'atomic_json_write'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'compute_sale_discount': ('loki_cli.models_pricing', 'compute_sale_discount'),
    'ensure_lmstudio_model_loaded': ('loki_cli.models_local', 'ensure_lmstudio_model_loaded'),
    'fetch_ai_gateway_pricing': ('loki_cli.models_pricing', 'fetch_ai_gateway_pricing'),
    'fetch_lmstudio_models': ('loki_cli.models_local', 'fetch_lmstudio_models'),
    'fetch_models_with_pricing': ('loki_cli.models_pricing', 'fetch_models_with_pricing'),
    'fetch_ollama_local_models': ('loki_cli.models_local', 'fetch_ollama_local_models'),
    'get_cached_wundercorp_inference_base_url': ('loki_cli.models_pricing', 'get_cached_wundercorp_inference_base_url'),
    'get_pricing_for_provider': ('loki_cli.models_pricing', 'get_pricing_for_provider'),
    'group_providers': ('loki_cli.models_catalog_static', 'group_providers'),
    'lmstudio_model_reasoning_options': ('loki_cli.models_local', 'lmstudio_model_reasoning_options'),
    'wundercorp_catalog_url': ('loki_cli.models_reasoning_caps', 'wundercorp_catalog_url'),
    'wundercorp_model_reasoning_capabilities': ('loki_cli.models_reasoning_caps', 'wundercorp_model_reasoning_capabilities'),
    'wundercorp_policy_allowed_ids': ('loki_cli.models_pricing', 'wundercorp_policy_allowed_ids'),
    'ollama_model_supports_thinking': ('loki_cli.models_local', 'ollama_model_supports_thinking'),
    'openrouter_model_reasoning_capabilities': ('loki_cli.models_reasoning_caps', 'openrouter_model_reasoning_capabilities'),
    'parse_openrouter_reasoning_capabilities': ('loki_cli.models_reasoning_caps', 'parse_openrouter_reasoning_capabilities'),
    'peek_cached_pricing': ('loki_cli.models_pricing', 'peek_cached_pricing'),
    'pricing_cache_scope': ('loki_cli.models_pricing', 'pricing_cache_scope'),
    'probe_lmstudio_models': ('loki_cli.models_local', 'probe_lmstudio_models'),
    'probe_ollama_local_models': ('loki_cli.models_local', 'probe_ollama_local_models'),
    'provider_group_for_slug': ('loki_cli.models_catalog_static', 'provider_group_for_slug'),
    'refresh_reasoning_caps_async': ('loki_cli.models_reasoning_caps', 'refresh_reasoning_caps_async'),
    'restrict_to_wundercorp_policy': ('loki_cli.models_pricing', 'restrict_to_wundercorp_policy'),
    'should_use_ollama_native_catalog': ('loki_cli.models_local', 'should_use_ollama_native_catalog'),
    'url_origin': ('loki_cli.urllib_security', 'url_origin'),
    'validate_requested_model': ('loki_cli.models_validate', 'validate_requested_model'),
    'warm_wundercorp_reasoning_caps_async': ('loki_cli.models_reasoning_caps', 'warm_wundercorp_reasoning_caps_async'),
    'warm_openrouter_reasoning_caps_async': ('loki_cli.models_reasoning_caps', 'warm_openrouter_reasoning_caps_async'),
}
