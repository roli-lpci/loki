"""Single source of truth for provider identity in Loki Agent."""
...
# -- Loki overlay: metadata models.dev doesn't provide ----------------------

@dataclass(frozen=True)
class LokiOverlay:
    """Loki-specific provider metadata layered on top of models.dev."""
...
LOKI_OVERLAYS: Dict[str, LokiOverlay] = {
    "moa": LokiOverlay(auth_type="virtual", base_url_override="moa://local"),
    "openrouter": LokiOverlay(is_aggregator=True, base_url_env_var="OPENROUTER_BASE_URL"),
    "wundercorp": LokiOverlay(auth_type="oauth_device_code", base_url_override="https://inference-api.wundercorp.com/v1"),
    "openai-codex": LokiOverlay(transport="codex_responses", auth_type="oauth_external",
                                  base_url_override="https://chatgpt.com/backend-api/codex"),
    "openai-api": LokiOverlay(transport="codex_responses", base_url_override="https://api.openai.com/v1",
                                base_url_env_var="OPENAI_BASE_URL"),
    "xai-oauth": LokiOverlay(transport="codex_responses", auth_type="oauth_external",
                               base_url_override="https://api.x.ai/v1", base_url_env_var="XAI_BASE_URL"),
    "qwen-oauth": LokiOverlay(auth_type="oauth_external", base_url_override="https://portal.qwen.ai/v1",
                                base_url_env_var="LOKI_QWEN_BASE_URL"),
    "lmstudio": LokiOverlay(extra_env_vars=("LM_API_KEY",), base_url_override="http://127.0.0.1:1234/v1",
                              base_url_env_var="LM_BASE_URL"),
    "copilot-acp": LokiOverlay(transport="codex_responses", auth_type="external_process",
                                 base_url_override="acp://copilot", base_url_env_var="COPILOT_ACP_BASE_URL"),
    "github-copilot": LokiOverlay(extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN")),
    "anthropic": LokiOverlay(transport="anthropic_messages", extra_env_vars=("ANTHROPIC_TOKEN", "CLAUDE_CODE_OAUTH_TOKEN")),
    "zai": LokiOverlay(extra_env_vars=("GLM_API_KEY", "ZAI_API_KEY", "Z_AI_API_KEY"), base_url_env_var="GLM_BASE_URL"),
    "kimi-for-coding": LokiOverlay(base_url_env_var="KIMI_BASE_URL"),
    "stepfun": LokiOverlay(extra_env_vars=("STEPFUN_API_KEY",),
                             base_url_override="https://api.stepfun.ai/step_plan/v1",
                             base_url_env_var="STEPFUN_BASE_URL"),
    "minimax": LokiOverlay(transport="anthropic_messages", base_url_env_var="MINIMAX_BASE_URL"),
    "minimax-oauth": LokiOverlay(transport="anthropic_messages", auth_type="oauth_external",
                                   base_url_override="https://api.minimax.io/anthropic"),
    "minimax-cn": LokiOverlay(transport="anthropic_messages", base_url_env_var="MINIMAX_CN_BASE_URL"),
    "deepseek": LokiOverlay(base_url_env_var="DEEPSEEK_BASE_URL"),
    "alibaba": LokiOverlay(base_url_env_var="DASHSCOPE_BASE_URL"),
    "alibaba-coding-plan": LokiOverlay(base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL"),
    "vercel": LokiOverlay(is_aggregator=True),
    "opencode": LokiOverlay(is_aggregator=True, base_url_env_var="OPENCODE_ZEN_BASE_URL"),
    "opencode-go": LokiOverlay(is_aggregator=True, base_url_env_var="OPENCODE_GO_BASE_URL"),
    "opencode-free": LokiOverlay(is_aggregator=True, base_url_override="https://opencode.ai/zen/v1", keyless=True),
    "kilo": LokiOverlay(is_aggregator=True, base_url_env_var="KILOCODE_BASE_URL"),
    "huggingface": LokiOverlay(is_aggregator=True, base_url_env_var="HF_BASE_URL"),
    "novita": LokiOverlay(is_aggregator=True, base_url_env_var="NOVITA_BASE_URL"),
    "xai": LokiOverlay(transport="codex_responses", base_url_override="https://api.x.ai/v1", base_url_env_var="XAI_BASE_URL"),
    "nvidia": LokiOverlay(base_url_override="https://integrate.api.nvidia.com/v1", base_url_env_var="NVIDIA_BASE_URL"),
    "xiaomi": LokiOverlay(base_url_env_var="XIAOMI_BASE_URL"),
    "tencent-tokenhub": LokiOverlay(base_url_env_var="TOKENHUB_BASE_URL"),
    "tencent-tokenplan": LokiOverlay(transport="anthropic_messages",
                                       base_url_override="https://api.lkeap.cloud.tencent.com/plan/anthropic",
                                       base_url_env_var="TOKENPLAN_BASE_URL"),
    "arcee": LokiOverlay(base_url_override="https://api.arcee.ai/api/v1", base_url_env_var="ARCEE_BASE_URL"),
    "gmi": LokiOverlay(extra_env_vars=("GMI_API_KEY",), base_url_override="https://api.gmi-serving.com/v1",
                         base_url_env_var="GMI_BASE_URL"),
    "fireworks": LokiOverlay(extra_env_vars=("FIREWORKS_API_KEY",),
                               base_url_override="https://api.fireworks.ai/inference/v1"),
    "actual": LokiOverlay(transport="chat_completions", extra_env_vars=("ACTUAL_API_KEY",),
                            base_url_override="https://api.actual.inc/v1", base_url_env_var="ACTUAL_BASE_URL"),
    "upstage": LokiOverlay(extra_env_vars=("UPSTAGE_API_KEY",), base_url_override="https://api.upstage.ai/v1",
                             base_url_env_var="UPSTAGE_BASE_URL"),
    "nebius-token-factory": LokiOverlay(extra_env_vars=("NEBIUS_API_KEY", "NEBIUS_TOKEN_FACTORY_API_KEY"),
                                          base_url_override="https://api.tokenfactory.nebius.com/v1",
                                          base_url_env_var="NEBIUS_BASE_URL"),
    "ollama-cloud": LokiOverlay(base_url_override="https://ollama.com/v1", base_url_env_var="OLLAMA_BASE_URL"),
    # Azure Foundry serves OpenAI- and Anthropic-style endpoints; transport comes from model.api_mode.
    "azure-foundry": LokiOverlay(base_url_env_var="AZURE_FOUNDRY_BASE_URL"),
    "bedrock": LokiOverlay(transport="bedrock_converse", auth_type="aws_sdk"),
    ...
    "vertex": LokiOverlay(auth_type="vertex"),
}
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "loki", "user-config"
...
def _overlay_pdef(canonical, ov: LokiOverlay, name, env_vars, base_url, doc, source) -> ProviderDef:
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    """Look up a built-in provider by id or alias: models.dev catalog merged with the Loki overlay;
    Loki-only overlay (wundercorp, openai-codex, …); plugin provider profiles with a concrete endpoint."""
    canonical = normalize_provider(name)
    mdev_info = _models_dev_info(canonical, allow_network)
    overlay = LOKI_OVERLAYS.get(canonical)
    if mdev_info is not None:
        ov = overlay or LokiOverlay()
    ...
    if overlay is not None:
        return _overlay_pdef(canonical, overlay, _LABEL_OVERRIDES.get(canonical, canonical), overlay.extra_env_vars,
                             overlay.base_url_override, "", "loki")
    # Plugin-registered profiles (plugins/model-providers/<name>/) absent from models.dev and
    # LOKI_OVERLAYS would otherwise be "Unknown provider" in /model, --provider and model-switch
...
def wundercorp_api_mode(model: str = "") -> str:
    """Wire protocol for a WunderCorp Portal model. Portal serves its ``anthropic/*`` catalog on a native
    Messages route alongside OpenAI-compatible chat/completions for everything else.

    ``anthropic/*`` rides chat/completions by default for now (``wundercorp.anthropic_wire``). Measured
    ...
    (WunderCorp/api#227 carries the diagnostics); flip the default back to ``native`` when it is
    ...
    Empty/unknown model defaults to ``chat_completions`` (the historical WunderCorp transport)."""
    if str(model or "").strip().lower().startswith("anthropic/"):
        # ``auto`` starts on chat too: it is safe on every upstream, and ``agent/wundercorp_wire.py``
        # promotes the session to native from the first response when the upstream allows it.
        return "anthropic_messages" if _wundercorp_anthropic_wire() == "native" else "chat_completions"
    return "chat_completions"


def _wundercorp_anthropic_wire() -> str:
    """``wundercorp.anthropic_wire``: ``"chat"`` (default), ``"native"``, or ``"auto"`` (chat, then per-session
    promotion decided from the first response; see ``agent/wundercorp_wire.py``). Anything else reads as ``chat``."""
    try:
        from loki_cli.config import load_config_readonly
        value = str(((load_config_readonly().get("wundercorp") or {}).get("anthropic_wire")) or "chat").strip().lower()
    except Exception:
        return "chat"
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    """API mode (wire protocol) for a provider/endpoint: host-mandated mode, then WunderCorp dual-wire
    ...
    if mandated is not None:
        return mandated
    if (provider or "").strip().lower() in {"wundercorp", "wundercorp-portal", "wundercorp"}:
        return wundercorp_api_mode(model)
...
def _lossy_alias_registry_pdef(raw: str, canonical: str) -> Optional[ProviderDef]:
    """Exact Loki registry ids win over LOSSY alias collapsing (kimi-coding-cn must stay distinct
    ...
    try:
        from loki_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
...
def _llamacpp_pdef() -> Optional[ProviderDef]:
    ...
    try:
        from loki_cli.local_runtime.endpoint import resolve_llamacpp_endpoint
