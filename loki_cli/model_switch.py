...
from loki_cli.providers import (
...
# Re-exported: callers/tests patch loki_cli.model_switch.<name>.
from loki_cli.model_switch_providers import list_authenticated_providers
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, str):
        ...
        # Pre-fix Loki wrote sentinel keys inside the user-facing ``models`` mapping.
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Loki.
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    ``discover_models: false``. A catalog Loki itself persisted (``discovered``) is never a pin."""
    if discovered:
        return False
    if isinstance(value, str):
...
_LOKI_MODEL_WARNING = (
    "WunderCorp, Inc. Loki 3 & 4 models are NOT agentic and are not designed "
    "for use with Loki Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.).")

# Match only the real WunderCorp, Inc. Loki 3 / 4 chat families; a bare substring check
# false-positived on tool-capable local Modelfiles like ``loki-brain:qwen3-14b-ctx16k``.
#   match:    WunderCorp/Loki-3-Llama-3.1-70B, loki-4-405b, openrouter/loki3:70b
#   no match: loki-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_WUNDERCORP_LOKI_NON_AGENTIC_RE = re.compile(r"(?:^|[/:])loki[-_ ]?[34](?:[-_.:]|$)", re.IGNORECASE)
...
def is_wundercorp_loki_non_agentic(model_name: str) -> bool:
    """True if *model_name* is a real WunderCorp Loki 3/4 chat model (single owner; cli.py uses it too)."""
    return bool(model_name and _WUNDERCORP_LOKI_NON_AGENTIC_RE.search(model_name))


def _check_loki_model_warning(model_name: str) -> str:
    """Warning string if *model_name* is a WunderCorp Loki 3/4 chat model, else ""."""
    return _LOKI_MODEL_WARNING if is_wundercorp_loki_non_agentic(model_name) else ""
...
def _direct_alias_source_identity() -> Optional[tuple]:
    ...
    try:
        from loki_constants import get_config_path
...
def resolve_startup_model_route(
    ...
    ``LokiCLI`` is constructed before the interactive ``/model`` pipeline runs; resolving here
    ...
    if current_provider:
        try:
            from loki_cli.providers import is_routing_aggregator, normalize_provider as _norm_prov
    ...
    try:
        from loki_cli.models import normalize_provider
...
def _resolve_alias_fallback(
    ...
    """Resolve an alias on the user's authenticated providers (``("openrouter", "wundercorp")`` when none given).
    ...
    results = (resolve_alias(raw_input, p) for p in authenticated_providers or ("openrouter", "wundercorp"))
...
def _ollama_configured_base() -> tuple[dict, str]:
    from loki_cli.models import _get_provider_config_dict
...
def _unknown_provider_message(explicit_provider: str) -> str:
    msg = (
        f"Unknown provider '{explicit_provider}'. Check 'loki model' for available "
    ...
    try:  # Surface common config issues that cause provider resolution failures
        ...
        if issues:
            msg += "\n\nRun 'loki doctor' — config issues detected:" + "".join(f"\n  • {ci.message}" for ci in issues[:3])
    except Exception:
        pass
    return msg
...
def _route_from_model_input(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    from loki_cli.models import detect_provider_for_model
    raw_input, current_provider = st.raw_input, st.current_provider
    try:
        from loki_cli.config import load_config
        from loki_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if st.resolved_alias or resolved_in_current_catalog or st.target_provider != current_provider:
        return None
    if current_provider == "wundercorp":
        # The welcome host serves wundercorp/welcome only; a model outside it needs an account or a key.
        ...
        if route_is_welcome_host(st.current_base_url) and st.new_model != GUEST_MODEL:
            return st.fail(
                f"{st.new_model} needs a WunderCorp account or an API key. "
...
def _creds_for_switched_provider(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    if st.explicit_provider and st.user_providers:
        from loki_cli.providers import resolve_user_provider
...
def _creds_for_current_provider(st: _Switch) -> None:
    ...
    from loki_cli.models_local import _get_ollama_request_headers, _same_ollama_native_root
...
def _copilot_api_mode(provider: str, model: str, api_key: str) -> str:
    from loki_cli.models import copilot_model_api_mode
...
def _opencode_api_mode(provider: str, model: str, api_key: str) -> str:
    ...
    # #16878.
    from loki_cli.models import opencode_model_api_mode
    return opencode_model_api_mode(provider, model)


def _wundercorp_api_mode(provider: str, model: str, api_key: str) -> str:
...
_PROVIDER_API_MODE_OVERRIDES: dict[str, Any] = {
    ...
    **dict.fromkeys(("wundercorp", "wundercorp-portal", "wundercorp"), _wundercorp_api_mode)}
...
def _build_switch_result(st: _Switch) -> ModelSwitchResult:
    ...
    from loki_cli.models import normalize_opencode_base_url, opencode_provider_family
    ...
    warnings = [w for w in (st.validation.get("message"), _check_loki_model_warning(st.new_model)) if w]
    ...
    try:
        from loki_cli.runtime_provider import _get_named_custom_provider, _custom_provider_request_overrides
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'list_picker_providers': ('loki_cli.model_switch_providers', 'list_picker_providers'),
    'prewarm_picker_cache_async': ('loki_cli.model_switch_providers', 'prewarm_picker_cache_async'),
}
