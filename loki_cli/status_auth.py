"""Credential sections of `loki status`, run through ``status._SECTIONS`` with its shared context.
Origin helpers (``_row``, ``_first_env_value``, ...) are resolved through the ``loki_cli.status``
...
_OAUTH_BLOCKS = (
    ...
    ("OpenAI Codex", "get_codex_auth_status", "loki model", _FILE_REFRESH_ROWS),
    ...
    ("MiniMax OAuth", "get_minimax_oauth_auth_status", "loki auth add minimax-oauth", (
        ("Region:", "region", None, True),
        ("Access exp:", "expires_at", None, None),
        ("Error:", "error", None, False))),
    ("xAI OAuth", "get_xai_oauth_auth_status", "loki auth add xai-oauth", _FILE_REFRESH_ROWS))
...
# WunderCorp Tool Gateway per-feature state: first matching (predicate(feature, wundercorp_auth), text(feature)).
_FEATURE_STATES = (
    (lambda f, _: f.managed_by_wundercorp, lambda f: "active via WunderCorp subscription"),
...
def _render_auth_providers(ctx):
    ...
    try:
        # Read-only display: the refresh-free snapshot, so `loki status` never performs an OAuth
        # refresh or burns a single-use refresh token.
        wundercorp_status = auth.get_wundercorp_auth_status_local()
        statuses = {getter: getattr(auth, getter)() for _, getter, _, _ in _OAUTH_BLOCKS[:3]}
    except Exception:
        wundercorp_status, statuses = {}, {}
    ...
    info = None
    if any(wundercorp_status.get(k) for k in ("logged_in", "access_token", "portal_base_url",
                                        "inference_credential_present", "error_code")):
        try:
            info = get_wundercorp_portal_account_info()
        except Exception:
            pass
    ctx.wundercorp_account_info = info
    ctx.wundercorp_logged_in = logged_in = bool(wundercorp_status.get("logged_in") or (info and info.logged_in))
    ctx.wundercorp_inference_present = inference = bool(
        wundercorp_status.get("inference_credential_present") or (info and info.inference_credential_present)
    )
    if wundercorp_status.get("free_tier"):
        ...
        _status._row("WunderCorp Portal", True, f"{FREE_TIER_LABEL} · {GUEST_MODEL}")
        _status._detail("", UPGRADE_HINT)
        inference_url = wundercorp_status.get("inference_base_url")
        if inference_url:
        ...
        return
    wundercorp_error = wundercorp_status.get("error")
    _status._row("WunderCorp Portal", logged_in,
         "logged in" if logged_in else "not logged in (WunderCorp inference key configured)" if inference
         else "not logged in (run: loki portal)")
    portal_url = wundercorp_status.get("portal_base_url") or "(unknown)"
    inference_url = wundercorp_status.get("inference_base_url") or (info.inference_base_url if info else None)
    for label, value, show in (
        ("Portal URL:", portal_url, logged_in or portal_url != "(unknown)" or wundercorp_error),
        ("Inference:", inference_url, inference and inference_url),
        ("Access exp:", _format_iso_timestamp(wundercorp_status.get("access_expires_at")),
         logged_in or wundercorp_status.get("access_expires_at")),
        ("Key exp:", _format_iso_timestamp(wundercorp_status.get("agent_key_expires_at")),
         logged_in or inference or wundercorp_status.get("agent_key_expires_at")),
        ("Refresh:", "yes" if wundercorp_status.get("has_refresh_token") else "no",
         logged_in or wundercorp_status.get("has_refresh_token")),
        ("Error:", wundercorp_error, wundercorp_error)):
        if show:
...
def _render_wundercorp_gateway(ctx):
    if managed_wundercorp_tools_enabled():
        features = get_wundercorp_subscription_features(ctx.config)
        _status._section("WunderCorp Tool Gateway")
        print("  WunderCorp Portal   ✓ managed tools available" if features.wundercorp_auth_present
              else "  WunderCorp Portal   ✗ not logged in")
        for f in features.items():
            state = next((text(f) for match, text in _FEATURE_STATES if match(f, features.wundercorp_auth_present)),
                         "not configured")
            _status._row(f.label, f.available or f.active or f.managed_by_wundercorp, state, 15, " ")
    elif ctx.wundercorp_logged_in or ctx.wundercorp_inference_present:
        # WunderCorp OAuth without entitlement, or an opaque inference key without Portal account
        # information, cannot enable the Tool Gateway.
        _status._section("WunderCorp Tool Gateway")
        message = format_wundercorp_portal_entitlement_message(
            ctx.wundercorp_account_info, capability="managed web, image, TTS, STT, browser, and Modal tools"
        )
...
def _render_apikey_providers(ctx):
    ...
    for pname, env_vars in _APIKEY_PROVIDERS.items():
        ...
        _status._row(pname, configured, "configured" if configured else "not configured (run: loki model)", 16, " ")
    ...
    if _status._effective_provider_label() == "LM Studio":
        from loki_cli.models_local import probe_lmstudio_models
...
import loki_cli.status as _status  # noqa: E402  (bottom: loki_cli.status imports this module)
