...
from loki_cli.auth_constants import (
    AuthError, DEFAULT_WUNDERCORP_PORTAL_URL, DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS,
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(
        ...
        f"Loki is waiting for the OAuth callback on {redirect_uri}\n"
...
def _resolve_verify(
    *, insecure: Optional[bool] = None, ca_bundle: Optional[str] = None,
    ...
    from loki_cli.auth import _default_verify
    ...
    effective_ca = (
        ca_bundle or tls_state.get("ca_bundle") or os.getenv("LOKI_CA_BUNDLE")
...
def _wundercorp_device_auth_timeout_message(portal_base_url: str) -> str:
    ...
    """
    portal = (portal_base_url or DEFAULT_WUNDERCORP_PORTAL_URL).rstrip("/")
    return (
...
def _poll_device_token_generic(
    ...
    """RFC 8628 device-code polling loop shared by the WunderCorp and xAI flows.
...
def _poll_for_token(
    ...
    """Poll the WunderCorp token endpoint until the user approves or the code expires."""
    ...
    return _poll_device_token_generic(
        ...
        # (web_server_oauth._wundercorp_promotion_poller surfaces it to the UI) both inherit the guidance.
        on_timeout=lambda: TimeoutError(_wundercorp_device_auth_timeout_message(portal_base_url)))
    ...
    *resolve* attempts a refresh, so a resolved token should be valid — but double-check the
    ...
    try:
        ...
        if isinstance(api_key, str) and api_key and not is_expiring(api_key, 60):
            print(f"Existing {display_name} credentials found in Loki auth store.")
