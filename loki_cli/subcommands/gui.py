...
def build_gui_parser(subparsers, *, cmd_gui: Callable) -> None:
    ...
    gui_parser = subparsers.add_parser(
        ...
        description="Launch the Loki Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        ...
        help="Force Desktop to ignore any loki CLI already on PATH during backend resolution")
    gui_parser.add_argument(
        "--loki-root",
        help="Override the Loki source root used by Desktop (sets LOKI_DESKTOP_ROOT)")
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets LOKI_DESKTOP_CWD)")
    gui_parser.add_argument(
    ...
    gui_parser.add_argument(
        "--identity", default="Loki Local Signing",
        help="Certificate name to create/use for --setup-tcc-identity (default: Loki Local Signing)",
    )
