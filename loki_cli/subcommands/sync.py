"""``loki sync`` subcommand parser — Skill Sync.
...
resolved WunderCorp token carries the access-gate claim AND a sync base URL is configured — the
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog="Examples:\n"
            "  loki sync status            what is synced, and from where\n"
            "  loki sync enable my-skill   include a skill in your sync\n"
            "  loki sync now               pull, then push\n"
            "  loki sync propose my-skill  share a skill with your team\n",
