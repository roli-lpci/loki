...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Loki Agent. Use 'loki debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    loki debug share              Upload debug report (asks for confirmation)
    loki debug share --yes        Skip confirmation (for scripts/CI)
    loki debug share --lines 500  Include more log lines
    loki debug share --expire 30  Keep dpaste.com fallback pastes for 30 days
    loki debug share --local      Print report locally (no upload)
    loki debug share --no-redact  Disable upload-time secret redaction
    loki debug share --wundercorp       Upload to WunderCorp-internal storage (private)
    loki debug delete <url>       Delete a previously uploaded paste
""")
    ...
    share_parser.add_argument(
        "--wundercorp", action="store_true",
        help="Upload the debug bundle to WunderCorp-internal storage (AWS S3) instead "
            "of a public paste service. The bundle is private — viewable only "
            "by WunderCorp staff (and allowlisted Discord mods) via a Google-login-"
            "gated viewer — and auto-deletes after 14 days. Still force-redacts "
            "secrets unless --no-redact is also passed.")
    delete_parser = debug_sub.add_parser(
        "delete", help="Delete a paste uploaded by 'loki debug share'")
