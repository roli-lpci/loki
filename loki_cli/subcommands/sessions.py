"""``loki sessions`` subcommand parser."""
...
def build_sessions_parser(subparsers, *, cmd_sessions: Callable) -> None:
    ...
    sessions_export.add_argument("output", nargs="?",
        help="Output path. JSONL: file path (use - for stdout, required). "
            "md/qmd: output directory (default: <loki home>/session-exports)")
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import", help="Import a Claude Code or Codex CLI session into Loki",
        description="Pull a conversation started in Claude Code (~/.claude/projects) "
            "or Codex CLI (~/.codex/sessions) into the Loki session store "
            "so it can be resumed with 'loki --resume <id>'. The foreign "
    ...
    # cmd_sessions lives in loki_cli/sessions_cmd.py; the parser is threaded
