...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp", help="Manage MCP servers and run Loki as an MCP server",
        description="Manage MCP server connections and run Loki as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'loki mcp add' to connect to a new server, or\n"
            "'loki mcp serve' to expose Loki conversations over MCP.")
    mcp_sub = mcp_parser.add_subparsers(dest="mcp_action")

    mcp_serve_p = mcp_sub.add_parser(
        "serve", help="Run Loki as an MCP server (expose conversations to other agents)")
    ...
    # Catalog (WunderCorp-approved MCPs shipped with the repo)
    mcp_sub.add_parser(
        "picker", help="Interactive catalog picker (also the default for `loki mcp`)")
    mcp_sub.add_parser("catalog", help="List WunderCorp-approved MCPs available for one-click install")
    mcp_install_p = mcp_sub.add_parser(
        "install", help="Install a catalog MCP by name (e.g. `loki mcp install n8n`)")
