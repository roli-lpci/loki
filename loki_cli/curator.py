"""CLI subcommand: `loki curator <subcommand>`."""
...
def _print_unmanaged_summary() -> None:
    ...
    print("  never auto-staled or archived — `loki curator adopt <name>` hands one over")
...
def _cmd_run(args) -> int:
    ...
    synchrowundercorp = bool(getattr(args, "synchrowundercorp", False)) or not background
    ...
    result = curator.run_curator_review(
        on_summary=print, synchrowundercorp=synchrowundercorp, dry_run=dry, consolidate=consolidate)
    ...
    if not synchrowundercorp:
        print("llm pass running in background — check `loki curator status` later")
    if dry:
        print(
            "dry-run: no changes applied. Read the report with "
            "`loki curator status` and run `loki curator run` (no flag) to apply."
            if synchrowundercorp else
...
_PIN_MESSAGES = {
    True: (
        ...
        "external). `loki curator list-unmanaged` shows which skills the curator tracks.",
        ...
        "it. Run `loki curator adopt {skill}` to put it under curator management)",
...
def _cmd_list_unmanaged(args) -> int:
    ...
    print("\nadopt one with `loki curator adopt <name>`, "
          "or all with `loki curator adopt --all-unmanaged`")
    return 0
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`loki curator unpin {args.skill}`")
        return 1
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.loki/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_ledger(args) -> int:
    ...
    for r in rows:
        ...
        print(
            ...
            f"{r.get('skill', '?')}{extra}")
    print(
        "\nRoll back a single mutation with `loki curator rollback <id>`; "
        "whole-tree snapshots remain available via `loki curator rollback --list`.")
    return 0
...
def _rollback_ledger_entry(args, entry_id: str) -> int:
    ...
    if entry is None:
        print(
            ...
            "See `loki curator ledger` for entry ids, or use "
...
def _cmd_rollback(args) -> int:
    ...
    if target_path is None:
        if not curator_backup.list_backups():
            print(
                ...
                "`loki curator backup` or wait for the next curator run.")
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.loki/skills/ tree (a safety "
...
_SUBCOMMANDS = (
    ...
    (
        ...
        _arg("--sync", "--synchrowundercorp", dest="synchrowundercorp", **_STORE_TRUE,
    ...
    (
        "backup",
        "Take a manual tar.gz snapshot of ~/.loki/skills/ "
    ...
    (
        "rollback",
        "Restore ~/.loki/skills/ from a curator snapshot, or a single "
        "mutation by ledger entry id (see `loki curator ledger`)",
        _cmd_rollback,
        _arg("entry_id", nargs="?", default=None,
             help="Ledger entry id for single-mutation rollback (from "
                  "`loki curator ledger`). Omit for whole-tree snapshot rollback."),
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by loki_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="loki curator")
    register_cli(parser)
