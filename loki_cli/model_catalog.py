...
``~/.loki/cache/model_catalog.json`` → master URL fetch; any fetch failure keeps the stale copy
(or ``{}``). ``get_curated_openrouter_models()`` / ``get_curated_wundercorp_models()`` are thin accessors
...
DEFAULT_CATALOG_URL = (
    "https://loki.computer/docs/api/model-catalog.json")
# The Doku site sits behind Vercel, which occasionally 403s non-browser clients (bot
# challenge); the raw GitHub copy is the same manifest and is not bot-gated.
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/wundercorp/loki/main/website/static/api/model-catalog.json",
)
...
def _cache_path() -> Path:
    ...
    return get_loki_home() / "cache" / "model_catalog.json"
...
def _fetch_manifest(url: str, timeout: float) -> dict[str, Any] | None:
    ...
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": _LOKI_USER_AGENT})
...
def refresh_catalogs() -> bool:
    """Force-refresh every remote catalog the picker reads (manifest, OpenRouter live list, WunderCorp Portal
    ...
    try:
        from loki_cli.models import fetch_wundercorp_recommended_models, fetch_openrouter_models

        fetch_openrouter_models(force_refresh=True)
        fetch_wundercorp_recommended_models(force_refresh=True)
    except Exception:
...
def get_curated_wundercorp_models() -> list[str] | None:
    """WunderCorp Portal's curated model ids from the manifest."""
    return [mid for mid, _ in _block_ids(_get_provider_block("wundercorp"))] or None
...
def get_default_model_from_cache(provider: str) -> str | None:
    """The manifest's labeled default for ``provider`` (the model Loki silently lands on when the
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    After ``loki update`` that file IS the newest catalog, so the picker stays current even when
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``loki model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
