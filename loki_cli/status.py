...
from loki_cli.auth import AuthError, resolve_provider
from loki_cli.colors import Colors, color
from loki_cli.config import get_env_path, get_env_value, get_loki_home, load_config
from loki_cli.models import provider_label
from loki_cli.runtime_provider import resolve_requested_provider
from loki_cli.vercel_auth import describe_vercel_auth
from loki_cli.status_auth import (  # renderers wired into _SECTIONS below
    _render_api_keys, _render_apikey_providers, _render_auth_providers, _render_wundercorp_gateway)
...
def _estop_status_line():
    """One-line pause banner for `loki status`, or None when not paused."""
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{f' — reason: {reason}' if reason else ''}; `loki resume` to lift)"
...
def _render_terminal(ctx):
    ...
    if terminal_env in _TERMINAL_ENV_ROWS:
        ...
        _kv_flag("SDK:", importlib.util.find_spec("vercel") is not None, "installed",
                 "missing (install: pip install 'loki-agent[vercel]')")
        ...
        # (fail-soft — never break `loki status`).
        try:
...
def _render_gateway(ctx):
    ...
    try:
        from loki_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
...
def _render_cron(ctx):
    ...
    jobs_file = get_loki_home() / "cron" / "jobs.json"
...
def _render_sessions(ctx):
    ...
    if gateway_rows:
        ...
        if freshest > 0:
            from loki_cli.timefmt import relative_time
            print(f"  Last activity:{relative_time(freshest):>13}")
    elif not (sessions_file := get_loki_home() / "sessions" / "sessions.json").exists():
        _kv("Active:", 0)
    else:
        try:
    ...
    try:
        from loki_cli.active_sessions import (
...
def _render_footer(ctx):
    _banner(("─" * 60, "  Run 'loki doctor' for detailed diagnostics", "  Run 'loki setup' to configure"),
            Colors.DIM)
    print()


# Print order of `loki status`; each renderer takes the shared _StatusContext.
_SECTIONS = (
    _render_header, _render_environment, _render_api_keys, _render_auth_providers, _render_wundercorp_gateway,
...
def show_status(args):
    """Show status of all Loki Agent components."""
    # Shared by section renderers: config, --deep, and the WunderCorp login facts Auth Providers derives
    # for the later WunderCorp Tool Gateway section.
    ctx = SimpleNamespace(deep=getattr(args, 'deep', False), config={}, wundercorp_logged_in=False,
                          wundercorp_inference_present=False, wundercorp_account_info=None)
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_wundercorp_portal_account_info': ('loki_cli.wundercorp_account', 'get_wundercorp_portal_account_info'),
