"""Billing and subscription handlers for the interactive CLI (mixed into ``LokiCLI``).
...
_CHARGE_ERROR_COPY = {
    ...
    "cli_billing_disabled": "  Remote spending is off for this account — a billing admin can turn it on from the portal's Loki Agent page.",
...
_KILLSWITCH_REASON_OVERVIEW = "A billing admin can turn it on from the portal's Loki Agent page to add funds here."
_KILLSWITCH_REASON_BUY = "A billing admin can turn it on from the portal's Loki Agent page before adding funds."
...
class CLIBillingMixin:
    ...
    def _print_logged_out(self, state, load_failed, cmd) -> None:
        ...
        if state.error:
            ...
            self._dim("Not logged into WunderCorp Portal.", icon="💳 ", lead=True)
            print(f"  Run `loki portal` to log in, then {cmd}.")
    ...
    def _step_up_remote_spending(self, *, explain, noninteractive_msg, declined_msg, not_granted_msg) -> bool:
        ...
        try:
            from loki_cli.auth import step_up_wundercorp_billing_scope
            granted = step_up_wundercorp_billing_scope(open_browser=True)
    ...
    # ── /usage — WunderCorp balance block ──

    def _print_wundercorp_credits_block(self) -> bool:
        """WunderCorp dollar balance block (two bars); True if anything printed. Shared dollar model first, then
        legacy ``wundercorp_credits_lines``. Agent-independent (TUI slash-worker has no live agent). Fail-open."""
        ...
        if usage is not None and usage.available:
            ...
            if head or tail:
                return True
        from agent.account_usage import wundercorp_credits_lines
        lines = wundercorp_credits_lines()
        if not lines:
            return False
        print()
        for line in lines:
    ...
    def _print_usage_cta(self) -> None:
        """The `/usage` call-to-action; mirrors the TUI's ``USAGE_CTA``. WunderCorp-account only."""
    ...
    def _subscription_preview_and_confirm(self, state, tier_id, *, allow_stepup=True):
        ...
        from loki_cli.wundercorp_billing import BillingError, BillingScopeRequired, post_subscription_preview
    ...
    def _subscription_apply(self, state, action, idempotency_key=None, *, allow_stepup=True):
        ...
        from cli import _cprint
        from loki_cli.wundercorp_billing import (
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        granted = self._step_up_remote_spending(
            ...
            noninteractive_msg="  Run `loki portal` and allow Remote Spending, then re-run /subscription.",
        ...
        try:
            from loki_cli import wundercorp_billing as _nb
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        print(f"  {_RULE}")
        self._dim('By confirming, you allow WunderCorp, Inc. to charge your card.')
        confirm_choices = [
    ...
    def _billing_submit_and_poll(self, state, amount, key, *, missing_msg, status_msg, on_scope=None):
        ...
        from cli import _cprint, _d
        from loki_cli.wundercorp_billing import BillingError, BillingScopeRequired, post_charge
        try:
    ...
    def _billing_poll_charge(self, state, charge_id, amount):
        ...
        from loki_cli.wundercorp_billing import BillingError, BillingTransient, get_charge_status
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from loki_cli.wundercorp_billing import BillingTransient, BillingRemoteSpendingRevoked, BillingSessionRevoked
        code = exc.error
        ...
        if isinstance(exc, BillingRemoteSpendingRevoked) or code == "remote_spending_revoked":
            ...
            print(f"  🔴 {who} Reconnect to restore — run `loki portal` to re-authorize.")
        elif isinstance(exc, BillingSessionRevoked) or code == "session_revoked":
            print("  🔴 Your session was logged out. Run `loki portal` to log in again.")
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        granted = self._step_up_remote_spending(
            ...
            noninteractive_msg="  Run `loki portal` and allow Remote Spending, then retry.",
        ...
        if not (fresh.logged_in and fresh.cli_billing_enabled):
            print("  Remote Spending is allowed for this terminal, but it's still off for this org. A billing admin can turn it on from the portal's Loki Agent page, then run /topup again.")
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        if reload_amt is None or threshold_amt is None or reload_amt <= threshold_amt:
            ...
            return
        self._dim(f"By confirming, you authorize WunderCorp, Inc. to charge {card.masked} whenever your balance reaches {format_money(threshold_amt)}. Turn off any time here or on the portal.", lead=True)
    ...
    def _billing_patch_auto_top_up(self, state, **kwargs) -> bool:
        ...
        from loki_cli.wundercorp_billing import BillingError, BillingScopeRequired, patch_auto_top_up
        try:
