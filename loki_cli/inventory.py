...
def build_models_payload(
    ...
    capabilities: bool = False, featured: bool = False, force_fresh_wundercorp_tier: bool = False,
    ...
    rows = list_authenticated_providers(
        ...
        custom_providers=ctx.custom_providers, force_fresh_wundercorp_tier=force_fresh_wundercorp_tier,
    ...
    if pricing:
        _apply_pricing(rows, force_fresh_wundercorp_tier=force_fresh_wundercorp_tier, cached_only=pricing_cache_only)
    if capabilities:
...
def _strip_aggregator_overlaps(rows: list[dict]) -> None:
    ...
    try:
        from loki_cli.providers import is_routing_aggregator
    except Exception:
        return
...
def _reasoning_catalog_reader(slug: str):
    ...
    try:
        from loki_cli.models_reasoning_caps import (
            wundercorp_model_reasoning_capabilities,
    ...
    readers = {
        "wundercorp": (warm_wundercorp_reasoning_caps_async, wundercorp_model_reasoning_capabilities),
...
def _apply_capabilities(rows: list[dict]) -> None:
    ...
    from loki_cli.models import model_supports_fast_mode
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`loki_cli.providers.custom_provider_aliases` — lets the frontend do a membership check instead.
    """
    from loki_cli.providers import custom_provider_aliases
...
def _append_unconfigured_rows(
    ...
    from loki_cli.models import CANONICAL_PROVIDERS, _model_requires_account_discovery
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else f"Configured provider is not authenticated; run `loki model` to reactivate. {tail}"
            )
...
def _anthropic_oauth_credentials_present() -> bool:
    """True when the user explicitly authenticated Anthropic via OAuth (Loki device flow or Claude Code
    ...
    try:
        ...
        readers = (read_loki_oauth_credentials, read_claude_code_credentials)
...
def _external_process_signed_in(slug: str) -> bool:
    ...
    try:
        from loki_cli.auth import PROVIDER_REGISTRY, get_external_process_provider_status
...
def _provider_is_keyless(slug: str) -> bool:
    """True when the provider's Loki overlay declares it keyless."""
    try:
        from loki_cli.providers import LOKI_OVERLAYS
        overlay = LOKI_OVERLAYS.get(slug)
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (f"paste {key_env} to activate" if auth_type == "api_key" and key_env
                          else f"run `loki model` to configure ({auth_type})")
...
def _apply_pricing(rows: list[dict], *, force_fresh_wundercorp_tier: bool = False, cached_only: bool = False) -> None:
    """Set ``row["pricing"] = {model_id: {input, output, cache | None, free}}``; for WunderCorp also
    ``free_tier`` (account is free-tier) and ``unavailable_models`` (paid models a free user can't pick).
    ``cached_only`` never hits the network: unknown WunderCorp entitlement fails closed (``free_tier_pending``,
    ...
    from loki_cli.models import (
        ...
        get_cached_wundercorp_free_tier,
        partition_wundercorp_models_by_tier,
    )

    wundercorp_free_tier: Optional[bool] = None  # resolved once (cached in models.py for the TTL window)
    ...
    for row in rows:
        ...
        try:
            ...
            raw_pricing = {}
        cached_wundercorp_tier: Optional[bool] = None
        if slug == "wundercorp" and cached_only:
            cached_wundercorp_tier = get_cached_wundercorp_free_tier()
            if cached_wundercorp_tier is None:
                ...
                if not row.get("warning"):  # say why every model renders locked
                    row["warning"] = ("Checking WunderCorp plan entitlement… models unlock on the "
                                      "next picker open or refresh.")
                continue
        if not raw_pricing:
            if slug == "wundercorp":
                row["free_tier"] = bool(cached_wundercorp_tier)
                row["pricing_pending"] = True
                row["unavailable_models"] = list(models) if cached_wundercorp_tier else []
            continue
        ...
        for mid in models:
            ...
            # Sale chrome is WunderCorp Portal-only (other catalogs' nested pricing.original is ignored); free
        ...
        if slug == "wundercorp":
            try:
                if wundercorp_free_tier is None:
                    wundercorp_free_tier = (cached_wundercorp_tier if cached_only
                                      else check_wundercorp_free_tier(force_fresh=force_fresh_wundercorp_tier))
                row["free_tier"] = bool(wundercorp_free_tier)
                row["unavailable_models"] = (
                    partition_wundercorp_models_by_tier(list(models), raw_pricing, free_tier=True)[1]
                    if wundercorp_free_tier else [])
...
def _local_runtime_row(ctx: "ConfigContext") -> dict | None:
    ...
    try:
        from loki_cli.local_runtime.bootstrap import staged_model_ids
        ...
        if not current:
            ...
            try:
                from loki_cli.local_runtime.endpoint import _state_endpoint
...
def _prewarm_pricing_async(
    ...
    from loki_cli.models_pricing import pricing_cache_scope
    ...
    endpoint_scope = tuple(sorted(
        ...
        for slug in slugs))
    prewarm_key = (loki_home_key(), endpoint_scope)
    ...
    with _pricing_prewarm_lock:
        ...
        thread = Thread(target=copy_context().run, args=(_worker,),
                        name="loki-picker-pricing-prewarm", daemon=True)
