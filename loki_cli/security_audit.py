"""On-demand supply-chain audit for Loki Agent installs.
...
def _discover_plugins(loki_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.loki/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, loki_home: Optional[Path] = None
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = loki_home or Path(get_loki_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `loki security audit`."""
    ...
    components = _discover_components(loki_home=home, **skips)
    ...
    try:
        findings = run_audit(loki_home=home, components=components)
