"""Coalesce concurrent synchrowundercorp HTTP reads before threadpool admission."""
import asyncio
import inspect
...
def coalesced_read(func, *, thread_runner=run_sync):
    """Return an async, in-flight-only wrapper around a synchrowundercorp read.
    ...
    async def wrapped(*args, **kwargs):
        ...
        key = (str(get_loki_home()), _freeze(bound.arguments))
