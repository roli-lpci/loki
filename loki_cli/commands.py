"""Slash command registry for the Loki CLI and gateway.
...
from loki_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Key in ``loki_cli.slash_exec.EXECUTORS`` (a string, not a callable: keeps this module
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Loki config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Loki works on across turns until achieved", "Session",
    ...
    CommandDef("plan", "Write a markdown implementation plan to .loki/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("config", "Show current configuration", "Configuration",
               cli_only=True, desktop="terminal"),
    CommandDef("settings", "Open the interactive CLI settings hub", "Configuration",
               cli_only=True, args_hint="[filter|jev [enable|disable|status]]", argument_mode="mixed",
               desktop="terminal"),
    CommandDef("jev", "Configure Loki Autorouter (powered by Jev)", "Configuration",
               cli_only=True, args_hint="[status|enable|disable|setup]",
               subcommands=("status", "enable", "disable", "setup", "on", "off"), desktop="terminal"),
    CommandDef("ads", "Control advertisement opt-in preference", "Configuration",
    ...
    CommandDef("wake", "Toggle the 'Hey Loki' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]", subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while Loki is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.loki/skills/ for newly installed or removed skills",
    ...
    CommandDef("go", "Activate an extensible task workflow", "Tools & Skills",
               cli_only=True, args_hint="[shopping|ops|status|off]",
               subcommands=("shopping", "ops", "status", "off"), busy_policy="dispatch", desktop="terminal"),
    CommandDef("ops", "Run your business with cross-functional operating lenses", "Tools & Skills",
               cli_only=True,
               args_hint="[overview|finance|sales|leads|crm|operations|inventory|people|hiring|customers|marketing|projects|risk|setup|profile|status|off]",
               subcommands=("overview", "finance", "sales", "leads", "crm", "operations",
                            "inventory", "people", "hiring", "customers", "marketing",
                            "projects", "risk", "setup", "profile", "status", "off"),
               busy_policy="dispatch", desktop="terminal"),
    ...
    CommandDef("subscription", "View your WunderCorp plan and change it in the browser", "Info",
               cli_only=True, aliases=("upgrade",)),
    CommandDef("login", "Sign in with a WunderCorp account (keeps your connectors)", "Info",
               busy_policy="dispatch", desktop="settings"),
    CommandDef("sso", "Manage WunderCorp browser SSO for connected services", "Info",
               args_hint="[status|login|logout]", subcommands=("status", "login", "logout"),
               cli_only=True, busy_policy="dispatch", desktop="terminal"),
    CommandDef("link", "Manage your Link wallet connection", "Info", aliases=("wallet",),
               args_hint="[status|connect|disconnect|user|payment-methods|address]",
               subcommands=("status", "connect", "disconnect", "user", "payment-methods", "address"),
               cli_only=True, busy_policy="dispatch", desktop="terminal"),
    CommandDef("topup", "Show your WunderCorp balance and manage billing on the portal", "Info"),
    ...
    CommandDef("update", "Update Loki Agent to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show Loki Agent version", "Info", aliases=("v",),
               busy_policy="dispatch", execute="version"),
    CommandDef("debug", "Upload debug report (system info + logs) and get shareable links", "Info",
               args_hint="[wundercorp|local]"),
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``loki skills list`` and the agent's ``/skill-name`` dispatch but
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'discord_skill_commands_by_category': ('loki_cli.commands_platforms', 'discord_skill_commands_by_category'),
    'slack_app_manifest': ('loki_cli.commands_platforms', 'slack_app_manifest'),
    'slack_native_slashes': ('loki_cli.commands_platforms', 'slack_native_slashes'),
    'slack_subcommand_map': ('loki_cli.commands_platforms', 'slack_subcommand_map'),
    'telegram_bot_commands': ('loki_cli.commands_platforms', 'telegram_bot_commands'),
    'telegram_menu_commands': ('loki_cli.commands_platforms', 'telegram_menu_commands'),
    'telegram_menu_max_commands': ('loki_cli.commands_platforms', 'telegram_menu_max_commands'),
}
