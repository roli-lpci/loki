"""WunderCorp Portal OAuth: device-code login, refresh, shared-store mirroring, JWT selection, status.

Split out of ``loki_cli/auth.py``; origin helpers are imported lazily inside each function
so ``loki_cli.auth.<name>`` patches still intercept (and no import cycle).
"""
...
from loki_cli.auth_codex import _pool_entries
from loki_cli.auth_constants import (
    _decode_jwt_claims, AUTH_LOCK_TIMEOUT_SECONDS, AuthError, DEFAULT_WUNDERCORP_CLIENT_ID,
    DEFAULT_WUNDERCORP_INFERENCE_URL, DEFAULT_WUNDERCORP_PORTAL_URL, DEFAULT_WUNDERCORP_SCOPE, DEFAULT_WUNDERCORP_WELCOME_URL,
    DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS, WUNDERCORP_AUTH_PATH_INVOKE_JWT, WUNDERCORP_BILLING_MANAGE_SCOPE,
    WUNDERCORP_DEVICE_CODE_SOURCE, WUNDERCORP_INFERENCE_INVOKE_SCOPE, WUNDERCORP_INVOKE_JWT_MIN_TTL_SECONDS,
    _wundercorp_err, httpx)

if TYPE_CHECKING:  # annotation-only; the runtime import would be a cycle
    from loki_cli.auth import ProviderConfig

# Log-record parity with the origin module (caplog tests pin "loki_cli.auth").
logger = logging.getLogger("loki_cli.auth")

_UNUSABLE_JWT_RELOGIN = "Re-authenticate with: loki auth add wundercorp"
...
def _unusable_invoke_jwt_error(reason: str, *, no_refresh_token: bool = False) -> AuthError:
    ...
    return _wundercorp_err(
        f"WunderCorp Portal access token is not a usable inference JWT ({reason}){detail}. "
...
def _oauth_trace(event: str, *, sequence_id: Optional[str] = None, **fields: Any) -> None:
    if os.getenv("LOKI_OAUTH_TRACE", "").strip().lower() not in {"1", "true", "yes", "on"}:
        return
...
# WunderCorp agent-key slots; a fresh login persists them as None, quarantine strips them.
_WUNDERCORP_EMPTY_AGENT_KEY_FIELDS: Dict[str, Any] = {
...
_WUNDERCORP_STALE_PORTAL_HOSTS: FrozenSet[str] = frozenset({"api.wundercorp.com"})
...
def _portal_entitlement_message(capability: str) -> str:
    ...
    from loki_cli.wundercorp_account import (
        format_wundercorp_portal_entitlement_message, get_wundercorp_portal_account_info)
    account_info = get_wundercorp_portal_account_info(force_fresh=True)
    return format_wundercorp_portal_entitlement_message(account_info, capability=capability) or ""


def _format_wundercorp_entitlement_auth_error(error: AuthError) -> str:
    with suppress(Exception):
        if message := _portal_entitlement_message("WunderCorp model access"):
            return message
    return f"{error} Check credits or billing in WunderCorp Portal, then retry."


def _migrate_stale_wundercorp_portal_url(providers: Dict[str, Any]) -> None:
    wundercorp = providers.get("wundercorp")
    if not isinstance(wundercorp, dict):
        return
    stored = (wundercorp.get("portal_base_url") or "").strip()
    if stored and urlparse(stored).hostname in _WUNDERCORP_STALE_PORTAL_HOSTS:
        logger.warning(
            "auth: migrating stale wundercorp portal_base_url %s -> %s", stored, DEFAULT_WUNDERCORP_PORTAL_URL)
        wundercorp["portal_base_url"] = DEFAULT_WUNDERCORP_PORTAL_URL


# Allowlist of hosts the WunderCorp Portal proxy will forward inference JWTs to — a bearer sent anywhere
# else would leak. Consulted only for URLs from the NETWORK side (Portal refresh responses);
# the WUNDERCORP_INFERENCE_BASE_URL env override bypasses it (documented dev/staging escape hatch, the
# user set it themselves).
_ALLOWED_WUNDERCORP_INFERENCE_HOSTS: FrozenSet[str] = frozenset({
    "inference-api.wundercorp.com",
    # Free-tier (anonymous) host: serves the single ``wundercorp/welcome`` model.
    "welcome-api.wundercorp.com"})


def _validate_wundercorp_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    if parsed.scheme != "https":
        logger.warning(
            "wundercorp: refusing non-https inference URL scheme %r from Portal response", parsed.scheme)
        return None
    if parsed.hostname not in _ALLOWED_WUNDERCORP_INFERENCE_HOSTS:
        logger.warning(
            "wundercorp: refusing inference URL host %r from Portal response "
...
def _wundercorp_inference_env_override() -> Optional[str]:
    """User-set ``WUNDERCORP_INFERENCE_BASE_URL`` override (trailing slash stripped) or None.
    ...
    """
    from loki_cli.auth import _optional_base_url
    return _optional_base_url(os.getenv("WUNDERCORP_INFERENCE_BASE_URL"))


def _wundercorp_portal_env_override() -> Optional[str]:
    """``LOKI_PORTAL_BASE_URL`` / ``WUNDERCORP_PORTAL_BASE_URL`` override or None.

    Documented dev/staging escape hatch (e.g. hosted agents on the staging Portal). Trusted env
    source: must NOT be gated by ``_WUNDERCORP_PORTAL_ALLOWED_HOSTS``, which rejects untrusted
    NETWORK-provided values persisted to auth.json, not operator config.
    """
    from loki_cli.auth import _optional_base_url
    return _optional_base_url(
        os.getenv("LOKI_PORTAL_BASE_URL") or os.getenv("WUNDERCORP_PORTAL_BASE_URL"))
...
def _wundercorp_invoke_jwt_is_usable(
    token: Any, *, scope: Any = None, expires_at: Any = None,
    min_ttl_seconds: int = WUNDERCORP_INVOKE_JWT_MIN_TTL_SECONDS) -> bool:
    from loki_cli.auth import _wundercorp_invoke_jwt_status
    return _wundercorp_invoke_jwt_status(
...
def _state_invoke_jwt_status(state: Dict[str, Any], token: Any) -> Optional[str]:
    """``_wundercorp_invoke_jwt_status`` for *token* using *state*'s scope / expires_at (patchable)."""
    from loki_cli.auth import _wundercorp_invoke_jwt_status
    return _wundercorp_invoke_jwt_status(
        token, scope=state.get("scope"), expires_at=state.get("expires_at"))


def _assert_wundercorp_inference_jwt_usable(state: Dict[str, Any], *, access_token: Any = None) -> None:
...
def _remaining_ttl(expires_at: Any, fallback_expires_in: Any) -> int:
    ...
    from loki_cli.auth import _coerce_ttl_seconds, _parse_iso_timestamp
...
def _wundercorp_jwt_expires_at(token: Any, fallback_expires_at: Any = None) -> Optional[str]:
    claims = _decode_jwt_claims(token)
    exp = claims.get("exp")
    if isinstance(exp, (int, float)):
        with suppress(Exception):
...
def _set_wundercorp_agent_key_from_invoke_jwt(
    state: Dict[str, Any], *, obtained_at: Optional[str] = None) -> None:
    from loki_cli.auth import _nonempty_str
    ...
    expires_at = _wundercorp_jwt_expires_at(access_token, state.get("expires_at"))
...
def _select_wundercorp_invoke_jwt(
    state: Dict[str, Any], *, access_token: Any = None, sequence_id: Optional[str] = None) -> None:
    from loki_cli.auth import _nonempty_str
    if _nonempty_str(access_token):
        state["access_token"] = access_token
    _set_wundercorp_agent_key_from_invoke_jwt(state)
    logger.debug("WunderCorp inference auth: using NAS invoke JWT")
    _oauth_trace(
        "wundercorp_invoke_jwt_selected", sequence_id=sequence_id,
...
_WUNDERCORP_EFFECTIVE_STATE_IGNORED_KEYS = frozenset({"expires_in", "agent_key_expires_in"})


def _wundercorp_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {k: v for k, v in state.items() if k not in _WUNDERCORP_EFFECTIVE_STATE_IGNORED_KEYS}


WUNDERCORP_SHARED_STORE_FILENAME = "wundercorp_auth.json"
_wundercorp_shared_lock_holder = threading.local()


def _wundercorp_shared_auth_dir() -> Path:
    """Directory of the shared WunderCorp token store: ``LOKI_SHARED_AUTH_DIR`` or ``<root>/shared/``.

    Outside any named profile so all profiles share it (``loki --profile X auth add wundercorp --type
    ...
    """
    override = os.getenv("LOKI_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from loki_constants import get_default_loki_root
    return get_default_loki_root() / "shared"


def _wundercorp_shared_store_path() -> Path:
    path = _wundercorp_shared_auth_dir() / WUNDERCORP_SHARED_STORE_FILENAME
    # Seat belt (mirrors the _auth_file_path() guard): under pytest, refuse a path under the real
    # user's Loki root so a test that forgot LOKI_SHARED_AUTH_DIR fails loudly instead of
    # corrupting cross-profile state.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from loki_constants import get_default_loki_root
        real_home_shared = (
            get_default_loki_root() / "shared" / WUNDERCORP_SHARED_STORE_FILENAME).resolve(strict=False)
        try:
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared WunderCorp auth store during test run: "
                f"{path}. Set LOKI_SHARED_AUTH_DIR to a tmp_path in your test fixture.")
    return path


@contextmanager
def _wundercorp_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    """Cross-profile lock for the shared WunderCorp OAuth store.
    ...
    """
    from loki_cli.auth import _file_lock
    try:
        lock_path = _wundercorp_shared_store_path().with_suffix(".lock")
    except RuntimeError:
        yield  # No LOKI_HOME yet (pre-setup): fall through without locking.
        return
    with _file_lock(
        lock_path, _wundercorp_shared_lock_holder, timeout_seconds,
        "Timed out waiting for shared WunderCorp auth lock"):
        yield
...
# OAuth fields mirrored between a profile's WunderCorp state and the shared cross-profile store.
_WUNDERCORP_SHARED_STATE_KEYS = (
...
def _merge_shared_wundercorp_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local WunderCorp state."""
    from loki_cli.auth import _nonempty_str, _parse_iso_timestamp, _read_shared_wundercorp_state
    shared = _read_shared_wundercorp_state() or {}
    ...
    if not refresh_changed and not shared_access_exp > local_access_exp:
        return False
    for key in _WUNDERCORP_SHARED_STATE_KEYS:
...
def _wundercorp_shared_shape(src: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "scope": src.get("scope") or DEFAULT_WUNDERCORP_SCOPE,
        "client_id": src.get("client_id") or DEFAULT_WUNDERCORP_CLIENT_ID,
        "portal_base_url": src.get("portal_base_url") or DEFAULT_WUNDERCORP_PORTAL_URL,
        ...
        "inference_base_url": src.get("inference_base_url") or (
            DEFAULT_WUNDERCORP_WELCOME_URL if src.get("auth_method") == "anonymous" else DEFAULT_WUNDERCORP_INFERENCE_URL),
...
def _write_shared_wundercorp_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the WunderCorp OAuth state to the shared store.

    Best-effort: failures are logged and swallowed; per-profile auth.json stays the source of truth.
    """
    from loki_cli.auth import _nonempty_str, _write_private_file_atomic
    ...
    if not is_guest and not (_nonempty_str(refresh_token) and _nonempty_str(state.get("access_token"))):
        return
    shared = {
        "_schema": 1, **_wundercorp_shared_shape(state),
        "updated_at": datetime.now(timezone.utc).isoformat()}
    try:
        with _wundercorp_shared_store_lock():
            path = _wundercorp_shared_store_path()
            _write_private_file_atomic(
                path, json.dumps(shared, indent=2, sort_keys=True), replace=os.replace)
        _oauth_trace(
            "wundercorp_shared_store_written", path=str(path),
            refresh_token_fp=_token_fingerprint(refresh_token))
    except Exception as exc:
        logger.debug("Failed to write shared WunderCorp auth store: %s", exc)


def _read_shared_wundercorp_state() -> Optional[Dict[str, Any]]:
    """Shared WunderCorp OAuth state when present and well-formed, else None.
    ...
    try:
        ...
        logger.debug("Shared WunderCorp auth store at %s is unreadable: %s", path, exc)
        return None
...
def _clear_shared_wundercorp_state(reason: str) -> None:
    """Remove the shared WunderCorp OAuth store after a terminal token failure."""
    try:
        with _wundercorp_shared_store_lock():
            _wundercorp_shared_store_path().unlink(missing_ok=True)
        _oauth_trace("wundercorp_shared_store_cleared", reason=reason)
    except Exception as exc:
        logger.debug("Failed to clear shared WunderCorp auth store: %s", exc)
...
def _quarantine_forensics(state: Dict[str, Any], error: AuthError, reason: str) -> Dict[str, Any]:
    ...
    provenance is client_id + agent_key_id (WunderCorp state has no session_id).
    """
    from loki_cli.auth import _auth_file_path
...
def _quarantine_wundercorp_oauth_state(state: Dict[str, Any], error: AuthError, *, reason: str) -> None:
    ...
    from loki_cli.auth import (
        _FLAT_OAUTH_TOKEN_KEYS, _last_auth_error_marker, invalidate_wundercorp_auth_status_cache)
    ...
    logger.warning(
        "WunderCorp OAuth state quarantined (terminal auth death): %s",
        json.dumps(_quarantine_forensics(state, error, reason), sort_keys=True, ensure_ascii=False))
    for key in (*_FLAT_OAUTH_TOKEN_KEYS, *_WUNDERCORP_EMPTY_AGENT_KEY_FIELDS):
        state.pop(key, None)
    state["last_auth_error"] = _last_auth_error_marker("wundercorp", error, reason=reason)
    _clear_shared_wundercorp_state(reason)
    invalidate_wundercorp_auth_status_cache()


def _quarantine_wundercorp_pool_entries(
    auth_store: Dict[str, Any], error: AuthError, *, reason: str) -> bool:
    """Remove singleton-seeded WunderCorp pool entries that contain dead OAuth state."""
    entries = _pool_entries(auth_store, "wundercorp")
    if entries is None:
        return False
    singleton_sources = {WUNDERCORP_DEVICE_CODE_SOURCE, f"manual:{WUNDERCORP_DEVICE_CODE_SOURCE}"}
    retained = [
        e for e in entries if not (isinstance(e, dict) and e.get("source") in singleton_sources)]
    removed = len(retained) != len(entries)
    if removed:
        auth_store["credential_pool"]["wundercorp"] = retained
        _oauth_trace("wundercorp_pool_device_code_quarantined", reason=reason, error_code=error.code)
    return removed


def _try_import_shared_wundercorp_state(*, timeout_seconds: float = 15.0) -> Optional[Dict[str, Any]]:
    """Rehydrate WunderCorp OAuth state from the shared store via a forced refresh.

    Returns auth_state ready for ``persist_wundercorp_credentials()``; None on any failure (expired
    token, portal unreachable) so the caller falls through to device-code.
    """
    from loki_cli.auth import (
        _read_shared_wundercorp_state, _write_shared_wundercorp_state, refresh_wundercorp_oauth_from_state,
        _is_terminal_wundercorp_refresh_error)
    try:
        with _wundercorp_shared_store_lock(timeout_seconds=_shared_lock_timeout(timeout_seconds)):
            shared = _read_shared_wundercorp_state()
            if not shared:
                return None
            # Full state dict so refresh_wundercorp_oauth_from_state has every field it needs.
            state: Dict[str, Any] = {
                **_wundercorp_shared_shape(shared), "agent_key": None, "agent_key_expires_at": None,
                "tls": {"insecure": False, "ca_bundle": None}}
            refreshed = refresh_wundercorp_oauth_from_state(
                state, timeout_seconds=timeout_seconds, force_refresh=True,
                on_state_update=lambda updated, _reason: _write_shared_wundercorp_state(updated))
            _write_shared_wundercorp_state(refreshed)
    except Exception as exc:
        is_auth = isinstance(exc, AuthError)
        _oauth_trace(
            "wundercorp_shared_import_failed", error_type=type(exc).__name__,
            **({"error_code": getattr(exc, "code", None)} if is_auth else {}))
        if is_auth and _is_terminal_wundercorp_refresh_error(exc):
            _clear_shared_wundercorp_state("shared_import_terminal_refresh_failure")
        logger.debug("Shared WunderCorp import failed: %s", exc)
        return None
    return refreshed
...
def _refresh_access_token(
    *, client: httpx.Client, portal_base_url: str, client_id: str, refresh_token: str,
    ...
    response = client.post(
        ...
        headers={"x-wundercorp-refresh-token": refresh_token},
        data={"grant_type": "refresh_token", "client_id": client_id})
    if response.status_code == 200:
        payload = response.json()
        if "access_token" not in payload:
            raise _wundercorp_err("Refresh response missing access_token", "invalid_token", relogin=True)
        return payload
    try:
        error_payload = response.json()
    except Exception as exc:
        raise _wundercorp_err("Refresh token exchange failed", relogin=True) from exc
    ...
    # self-heal hook) redeemed Loki's refresh_token without persisting the rotated token, so the
    ...
    if code == "refresh_token_reused" or "reuse" in description.lower():
        description = (
            "WunderCorp Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another Loki install sharing "
            "~/.loki/auth.json) called POST /api/oauth/token with Loki's "
            "refresh token without persisting the rotated token back.\n"
            "WunderCorp refresh tokens are single-use — only Loki may call the "
            "refresh endpoint. For health checks, use `loki auth status` "
            "instead.\n"
            "Re-authenticate with: loki auth add wundercorp")
        relogin = True
    raise _wundercorp_err(description, code, relogin=relogin)


def _refresh_wundercorp_or_quarantine(
    *, client: httpx.Client, auth_store: Dict[str, Any], state: Dict[str, Any],
    ...
    from loki_cli.auth import _refresh_access_token, _is_terminal_wundercorp_refresh_error
    try:
        return _refresh_access_token(
            client=client, portal_base_url=portal_base_url, client_id=client_id,
            refresh_token=refresh_token)
    except AuthError as exc:
        if _is_terminal_wundercorp_refresh_error(exc):
            _quarantine_wundercorp_oauth_state(state, exc, reason=reason)
            _quarantine_wundercorp_pool_entries(auth_store, exc, reason=reason)
            persist()
        raise


def _apply_wundercorp_refreshed_tokens(
    state: Dict[str, Any], refreshed: Dict[str, Any], refresh_token: str, *,
    inference_base_url: Optional[str] = None) -> None:
    """Write a successful WunderCorp token-refresh payload into *state* (tokens + expiry fields).
    ...
    *inference_base_url*, when given, is the healed network-provenance URL to persist alongside
    ...
    """
    from loki_cli.auth import _coerce_ttl_seconds
...
def _healed_wundercorp_inference_url(refreshed: Dict[str, Any]) -> str:
    ...
    """
    url = _validate_wundercorp_inference_url_from_network(refreshed.get("inference_base_url"))
    return url or DEFAULT_WUNDERCORP_INFERENCE_URL


def _wundercorp_http_client(timeout_seconds: float, verify: Any) -> httpx.Client:
    return httpx.Client(
...
def fetch_wundercorp_models(
    *, inference_base_url: str, api_key: str, timeout_seconds: float = 15.0,
    verify: bool | str = True) -> List[str]:
    """Fetch available model IDs from the WunderCorp inference API."""
    from loki_cli.auth import _nonempty_str
    with _wundercorp_http_client(timeout_seconds, verify) as client:
        response = client.get(
    ...
    if response.status_code != 200:
        ...
        raise _wundercorp_err(description, "models_fetch_failed")
    ...
    for item in data:
        ...
        # Loki models aren't reliable for agentic tool-calling
        if _nonempty_str(model_id) and "loki" not in model_id.lower():
...
def refresh_wundercorp_oauth_pure(
    ...
    """Refresh WunderCorp OAuth state without mutating auth.json directly.
    ...
    """
    return refresh_wundercorp_oauth_from_state(
        {
...
def refresh_wundercorp_oauth_from_state(
    src: Dict[str, Any], *, timeout_seconds: float = 15.0, force_refresh: bool = False,
    on_state_update: Optional[Callable[[Dict[str, Any], str], None]] = None) -> Dict[str, Any]:
    """Refresh WunderCorp OAuth from a state dict (defaults filled in) without mutating auth.json."""
    from loki_cli.auth import (
        _assert_wundercorp_inference_jwt_usable, _refresh_access_token, _resolve_verify,
        _select_wundercorp_invoke_jwt)
    tls = src.get("tls") or {}
    insecure, ca_bundle = tls.get("insecure"), tls.get("ca_bundle")
    state: Dict[str, Any] = {
        "access_token": src.get("access_token", ""), "refresh_token": src.get("refresh_token", ""),
        "client_id": src.get("client_id") or DEFAULT_WUNDERCORP_CLIENT_ID,
        "portal_base_url": (src.get("portal_base_url") or DEFAULT_WUNDERCORP_PORTAL_URL).rstrip("/"),
        "inference_base_url": (
            src.get("inference_base_url") or DEFAULT_WUNDERCORP_INFERENCE_URL).rstrip("/"),
        "token_type": src.get("token_type") or "Bearer",
        "scope": src.get("scope") or DEFAULT_WUNDERCORP_SCOPE,
        "obtained_at": src.get("obtained_at"), "expires_at": src.get("expires_at"),
    ...
    with _wundercorp_http_client(timeout_seconds or 15.0, verify) as client:
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise _wundercorp_err("No refresh token is available for WunderCorp Portal.", relogin=True)
            refreshed = _refresh_access_token(
                client=client, portal_base_url=state["portal_base_url"],
                client_id=state["client_id"], refresh_token=refresh_token_value)
            _apply_wundercorp_refreshed_tokens(
                state, refreshed, refresh_token_value,
                inference_base_url=_healed_wundercorp_inference_url(refreshed))
            if on_state_update is not None:
                on_state_update(dict(state), "post_refresh_access_token")
        _assert_wundercorp_inference_jwt_usable(state)
        _select_wundercorp_invoke_jwt(state)
    return state


def persist_wundercorp_credentials(creds: Dict[str, Any], *, label: Optional[str] = None):
    """Persist WunderCorp OAuth credentials as the singleton provider state.

    WunderCorp credentials are read from ``providers.wundercorp`` (401 recovery, pool seeding) AND
    ``credential_pool.wundercorp`` (runtime ``pool.select()``); a pool-only write broke expiry recovery.
    So: write the singleton, mirror to the shared store, then ``load_pool("wundercorp")`` upserts the
    canonical ``device_code`` entry in place. ``label`` rides in the singleton so re-seeding keeps
    it.
    """
    from loki_cli.auth import _save_active_provider_state, _write_shared_wundercorp_state
    from agent.credential_pool import load_pool
    state = dict(creds)
    if label and str(label).strip():
        state["label"] = str(label).strip()
    _save_active_provider_state("wundercorp", state)
    _write_shared_wundercorp_state(state)
    pool = load_pool("wundercorp")
    return next((e for e in pool.entries() if e.source == WUNDERCORP_DEVICE_CODE_SOURCE), None)


def _sync_wundercorp_pool_from_auth_store() -> None:
    """Best-effort pool reseed after providers.wundercorp changes; never fail login."""
    try:
        from agent.credential_pool import load_pool
        load_pool("wundercorp")
    except Exception as exc:
        logger.debug("Failed to sync WunderCorp credential pool from auth store: %s", exc)


def _wundercorp_effective_routing(state: Dict[str, Any]) -> tuple[str, str, str, str]:
    ...
    layers the runtime-only ``WUNDERCORP_INFERENCE_BASE_URL`` override on top and is never persisted.
    """
    from loki_cli.auth import _WUNDERCORP_PORTAL_ALLOWED_HOSTS, _optional_base_url
    portal_url = (
        _optional_base_url(state.get("portal_base_url")) or os.getenv("LOKI_PORTAL_BASE_URL")
        or os.getenv("WUNDERCORP_PORTAL_BASE_URL") or DEFAULT_WUNDERCORP_PORTAL_URL).rstrip("/")
    ...
    env_portal_override = _wundercorp_portal_env_override()
    if env_portal_override:
        ...
        if not portal_host or portal_host not in _WUNDERCORP_PORTAL_ALLOWED_HOSTS or not trusted_scheme:
            logger.warning(
                "auth: ignoring invalid portal_base_url %r "
                "(host %r or scheme not allowed), using default",
                portal_url, portal_host)
            portal_url = DEFAULT_WUNDERCORP_PORTAL_URL
    ...
    stored_inference_url = (
        _validate_wundercorp_inference_url_from_network(
            _optional_base_url(state.get("inference_base_url")))
        or (DEFAULT_WUNDERCORP_WELCOME_URL if is_guest_state(state) else DEFAULT_WUNDERCORP_INFERENCE_URL))
    return (
        portal_url, stored_inference_url, _wundercorp_inference_env_override() or stored_inference_url,
        str(state.get("client_id") or DEFAULT_WUNDERCORP_CLIENT_ID))


class _WunderCorpRuntimeResolve:
    """Working set for one ``resolve_wundercorp_runtime_credentials`` call.
    ...
    def _reload_routing(self) -> None:
        (self.portal_base_url, self.stored_inference_base_url, self.inference_base_url,
         self.client_id) = _wundercorp_effective_routing(self.state)

    def persist(self, reason: str) -> None:
        from loki_cli.auth import _save_provider_state_to_source, _write_shared_wundercorp_state
        state = self.state
        persisted = _wundercorp_effective_provider_state(self._persisted_state)
        if _wundercorp_effective_provider_state(state) == persisted:
            _oauth_trace("wundercorp_state_persist_skipped", sequence_id=self.sequence_id, reason=reason)
            return
        try:
            _save_provider_state_to_source(self.auth_store, "wundercorp", state, self._source_path)
        except Exception as exc:
            _oauth_trace(
                "wundercorp_state_persist_failed", sequence_id=self.sequence_id, reason=reason,
                error_type=type(exc).__name__)
            raise
        _oauth_trace(
            "wundercorp_state_persisted", sequence_id=self.sequence_id, reason=reason,
            refresh_token_fp=_token_fingerprint(state.get("refresh_token")),
            access_token_fp=_token_fingerprint(state.get("access_token")))
        self._persisted_state = dict(state)
    ...
    def shared_lock(self):
        return _wundercorp_shared_store_lock(timeout_seconds=_shared_lock_timeout(self.timeout_seconds))
    ...
    def merge_shared(self) -> bool:
        ...
        if not _merge_shared_wundercorp_oauth_state(self.state):
            return False
    ...
    def refresh(self, client: httpx.Client, invoke_jwt_status: Optional[str]) -> None:
        ...
        refreshed = _refresh_wundercorp_or_quarantine(
        ...
        # WUNDERCORP_INFERENCE_BASE_URL env override is layered on for the client/return value only.
        self.stored_inference_base_url = _healed_wundercorp_inference_url(refreshed)
        self.inference_base_url = _wundercorp_inference_env_override() or self.stored_inference_base_url
        _apply_wundercorp_refreshed_tokens(
    ...
    def ensure_usable_access_token(self, client: httpx.Client) -> None:
        ...
        from loki_cli.anon_auth import is_guest_state, refresh_guest_state
        ...
        if not self.has_access_token():
            raise _wundercorp_err("No access token found for WunderCorp Portal login.", relogin=True)
...
def resolve_wundercorp_runtime_credentials(
    *, timeout_seconds: float = 15.0, insecure: Optional[bool] = None,
    ...
    """Resolve WunderCorp inference credentials for runtime use (refreshing under the auth-store lock).
    ...
    """
    from loki_cli.anon_auth import AnonCredentialDead, clear_dead_guest, ensure_portal_identity
    try:
        return _resolve_wundercorp_runtime_credentials(
        ...
        from loki_cli.auth import get_provider_auth_state
        dead = get_provider_auth_state("wundercorp") or {}
...
def _resolve_wundercorp_runtime_credentials(
    *, timeout_seconds: float = 15.0, insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None, force_refresh: bool = False,
    stale_access_token: Optional[str] = None) -> Dict[str, Any]:
    """Resolve WunderCorp inference credentials for runtime use (refreshing under the auth-store lock).
    ...
    """
    from loki_cli.auth import (
        _assert_wundercorp_inference_jwt_usable, _auth_file_path, _provider_state_transaction,
        _resolve_verify, _select_wundercorp_invoke_jwt, _sync_wundercorp_pool_from_auth_store,
        _tls_state_from_verify)
    with _provider_state_transaction("wundercorp") as (auth_store, state, state_source_path):
        if not state:
            raise _wundercorp_err("Loki is not logged into WunderCorp Portal.", relogin=True)
        run = _WunderCorpRuntimeResolve(
        ...
        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)
        _oauth_trace(
            "wundercorp_runtime_credentials_start", sequence_id=run.sequence_id,
            refresh_token_fp=_token_fingerprint(state.get("refresh_token")))
        with _wundercorp_http_client(timeout_seconds or 15.0, verify) as client:
            run.ensure_usable_access_token(client)
            _assert_wundercorp_inference_jwt_usable(state, access_token=run.access_token)
            _select_wundercorp_invoke_jwt(
        ...
        run.persist("resolve_wundercorp_runtime_credentials_final")
    if run.persisted_any:
        _sync_wundercorp_pool_from_auth_store()
    api_key = state.get("agent_key")
    if not isinstance(api_key, str) or not api_key:
        raise _wundercorp_err("Failed to resolve a WunderCorp inference API key", "server_error")
    expires_at = state.get("agent_key_expires_at")
    return {
        "provider": "wundercorp", "base_url": run.inference_base_url, "api_key": api_key,
        "key_id": state.get("agent_key_id"), "expires_at": expires_at,
        "expires_in": _remaining_ttl(expires_at, state.get("agent_key_expires_in")),
        "source": WUNDERCORP_AUTH_PATH_INVOKE_JWT,
        ...
        "auth_path": WUNDERCORP_AUTH_PATH_INVOKE_JWT,
        "state_path": str(state_source_path or _auth_file_path())}


def _empty_wundercorp_auth_status() -> Dict[str, Any]:
    return {
...
def _snapshot_wundercorp_pool_status() -> Dict[str, Any]:
    ...
    ``resolve_wundercorp_runtime_credentials()`` refreshes.
    """
    from loki_cli.auth import _parse_iso_timestamp
    try:
        from agent.credential_pool import load_pool
        pool = load_pool("wundercorp")
        entries = list(pool.entries()) if pool and pool.has_credentials() else []
        if not entries:
            return _empty_wundercorp_auth_status()
        ...
        return {
            ...
            "portal_base_url": (
                (attr("portal_base_url") or DEFAULT_WUNDERCORP_PORTAL_URL) if is_portal_oauth else None),
            "inference_base_url": (
...
def _wundercorp_status_from_state(
    state: Dict[str, Any], *, logged_in: bool, source: str) -> Dict[str, Any]:
    """Auth-store-backed WunderCorp status snapshot (shared by the live and refresh-free variants)."""
    from loki_cli.anon_auth import is_guest_state
    access_token = state.get("access_token")
...
def _compute_wundercorp_auth_status() -> Dict[str, Any]:
    """Uncached implementation of get_wundercorp_auth_status(). See that function."""
    from loki_cli.auth import get_provider_auth_state, resolve_wundercorp_runtime_credentials
    state = get_provider_auth_state("wundercorp")
    if not state:
        return _snapshot_wundercorp_pool_status()
    base_status = _wundercorp_status_from_state(
        state, logged_in=bool(state.get("access_token")), source="auth_store")
    try:
        creds = resolve_wundercorp_runtime_credentials()
        refreshed_state = get_provider_auth_state("wundercorp") or state
        base_status.update({
            "logged_in": True,
            "portal_base_url": (
...
def get_wundercorp_auth_status_local() -> Dict[str, Any]:
    """Refresh-free WunderCorp auth snapshot for read-only display surfaces.

    NEVER calls ``resolve_wundercorp_runtime_credentials()`` (no refresh POST / single-use token spent);
    ...
    status = _wundercorp_status_from_state(state, logged_in=logged_in, source="auth_store_local")
...
# Enum values reported on the dashboard /api/status as ``wundercorp_session_valid``. NAS's health sweep
...
WUNDERCORP_SESSION_VALID = "valid"
WUNDERCORP_SESSION_TERMINAL = "terminal"
WUNDERCORP_SESSION_UNKNOWN = "unknown"


def get_wundercorp_session_validity() -> str:
    """Classify the WunderCorp bootstrap session for the dashboard /api/status probe.
    ...
    try:
        ...
        state = None
    if not state:
        return WUNDERCORP_SESSION_UNKNOWN
    ...
    if _terminal_quarantine_marker(state) is not None:
        return WUNDERCORP_SESSION_TERMINAL
    if _state_invoke_jwt_status(state, state.get("access_token")) is None:
        return WUNDERCORP_SESSION_VALID
...
def _pool_first_oauth_status(
    ...
    Pool first (where `loki auth` / `loki model` store device_code tokens), then
...
def _wundercorp_device_code_login(
    *, portal_base_url: Optional[str] = None, inference_base_url: Optional[str] = None,
    ...
    """Run the WunderCorp device-code flow and return full OAuth state without persisting."""
    from loki_cli.auth import (
        ...
        _tls_state_from_verify, format_auth_error, refresh_wundercorp_oauth_from_state)
    pconfig = PROVIDER_REGISTRY["wundercorp"]
    portal_base_url = (
        portal_base_url or os.getenv("LOKI_PORTAL_BASE_URL") or os.getenv("WUNDERCORP_PORTAL_BASE_URL")
        or pconfig.portal_base_url).rstrip("/")
    requested_inference_url = (
        inference_base_url or os.getenv("WUNDERCORP_INFERENCE_BASE_URL")
    ...
    if _is_remote_session():
        open_browser = False
    print(f"Starting Loki login via {pconfig.name}...")
    ...
    auth_state = {
        ...
        **_WUNDERCORP_EMPTY_AGENT_KEY_FIELDS}
    try:
        return refresh_wundercorp_oauth_from_state(
            auth_state, timeout_seconds=timeout_seconds, force_refresh=False)
    except AuthError as exc:
        if exc.code == "subscription_required":
            portal_url = auth_state.get("portal_base_url", DEFAULT_WUNDERCORP_PORTAL_URL).rstrip("/")
            print()
            print(format_auth_error(exc))
            print(f"  Subscribe here: {portal_url}/billing")
            print()
            print("After subscribing, run `loki model` again to finish setup.")
            raise SystemExit(1)
        raise


def _mirror_wundercorp_state_best_effort(auth_state: Dict[str, Any]) -> None:
    """Mirror to the shared store + reseed the pool, swallowing all errors (same as _login_wundercorp)."""
    from loki_cli.auth import _sync_wundercorp_pool_from_auth_store, _write_shared_wundercorp_state
    with suppress(Exception):
        _write_shared_wundercorp_state(auth_state)
    with suppress(Exception):
        _sync_wundercorp_pool_from_auth_store()


def step_up_wundercorp_billing_scope(
    *, open_browser: bool = True, timeout_seconds: float = 15.0,
    ...
    server silently downscopes and this returns False. Persists like ``_login_wundercorp`` minus the
    model picker.
    """
    from loki_cli.auth import (
        PROVIDER_REGISTRY, _wundercorp_device_code_login, _save_active_provider_state,
        get_provider_auth_state)
    prior = get_provider_auth_state("wundercorp") or {}
    pconfig = PROVIDER_REGISTRY["wundercorp"]
    ...
    requested = list(dict.fromkeys([
        *(prior_scope or [WUNDERCORP_INFERENCE_INVOKE_SCOPE, "tool:invoke"]), WUNDERCORP_BILLING_MANAGE_SCOPE]))
    auth_state = _wundercorp_device_code_login(
    ...
    _save_active_provider_state("wundercorp", auth_state)
    _mirror_wundercorp_state_best_effort(auth_state)
    granted = auth_state.get("scope")
    return isinstance(granted, str) and WUNDERCORP_BILLING_MANAGE_SCOPE in granted.split()


def _pick_wundercorp_model_after_login(
    auth_state: Dict[str, Any], inference_base_url: str) -> Optional[str]:
    """Fetch the curated WunderCorp model list (tier/policy-filtered) and run the interactive picker.
    ...
    """
    from loki_cli.auth import _prompt_model_selection
    runtime_key = auth_state.get("agent_key") or auth_state.get("access_token")
    if not isinstance(runtime_key, str) or not runtime_key:
        raise _wundercorp_err("No runtime API key available to fetch models", "invalid_token")
    from loki_cli.models import (
        get_curated_wundercorp_model_ids,
        check_wundercorp_free_tier,
        partition_wundercorp_models_by_tier,
        union_with_portal_free_recommendations,
        union_with_portal_paid_recommendations,
    )
    from loki_cli.models_pricing import (
        get_pricing_for_provider,
        wundercorp_policy_allowed_ids,
        restrict_to_wundercorp_policy,
    )
    model_ids = get_curated_wundercorp_model_ids()
    ...
    if model_ids:
        pricing = get_pricing_for_provider("wundercorp")
        # Force fresh account data so recent credit purchases are reflected immediately.
        free_tier = check_wundercorp_free_tier(force_fresh=True)
        # Narrow before the tier split, so a rescued id still has to pass the free/paid predicate.
        _policy_allowed = wundercorp_policy_allowed_ids()
        if free_tier:
            with suppress(Exception):
                unavailable_message = _portal_entitlement_message("paid WunderCorp models")
        ...
        model_ids = restrict_to_wundercorp_policy(model_ids, _policy_allowed, rescue_empty=True)
        _policy_narrowed = model_ids != _before_policy
        if free_tier:
            model_ids, unavailable_models = partition_wundercorp_models_by_tier(
                model_ids, pricing, free_tier=True)
    if model_ids:
        from loki_cli.wundercorp_account import wundercorp_policy_notice
        _policy_notice = wundercorp_policy_notice(removed=_policy_narrowed)
        if _policy_notice:
        ...
        return _prompt_model_selection(
            ...
            unavailable_message=unavailable_message, confirm_provider="wundercorp",
            confirm_base_url=inference_base_url, confirm_api_key=runtime_key)
    if unavailable_models:
        _url = (_portal or DEFAULT_WUNDERCORP_PORTAL_URL).rstrip("/")
        print("No free models currently available.")
        print(unavailable_message or f"Upgrade at {_url} to access paid models.")
    else:
        print("No curated models available for WunderCorp Portal.")
    return None


def _offer_shared_wundercorp_import(timeout_seconds: float) -> Optional[Dict[str, Any]]:
    """Codex-style auto-import: offer to rehydrate a WunderCorp credential from another profile.
    ...
    """
    from loki_cli.auth import _prompt_yes_no, _read_shared_wundercorp_state
    from loki_cli.anon_auth import is_guest_state
    shared = _read_shared_wundercorp_state()
    if not shared or is_guest_state(shared):
        ...
        # (persist_wundercorp_credentials overwrites the singleton and the shared store).
        return None
    try:
        shared_path = _wundercorp_shared_store_path()
    except RuntimeError:
        shared_path = None
    print()
    print(f"Found existing WunderCorp OAuth credentials at {shared_path}" if shared_path
          else "Found existing shared WunderCorp OAuth credentials")
    if not _prompt_yes_no("Import these credentials? [Y/n]: ", default="y"):
        return None
    print("Rehydrating WunderCorp session from shared credentials...")
    auth_state = _try_import_shared_wundercorp_state(timeout_seconds=timeout_seconds)
...
def _restore_active_provider(prior_active_provider: Any) -> None:
    """Undo the ``active_provider="wundercorp"`` that ``_save_provider_state`` wrote during login."""
...
def _login_wundercorp(args, pconfig: ProviderConfig) -> None:
    """WunderCorp Portal device authorization flow."""
    from loki_cli.auth import (
        _auth_store_lock, _load_auth_store, _wundercorp_device_code_login, _save_active_provider_state,
        _save_model_choice, _sync_wundercorp_pool_from_auth_store, _update_config_for_provider,
        _write_shared_wundercorp_state, format_auth_error)
    timeout_seconds = getattr(args, "timeout", None) or 15.0
    ca_bundle = (
        getattr(args, "ca_bundle", None) or os.getenv("LOKI_CA_BUNDLE")
        or os.getenv("SSL_CERT_FILE"))
    try:
        auth_state = _offer_shared_wundercorp_import(timeout_seconds)
        if auth_state is None:
            auth_state = _wundercorp_device_code_login(
        ...
        # Snapshot BEFORE _save_provider_state overwrites active_provider to "wundercorp", so a
        ...
        saved_to = _save_active_provider_state("wundercorp", auth_state)
        ...
        selected_model = None
        try:
            selected_model = _pick_wundercorp_model_after_login(auth_state, inference_base_url)
        except Exception as exc:
        ...
        # switch to WunderCorp with a mismatched model; the WunderCorp tokens stay saved for future use.
        if not selected_model:
            _restore_active_provider(prior_active_provider)
            print()
            print("No provider change. WunderCorp credentials saved for future use.")
            print("  Run `loki model` again to switch to WunderCorp Portal.")
            return
        config_path = _update_config_for_provider(
            "wundercorp", inference_base_url, default_model=selected_model)
        _save_model_choice(selected_model)
        print(f"Default model set to: {selected_model}")
        print(f"  Config updated: {config_path} (model.provider=wundercorp)")
