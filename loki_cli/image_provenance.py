"""Image-authored deployment provenance for immutable Loki runtimes.

The image bakes ``/etc/loki/image-provenance.json`` outside ``$LOKI_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/loki/image-provenance.json")
