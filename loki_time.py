"""Timezone-aware clock for Loki.

``now()`` returns a tz-aware datetime in the user's configured IANA timezone. Resolution order:
``LOKI_TIMEZONE`` env var, then ``timezone`` in ``~/.loki/config.yaml``, else server-local
...
# LOKI_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _timezone_cache_identity() -> Tuple[str, str]:
    tz_env = os.getenv("LOKI_TIMEZONE", "").strip()
