"""Session-scoped context variables for the Loki gateway.

Replaces the old ``os.environ``-based ``LOKI_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "LOKI_SESSION_PLATFORM", "LOKI_SESSION_SOURCE", "LOKI_SESSION_CHAT_ID",
    "LOKI_SESSION_CHAT_TYPE", "LOKI_SESSION_CHAT_NAME", "LOKI_SESSION_THREAD_ID",
    "LOKI_SESSION_USER_ID", "LOKI_SESSION_USER_ID_ALT", "LOKI_SESSION_USER_NAME",
    "LOKI_SESSION_SCOPE_ID", "LOKI_SESSION_KEY", "LOKI_SESSION_ID",
    "LOKI_UI_SESSION_ID", "LOKI_SESSION_MESSAGE_ID", "LOKI_SESSION_PROFILE",
    "LOKI_BROWSER_CONTROL_PRINCIPAL", "LOKI_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "LOKI_CRON_SESSION", "LOKI_SESSION_PARENT_CHAT_ID",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("LOKI_SESSION_ASYNC_DELIVERY", default=_UNSET)
...
_SESSION_HISTORY_DELIVERY = ContextVar("LOKI_SESSION_HISTORY_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("LOKI_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("LOKI_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("LOKI_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``LOKI_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``LOKI_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds LOKI_SESSION_PLATFORM, CLI/TUI/
# desktop bind LOKI_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``LOKI_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("LOKI_PLATFORM") or get_session_env("LOKI_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("LOKI_SESSION_SOURCE", ""))
...
def declare_stateless_channel() -> None:
    ...
    See wundercorp/loki#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    (``LOKI_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
