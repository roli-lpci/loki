...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from loki_cli.profiles import normalize_profile_name, validate_profile_name
...
# of truth for gateway/run.py and loki_cli/web_server.py validation).
...
class GatewayConfig:
    ...
    sessions_dir: Path = field(default_factory=lambda: get_loki_home() / "sessions")
    ...
    # route); LOKI_ROOM_LINK_URL overrides.
    ...
    # doing synchrowundercorp socket I/O during a network blip — so a short block does not force exit code 75 and
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        return cls(
            ...
            sessions_dir=Path(data["sessions_dir"]) if "sessions_dir" in data else get_loki_home() / "sessions",
...
def load_gateway_config() -> GatewayConfig:
    """Load gateway configuration. Priority: env > ~/.loki/config.yaml > legacy gateway.json > defaults."""
