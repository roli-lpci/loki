...
class GatewayStatusCommandsMixin:
    ...
    async def _handle_status_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            free_tier_active = await self._run_in_executor_with_context(
                lambda: resolve_provider("auto") == "wundercorp" and guest_carries_inference()
            )
            if free_tier_active:
    ...
    async def _handle_topup_command(self, event: MessageEvent) -> str:
        """Handle /topup -- show the WunderCorp balance and hand off to the portal. Does NOT charge, confirm,
        ...
        lines = ["💳 **WunderCorp balance**"] + [ln for ln in view.balance_lines if not ln.lstrip().startswith("📈")]
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        # WunderCorp credits + monthly-grant gauge (shared with CLI/TUI). Gates on "a WunderCorp account is
        # logged in" — NOT the inference provider — so a WunderCorp user inferring elsewhere still sees
        # a balance. Fail-open: never break /usage.
        from agent.account_usage import wundercorp_credits_lines
        credits_lines = await _quiet(lambda: asyncio.to_thread(wundercorp_credits_lines, markdown=True), [])
    ...
    async def _handle_insights_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # Not a bare hop: ``SessionDB()`` resolves ``get_loki_home()`` at call time, a
