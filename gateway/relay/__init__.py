"""Relay/connector support package for the Loki gateway.
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        try:
            from loki_cli.skin_engine import get_active_skin  # late import: boot-safe
        ...
        # prefix every reply "**Loki Agent:**" and shadow the connector's
...
def _resolve_relay_identity_token() -> str:
    ...
    Canonical resolver shared by runtime self-provision and ``loki gateway enroll``.
    Modes, in precedence order:
      ...
      2.  WunderCorp Portal (default): ``resolve_wundercorp_access_token()``.
    ...
    if not token_url:
        ...
        return resolve_wundercorp_access_token()
...
def self_provision_relay() -> bool:
    ...
    ``~/.loki/.env``), so a hosted container re-provisions every boot; the
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'loki'}"
