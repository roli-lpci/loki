...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``loki setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``loki gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved LOKI_HOME and overlay the
        ...
        # `loki` invocation, so the import happens only when a lookup asks for it.
