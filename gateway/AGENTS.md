...
(`loki_cli/AGENTS.md`). Each adapter picks a base toolset (Telegram → `"messaging"`).
...
`if canonical == ...` chains. Registry + adding a command: `loki_cli/AGENTS.md`.
...
duplicate-final incident (NS-658 canary ledger, loki#85796 / gateway-gateway#210); violating any
...
(or `LOKI_BACKGROUND_NOTIFICATIONS`): `concise` (default; one line, failures append an output
...
`/login` is registered in `loki_cli/commands.py` with `busy_policy="dispatch"` and
...
**It binds the whole install.** The sign-in writes the singleton `providers.wundercorp`, so whoever
...
On completion the handler **evicts** every cached agent still on `wundercorp/welcome` and clears any
...
`loki serve` (control plane, desktop-spawned child) dies with the app — by design. The messaging
gateway (`gateway run`) SURVIVES the app: the serve backend's `/api/gateway/*` endpoints spawn it
detached (`_spawn_loki_action` — `start_new_session` / `DETACHED_PROCESS`), so `before-quit`'s
