...
SCALE_TO_ZERO_ENV = "LOKI_SCALE_TO_ZERO"  # stamped by NAS when the Labs toggle is on
...
def dashboard_client_heartbeat_path(loki_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under LOKI_HOME."""
    if loki_home is None:
        from loki_constants import get_loki_home
        loki_home = get_loki_home()
    return Path(loki_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
