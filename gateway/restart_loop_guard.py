...
Defenses 1-2 (``_LOKI_GATEWAY`` guard on ``loki gateway stop|restart`` /
...
persisted to ``<LOKI_HOME>/gateway/restart_loop.json`` and CHAIN while gaps stay
...
def _state_path():
    return get_loki_home() / "gateway" / "restart_loop.json"
