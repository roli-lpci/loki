...
class GatewayNotificationsMixin:
    ...
    class _UpdatePaths:
        """Marker files ``loki update --gateway`` and its watcher exchange under LOKI_HOME."""
    ...
    def _update_paths(cls) -> "GatewayNotificationsMixin._UpdatePaths":
        ...
        return cls._UpdatePaths(
            pending=_loki_home / ".update_pending.json",
            claimed=_loki_home / ".update_pending.claimed.json", output=_loki_home / ".update_output.txt",
            exit_code=_loki_home / ".update_exit_code",
            prompt=_loki_home / ".update_prompt.json", response=_loki_home / ".update_response",
        )
    ...
    async def _watch_update_progress(
        ...
        """Watch ``loki update --gateway``, streaming output + forwarding prompts.
        ...
        while loop.time() < deadline:
            if paths.exit_code.exists():
                ...
                with _log_suppressed(logging.WARNING, "Update final notification failed: %s"):
                    ...
                    await target.send(
                        "✅ Loki update finished." if exit_code == 0
                        else "❌ Loki update failed (exit code {}).".format(exit_code)
                    )
        ...
        if not paths.exit_code.exists():
            ...
            await _flush_buffer()
            with suppress(Exception):
                await target.send("❌ Loki update timed out after 30 minutes.")
            self._clear_update_markers(paths, session_key)
        ...
        try:
            ...
            if chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "…" + output[-3500:]
                    status = "✅ Loki update finished." if exit_code == 0 else "❌ Loki update failed."
                    msg = f"{status}\n\n```\n{output}\n```"
                else:
                    msg = (
                        "✅ Loki update finished successfully." if exit_code == 0 else
                        "❌ Loki update failed. Check the gateway logs or run `loki update` manually for details."
                    )
    ...
    async def _send_restart_notification(self) -> Optional[tuple[str, str, Optional[str]]]:
        ...
        from gateway.run import _loki_home, _non_conversational_metadata
        notify_path = _loki_home / ".restart_notify.json"
    ...
    def _free_tier_startup_line(self) -> Optional[str]:
        """Extra startup line when the gateway's inference is carried by the WunderCorp free tier; None otherwise.
        ...
        try:
            ...
            if not guest_carries_inference():
                return None
            if resolve_provider("auto") != "wundercorp":
                return None
        except Exception as exc:
            logger.debug("Free tier startup line skipped: %s", exc)
            return None
        return "Inference: WunderCorp free tier (wundercorp/welcome). Sign in for more: /login"
    ...
    async def _send_home_channel_startup_notifications(
        ...
        message = "♻️ Gateway online — Loki is back and ready."
    ...
    async def _send_session_db_warning_notifications(self) -> None:
        ...
        from loki_state import _default_db_path, classify_persistence_error, format_session_db_unavailable
        if classify_persistence_error(error) == "corrupt":
            # Copy-pasteable, so name the real store (profiles / LOKI_HOME do not live under ~/.loki).
            db_path = _default_db_path()
            backups_dir = get_default_loki_root() / "backups"
            message = (
                "⚠️ Session database corruption detected. Messages may not be "
                "persisted. Recovery options:\n"
                "1. Run `loki doctor --fix`\n"
                "2. Stop the gateway, then recover with:\n"
                f"   loki sessions recover --source {db_path} "
                "--inspect-only\n"
                "   (if it reports recoverable) loki sessions recover "
    ...
    async def _inject_watch_notification(
        ...
        if not source:
            # API-server sessions bind the RAW X-Loki-Session-Id key, not a structured ``agent:...`` key.
