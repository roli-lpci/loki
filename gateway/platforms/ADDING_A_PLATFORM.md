...
There are two ways to add a platform to the Loki gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.loki/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Loki code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `loki gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `loki_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Loki core.
...
"loki-your-platform": {
    ...
    "tools": _LOKI_CORE_TOOLS,
    "includes": []
},
```

And add it to the `loki-gateway` composite:

```python
"loki-gateway": {
    "includes": [..., "loki-your-platform"]
}
```
...
## 12. Status Display (`loki_cli/status.py`)
...
## 13. Gateway Setup Wizard (`loki_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ loki_cli/ toolsets.py \
  --include="*.py" -l | sort -u
