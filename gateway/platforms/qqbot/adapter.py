...
class QQAdapter(BasePlatformAdapter):
    ...
    async def _send_identify(self) -> None:
        ...
        payload = {"op": 2, "d": {
            ...
            "shard": [0, 1],
            "properties": {"$os": "macOS", "$browser": "loki-agent", "$device": "loki-agent"}}}
    ...
    def _create_task(coro):
        ...
        (tests call _dispatch_payload synchrowundercorply)."""
        try:
    ...
    def _dispatch_payload(self, payload: Dict[str, Any]) -> None:
        """Route inbound WebSocket payloads (dispatch synchrowundercorply, spawn async handlers)."""
    ...
    async def _default_interaction_dispatch(self, event: InteractionEvent) -> None:
        ...
        ``~/.loki/.update_response``; anything else is ignored at DEBUG."""
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``.update_response``, polled by the detached ``loki update --gateway`` watcher."""
        try:
    ...
    async def send_update_prompt(
        ...
        are written to ``~/.loki/.update_response`` by the interaction callback."""
