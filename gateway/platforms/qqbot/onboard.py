"""QQBot scan-to-configure (QR code onboard). Mirrors the Feishu pattern: synchrowundercorp HTTP + one entry-point
...
def _portal_post(path: str, payload: dict, timeout: float, fail_msg: str) -> dict:
    """Synchrowundercorp POST to the portal host; raises RuntimeError on non-zero ``retcode``."""
    import httpx
