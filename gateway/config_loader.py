...
``gateway.<key>`` form (what ``loki config set gateway.<key>`` produces) is consulted only when the
...
#
# Top-level settings are also accepted nested under ``gateway:`` (what ``loki config set
...
def load_yaml_layer(home: Path, gw_data: dict) -> None:
    ...
    # loki_cli.config.load_config, so managed quick_commands / stt would otherwise be ignored).
