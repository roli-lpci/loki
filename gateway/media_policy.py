...
``validate_media_delivery_path`` reads ``LOKI_MEDIA_DELIVERY_STRICT`` (gateway.strict),
``LOKI_MEDIA_ALLOW_DIRS`` (gateway.media_delivery_allow_dirs) and
``LOKI_MEDIA_TRUST_RECENT_FILES`` (gateway.trust_recent_files).  Every delivery
entrypoint (gateway startup, ``loki cron run``, ``loki send``) calls
...
_FLAG_ENVS = (("strict", "LOKI_MEDIA_DELIVERY_STRICT"), ("trust_recent_files", "LOKI_MEDIA_TRUST_RECENT_FILES"))
_ALLOW_DIRS_ENV = "LOKI_MEDIA_ALLOW_DIRS"
