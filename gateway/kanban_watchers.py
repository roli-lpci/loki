...
class GatewayKanbanWatchersMixin:
    ...
    def _kanban_dispatcher_boot(self) -> Optional[tuple]:
        ...
        try:
            ...
            return None
        env_override = os.environ.get("LOKI_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban dispatcher: disabled via LOKI_KANBAN_DISPATCH_IN_GATEWAY env")
            return None
        try:
            cfg = _load_config()
        except Exception as exc:
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        loop exits and an external `loki kanban daemon` is expected. Each
        ...
        while self._running:
            ...
            try:
                # Emergency stop (`loki pause`): no auto-decompose or
