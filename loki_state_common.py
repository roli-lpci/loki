"""Shared constants and helpers for the SessionDB family of modules.  Lives outside loki_state so
...
# 'new_session' (loki_cli/cli_session_mixin.py) without a reset child row.  A compression rotation must
...
# optimized via ``loki sessions optimize-storage``. A legacy DB sits at
...
--
-- A host-declared conversation key (X-Loki-Session-Key / build_session_key)
...
# and must not serve reads until `loki sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `loki db optimize`. This is
...
# claims progress under SQLite transaction authority).  Mirrors `loki_state_repair._cross_process_repair_lock`:
...
# `<db>.repair.lock` (offline schema surgery, minutes in VACUUM).  Lives here: mixins cannot import loki_state.

# ── Cross-process full-FTS-rebuild admission (single authority) ────────────── Several independent Loki
# processes routinely share one state.db (gateway service, the Desktop app's `loki serve` backend,
...
# authority and is intentionally multi-process. Semantics mirror `loki_state_repair._cross_process_repair_lock`
...
# indeterminate liveness still defers. It lives here (not loki_state) because the search/schema mixins
# cannot import loki_state (cycle). The lock file is `<db>.fts_rebuild.lock`, distinct from
...
def _describe_lock_holder(record) -> str:
    ...
    if not isinstance(record, dict) or "pid" not in record:
        return "unknown (no holder record; pre-fix writer or non-Loki)"
    age = ""
