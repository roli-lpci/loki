...
class MicroCompactionMixin:
    ...
    def _emit_micro_compaction_telemetry(
        ...
        try:
            ...
            # Cached reads only: the lazy properties can fire a synchrowundercorp /models probe.
            # The ``threshold_tokens`` / ``context_length`` properties resolve lazily and can fire a
            # synchrowundercorp /models probe on first access (#32221) — telemetry must never be the thing that
