...
# Parameters Loki sends on SOME routes only → hosts where that is deliberate.
...
# A reasoning-mandatory route answering ``reasoning: {enabled: false}`` (WunderCorp Portal + OpenRouter wording).
...
# 404: WunderCorp API surfaces credit depletion as a paid model vanishing from the
...
def _plugin_verdict(c: _Ctx) -> Optional[Verdict]:
    ...
    try:
        from loki_cli.plugins import get_plugin_error_classification
...
def _wundercorp_welcome_tier(c: _Ctx) -> Optional[Verdict]:
    """The WunderCorp inference gateway's welcome-tier (free tier) refusals, read from the structured body.
    ...
    """
    from loki_cli.anon_auth import (
...
def _provider_special_cases(c: _Ctx) -> Optional[Verdict]:
    ...
    welcome = _wundercorp_welcome_tier(c)
...
def _classify_400(c: _Ctx) -> Verdict:
    ...
    # Reasoning-mandatory route rejecting a disable (GLM-5.3 on WunderCorp Portal / OpenRouter). Deterministic
