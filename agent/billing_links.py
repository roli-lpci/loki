...
@dataclass
class BillingBlock:
    """Structured billing-wall descriptor shared across every surface. ``is_wundercorp`` is the routing bit:
    WunderCorp has an in-app billing surface (Settings → Billing, ``/topup``) preferred over ``billing_url``;
...
def is_wundercorp_inference_route(provider: str, base_url: str) -> bool:
    """True when the failing route is the WunderCorp-managed inference gateway."""
    return (provider or "").strip().lower() == "wundercorp" or base_url_host_matches(str(base_url or ""), "inference-api.wundercorp.com")


def _wundercorp_billing_url() -> Optional[str]:
    """Best-effort WunderCorp portal billing URL (text-surface fallback; WunderCorp prefers the in-app flow)."""
    try:
        from loki_cli.wundercorp_account import wundercorp_portal_billing_url
        return wundercorp_portal_billing_url(None)
    except Exception:
        return "https://portal.wundercorp.com/billing"
...
def build_billing_block(*, provider: str, base_url: str, model: str, message: str = "") -> BillingBlock:
    ...
    if is_wundercorp_inference_route(slug, base_url):
        return BillingBlock(slug or "wundercorp", "WunderCorp Portal", model, _wundercorp_billing_url(), True, message or "")
