...
Installs go to a Loki-owned staging dir, ``<LOKI_HOME>/lsp/bin/``, so the
...
missing binary skips the server and ``loki lsp status`` reports it).
Installs run synchrowundercorply the first time a server is needed, serialized
...
# ``bin`` in ``<LOKI_HOME>/lsp/bin/`` first, then on PATH.  ``extra_pkgs``
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # the host so `loki lsp status` reports its presence.
...
def loki_lsp_bin_dir() -> Path:
    """Return the Loki-owned bin staging dir for LSP servers."""
    from loki_constants import get_loki_home

    p = get_loki_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(loki_lsp_bin_dir() / name):
...
def _link_into_bin(target: Path) -> str:
    ...
    link = loki_lsp_bin_dir() / target.name
    if not link.exists():
        try:
...
def _install_npm(pkg: str, bin_name: str, extra_pkgs: Optional[list] = None) -> Optional[str]:
    ...
    # Managed npm first: $LOKI_HOME/node isn't on an arbitrary process's
    # PATH, so a bare which() would miss the Node that Loki installed.
    npm = find_node_executable("npm")
    if npm is None:
        logger.info("[install] cannot install %s: no usable npm found", pkg)
        return None
    staging = loki_lsp_bin_dir().parent  # <LOKI_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    ...
    pip_target = loki_lsp_bin_dir().parent / "python-packages"
...
def detect_status(pkg: str) -> str:
    """Return ``installed``, ``missing``, or ``manual-only`` (for ``loki lsp status``; spawns nothing)."""
    recipe = INSTALL_RECIPES.get(pkg)
...
__all__ = ["INSTALL_RECIPES", "try_install", "detect_status", "loki_lsp_bin_dir"]
