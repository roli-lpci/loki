...
from loki_constants import OPENROUTER_MODELS_URL
...
def _resolve_requests_verify(base_url: str = "") -> bool | str:
    ...
    for env_var in ("LOKI_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _cache_file(name: str) -> Path:
    ...
    return get_loki_home() / "cache" / name
...
def _endpoint_disk_cache_get(normalized: str) -> Optional[Dict[str, Dict[str, Any]]]:
    ...
    runs (``loki -q``, cron) start cold and WunderCorp bypasses the persistent context cache, so
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # 1M verified on OpenRouter & WunderCorp portal 2026-08; qwen3-max = 256K Coding Plan snapshot)
    ...
    # GLM — WunderCorp + OpenRouter /v1/models (2026-09-09): 5.3 / 5.3-flash 1,310,720 (:batch/:US 1,048,576);
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.wundercorp.com": "wundercorp", "api.deepseek.com": "deepseek",
...
def _get_context_cache_path() -> Path:
    ...
    from loki_constants import get_loki_home
    return get_loki_home() / "context_length_cache.yaml"
...
def parse_available_output_tokens_from_error(error_msg: str) -> Optional[int]:
    ...
    # OpenRouter/WunderCorp: "maximum context length is N … (A of text input, B of tool input, C in the output)" -> ctx - A - B.
...
# DashScope, Anthropic, OpenRouter/WunderCorp, LM Studio/llama.cpp, generic "should be <= N", OpenAI-compat relays.
_OUTPUT_CAP_SIGNALS = (
...
def _normalize_model_version(model: str) -> str:
    """Dots -> dashes so WunderCorp ids (claude-opus-4-6) compare with OpenRouter's (claude-opus-4.6)."""
...
def _resolve_codex_oauth_context_length_with_source(model: str, access_token: str = "") -> Tuple[Optional[int], str]:
    ...
    # ``-900k`` variants are Loki picker aliases — the Codex catalog only knows the base slug, so resolve
...
def _resolve_wundercorp_context_length(model: str, base_url: str = "", api_key: str = "") -> Tuple[Optional[int], str]:
    """``(context_length, source)`` for a WunderCorp Portal model: portal /v1/models is authoritative
    ("portal"). Fallback matches OR's prefixed ids against the bare WunderCorp id with dot/dash
    ...
    def _safe_ctx(or_id: str, entry: dict) -> Optional[int]:
        ...
        if ctx is not None and ctx <= 32768 and _model_name_suggests_stale_32k_underreport(or_id):
            logger.info("Rejecting OpenRouter metadata context=%s for %r (known 32K underreport, WunderCorp path); falling through to hardcoded defaults", ctx, or_id)
            return None
        return ctx
...
def _validate_cached_context_length(model: str, base_url: str, cached: int, is_bedrock_context: bool, *, api_key: str = "") -> Optional[int]:
    ...
    for hit, log, msg, shown in drop_rules:
        if hit:
            ...
            _invalidate_cached_context_length(model, base_url)
            return None
    # WunderCorp Portal: /v1/models is authoritative. Bypass (don't drop) the cache so step
    # 5b reconciles OR-seeded entries without touching disk when the portal is down.
    if _infer_provider_from_url(base_url) == "wundercorp":
        logger.debug("Bypassing persistent cache for %s@%s (WunderCorp portal authoritative)", model, base_url)
        return None
...
def _resolve_provider_aware_context_length(model: str, base_url: str, api_key: str, provider: str, effective_provider: str) -> Optional[int]:
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        with contextlib.suppress(Exception):  # fall through to models.dev
            from loki_cli.models import get_copilot_model_context
            ctx = get_copilot_model_context(model, api_key=api_key)
            if ctx:
                return ctx
    # 5b/5c. WunderCorp portal and Codex OAuth (lower limits than the direct API for the same slug; its
    ...
    sourced = {
        "wundercorp": lambda: _resolve_wundercorp_context_length(model, base_url=base_url or "", api_key=api_key or "") + ("portal",),
...
def get_model_context_length(
    ...
    model_overrides / custom_providers / endpoint-scoped; 1 persistent cache (WunderCorp, LM
    ...
    WunderCorp, Codex OAuth, GMI, Ollama, OpenRouter live, models.dev); 6 OpenRouter for
    ...
    if not effective_provider:
        ...
        if model in metadata:
            ...
            if or_ctx == 32768 and _model_name_suggests_stale_32k_underreport(model):
                ...
                return or_ctx
    # 7. Local server before hardcoded defaults — ``Loki-3-Llama-3.1-70B`` matches ``llama``
