...
from loki_cli.timeouts import get_provider_request_timeout
...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'loki memory status'. Note: "
        "systemd/gateway services do not inherit ~/.loki/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    with suppress(Exception):
        from loki_cli.providers import LOKI_OVERLAYS, get_provider
        overlay = LOKI_OVERLAYS.get(provider)
    ...
    with suppress(Exception):
        ...
        from loki_cli.models import normalize_provider as normalize_model_provider
        from loki_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: loki config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    """Per-profile marker path (``$LOKI_HOME`` is profile-scoped; not a config key)."""
    return get_loki_home() / ".codex_gpt55_autoraise_notice"
...
def _resolve_api_mode(agent, api_mode, provider_name, base_url):
    ...
    if is_actual_route(agent.provider, base_url):
    ...
    elif agent.provider in {"wundercorp", "wundercorp-portal", "wundercorp"}:
        ...
        agent.api_mode = wundercorp_api_mode(agent.model)
    else:
        ...
        try:
            ...
            from loki_cli.providers import host_mandated_api_mode as _host_mandated_api_mode
...
def _finalize_routing(agent, api_mode, credential_pool):
    ...
    # The WunderCorp agent key lives ~1 h. Without the proactive refresher every agent in the process
    ...
    # WunderCorp model policy follows the ROUTE (the welcome host serves one model); a credential-pool
    # swap can change the route later, so ``_swap_credential`` applies the same helper again.
    from loki_cli.anon_auth import pin_model_for_route
...
_TURN_STATE: Dict[str, Any] = {
    ...
    # Credits tracking (dev-only, LOKI_DEV_CREDITS) from x-wundercorp-credits-* headers; session
...
def _init_prompt_cache_config(agent):
    ...
    agent._cache_ttl = "5m"
    with suppress(Exception):
        from loki_cli.config import load_config_readonly as _load_pc_cfg
...
def _setup_logging(agent):
    ...
    from loki_logging import setup_logging, setup_verbose_logging
    setup_logging(loki_home=_ra()._loki_home)
    ...
    # would starve the root file handlers. Noise reduction belongs in loki_logging.
...
def _init_anthropic_client(agent, api_key, base_url, _provider_timeout):
    ...
    if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
        try:
            from loki_cli.auth import build_minimax_oauth_token_provider
...
def _init_bedrock_client(agent, base_url):
    ...
    with suppress(Exception):
        from loki_cli.config import load_config_readonly as _load_br_cfg
...
def _routed_client_kwargs(agent, fallback_model, _provider_timeout) -> Dict[str, Any]:
    ...
    if _routed_client is not None:
        from loki_cli.providers import is_actual_route, normalize_provider
        effective_provider = getattr(_routed_client, "_loki_aux_effective_provider", "")
    ...
    if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
        ...
        raise RuntimeError(
            ...
            f"variable, or switch to a different provider with `loki model`."
        )
    raise RuntimeError(
        "No LLM provider configured. Run `loki model` to "
        "select a provider, or run `loki setup` for first-time "
        "configuration."
    )
...
_HOST_DEFAULT_HEADERS: List[tuple[str, Callable[[Any, str], Dict[str, str]]]] = [
    ...
    ("githubcopilot.com", _lazy_headers("loki_cli.models", "copilot_default_headers")),
    ...
    ("x.ai", _lazy_headers("tools.xai_http", "loki_xai_default_headers")),
]
...
def _load_tools(agent, enabled_toolsets, disabled_toolsets):
    # A multiplexed gateway may have switched LOKI_HOME since model_tools was imported;
    ...
    # Kanban guidance is session-static (kanban_show iff LOKI_KANBAN_TASK); resolve once.
...
def _publish_session_id(session_id: str) -> None:
    ...
    try:
        ...
        if not delegated_child:
            os.environ["LOKI_SESSION_ID"] = session_id
...
def _init_session_state(agent, session_id, session_db, parent_session_id, reasoning_config, max_tokens,
    ...
    # ~/.loki/sessions/ — kept unconditionally for request_dump_*.json debug breadcrumbs.
    agent.logs_dir = get_loki_home() / "sessions"
    ...
    # Process-scoped --yolo is persisted so `loki --resume` restores the bypass
...
def _compression_codex_settings(cfg: Dict[str, Any]) -> tuple[str, bool, Optional[int]]:
    ...
    if app_server_auto not in {"native", "loki", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, loki, off.",
            app_server_auto,
        )
...
def _configured_default_base_url(_agent_cfg, _model_cfg, _custom_providers) -> str:
    ...
    if _norm in _RUNTIME_FIRST_PROVIDER_IDS:
        ...
        with suppress(Exception):
            from loki_cli.auth import resolve_provider as resolve_auth_provider
...
def _select_context_engine(_agent_cfg):
    ...
    if _selected_engine is None:
        try:
            from loki_cli.plugins import get_plugin_context_engine
...
def _enforce_minimum_context(agent):
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Loki Agent.  Choose a model with at least "
...
def _warn_nonagentic_loki_model(agent):
    # WunderCorp Loki 3/4 are chat models, not tool-call-tuned. cli.py show_banner() already
    ...
    if agent.quiet_mode or (agent.platform or "cli") == "cli":
        return
    with suppress(Exception):
        from loki_cli.model_switch import _check_loki_model_warning
        _loki_warn = _check_loki_model_warning(agent.model or "")
        if _loki_warn:
            _user_msg = (
                "⚠ WunderCorp, Inc. Loki 3 & 4 models are NOT agentic — they "
    ...
    if agent.context_compressor:
        try:
            agent.context_compressor.on_session_start(
                agent.session_id, loki_home=str(get_loki_home()),
...
def init_agent(
    ...
    Non-obvious parameters:
      ...
      skip_context_files: skip SOUL.md/.loki.md/AGENTS.md/CLAUDE.md/.cursorrules injection;
        load_soul_identity keeps ~/.loki/SOUL.md as identity regardless.
    """
    _install_safe_stdio()
    ...
    try:
        from loki_cli.config import load_config_readonly as _load_agent_config
    ...
    _warn_nonagentic_loki_model(agent)
