"""Cross-session rate limit guard for WunderCorp Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway, cron,
auxiliary) can check whether WunderCorp Portal is currently rate-limited before making
requests. Without it each 429 fans out into up to 9 calls per turn (3 SDK
retries x 3 Loki retries), all counted against RPH.
"""
...
def _state_path() -> str:
    """Path to the WunderCorp rate limit state file."""
    try:
        from loki_constants import get_loki_home
        base = get_loki_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".loki")
    return os.path.join(base, "rate_limits", "wundercorp.json")
...
def record_wundercorp_rate_limit(
    *, headers: Optional[Mapping[str, str]] = None, error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that WunderCorp Portal is rate-limited in the shared state file.
    ...
    try:
        ...
        logger.info("WunderCorp rate limit recorded: resets in %.0fs (at %.0f)", reset_at - now, reset_at)
    except Exception as exc:
        logger.debug("Failed to write WunderCorp rate limit state: %s", exc)


def wundercorp_rate_limit_remaining() -> Optional[float]:
...
def clear_wundercorp_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful WunderCorp request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear WunderCorp rate limit state: %s", exc)
...
def is_genuine_wundercorp_rate_limit(
    *, headers: Optional[Mapping[str, str]] = None, last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from WunderCorp Portal is a real account rate limit.

    WunderCorp multiplexes upstream providers behind one key, so a 429 may be one
    upstream model out of capacity (clears in seconds) rather than our quota;
    tripping the breaker on that would block every WunderCorp model for minutes.
