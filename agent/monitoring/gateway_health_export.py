...
_OBSERVABLE_METRIC_NAMES = (
    "loki.gateway.up", "loki.gateway.state", "loki.gateway.active_agents", "loki.gateway.busy",
    "loki.gateway.drainable", "loki.gateway.restart_requested", "loki.gateway.background_work",
    "loki.gateway.background_delegations", "loki.platform.up", "loki.platform.degraded",
    "loki.cron.scheduler.heartbeat_age_seconds", "loki.cron.scheduler.last_success_age_seconds",
    "loki.cron.scheduler.catch_up_occurrences", "loki.cron.jobs.enabled", "loki.cron.jobs.running",
    "loki.cron.jobs.overdue",
)
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="loki-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("loki.gateway.background_work", _read_background_work_count),
            ("loki.gateway.background_delegations", _read_background_delegations_count),
        ):
...
def _start_metric_provider(config: Dict[str, Any], sdk: Dict[str, Any]) -> Any:
    ...
    meter = provider.get_meter("loki.gateway.health")
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("loki.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'loki-agent[otlp]'", exc_info=True)
    ...
    if diagnostics_on:
        try:
            ...
            thread = threading.Thread(target=_run, name="loki-gateway-health-export", daemon=True)
            thread.start()
