...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "loki.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("loki.gateway.up", int(bool(gateway_running))),
        metric("loki.gateway.active_agents", active_agents),
        metric("loki.gateway.busy", int(bool(busy))),
        metric("loki.gateway.drainable", int(bool(drainable))),
        metric("loki.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("loki.gateway.state", 1, **{"loki.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"loki.platform": str(platform), "loki.platform.state": state}
        metrics.append(metric("loki.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("loki.platform.degraded", int(is_degraded), **pattrs, **{"loki.error_code": error_code}))
        if is_degraded:
