...
def _constants_path(getter_name: str) -> Path:
    """Call ``loki_constants.<getter_name>()`` (local import avoids cycles); ``~/.loki`` on any failure."""
    try:
        import loki_constants

        return getattr(loki_constants, getter_name)()
    except Exception:
        return Path(os.path.expanduser("~/.loki"))


def _loki_home_path() -> Path:
    """Active LOKI_HOME (profile-aware). Tests monkeypatch this name."""
    return _constants_path("get_loki_home")


def _loki_root_path() -> Path:
    """Loki root dir (parent of any profile, never per-profile)."""
    return _constants_path("get_default_loki_root")


def _loki_dirs() -> list[Path]:
    """Resolved active LOKI_HOME and global root, deduplicated.

    Both are checked so credential stores at <root>/... stay guarded when
    running under a profile (LOKI_HOME = <root>/profiles/<name>).
    """
    return list(dict.fromkeys(_resolve_each((_loki_home_path(), _loki_root_path()))))
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    loki_files = (".env", ".anthropic_oauth.json", os.path.join("cache", "bws_cache.enc.json"))
    paths = [
        *(os.path.join(home, *f) for f in home_files),
        *(str(base / f) for f in loki_files for base in (_loki_home_path(), _loki_root_path())),
...
def get_safe_write_roots() -> set[str]:
    """Resolved LOKI_WRITE_SAFE_ROOT paths (``os.pathsep``-separated list)."""
    roots: set[str] = set()
    for path in filter(None, os.getenv("LOKI_WRITE_SAFE_ROOT", "").split(os.pathsep)):
...
# LOKI_HOME / root subpaths that the agent's generic file tools must not
...
_LOKI_PROTECTED_SUBPATHS = ("state.db", "sessions", "mcp-tokens", "pairing")
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    for base in _loki_dirs():
        for sub in _LOKI_PROTECTED_SUBPATHS:
            with suppress(Exception):
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside LOKI_WRITE_SAFE_ROOT "
...
# Exact-file credential stores under LOKI_HOME / <root>. The agent never
...
# Directory-prefix read denies under LOKI_HOME / <root>: (subdir, message for
...
_READ_DENIED_DIRS = (
    ("mcp-tokens",
     "is the Loki MCP token directory and cannot be read directly.",
     "is a Loki MCP token file and cannot be read directly."),
    ("browser-profile",
     "is the Loki real-profile browser snapshot directory (copied cookies/logins) and cannot be read directly.",
     "is inside the Loki real-profile browser snapshot (copied cookies/logins) and cannot be read directly."),
    # vault.key + vault.json.enc sit side by side; key + ciphertext = plaintext, so the whole dir is one credential.
    ("vault",
     "is the Loki credential vault directory and cannot be read directly (secrets are filled server-side by browser_vault_fill).",
     "is inside the Loki credential vault (encrypted secrets + local key) and cannot be read directly (browser_vault_fill resolves them server-side)."),
)


def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Loki path.

    Blocked: internal skill-hub caches (prompt-injection carriers), credential
    stores under LOKI_HOME and the global root (exact files, plus anything
    ...
    loki_dirs = _loki_dirs()
    reason = None
    if any(_is_under(resolved, hd / "skills" / ".hub") for hd in loki_dirs):
        reason = (
            "is an internal Loki cache file and cannot be read directly to prevent "
            "prompt injection. Use the skills_list or skill_view tools instead."
        )
    elif any(resolved in _resolve_each(hd / name for hd in loki_dirs) for name in _CREDENTIAL_FILE_NAMES):
        reason = (
            "is a Loki credential store and cannot be read directly. Provider tools "
            "consume these credentials through internal channels." + _DID_SUFFIX
        )
    else:
        for subdir, dir_msg, file_msg in _READ_DENIED_DIRS:
            for blocked_dir in _resolve_each(hd / subdir for hd in loki_dirs):
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Loki read (see ``get_read_block_error``).
...
def _resolve_active_profile_name() -> str:
    """Active profile name from LOKI_HOME: ``~/.loki`` -> ``"default"``,
    ``~/.loki/profiles/X`` -> ``"X"``; ``"default"`` on any resolution failure."""
    try:
        parts = _loki_home_path().resolve().relative_to(_loki_root_path().resolve() / "profiles").parts
...
#   <LOKI_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.loki/...
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Loki state: ``None``
    for non-mirror paths, else ``target_path`` (resolved), ``mirror_root`` (the
    ``…/home/.loki`` prefix) and ``inner_path`` (what the agent meant on the host)."""
    target = _resolve_target(path)
    parts = target.parts if target is not None else ()
    # Need at least: sandboxes / <backend> / <task> / home / .loki / <thing>; inner_idx = the .loki part.
    inner_idx = next(
        (i + 4 for i, part in enumerate(parts)
         if part == "sandboxes" and i + 5 < len(parts) and parts[i + 3] == "home" and parts[i + 4] == ".loki"),
        None,
    )
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "Writes here land on a copy that the host Loki process never reads — the "
        "authoritative file is likely {inner_path!r} under the real LOKI_HOME.",
...
def classify_container_mirror_target(path: str, mirror_prefix: str | None = None) -> Optional[dict]:
    ...
    the bind mount strips the ``sandboxes/`` prefix (the agent sees plain ``/root/.loki/…``),
...
def get_container_mirror_warning(path: str, mirror_prefix: str | None = None) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "the container's bind-mounted home — a per-task mirror that the host Loki "
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Loki scope, or is inside
    ...
    try:
        ...
        root_real = _loki_root_path().resolve()
