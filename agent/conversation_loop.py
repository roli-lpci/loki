...
from agent.turn_api_call import handle_api_interrupt, wundercorp_rate_limit_guard, perform_api_call
...
from loki_logging import set_session_context
from tools.skill_provenance import set_current_write_origin
from utils import base_url_host_matches

logger = logging.getLogger(__name__)

# Must mirror _STALE_TOOL_CALL_MARKER_RE in loki_state.py; kept local so importing
# loki_state (module-level DEFAULT_DB_PATH) is not forced at load time.
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Loki tool use: model=%s provider=%s base_url=%s "
    ...
    return (
        f"Ollama loaded `{model}` with only {runtime_ctx:,} tokens of runtime context, but Loki "
        ...
        "again. A known-good starting point is 65,536 tokens. In Loki config, set "
...
def _maybe_grow_local_window(agent: Any, compressor: Any,
    ...
    if provider not in ("llamacpp", "llama.cpp", "llama-cpp", "custom") or not (
        ...
        return None
    try:
        from loki_cli.local_runtime.growth import maybe_grow_window
...
def _wundercorp_entitlement_message(capability: str) -> str:
    try:
        from loki_cli.wundercorp_account import (
            format_wundercorp_portal_entitlement_message,
            get_wundercorp_portal_account_info,
        )
        account_info = get_wundercorp_portal_account_info(force_fresh=True)
        return format_wundercorp_portal_entitlement_message(
...
def _print_wundercorp_entitlement_guidance(agent, capability: str) -> bool:
    return _print_guidance(agent, _wundercorp_entitlement_message(capability))
...
def _is_wundercorp_inference_route(provider: str, base_url: str) -> bool:
    return (provider or "").strip().lower() == "wundercorp" or base_url_host_matches(
        str(base_url or ""), "inference-api.wundercorp.com"
    )
...
def _billing_or_entitlement_message(
    *, capability: str, provider: str, base_url: str, model: str, unverified: bool = False
    ...
    if _is_wundercorp_inference_route(provider, base_url):
        return _wundercorp_entitlement_message(capability)
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            return "\n".join([
                ...
                "Retry with a fresh credential state: `loki auth reset anthropic`. Until that "
...
def _redecorate_prompt_cache_for_provider(
    ...
    if prepared is not None and getattr(agent, "provider", None) == "moa":
        # Prepared MoA state is canonical: the synchrowundercorp acting-aggregator
...
def _decode_inline_moa_turn(user_message, persist_user_message):
    ...
    try:
        from loki_cli.moa_config import decode_moa_turn
...
def _run_api_retry_loop(agent, s: _LoopState) -> Optional[Dict[str, Any]]:
    ...
    while s.retry_count < s.max_retries:
        _ng = _run_phase(wundercorp_rate_limit_guard, agent, s)
...
def _run_conversation_turn(
    ...
    # Adopt ~/.loki/.env credential/base-url edits made since the last turn — a
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'PARTIAL_STREAM_STUB_ID': ('loki_constants', 'PARTIAL_STREAM_STUB_ID'),
