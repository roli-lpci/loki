...
# the pin reaches the retry's whole synchrowundercorp call chain and cannot leak into the stalled attempt or any
...
_SUMMARY_ROUTE_PIN: contextvars.ContextVar[Optional[Dict[str, Any]]] = (
    contextvars.ContextVar("loki_summary_route_pin", default=None)
)
...
# (loki_state._rows_to_conversation), so any code that mutates a loaded or flushed dict's content in place
...
def _today_for_prompt() -> str:
    ...
    try:
        ...
        # via loki_time.now(). The compaction summary is a mid-conversation message that is NOT part of
        ...
        return _loki_now().strftime("%Y-%m-%d")
...
class ContextCompressor(SummaryDispatchMixin, MicroCompactionMixin, ContextEngine):
    ...
    def __init__(
        ...
        # get_model_context_length() probe to run inside __init__ and re-introduce the exact synchrowundercorp
