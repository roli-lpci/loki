...
def _codex_native_auto_compaction(agent: Any) -> bool:
    """Codex app-server threads are compacted by the codex agent itself; Loki only
    initiates compaction in "loki" mode."""
    return (
        # See #36801.
...
def _preflight_compression(
    ...
    if _preflight_deferred:
        ...
        logger.info(
            "Skipping Loki preflight compression for codex app-server "
            "(mode=%s); Loki will not start thread compaction here.",
