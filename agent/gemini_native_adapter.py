...
try:
    import loki_cli as _loki_cli

    _LOKI_VERSION = str(_loki_cli.__version__)
except Exception:
    _LOKI_VERSION = "0.0.0"
_API_CLIENT = f"loki-agent/{_LOKI_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "Loki typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.loki/.env and "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.loki/.env (get one at https://aistudio.google.com/app/apikey), or run `loki setup` to "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Loki retry/error classification."""
...
class GeminiNativeClient:
    ...
    # (No LOKI_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
