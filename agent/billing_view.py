...
def build_billing_state(*, timeout: float = 15.0) -> BillingState:
    """Fetch + parse ``/api/billing/state``; fail-open. ``LOKI_DEV_BILLING_FIXTURE`` short-circuits to a fixture."""
...
def _dev_fixture_billing_state() -> Optional[BillingState]:
    """``LOKI_DEV_BILLING_FIXTURE`` -> :class:`BillingState` for offline UX; None when unset.
    ...
    """
    name = (os.getenv("LOKI_DEV_BILLING_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    common: dict[str, Any] = dict(
        ...
        charge_presets=(Decimal("10"), Decimal("25"), Decimal("50")), portal_url="https://portal.wundercorp.com/billing?topup=open",
    )
    ...
    if name not in overrides:
        return BillingState(logged_in=False, error=f"unknown LOKI_DEV_BILLING_FIXTURE: {name}")
