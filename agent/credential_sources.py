"""Unified removal contract for every credential source Loki reads from.

Readers live in ``agent.credential_pool``; what is unified here is **removal**:
``loki auth remove <provider> <N>`` must make the entry stay gone across
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.loki/.env; hint when the shell exports it."""
    from loki_cli.config import get_env_path, remove_env_value
    ...
    # .env as utf-8-sig like loki_cli/config.py: a BOM-sensitive read would
    ...
    if env_in_process and not env_in_dotenv:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Loki.",
            f"  The pool entry is now suppressed — Loki will ignore "
            f"{env_var} until you run `loki auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_loki_pkce(provider: str, removed) -> RemovalResult:
    """~/.loki/.anthropic_oauth.json is ours — delete it outright."""
    from loki_constants import get_loki_home

    result = RemovalResult()
    oauth_file = get_loki_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Loki Anthropic OAuth credentials")
...
def _remove_auth_store_oauth(provider: str, removed) -> RemovalResult:
    """Clear auth.json ``providers.<provider>`` (wundercorp, minimax-oauth, xai-oauth, openai-codex).
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.append(
        "Run `loki model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.extend([
        ...
        "Run `loki auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `loki auth add copilot` to re-enable if needed.",
    ])
...
_REGISTRY: List[RemovalStep] = [
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `loki auth add anthropic` to re-enable if needed.",
        ),
        description="~/.claude/.credentials.json",
    ),
    RemovalStep(
        provider="anthropic", source_id="loki_pkce",
        remove_fn=_remove_loki_pkce,
        description="~/.loki/.anthropic_oauth.json",
    ),
    RemovalStep(
        provider="wundercorp", source_id="device_code",
        remove_fn=_remove_auth_store_oauth,
        description="auth.json providers.wundercorp",
    ),
    RemovalStep(
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `loki auth add qwen-oauth` to re-enable if needed.",
        ),
