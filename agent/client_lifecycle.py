...
def _routermint_headers() -> dict:
    ...
    return {"User-Agent": f"LokiAgent/{_LOKI_VERSION}"}
...
_ROUTE_DEFAULT_HEADERS = (
    ...
    ("githubcopilot.com", lambda self, url: _lazy_attr("loki_cli.models", "copilot_default_headers")()),
    ...
    ("x.ai", lambda self, url: _lazy_attr("tools.xai_http", "loki_xai_default_headers")()),
)
...
class ClientLifecycleMixin:
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from loki_cli import auth as _auth
            resolve = (
    ...
    def _try_refresh_wundercorp_client_credentials(self, *, force: bool = True, require_account: str | None = None) -> bool:
        # Portal serves anthropic/* on the native Messages route, so either client kind may hold the expiring JWT.
        if self.provider != "wundercorp" or self.api_mode not in ("chat_completions", "anthropic_messages"):
            return False
        try:
            from loki_cli.auth import resolve_wundercorp_runtime_credentials
            timeout = env_float("LOKI_WUNDERCORP_TIMEOUT_SECONDS", 15)
            ...
            logger.debug("WunderCorp credential refresh failed: %s", exc)
            return False
        api_key, base_url = creds.get("api_key"), creds.get("base_url")
        if not _valid_credential_pair(api_key, base_url):
            return False
        ...
        if require_account is not None:
            try:
                from loki_cli.auth_constants import _decode_jwt_claims
                new_account = _decode_jwt_claims(str(api_key)).get("sub")
            except Exception:
                new_account = None
            if str(new_account or "") != require_account:
                logger.info(
                    "WunderCorp pre-expiry adoption skipped: the store's key belongs to a different account "
        ...
        if self.api_mode == "anthropic_messages":
            ...
            return True
        # WunderCorp requests should not inherit OpenRouter-only attribution headers.
        self._client_kwargs.pop("default_headers", None)
        return self._adopt_openai_credentials(api_key, base_url, reason="wundercorp_credential_refresh")
    ...
    _WUNDERCORP_KEY_ADOPT_SKEW_S = 180

    def _adopt_wundercorp_key_before_expiry(self) -> bool:
        """Swap in a fresh WunderCorp agent key BEFORE the one in hand expires, so the request never 401s.
        ...
        """
        if getattr(self, "provider", "") != "wundercorp" or not getattr(self, "api_key", None):
            return False
        try:
            from loki_cli.auth_constants import _decode_jwt_claims
        ...
        if not account or not isinstance(exp, (int, float)) or exp - time.time() > self._WUNDERCORP_KEY_ADOPT_SKEW_S:
            return False
        return self._try_refresh_wundercorp_client_credentials(force=False, require_account=str(account))
    ...
    def _resolve_env_credentials(self) -> Optional[tuple]:
        ...
        if pconfig and getattr(pconfig, "auth_type", "") == "api_key" and getattr(pconfig, "api_key_env_vars", ()):
            ...
            if self.provider == "actual":
                ...
                from loki_cli.runtime_provider import _config_base_url_for_provider, _get_model_config
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ``~/.loki/.env`` credential/base-url edits at the turn boundary (a Settings save updates ``.env``
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        if not self._is_copilot_provider():
            return False
        try:
            from loki_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token, evict_cached_exchanged_token
    ...
    def _apply_client_headers_for_base_url(self, base_url: str, *, apply_user_headers: bool = True) -> None:
        ...
        if self.api_mode not in ("anthropic_messages", "bedrock_converse"):
            try:
                from loki_cli.config import apply_custom_provider_extra_headers_to_client_kwargs
    ...
    def _swap_credential(self, entry) -> bool:
        ...
        from loki_cli.anon_auth import route_can_serve_model
