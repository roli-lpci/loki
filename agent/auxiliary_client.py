...
Text auto chain: main provider+model → OpenRouter → WunderCorp Portal → custom endpoint →
native Anthropic → direct API-key providers → None. Vision auto chain: main
provider (if a supported vision backend) → OpenRouter → WunderCorp → Anthropic → custom.
...
from loki_constants import OPENROUTER_BASE_URL
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_verify``, ``LOKI_CA_BUNDLE`` / ``SSL_CERT_FILE``); any failure → httpx default (``True``)."""
    try:
        from agent.ssl_verify import resolve_httpx_verify
        from loki_cli.config import get_custom_provider_tls_settings, load_config_readonly
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `loki update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    with contextlib.suppress(Exception):
        from loki_cli.models import OPENCODE_ZEN_FREE_KEYLESS_PLACEHOLDER, opencode_zen_free_headers
    ...
    # Loki owns aux retry/fallback policy; the SDK default (max_retries=2) would triple
    # wall time on a hung endpoint before Loki sees one failure.
    # Loki owns auxiliary retry + provider/model fallback policy (the same-provider transient retry in
    ...
    # so a 120s timeout can stall ~360s before Loki sees a single failure (issue #54465). Disable
    # SDK-internal retries by default and let Loki control the budget; explicit callers can still override
    # via kwargs.
...
def _run_protected_sync_provider_call(callback: Callable[[dict[str, Any]], Any], kwargs: dict[str, Any]) -> Any:
    ...
    threading.Thread(
        target=provider_context.run, args=(_provider_worker,), name="loki-protected-aux-provider",
        daemon=True).start()
    while True:
...
def _client_declares(client_obj: Any, flag: str) -> bool:
    ...
    not isinstance, so out-of-tree clients can opt out of wrappers unimported (cf. SUPPORTS_LOKI_TOOL_CALLS)."""
    try:
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    """
    is_wundercorp = provider_id.strip().lower() == "wundercorp"
    try:
        from loki_cli.auth import resolve_api_key_provider_credentials
        from loki_cli.models_pricing import fetch_models_with_pricing
        ...
        if not api_key and is_wundercorp:
            # WunderCorp is OAuth (resolver raises); anonymous reads return the full catalog.
            try:
                from loki_cli.models_pricing import _resolve_wundercorp_pricing_credentials
                api_key, base_url = _resolve_wundercorp_pricing_credentials()
            except Exception:
                logger.debug("No WunderCorp credentials for catalog", exc_info=True)
        if not base_url:
        ...
        if base_url.endswith("/v1"):  # fetch_models_with_pricing appends /v1/models
            base_url = base_url[:-3]
        # WunderCorp-only args must match the pickers' or the seeded cache loses sale chrome and
        # policy-catalog expiry.
        _wundercorp_kwargs = {}
        if is_wundercorp:
            from loki_cli.models_pricing import _WUNDERCORP_CATALOG_TTL_SECONDS
            _wundercorp_kwargs = {"include_sale_original": True, "cache_ttl_seconds": _WUNDERCORP_CATALOG_TTL_SECONDS}
        catalog = fetch_models_with_pricing(
            api_key=api_key or None, base_url=base_url, timeout=3.0, **_wundercorp_kwargs) or {}
    except Exception:
    ...
    if is_wundercorp:
        ...
        try:
            from loki_cli.models_pricing import wundercorp_policy_allowed_ids, restrict_to_wundercorp_policy
            ids = restrict_to_wundercorp_policy(ids, wundercorp_policy_allowed_ids())
        except Exception:
            logger.debug("WunderCorp policy filter unavailable", exc_info=True)
...
def _get_aux_model_for_provider(provider_id: str, *, prefer_fast: bool = False) -> str:
    ...
    if picked and provider_id.strip().lower() == "wundercorp":
        try:
            from loki_cli.models_pricing import wundercorp_policy_allowed_ids, restrict_to_wundercorp_policy
            allowed = wundercorp_policy_allowed_ids()
            if allowed and not restrict_to_wundercorp_policy([picked], allowed):
                return ""
        except Exception:
            logger.debug("WunderCorp policy check unavailable", exc_info=True)
    return picked
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://loki.computer",
    "X-Title": "Loki Agent",
...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    try:
        from loki_cli.config import cfg_get, load_config
        _cfg = load_config()
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Precedence env > config > default: ``LOKI_OPENROUTER_CACHE`` overrides
    ``openrouter.response_cache``; ``LOKI_OPENROUTER_CACHE_TTL`` (1-86400 s) overrides
    ``openrouter.response_cache_ttl``. ``or_config=None`` reads from disk.
    """
    headers = dict(_OR_HEADERS_BASE)
    if or_config is None:
        try:
            from loki_cli.config import load_config_readonly
            or_config = load_config_readonly().get("openrouter", {})
        except Exception:
            or_config = {}
    env_cache = os.environ.get("LOKI_OPENROUTER_CACHE", "").strip().lower()
    if not (env_cache in {"1", "true", "yes", "on"} if env_cache else or_config.get("response_cache", False)):
        return headers
    headers["X-OpenRouter-Cache"] = "true"
    env_ttl = os.environ.get("LOKI_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
...
_NVIDIA_NIM_CLOUD_HEADERS = {"X-BILLING-INVOKE-ORIGIN": "LokiAgent"}
...
_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://loki.computer",
    "X-Title": "Loki Agent",
    "User-Agent": f"LokiAgent/{_LOKI_VERSION}",
}

# WunderCorp Portal attribution extra_body. Tags come from agent.portal_tags so the client= marker
# tracks loki_cli.__version__ — never inline a literal here.
from agent.portal_tags import wundercorp_portal_tags as _wundercorp_portal_tags


def _wundercorp_extra_body() -> dict:
    """Fresh WunderCorp Portal ``extra_body`` (per call, so a hot-reloaded version is reflected)."""
    return {"tags": _wundercorp_portal_tags()}


# Set at resolve time — True if the auxiliary client points to WunderCorp Portal
auxiliary_is_wundercorp: bool = False
...
_WUNDERCORP_MODEL = "google/gemini-3.6-flash"
_WUNDERCORP_DEFAULT_BASE_URL = "https://inference-api.wundercorp.com/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_loki_home() / "auth.json"
...
def _to_openai_base_url(base_url: str) -> str:
    ...
    if base_url_hostname(url) == "api.actual.inc":
        from loki_cli.auth import normalize_actual_base_url
...
def _pool_runtime_api_key(entry: Any) -> str:
    # runtime_api_key handles provider-specific fallback (e.g. agent_key for wundercorp); None entry → "".
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "wundercorp":
        # Canonical auth-layer reader so the env override shares one normalization path.
        from loki_cli.auth import _wundercorp_inference_env_override
        env_url = _wundercorp_inference_env_override()
        if env_url:
            return env_url
...
def _wundercorp_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("LOKI_WUNDERCORP_MIN_KEY_TTL_SECONDS", "1800")))
...
class _CodexCompletionsAdapter:
    ...
    def _build_responses_kwargs(self, kwargs: Dict[str, Any]) -> Tuple[Dict[str, Any], str, Any]:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Codex only knows the base slug; strip the Loki ``-900k`` picker suffix.
    ...
    def create(self, **kwargs) -> Any:
        from loki_cli.providers import is_actual_route

        if is_actual_route(
            getattr(self._client, "_loki_aux_effective_provider", ""),
...
class _AnthropicCompletionsAdapter:
    ...
    def __init__(self, real_client: Any, model: str, is_oauth: bool = False, base_url: str | None = None):
        ...
        # Caller URL first; fall back to the SDK client's host only for WunderCorp Portal — a blanket
        ...
        if not self._base_url:
            ...
            if candidate:
                with contextlib.suppress(Exception):
                    from agent.anthropic_endpoints import _is_wundercorp_portal_endpoint
                    if _is_wundercorp_portal_endpoint(candidate):
    ...
    def create(self, **kwargs) -> Any:
        ...
        # (already TRANSLATED to native fields — raw would 400 on strict gateways) and ``_`` Loki plumbing.
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``loki_cli.runtime_provider._detect_api_mode_for_url`` so aux and main agree: any
...
def _maybe_wrap_anthropic(
    ...
    """
    # Anthropic/Bedrock/Codex wrappers, plus any client declaring LOKI_SKIP_TRANSPORT_WRAP
    ...
    if (
        ...
        or _client_declares(client_obj, "LOKI_SKIP_TRANSPORT_WRAP")
    ):
        return client_obj
...
def _read_wundercorp_auth() -> Optional[dict]:
    """WunderCorp provider state dict from the credential pool or ~/.loki/auth.json; None when not active with tokens."""
    pool_present, entry = _select_pool_entry("wundercorp")
    if pool_present:
        if entry is None:
            return None
        return {
            ...
            "inference_base_url": _pool_runtime_base_url(entry, _WUNDERCORP_DEFAULT_BASE_URL),
    ...
    try:
        ...
        if data.get("active_provider") != "wundercorp":
            return None
        provider = data.get("providers", {}).get("wundercorp", {})
        # Must have at least an access_token or agent_key.
        if not provider.get("agent_key") and not provider.get("access_token"):
            return None
        return provider
    except Exception as exc:
        logger.debug("Could not read WunderCorp auth: %s", exc)
        return None


def _wundercorp_api_key(provider: dict) -> str:
    """Extract a usable WunderCorp inference JWT from stored auth state."""
    from loki_cli.auth import _wundercorp_invoke_jwt_is_usable
    for token_key, expiry_key in (("agent_key", "agent_key_expires_at"), ("access_token", "expires_at")):
        token = provider.get(token_key)
        if not isinstance(token, str) or not token.strip():
            continue
        if _wundercorp_invoke_jwt_is_usable(token, scope=provider.get("scope"), expires_at=provider.get(expiry_key)):
            return token
    return ""


def _resolve_wundercorp_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Resolve WunderCorp auxiliary credentials from the selected pool entry."""
    try:
        from loki_cli.auth import _agent_key_is_usable
        pool = load_pool("wundercorp")
    except Exception as exc:
        logger.debug("Auxiliary WunderCorp pool credential resolution failed: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Auxiliary WunderCorp pool selection failed: %s", exc)
        return None
    if entry is None:
        return None
    ...
    if force_refresh or not _agent_key_is_usable(_entry_state(entry), _wundercorp_min_key_ttl_seconds()):
        try:
            refreshed = pool.try_refresh_current()
        except Exception as exc:
            logger.debug("Auxiliary WunderCorp pool refresh failed: %s", exc)
            refreshed = None
        if refreshed is None:
            return None
        entry = refreshed
    api_key = _wundercorp_api_key(_entry_state(entry))
    base_url = _pool_runtime_base_url(entry, _WUNDERCORP_DEFAULT_BASE_URL)
    if not api_key or not base_url:
        return None
    return api_key, base_url


def _resolve_wundercorp_runtime_api(
    *, force_refresh: bool = False, stale_access_token: Optional[str] = None
) -> Optional[tuple[str, str]]:
    """Fresh WunderCorp runtime credentials (pool first, then auth store + JWT refresh) — mirrors the main
    ...
    pooled = _resolve_wundercorp_pool_runtime_api(force_refresh=force_refresh)
    if pooled is not None:
        return pooled
    try:
        from loki_cli.auth import resolve_wundercorp_runtime_credentials
        creds = resolve_wundercorp_runtime_credentials(
            timeout_seconds=env_float("LOKI_WUNDERCORP_TIMEOUT_SECONDS", 15),
            force_refresh=force_refresh,
            stale_access_token=stale_access_token or None,
        )
    except Exception as exc:
        logger.debug("Auxiliary WunderCorp runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    """
    try:
        from loki_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL, _xai_validate_inference_base_url
        ...
        if pool and pool.has_credentials():
            ...
            if entry is not None:
                ...
                base_url = _xai_validate_inference_base_url(
                    _url(os.getenv("LOKI_XAI_BASE_URL", ""))
    ...
    try:
        from loki_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    ...
    if pool_present:
        ...
        if token:
            return token
    try:
        from loki_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from loki_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            with contextlib.suppress(ImportError):
                from loki_cli.auth import is_provider_explicitly_configured
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            from loki_cli.models import copilot_default_headers
...
def _endpoint_default_headers(
    ...
    if base_url_host_matches(base_url, "api.kimi.com"):
        ...
        from loki_cli.copilot_auth import copilot_request_headers
        ...
        from tools.xai_http import loki_xai_default_headers
        headers = dict(loki_xai_default_headers())
    else:
...
def _aux_openrouter_settings() -> Tuple[bool, str]:
    ...
    try:
        from loki_cli.config import cfg_get, load_config_readonly
...
def _try_wundercorp(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Cross-session rate guard: another session's 429 means skip WunderCorp rather than pile onto the tapped RPH bucket.
    with contextlib.suppress(Exception):
        from agent.wundercorp_rate_guard import wundercorp_rate_limit_remaining
        _remaining = wundercorp_rate_limit_remaining()
        if _remaining is not None and _remaining > 0:
            logger.debug("Auxiliary: skipping WunderCorp Portal (rate-limited, resets in %.0fs)", _remaining)
            _mark_provider_unhealthy("wundercorp", ttl=_remaining)
            return None, None
    wundercorp = _read_wundercorp_auth()
    runtime = _resolve_wundercorp_runtime_api(force_refresh=False)
    if runtime is None and not wundercorp:
        logger.warning("Auxiliary WunderCorp client unavailable: no WunderCorp authentication found (run: loki auth).")
        _mark_provider_unhealthy("wundercorp", ttl=60)
        return None, None
    if runtime is None and wundercorp:
        logger.debug("Auxiliary WunderCorp: runtime JWT refresh failed; checking stored auth.json token.")
    if runtime is not None:
        api_key, base_url = runtime
    else:
        api_key = _wundercorp_api_key(wundercorp or {})
        if not api_key:
            logger.warning(
                "Auxiliary WunderCorp client unavailable: no usable inference JWT found "
                "(run: loki auth add wundercorp)."
            )
            _mark_provider_unhealthy("wundercorp", ttl=60)
            return None, None
        base_url = str(
            (wundercorp or {}).get("inference_base_url") or os.getenv("WUNDERCORP_INFERENCE_BASE_URL", _WUNDERCORP_DEFAULT_BASE_URL)
        ).rstrip("/")
    ...
    from loki_cli.anon_auth import GUEST_MODEL, route_is_welcome_host
    global auxiliary_is_wundercorp
    if route_is_welcome_host(base_url):
        auxiliary_is_wundercorp = True
        logger.debug("Auxiliary/%s: WunderCorp free tier; using %s", lane, GUEST_MODEL)
        return _create_openai_client(api_key=api_key, base_url=base_url), GUEST_MODEL
    auxiliary_is_wundercorp = True
    logger.debug("Auxiliary client: WunderCorp Portal")
    # Portal recommended-models is authoritative (tier-aware); _WUNDERCORP_MODEL when unreachable/null.
    # Probes skip the lookup: exact model is irrelevant and it hits the network.
    model = _WUNDERCORP_MODEL
    if not _aux_probe_active():
        try:
            from loki_cli.models import get_wundercorp_recommended_aux_model
            recommended = get_wundercorp_recommended_aux_model(vision=vision)
            if recommended:
                model = recommended
...
def _refresh_wundercorp_recommended_model(*, vision: bool, stale_model: Optional[str]) -> Optional[str]:
    """Fresh Portal recommended model after a stale-model 404 (long-lived processes pin dropped models).

    Returns the fresh recommendation, else ``_WUNDERCORP_MODEL``, whichever differs from ``stale_model``; None if neither.
    """
    stale = (stale_model or "").strip().lower()
    fresh: Optional[str] = None
    try:
        from loki_cli.models import get_wundercorp_recommended_aux_model
        fresh = get_wundercorp_recommended_aux_model(vision=vision, force_refresh=True)
    except Exception as exc:
        logger.debug("WunderCorp recommended-model refresh failed (%s); using default %s", exc, _WUNDERCORP_MODEL)
    if fresh and fresh.strip().lower() != stale:
        return fresh
    return _WUNDERCORP_MODEL if _WUNDERCORP_MODEL.strip().lower() != stale else None
...
def _read_main_field(field: str, *, readonly: bool, lower: bool = False) -> str:
    ...
    with contextlib.suppress(Exception):
        from loki_cli import config as _cfg_mod
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        ...
        from loki_cli.moa_config import resolve_moa_preset
...
def _resolve_custom_runtime() -> Tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    try:
        from loki_cli.runtime_provider import resolve_runtime_provider
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `loki setup` or `loki model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key, base_url=base_url, default_headers=loki_xai_default_headers()
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    *, model: Optional[str] = None, explicit_api_key: Optional[str] = None,
    ...
    try:
        from loki_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _get_provider_chain() -> List[tuple]:
    ...
    """
    return [
        ("openrouter", _try_openrouter), ("wundercorp", _try_wundercorp),
...
_AUX_UNHEALTHY_LABEL_ALIASES = {
    "openrouter": "openrouter", "wundercorp": "wundercorp", "custom": "local/custom",
...
def _wundercorp_portal_account_has_fresh_paid_access() -> bool:
    """Return True only when the fresh WunderCorp account API says paid access is allowed."""
    try:
        from loki_cli.wundercorp_account import get_wundercorp_portal_account_info
        return get_wundercorp_portal_account_info(force_fresh=True).paid_service_access is True
    except Exception as exc:
        logger.debug("Auxiliary WunderCorp paid-entitlement refresh check failed: %s", exc)
        return False
...
_POOL_PROVIDER_BY_HOST = (
    ...
    ("inference-api.wundercorp.com", "wundercorp"), ("api.anthropic.com", "anthropic"),
...
_AUTH_REFRESH_PROVIDER_BY_HOST = (
    ...
    ("api.anthropic.com", "anthropic"), ("inference-api.wundercorp.com", "wundercorp"),
)
...
def _refresh_copilot_credentials() -> bool:
    from loki_cli.copilot_auth import _jwt_cache, _token_fingerprint, exchange_copilot_token, resolve_copilot_token
...
def _refresh_codex_credentials() -> bool:
    from loki_cli.auth import resolve_codex_runtime_credentials
    return _creds_have_api_key(resolve_codex_runtime_credentials(force_refresh=True))


def _refresh_wundercorp_credentials() -> bool:
    from loki_cli.auth import resolve_wundercorp_runtime_credentials
    return _creds_have_api_key(resolve_wundercorp_runtime_credentials(
        timeout_seconds=env_float("LOKI_WUNDERCORP_TIMEOUT_SECONDS", 15), force_refresh=True
    ))
...
_CREDENTIAL_REFRESHERS: Dict[str, Callable[..., bool]] = {
    ...
    "wundercorp": _refresh_wundercorp_credentials, "anthropic": _refresh_anthropic_credentials,
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_loki_fallback_destination", None)
...
def _replan_synchrowundercorp_cache_sections(
    messages: list, tools: Optional[list], *, destination: _FallbackDestination
) -> tuple[list, list]:
    """Strip source decoration and plan one synchrowundercorp destination locally."""
...
def _fallback_request_kwargs(
    ...
    fallback_messages, fallback_tools = _replan_synchrowundercorp_cache_sections(messages, tools, destination=destination)
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    from loki_cli.fallback_config import resolve_entry_api_key
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        with contextlib.suppress(Exception):
            client._loki_fallback_destination = _fallback_destination_from_entry(entry, client, resolved_model)
...
def _try_main_fallback_chain(
    ...
    try:
        from loki_cli.config import load_config_readonly
        from loki_cli.fallback_config import get_fallback_chain
...
def _warn_stale_openai_base_url(runtime_provider: str) -> None:
    ...
    ~/.loki/.env value after `loki model` poisons routing)."""
    ...
    if (_env_base and _cfg_provider and _cfg_provider != "custom" and not _cfg_provider.startswith("custom:")):
        logger.warning(
            ...
            "Run: loki model to reconfigure, or remove "
            "OPENAI_BASE_URL from ~/.loki/.env",
...
def _try_main_provider_route(
    ...
    if runtime_base_url and main_provider == "custom":
        ...
        with contextlib.suppress(ImportError):
            from loki_cli.runtime_provider import _get_named_custom_provider
...
def _discovery_chain_allowed(main_provider: str, task: Optional[str] = None) -> bool:
    ...
    compression silently charged to a WunderCorp Portal balance)."""
    ...
    logger.warning(
        ...
        "configured — refusing to guess another logged-in provider. Re-authenticate (`loki model`) "
...
def _resolve_auto_route(
    ...
    top-level chain; (3) OpenRouter → WunderCorp → custom → Codex → API-key providers, only with no policy
    and no working main client."""
    global auxiliary_is_wundercorp
    auxiliary_is_wundercorp = False  # Reset — _try_wundercorp() will set True if it wins
    runtime = _normalize_main_runtime(main_runtime)
...
def _effective_provider_for_client(client: Any, fallback: str) -> str:
    ...
    effective_provider = getattr(client, "_loki_aux_effective_provider", "")
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if _client_declares(sync_client, "LOKI_SKIP_ASYNC_WRAP"):
        return sync_client, model
    ...
    # Loki owns the auxiliary retry/timeout budget; disable SDK-internal retries.
    # See #54465.
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from loki_cli.model_normalize import normalize_model_for_provider
...
def _is_actual_auxiliary_route(req: _ResolveRequest, base_url: str) -> bool:
    from loki_cli.auth import normalize_actual_base_url
    from loki_cli.providers import is_actual_route
    from loki_cli.route_identity import normalize_route_base_url
...
def _wrap_transport(req: _ResolveRequest, client_obj: Any, final_model_str: str,
    ...
    if _is_actual_auxiliary_route(req, base_url_str):
        ...
        client._loki_aux_effective_provider = "actual"
        return client
    needs_codex = not (
...
def _resolve_auto_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if routed_client is not None and effective_provider:
        try:
            setattr(routed_client, "_loki_aux_effective_provider", effective_provider)
...
def _resolve_wundercorp_branch(req: _ResolveRequest) -> _ResolveResult:
    """WunderCorp Portal (OAuth)."""
    model = req.model
    # Vision: caller flag, _PROVIDER_VISION_MODELS override, or a known vision id.
    client, default = _try_wundercorp(vision=(req.is_vision or model in _PROVIDER_VISION_MODELS.values()
                                        or (model or "").strip().lower() == "mimo-v2-omni"))
    if client is None:
        logger.warning("resolve_provider_client: wundercorp requested but WunderCorp Portal not configured (run: loki auth)")
        return None, None
    ...
    from loki_cli.providers import wundercorp_api_mode
    client = _maybe_wrap_anthropic(
        client, final_model, str(getattr(client, "api_key", "") or ""),
        str(getattr(client, "base_url", "") or ""), wundercorp_api_mode(final_model),
    )
...
def _resolve_openai_codex_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if not model:
        ...
        return None, None
    no_token_msg = "resolve_provider_client: openai-codex requested but no Codex OAuth token found (run: loki model)"
    if req.raw_codex:
...
def _resolve_xai_oauth_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    return _route_or_warn(req, client, default,
                          ...
                          "OAuth token found (run: loki model -> xAI Grok OAuth — SuperGrok / Premium+)")
...
def _resolve_named_custom_branch(req: _ResolveRequest) -> Optional[_ResolveResult]:
    ...
    # canonical name (e.g. ``wundercorp``) still defer to the built-in.
    custom_entry = None
...
def _resolve_azure_foundry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    return _route_or_warn(req, client, default_model,
                          ...
                          "runtime resolution failed (run: loki doctor for diagnostics)")
...
def _resolve_api_key_branch(req: _ResolveRequest, pconfig: Any, resolve_creds: Callable) -> _ResolveResult:
    ...
    try:
        from loki_cli.models import opencode_zen_free_runtime as _oc_free_rt
    ...
    if provider == "copilot" and final_model and not req.raw_codex:
        with contextlib.suppress(ImportError):
            from loki_cli.models import _should_use_copilot_responses_api
...
def _resolve_registry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    try:
        ...
        logger.debug("loki_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if auth_type == "vertex":
        ...
        # wundercorp / openai-codex / xai-oauth already returned from their explicit branches.
...
_EXPLICIT_PROVIDER_BRANCHES: Dict[str, Callable[[_ResolveRequest], _ResolveResult]] = {
    ...
    "wundercorp": _resolve_wundercorp_branch,
...
def resolve_provider_client(
    ...
    # Excluded: ``auto`` (a stale main slug could pair with any picked provider) and WunderCorp + vision (the
    # Portal's tier-aware vision recommendation must win over a text-only model).
    if not model and provider != "auto" and not (provider == "wundercorp" and is_vision):
        ...
        # not explicitly request one. (# compression-current-model) WunderCorp + vision is the one carve-out: the
        # branch below resolves its model from the Portal's tier-aware vision recommendation
        # (``_try_wundercorp(vision= True)``), and ``final_model = model or default`` means anything pre-filled
...
_VISION_AUTO_PROVIDER_ORDER = ("openrouter", "wundercorp", "deepinfra")
...
# Strict (explicitly requested) vision backends by normalized provider name. wundercorp MUST go
# through resolve_provider_client so anthropic/* picks wrap onto /v1/messages (a bare _try_wundercorp
...
_STRICT_VISION_BACKENDS: Dict[str, Callable[[Optional[str]], Tuple[Optional[Any], Optional[str]]]] = {
    ...
    "wundercorp": lambda model: resolve_provider_client("wundercorp", model, is_vision=True),
...
def get_available_vision_backends() -> List[str]:
    """Available vision backends in auto-selection order (active provider → OpenRouter → WunderCorp → DeepInfra).
...
def _vision_main_provider_client(
    ...
    if main_provider == "wundercorp":
        # WunderCorp picks its vision model from Portal tier-aware slots inside _try_wundercorp(vision=True);
...
def _vision_auto_route(
    ...
    """Auto-detect order: 1. main provider + model, 2. OpenRouter, 3. WunderCorp Portal, 4. DeepInfra, 5. stop."""
...
def get_auxiliary_extra_body() -> dict:
    """Return extra_body kwargs (WunderCorp Portal product tags when WunderCorp-backed, else {})."""
    return _wundercorp_extra_body() if auxiliary_is_wundercorp else {}
...
def auxiliary_max_tokens_param(value: int, *, model: Optional[str] = None) -> dict:
    ...
    direct_openai_family = (
        not _scoped_key_env("OPENROUTER_API_KEY") and _read_wundercorp_auth() is None
...
def _refresh_wundercorp_auxiliary_client(
    *, cache_provider: str, model: Optional[str], async_mode: bool, base_url: Optional[str] = None,
    ...
    """Refresh WunderCorp runtime creds, rebuild the client, and replace the cache entry.
    ...
    """
    runtime = _resolve_wundercorp_runtime_api(force_refresh=True, stale_access_token=api_key)
    if runtime is None:
        return None, model
...
def _preserve_provider_with_base_url(prov: Optional[str]) -> bool:
    ...
    if normalized in {"", "auto", "custom"} or normalized.startswith("custom:"):
        return False
    try:
        from loki_cli.providers import get_provider
        ...
        return normalized in {
            "anthropic", "copilot", "copilot-acp", "minimax-oauth", "wundercorp", "openai-codex", "qwen-oauth", "xai-oauth",
        }
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    if not isinstance(task_config, dict):
        task_config = {}
    try:
        from loki_cli.plugins import get_plugin_auxiliary_tasks
...
def _fast_lane_config_fields(config: Dict[str, Any]) -> tuple[str, str, bool]:
    ...
    from loki_constants import parse_reasoning_effort
...
_WUNDERCORP_PROVIDER_NAMES = frozenset({"wundercorp", "wundercorp-portal", "wundercorp"})


def _wundercorp_on_messages_wire(provider_norm: str, model: str) -> bool:
    """True when a WunderCorp Portal route serves ``model`` over /v1/messages (dual-wire catalog)."""
    if provider_norm not in _WUNDERCORP_PROVIDER_NAMES:
        return False
    from loki_cli.providers import wundercorp_api_mode
    return wundercorp_api_mode(model) == "anthropic_messages"
...
def _forwards_max_tokens(provider: str, provider_norm: str, model: str, effective_base: str, task: Optional[str]) -> bool:
    ...
    return (
        ...
        or _wundercorp_on_messages_wire(provider_norm, model)
...
def _merge_aux_extra_body(
    ...
    """Caller extra_body + profile body/reasoning + generic reasoning fallback + WunderCorp tags."""
    ...
    if provider_norm in _WUNDERCORP_PROVIDER_NAMES:
        if "tags" not in merged_extra:
            merged_extra["tags"] = _wundercorp_portal_tags()
...
def _build_call_kwargs(
    ...
    if reasoning_config and isinstance(reasoning_config, dict):
        ...
        if (
            provider_norm == "anthropic" or _wundercorp_on_messages_wire(provider_norm, model)
...
def _managed_local_netloc() -> str:
    ...
    if now - ts < _MANAGED_LOCAL_STATE_TTL_S:
        return cached
    try:
        from loki_cli.local_runtime.supervisor import state_path
...
def _is_managed_local_endpoint(base_url: Optional[str]) -> bool:
    """True when *base_url* targets the llama-server this Loki manages."""
    if not base_url:
        return False
...
def _resolve_call_client(
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is None:
                    raise RuntimeError(
                        ...
                        f"a different provider with `loki model`.")
    ...
    if client is None:
        raise RuntimeError(f"No LLM provider configured for task={task} "
                           f"provider={resolved_provider}. Run: loki setup")
...
def _refreshed_wundercorp_step(route: _LadderRoute, kwargs: Dict[str, Any], message: str) -> Optional[_LadderStep]:
    """Rebuild the WunderCorp client after a credential event; None when nothing refreshed."""
    refreshed_client, refreshed_model = _refresh_wundercorp_auxiliary_client(
        cache_provider=route.resolved_provider or "wundercorp", model=route.final_model,
...
def _ladder_wundercorp_rungs(
    first_err: Exception, route: _LadderRoute, kwargs: Dict[str, Any], client_is_wundercorp: bool,
):
    """WunderCorp-only rungs: stale-model self-heal, paid-account refresh, 401 refresh.
    ...
    if _is_model_not_found_error(first_err) and client_is_wundercorp:
        healed_model = _refresh_wundercorp_recommended_model(
            vision=(task == "vision"), stale_model=kwargs.get("model"))
        if healed_model and healed_model != kwargs.get("model"):
            logger.warning("Auxiliary %s%s: model %r no longer in WunderCorp catalog; "
    ...
    if _is_payment_error(first_err) and client_is_wundercorp and _wundercorp_portal_account_has_fresh_paid_access():
        step = _refreshed_wundercorp_step(
            route, kwargs,
            "Auxiliary %s%s: refreshed WunderCorp runtime credentials after paid account check, retrying")
        if step is not None:
            resp, first_err = yield from _rung(
                step, lambda exc: _credential_rung_accepts(exc) or _is_connection_error(exc))
            if first_err is None:
                return resp, None
    if _is_auth_error(first_err) and client_is_wundercorp:
        step = _refreshed_wundercorp_step(
            route, kwargs, "Auxiliary %s%s: refreshed WunderCorp runtime credentials after 401, retrying")
        if step is not None:
...
def _aux_recovery_ladder(
    ...
    strips → WunderCorp heal/refresh → credential refresh/pool rotation → provider fallback.
    ...
    if first_err is None:
        return resp
    client_is_wundercorp = (resolved_provider == "wundercorp"
                      or base_url_host_matches(base_info, "inference-api.wundercorp.com"))
    resp, first_err = yield from _ladder_wundercorp_rungs(first_err, route, kwargs, client_is_wundercorp)
    if first_err is None:
        return resp
    resp, first_err = yield from _ladder_credential_rungs(first_err, route, kwargs, client_is_wundercorp)
    if first_err is None:
        return resp
...
def _call_llm_impl(
    ...
    """Centralized synchrowundercorp LLM call: resolve provider/model, auth, kwargs, fallbacks.
...
async def async_call_llm(
    ...
    """Run an asynchrowundercorp auxiliary LLM request under the configured limit."""
...
async def _async_call_llm_impl(
    ...
    """Centralized asynchrowundercorp LLM call; see call_llm() for full documentation.
...
WUNDERCORP_EXTRA_BODY = _wundercorp_extra_body()
