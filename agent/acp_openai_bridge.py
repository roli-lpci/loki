"""OpenAI-shape bridge shared by Loki' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Loki' tool schemas travel INTO the
...
read/edit/execute tools forwards only Loki' agent-level tools, since re-offering overlapping ones
makes Loki redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``loki_projected_messages``)
    Loki reads off the ``create`` result; a plain list would drop them on the stream path."""
