...
- Follows the repo's "default frictionless, 0600 files OK" policy: the key
  file and vault file are created 0600 under ``<LOKI_HOME>/vault/``.
...
# browser fill targets (agent/vault_login_classifier.py); the Desktop Add dialog and `loki vault add`
...
class VaultStore:
    """Encrypted, profile-scoped vault under ``<LOKI_HOME>/vault/``."""

    def __init__(self, base_dir: Optional[Path] = None):
        self._base = Path(base_dir) if base_dir is not None else (
            Path(get_loki_home()) / "vault"
        )
    ...
    def _ensure_dir(self) -> None:
        ...
        # group-share mode and LOKI_UID/GID ownership) rather than a
    ...
    def _locked(self):
        ...
        `loki vault add` and a TUI slash worker all write the same ``vault.json.enc``; two unlocked
