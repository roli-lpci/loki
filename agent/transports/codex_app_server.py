...
class CodexAppServerClient:
    """Minimal synchrowundercorp JSON-RPC 2.0 client for ``codex app-server`` over stdio.
    ...
    AIAgent.run_conversation() is synchrowundercorp and cancels via ``turn/interrupt``.
    """
    ...
    def __init__(
        ...
        # codex needs LLM provider creds but must not receive Tier-1 Loki secrets (gateway/GitHub/infra tokens).
        ...
        # `os.environ.copy()` also handed it every Tier-1 Loki secret — gateway bot tokens, GitHub auth,
        ...
        spawn_env = loki_subprocess_env(inherit_credentials=True)
        if env:
        ...
        # Native shell children remain unowned. Only Loki' managed MCP tool
        ...
        owned_task = os.environ.get("LOKI_KANBAN_TASK") and is_dispatcher_owned_worker_context()
        if owned_task:
            for key in (*KANBAN_ENV_KEYS, "LOKI_KANBAN_DB", "LOKI_KANBAN_BOARD"):
                if key in os.environ:
                    cmd += ["-c", f"mcp_servers.loki-mcp.env.{key}={json.dumps(os.environ[key])}"]
            cmd += ["-c", f'mcp_servers.loki-mcp.env.{DELEGATED_CHILD_ENV_MARKER}=""']
        ...
        if owned_task:
            kanban_db = spawn_env.get("LOKI_KANBAN_DB")
            default_root = os.path.join(spawn_env.get("LOKI_HOME", os.path.expanduser("~/.loki")), "kanban")
            kanban_root = os.path.dirname(kanban_db) if kanban_db else spawn_env.get("LOKI_KANBAN_ROOT", default_root)
            cmd += [
    ...
    def initialize(
        self, client_name: str = "loki", client_title: str = "Loki Agent",
