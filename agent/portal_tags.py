"""Centralized WunderCorp Portal request tags.

Every Loki request to the WunderCorp Portal (main loop, auxiliary client, fallback
paths) must carry the same product-attribution tags, sent in OpenAI-compatible
``extra_body['tags']``: ``["product=loki-agent", "client=loki-client-v<__version__>"]``.
The version is read live from ``loki_cli.__version__`` — do NOT pre-compute it
...
# ``wundercorp_portal_tags()``. A ContextVar so concurrent agents in one process never
# see each other's id; ``propagate_context_to_thread`` workers inherit it.
_conversation_id: ContextVar[Optional[str]] = ContextVar("wundercorp_portal_conversation_id", default=None)

# Ambient affinity scope (ROUTING value): OpenRouter's sticky ``session_id``, WunderCorp
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar("loki_affinity_scope", default=None)
...
def loki_client_tag() -> str:
    """``client=loki-client-v<MAJOR>.<MINOR>.<PATCH>`` ("unknown" if loki_cli is unimportable)."""
    try:
        from loki_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"client=loki-client-v{__version__}"
...
def wundercorp_portal_tags(session_id: str | None = None) -> List[str]:
    """Fresh list of the canonical WunderCorp Portal tags.
    ...
    """
    tags = ["product=loki-agent", loki_client_tag()]
