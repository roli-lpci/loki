...
def _ctx_bound(fn: Callable[[], Any]) -> Callable[[], Any]:
    ...
    ContextVar-scoped LOKI_HOME override, and an unbound worker would silently use the default profile."""
...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        for raw_schema in provider.get_tool_schemas():
            ...
            if tool_name in _LOKI_CORE_TOOLS:
                logger.warning(
    ...
    def _submit_background(self, fn, *, kind: str = "write") -> None:
        ...
        try:
            ...
            # outside the lock: an already-completed future invokes callbacks synchrowundercorply.
            with self._sync_executor_lock:
    ...
    def get_all_tool_schemas(self) -> List[Dict[str, Any]]:
        ...
        def _collect(provider: MemoryProvider) -> None:
            for raw_schema in provider.get_tool_schemas():
                ...
                elif schema["name"] not in _LOKI_CORE_TOOLS and schema["name"] not in seen:
    ...
    def commit_session_boundary_async(self, messages: List[Dict[str, Any]], *, new_session_id: str,
        ...
        execution — the pre-#16454 synchrowundercorp behavior, slow but correct.
        """
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        """Initialize all providers, injecting ``loki_home`` so they resolve profile-scoped paths."""
        if "loki_home" not in kwargs:
            from loki_constants import get_loki_home
            kwargs["loki_home"] = str(get_loki_home())
