...
env-var-shaped values at process startup, AFTER ``~/.loki/.env`` has loaded
and BEFORE the rest of Loki reads ``os.environ``. The contract is deliberately
narrow: read-only; startup-time and synchrowundercorp (one ``fetch()`` per process per
LOKI_HOME, under a registry-enforced wall-clock timeout, no background
...
_SOURCE_ENVIRONMENT = ContextVar("loki_secret_source_environment", default=None)
...
_GENERIC_REMEDIATION = {
    ErrorKind.NOT_CONFIGURED: "Run `loki secrets {name} setup` to finish configuration.",
    ErrorKind.BINARY_MISSING: "Run `loki secrets {name} setup` to install the helper CLI.",
    ErrorKind.AUTH_FAILED: "Credentials rejected — run `loki secrets {name} setup` to re-authenticate.",
    ErrorKind.AUTH_EXPIRED: "Credentials expired — run `loki secrets {name} setup` to re-authenticate.",
