...
a cache problem must never block Loki startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """``home_path`` as resolved by ``load_loki_dotenv()``, else ``$LOKI_HOME``/``~/.loki``."""
    if home_path is None:
        from loki_constants import get_loki_home

        home_path = get_loki_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend at ``<loki_home>/cache/<basename>``::
