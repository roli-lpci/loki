...
``loki pause`` writes a sentinel at ``$LOKI_HOME/ESTOP``; ``loki resume``
...
(fail safe, e.g. ``touch ~/.loki/ESTOP``). Ported from gastownhall/gastown estop.go (MIT).
"""
...
# Same profile-aware / fleet-root resolvers the file-safety guards use (fail-open to ~/.loki).
from agent.file_safety import _loki_home_path as _loki_home, _loki_root_path as _canonical_root
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `loki pause`."""
    return _loki_home() / SENTINEL_NAME


def _candidate_sentinel_paths() -> list:
    """Profile home first, then the fleet root if it is a different directory: a profile
    gateway (LOKI_HOME=~/.loki/profiles/<n>) must still honor an operator's ~/.loki/ESTOP."""
...
def paused_reply() -> Optional[str]:
    ...
    return f"⏸️ Loki is paused{tag}. New work is on hold; run `loki resume` to pick things back up."
...
def check_paused(component: str, logger: logging.Logger) -> bool:
    ...
    if first:
        ...
        logger.info(
            "%s dispatch paused by global emergency stop%s — remove with `loki resume` (%s)",
