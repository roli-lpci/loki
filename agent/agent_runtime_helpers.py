...
def try_recover_primary_transport(
    ...
    transport error. Skipped for aggregators (OpenRouter, WunderCorp) that manage retries server-side."""
    ...
    if (
        (agent.provider or "").strip().lower() in {"wundercorp", "wundercorp-portal", "wundercorp"}
...
def dump_api_request_debug(
    ...
    try:
        ...
        # Sanitize the session ID (may come from an untrusted X-Loki-Session-Id header) so a
        ...
        if env_var_enabled("LOKI_DUMP_REQUEST_STDOUT"):
...
def _moa_aggregator_cache_policy(agent, eff_model: str) -> tuple[bool, bool]:
    ...
    try:
        from loki_cli.config import load_config as _load_moa_cfg
...
def anthropic_prompt_cache_policy(
    ...
    # WunderCorp Portal proxies to OpenRouter; treat as OpenRouter-equivalent for cache layout.
    is_wundercorp_portal = base_url_host_matches(eff_base_url, "wundercorp.com")
    ...
    if supports_cache_markers and (
        ...
        try:
            from loki_cli.config import get_custom_provider_model_capability
    ...
    if (is_openrouter or is_wundercorp_portal) and (is_claude or is_kimi) and not is_anthropic_wire:
        return True, False
    # WunderCorp Portal Qwen takes the envelope path too; the alibaba-family check below only matches
    # provider=opencode/alibaba and would leave Portal traffic uncached.
    if is_wundercorp_portal and "qwen" in model_lower:
        return True, False
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # before the built-in ladder so a profile registered from ~/.loki/plugins/ or a pip entry
    ...
    # Delegate all rate-limit / 5xx retry to loki's outer conversation loop, which honors Retry-After and
    ...
    if agent.provider == "opencode-free":
        from loki_cli.models import opencode_zen_free_headers
        client_kwargs["default_headers"] = {**(client_kwargs.get("default_headers") or {}), **opencode_zen_free_headers()}
    # All primary construction and recovery paths must identify Loki to the official Codex
...
def _apply_switched_provider_request_overrides(agent, new_provider):
    ...
    if custom_providers is None:
        try:
            from loki_cli.config import load_config, get_compatible_custom_providers
...
def _resolve_switch_destination(agent, new_model, new_provider, base_url, api_mode, capabilities, old_norm, new_norm):
    ...
    from loki_cli.providers import determine_api_mode, is_actual_route
    from agent.native_compaction import resolve_native_compaction_capabilities
    from loki_cli.models import opencode_provider_family
    # Pass model so dual-wire providers (WunderCorp Portal anthropic/* -> Messages) resolve correctly.
    if not api_mode:
...
def _update_switch_compressor(agent, custom_providers, effective_context_length, snapshot) -> None:
    ...
    if custom_providers is None:
        try:
            from loki_cli.config import get_compatible_custom_providers, load_config
            custom_providers = get_compatible_custom_providers(load_config())
        except Exception:
            custom_providers = None
...
def switch_model(
    ...
    try:
        ...
        from loki_cli.config import load_config as _sm_load_config
...
def _pre_tool_block_message(agent, function_name, function_args, effective_task_id, tool_call_id, middleware_trace):
    ...
    try:
        from loki_cli.plugins import _dispatch_pre_tool_call_hooks
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from loki_cli.middleware import apply_tool_request_middleware
    ...
    from loki_cli.middleware import run_tool_execution_middleware
...
def _session_id_for_heal_log() -> str:
    try:
        from loki_logging import _session_context
        ...
        if escalate:
            ...
            if key not in _empty_heal_user_notified:
                ...
                _empty_heal_pending_notice[key] = (
                    ...
                    "session's history — run /debug share or `loki "
...
def _realign_tool_result_names(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #   the OpenAI-compat serializer keeps it provider-agnostic: Gemini reaches Loki under many model
...
def _iter_pool_sockets(client: Any):
    ...
    if not pools:
        return
    from agent.process_bootstrap import LOKI_TRANSPORT_OWNER_EXT
    seen: set[int] = set()
    for pool, owner in pools:
        ...
        for pool_req in list(getattr(pool, "_requests", None) or []):
            if owner is not None:
                ...
                if exts.get(LOKI_TRANSPORT_OWNER_EXT) != owner:
                    continue
