...
from loki_cli.auth import AuthError, _read_codex_tokens, resolve_codex_runtime_credentials
...
def _wundercorp_snapshot(windows: list, details: list, tail: list, *, source: str, plan: Optional[str] = None) -> Optional[AccountUsageSnapshot]:
    """WunderCorp snapshot with *tail* lines appended, or None when there is nothing to show."""
    if not windows and not details:
        return None
    return _snapshot("wundercorp", source, windows, details + tail, title="WunderCorp credits", plan=plan)


def build_wundercorp_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    """WunderCorpPortalAccountInfo → /usage snapshot: dollar magnitudes + renewal date + portal CTA, plus a ``% used``
    gauge when the portal supplies ``monthly_credits``. Fail-open → None."""
    try:
        from loki_cli.wundercorp_account import wundercorp_portal_topup_url
        ...
        return _wundercorp_snapshot(windows, details, [f"Top up: {wundercorp_portal_topup_url(account_info)}", "(or run /topup)"],
                              source="portal-account", plan=getattr(sub, "plan", None) if sub is not None else None)
    except (AttributeError, TypeError):
        return None


def _wundercorp_logged_in() -> bool:
    """Cheap local auth-state check: a WunderCorp access token is present. Fail-open False."""
    try:
        from loki_cli.auth import get_provider_auth_state
        tok = (get_provider_auth_state("wundercorp") or {}).get("access_token")
...
def _fetch_portal_account(timeout: float):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
        return pool.submit(get_wundercorp_portal_account_info, force_fresh=True).result(timeout=timeout)


def wundercorp_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    """Rendered WunderCorp-credits /usage lines, or [] when there's nothing to show. Independent of any live agent
    (logged-in gate, then a bounded portal fetch); shared by CLI ``_show_usage`` and the TUI ``session.usage`` RPC.
    Fail-open: any hiccup or timeout → []. LOKI_DEV_CREDITS_FIXTURE renders from the fixture instead of the portal."""
    try:
    ...
    if not _wundercorp_logged_in():
        return []
    try:
        snapshot = build_wundercorp_credits_snapshot(_fetch_portal_account(timeout))
...
def _snapshot_from_credits_state(state) -> Optional[AccountUsageSnapshot]:
    ...
    try:
        ...
        if getattr(state, "paid_access", True) is False:
            details.append(_DEPLETED_LINE)
        return _wundercorp_snapshot(windows, details, ["(dev fixture — LOKI_DEV_CREDITS_FIXTURE)"], source="dev-fixture")
    except (AttributeError, TypeError):
        return None
...
def build_credits_view(*, markdown: bool = False, timeout: float = 10.0) -> CreditsView:
    ...
    balance_lines = [
        line
        for line in render_account_usage_lines(build_wundercorp_credits_snapshot(account), markdown=markdown)
    ...
    return CreditsView(
        ...
        identity_line=("Topping up as " + " / ".join(who)) if who else None, topup_url=wundercorp_portal_topup_url(account),
...
def _codex_reset_outcome(body: dict, available: int) -> CodexResetRedeemResult:
    ...
    if code == "reset":
        ...
        try:
            from loki_cli.auth import clear_codex_pool_quota_cooldowns
...
def redeem_codex_reset_credit(
    *, base_url: Optional[str] = None, api_key: Optional[str] = None, force: bool = False,
    ...
    try:
        ...
        return _unavailable("No Codex credentials available. Run `loki auth` to sign in with your ChatGPT account.")
    ...
    try:
        ...
        if code in (401, 403):
            return _unavailable(f"Codex backend rejected the request (HTTP {code}). Reset credits require ChatGPT-account "
                                "(OAuth) auth — run `loki auth` and sign in with your ChatGPT account.")
