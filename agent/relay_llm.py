"""Core NeMo Relay adapters for physical Loki provider attempts."""
...
def _relay_operation_name(provider_name: str, metadata: dict[str, Any] | None) -> str:
    """Return Relay's canonical operation name when Loki knows the API mode."""
...
def _relay_metadata(provider_name: str, metadata: dict[str, Any] | None) -> dict[str, Any]:
    ...
    relay_metadata.setdefault("loki.provider", provider_name)
...
class _ManagedAttempt:
    ...
    def run_callback(self, callback: Callable[..., Any], *args: Any) -> Any:
        """Run a Loki callback in a fresh copy of the captured context.
        ...
        def guarded() -> Any:
            # See #77244.
            # See #77244.
            # Loki-side callbacks run while the native pipeline drives this stream; nested relay calls
...
def _current_session_id() -> str | None:
    """Return the inherited Loki turn's session id, or None outside a live turn."""
...
def execute(
    ...
    ``session_id`` defaults to the inherited Loki turn's session (unmanaged when there is none)."""
...
# Run under the inherited Loki turn when present (callers that do not know a session id).
...
def stream_current(
    ...
    """Run a provider stream under the inherited Loki turn when present.
    ...
    # iterated synchrowundercorply on that loop, which asyncio forbids; the outer stream tracks this attempt.
...
class ManagedLlmStream(Iterator[Any]):
    """Synchrowundercorp view of one Relay-managed provider stream, driven from the worker thread."""
...
class AnthropicStreamAccumulator:
    ...
    def response(self, base: Any = None) -> Any:
        """Return the attribute-shaped response consumed by Loki."""
        ...
        with turn.logical_llm_lock:
            ...
            if handle is None:
                ...
                handle = turn.logical_llm_calls[request_id] = runtime.run_in_session(
                    ...
                    metadata=relay_runtime.runtime_metadata(runtime.runtime_id, **{"loki.call_role": call_role}),
                )
...
def _complete_logical(
    ...
    with turn.finalize_lock:
        ...
        try:
            ...
            logger.warning("Loki Relay logical LLM finalization failed", exc_info=True)
            return
        with turn.logical_llm_lock:
...
def _run_awaitable(
    value: Any, *, loop_error: str = "Synchrowundercorp Relay LLM execution cannot run on an event-loop thread",
) -> Any:
