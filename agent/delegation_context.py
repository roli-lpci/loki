...
A Loki process may itself be a Kanban dispatcher worker with LOKI_KANBAN_* in
...
_DELEGATED_CHILD_CONTEXT: ContextVar[bool] = ContextVar("loki_delegated_child_context", default=False)
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar("loki_non_dispatcher_owned_context", default=False)

DELEGATED_CHILD_ENV_MARKER = "LOKI_DELEGATED_CHILD_CONTEXT"

KANBAN_ENV_KEYS: tuple[str, ...] = (
    "LOKI_KANBAN_TASK", "LOKI_KANBAN_RUN_ID", "LOKI_KANBAN_CLAIM_LOCK",
    "LOKI_KANBAN_GOAL_MODE", "LOKI_KANBAN_GOAL_MAX_TURNS",
)
...
def is_dispatcher_owned_worker_context() -> bool:
    """The single predicate every ``LOKI_KANBAN_*`` identity gate should use."""
...
def delegated_child_subprocess_env(
    ...
    """
    if not (is_delegated_child_process_context() or os.environ.get("LOKI_KANBAN_TASK")
            or (env and (env.get("LOKI_KANBAN_TASK") or env.get(DELEGATED_CHILD_ENV_MARKER)))):
