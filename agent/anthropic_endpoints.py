...
headers, and request quirks (MiniMax, Kimi/Moonshot, DeepSeek, OpenCode, Azure AI Foundry, WunderCorp
...
def _is_deepseek_anthropic_endpoint(base_url: str | None) -> bool:
    ...
    keep-unsigned policy used for Kimi's ``/coding`` endpoint. See loki-agent#16748.
    """
    return base_url_host_matches(base_url or "", "api.deepseek.com") and "/anthropic" in _normalized_lower(base_url)


def _is_wundercorp_portal_endpoint(base_url: str | None) -> bool:
    """WunderCorp Portal's Anthropic Messages route (Bearer JWT, verbatim catalog ids, native
    thinking-signature replay). Trusted hosts only: prod ``inference-api.wundercorp.com`` or the
    operator-set ``WUNDERCORP_INFERENCE_BASE_URL`` host (exact hostname equality, so neither lookalike
    domains nor sibling hosts of the override match)."""
    if base_url_host_matches(base_url or "", "inference-api.wundercorp.com"):
        return True
    try:
        from loki_cli.auth import _wundercorp_inference_env_override
        override = _wundercorp_inference_env_override()
    except Exception:
        return False
...
def _requires_bearer_auth(base_url: str | None) -> bool:
    ...
    Foundry, Palantir Foundry's LLM proxy, CommandCode, WunderCorp Portal. Palantir/CommandCode use
    hostname matching (not substring) so ``evil.com/palantirfoundry`` paths don't trigger it."""
    normalized = _normalized_lower(base_url)
    return (
        _is_wundercorp_portal_endpoint(base_url)
