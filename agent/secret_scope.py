...
_GLOBAL_ENV_EXACT = frozenset({
    # Loki runtime / deployment
    "LOKI_HOME", "LOKI_PROFILE", "LOKI_GATEWAY_LOCK_DIR",
    "LOKI_MAX_ITERATIONS", "LOKI_API_TIMEOUT",
    "LOKI_REDACT_SECRETS", "LOKI_WUNDERCORP_TIMEOUT_SECONDS",
    "_LOKI_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "LOKI_KANBAN_DB", "LOKI_KANBAN_WORKSPACES_ROOT", "LOKI_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "LOKI_KANBAN_",
    "LOKI_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    from loki_cli.config import _parse_env_value
...
def build_profile_secret_scope(loki_home: Path) -> Dict[str, str]:
    ...
    secrets = load_env_file(Path(loki_home) / ".env")
    try:
        from loki_cli.env_loader import get_secret_source_values
        external_secrets = get_secret_source_values(Path(loki_home))
    except Exception:
        external_secrets = {}
