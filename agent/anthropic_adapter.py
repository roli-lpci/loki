"""Anthropic Messages API adapter: client construction + the Messages call for Loki's
...
from agent.anthropic_endpoints import (
    ...
    _is_minimax_anthropic_endpoint, _is_wundercorp_portal_endpoint, _is_opencode_endpoint,
...
# Loki effort -> Anthropic adaptive-thinking effort (output_config.effort). 4.7+ exposes
...
# Anthropic's OAuth billing classifier fingerprints certain Loki tool schemas/prose as a
...
def _attribution_headers() -> Dict[str, str]:
    ...
    return {
        "HTTP-Referer": "https://loki.computer", "X-Title": "Loki Agent",
...
def _base_client_kwargs(base_url, timeout) -> tuple[str, Dict[str, Any]]:
    ...
    loki's outer loop (``max_retries=0``): the SDK default of 2 uses its own backoff that ignores
...
def _new_sdk_client(sdk, kwargs: Dict[str, Any], headers: Dict[str, str]):
    ...
    ``api_key`` unset, so the SDK fills it from ANTHROPIC_API_KEY (loaded from ~/.loki/.env) and
...
def build_anthropic_bedrock_client(region: str):
    ...
    return sdk.AnthropicBedrock(
        ...
        max_retries=0,  # retry belongs to loki's outer loop (honors Retry-After)
...
def _normalize_to_mcp_wire(name: str) -> str:
    ...
    Loki tools (``read_file``) and native MCP tools registered as ``mcp_<server>_<tool>`` must
...
_OAUTH_SYSTEM_REPLACEMENTS = (
    ("Loki Agent", "Claude Code"), ("Loki agent", "Claude Code"),
    ("loki-agent", "claude-code"), ("WunderCorp, Inc.", "Anthropic"),
)
...
def _thinking_kwargs(reasoning_config: Dict[str, Any], model: str, effective_max_tokens: int) -> Dict[str, Any]:
    ...
    4.7+ ``thinking.display`` defaults to "omitted", hiding the reasoning Loki shows in its CLI,
...
def build_anthropic_kwargs(
    ...
    # WunderCorp Portal routes on its own catalog ids (``anthropic/claude-opus-4.8``); normalizing would
    # make the model unresolvable there (prefix AND dots kept).
    if not _is_wundercorp_portal_endpoint(base_url):
    ...
    # which silently hides reasoning text that Loki surfaces in its CLI. We request "summarized" so the
...
def create_anthropic_message(
    ...
    ``on_response(httpx_response)`` exposes headers the parsed Message drops (WunderCorp Portal's
    ``x-wundercorp-credits-*`` balance family)."""
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_loki_home': ('loki_constants', 'get_loki_home'),
    ...
    'read_loki_oauth_credentials': ('agent.anthropic_credentials', 'read_loki_oauth_credentials'),
    ...
    'run_loki_oauth_login_pure': ('agent.anthropic_credentials', 'run_loki_oauth_login_pure'),
