...
YAML files in ``<LOKI_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``LOKI_BUNDLES_DIR`` override (tests) or ``<LOKI_HOME>/skill-bundles``."""
    override = os.environ.get("LOKI_BUNDLES_DIR")
    return Path(override).expanduser() if override else get_loki_home() / "skill-bundles"
...
# File-level CRUD — used by `loki bundles`.
