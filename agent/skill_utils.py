...
from loki_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_kanban() -> bool:
    ...
    if os.getenv("LOKI_KANBAN_TASK") or os.getenv("LOKI_KANBAN_BOARD"):
        try:
...
def _load_raw_config() -> Dict[str, Any]:
    """Read config.yaml with an mtime+size keyed cache (no loki_cli.config import)."""
...
def _home_relative(p: Path) -> Path:
    """Anchor a relative config path at LOKI_HOME; absolute paths pass through."""
    from loki_constants import get_loki_home
    return p if p.is_absolute() else get_loki_home() / p


# Never disableable: `loki-agent` is the agent's own operating manual and the
# system prompt points at it unconditionally.
ESSENTIAL_SKILLS: frozenset = frozenset({"loki-agent"})


def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    """Disabled skill names from config.yaml: global list ∪ platform list
    (*platform* defaults to ``LOKI_PLATFORM`` / ``LOKI_SESSION_PLATFORM``)."""
    skills_cfg = _skills_cfg()
    if skills_cfg is None:
        return set()
    from gateway.session_context import get_session_env
    resolved_platform = platform or os.getenv("LOKI_PLATFORM") or get_session_env("LOKI_SESSION_PLATFORM")
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``loki config set`` stores lists as quoted JSON/Python-literal strings;
...
def get_external_skills_dirs() -> List[Path]:
    ...
    are ``~``/``${VAR}`` expanded, relative to LOKI_HOME; the local skills dir is skipped."""
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_skill_create_dir() -> Optional[Path]:
    ...
    relative to LOKI_HOME; a value equal to the local skills dir counts as unset."""
...
def display_skill_create_dir() -> str:
    ...
    if create_dir is None:
        return f"{display_loki_home()}/skills/"
...
def get_all_skills_dirs() -> List[Path]:
    """Skill dirs: local ``~/.loki/skills/`` first, then create_dir, then external.
...
# Project-local skills (<root>/.loki/skills, <root>/.agents/skills; root = nearest
...
PROJECT_SKILLS_SUBDIRS = (os.path.join(".loki", "skills"), os.path.join(".agents", "skills"))
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    (LOKI_HOME itself may live inside a git checkout)."""
    local_skills = get_skills_dir().resolve()
    dirs: List[Path] = []
...
# cached under LOKI_HOME, never inside the repo); "dangerous" excludes the
# skill from index, list, view and slash commands ("caution" loads, as on the hub).

# ── Project skill quarantine (scan-time injection defense) ──────────────── Trust (`loki skills trust`)
...
# high-confidence findings only. The scan cache lives under LOKI_HOME, never inside the repo (we don't
...
def is_quarantined_project_skill(skill_md) -> bool:
    ...
    try:
        ...
        from loki_constants import get_loki_home
        cache_dir = get_loki_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    # (which follows the live profile-scoped LOKI_HOME), so normalization
    ...
    # ~/.loki/skills/<name> may be a symlink to a checkout elsewhere, and
...
def _loki_metadata(frontmatter: Dict[str, Any]) -> Dict[str, Any]:
    """``metadata.loki`` mapping from frontmatter, or ``{}`` when malformed."""
    metadata = frontmatter.get("metadata")
    loki = metadata.get("loki") if isinstance(metadata, dict) else None
    return loki if isinstance(loki, dict) else {}
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    loki = _loki_metadata(frontmatter)
    return {key: loki.get(key, []) for key in _CONDITION_KEYS}


def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Extract ``metadata.loki.config`` declarations (key/description/default/prompt).
    Entries missing ``key`` or ``description`` are skipped; ``prompt`` defaults to the description."""
    raw = _loki_metadata(frontmatter).get("config")
