...
def build_subscription_state(*, timeout: float = 15.0) -> SubscriptionState:
    ...
    ``LOKI_DEV_SUBSCRIPTION_FIXTURE`` short-circuits to a fixture so every state is testable offline."""
...
_DEV_FIXTURE_PORTAL = "https://portal.wundercorp.com/billing"
...
def dev_fixture_subscription_state() -> Optional[SubscriptionState]:
    """``LOKI_DEV_SUBSCRIPTION_FIXTURE`` (``free | mid | top | not-admin | downgrade | cancel | team |
    logged-out``) -> fixture state; None when unset; unknown name → logged-out with ``error`` set."""
    name = (os.getenv("LOKI_DEV_SUBSCRIPTION_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    if name not in states:
        return SubscriptionState(logged_in=False, error=f"unknown LOKI_DEV_SUBSCRIPTION_FIXTURE: {name}")
