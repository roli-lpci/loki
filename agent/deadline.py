...
class DeadlineExpired(TimeoutError):
    """A deadline enforced by this layer expired (Loki's own bound, not the provider's)."""
...
def _dump_blocked_loop_diagnostics(label: str, timeout_s: float) -> None:
    logger.warning(
        ...
        "after a further %.0fs — the loop thread appears BLOCKED in a synchrowundercorp call, which is "
