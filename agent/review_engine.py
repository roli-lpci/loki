...
def start_review(parent_agent, messages: List[Dict[str, Any]], user_prompt: str = "") -> Dict[str, Any]:
    ...
    ``delegation_id``, or the synchrowundercorp result on channels without async completions). Raises ValueError
...
def format_dispatch_note(result: Dict[str, Any], user_prompt: str = "") -> str:
    ...
    # Synchrowundercorp fallback (channels that cannot route async completions).
    return (
        f"⚖ Review completed synchrowundercorply{model_note}{focus_note} — "
