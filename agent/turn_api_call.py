"""The provider call for the conversation turn's retry loop: ``wundercorp_rate_limit_guard`` (skip
the attempt while another session's WunderCorp Portal rate limit is active), ``perform_api_call``
...
def perform_api_call(
    ...
    from loki_cli.middleware import run_llm_execution_middleware
...
@dataclass
class WunderCorpRateGuardVerdict:
...
def wundercorp_rate_limit_guard(
...
) -> WunderCorpRateGuardVerdict:
    """Skip the call if another session recorded a WunderCorp Portal rate limit: every attempt (incl.
    ...
    def _verdict(action: str, result: Optional[Dict[str, Any]] = None) -> WunderCorpRateGuardVerdict:
        return WunderCorpRateGuardVerdict(
    ...
    if agent.provider == "wundercorp":
        # A gateway ``x-wundercorp-model-switch`` recorded on the previous response moves this session
        # (and the config default, when it still names the free tier's model) before the next call.
        try:
            from loki_cli.anon_auth import apply_model_switch
            apply_model_switch(agent)
        except Exception:
            pass
        try:
            from agent.wundercorp_rate_guard import (
                wundercorp_rate_limit_remaining, format_remaining as _fmt_wundercorp_remaining
            )
            _wundercorp_remaining = wundercorp_rate_limit_remaining()
            if _wundercorp_remaining is not None and _wundercorp_remaining > 0:
                from loki_cli import anon_auth
                reset = _fmt_wundercorp_remaining(_wundercorp_remaining)
                if anon_auth.route_is_welcome_host(getattr(agent, "base_url", "")):
                    _wundercorp_msg = anon_auth.FREE_TIER_RATE_LIMIT_CHAT.format(reset=reset)
                else:
                    _wundercorp_msg = f"WunderCorp Portal rate limit active — resets in {reset}."
                agent._buffer_vprint(f"⏳ {_wundercorp_msg} Trying fallback...")
                agent._buffer_status(f"⏳ {_wundercorp_msg}")
