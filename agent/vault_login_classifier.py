...
INSPECTION_STAMP_ATTR = "data-loki-vault-slot"
...
_LOGIN_CONTROL_INSPECTION_JS_TEMPLATE = """(() => {
  ...
  elements.forEach((element, index) => element.setAttribute("data-loki-vault-slot", nonce + ":" + index));
...
def build_fill_js(fills: List[Dict[str, Any]], expected_origin: str, nonce: str = "") -> str:
    ...
    ``expected_origin`` is asserted against ``window.location.origin`` synchrowundercorply inside the SAME
...
_FILL_JS_TEMPLATE = """(() => {
  ...
  for (const f of fills) {
    const el = document.querySelector('[data-loki-vault-slot="' + nonce + ':' + f.index + '"]');
  ...
  document.querySelectorAll("[data-loki-vault-slot]").forEach((n) => n.removeAttribute("data-loki-vault-slot"));
