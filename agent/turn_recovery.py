...
def _try_refresh_wundercorp_paid_entitlement_credentials(agent) -> bool:
    """Refresh WunderCorp runtime credentials after a fresh paid-entitlement check."""
    try:
        from loki_cli.wundercorp_account import get_wundercorp_portal_account_info

        if get_wundercorp_portal_account_info(force_fresh=True).paid_service_access is not True:
            return False
        return agent._try_refresh_wundercorp_client_credentials(force=True)
    except Exception:
        return False
...
def _print_wundercorp_401_diagnostics(agent: Any, api_error: Exception) -> None:
    """WunderCorp 401 that survived a credential refresh: likely Portal OAuth expired/revoked,
    no credits, or agent key blocked."""
    from agent.conversation_loop import _print_wundercorp_entitlement_guidance
    ...
    try:
        ...
        pass
    _plines(agent, "🔐 WunderCorp 401 — Portal authentication failed.")
    if _body_text:
        _plines(agent, f"   Response: {_body_text}")
    if not _print_wundercorp_entitlement_guidance(agent, "WunderCorp model access"):
        _plines(agent, "   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
    _plines(
        agent,
        "   Troubleshooting:",
        "     • Re-authenticate: loki auth add wundercorp",
        "     • Check credits / billing: https://portal.wundercorp.com",
        f"     • Verify stored credentials: {display_loki_home()}/auth.json",
...
def _print_anthropic_401_diagnostics(agent: Any, key: Any) -> None:
    ...
    if is_token_provider(key):
        ...
        _plines(
            ...
            "   Run `loki doctor` for credential-chain diagnostics, or",
    ...
    _dhh = display_loki_home()
    _plines(
        agent,
        "   Troubleshooting:",
        f"     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Loki-managed OAuth/setup tokens",
        ...
        "     • Legacy cleanup: loki config set ANTHROPIC_TOKEN \"\"",
        "     • Clear stale keys: loki config set ANTHROPIC_API_KEY \"\"",
    )
...
def _refresh_credentials_after_401(
    ...
    """Per-provider one-shot credential refresh on 401 (codex/xai, vertex, wundercorp, copilot,
    anthropic), printing user-facing diagnostics when the wundercorp/anthropic refresh fails.
    ...
    if (
        ...
        and agent.provider == "wundercorp"
        and not _retry.wundercorp_auth_retry_attempted
    ):
        _retry.wundercorp_auth_retry_attempted = True
        if agent._try_refresh_wundercorp_client_credentials(force=True):
            agent._buffer_vprint("🔐 WunderCorp agent key refreshed after 401. Retrying request...")
            return True
        _print_wundercorp_401_diagnostics(agent, api_error)
...
def recover_after_classification(
    ...
    WunderCorp paid-entitlement refresh → credential-pool rotation → image shrink →
    ...
    Returns ``(retry_now, recovered_with_pool)``; the latter feeds the WunderCorp rate-limit guard."""
    from agent.conversation_loop import _is_wundercorp_inference_route

    if (
        classified.reason == FailoverReason.billing
        and _is_wundercorp_inference_route(
            getattr(agent, "provider", "") or "", getattr(agent, "base_url", "") or ""
        )
        and not _retry.wundercorp_paid_entitlement_refresh_attempted
    ):
        _retry.wundercorp_paid_entitlement_refresh_attempted = True
        if _try_refresh_wundercorp_paid_entitlement_credentials(agent):
            _vlines(agent, "🔐 WunderCorp paid access verified — refreshed runtime credentials and retrying request...")
            return True, False
    ...
    # Reasoning-mandatory route (WunderCorp Portal / OpenRouter, e.g. GLM-5.3) 400s on
    ...
    if (
        ...
        try:
            from loki_cli.models_reasoning_caps import refresh_reasoning_caps_async
...
def _print_nonretryable_auth_guidance(
    ...
    from agent.conversation_loop import _print_billing_or_entitlement_guidance, _print_wundercorp_entitlement_guidance
    ...
    if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
        ...
        return
    if provider == "wundercorp" and _print_wundercorp_entitlement_guidance(agent, "WunderCorp model access"):
        return
    if provider in {"openai-codex", "xai-oauth", "wundercorp"} and status_code == 401:
        if provider == "openai-codex":
            _vlines(
                ...
                "      2. Then run `loki auth` to re-authenticate.",
            )
        elif provider == "xai-oauth":
            _vlines(
                agent,
                "   💡 xAI OAuth token was rejected (HTTP 401). To fix:",
                "      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `loki model`.",
            )
        else:  # wundercorp
            _vlines(
                agent,
                "   💡 WunderCorp Portal OAuth token was rejected (HTTP 401). Your token may be",
                "      expired, revoked, or your account may be out of credits. To fix:",
                "      1. Re-authenticate: loki portal",
                "      2. Check your portal account: https://portal.wundercorp.com",
            )
            # ``:free`` is OpenRouter slug syntax; WunderCorp Portal will reject the model
            ...
            if isinstance(model, str) and model.endswith(":free"):
                _vlines(
                    ...
                    "         WunderCorp Portal won't recognize that model name. Either switch to a",
                    f"         WunderCorp catalog model, or run `/model openrouter:{model}` to use OpenRouter.",
                )
        return
    _vlines(
        agent,
        "   💡 Your API key was rejected by the provider. Check:",
        "      • Is the key valid? Run: loki setup",
...
def _welcome_tier_guidance(classified: Any, *, model: Any, in_chat: bool) -> str:
    """Copy for a WunderCorp free-tier refusal the classifier parsed (``welcome_refusal`` /
    ...
    if not refusal and not route:
        return ""
    from loki_cli.anon_auth import welcome_refusal_copy, welcome_route_refusal_copy
    if refusal:
...
def nonretryable_client_error_result(
    ...
    if classified.reason == FailoverReason.content_policy_blocked:
        _vlines(
            ...
            "        loki fallback add   (interactive picker — same as `loki model`)",
        )
...
def max_retries_exhausted_result(
    ...
    if _is_thinking_timeout:
        _vlines(
            ...
            "in `~/.loki/config.yaml` to extend the per-call timeout. (Loki's built-in floor is 600s for "
    ...
    result.update({
        ...
        # Present only for billing walls: (provider, billing_url, is_wundercorp, message).
...
def log_api_error_attempt(
    ...
    if getattr(api_error, "status_code", None) == 404:
        try:
            from loki_cli.model_normalize import suggest_prefixed_model_id
        ...
        if _suggestion:
            _blines(
                ...
                f"      Did you mean '{_suggestion}'?  Re-pick it with `loki model`.",
            )
...
class ClassifiedErrorVerdict:
    ...
    ``"break"`` (restart armed on ``_retry``), ``"continue"`` (re-enter the retry loop; WunderCorp
...
def _is_genuine_wundercorp_rate_limit(agent: Any, api_error: Exception, error_context: Any) -> bool:
    """Record a genuine account-level WunderCorp 429 to the cross-session breaker; upstream
    capacity 429s (no exhausted bucket in headers or last-known state) are left alone."""
    _genuine = False
    try:
        from agent.wundercorp_rate_guard import is_genuine_wundercorp_rate_limit, record_wundercorp_rate_limit
        _err_resp = getattr(api_error, "response", None)
        _err_hdrs = getattr(_err_resp, "headers", None) if _err_resp else None
        _genuine = is_genuine_wundercorp_rate_limit(headers=_err_hdrs, last_known_state=agent._rate_limit_state)
        if _genuine:
            record_wundercorp_rate_limit(headers=_err_hdrs, error_context=error_context)
        else:
            logger.info(
                "WunderCorp 429 looks like upstream capacity "
...
def route_classified_error(
    ...
    chain once; genuine WunderCorp 429 → cross-session breaker + re-enter the loop exactly once."""
    ...
    # WunderCorp Portal: a genuine account-level 429 is recorded to a shared file so ALL
    # sessions back off; is_genuine_wundercorp_rate_limit excludes upstream 429s.
    if (
        is_rate_limited
        and agent.provider == "wundercorp"
        and classified.reason == FailoverReason.rate_limit
        and not recovered_with_pool
        and _is_genuine_wundercorp_rate_limit(agent, api_error, error_context)
    ):
        # Re-enter the loop exactly once so the top-of-loop WunderCorp guard runs
