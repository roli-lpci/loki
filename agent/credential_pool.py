...
def _is_source_suppressed_fn() -> Callable[[str, str], bool]:
    """``loki_cli.auth.is_source_suppressed`` (late-bound), or an always-False stub."""
    try:
        from loki_cli.auth import is_source_suppressed
...
# singleton to re-seed from and the user can re-add via ``loki auth add``.
...
# selection. On Windows several Loki processes share one rotating log guarded by concurrent-log-handler's
...
# readiness handshake ("Timed out connecting to Loki backend after 15000ms"). Logging the condition at
...
_EXTRA_KEYS = frozenset({
    ...
    # WunderCorp guest identity (``auth_method: anonymous``): the anon_ credential is the refresh material.
...
# WunderCorp singleton metadata mirrored between auth.json state and ``entry.extra``.
_WUNDERCORP_EXTRA_STATE_KEYS = (
...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "wundercorp":
            # WunderCorp stores the runtime inference credential in agent_key for
            ...
            for token, expires_at in (
                ...
                if (
                    ...
                    and token.strip()
                    and auth_mod._wundercorp_invoke_jwt_is_usable(
...
def custom_provider_pool_key_candidates(
    ...
    ``loki auth add <key>`` stores ``providers.<key>`` credentials under the
...
def _guarded_global_root(global_path: Optional[Path]) -> Optional[Path]:
    ...
    never write the real user's ``~/.loki/auth.json`` even when LOKI_HOME
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".loki" / "auth.json"
            try:
...
def _write_through_provider_state_to_global_root(
    ...
    Best-effort write-through for the multi-profile rotation hazard: wundercorp,
    ...
    ``loki_cli.auth._write_through_xai_oauth_to_global_root``.
...
def _singleton_target_for_entry(pool: "CredentialPool", entry: "PooledCredential") -> Optional[Path]:
    """Root ``.anthropic_oauth.json`` when *entry* is a borrowed loki_pkce row, else None."""
    if entry.source != "loki_pkce" or entry.id not in getattr(pool, "_borrowed_root_ids", ()):
        return None
    try:
        from agent.anthropic_credentials import _root_loki_oauth_file
        return _root_loki_oauth_file()
    except Exception:
        return None
...
# ``loki_cli.auth`` refresh function and terminal-error predicate (looked
...
REFRESHABLE_OAUTH_PROVIDERS = frozenset({"anthropic", "wundercorp", *_TOKENS_SINGLETON_PROVIDERS})
...
_REFRESH_TIMEOUT_ENV_VARS = {
    "openai-codex": "LOKI_CODEX_REFRESH_TIMEOUT_SECONDS",
    "xai-oauth": "LOKI_XAI_REFRESH_TIMEOUT_SECONDS",
}
...
class CredentialPool(CredentialPoolAdminMixin):
    ...
    def _sync_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        A fresh ``loki model`` / ``loki auth`` login writes new tokens
    ...
    def _sync_wundercorp_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a WunderCorp device_code entry from auth.json ``providers.wundercorp`` if state differs.

        Another process refreshing via ``resolve_wundercorp_runtime_credentials``
        writes fresh tokens under ``_auth_store_lock``; adopting them avoids a
        "refresh token reuse" revocation on the WunderCorp Portal.
        """
        if self.provider != "wundercorp" or entry.source != "device_code":
            return entry
        try:
            with _auth_store_lock():
                state = _load_provider_state(_load_auth_store(), "wundercorp")
            if not state:
                return entry
            comparable = {
                key: state.get(key)
            ...
            if not any(v not in (None, "") and getattr(entry, k, None) != v for k, v in comparable.items()):
                return entry
            logger.debug("Pool entry %s: syncing WunderCorp state from auth.json", entry.id)
            ...
            extra_updates.update(
                {k: state[k] for k in _WUNDERCORP_EXTRA_STATE_KEYS if state.get(k) is not None}
            )
            return self._adopt(entry, extra=extra_updates, **field_updates)
        except Exception as exc:
            logger.debug("Failed to sync WunderCorp entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        token. Applies to WunderCorp, OpenAI Codex and xAI OAuth singletons.
        ...
        if entry.source != "device_code" or self.provider not in ("wundercorp", *_TOKENS_SINGLETON_PROVIDERS):
            return
        try:
            with _auth_store_lock():
    ...
    def _apply_entry_to_singleton_state(self, entry: PooledCredential, state: Dict[str, Any]) -> bool:
        ...
        if self.provider == "wundercorp":
            ...
            for extra_key in _WUNDERCORP_EXTRA_STATE_KEYS:
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # across Loki processes, or two processes adopt the same on-disk
    ...
    def _fail_closed_unpersisted_rotation(
        ...
        *,
        ...
        For ``claude_code`` / ``loki_pkce`` the singleton file — not
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        env_var = _REFRESH_TIMEOUT_ENV_VARS.get(self.provider, "LOKI_ANTHROPIC_REFRESH_TIMEOUT_SECONDS")
    ...
    def _commit_anthropic_rotation(
        ...
        and other profiles see it). loki_pkce -> ~/.loki/.anthropic_oauth.json
        ...
        ``endswith``: manual:loki_pkce is pool-owned and a singleton for it
        would be a second authority for the same refresh-token family.
        """
        if entry.source == "claude_code":
            store = "~/.claude/.credentials.json"
        elif entry.source == "loki_pkce":
            store = "~/.loki/.anthropic_oauth.json"
        else:
            return
        try:
            ...
            if entry.source == "claude_code":
                ...
                ac._write_loki_oauth_credentials(*args, target=_singleton_target_for_entry(self, entry))
    ...
    def _refresh_anthropic(self, entry: PooledCredential) -> PooledCredential:
        ...
        refreshed = refresh_anthropic_oauth_pure(entry.refresh_token, use_json=entry.source.endswith("loki_pkce"))
        updated = replace(
            entry,
    ...
    def _refresh_entry_impl(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "anthropic":
            ...
            elif self.provider == "wundercorp":
                stale_key = entry.runtime_api_key or entry.agent_key or entry.access_token
                synced = self._sync_wundercorp_entry_from_auth_store(entry)
                if synced is not entry:
                    ...
                    if force and entry.runtime_api_key and entry.runtime_api_key != stale_key:
                        logger.debug("WunderCorp entry %s: adopting peer-rotated token, skipping refresh", entry.id)
                        return entry
                auth_mod.resolve_wundercorp_runtime_credentials(force_refresh=force, stale_access_token=stale_key or None)
                updated = self._sync_wundercorp_entry_from_auth_store(entry)
            else:
                return entry
    ...
    def _recover_failed_refresh(self, entry: PooledCredential, exc: Exception) -> Optional[PooledCredential]:
        ...
        if self.provider == "anthropic":
            if entry.source == "claude_code":
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            synced.refresh_token, use_json=synced.source.endswith("loki_pkce"),
                        )
                ...
                # Backstop for pool-owned sources (loki_pkce, manual:dashboard_pkce):
            ...
            # entries from the pool (mirrors the WunderCorp quarantine path).
            ...
            synced = self._sync_wundercorp_entry_from_auth_store(entry)
            if synced.refresh_token != entry.refresh_token:
                logger.debug("WunderCorp refresh failed but auth.json has newer tokens — adopting")
            ...
            if isinstance(exc, TimeoutError):
                ...
                # pool for ~120 sessions ("matched no wundercorp entry ... pool size
                # 0"). The caller's retry re-syncs once the winner persisted.
                logger.debug("WunderCorp refresh skipped: auth store lock busy; not benching entry")
                return entry
            if auth_mod._is_terminal_wundercorp_refresh_error(exc):
                logger.debug("WunderCorp refresh token is terminally invalid; clearing local token state")
                self._clear_terminal_wundercorp_state(entry, exc)
                self._quarantine_sources(
                    entry,
                    {auth_mod.WUNDERCORP_DEVICE_CODE_SOURCE, f"manual:{auth_mod.WUNDERCORP_DEVICE_CODE_SOURCE}"},
                )
                return None
        self._mark_exhausted(entry, None)
        return None
    ...
    def _clear_terminal_wundercorp_state(self, entry: PooledCredential, exc: Exception) -> None:
        try:
            with _auth_store_lock():
                auth_store = _load_auth_store()
                state = _load_provider_state(auth_store, "wundercorp") or {
                ...
                if not store_refresh or store_refresh == str(entry.refresh_token or "").strip():
                    auth_mod._quarantine_wundercorp_oauth_state(state, exc, reason="credential_pool_refresh_failure")
                    auth_mod._quarantine_wundercorp_pool_entries(auth_store, exc, reason="credential_pool_refresh_failure")
                    _save_provider_state(auth_store, "wundercorp", state)
                    _save_auth_store(auth_store)
        except Exception as clear_exc:
            logger.debug("Failed to clear terminal WunderCorp OAuth state: %s", clear_exc)
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        # WunderCorp refresh can require network access and happens when runtime
    ...
    def _resync_stale_entry(self, entry: PooledCredential) -> PooledCredential:
        ...
        The user may have re-authed (``loki model`` / ``loki auth``, the
        ...
        if self.provider == "wundercorp":
            return self._sync_wundercorp_entry_from_auth_store(entry)
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `loki auth add %s`",
...
class _Seeder:
    ...
    def upsert(self, source: str, payload: Dict[str, Any]) -> bool:
        """Upsert unless suppressed (``loki auth remove`` must stay stable across loads)."""
...
def _seed_anthropic_singletons(seed: _Seeder) -> None:
    # Only auto-discover external credentials (Claude Code, Loki PKCE) when
    ...
    # API-key vs OAuth is a user-visible choice at `loki setup`. The API-key
    ...
    # ~/.loki/.env over os.environ, as `_seed_from_env` does.
    _env_file = load_env()
    ...
    if _env_val("ANTHROPIC_API_KEY") and not anthropic_oauth_env:
        ...
        seed.changed |= _retain_sources_not_in(seed.entries, {"loki_pkce", "claude_code"})
        return

    from agent.anthropic_credentials import (
        read_claude_code_credentials,
        read_loki_oauth_credentials,
    )

    for source_name, creds in (
        ("loki_pkce", read_loki_oauth_credentials()),
...
def _seed_wundercorp_singleton(seed: _Seeder, auth_store: Dict[str, Any]) -> None:
    ...
    if not (state and has_runtime_material):
        return
    # Prefer a user-supplied label embedded in the singleton state (``loki
    # auth add wundercorp --label <name>``) over the token-derived fingerprint.
    ...
    seed.upsert("device_code", {
        ...
        # ones (#15099).
        **{key: state.get(key) for key in _WUNDERCORP_EXTRA_STATE_KEYS},
...
def _seed_copilot_singleton(seed: _Seeder) -> None:
    ...
    try:
        from loki_cli.copilot_auth import (
            COPILOT_ENV_VARS,
...
def _seed_qwen_singleton(seed: _Seeder) -> None:
    ...
    try:
        from loki_cli.auth import resolve_qwen_runtime_credentials
...
def _seed_tokens_singleton(seed: _Seeder, auth_store: Dict[str, Any]) -> None:
    ...
    Loki owns its own Codex auth state and does NOT auto-import
    ...
    explicit one-time prompt via `loki auth openai-codex`.
    """
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
    ...
    elif provider == "wundercorp":
        _seed_wundercorp_singleton(seed, auth_store)
        ...
        # `loki auth remove openai-codex` suppresses device_code; without
...
def get_env_prefer_dotenv(key: str) -> str:
    """Resolve a credential env var, preferring ~/.loki/.env over os.environ.
...
def _warn_env_ingestion_once(provider: str, env_var: str) -> None:
    ...
    logger.warning(
        ...
        "loki auth remove %s <n> to suppress.",
        env_var,
        provider,
...
def _env_payload(*, env_var: str, token: str, base_url: str) -> Dict[str, Any]:
    ...
    try:
        from loki_cli.env_loader import get_secret_source
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # requested (an `loki auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons and Loki PKCE disappear when their backing file is gone.
        return is_borrowed_credential_source(entry.source, entry.provider) or entry.source == "loki_pkce"
