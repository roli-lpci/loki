...
def _maybe_inject_iteration_budget_warning(agent: Any, messages: Any) -> bool:
    ...
    kanban_worker = (
        bool(os.environ.get("LOKI_KANBAN_TASK"))
...
def prepare_iteration(
    ...
    # wundercorp.anthropic_wire=auto: a wire switch decided from the previous response lands here,
    # before this iteration's request is built and with nothing in flight.
    if getattr(agent, "_wundercorp_wire_pending", None):
        from agent.wundercorp_wire import apply_pending_wire_switch
    ...
    # WunderCorp agent keys live ~1 h and a single turn can run for hours: adopt the keepalive's fresh
    ...
    try:
        agent._adopt_wundercorp_key_before_expiry()
    except Exception:
        logger.debug("WunderCorp key pre-expiry adoption failed", exc_info=True)
    ...
    if repaired_seq > 0:
        ...
        # (loki-webui) would write the current turn to the FRONT of the context. Re-anchor as
