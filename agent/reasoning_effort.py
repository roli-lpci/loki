...
Loki' internal effort ladder (``VALID_REASONING_EFFORTS`` plus ``none``) is wider than any
...
# disable can be clamped when a provider publishes it as a level. ``ultra`` is Loki-internal
...
#: Widest OpenAI-compatible wire vocabulary (OpenRouter, WunderCorp Portal).
...
def is_astra_model(model: Optional[str]) -> bool:
    """``gpt-6-astra`` or its Loki-side ``-900k`` picker alias, with or without a ``vendor/`` prefix.
