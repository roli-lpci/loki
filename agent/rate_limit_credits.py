...
class RateLimitCreditsMixin:
    ...
    def _capture_wundercorp_model_switch(self, http_response: Any) -> None:
        """Record the WunderCorp gateway's ``x-wundercorp-model-switch`` header (a named account asked for the
        free tier's model; the gateway served its backing model and named it). Applied between
        calls by ``loki_cli.anon_auth.apply_model_switch``. Fail-open."""
        headers = _response_headers(http_response)
        if not headers:
            return
        try:
            from loki_cli.anon_auth import note_model_switch
    ...
    def _capture_credits(self, http_response: Any) -> None:
        """Parse x-wundercorp-credits-* headers, cache CreditsState, fire threshold notices.

        The PARSE is swallowed (miss → keep last-known); notice EVALUATION WARNS on failure so a
        depletion-notice bug cannot vanish silently. LOKI_DEV_CREDITS_FIXTURE injects a chosen state instead.
        """
        try:
        ...
        if fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset LOKI_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        dev = is_truthy_value(os.environ.get("LOKI_DEV_CREDITS"))
        ...
        if state is None:
            if dev:
                logger.info("credits ▸ response had no valid x-wundercorp-credits-* headers "
                            "(miss — producer off / non-WunderCorp path / >TTL stale)")
            return

        _adopt_credits_state(self, state)
        if dev:
            # LOKI_DEV_CREDITS streams each capture to agent.log (`loki logs -f`, grep 'credits ▸').
