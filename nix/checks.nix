...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      loki-agent = self'.packages.default;
      lokiVenv = loki-agent.lokiVenv;
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.loki-agent = programs;
              services.loki-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.loki-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.loki-agent);
      ...
      configKeys = pkgs.runCommand "loki-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${lokiVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "loki-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "loki-build-package" { } ''
          echo "PASS: package built at ${loki-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "loki-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.LOKI_TEST = "1";
                environmentFiles = [ "/run/secrets/loki-env" ];
                lokiHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "loki" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "loki" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.lokiAgentSetup.data;

            failures =
              lib.optional (names != [
                "loki-agent"
                "loki-backend"
              ]) "expected loki-agent + loki-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/loki gateway" (argvOf "loki-agent")
              ) "gateway process does not run `loki gateway`: ${argvOf "loki-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/loki serve" (argvOf "loki-backend")
              ) "backend process does not run `loki serve`: ${argvOf "loki-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "loki-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/loki-check/.loki" (envOf n)) names
              ) "gateway and backend must share one LOKI_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.LOKI_HOME or null != "/home/loki-check/.loki"
              ) "programs.loki-agent.enable must export LOKI_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "loki-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/loki-check/.loki/SOUL.md" activation
              ) "lokiHomeFiles must install into LOKI_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads LOKI_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "LOKI_MANAGED=home-manager" (envOf n)) names
              ) "processes must report LOKI_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "loki-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "loki-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            cases = [
              ...
              {
                name = "lokiHomeFiles needs no directory";
                ok = accepts { lokiHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "loki-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one LOKI_HOME ───────────────
        # `programs.enable` exports LOKI_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.loki
        # while the services use the LOKI_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/loki-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                lokiHome = "/home/loki-check/.loki-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "loki-desktop") cfg.home.packages;
            ...
            # second Loki runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "loki-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.loki-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.loki-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one loki-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "LOKI_HOME" != "/home/loki-check/.loki-work"
              ) "the launcher must carry LOKI_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "LOKI_HOME")})"
              ++ lib.optional (
                setValue "LOKI_MANAGED" != "home-manager"
              ) "the launcher must report LOKI_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "LOKI_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "LOKI_DESKTOP_CLI" != "${lib.head agentPackages}/bin/loki"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "LOKI_DESKTOP_CLI")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "LOKI_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "LOKI_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "LOKI_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "LOKI_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "LOKI_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "loki-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares LOKI_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set LOKI_HOME" wrapper
              ) "the launcher must carry LOKI_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "LOKI_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "LOKI_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? loki-backend || cfg.launchd.agents ? loki-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "loki-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `loki`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.loki-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.loki-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "loki-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "loki-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "loki" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."loki-agent-setup".text;

            failures =
              lib.optional (names != [
                "loki-agent"
                "loki-backend"
              ]) "expected loki-agent + loki-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/loki gateway" (execOf "loki-agent")
              ) "gateway unit does not run `loki gateway`: ${execOf "loki-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/loki dashboard" (execOf "loki-backend")
              ) "backend unit does not run `loki dashboard`: ${execOf "loki-backend"}"
              ++ lib.optional (
                units.loki-agent.environment.LOKI_HOME != units.loki-backend.environment.LOKI_HOME
              ) "gateway and backend must share one LOKI_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/loki/.loki/SOUL.md" activation
              ) "lokiHomeFiles must install into LOKI_HOME";
          ...
          in
          pkgs.runCommand "loki-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs loki, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.loki-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/loki serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec loki directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "loki-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "loki-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec loki, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/loki dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "loki-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $LOKI_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          ...
          in
          pkgs.runCommand "loki-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'LOKI_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--loki-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "loki-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "loki-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${loki-agent}/bin/loki || (echo "FAIL: loki binary missing"; exit 1)
          test -x ${loki-agent}/bin/loki-agent || (echo "FAIL: loki-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${loki-agent}/bin/loki --version 2>&1 | grep -qi "loki" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "loki-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in loki loki-agent loki-acp; do
            test -x ${loki-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "loki-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking loki --help ==="
          ${loki-agent}/bin/loki --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${loki-agent}/bin/loki --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "loki-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${loki-agent}/share/loki-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${loki-agent}/share/loki-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "LOKI_BUNDLED_SKILLS" ${loki-agent}/bin/loki || \
            (echo "FAIL: LOKI_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: LOKI_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${loki-agent}/share/loki-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${loki-agent}/share/loki-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "LOKI_OPTIONAL_SKILLS" ${loki-agent}/bin/loki || \
            (echo "FAIL: LOKI_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, LOKI_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "loki-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${loki-agent}/share/loki-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${loki-agent}/share/loki-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "LOKI_BUNDLED_PLUGINS" ${loki-agent}/bin/loki || \
            (echo "FAIL: LOKI_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: LOKI_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "loki-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${loki-agent}/share/loki-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${loki-agent}/share/loki-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${loki-agent}/share/loki-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "LOKI_BUNDLED_LOCALES" ${loki-agent}/bin/loki || \
            (echo "FAIL: LOKI_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: LOKI_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + LOKI_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && LOKI_BUNDLED_LOCALES=${loki-agent}/share/loki-agent/locales \
            ${lokiVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with LOKI_BUNDLED_LOCALES set"; exit 1)
        ...
        # LOKI_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "loki-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${loki-agent}/share/loki-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${loki-agent}/share/loki-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "LOKI_OPTIONAL_MCPS" ${loki-agent}/bin/loki || \
            (echo "FAIL: LOKI_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: LOKI_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${loki-agent}/bin/loki mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: loki mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "loki-bundled-tui" { } ''
          ...
          grep -q "LOKI_TUI_DIR" ${loki-agent}/bin/loki || \
            (echo "FAIL: LOKI_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: LOKI_TUI_DIR set in wrapper"
        ...
        # Verify LOKI_NODE is set in wrapper and points to Node 26+
        # (Loki pins its toolchain to Node 26 everywhere)
        loki-node = pkgs.runCommand "loki-node-version" { } ''
          set -e
          echo "=== Checking LOKI_NODE in wrapper ==="
          grep -q "LOKI_NODE" ${loki-agent}/bin/loki || \
            (echo "FAIL: LOKI_NODE not set in wrapper"; exit 1)
          echo "PASS: LOKI_NODE present in wrapper"

          LOKI_NODE=$(sed -n "s/^export LOKI_NODE='\(.*\)'/\1/p" ${loki-agent}/bin/loki)
          test -x "$LOKI_NODE" || (echo "FAIL: LOKI_NODE=$LOKI_NODE not executable"; exit 1)
          echo "PASS: LOKI_NODE executable at $LOKI_NODE"

          NODE_MAJOR=$("$LOKI_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Loki requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All LOKI_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify LOKI_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "loki-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(LOKI_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking LOKI_MANAGED guards ==="
          check_blocked "config set" ${loki-agent}/bin/loki config set model foo
          check_blocked "config edit" ${loki-agent}/bin/loki config edit
        ...
        extra-python-packages = let
          ...
          lokiWithExtra = loki-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "loki-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${lokiWithExtra}/bin/loki || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${lokiWithExtra}/bin/loki || \
          ...
          if grep -q "PYTHONPATH" ${loki-agent}/bin/loki; then
        ...
        extra-dependency-groups = let
          lokiWithGroups = loki-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "loki-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${lokiWithGroups}"
          echo "venv: ${lokiWithGroups.lokiVenv}"
        ...
        messaging-variant = pkgs.runCommand "loki-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.lokiVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "loki-config-roundtrip" {
          ...
          merge_and_load() {
            local loki_home="$1"
            export LOKI_HOME="$loki_home"
            ${configMergeScript} ${nixSettings} "$loki_home/config.yaml"
            ${lokiVenv}/bin/python3 -c '
import json, sys
