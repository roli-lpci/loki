# nix/packages.nix — Loki Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./loki-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.lokiTui;
        web = full.lokiWeb;
        desktop = full.lokiDesktop;

        update-npm-lockfile = full.lokiNpmLib.updateNpmLockfile;
      };
    };
}
