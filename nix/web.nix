# nix/web.nix — Loki Web Dashboard (Vite/React) frontend build
{ lokiNpmLib, ... }:
lokiNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @wundercorp/loki-shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../loki_cli/web_dist for the
