...
#
# `services.loki-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  lokiHome, programs.loki-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "loki-config-attrs";
    description = "Loki YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $LOKI_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Loki Agent";
      ...
      package = mkOption {
        ...
        description = "The loki-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Loki configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `loki config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $LOKI_HOME/.env. Loki reads that file at each start, with
          load_loki_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."loki/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Loki refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Loki reads SOUL.md and memories/ from LOKI_HOME, so put those
          files in `lokiHomeFiles`.
      ...
      lokiHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into LOKI_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Loki reads SOUL.md and the memory files from LOKI_HOME and not
          from the working directory. Declare those files here, or Loki
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the loki plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Loki discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          loki_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as loki (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in loki-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `loki gateway`.";
      };
      ...
      # ── The backend: `loki serve` or `loki dashboard` ──────────────
      # `loki serve` and `loki dashboard` are the same entry point,
      # loki_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Loki Desktop
      ...
      # controls an external gateway, with `loki gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Loki
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            LOKI_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Loki Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."loki/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `loki` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.loki-agent.installPackage was removed. Loki now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.loki-agent.enable = ${lib.boolToString (value != false)};  # the loki CLI, and LOKI_HOME for your shells
        programs.loki-agent.desktop.enable = true;  # the desktop application

      `services.loki-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "loki-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "loki-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "loki-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "loki-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "loki-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # LOKI_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and Loki make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${lokiHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Loki
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${lokiHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${lokiHome}/config.yaml
            ${run}chmod ${modes.config} ${lokiHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "loki-managed" managedSystem} ${lokiHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${lokiHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${lokiHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${lokiHome}/auth.json"
        else
          ''
            if [ ! -e ${lokiHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${lokiHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree lokiHome cfg.lokiHomeFiles}
      ...
      ${run}find ${lokiHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "loki-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${lokiHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  #
  # `exec` on the last line keeps loki as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "loki-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "loki-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "loki-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          LOKI_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export LOKI_DASHBOARD_SESSION_TOKEN

          if [ -z "$LOKI_DASHBOARD_SESSION_TOKEN" ]; then
            echo "loki-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "loki-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "loki-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "loki-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "loki-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "loki-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "loki-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Loki Agent web dashboard and desktop backend"
    else
      "Loki Agent backend for Loki Desktop";

  # The environment that each Loki process needs, from either module.
  #
  # managedSystem gives the value of LOKI_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Loki an identity and a memory, use
          ${optionPath}.lokiHomeFiles instead. Those files go to
          LOKI_HOME. Loki reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of LOKI_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
