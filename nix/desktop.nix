# nix/desktop.nix — Loki Desktop (Electron) app build + wrapper
#
# `lokiAgent` is the fully-built `.#default` package — it ships the
# `loki` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `LOKI_DESKTOP_CLI` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Loki CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # LOKI_HOME and LOKI_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "loki-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "loki-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = lokiNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "loki-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe lokiNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/loki-desktop $out/bin
    cp -r ${renderer}/* $out/share/loki-desktop/
    ...
    substituteInPlace $out/share/loki-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/loki-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # LOKI_DESKTOP_CLI to the absolute path of the nix-built `loki`
    # binary so the desktop's resolver step 4 ("existing Loki CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/loki-desktop \
      --add-flags "$out/share/loki-desktop" \
      --set LOKI_DESKTOP_CLI "${lib.getExe lokiAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/loki.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../loki_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/loki-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/loki.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/loki.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Loki Agent";
    homepage = "https://github.com/WunderCorp-Inc/loki-agent";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "loki-desktop";
  };
}
