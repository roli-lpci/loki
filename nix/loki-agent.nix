...
let
  mkLokiVenv =
    ...
    callPackage ./python.nix {
      ...
      pythonSrc = lokiNpmLib.pythonSrc;
      dependency-groups = [ "all" ] ++ extraDependencyGroups;
    };

  lokiVenv = (mkLokiVenv extraDependencyGroups).venv;
  ...
  # lib.nix) — the wrapper exposes them via LOKI_OPTIONAL_SKILLS, the
  ...
  # exposed via LOKI_OPTIONAL_MCPS.
...
stdenv.mkDerivation (finalAttrs: {
  ...
  installPhase = ''
    ...
    ln -s ${bundledSkills} $out/share/loki-agent/skills
    ln -s ${bundledOptionalSkills} $out/share/loki-agent/optional-skills
    ln -s ${bundledPlugins} $out/share/loki-agent/plugins
    ...
    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${lokiVenv}/bin/${name} $out/bin/${name} \
          ...
          --set LOKI_OPTIONAL_SKILLS $out/share/loki-agent/optional-skills \
  ...
  passthru =
    let
      devPython = (mkLokiVenv (extraDependencyGroups ++ [ "dev" ])).editableVenv;
    in
    {
      inherit
        lokiTui
      ...
      devShellHook = ''
        export LOKI_PYTHON=${devPython}/bin/python3
      '';
