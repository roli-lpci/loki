...
# "tui-ui/package.json").  This file collects them all and passes the
...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      lokiNpmLib = self'.packages.default.passthru.lokiNpmLib;
      ...
      lokiAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "loki" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../loki} $out/bin/loki
          '')
        ...
        shellHook = ''
          ${lokiAgentDevShellHook}
          ${lokiNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export LOKI_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Loki Agent dev shell in $LOKI_PYTHON_SRC_ROOT"
          echo "Ready. Run 'loki' or 'sandbox loki' to start."
        '';
      };
    };
}
