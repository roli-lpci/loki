...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Loki derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            loki-agent = prev.loki-agent.overrideAttrs (_old: {
              LOKI_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $LOKI_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$LOKI_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        loki-agent = prev.loki-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "loki-agent-env" {
    loki-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "loki-agent-editable-env" {
    loki-agent = dependency-groups;
  };
}
