# nix/nixosModules.nix — the NixOS module for loki-agent
#
...
#
# Container mode: loki runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $LOKI_HOME/.env
# and read by loki at startup — no container recreation needed for env changes.
#
# Tool resolution: the loki wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      ...
      # LOKI_HOME via addToSystemPackages, so they can save settings through the
      ...
      # loki user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "loki-container-entrypoint" ''
        set -eu

        LOKI_UID="''${LOKI_UID:?LOKI_UID must be set}"
        LOKI_GID="''${LOKI_GID:?LOKI_GID must be set}"

        # ── Group: ensure a group with GID=$LOKI_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$LOKI_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="loki"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$LOKI_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$LOKI_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$LOKI_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$LOKI_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="loki"
          TARGET_HOME="/home/loki"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$LOKI_UID" -g "$LOKI_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$LOKI_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$LOKI_UID:$LOKI_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure LOKI_HOME is owned by the target user.
        ...
        if [ -n "''${LOKI_HOME:-}" ] && [ -d "$LOKI_HOME" ]; then
          find "$LOKI_HOME" \! -user "$LOKI_UID" -exec chown "$LOKI_UID:$LOKI_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/loki-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/loki-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/loki ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/loki
          chmod 0440 /etc/sudoers.d/loki
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$LOKI_UID" --regid="$LOKI_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $LOKI_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "loki-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        loki_bin=${containerDataDir}/current-package/bin/loki
      '';

      # Default: /var/lib/loki/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the loki group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit lokiHome; };
    ...
    in
    {
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              ...
              description = "State directory. Contains .loki/ subdir (LOKI_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the loki CLI to environment.systemPackages and export
                LOKI_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.loki symlink to the service
                  stateDir. These users are automatically added to the loki group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the loki CLI to system PATH and export LOKI_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.loki/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.LOKI_HOME = lokiHome;
          })
              ...
              ++ [
                {
                  ...
                  message = "services.loki-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the loki user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.loki-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed loki
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure loki is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${lokiHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${lokiHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."loki-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${lokiHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${lokiHome} ${cfg.workingDirectory}
                  ...
                  find ${lokiHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${lokiHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${lokiHome}/$_subdir"
                    chmod 2770 "${lokiHome}/$_subdir"
                    find "${lokiHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg lokiHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${lokiHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${lokiHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${lokiHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.loki";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${lokiHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "loki-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.loki -> stateDir/.loki for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.loki-agent = {
              ...
              # $LOKI_HOME/.env by the activation script. load_loki_dotenv()
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.loki-agent = {
              description = "Loki Agent Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  LOKI_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  LOKI_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env LOKI_UID="$LOKI_UID" \
                    --env LOKI_GID="$LOKI_GID" \
                    --env LOKI_HOME=${containerDataDir}/.loki \
                    --env LOKI_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/loki gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
