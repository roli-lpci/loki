...
# 5. Symlinks the 'loki' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Loki Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/loki-uv-install.$$.log")"
...
if is_termux; then
    # Loki currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Loki requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink loki into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up loki command..."

LOKI_BIN="$SCRIPT_DIR/venv/bin/loki"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$LOKI_BIN" "$COMMAND_LINK_DIR/loki"
echo -e "${GREEN}✓${NC} Symlinked loki → $COMMAND_LINK_DISPLAY_DIR/loki"
...
# ============================================================================
# Seed bundled skills into ~/.loki/skills/
# ============================================================================

LOKI_SKILLS_DIR="${LOKI_HOME:-$HOME/.loki}/skills"
mkdir -p "$LOKI_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.loki/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$LOKI_SKILLS_DIR/" 2>/dev/null || true
...
if is_termux; then
    echo "  loki gateway       # Run gateway in foreground"
else
    echo "  loki gateway install # Install gateway service (messaging + cron)"
fi
echo "  loki cron list     # View scheduled jobs"
echo "  loki doctor        # Diagnose issues"
echo ""
