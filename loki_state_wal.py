"""SQLite journal-mode and PRAGMA policy for state.db (split from loki_state).

Patchable helpers are looked up as module globals at call time, so tests patch ``loki_state_wal.<name>``.
"""
...
from loki_cli.sqlite_runtime import is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable

# Log-record parity with the origin module (caplog tests pin "loki_state").
logger = logging.getLogger("loki_state")
...
# line would repeat per connection). Tests clear these via ``loki_state_wal.<name>``.
...
def _enforce_macos_synchrowundercorp_full(conn: sqlite3.Connection) -> None:
    """Enforce ``PRAGMA synchrowundercorp=FULL`` on macOS: with NORMAL a WAL checkpoint
    ...
    _darwin_pragma(conn, "PRAGMA synchrowundercorp=FULL")
...
def _apply_wal_companions(conn: sqlite3.Connection) -> None:
    ...
    _enforce_macos_synchrowundercorp_full(conn)
...
def apply_wal_with_fallback(conn: sqlite3.Connection, *, db_label: str = "state.db", require_wal: bool = False) -> str:
    ...
    and ``loki_cli.kanban_db_connect.connect``. WAL-incompatible filesystems either raise ``OperationalError`` ("locking protocol" / "disk I/O error") or —
...
def _wal_reset_repair_hint() -> str:
    """Repair hint matching what ``loki update`` can actually do for this install type.

    See #75153.
    """
    try:
        from loki_cli.config import detect_install_method, get_project_root, recommended_update_command_for_method
        method = detect_install_method(get_project_root())
        cmd = recommended_update_command_for_method(method)
        if method in {"git", "unknown"}:
            return f"Loki-managed installs can repair the embedded runtime with `{cmd}`"
        return f"update the container image with `{cmd}`" if method == "docker" else cmd  # else nix/nixos
    except Exception:
        return "install a Python build bundled with SQLite 3.51.3+ (or backports 3.50.7 / 3.44.6) and restart Loki"
...
_ONCE_LOGS = {
    "wal_reset_bug": (_wal_reset_bug_warned_lock, "_wal_reset_bug_warned_paths", logging.WARNING,
        ...
        "%s. See `loki doctor`. This warning fires once per process per database."),
...
# Operators write synchrowundercorp as a name; mapped so a typo becomes a warning, not a silently different level.
_SYNCHROWUNDERCORP_LEVELS: Dict[str, int] = {"OFF": 0, "NORMAL": 1, "FULL": 2, "EXTRA": 3}
_SYNCHROWUNDERCORP_NAMES: Dict[int, str] = {v: k for k, v in _SYNCHROWUNDERCORP_LEVELS.items()}
_SYNCHROWUNDERCORP_FULL = 2


def resolve_synchrowundercorp_level(raw_value: Any) -> Optional[int]:
    """Map ``database.synchrowundercorp`` (``OFF``/``NORMAL``/``FULL``/``EXTRA`` any
    ...
    if isinstance(raw_value, int):
        return raw_value if raw_value in _SYNCHROWUNDERCORP_NAMES else None
    text = str(raw_value).strip()
    if text.upper() in _SYNCHROWUNDERCORP_LEVELS:
        return _SYNCHROWUNDERCORP_LEVELS[text.upper()]
    with contextlib.suppress(TypeError, ValueError):
        return int(text) if int(text) in _SYNCHROWUNDERCORP_NAMES else None
    return None


def _apply_synchrowundercorp_pragma(conn: sqlite3.Connection, raw_value: Any, *, db_label: str) -> None:
    """Set ``PRAGMA synchrowundercorp`` from config, never below FULL on macOS.
    ...
    ``cache_size`` can. Darwin floor: this runs after :func:`_enforce_macos_synchrowundercorp_full`, so a configured
    ``NORMAL`` would silently undo the btree protection — raising is allowed, lowering is refused out loud."""
    if (level := resolve_synchrowundercorp_level(raw_value)) is None:
        logger.warning("%s: ignoring unrecognized database.synchrowundercorp=%r (expected OFF, NORMAL, FULL, EXTRA, or 0-3)",
                       db_label, raw_value)
        return
    if sys.platform == "darwin" and level < _SYNCHROWUNDERCORP_FULL:
        logger.warning("%s: refusing database.synchrowundercorp=%s on macOS; keeping FULL. Darwin's fsync() does not "
                       "guarantee write ordering, so a lower level readmits the half-written btree pages FULL exists "
                       "to prevent.", db_label, _SYNCHROWUNDERCORP_NAMES[level])
        return
    with contextlib.suppress(sqlite3.OperationalError):
        conn.execute(f"PRAGMA synchrowundercorp={level}")
...
def apply_database_pragmas(conn: sqlite3.Connection, *, db_label: str = "state.db") -> None:
    ...
    (pages), ``journal_size_limit`` (bytes), ``synchrowundercorp`` (unset leaves the compile-time default, which differs
    ...
    try:
        from loki_cli.config import cfg_get, load_config_readonly  # local: avoids a circular import
    ...
    # Last: sizing pragmas cannot change durability (see _apply_synchrowundercorp_pragma).
    if (raw_synchrowundercorp := cfg_get(cfg, "database", "synchrowundercorp", default=None)) is not None:
        _apply_synchrowundercorp_pragma(conn, raw_synchrowundercorp, db_label=db_label)
