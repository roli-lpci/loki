# Loki Agent Security Policy

This document describes Loki Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/wundercorp/loki/security/advisories/new)
or **security@wundercorp.com**. Do not open public issues for
security vulnerabilities. **Loki Agent does not operate a bug
bounty program.**
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Loki Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Loki Agent's documentation
...
Loki Agent supports two OS-level isolation postures. They address
...
Loki Agent supports this in two ways:

- **Loki Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Loki Agent's in-process heuristics
...
Loki Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Loki Agent
itself. Bugs in Loki Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Loki Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Loki Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Loki Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  loki-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Loki Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Loki Agent. Bugs in Loki
...
- Loki Agent includes supply-chain guards for MCP server
