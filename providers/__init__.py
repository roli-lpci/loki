...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with loki-agent)
2. User plugins: ``$LOKI_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``loki_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$LOKI_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$LOKI_HOME/plugins/model-providers/`` if it exists."""
    try:
        from loki_constants import get_loki_home

        d = get_loki_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$LOKI_HOME/plugins/`` if it exists.

    This is where ``loki plugins install`` clones a plugin — flat, one
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple LOKI_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_loki_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``loki_agent.plugins``
    ...
    * **Provider targets only.** The ``loki_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$LOKI_HOME``) keep their
    ...
    try:
        from loki_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$LOKI_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``loki plugins install`` at
          ``$LOKI_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    # 0. Pip-installed plugins — entry points in the ``loki_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$LOKI_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with loki-agent.
    ...
    # 2. User plugins — under $LOKI_HOME/plugins/model-providers/<name>/.
    ...
    # 2b. Plugins installed by ``loki plugins install`` / the plugin index.
    #     Those clone into $LOKI_HOME/plugins/<name>/ — flat, NOT under
