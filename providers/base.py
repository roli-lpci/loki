...
def _profile_user_agent() -> str:
    """Return a ``loki-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from loki_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"loki-cli/{_ver}"
    except Exception:
        return "loki-cli"
...
class ProviderProfile:
    ...
    # describe how to launch it; loki_cli/auth.py's
    ...
    def create_client(self, **client_kwargs: Any) -> Any | None:
        ...
        a profile registered from ``~/.loki/plugins/model-providers/`` or a
    ...
    def fetch_models(
        ...
        *,
        ...
        # loki-cli UA so the catalog endpoint is reachable.
