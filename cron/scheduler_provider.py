...
def _existing_profile_homes(profile_homes: list) -> list:
    ...
    back on disk and in ``loki profile list`` (#47368). Filtering on directory existence leaves a deleted
...
class CronScheduler(ABC):
    ...
    def claim_fire(self, job_id: str, *, force: bool = False, manual: bool = False) -> dict | None:
        """Durably claim one fire + create its audit attempt. Transports call this synchrowundercorply
...
def fire_overdue_jobs(
    ...
    from cron.jobs import (
        ONESHOT_GRACE_SECONDS, _ensure_aware, _loki_now, is_job_runnable, load_jobs,
    )
    ...
    for job in load_jobs():
        ...
        try:
            # Claim synchrowundercorply (CAS loss = external retry beat us), run off-thread: never block.
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        # the process-global LOKI_HOME (the default profile) is ticked. Heartbeats and recovery are also
        # scoped per profile so `loki cron status` reflects liveness for every profile independently.
        if profile_homes:
        ...
        # Heartbeat before the first sleep so `loki cron status` sees a live ticker immediately.
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # Persist the reason so `loki cron status` (separate process) shows WHY.
