...
(``loki cron notepad <job_id> set ...``) via the terminal tool; no model tool is added.
"""
...
# ticks (set_loki_home_override) cannot leak one profile's notepad rows into the import-time home
...
def _current_notepad_file() -> Path:
    return NOTEPAD_FILE or (get_loki_home().resolve() / "cron" / "notepad.db")
...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    prepare_ledger(conn, db_label="cron/notepad.db", synchrowundercorp_full=False)
    conn.execute(
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`loki cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `loki cron notepad {job_id} delete "
