...
import { SelectionSwitcher } from "@wundercorp/loki-ui/ui/components/selection-switcher";
...
import { useBelowBreakpoint } from "@wundercorp/loki-ui/hooks/use-below-breakpoint";
...
const SIDEBAR_COLLAPSED_KEY = "loki-sidebar-collapsed";
...
export default function App() {
  ...
  // asynchrowundercorply from /api/dashboard/plugins, so on initial render
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="loki-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="loki-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateLoki = status?.can_update_loki === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'loki update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "loki update";
    return (
      t.status.updateLokiConfirmMessage ??
      `This will run 'loki update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateLokiConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateLoki) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateLoki,
      runningLabel: t.status.updatingLoki,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Loki gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateLokiConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateLokiConfirmTitle ?? `${t.status.updateLoki}?`}
    />
    </>
  );
}
