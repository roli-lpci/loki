...
import { Badge } from "@wundercorp/loki-ui/ui/components/badge";
import { Button } from "@wundercorp/loki-ui/ui/components/button";
import { Spinner } from "@wundercorp/loki-ui/ui/components/spinner";
import { H2 } from "@wundercorp/loki-ui/ui/components/typography/h2";
import { Card, CardContent } from "@wundercorp/loki-ui/ui/components/card";
import { Checkbox } from "@wundercorp/loki-ui/ui/components/checkbox";
import { Input } from "@wundercorp/loki-ui/ui/components/input";
import { Label } from "@wundercorp/loki-ui/ui/components/label";
import { Select, SelectOption } from "@wundercorp/loki-ui/ui/components/select";
import { Toast } from "@wundercorp/loki-ui/ui/components/toast";
import { useToast } from "@wundercorp/loki-ui/hooks/use-toast";
import { useConfirmDelete } from "@wundercorp/loki-ui/hooks/use-confirm-delete";
import { ConfirmDialog } from "@wundercorp/loki-ui/ui/components/confirm-dialog";
import { useModalBehavior } from "@/hooks/useModalBehavior";
import { DeleteConfirmDialog } from "@/components/DeleteConfirmDialog";
import { LokiConsoleModal } from "@/components/LokiConsoleModal";
...
export default function SystemPage() {
  ...
  // spawned `loki import` runs non-interactively (stdin is /dev/null), so
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      if (status?.can_update_loki === false) return;
      setCheckingUpdate(true);
      try {
        const info = await api.checkLokiUpdate(force);
        setUpdateInfo(info);
        if (force) {
    ...
    [showToast, status?.can_update_loki],
  );
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_loki === false) {
      showToast(
        "Loki updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
    try {
      const resp = await api.updateLoki();
      if (!resp.ok) {
        showToast(
          resp.message ??
      ...
      setActiveAction(resp.name ?? "loki-update");
  ...
  const canUpdateLoki = status?.can_update_loki !== false;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <ConfirmDialog
        open={canUpdateLoki && updateConfirmOpen}
        onCancel={() => setUpdateConfirmOpen(false)}
        onConfirm={() => void applyUpdate()}
        title="Update Loki?"
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'loki update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'loki update' (${updateInfo?.update_command ?? "loki update"}) and restart the gateway when it finishes.`
        }
        ...
        <Card>
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Loki Agent</div>
                <div className="flex items-center gap-2">
                  <span>v{stats?.loki_version}</span>
                  {canUpdateLoki &&
                    updateInfo &&
      ...
      <section className="flex flex-col gap-3">
        <H2 variant="sm" className="flex items-center gap-2 text-muted-foreground">
          <Globe className="h-4 w-4" /> WunderCorp Portal
        </H2>
        <Card>
          <CardContent className="flex flex-col gap-3 py-4">
            <div className="flex items-center gap-3">
              ...
              <a
                href={portal?.subscription_url || "https://portal.wundercorp.com/manage-subscription"}
                target="_blank"
                rel="noreferrer"
            ...
            {!portal?.logged_in && (
              <p className="text-xs text-muted-foreground">
                Log in with <span className="font-mono">loki portal</span>.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$LOKI_HOME/backups/loki-backup.zip"
                />
              </div>
              <Button
                size="sm"
            ...
            <ConfirmDialog
              ...
              title="Restore full Loki backup?"
              description={`This will overwrite your current Loki configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the Loki team. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>
