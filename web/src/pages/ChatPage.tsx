/**
 * ChatPage — embeds `loki --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (loki_cli/web_server.py)                          .
 *          ▼                                                              .
 *     POSIX PTY → `node tui-ui/dist/entry.js` → tui_gateway + AIAgent     .
 */
...
import { Typography } from "@wundercorp/loki-ui/ui/components/typography/index";
...
const PTY_ATTACH_TOKEN_KEY = "loki.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__LOKI_SESSION_TOKEN__ &&
    !window.__LOKI_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `loki dashboard`, not directly."
      : null,
  );
  ...
  const [chatPanelCollapsed, setChatPanelCollapsed] = useState(
    () => localStorage.getItem("loki-chat-panel-collapsed") === "1",
  );
  const toggleChatPanel = useCallback(() => {
    setChatPanelCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem("loki-chat-panel-collapsed", next ? "1" : "0");
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const gated = !!window.__LOKI_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Loki TUI has enabled xterm mouse-events
    ...
    //      payloads upload to LOKI_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to LOKI_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[loki-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__LOKI_AUTH_REQUIRED__``) this
    // awaits a single-use ticket via /api/auth/ws-ticket before opening;
    // in loopback mode it resolves synchrowundercorply against the injected
    // session token. The IIFE keeps the outer effect synchrowundercorp so its
    ...
    forceFreshPtyRef.current = false;
    // A connect attempt is now in flight — set synchrowundercorply (before the async
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets LOKI_HOME pointed at the
    ...
    // forwarding them into Loki. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="loki-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
