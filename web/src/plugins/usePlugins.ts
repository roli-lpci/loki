...
import { api, LOKI_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "loki:plugin-manifests";
...
export function usePlugins() {
  ...
  // registered synchrowundercorply on the first render after a refresh.
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${LOKI_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${LOKI_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?loki_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-loki-plugin", manifest.name);
