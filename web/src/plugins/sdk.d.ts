/**
 * Loki Dashboard Plugin SDK — typed contract (SPIKE)
 * ====================================================
 *
 * This is the public type surface for ``window.__LOKI_PLUGIN_SDK__`` and
 * ``window.__LOKI_PLUGINS__``, the globals the dashboard host exposes to
 ...
 * window.__LOKI_PLUGIN_SDK__`` because:
 *   1. The runtime object is assembled from many internal modules
 *      (``@/lib/api``, ``@wundercorp/loki-ui``, …). Deriving the type would
 ...
 *
 * Versioning: bump ``LokiPluginSDK["sdkVersion"]`` (and the
 ...
 *   - Ship as a published ``@wundercorp/loki-dashboard-plugin-sdk`` types package, or
 ...
 *     (pulls @wundercorp/loki-ui types into the contract) or is the loose
 ...
 * of calling ``fetch`` with a hand-read ``window.__LOKI_SESSION_TOKEN__``.
 */
...
// ---------------------------------------------------------------------------
// Registry surface (window.__LOKI_PLUGINS__)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// SDK surface (window.__LOKI_PLUGIN_SDK__)
// ---------------------------------------------------------------------------

export interface LokiPluginSDK {
   ...
   * ``Promise<T>``, a few return a URL string synchrowundercorply); plugins call the
  ...
  /**
   * Shared UI primitives (WunderCorp DS / shadcn). Typed permissively at the
...
declare global {
  interface Window {
    __LOKI_PLUGIN_SDK__?: LokiPluginSDK;
    __LOKI_PLUGINS__?: PluginRegistry;
  }
}
