...
interface LokiConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function LokiConsoleModal({ open, onClose }: LokiConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("loki> ");
  const inputPromptRef = useRef("loki> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "loki> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Loki Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="loki-console-title"
    >
      <div
        className={cn(
          themedBody,
