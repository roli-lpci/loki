#!/usr/bin/env python3
"""SQLite state store for Loki Agent: session metadata, message history, model
...
from loki_state_common import escape_like as _escape_like, stat_db_file_identity as _stat_db_file_identity
from loki_state_errors import (
...
from loki_state_readpool import _READ_POOL_MAX, _proc_fd_targets, _read_budget_for
from loki_state_sessions import SessionSessionsMixin
from loki_state_fts import SessionFtsSetupMixin, load_fts5_cjk_extension
from loki_state_portability import SessionPortabilityMixin
from loki_state_telegram import SessionTelegramTopicsMixin
from loki_state_schema import SessionSchemaMixin
import loki_state_holders as _state_holders
from loki_state_dbfile import (
...
from loki_state_messages import SessionMessagesMixin
from loki_state_wal import _WAL_INCOMPAT_MARKERS, apply_database_pragmas, apply_wal_with_fallback
from loki_state_repair import _claim_repair_attempt, preflight_db_writability, repair_state_db_schema
from loki_state_titles import SessionTitlesMixin
from loki_state_usage import SessionUsageMixin
from loki_state_maintenance import SessionMaintenanceMixin
from loki_state_gateway import SessionGatewayMixin
from loki_state_compression import SessionCompressionMixin
from loki_state_search import SessionSearchMixin
...
DEFAULT_DB_PATH = _IMPORT_DEFAULT_DB_PATH = get_loki_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    ``get_loki_home()`` is resolved fresh (a runtime LOKI_HOME redirect works regardless of import)."""
    return DEFAULT_DB_PATH if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH else get_loki_home() / "state.db"


# Live-DB guard knobs live HERE (not in loki_state_guard): the hermetic conftest monkeypatches
# ``loki_state._STATE_DB_GUARD_BYPASS`` (``@pytest.mark.live_system_guard_bypass`` escape hatch)
# and ``_EXTRA_DENY_ROOTS`` (the pre-sandbox root, so custom-LOKI_HOME deployments are covered).
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    ``LOKI_HOME`` together, which is precisely the state in which it writes to production (#82770).
    """
    ...
    for root in roots:
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Loki root {root}). "
                "Tests must run against a temporary LOKI_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "LOKI_HOME. If this test genuinely needs the live database, mark it with "
...
def divert_session_transcript_jsonl(session_id: str, messages) -> "Optional[Path]":
    """Append pending messages to LOKI_HOME/sessions/<id>.jsonl (state.db was replaced under a
    live process). Returns the path, or None if nothing to write."""
    sid = str(session_id or "").strip()
    if not sid or not messages:
        return None
    sessions_dir = get_loki_home() / "sessions"
...
# connection per resolved path via loki_state_registry.acquire(); one-shots use SessionDB() + close().
...
# ── Process-wide shared SessionDB registry (#90837) ── lives in loki_state_registry.py (acquire /
# release / close_all / release_or_close). Long-lived in-process callers (gateway, tui_gateway, cron,
# in-process tools) share ONE writer connection per resolved path via loki_state_registry.acquire(); CLI
...
class SessionDB(
    ...
    # SQLite's deterministic busy handler convoys under many loki processes: keep its
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        # Opened via loki_state_registry.acquire(): close() releases a refcount instead.
        # Set True when this instance is opened via loki_state_registry.acquire(). Makes close() a no-op so the
    ...
    def _open_writer(self) -> None:
        ...
        try:
            ...
            if not repair_state_db_schema(self.db_path).get("repaired"):
                raise
            self._connect_and_init_with_lock_patience()
        # FTS optimization is OPT-IN (`loki db optimize`); no background worker races session lifecycle.
    ...
    def _handle_quarantine_if_invalid(self, already_locked: bool = False) -> None:
        ...
        msg = (
            ...
            f"Restore from {self.db_path.parent / 'state-snapshots'} via `loki snapshot list` / "
            f"`loki snapshot restore <id>` if available, or salvage the preserved bytes with "
            f"`loki sessions recover --source {qpath or self.db_path}`. "
            ...
            try:
                ...
                if isinstance(exc, sqlite3.OperationalError):
                    if "locked" in err_msg or "busy" in err_msg:
                        ...
                        raise sqlite3.OperationalError(
                            f"database is locked (another Loki process held the "
    ...
    def _halt_db_corrupt(self, exc: BaseException) -> None:
        ...
        logger.error(
            ...
            "gateway and run `loki sessions recover --source %s --inspect-only`.", self.db_path, exc,
            self.db_path,
        )
    ...
    def _foreign_state_db_holders(self) -> List[Tuple[int, str]]:
        """Foreign processes holding this DB or its WAL sidecars (see loki_state_holders)."""
    ...
    def close(self):
        ...
        When this instance is shared (opened via ``loki_state_registry.acquire``), ``close()`` RELEASES one
        ...
        if self._shared_registry_owned:
            from loki_state_registry import release
            release(self)
            return
            ...
            if self._conn:
                ...
                if quarantine_reason is not None:
                    logger.warning(
                        ...
                        "before restarting, then run `loki sessions recover --source %s --inspect-only`.",
...
_PLUGIN_COMPAT_LAZY = {
    'AUTO_VACUUM_MIN_FREELIST_RATIO': ('loki_state_common', 'AUTO_VACUUM_MIN_FREELIST_RATIO'),
    'ActivityProvenance': ('agent.session_activity', 'ActivityProvenance'),
    'CompressionSessionBusyError': ('loki_state_errors', 'CompressionSessionBusyError'),
    'CompressionSessionClosedError': ('loki_state_errors', 'CompressionSessionClosedError'),
    'DEFERRED_INDEX_SQL': ('loki_state_common', 'DEFERRED_INDEX_SQL'),
    'FTS_CJK_STALE_KEY': ('loki_state_common', 'FTS_CJK_STALE_KEY'),
    'FTS_CJK_TABLE_SQL': ('loki_state_fts', 'FTS_CJK_TABLE_SQL'),
    'FTS_CJK_TRIGGER_SQL': ('loki_state_fts', 'FTS_CJK_TRIGGER_SQL'),
    'FTS_REBUILD_DEFERRAL_KEY': ('loki_state_common', 'FTS_REBUILD_DEFERRAL_KEY'),
    'FTS_SQL': ('loki_state_common', 'FTS_SQL'),
    'FTS_STALE_KEY': ('loki_state_common', 'FTS_STALE_KEY'),
    'FTS_STORAGE_VERSION': ('loki_state_common', 'FTS_STORAGE_VERSION'),
    'FTS_TRIGRAM_SQL': ('loki_state_common', 'FTS_TRIGRAM_SQL'),
    'LEGACY_FTS_SQL': ('loki_state_common', 'LEGACY_FTS_SQL'),
    'LEGACY_FTS_TRIGRAM_SQL': ('loki_state_common', 'LEGACY_FTS_TRIGRAM_SQL'),
    'MAX_FTS5_QUERY_CHARS': ('loki_state_common', 'MAX_FTS5_QUERY_CHARS'),
    'PERSISTENCE_ERROR_CAUSES': ('loki_state_errors', 'PERSISTENCE_ERROR_CAUSES'),
    'SCHEMA_SQL': ('loki_state_common', 'SCHEMA_SQL'),
    'SCHEMA_VERSION': ('loki_state_common', 'SCHEMA_VERSION'),
    'SESSION_STATUS_COMPLETE': ('loki_state_sessions', 'SESSION_STATUS_COMPLETE'),
    'SESSION_STATUS_EMPTY': ('loki_state_sessions', 'SESSION_STATUS_EMPTY'),
    'SESSION_STATUS_ERROR': ('loki_state_sessions', 'SESSION_STATUS_ERROR'),
    'SESSION_STATUS_INTERRUPTED': ('loki_state_sessions', 'SESSION_STATUS_INTERRUPTED'),
    ...
    'SessionTurnLeaseLostError': ('loki_state_errors', 'SessionTurnLeaseLostError'),
    'WalUnsupportedError': ('loki_state_wal', 'WalUnsupportedError'),
    'apply_durability_barriers': ('loki_state_repair', 'apply_durability_barriers'),
    'classify_session_status': ('loki_state_sessions', 'classify_session_status'),
    'collect_state_db_stats': ('loki_state_dbfile', 'collect_state_db_stats'),
    'count_db_holders': ('loki_state_dbfile', 'count_db_holders'),
    'describe_skill_invocation': ('agent.skill_commands', 'describe_skill_invocation'),
    'fts5_cjk_so_path': ('loki_state_fts', 'fts5_cjk_so_path'),
    'is_advisory_lock_contention': ('loki_state_common', 'is_advisory_lock_contention'),
    'is_automatic_end_reason': ('loki_state_common', 'is_automatic_end_reason'),
    'is_disk_full_error': ('loki_state_errors', 'is_disk_full_error'),
    'is_sqlite_wal_reset_vulnerable': ('loki_state_wal', 'is_sqlite_wal_reset_vulnerable'),
    'is_transient_sqlite_error': ('loki_state_errors', 'is_transient_sqlite_error'),
    'iter_deleted_sqlite_sidecar_holders': ('loki_state_dbfile', 'iter_deleted_sqlite_sidecar_holders'),
    'release_or_close': ('loki_state_registry', 'release_or_close'),
    'report_startup_progress': ('loki_startup_watchdog', 'report_startup_progress'),
    'resolve_journal_mode': ('loki_state_wal', 'resolve_journal_mode'),
    'resolve_synchrowundercorp_level': ('loki_state_wal', 'resolve_synchrowundercorp_level'),
    'sanitize_context': ('agent.memory_manager', 'sanitize_context'),
    'sqlite_source_id': ('loki_state_wal', 'sqlite_source_id'),
    'workspace_key': ('loki_state_sessions', 'workspace_key'),
}
