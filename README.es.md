...
**El agente de IA con mejora continua creado por [WunderCorp, Inc.](https://wundercorp.co).** Es el único agente con un bucle de aprendizaje integrado: crea habilidades a partir de la experiencia, las mejora durante el uso, se impulsa a sí mismo a persistir el conocimiento, busca en sus propias conversaciones pasadas y construye un modelo cada vez más profundo de quién eres a lo largo de las sesiones. Ejecútalo en un VPS de $5, un clúster de GPUs o infraestructura sin servidor que cuesta casi nada cuando está inactivo. No está atado a tu laptop — habla con él desde Telegram mientras trabaja en una VM en la nube.

Usa cualquier modelo que quieras — [WunderCorp Portal](https://portal.wundercorp.com), [OpenRouter](https://openrouter.ai) (más de 200 modelos), [NovitaAI](https://novita.ai), [NVIDIA NIM](https://build.nvidia.com) (Nemotron), [Xiaomi MiMo](https://platform.xiaomimimo.com), [z.ai/GLM](https://z.ai), [Kimi/Moonshot](https://platform.moonshot.ai), [MiniMax](https://www.minimax.io), [Hugging Face](https://huggingface.co), OpenAI, o tu propio endpoint. Cambia con `loki model` — sin cambios de código, sin dependencias.
...
> **Nota:** En Windows nativo, Loki funciona sin WSL — la CLI, el gateway, la TUI y las herramientas funcionan de forma nativa. Si prefieres usar WSL2, el comando de Linux/macOS de arriba también funciona allí. ¿Encontraste un error? Por favor [crea un issue](https://github.com/wundercorp/loki/issues).
...
El instalador se encarga de todo: uv, Python 3.11, Node.js, ripgrep, ffmpeg, **y un Git Bash portátil** (MinGit, descomprimido en `%LOCALAPPDATA%\loki\git` — no requiere administrador, completamente aislado de cualquier instalación de Git del sistema). Loki usa este Git Bash incluido para ejecutar comandos de shell.
...
```bash
loki              # CLI interactiva — inicia una conversación
loki model        # Elige tu proveedor y modelo LLM
loki tools        # Configura qué herramientas están habilitadas
loki config set   # Establece valores de configuración individuales
loki gateway      # Inicia el gateway de mensajería (Telegram, Discord, etc.)
loki setup        # Ejecuta el asistente de configuración completo
loki claw migrate # Migra desde OpenClaw (si vienes de OpenClaw)
loki update       # Actualiza a la última versión
...
## Evita la colección de claves API — WunderCorp Portal

Loki funciona con cualquier proveedor que quieras — eso no cambiará. Pero si prefieres no recopilar cinco claves API separadas para el modelo, búsqueda web, generación de imágenes, TTS y un navegador en la nube, **[WunderCorp Portal](https://portal.wundercorp.com)** las cubre todas bajo una sola suscripción:
...
Loki tiene dos puntos de entrada: inicia la interfaz de terminal con `loki`, o ejecuta el gateway y habla con él desde Telegram, Discord, Slack, WhatsApp, Signal o Email. Una vez en una conversación, muchos comandos de barra son compartidos entre ambas interfaces.

| Acción                              | CLI                                           | Plataformas de mensajería                                                         |
| ----------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------- |
| Empezar a chatear                   | `loki`                                      | Ejecuta `loki gateway setup` + `loki gateway start`, luego envía un mensaje al bot |
...
Si vienes de OpenClaw, Loki puede importar automáticamente tu configuración, memorias, habilidades y claves API.

**Durante la configuración inicial:** El asistente de configuración (`loki setup`) detecta automáticamente `~/.openclaw` y ofrece migrar antes de que comience la configuración.

**En cualquier momento después de instalar:**

```bash
loki claw migrate              # Migración interactiva (preset completo)
loki claw migrate --dry-run    # Vista previa de qué se migraría
loki claw migrate --preset user-data   # Migrar sin secretos
loki claw migrate --overwrite  # Sobreescribir conflictos existentes
```
...
- **Habilidades** — habilidades creadas por el usuario → `~/.loki/skills/openclaw-imports/`
...
Consulta `loki claw migrate --help` para todas las opciones, o usa la habilidad `openclaw-migration` para una migración guiada interactiva por el agente con vistas previas de dry-run.
...
Inicio rápido para colaboradores — clona y comienza con `setup-loki.sh`:

```bash
git clone https://github.com/wundercorp/loki.git
cd loki-agent
./setup-loki.sh     # instala uv, crea venv, instala .[all], enlaza ~/.local/bin/loki
./loki              # detecta automáticamente el venv, no necesitas hacer `source` primero
```
...
- 🔌 [computer-use-linux](https://github.com/avifenesh/computer-use-linux) — Servidor MCP de control de escritorio Linux para Loki y otros hosts MCP, con árboles de accesibilidad AT-SPI, entrada Wayland/X11, capturas de pantalla y targeting de ventanas del compositor.
...
Creado por [WunderCorp, Inc.](https://wundercorp.co).
