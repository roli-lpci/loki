...
const lockedLog = [
  'error: failed to remove file `C:/install/venv/Lib/site-packages/../../Scripts/loki.exe`: Access is denied. (os error 5)',
  'File "C:/install/venv/Scripts/loki.exe/__main__.py", line 10, in <module>',
...
const base = {
  ...
  installMethod: 'installer-script', updateMethod: 'loki-update',
  error: 'E2E ASSERTION FAILED: loki update exited 1 (expected 0)', logs: { update: lockedLog },
}
...
describe('known install failures', () => {
  it('recognizes the released launcher self-lock, not generic access denied', () => {
    ...
    expect(matchKnownFailure({ ...base, logs: { update: lockedLog.replaceAll('loki.exe', 'other.exe') } })).toBeNull()
  ...
  it('matches manual-only app updates only for the three proven July script cases', () => {
    const sample = {
      ...
      updateMethod: 'loki-desktop-app-update',
      error: 'E2E ASSERTION FAILED: app driven via captured loki desktop spec; update completed',
      logs: { desktop: '[loki] [updates] no staged updater; surfacing manual `loki update` for CLI install at C:/install\n[loki] [updates] manual: loki update\n' },
    }
    ...
    expect(matchKnownFailure({ ...sample, logs: { desktop: '[updates] manual: loki update' } })).toBeNull()
  })
...
it('renders known receipts as footnotes, without suppressing a red job', async () => {
  ...
  const name = 'windows: installer-script -> loki-update (v2026.3.12 -> HEAD)'
