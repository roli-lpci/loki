 ...
 * dev:mock dev flow.
 *
 * Implements just enough of the /v1/* surface for `loki serve` to resolve a
...
/**
 * Drives a real code edit followed by two finish attempts. Loki should add
...
export function createBackgroundReleaseHandle(): BackgroundReleaseHandle {
  const path = nodePath.join(
    os.tmpdir(),
    `loki-e2e-bg-release-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
  )
...
/** Create an isolated LOKI_HOME + Electron user-data dir in the OS temp dir. */
function createDevSandbox(): DevSandbox {
  const root = fs.mkdtempSync(nodePath.join(os.tmpdir(), `loki-dev-mock-${Date.now()}`))
  const lokiHome = nodePath.join(root, 'loki-home')
...
function writeMockConfig(lokiHome: string, mockUrl: string): void {
  fs.writeFileSync(
    nodePath.join(lokiHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(nodePath.join(lokiHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function runDevLaunch(): Promise<void> {
  ...
  writeMockConfig(sandbox.lokiHome, mock.url)
  console.log(`  LOKI_HOME: ${sandbox.lokiHome}`)
  ...
  const env: Record<string, string> = {
    ...
    LOKI_DESKTOP_ROOT: repoRoot,
    LOKI_DESKTOP_APP_NAME: `LokiDevMock-${Date.now()}`,
  }
