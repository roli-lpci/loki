...
Loki Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``LOKI_NIX_BUILD=1`` on the
Loki package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("LOKI_NIX_BUILD") == "1"
...
_BLOCK_MESSAGE = (
    ...
    "the Loki Nix derivation sets LOKI_NIX_BUILD=1. If it does, file a bug."
)
...
# Root single-file modules (``run_agent``, ``loki_state``, ``toolsets``...)
