#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `loki kanban list` and
...
def loki_available() -> bool:
    return shutil.which("loki") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed loki CLI."""
    try:
        out = subprocess.run(
            ["loki", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `loki kanban list`
    out = subprocess.run(
        ["loki", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def main():
    ...
    if not loki_available():
        print("ERROR: 'loki' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
