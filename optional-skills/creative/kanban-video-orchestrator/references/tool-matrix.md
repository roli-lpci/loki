...
Maps each role archetype to the Loki skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public loki-agent
...
## Loki skills relevant to video production

### Visual / rendering skills (`loki-agent/skills/creative/`)
...
### Design / pre-production skills (`loki-agent/skills/creative/`)
...
### Audio / media skills (`loki-agent/skills/creative/` + `skills/media/`)
...
These are **not** Loki skills but external CLIs / APIs that profiles invoke.
...
## Built-in Loki tools for media review

These are native Loki tools — not invoked via terminal but through their own
...
key is present in `${LOKI_HOME:-~/.loki}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Loki profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${LOKI_HOME:-~/.loki}/.env` → environment variable.
...
When a new Loki-public video skill ships:
