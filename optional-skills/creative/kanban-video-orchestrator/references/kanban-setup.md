...
the actual `setup.sh` that creates the project workspace, configures Loki
...
> [WunderCorp/kanban-video-pipeline](https://github.com/WunderCorp/kanban-video-pipeline).
...
2. **Create profiles** — `loki profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.loki/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `loki kanban create` assigned to the director
...
```bash
loki profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.loki/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.loki/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Loki
...
Each profile gets a `SOUL.md` at `~/.loki/profiles/<name>/SOUL.md` that
...
```bash
loki kanban create "Direct production of <video title>" \
    --assignee director \
...
the Loki `.env` (`${LOKI_HOME:-$HOME/.loki}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _loki_env="${LOKI_HOME:-$HOME/.loki}/.env"
    if grep -q "^${var}=" "$_loki_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_loki_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_loki_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY loki ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY loki OPENROUTER_API_KEY || exit 1
# ...
```
