...
loki kanban watch --tenant <project-slug>

# Snapshot of the board
loki kanban list --tenant <project-slug>
loki kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
loki kanban stats --tenant <project-slug>

# Visual dashboard (browser)
loki dashboard

# Inspect a specific task (includes comments + events)
loki kanban show <task-id>

# Follow a single task's event stream
loki kanban tail <task-id>
```

Verify available subcommands with `loki kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `loki kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `loki kanban block <id>` then `loki kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `loki kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `loki kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `loki profile list` (profile exists?), `loki status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(loki kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
loki kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
loki kanban block <task-id>
loki kanban archive <task-id>

# Diagnose what happened
loki kanban show <task-id>      # task body, comments, recent events
loki kanban tail <task-id>      # follow the live event stream
loki kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `loki kanban create` for the director
...
while true; do
    clear
    loki kanban list --tenant <slug>
    echo "---"
    loki kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `loki kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `loki profile list` and start
