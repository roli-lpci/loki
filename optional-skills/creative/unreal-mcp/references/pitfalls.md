...
### 1. Start order: editor first, Loki second

Loki probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Loki session.
...
restart the editor, restart the Loki session.
...
server fails to bind (Output Log) or Loki' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.loki/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Loki

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Loki' connection
lives in `~/.loki/config.yaml` via `loki mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Loki — just don't
mistake it for the Loki setup step.
...
explicitly warns against overlapping calls. Loki executes same-turn tool
...
### 9. Timeouts: Loki gives up before Unreal does

Loki' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Loki runs elsewhere (SSH backend,
