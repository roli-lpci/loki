...
LOKI_HOME_DIR="${LOKI_HOME:-$HOME/.loki}"
LOKI_CFG="${LOKI_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Loki config has twozero_td MCP entry ──
if [[ ! -f "$LOKI_CFG" ]]; then
    echo -e " ${FAIL} Loki config not found at ${LOKI_CFG}"
    manual_steps+=("Create ${LOKI_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$LOKI_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Loki config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Loki config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${LOKI_CFG} manually"); }
    manual_steps+=("Restart Loki session to pick up config change")
fi
