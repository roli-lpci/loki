...
Usage (via cron with --no-agent):
    ...
    loki cron create loki-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$LOKI_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name loki-issues --repo wundercorp/loki --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Loki .env file
(``${LOKI_HOME:-~/.loki}/.env``) to avoid the 60 req/hr
