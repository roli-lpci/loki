...
Usage (via cron with --no-agent):
    ...
    loki cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$LOKI_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Loki-Watcher/1.0"})
