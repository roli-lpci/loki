...
Usage:
  ...
  <server_name> matches the files in $LOKI_HOME/mcp-tokens/<server>.json etc.
...
def _loki_home():
    # Prefer Loki' own resolver (profile-safe); fall back to env then ~/.loki.
    try:
        from loki_constants import get_loki_home
        return str(get_loki_home())
    except Exception:
        return os.environ.get("LOKI_HOME") or os.path.expanduser("~/.loki")


def _tokens_dir():
    return os.path.join(_loki_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "loki-diag", "version": "1.0"}},
        },
        headers={
