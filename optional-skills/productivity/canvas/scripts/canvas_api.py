#!/usr/bin/env python3
"""Canvas LMS API CLI for Loki Agent.
...
def _check_config():
    ...
    if missing:
        loki_env = os.path.join(
            os.environ.get("LOKI_HOME", os.path.expanduser("~/.loki")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {loki_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Loki Agent"
    )
