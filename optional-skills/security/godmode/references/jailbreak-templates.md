...
## 5. GODMODE FAST — Loki 4 405B (Zero Refusal)

**Model:** `wundercorp/loki-4-405b`
**Strategy:** Loki models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Loki Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.loki/config.yaml`:
