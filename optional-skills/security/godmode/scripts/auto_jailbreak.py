...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("LOKI_HOME", os.path.expanduser("~/.loki")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("LOKI_HOME", Path.home() / ".loki")) / "skills" / "red-teaming" / "godmode"
...
LOKI_HOME = Path(os.getenv("LOKI_HOME", Path.home() / ".loki"))
CONFIG_PATH = LOKI_HOME / "config.yaml"
PREFILL_PATH = LOKI_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # WunderCorp/Loki models — already uncensored, just needs clean prompt
    "loki": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "loki" in model_lower or "wundercorp" in model_lower:
        return "loki"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Loki config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.loki/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Loki for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Loki for changes to take effect.")
