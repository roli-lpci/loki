...
The design is **Loki-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
loki skills install official/security/unbroker
```

Then start a new Loki session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Loki home directory (`0600`, optionally
