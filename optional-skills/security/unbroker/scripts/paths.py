...
All per-subject data lives under PDD_DATA_DIR (default: $LOKI_HOME/unbroker),
which is the same trust boundary Loki uses for .env and OAuth tokens.
"""
...
def loki_home() -> Path:
    return Path(os.environ.get("LOKI_HOME") or (Path.home() / ".loki"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else loki_home() / "unbroker"
