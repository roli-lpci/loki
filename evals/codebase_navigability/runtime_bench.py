...
Each probe runs in a FRESH subprocess with an isolated LOKI_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "LOKI_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "LOKI_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("LOKI_SESSION", "LOKI_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "loki_cli.main", "gateway.run", "tools.registry", "loki_state", "tui_gateway.server", "loki_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "loki-agent" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `loki --version`, `loki --help`, `loki doctor --help`, `loki config get model` (no network)
CLI = {"version": ["loki_cli/main.py", "--version"], "help": ["loki_cli/main.py", "--help"], "config_get": ["loki_cli/main.py", "config", "get", "model"], "tools_list": ["loki_cli/main.py", "tools", "--help"], "skills_help": ["loki_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_loki_default_ms", lambda: resolve_toolset("loki-default"), 200)
...
try:
    import loki_state, tempfile, uuid
    from pathlib import Path
    db=loki_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(loki_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())
