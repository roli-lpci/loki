...
def main():
        ...
        def do_POST(self):
            ...
            requests.append({"grant_type": payload.get("grant_type"),
                             "target_grant": (payload.get("refresh_token") == ["fixture-refresh-1"]
                                              or self.headers.get("x-wundercorp-refresh-token") == "fixture-refresh-1")})
            body = ({"access_token": wundercorp_new_token if self.path == "/api/oauth/token" else "fixture-new-access",
    ...
    wundercorp_new_token = token("singleton-renewed")
    ...
    cases.extend((name, "wundercorp", ["refresh", "wundercorp", target], None) for name, target in
                 (("wundercorp-independent", "row0"), ("wundercorp-singleton", "row1")))
    results = []
    try:
            ...
            with tempfile.TemporaryDirectory(prefix="auth-pool-pty-") as temp:
                ...
                if provider == "wundercorp":
                    ...
                    state = dict(access_token=token("singleton"), refresh_token="fixture-refresh-1",
                                 ...
                                 scope="inference:invoke", inference_base_url="https://inference-api.wundercorp.com/v1")
                ...
                env = {k: v for k, v in os.environ.items() if not any(t in k for t in
                       ("TOKEN", "API_KEY", "SECRET", "PASSWORD", "LOKI", "PYTEST"))}
                env.update(HOME=temp, LOKI_HOME=temp, LOKI_SHARED_AUTH_DIR=str(home / "shared"), PYTHONPATH=str(Path(args.repo).absolute()), TERM="xterm")
                ...
                results.append({"case": name, "exit": exit_code, "output": output,
                                ...
                                "target_rotated": any(e["id"] == "row1" and e.get("access_token") == (wundercorp_new_token if provider == "wundercorp" else "fixture-new-access") for e in disk),
    ...
    independent = next(r for r in results if r["case"] == "wundercorp-independent")
    ...
    singleton = next(r for r in results if r["case"] == "wundercorp-singleton")
