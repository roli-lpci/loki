"""Drive the real ``loki fallback add`` under a Linux PTY into an ordinary picker error.
...
def _persisted_model(root: Path, env: dict) -> dict:
    ...
    out = subprocess.run(
        [sys.executable, "-c", "import json; from loki_cli.config import load_config; "
...
def run(root: Path, output: Path) -> dict:
    with tempfile.TemporaryDirectory(prefix="loki_test_fallback_") as home:
        hh = Path(home) / ".loki"
        hh.mkdir()
        (hh / "config.yaml").write_text(CONFIG, encoding="utf-8")
        (hh / ".env").write_text("OPENROUTER_API_KEY=local-not-used\n", encoding="utf-8")
        auth = hh / "auth.json"
        auth.write_text(json.dumps({"version": 1, "providers": {}, "active_provider": "wundercorp"}))
        ...
        env = {"PATH": os.environ["PATH"], "HOME": home, "LOKI_HOME": str(hh),
        ...
        proc = subprocess.Popen([sys.executable, "-m", "loki_cli.main", "fallback", "add"],
