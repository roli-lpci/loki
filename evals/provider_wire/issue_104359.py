...
home = tempfile.mkdtemp(prefix="loki-104359-")
os.environ.clear()
os.environ.update(
    HOME=home,
    LOKI_HOME=home + "/.loki",
...
Path(os.environ["LOKI_HOME"], "config.yaml").write_text(yaml.safe_dump(config), encoding="utf-8")
...
db = SessionDB(Path(os.environ["LOKI_HOME"]) / "state.db")
