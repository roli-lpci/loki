...
def main():
    ...
    home = Path(tempfile.mkdtemp(prefix='loki-acp-empty-'))
    loki = home / '.loki'
    loki.mkdir()
    (loki / 'config.yaml').write_text(
    ...
    env.update(HOME=str(home), LOKI_HOME=str(loki), PYTHONPATH=os.pathsep.join([str(repo), os.environ.get('PYTHONPATH', '')]),
               LOKI_ACP_SKIP_CONFIGURED_MCP='1', OPENAI_API_KEY='local-fixture-key',
    ...
    def rows():
        with sqlite3.connect(loki / 'state.db') as db:
