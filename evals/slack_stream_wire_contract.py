...
async def run(repo: str, out: str):
    ...
    for m in list(sys.modules):
        if m.startswith(("gateway", "plugins", "tools", "loki", "agent")):
            del sys.modules[m]
    ...
    r1 = await adapter.send_native_task_card_progress(
        ...
        metadata=metadata, fallback_text="Loki is working\n- terminal - ls - running")
...
def main():
    ...
    a = ap.parse_args()
    os.environ.setdefault("LOKI_HOME", os.path.join(os.path.dirname(a.out), "loki-home-probe"))
    os.makedirs(os.environ["LOKI_HOME"], exist_ok=True)
