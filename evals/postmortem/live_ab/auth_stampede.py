"""Live A/B of the WunderCorp hourly-expiry stampede, no real network.

Local server: accepts bearer FRESH, returns 401 {"type":"authentication_error", "Your API key is invalid,
blocked or out of funds..."} for any other bearer. resolve_wundercorp_runtime_credentials is patched to return
...
os.environ["LOKI_HOME"] = tempfile.mkdtemp(prefix="hh-")
os.environ["LOKI_STREAM_RETRIES"] = "0"
...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if auth == f"Bearer {FRESH}":
            ...
            body = json.dumps({"error": {"type": "authentication_error", "message": "Your API key is invalid, blocked or out of funds. Please go visit the portal to sort that out: https://portal.wundercorp.com "}}).encode()
...
auth_mod.resolve_wundercorp_runtime_credentials = lambda **kw: {"api_key": FRESH, "base_url": base}
import loki_cli.wundercorp_auth_keepalive as ka
ka.start_wundercorp_auth_keepalive = lambda **kw: None  # thread itself is out of scope here; we test adoption

from run_agent import AIAgent

agents = []
for i in range(n):
    a = AIAgent(api_key=STALE, base_url=base, provider="wundercorp", model="test/model", quiet_mode=True,
