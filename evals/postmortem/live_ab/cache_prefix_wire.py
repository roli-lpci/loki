...
message prefix between call N and N+1. If Loki strips prior-turn thinking, call N+1's messages[:k]
...
#   python cache_prefix_wire.py <repo_root> <A|B> [--loki-home DIR]   (default LOKI_HOME: the real one, for credentials)
sys.path.insert(0, sys.argv[1])
os.environ.setdefault("LOKI_HOME", os.path.expanduser("~/.loki"))
if "--loki-home" in sys.argv:
    os.environ["LOKI_HOME"] = sys.argv[sys.argv.index("--loki-home") + 1]
...
rt = resolve_runtime_provider(requested="wundercorp", target_model="anthropic/claude-fable-5.1")
sid = f"f0wire_{arm}_{int(time.time())}"
ag = AIAgent(model="anthropic/claude-fable-5.1", provider="wundercorp", base_url=rt.get("base_url"), api_key=rt.get("api_key"),
...
time.sleep(1)
log = subprocess.run(f"grep -h '\\[{sid}\\]' ~/.loki/logs/agent.log | grep 'API call #'", shell=True, capture_output=True).stdout.decode("utf-8", "replace")
