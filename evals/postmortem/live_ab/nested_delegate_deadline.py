...
real result. Uses glm-5.3 via WunderCorp for cost. Deadline shortened via config to keep the run short."""
...
# Temp LOKI_HOME with the real auth + a config that shortens the generic sequential deadline to 40 s, so the
...
home = tempfile.mkdtemp(prefix="dl_home_"); os.environ["LOKI_HOME"] = home
real_home = os.environ.get("LOKI_HOME_SOURCE", os.path.expanduser("~/.loki"))  # credentials are copied from here into a temp home
...
rt = resolve_runtime_provider(requested="wundercorp", target_model=MODEL)
sid = f"dl_{arm}_{int(time.time())}"
ag = AIAgent(model=MODEL, provider="wundercorp", base_url=rt.get("base_url"), api_key=rt.get("api_key"), api_mode=rt.get("api_mode"),
...
# at depth>0 delegate_task runs synchrowundercorply inside the tool call, which is the path under the deadline.
