...
def main():
    ...
    env = {"PATH": "/usr/bin:/bin", "HOME": str(root / "home"),
           "LOKI_HOME": str(root / "home/.loki"),
           "XDG_DATA_HOME": str(root / "xdg"), "LANG": "C.UTF-8",
           "LOKI_NONINTERACTIVE": "1"}
    Path(env["LOKI_HOME"]).mkdir(parents=True)
    venv_dir = root / "venv"
    ...
    install = (
        ...
        "from loki_cli.linux_desktop_entry import install_desktop_entry; "
        f"sys.argv[0]={str(repo / 'loki')!r}; "
    ...
    rows["venv_import"] = run([str(python), "-I", "-c", "import yaml,loki_cli.main; print(yaml.__version__)"], env, "/")
    rows["base_import_negative"] = run([str(python.resolve()), "-I", "-c", "import yaml,loki_cli.main"], env, "/")
    ...
    custom = original.replace(b"Name=Loki\n", b"Name=Loki custom\n").replace(b"Terminal=false", b"Terminal=true")
    entry.write_bytes(custom)
    config = Path(env["LOKI_HOME"]) / "config.yaml"
