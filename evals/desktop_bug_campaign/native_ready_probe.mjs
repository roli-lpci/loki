...
const home = mkdtempSync(join(tmpdir(), 'loki-native-ready-'))
const token = randomUUID()
const env = {
  PATH: process.env.PATH, HOME: home, LOKI_HOME: home,
  LANG: 'C.UTF-8', PYTHONUNBUFFERED: '1', LOKI_NONINTERACTIVE: '1',
  LOKI_DASHBOARD_SESSION_TOKEN: token, LOKI_SERVE_HEADLESS: '1',
  LOKI_PARENT_PID: String(process.pid),
}
const child = spawn(process.argv[2] || join(repo, '.venv/bin/python'),
  ['-m', 'loki_cli.main', 'serve', '--host', '127.0.0.1', '--port', '0', '--isolated'],
  { cwd: repo, env, stdio: ['ignore', 'pipe', 'pipe'] })
let tail = ''
for (const stream of [child.stdout, child.stderr]) {
  stream.on('data', chunk => { tail = (tail + chunk.toString()).slice(-32768) })
}
// Match production's synchrowundercorp attach-then-wait order, without synthetic output.
...
try {
  ...
  for (let i = 0; i < 20; i++) {
    for (const path of ['/api/status', '/api/profiles']) {
      const response = await fetch(`http://127.0.0.1:${port}${path}`, { headers: { 'X-Loki-Session-Token': token } })
