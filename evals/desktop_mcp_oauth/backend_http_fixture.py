...
def run_probe(repo, receipt):
    ...
    owner = Path(os.environ["LOKI_HOME"])
    ...
    with serving(Provider()) as provider, serving(CallbackServer(
        ...
        reloaded = asyncio.run(LokiTokenStorage("positive", loki_home=owner).get_tokens())
        ...
        override = set_loki_home_override(other)
        try:
...
def main():
    ...
    if args.cold_probe:
        sys.path.insert(0, str(repo))
        logging.disable(logging.CRITICAL)
        from loki_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    if not args.isolated_worker:
        with tempfile.TemporaryDirectory(prefix="loki-oauth-http-") as temp:
            home = Path(temp)
            (home / ".loki").mkdir()
            env = {"PATH": os.environ.get("PATH", ""), "HOME": temp,
                   "LOKI_HOME": str(home / ".loki"), "LANG": "C.UTF-8", "TZ": "UTC",
                   "PYTHONWUNDERCORPERSITE": "1"}
