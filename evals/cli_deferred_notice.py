...
def run_case(root, output, name, behind, early=False, cancel=False):
    with tempfile.TemporaryDirectory(prefix="loki_test_notice_") as home:
        hh = Path(home) / ".loki"
        hh.mkdir()
        ...
        from loki_cli.banner import VERSION
        ...
        bootstrap = ("import loki_cli.main as m; import loki_cli.banner as b; "
        ...
        try:
            ...
            result = {"case": name, "ready": ready, "exited": exited,
                      ...
                      "loaded_worktree": str(root / "loki_cli/banner.py") in text,
