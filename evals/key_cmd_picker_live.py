...
Run with the Loki venv Python and --repo CHECKOUT --output RECEIPT_DIR.
No user environment/config is inherited by Loki children. Unix PTY required.
"""
import argparse
import http.server
import json
...
def rows_worker():
    ...
    from loki_cli.model_switch_providers import (
    ...
    b = _PickerBuild(current_provider="", current_base_url="", current_model="",
                     max_models=None, for_picker=True, force_fresh_wundercorp_tier=False,
...
def cli_pty(repo, env):
    ...
    proc = subprocess.Popen([sys.executable, "-m", "loki_cli.main", "model"],
    ...
    try:
        for schema in ("providers", "custom_providers"):
            for surface in ("cli", "rows"):
                ...
                with tempfile.TemporaryDirectory(prefix="loki-keycmd-live-") as tmp:
                    home = Path(tmp)
                    state = home / ".loki"
                    state.mkdir()
                    ...
                    env = {"HOME": str(home), "LOKI_HOME": str(state),
