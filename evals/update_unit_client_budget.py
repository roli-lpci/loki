...
os.environ.clear()
os.environ.update(allowed, HOME=str(home), LOKI_HOME=str(home / "loki"))
...
def scoped_systemctl(argv, *, timeout):
    if "list-units" in argv:
        ...
        argv = [arg for arg in argv if arg not in ("loki-gateway*", "loki-serve*")] + [unit + ".service"]
...
unit = f"loki-serve-audit-089c35aa-{tag}"
cmd = ["systemctl", "--user"]
def run(args):
