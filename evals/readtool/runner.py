"""Run the read-tool eval through the REAL Loki AIAgent.

For each task: fresh temp LOKI_HOME, fresh fixture workspace, real
...
Usage:
  python3 evals/readtool/runner.py --model anthropic/claude-opus-4.8 \\
      --provider wundercorp --reps 3 --label baseline
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    loki_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".loki"
    ...
    try:
        ...
        shutil.rmtree(loki_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.loki/.env; set +a  — then relaunch."
        )
