"""
Tests for mcp_serve — Loki MCP server.
...
def _isolate_loki_home(tmp_path, monkeypatch):
    """Redirect LOKI_HOME to a temp directory."""
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking loki_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <LOKI_HOME>/state.db for its mtime gate; the autouse
        # fixture points LOKI_HOME at tmp_path.
        db_path = tmp_path / "state.db"
