...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("loki.gateway.up"), _M("loki.gateway.active_agents"),
        _M("loki.gateway.busy"), _M("loki.gateway.drainable"),
        _M("loki.gateway.restart_requested"),
        _M("loki.platform.up"), _M("loki.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("loki.cron.scheduler.heartbeat_age_seconds"),
        _M("loki.cron.scheduler.last_success_age_seconds"),
        _M("loki.cron.scheduler.catch_up_occurrences"),
        _M("loki.cron.jobs.enabled"), _M("loki.cron.jobs.running"),
        _M("loki.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Loki Agent-owned Relay transport health" in text
