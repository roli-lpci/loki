...
def progress(tmp_path):
    ...
    status = tmp_path / "loki-update-status"
...
# Stands in for `loki update`, and reports the stage that was on screen
...
n="$(cat "$LOKI_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$LOKI_TEST_CALLS"
for f in "$TMPDIR"/loki-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$LOKI_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$LOKI_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "loki-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    loki = install_root / "venv" / "bin" / "loki"
    loki.write_text(FAKE_LOKI)
    loki.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "LOKI_TEST_CAPTURE": str(capture),
        "LOKI_TEST_CALLS": str(calls),
        "LOKI_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".loki-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`loki update` is the longest wait in the hand-off and the one the
