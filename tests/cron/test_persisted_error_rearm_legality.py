...
def cron_store(tmp_path, monkeypatch):
    ...
    (loki_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("LOKI_HOME", str(loki_home))
    monkeypatch.setattr(J, "LOKI_DIR", loki_home)
    monkeypatch.setattr(J, "CRON_DIR", loki_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", loki_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", loki_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_loki_now", lambda: now):
            due = J.get_due_jobs()
