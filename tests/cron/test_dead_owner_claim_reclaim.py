...
A one-shot ``loki cron run <job_id>`` used to background-dispatch the run
...
1. ``loki_cli.cron._job_action("run", ...)`` declares the channel stateless
   before invoking the cron API, so the background-dispatch path is gated off
   and the run executes synchrowundercorply to completion in the CLI process.
...
def _orphan_claimed_row(executions, job_id: str) -> str:
    ...
    Mirrors what a one-shot ``loki cron run`` leaves behind: a row stuck in
...
class TestOneShotCliRunIsSynchrowundercorp:
    @pytest.fixture(autouse=True)
    ...
    def test_cli_run_declares_stateless_channel_before_dispatch(self, monkeypatch):
        """`loki cron run` must gate off async delivery so the run executes
        synchrowundercorply in the CLI process instead of on a doomed daemon thread."""
    ...
    def test_background_dispatch_refused_when_channel_stateless(self, monkeypatch):
        ...
        tool's background dispatcher must fall back to synchrowundercorp execution
        ...
        monkeypatch.setenv("LOKI_SESSION_KEY", "inherited-gateway-session")
