...
- Unlimited timeout (LOKI_CRON_TIMEOUT=0)
- Backward compat: LOKI_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """LOKI_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """LOKI_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("LOKI_CRON_TIMEOUT", "1200")
...
class TestSysPathOrdering:
    ...
    def test_loki_time_importable(self):
        """loki_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _loki_now
        assert callable(_loki_now)
