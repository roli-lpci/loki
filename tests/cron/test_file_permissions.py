...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("loki_cli.config.get_config_path", return_value=config_path), \
             patch("loki_cli.config.ensure_loki_home"):
    ...
    def test_ensure_loki_home_sets_0700(self):
        home = Path(self.tmpdir) / ".loki"
        with patch("loki_cli.config.get_loki_home", return_value=home):
