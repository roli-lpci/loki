...
the shared /blueprint command handler (loki_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated LOKI_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("loki://blueprint/morning-brief?")
