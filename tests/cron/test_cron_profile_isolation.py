...
profile's LOKI_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.loki/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active LOKI_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever LOKI_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    LOKI_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import loki_constants

    monkeypatch.setattr(
        loki_constants, "_get_platform_default_loki_home", lambda: root
    )
    monkeypatch.setenv("LOKI_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile LOKI_HOME (<root>/profiles/<name>), the cron store
    ...
    assert loki_constants.get_loki_home().resolve() == profile_home.resolve()
    assert loki_constants.get_default_loki_root().resolve() == root.resolve()

    # cron/jobs.py computes LOKI_DIR from get_loki_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.LOKI_DIR.resolve() == profile_home.resolve()
        assert (
