...
Covers:
  * LOKI_AGENT_HELP_GUIDANCE degrades to the docs-only variant when the
...
class TestLokiAgentHelpGuidance:
    def test_skill_variant_used_when_skill_view_present(self):
        from agent.prompt_builder import LOKI_AGENT_HELP_GUIDANCE
        assert "skill_view(name='loki-agent')" in LOKI_AGENT_HELP_GUIDANCE
...
class TestEssentialSkillsUndisableable:
    def test_agent_side_reader_strips_essential(self, monkeypatch, tmp_path):
        ...
        assert "loki-agent" not in disabled
...
class TestEssentialOnlySync:
    def test_opted_out_sync_seeds_only_essential(self, monkeypatch, tmp_path):
        """A profile with .no-bundled-skills still gets the loki-agent skill."""
        ...
        for cat, name in [
            ("autonomous-ai-agents", "loki-agent"),
            ("media", "gif-search"),
        ]:
            d = bundled / cat / name
        ...
        monkeypatch.setattr(ss, "_loki_home", lambda: home)
        ...
        assert result["copied"] == ["loki-agent"]
        assert (home / "skills" / "autonomous-ai-agents" / "loki-agent" / "SKILL.md").exists()
