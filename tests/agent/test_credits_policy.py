...
class TestIsFreeTierModel:
    ...
    def test_pricing_cache_peek_zero_priced_model(self, monkeypatch):
        ...
        monkeypatch.setattr(models_pricing, "_pricing_cache",
            {
                "https://inference-api.wundercorp.com": {
                    "some/zero-priced": {"prompt": "0", "completion": "0"},
                    "some/paid": {"prompt": "0.000001", "completion": "0.000002"},
                }
            },
        )
        # The agent holds the /v1-suffixed URL (DEFAULT_WUNDERCORP_INFERENCE_URL) —
        # the helper must normalize it down to the picker's cache key.
        base = "https://inference-api.wundercorp.com/v1"
        ...
        assert is_free_tier_model("some/zero-priced", "https://inference-api.wundercorp.com/") is True
        assert is_free_tier_model("some/zero-priced", "https://inference-api.wundercorp.com/v1/") is True


    def test_wundercorp_welcome_host_is_free_without_pricing(self, monkeypatch):
        ...
        monkeypatch.setattr(models_pricing, "_pricing_cache", {})
        assert is_free_tier_model("wundercorp/welcome", "https://welcome-api.wundercorp.com/v1") is True
        assert is_free_tier_model("some/other", "https://welcome-api.wundercorp.com") is True

    def test_paid_wundercorp_host_still_needs_pricing_evidence(self, monkeypatch):
        """The free-tier rule is the host, not the model name: the paid inference host can serve
        wundercorp/welcome to a named account, and a depleted named account still sees the notice."""
        from agent.credits_tracker import is_free_tier_model
        from loki_cli import models_pricing

        monkeypatch.setattr(models_pricing, "_pricing_cache", {})
        assert is_free_tier_model("wundercorp/welcome", "https://inference-api.wundercorp.com/v1") is False
        assert is_free_tier_model("some/paid", "https://inference-api.wundercorp.com/v1") is False
        assert is_free_tier_model("wundercorp/welcome", "") is False
    ...
    def test_exception_fails_open_to_false(self, monkeypatch):
        ...
        assert is_free_tier_model("some/model", "https://inference-api.wundercorp.com") is False
    ...
    def test_stealth_prefix_detected_as_free(self):
        ...
        assert is_free_tier_model("stealth/ox-alpha", "https://inference-api.wundercorp.com/v1") is True
