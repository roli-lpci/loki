...
def curator_env(tmp_path, monkeypatch):
    """Isolated LOKI_HOME + freshly reloaded curator + skill_usage modules."""
    home = tmp_path / ".loki"
    ...
    # Teardown: a curator review launched with synchrowundercorp=False spawns a
    # daemon "curator-review" thread that calls save_state() when it finishes.
    # save_state() resolves the state path from LOKI_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.loki once monkeypatch
    ...
    # thread here, while LOKI_HOME is still pinned to this test's tmp home
...
def test_run_review_records_state(curator_env):
    ...
    result = c.run_curator_review(synchrowundercorp=True)
...
def test_dry_run_injects_report_only_banner(curator_env, monkeypatch):
    ...
    c.run_curator_review(synchrowundercorp=True, dry_run=True, consolidate=True)
...
def test_run_review_synchrowundercorp_invokes_llm_stub(curator_env, monkeypatch):
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
    _write_skill(skills_dir, "a")
...
def test_review_prompt_tells_reviewer_to_read_before_writing(curator_env, monkeypatch):
    ...
    c.run_curator_review(synchrowundercorp=True, consolidate=True)
...
def test_cli_pin_refuses_bundled_skill(curator_env, capsys):
    from loki_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `loki model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from loki_cli.main_provider_setup import _AUX_TASKS
    from loki_cli.web_server_config import _AUX_TASK_SLOTS
    ...
    # 2. loki_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. loki_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
...
def test_review_fork_restricts_toolsets_to_skills_only(curator_env, monkeypatch):
    ...
    ``loki curator rollback`` restored a hollow skill. Removing the toolset
...
def test_review_fork_toolset_surface_excludes_execution_tools():
    ...
    already-stripped package, and ``loki curator rollback`` restores a
...
def test_review_prompt_does_not_steer_terminal_writes():
    ...
    ``mkdir -p ~/.loki/skills/<umbrella>/references/ && mv ...`` its
