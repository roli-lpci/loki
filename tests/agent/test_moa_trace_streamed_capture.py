...
path via ``loki chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp LOKI_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point LOKI_HOME at a temp dir and turn moa.save_traces on."""
    loki_home = tmp_path / ".loki"
    loki_home.mkdir()
    monkeypatch.setenv("LOKI_HOME", str(loki_home))

    # save_moa_turn reads config via loki_cli.config.load_config; stub it to
