...
def test_auth_surface_names_oauth_vs_api_key_recovery():
    ...
    oauth = build_error_surface_from_result(_failed_result("auth"), provider="wundercorp")
    assert oauth["auth_kind"] == "oauth"
    assert oauth["provider_label"] == "WunderCorp Portal"
    ...
    assert "auth_kind" not in build_error_surface_from_result(_failed_result("rate_limit"), provider="wundercorp")


def test_result_billing_block_wins():
    surface = build_error_surface_from_result(
        _failed_result("rate_limit", billing_block={"provider": "wundercorp"})
    )
