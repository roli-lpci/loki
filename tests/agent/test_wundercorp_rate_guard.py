"""Tests for agent/wundercorp_rate_guard.py — cross-session WunderCorp Portal rate limit guard."""
...
def rate_guard_env(tmp_path, monkeypatch):
    ...
    loki_home = str(tmp_path / ".loki")
    os.makedirs(loki_home, exist_ok=True)
    monkeypatch.setenv("LOKI_HOME", loki_home)
    # Clear any cached module-level imports
    return loki_home


class TestRecordWunderCorpRateLimit:
    """Test recording rate limit state."""

    def test_records_with_header_reset(self, rate_guard_env):
        from agent.wundercorp_rate_guard import record_wundercorp_rate_limit, _state_path

        headers = {"x-ratelimit-reset-requests-1h": "1800"}
        record_wundercorp_rate_limit(headers=headers)
    ...
    def test_custom_default_cooldown(self, rate_guard_env):
        ...
        record_wundercorp_rate_limit(headers=None, default_cooldown=120.0)
...
class TestWunderCorpRateLimitRemaining:
    """Test checking remaining rate limit time."""


    def test_returns_remaining_seconds_when_active(self, rate_guard_env):
        from agent.wundercorp_rate_guard import record_wundercorp_rate_limit, wundercorp_rate_limit_remaining

        record_wundercorp_rate_limit(headers={"x-ratelimit-reset-requests-1h": "600"})
        remaining = wundercorp_rate_limit_remaining()
    ...
    def test_returns_none_when_expired(self, rate_guard_env):
        from agent.wundercorp_rate_guard import wundercorp_rate_limit_remaining, _state_path
        ...
        assert wundercorp_rate_limit_remaining() is None
        # File should be cleaned up
        assert not os.path.exists(_state_path())



class TestClearWunderCorpRateLimit:
    """Test clearing rate limit state."""

    def test_clears_existing_file(self, rate_guard_env):
        from agent.wundercorp_rate_guard import (
            record_wundercorp_rate_limit,
            clear_wundercorp_rate_limit,
            wundercorp_rate_limit_remaining,
            _state_path,
        )

        record_wundercorp_rate_limit(headers={"retry-after": "600"})
        assert wundercorp_rate_limit_remaining() is not None
    ...
    def test_clear_when_no_file(self, rate_guard_env):
        from agent.wundercorp_rate_guard import clear_wundercorp_rate_limit
...
class TestFormatRemaining:
    ...
    def test_seconds(self):
        from agent.wundercorp_rate_guard import format_remaining
...
class TestParseResetSeconds:
    ...
    def test_case_insensitive_headers(self, rate_guard_env):
        from agent.wundercorp_rate_guard import _parse_reset_seconds
...
class TestAuxiliaryClientIntegration:
    ...
    def test_try_wundercorp_skips_when_rate_limited(self, rate_guard_env, monkeypatch):
        from agent.wundercorp_rate_guard import record_wundercorp_rate_limit

        # Record a rate limit
        record_wundercorp_rate_limit(headers={"retry-after": "600"})

        # Mock _read_wundercorp_auth to return valid creds (would normally succeed)
        import agent.auxiliary_client as aux
        monkeypatch.setattr(aux, "_read_wundercorp_auth", lambda: {
            "access_token": "test-token",
            "inference_base_url": "https://api.wundercorp.test/v1",
        })

        result = aux._try_wundercorp()
        assert result == (None, None)

    def test_try_wundercorp_works_when_not_rate_limited(self, rate_guard_env, monkeypatch):
        import agent.auxiliary_client as aux

        # No rate limit recorded — _try_wundercorp should proceed normally
        # (will return None because no real creds, but won't be blocked
        # by the rate guard)
        monkeypatch.setattr(aux, "_read_wundercorp_auth", lambda: None)
        result = aux._try_wundercorp()
        assert result == (None, None)


class TestIsGenuineWunderCorpRateLimit:
    """Tell a real account-level 429 apart from an upstream-capacity 429.

    WunderCorp Portal multiplexes upstreams (DeepSeek, Kimi, MiMo, Loki).
    ...
    def test_exhausted_hourly_bucket_in_429_headers_is_genuine(self):
        from agent.wundercorp_rate_guard import is_genuine_wundercorp_rate_limit
    ...
    def test_a_welcome_host_429_with_exhausted_buckets_trips_the_breaker(
        ...
        assert is_genuine_wundercorp_rate_limit(headers=headers) is True
        record_wundercorp_rate_limit(headers=headers)
        assert wundercorp_rate_limit_remaining() > 0
        verdict, _buffered, statuses = TestWelcomeRouteCopy._drive_guard(
            "https://welcome-api.wundercorp.com/v1", monkeypatch
        )
    ...
    def test_bare_429_with_no_headers_is_upstream(self):
        ...
        assert is_genuine_wundercorp_rate_limit(headers=None) is False
        assert is_genuine_wundercorp_rate_limit(headers={}) is False
        assert is_genuine_wundercorp_rate_limit(
    ...
    def test_last_known_state_all_healthy_stays_upstream(self):
        ...
        last_state = parse_rate_limit_headers(prior_headers, provider="wundercorp")
        assert is_genuine_wundercorp_rate_limit(
...
class TestWelcomeRouteCopy:
    ...
    def _drive_guard(base_url, monkeypatch):
        ...
        from agent import wundercorp_rate_guard
        from agent.turn_api_call import wundercorp_rate_limit_guard

        monkeypatch.setattr(wundercorp_rate_guard, "wundercorp_rate_limit_remaining", lambda: 600)
        buffered = []
        statuses = []
        ...
        verdict = wundercorp_rate_limit_guard(
            agent,
            _retry=None,
            api_messages=[],
            messages=[],
    ...
    def test_the_welcome_host_rate_limit_message_names_the_slash_command(self, monkeypatch):
        ...
        assert "WunderCorp Portal" not in expected
        assert buffered == [f"⏳ {expected} Trying fallback..."]

    def test_a_non_welcome_route_keeps_todays_sentence(self, monkeypatch):
        verdict, buffered, statuses = self._drive_guard(
            "https://inference-api.wundercorp.com/v1", monkeypatch
        )

        expected = "WunderCorp Portal rate limit active — resets in 10m."
        assert verdict.action == "return"
        assert statuses == [f"⏳ {expected}"]
