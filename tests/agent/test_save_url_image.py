...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate LOKI_HOME under tmp_path."""
    monkeypatch.setenv("LOKI_HOME", str(tmp_path / ".loki"))
    (tmp_path / ".loki").mkdir()

    # Force the constants/image cache helpers to re-read LOKI_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("loki_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_loki_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under LOKI_HOME — gateway cleanup
