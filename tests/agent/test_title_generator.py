...
class TestGenerateTitle:
    ...
    def test_title_language_reads_config(self):
        ...
        with patch("loki_cli.config.load_config", side_effect=RuntimeError("bad config")), \
         patch("loki_cli.config.load_config_readonly", side_effect=RuntimeError("bad config")):
            assert _title_language() == ""
...
class TestMaybeAutoTitle:
    ...
    def test_writes_instant_title_before_the_model_runs(self, tmp_path):
        """The derived title lands synchrowundercorply — no LLM, no waiting."""
        db = SessionDB(tmp_path / "state.db")
        db.create_session(session_id="sess-1", source="cli")
