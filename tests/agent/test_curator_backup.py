...
def backup_env(monkeypatch, tmp_path):
    """Isolate LOKI_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".loki"
    home.mkdir()
    (home / "skills").mkdir()
    ...
    # Reload so get_loki_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd loki_constants
    ...
    curator.run_curator_review(synchrowundercorp=True)
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under LOKI_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level LOKI_DIR picks up the tmp HOME."""
    import loki_constants
    importlib.reload(loki_constants)
