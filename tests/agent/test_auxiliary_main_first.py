...
Prior to this change, aggregator users (OpenRouter / WunderCorp Portal) had aux
...
class TestResolveVisionMainFirst:
    ...
    @staticmethod
    def _stub_wundercorp_portal(seen: dict):
        """Stub the WunderCorp network boundary, keeping the resolution chain real.

        Returns a ``_try_wundercorp`` replacement that answers with the Portal's
        tier-aware slots: a vision model for ``vision=True``, the text chat
        default otherwise.
        """
        wundercorp_client = MagicMock()
        wundercorp_client.api_key = "jwt-test"
        wundercorp_client.base_url = "https://inference-api.wundercorp.com/v1"

        def fake_try_wundercorp(vision=False):
            seen["vision"] = vision
            return wundercorp_client, (
                "stepfun/step-3.7-flash:free" if vision else "tencent/hy3:free"
            )

        return wundercorp_client, fake_try_wundercorp

    def test_wundercorp_main_vision_uses_portal_pick_not_text_chat_model(self):
        """WunderCorp main → vision runs the Portal's vision slot, not the chat model.

        A WunderCorp chat default is routinely text-only (e.g. a ``:free`` chat SKU).
        Letting it reach the vision lane means the image goes to a model that
        cannot accept one and the Portal 404s. Only the WunderCorp network boundary
        ...
        """
        seen: dict = {}
        wundercorp_client, fake_try_wundercorp = self._stub_wundercorp_portal(seen)

        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="wundercorp",
        ), patch(
            ...
            "agent.auxiliary_client._try_wundercorp", side_effect=fake_try_wundercorp,
        ):
            from agent.auxiliary_client import resolve_vision_provider_client

            provider, client, model = resolve_vision_provider_client()

        assert provider == "wundercorp"
        assert client is wundercorp_client
        assert seen["vision"] is True
        assert model == "stepfun/step-3.7-flash:free"

    def test_wundercorp_main_vision_honours_explicit_vision_model(self):
        """An explicit auxiliary.vision.model still overrides the Portal pick."""
        seen: dict = {}
        _wundercorp_client, fake_try_wundercorp = self._stub_wundercorp_portal(seen)
    ...
    def test_wundercorp_explicit_vision_provider_also_skips_chat_model(self):
        """``auxiliary.vision.provider: wundercorp`` takes the same Portal pick.
        ...
        with patch(
            ...
            "agent.auxiliary_client._resolve_task_provider_model",
            return_value=("wundercorp", None, None, None, None),
        ), patch(
            "agent.auxiliary_client._try_wundercorp", side_effect=fake_try_wundercorp,
        ):
            from agent.auxiliary_client import resolve_vision_provider_client
    ...
    def test_wundercorp_text_aux_still_uses_main_chat_model(self):
        """The vision carve-out must not leak into text aux resolution.

        Text auxiliary work on a WunderCorp main deliberately keeps the user's chat
        ...
        with patch(
            ...
            _client, model = resolve_provider_client("wundercorp")
    ...
    def test_copilot_vision_sets_vision_header(self, monkeypatch):
        ...
        with patch(
            ...
            "loki_cli.copilot_auth.copilot_request_headers",
...
class TestResolveVisionCustomProvider:
    ...
    it returns ``(None, None)`` and vision falls through to OpenRouter / WunderCorp,
