"""Regression tests for the codex_app_server → Loki UI event bridge.
...
class TestCodexItemToToolName:
    ...
    def test_loki_tools_mcp_server_emits_bare_tool_name(self):
        """The loki-tools MCP server wraps Loki' own tools for codex;
        ...
        mcp.loki-tools.* namespacing (from #26541 by @simpolism)."""
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "loki-tools", "tool": "web_search"}
        ) == "web_search"
        assert _codex_item_to_tool_name(
            {"type": "mcpToolCall", "server": "loki-tools", "tool": "browser_navigate"}
        ) == "browser_navigate"
...
class TestToolProgressDispatch:
    ...
    def test_web_search_builtin_fires_started_and_completed(self):
        ...
        assert calls[0].args[2] == "loki agent docs"
        assert calls[0].args[3] == {"query": "loki agent docs"}
