...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated LOKI_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="loki_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".loki"))
    prev_home = os.environ.get("LOKI_HOME")
    os.environ["LOKI_HOME"] = os.path.join(test_home, ".loki")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("loki_"):
            del sys.modules[mod]
    ...
    try:
        ...
        # references into loki_cli.*/tools.* and their monkeypatches would otherwise
        ...
        if prev_home is None:
            ...
            os.environ["LOKI_HOME"] = prev_home
