...
manipulation. Loki keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point LOKI_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_loki_home /
    get_default_loki_root) instead of monkeypatching private helpers —
    ...
    LOKI_HOME=<root>/profiles/<name> makes get_default_loki_root()
