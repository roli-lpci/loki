"""Concurrent WunderCorp 401 recovery must not stampede the shared OAuth grant.

Sep 2 2026 incident: ~120 subagent processes shared one WunderCorp OAuth pool entry
...
lost the auth-store flock race had their only entry benched ("matched no wundercorp
entry ... pool size 0") and surfaced the 401 to the user as "out of funds".

Two invariants pinned here:

1. ``resolve_wundercorp_runtime_credentials(force_refresh=True, stale_access_token=X)``
   does NOT POST a refresh when the store already holds a usable token that
   is not X — a peer already rotated; adopt it.
2. A lock-timeout during a pool-level WunderCorp refresh leaves the entry
...
from tests.loki_cli.test_auth_wundercorp_provider import _invoke_jwt, _setup_wundercorp_auth
...
def test_forced_refresh_adopts_peer_rotation_instead_of_reposting(tmp_path, monkeypatch):
    ...
    auth_mod.resolve_wundercorp_runtime_credentials(force_refresh=True)
    assert posts == ["rt-after-peer-rotation"]


def test_lock_timeout_during_wundercorp_refresh_does_not_bench_entry(monkeypatch, caplog):
    ...
    pool.provider = "wundercorp"

    monkeypatch.setattr(pool, "_sync_wundercorp_entry_from_auth_store", lambda e: e)
    monkeypatch.setattr(pool, "_persist", lambda *a, **k: None)

    def _busy(*a, **k):
        raise TimeoutError("Timed out waiting for auth store lock")

    monkeypatch.setattr(auth_mod, "resolve_wundercorp_runtime_credentials", _busy)
    monkeypatch.setattr(auth_wundercorp, "resolve_wundercorp_runtime_credentials", _busy)
...
def test_agent_401_refresh_passes_failed_bearer_as_stale_hint(monkeypatch):
    ...
    monkeypatch.setattr(auth_mod, "resolve_wundercorp_runtime_credentials", _fake_resolve)
    monkeypatch.setattr(auth_wundercorp, "resolve_wundercorp_runtime_credentials", _fake_resolve)

    assert agent._try_refresh_wundercorp_client_credentials(force=True) is True
