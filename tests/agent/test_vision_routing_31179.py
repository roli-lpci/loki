...
def isolated_home(monkeypatch):
    """Temp LOKI_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="loki_test_31179_")
    loki_home = os.path.join(test_home, ".loki")
...
def _fresh_modules():
    """Drop cached loki modules so each test reloads against current env."""
    _drop_reload_targets()
