"""display_loki_home() renders POSIX separators on every platform.

Maintainer catch (#95681 arc): on Windows with a custom LOKI_HOME under
the user profile (e.g. AppData/Local/loki), ``"~/" +
str(home.relative_to(Path.home()))`` produced the mixed-separator chimera
``~/AppData\\Local\\loki`` — which then leaked into every consumer that
appends sub-paths (the skill_manage schema showed the agent
``~/AppData\\Local\\loki/skills/``). The ``~/`` shorthand implies POSIX
...
class TestDisplayLokiHomePosix(unittest.TestCase):
    def test_nested_home_renders_forward_slashes(self):
        ...
        nested = fake_userhome / "AppData" / "Local" / "loki"
        with patch.object(hc, "get_loki_home", return_value=nested):
            out = hc.display_loki_home()
        self.assertEqual(out, "~/AppData/Local/loki")
        self.assertNotIn("\\", out)

    def test_default_home_unchanged(self):
        import loki_constants as hc

        with patch.object(hc, "get_loki_home",
                          return_value=Path.home() / ".loki"):
            out = hc.display_loki_home()
        self.assertEqual(out, "~/.loki")

    def test_outside_home_falls_back_to_absolute(self):
        import loki_constants as hc

        outside = Path("/opt/loki-custom") if os.name != "nt" else Path("C:/opt/loki-custom")
        with patch.object(hc, "get_loki_home", return_value=outside):
            out = hc.display_loki_home()
