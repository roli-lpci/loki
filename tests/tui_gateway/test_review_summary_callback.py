    ...
    with patch.dict(
        "sys.modules",
        {
            "loki_constants": MagicMock(
                get_loki_home=MagicMock(return_value="/tmp/loki_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'loki-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'loki-release' patched"
    }
