...
guard is covered in ``tests/loki_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``loki_cli.model_switch_providers.prewarm_picker_cache_async``
...
import loki_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from loki_cli.model_switch,
