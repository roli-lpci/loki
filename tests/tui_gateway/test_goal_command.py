...
def loki_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves LOKI_HOME.
...
@pytest.fixture()
def server(loki_home, monkeypatch):
    ...
    # Pin config resolution to the isolated LOKI_HOME. Sibling test
    ...
    # module-level ``_loki_home = get_loki_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_loki_home", loki_home)
...
class _InlineThread:
    """Run a turn synchrowundercorply so its automatic follow-up is observable."""
...
def test_moa_bare_returns_usage(server, session, loki_home):
    _write_moa_config(loki_home, """
moa:
