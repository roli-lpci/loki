...
def server():
    ...
    # e.g. loki_cli.active_sessions would bind the mocked get_loki_home
    ...
    with patch.dict("sys.modules", {
        "loki_constants": MagicMock(get_loki_home=MagicMock(return_value="/tmp/loki_test")),
        "loki_cli.env_loader": MagicMock(),
        "loki_cli.banner": MagicMock(),
        "loki_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `LOKI_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("LOKI_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("LOKI_TUI_GATEWAY_NO_FLUSH", raising=False)
    ...
    resp = server.handle_request(
        {
            ...
            # eager_build: exercise the synchrowundercorp build path (this test
...
def test_idle_reaper_rearms_missing_ws_orphan_timer(server, monkeypatch, tmp_path, closed_transport):
    ...
    successor = subprocess.run(
        [
            ...
            "-c",
            (
                "from loki_cli.active_sessions import try_acquire_active_session; "
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/loki-agent-dev": {"name": "loki-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "loki-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_scopes_skill_lookup_to_session_profile(server, tmp_path):
    ...
    profile_home rather than the gateway process's ambient LOKI_HOME
    ...
    # The gateway's own LOKI_HOME (the test-isolation tempdir, no
...
def test_dispatch_runs_short_handlers_inline(server):
    """Non-long handlers return their response synchrowundercorply from dispatch()."""
...
def test_voice_and_wake_handlers_are_pool_routed(voice_or_wake_method, server):
    ...
    STT provider auto-detect → a SYNCHROWUNDERCORP faster-whisper lazy install (uv/pip
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `loki config set` does)
    ...
    import loki_cli.skin_engine as skin_engine

    (tmp_path / "skins").mkdir()
    (tmp_path / "skins" / "midnight.yaml").write_text(
        "name: midnight\ndescription: t\ncolors:\n  banner_title: '#00ffcc'\n  background: '#001010'\n"
    )
    monkeypatch.setattr(skin_engine, "get_loki_home", lambda: tmp_path)
    ...
    # Activate midnight, as `loki config set display.skin midnight` would.
