...
runs synchrowundercorply — no mocks, no pre-seeded sessions.
...
def live_home(monkeypatch):
    """A REAL isolated LOKI_HOME with a config.yaml + state.db on disk."""
    tmp = Path(tempfile.mkdtemp(prefix="loki-live-staleprov-"))
    home = tmp / ".loki"
    ...
    # loki_constants caches the resolved home at first read — the env var
    ...
    for var in list(os.environ):
        if var.endswith("_API_KEY") or var in ("OPENROUTER_KEY", "WUNDERCORP_KEY"):
            monkeypatch.delenv(var, raising=False)

    import loki_cli.config as hconfig
    ...
    # conftest also re-pins loki_state.DEFAULT_DB_PATH per test; pin it to
    # THIS home so server._get_db() opens the same real state.db we seed.
    monkeypatch.setattr(loki_state, "DEFAULT_DB_PATH", home / "state.db")
    monkeypatch.setattr(server, "_db", None, raising=False)
    monkeypatch.setattr(server, "_db_error", None, raising=False)
    monkeypatch.setattr(server, "_loki_home", str(home), raising=False)
    yield home, server
