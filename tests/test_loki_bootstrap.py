"""Tests for loki_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Loki entry point
(loki, loki, loki-agent, loki-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Loki entry point imports loki_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported loki_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("loki_bootstrap", None)
    import loki_bootstrap  # noqa: WPS433
    return loki_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Loki CLI
...
class TestEntryPointsImportBootstrap:
    """Every Loki entry point must import loki_bootstrap as its
    ...
    # Entry points that invoke Loki as a process.  Each one must
    # import loki_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "loki_cli/main.py",   # loki CLI (console_script)
        "run_agent.py",          # loki-agent (console_script)
        "acp_adapter/entry.py",  # loki-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import loki_bootstrap' and that
        ...
        half-finished ``loki update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``loki_bootstrap`` as a top-level module) leaves loki
        ...
        """
        # Resolve relative to the loki-agent repo root.  Tests live
        # at tests/test_loki_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # loki start even when loki_bootstrap hasn't been
        ...
        assert first_import_name == "loki_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'loki_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import loki_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Loki's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("LOKI_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["LOKI_PYTHON_SRC_ROOT"] = env
            elif "LOKI_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["LOKI_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/loki")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("LOKI_PYTHON_SRC_ROOT", None)
            else:
                os.environ["LOKI_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/loki", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Loki root.  The guard must relocate Loki to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/loki"])
        assert result[0] == "/opt/loki"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Loki root.
        assert result.index("/opt/loki") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["LOKI_PYTHON_SRC_ROOT"] = "/env/loki"
            hb.harden_import_path()
            assert sys.path[0] == "/env/loki"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("LOKI_PYTHON_SRC_ROOT", None)
