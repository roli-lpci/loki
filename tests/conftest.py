"""Shared fixtures for the loki-agent test suite.
...
2. **Isolated LOKI_HOME.** LOKI_HOME points to a per-test tempdir so
   code reading ``~/.loki/*`` via ``get_loki_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".loki"`` instead of the canonical
   ``get_loki_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No LOKI_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox LOKI_HOME before ANY test module is imported ──────────────────
# `loki_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_loki_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.loki/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes LOKI_HOME, but fixtures run
...
# the REAL Loki root — capture it BEFORE the sandbox rewires LOKI_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.loki (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("LOKI_KANBAN_HOME", "").strip()
_PRE_SANDBOX_LOKI_HOME = os.environ.get("LOKI_HOME", "")


def _loki_home_points_at_production(value: str) -> bool:
    """True when a pre-set LOKI_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    LOKI_HOME=~/.loki``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``loki_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.loki`` — the escape vector that landed pytest
    ...
    LOKI_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".loki").resolve()
    except Exception:
        return True
...
if _loki_home_points_at_production(os.environ.get("LOKI_HOME", "")):
    _SESSION_LOKI_HOME = tempfile.mkdtemp(prefix="loki-test-home-")
    os.environ["LOKI_HOME"] = _SESSION_LOKI_HOME
    atexit.register(shutil.rmtree, _SESSION_LOKI_HOME, True)
...
# like a real CLI") — which used to disarm loki_state's live-DB guard in
# the child at the same moment the child lost the LOKI_HOME redirect.
# LOKI_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# loki_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export LOKI_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["LOKI_TEST_ISOLATION"] = os.environ.get("LOKI_HOME", "") or "1"

#: LOKI_HOME as it stood when conftest was imported - i.e. before any test
...
LOKI_HOME_AT_CONFTEST_IMPORT = os.environ.get("LOKI_HOME", "")
...
# LOKI_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_LOKI_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``loki_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Loki agent session inherits them and
    # loki_constants home-resolution helpers prefer them over monkeypatched
    ...
    "LOKI_MAX_ITERATIONS",
    "LOKI_SESSION_PLATFORM",
    "LOKI_SESSION_CHAT_ID",
    "LOKI_SESSION_CHAT_NAME",
    "LOKI_SESSION_CHAT_TYPE",
    "LOKI_SESSION_THREAD_ID",
    "LOKI_SESSION_SOURCE",
    "LOKI_SESSION_KEY",
    "LOKI_GATEWAY_SESSION",
    "LOKI_CRON_SESSION",
    "_LOKI_GATEWAY",
    "LOKI_PLATFORM",
    "LOKI_MODEL",
    "LOKI_INFERENCE_MODEL",
    "LOKI_INFERENCE_PROVIDER",
    "LOKI_TUI_PROVIDER",
    "LOKI_MANAGED",
    "LOKI_MANAGED_DIR",
    "LOKI_DEV",
    "LOKI_CONTAINER",
    "LOKI_EPHEMERAL_SYSTEM_PROMPT",
    "LOKI_TIMEZONE",
    "LOKI_REDACT_SECRETS",
    "LOKI_BACKGROUND_NOTIFICATIONS",
    "LOKI_EXEC_ASK",
    "LOKI_HOME_MODE",
    "LOKI_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.loki/kanban.db instead of the per-test LOKI_HOME.
    "LOKI_KANBAN_DB",
    "LOKI_KANBAN_BOARD",
    "LOKI_KANBAN_HOME",
    "LOKI_KANBAN_WORKSPACES_ROOT",
    "LOKI_KANBAN_LOGS_ROOT",
    "LOKI_KANBAN_TASK",
    "LOKI_KANBAN_WORKSPACE",
    "LOKI_KANBAN_RUN_ID",
    "LOKI_KANBAN_CLAIM_LOCK",
    "LOKI_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "loki" host.
    "LOKI_HONCHO_HOST",
    # Dashboard OAuth auth gate (PR #30156). When set, the bundled
    # dashboard-auth `wundercorp` plugin auto-registers itself on plugin discovery,
    ...
    "LOKI_DASHBOARD_OAUTH_CLIENT_ID",
    "LOKI_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and LOKI_HOME to per-test tempdirs so code that
    reads ``~/.loki/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral LOKI_* vars that could change test semantics.
    for name in _LOKI_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("LOKI_HONCHO_HOST", "loki")

    # 3. Redirect LOKI_HOME to a per-test tempdir. Code that reads
    #    ``~/.loki/*`` via ``get_loki_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.loki/*`` via
    #    ``Path.home() / ".loki"`` instead of ``get_loki_home()``
    #    is a bug to fix at the callsite.
    fake_loki_home = tmp_path / "loki_test"
    fake_loki_home.mkdir()
    (fake_loki_home / "sessions").mkdir()
    (fake_loki_home / "cron").mkdir()
    (fake_loki_home / "memories").mkdir()
    (fake_loki_home / "skills").mkdir()
    monkeypatch.setenv("LOKI_HOME", str(fake_loki_home))
    ...
    # loki_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("LOKI_TEST_ISOLATION", str(fake_loki_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("LOKI_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. loki_state computes ``DEFAULT_DB_PATH = get_loki_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from loki_state import ...``) that
    ...
    loki_state_mod = sys.modules.get("loki_state")
    if loki_state_mod is not None and hasattr(loki_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            loki_state_mod, "DEFAULT_DB_PATH", fake_loki_home / "state.db"
        )
    ...
    monkeypatch.setenv("LOKI_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.loki/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import loki_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path LOKI_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_loki_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from loki_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.loki. This autouse
...
# allow-list because test-level fixtures legitimately move LOKI_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session LOKI_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``LOKI_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Loki root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_LOKI_HOME and not _loki_home_points_at_production(
        _PRE_SANDBOX_LOKI_HOME
    ):
        # LOKI_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_loki_root().resolve()
    # No pre-existing LOKI_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".loki").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.loki`` captured at import time. Hermetic tests that legitimately
    move LOKI_HOME to sibling tempdirs are unaffected.

    Only patches when ``loki_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("loki_cli.kanban_db")
    _kdbc = sys.modules.get("loki_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import loki_cli.kanban_db`
...
# ``loki_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Loki root. This fixture wires the
# test-side knobs:
...
#     installs where LOKI_HOME is not ~/.loki) into the guard's
...
# so subprocess children that import loki_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("loki_state")
    ...
    if _PRE_SANDBOX_LOKI_HOME and not _loki_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_LOKI_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Loki home override redirects every later
    # ``get_loki_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from loki_constants import get_loki_home_override, set_loki_home_override

        if get_loki_home_override() is not None:
            set_loki_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal loki config dict suitable for unit tests."""
    return {
...
def _ensure_current_event_loop(request):
    ...
    Python 3.11+ no longer guarantees a current loop for plain synchrowundercorp tests.
...
# environment and finds the developer's live ``loki-gateway`` process
...
# correlated with a ``tests/loki_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> loki-gateway``
...
def _wal_is_usable() -> bool:
    """True when Loki will actually put a database into WAL mode here.

    Loki refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Loki can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Loki managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``loki_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_loki_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_loki_home`` fixture
    redirects ``LOKI_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.loki/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``loki_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["LOKI_VOICE_TTS"] = "1"``. The
...
#      ``loki_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``loki_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``loki_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``loki_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Loki runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.loki``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Loki falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Loki uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart loki-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting loki/python patterns are also blocked.
    """
    ...
    def _matches_loki_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _LOKI_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_loki_gateway(cmd_str):
            return False
        try:
            tokens = _shlex.split(cmd_str)
        except ValueError:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch loki-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m loki_cli.main".
                if (
                    "loki" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live loki-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting loki/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `loki update` (or the
        # equivalent `python -m loki_cli.main update`).  These commands
        ...
        if "update" in low and (
            # loki update / loki update --gateway / setsid bash -c ... loki update
            ("loki" in low and "update" in low.split())
            or
            # python -m loki_cli.main update --gateway
            ("loki_cli" in low and "update" in low.split())
            or
            # venv/bin/loki update  (absolute path variant used in tests)
            (".venv/bin/loki" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`loki update` against the real checkout, fetching "
        ...
        # Block spawning a REAL gateway runtime (``python -m loki_cli.main
        # gateway run|start|restart``). ``_spawn_loki_action`` launches it
        # with start_new_session=True, so it outlives the pytest worker; the
        # child inherits the pytest-tmp LOKI_HOME, resolves the DEVELOPER's
        # ``loki-gateway`` systemd unit (a tmp home hashes to no profile
        ...
        # A gateway launched INSIDE a container (`docker exec … loki gateway start`) cannot
        ...
        if (
            ...
            raise RuntimeError(
                ...
                "loki gateway runtime that outlives the test (it is "
                "detached), restarts the developer's live gateway, and "
                "holds the webhook port. Patch the spawn seam where "
                "production reads it (loki_cli.web_server_gateway."
                "_spawn_loki_action), or mark with "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_LOKI_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import loki_cli.voice as _voice
    except Exception:
