...
The ``loki update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m loki_cli.main gateway run`` — image name
    ``pythonw``, not ``loki.exe`` — so the ``taskkill /IM loki.exe`` guard
    ...
    # The loki.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM loki.exe' in text
