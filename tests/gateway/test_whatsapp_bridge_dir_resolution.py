...
(/opt/loki/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable LOKI_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_loki_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable LOKI_HOME."""
    ...
    monkeypatch.setattr(
        "loki_constants.get_loki_home", lambda: loki_home
    )
    ...
    expected = loki_home / "scripts" / "whatsapp-bridge"
