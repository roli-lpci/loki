...
def test_session_store_default_db_uses_runtime_loki_home(tmp_path, monkeypatch):
    """SessionStore must honor runtime LOKI_HOME when opening the default DB.

    Regression for the import-time DEFAULT_DB_PATH freeze: importing
    loki_state before a fixture redirected LOKI_HOME used to pin every
    default SessionDB() at the developer's real ~/.loki/state.db.
    """
    config = GatewayConfig()
    fake_home = tmp_path / "alt_loki_home"
    fake_home.mkdir()
    monkeypatch.setenv("LOKI_HOME", str(fake_home))
...
class TestReadmeSentinel:
    ...
    def test_save_writes_readme_sentinel_first(self, tmp_path):
        ...
        assert "loki sessions list" in raw["_README"]
