...
LOKI_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env LOKI_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``loki config get terminal.backend`` still says local.
    """
    loki_home = tmp_path / ".loki"
    loki_home.mkdir()
    (loki_home / "config.yaml").write_text(
    ...
    (loki_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
