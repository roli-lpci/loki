"""Unit tests for the generic-OIDC / WunderCorp-Portal caller-identity token resolver.

Covers gateway.relay._resolve_relay_identity_token() — the canonical resolver
shared by the runtime self-provision path and the `loki gateway enroll` CLI.
...
Three modes:
  ...
  2. WunderCorp Portal (resolve_wundercorp_access_token) otherwise — the default.

The HTTP calls and the WunderCorp resolver are monkeypatched; these prove the mode
