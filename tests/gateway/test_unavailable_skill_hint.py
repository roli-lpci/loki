...
* ``disabled`` membership is checked by the declared name, because that
  is what :func:`loki_cli.skills_config.save_disabled_skills` stores.
"""
...
def tmp_skills(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated skills dir + LOKI_HOME so the real user config is untouched."""
    home = tmp_path / ".loki"
    home.mkdir()
    (home / "skills").mkdir()
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `loki skills config` writes).
    monkeypatch.setattr(
    ...
    assert "loki skills config" in msg
