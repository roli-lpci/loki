...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/wundercorp/loki/issues/23994:
    reply_to_mode 'off' was ignored when sending via Loki-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    Regression tests for https://github.com/wundercorp/loki/issues/87051:
    ...
    response in a different chat lane than the Loki topic the session runs
    in. The no-anchor branch must prefer the Loki topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_loki_topic_thread_id(self):
