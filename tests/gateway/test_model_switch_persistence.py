...
class TestIsIntentionalModelSwitch:
    ...
    default for a reason the system produced: a /model override, or the WunderCorp gateway moving the
    session off the ``wundercorp/welcome`` alias the config still carries."""
    ...
    def test_server_model_switch_off_the_welcome_alias_is_intentional(self):
        ...
        agent = SimpleNamespace(model="z-ai/glm-5.3-flash", _wundercorp_model_switch=("wundercorp/welcome", "z-ai/glm-5.3-flash"))
        assert runner._is_intentional_model_switch(sk, agent, "wundercorp/welcome") is True
        ...
        assert runner._is_intentional_model_switch(sk, agent, "wundercorp/welcome") is False
...
class TestOneTurnNeverPersisted:
    ...
    def _runner_with_store(tmp_path, monkeypatch):
        ...
        from gateway.run import GatewayRunner
        from loki_cli.model_switch import ModelSwitchResult
        ...
        monkeypatch.setattr("loki_constants.get_loki_home", lambda: loki_home)
        monkeypatch.setattr("loki_cli.config.get_loki_home", lambda: loki_home)
