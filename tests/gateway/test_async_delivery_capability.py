...
class TestAsyncDeliverySupported:
    ...
    def test_set_false_is_unsupported(self):
        ...
        try:
            ...
            assert get_session_env("LOKI_SESSION_PLATFORM") == "api_server"
        finally:
...
class TestDeclareStatelessChannel:
    """``loki -z`` and cron cannot receive a completion after their turn ends.

    Cron clears the ``LOKI_SESSION_*`` routing keys, so an async delegation's
