...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.loki/loki-agent/utils.py)
...
2. ``loki update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``loki_cli/main.py`` (the ``loki update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``loki_constants`` after the
