...
(non-managed) routing. When a user picked FAL through the WunderCorp-subscription
...
the user's personal FAL_KEY instead of the WunderCorp Tool Gateway (real incident:
...
exactly ONE provider string per category — ``image_gen.provider: wundercorp`` for
the managed "WunderCorp Subscription" row, ``image_gen.provider: fal`` for the
BYOK FAL row — and any legacy ``use_gateway`` key is popped so the
read-time shim (use_gateway: true ⇒ wundercorp) cannot override the fresh pick.
...
def _quiet(monkeypatch):
    import loki_cli.tools_config_providers as tc
...
def test_image_gen_selector_preserves_managed_selection(monkeypatch):
    """Managed pick: the 'wundercorp' provider string must survive the selector."""
    _quiet(monkeypatch)
    config = {}
...
def test_image_and_video_selectors_share_the_selection_contract(monkeypatch):
    ...
    for use_gateway, expected in ((True, "wundercorp"), (False, "fal")):
        config = {
...
def _quiet_reconfigure(monkeypatch):
    ...
    monkeypatch.setattr(ns, "ensure_wundercorp_portal_access", lambda **k: True)
...
def test_reconfigure_managed_fal_row_keeps_managed_selection(monkeypatch):
    ...
    branch wrote the managed one — a WunderCorp Subscription user re-entering the
    ...
    managed_row = {
        ...
        "managed_wundercorp_feature": "image_gen",
...
def test_reconfigure_direct_fal_row_writes_vendor_selection(monkeypatch):
    ...
    legacy use_gateway key so the read-time shim can't resurrect 'wundercorp'."""
    _quiet_reconfigure(monkeypatch)
    import loki_cli.tools_config as tc
