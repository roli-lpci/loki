...
loki_cli.config.load_config, so the managed overlay has to be wired into each.
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # loki_time checks an env override first; ensure it's unset so config wins.
    ...
    assert loki_time._resolve_timezone_name() == "Asia/Tokyo"
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (LOKI_TIMEZONE, LOKI_REDACT_SECRETS, LOKI_MAX_ITERATIONS,
    ...
    # writing LOKI_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml
