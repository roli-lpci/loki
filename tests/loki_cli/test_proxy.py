"""Tests for the `loki proxy` subcommand and its upstream adapters."""
...
from loki_cli.proxy.adapters import ADAPTERS, get_adapter
from loki_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from loki_cli.proxy.adapters.wundercorp_portal import WunderCorpPortalAdapter
from loki_cli.proxy.adapters.xai import XAIGrokAdapter
...
# ---------------------------------------------------------------------------
# WunderCorpPortalAdapter
# ---------------------------------------------------------------------------


def _write_auth_store(loki_home: Path, wundercorp_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given wundercorp state into a hermetic LOKI_HOME."""
    auth_path = loki_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {"wundercorp": wundercorp_state},
    }))
    return auth_path
...
def test_wundercorp_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
    ...
    adapter = WunderCorpPortalAdapter()
    results: list = []
    errors: list = []
    ...
    with patch(
        "loki_cli.proxy.adapters.wundercorp_portal.resolve_wundercorp_runtime_credentials",
...
def _write_xai_pool_entry(
    ...
    *,
    ...
    """Write an xai-oauth pool entry into a hermetic LOKI_HOME."""
    auth_path = loki_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_xai_adapter_retry_rotates_pool_entry_on_429(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("loki_cli.auth.refresh_xai_oauth_pure", _refresh_must_not_run)
...
from loki_cli.proxy.server import create_app  # noqa: E402
