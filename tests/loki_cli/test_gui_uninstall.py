"""Tests for loki_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import loki_cli.gui_uninstall as gu


def _make_agent(loki_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = loki_home / "loki-agent"
    (agent_root / "loki_cli").mkdir(parents=True)
    (agent_root / "loki_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(loki_home: Path) -> None:
    """Create the source-built GUI artifacts a `loki desktop` run produces."""
    desktop = loki_home / "loki-agent" / "apps" / "desktop"
    ...
    (loki_home / "loki-agent" / "node_modules").mkdir(parents=True)
    (loki_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(loki_home: Path) -> None:
    (loki_home / "config.yaml").write_text("x: 1\n")
    (loki_home / ".env").write_text("KEY=secret\n")
    (loki_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(loki_home)
    _make_gui_build(loki_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(loki_home)
    ...
    assert summary["loki_home"] == str(loki_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `loki desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    loki_home = tmp_path / ".loki"
    _make_agent(loki_home)
    icon = lde.icon_path(loki_home / "loki-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(loki_home)
    ...
    assert lde.icon_path(loki_home / "loki-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (loki_home / "loki-agent" / "loki_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".loki")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import loki_cli.uninstall as uninstall
