...
- Bare `loki setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `loki setup --quick` runs the narrower "fill in missing items" flow.
- `loki setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("loki_cli.setup.ensure_loki_home", {}),
        ("loki_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("loki_cli.config.is_managed", {"return_value": False}),
        ("loki_cli.setup.load_config", {"return_value": {}}),
        ("loki_cli.setup.save_config", {}),
        ("loki_cli.setup.get_env_value", {"return_value": None}),
        ("loki_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("loki_cli.setup._print_setup_summary", {}),
        ("loki_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("loki_cli.setup.load_config", {"return_value": {}}),
        ("loki_cli.setup.save_config", {}),
        ("loki_cli.auth.get_active_provider", {"return_value": None}),
        ("loki_cli.setup.get_env_value", {"return_value": None}),
        ("loki_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `loki setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                ...
                tools="loki_cli.setup.setup_tools",
            )
            from loki_cli.setup import run_setup_wizard
    ...
    def test_reconfigure_on_fresh_install_falls_through(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("loki_cli.setup.prompt_choice", {"return_value": 0}),
                first="loki_cli.setup_quick._run_first_time_quick_setup",
            )
            from loki_cli.setup import run_setup_wizard
            from loki_cli import setup as setup_mod
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "loki_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["loki", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
