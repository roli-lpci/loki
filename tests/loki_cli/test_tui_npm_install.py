...
@pytest.fixture
def main_mod():
    import loki_cli.main as m

    return m


def _touch_ink(root: Path) -> None:
    ink = root / "node_modules" / "@loki" / "ink" / "package.json"
...
def test_make_tui_argv_uses_bundled_tui_when_workspace_missing(
    ...
    loki_cli/tui_dist/entry.js but never ships tui-ui/ (that directory only
    ...
    ever touching the (missing) tui-ui workspace or git.
    """
    _touch_ink(tmp_path)
...
#
# In a shared workspace checkout the launch install is scoped to the tui-ui
...
def _write_ws(root: Path, ws_lock: str, hidden_lock: str) -> Path:
    """Lay out a workspace root + tui-ui member and return the tui-ui dir.

    ``@wundercorp/loki-ink`` and the marker live at the workspace root (hoisted);
    ``tui-ui/`` has no lockfile of its own so ``_workspace_root`` treats the
    parent as the workspace root and the launch scopes to ``--workspace tui-ui``.
    """
    ...
    # parent as the workspace root and the launch scope to --workspace tui-ui.
    (tui_dir / "package.json").write_text('{"name":"loki-tui"}')
    return tui_dir


def test_no_install_when_only_other_workspace_deps_missing(tmp_path: Path, main_mod) -> None:
    """Deps that belong to apps/desktop / web (never installed by the tui-ui
    scoped install) must not trigger a reinstall on every launch (#66978)."""
    tui_dir = _write_ws(
        tmp_path,
        '{"packages":{'
        '"tui-ui":{"dependencies":{"foo":"1.0.0"}},'
...
def test_need_install_when_ui_tui_dep_missing_in_workspace_layout(tmp_path: Path, main_mod) -> None:
    """A genuinely missing tui-ui dependency is still caught after scoping."""
    tui_dir = _write_ws(
        tmp_path,
        '{"packages":{'
        '"tui-ui":{"dependencies":{"foo":"1.0.0","bar":"1.0.0"}},'
        '"node_modules/foo":{"version":"1.0.0"},'
...
def test_need_install_when_linked_workspace_dep_missing(tmp_path: Path, main_mod) -> None:
    """The closure follows workspace symlinks (@wundercorp/loki-ink → tui-ui/packages/…)
    so a linked workspace's own missing dep triggers a reinstall."""
    tui_dir = _write_ws(
        tmp_path,
        '{"packages":{'
        '"tui-ui":{"dependencies":{"@wundercorp/loki-ink":"*"}},'
        '"node_modules/@wundercorp/loki-ink":{"link":true,"resolved":"tui-ui/packages/loki-ink"},'
        '"tui-ui/packages/loki-ink":{"dependencies":{"inkdep":"1.0.0"}},'
...
def test_workspace_closure_includes_dev_deps_of_scoped_workspace(main_mod) -> None:
    """tui-ui's devDependencies (esbuild/typescript build toolchain) are part of
    ...
    closure = main_tui_launch._npm_lock_workspace_closure(packages, "tui-ui")
...
def test_workspace_closure_returns_none_when_start_absent(main_mod) -> None:
    ...
    assert main_tui_launch._npm_lock_workspace_closure({"node_modules/foo": {}}, "tui-ui") is None


def test_workspace_closure_includes_dev_deps_of_selected_child_workspace(main_mod) -> None:
    """On Termux the install also scopes to tui-ui's child packages/* workspaces,
    ...
    packages = {
        "tui-ui": {"dependencies": {"@wundercorp/loki-ink": "*"}},
        "node_modules/@wundercorp/loki-ink": {
            "link": True,
            "resolved": "tui-ui/packages/loki-ink",
        },
        "tui-ui/packages/loki-ink": {"devDependencies": {"child-dev-only": "1"}},
        "node_modules/child-dev-only": {},
    }
    # Only tui-ui selected (desktop): the child's dev dep is not installed.
    desktop = main_tui_launch._npm_lock_workspace_closure(packages, {"tui-ui"})
    assert "node_modules/child-dev-only" not in desktop
    # tui-ui + child selected (Termux): the child's dev dep is in the closure.
    termux = main_tui_launch._npm_lock_workspace_closure(
        packages, {"tui-ui", "tui-ui/packages/loki-ink"}
    )
...
def test_termux_install_catches_missing_child_workspace_dev_dep(
    ...
    """On Termux the launch install selects tui-ui/packages/* too, installing
    ...
    ws_lock = (
        ...
        '"tui-ui":{"dependencies":{"@wundercorp/loki-ink":"*"}},'
        '"node_modules/@wundercorp/loki-ink":{"link":true,"resolved":"tui-ui/packages/loki-ink"},'
        '"tui-ui/packages/loki-ink":{"devDependencies":{"child-dev-only":"1.0.0"}},'
    ...
    child = tui_dir / "packages" / "loki-ink"
    child.mkdir(parents=True, exist_ok=True)
    (child / "package.json").write_text('{"name":"@wundercorp/loki-ink"}')
...
def test_make_tui_argv_npm_install_forces_include_dev(
    ...
    """The TUI-launch npm install must force --include=dev: tui-ui's build
...
def test_make_tui_argv_decodes_dev_prebuild_with_utf8_replace(
    ...
    ink_dir = tmp_path / "packages" / "loki-ink"
    ink_dir.mkdir(parents=True)
...
def test_make_tui_argv_exits_with_recovery_hint_when_workspace_unrecoverable(
    ...
    """Missing tui-ui + no git checkout → clean error, never touches node/npm."""
    monkeypatch.delenv("LOKI_TUI_DIR", raising=False)
    ...
    # tui-ui/ deliberately does not exist under tmp_path, and there is no
...
def test_no_stray_lockfiles_in_workspace_subdirs(main_mod) -> None:
    ...
    # NOT have their own lockfile.  (tui-ui/packages/* members are
    ...
    # Also sweep tui-ui/packages/* (loki-ink etc.)
    tui_pkgs = root / "tui-ui" / "packages"
    if tui_pkgs.is_dir():
...
def test_make_tui_argv_omits_workspace_and_scrubs_esbuild_override(
    ...
    """When tui-ui/ has its own package-lock.json, _workspace_root returns
    tui_dir itself.  npm install --workspace tui-ui would fail in that case
    because npm cannot find a workspace named "tui-ui" inside tui-ui/.
...
class TestPersistentNpmUserconfig:
    """$LOKI_HOME/npmrc must reach every npm lifecycle child process (#106373)."""

    def test_loki_home_npmrc_sets_userconfig(self, tmp_path, monkeypatch):
        home = tmp_path / "home"
        home.mkdir()
