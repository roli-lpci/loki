...
``loki*.exe`` shims aside (``loki.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``loki`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("loki", "loki-agent", "loki-acp", "loki-gateway"):
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "loki.exe" in names, "loki.exe must be restored after a no-op install"
    assert "loki-acp.exe" in names
    assert "loki-gateway.exe" in names
    assert (scripts / "loki.exe").read_bytes() == b"MZ-old-loki"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "loki.exe").read_bytes() == b"MZ-new-loki"
...
# ---------------------------------------------------------------------------
# loki_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "loki.exe" in names, "loki.exe must be restored after a no-op recovery install"
    assert (scripts / "loki.exe").read_bytes() == b"MZ-old-loki"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "loki.exe").write_bytes(b"MZ-new-loki")
