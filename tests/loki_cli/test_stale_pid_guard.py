...
- ``loki_cli/_subprocess_compat.pid_is_loki`` / ``kill_process_tree``
- ``loki_cli/dashboard_procs._kill_stale_dashboard_processes`` (win32)
- ``loki_cli/update_cmd._stop_process_trees``
...
from loki_cli import _subprocess_compat
...
class TestPidIsLoki:
    ...
    def test_non_windows_is_unconditional_pass(self):
        ...
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", False):
            assert _subprocess_compat.pid_is_loki(1234) is True
    ...
    def test_non_windows_still_rejects_recycled_identity(self):
        ...
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", False), mock.patch.object(
            ...
            assert _subprocess_compat.pid_is_loki(
                1234, expected_start_time=123
            ) is False

    def test_loki_match_requires_token_boundary(self):
        # "loki" buried inside an unrelated path segment must not match.
        assert _subprocess_compat._text_names_loki(
            r"c:\users\slokia\app.exe"
        ) is False
        assert _subprocess_compat._text_names_loki(
            r"C:\Users\x\.loki-runtime\python.exe -m loki_cli.main"
        ) is True
        assert _subprocess_compat._text_names_loki(
    ...
    def test_invalid_pid_inputs_do_not_crash(self):
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", True):
            assert _subprocess_compat.pid_is_loki(-1) is False
            assert _subprocess_compat.pid_is_loki(0) is False
            assert _subprocess_compat.pid_is_loki("not-a-pid") is False
            assert _subprocess_compat.pid_is_loki(True) is False

    def test_probe_matches_loki_like_process(self):
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", True), mock.patch.object(
            ...
            _subprocess_compat, "_process_command_is_loki", return_value=True
        ):
            assert _subprocess_compat.pid_is_loki(1234) is True
    ...
    def test_probe_rejects_foreign_process(self):
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", True), mock.patch.object(
            ...
            _subprocess_compat, "_process_command_is_loki", return_value=False
        ):
            assert _subprocess_compat.pid_is_loki(1234) is False
    ...
    def test_missing_pid_real_probe_fails_closed(self):
        # A PID that cannot exist must never be judged Loki-owned.
        assert _subprocess_compat.pid_is_loki(2**24) is False
...
class TestKillProcessTree:
    ...
    def test_retained_handle_is_taskkilled_without_probe(self):
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", True), mock.patch.object(
            _subprocess_compat, "pid_is_loki"
...
class TestStopProcessTrees:
    ...
    def test_foreign_pids_only_probed(self):
        with mock.patch(
            ...
            "loki_cli._subprocess_compat.pid_is_loki", return_value=False
        ), mock.patch.object(update_cmd.subprocess, "run") as run:
            update_cmd._stop_process_trees([1111, 2222])
        run.assert_not_called()

    def test_loki_pid_probed_then_taskkilled(self):
        with mock.patch(
            "gateway.status.get_process_start_time", return_value=123
        ), mock.patch(
            "loki_cli._subprocess_compat.pid_is_loki", return_value=True
        ), mock.patch.object(
...
class TestKillStaleDashboardProcesses:
    ...
    def test_foreign_pid_reported_not_killed(self):
        ...
        assert result["failed"] == [
            (12345, "not loki-owned or process identity changed")
        ]
        run.assert_not_called()

    def test_loki_pid_killed(self):
        with self._patch_find(), mock.patch.object(
            dashboard_procs.sys, "platform", "win32"
        ), mock.patch(
            "gateway.status.get_process_start_time", return_value=123
