"""Regression tests for #34107 — Docker UID/GID handling in ensure_loki_home.

When Loki runs in Docker with ``LOKI_UID=1000`` / ``LOKI_GID=911``,
the entrypoint chowns the top-level ``LOKI_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_loki_home()`` — especially
...
The fix is a ``_chown_to_loki_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_loki_uid_gid
# ---------------------------------------------------------------------------


class TestResolveLokiUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("LOKI_UID", "1000")
        monkeypatch.setenv("LOKI_GID", "911")
        from loki_cli.config import _resolve_loki_uid_gid
...
class TestChownToLokiUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_loki_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("LOKI_UID", raising=False)
        monkeypatch.delenv("LOKI_GID", raising=False)
        from loki_cli import config as cfg
