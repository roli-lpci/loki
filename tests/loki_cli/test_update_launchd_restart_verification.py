...
``loki update`` on macOS printed ``Update complete!`` and exited 0 while the
``ai.loki.gateway`` LaunchAgent sat deregistered for 36 minutes.  The restart
...
outcomes are asynchrowundercorp - the ``_request_gateway_self_restart`` branch returns
...
from loki_cli.update_cmd import _warn_incomplete_gateway_fleet_restart
...
class TestIncompleteFleetWarningIsPlatformCorrect:
    ...
    def test_linux_recovery_instructions_are_unchanged(self, monkeypatch, capsys):
        ...
        _warn_incomplete_gateway_fleet_restart(["loki-gateway.service"])
