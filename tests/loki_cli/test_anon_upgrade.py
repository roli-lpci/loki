"""``loki auth upgrade``: the free tier signs into a WunderCorp account, keeping its connectors.
...
from loki_cli.auth import _auth_file_path, _load_auth_store

WELCOME = "https://welcome-api.wundercorp.com/v1"
INFERENCE = "https://inference-api.wundercorp.com/v1"
...
class FakePortal:
    ...
    def handler(self, request: httpx.Request) -> httpx.Response:
        ...
        if path == "/api/oauth/token":
            ...
            self.token_grants += 1
            return httpx.Response(200, json={
                "access_token": _jwt(sub="nas_user:9", client_id="loki-cli", account_tier="free"),
...
class TestUpgrade:
    ...
    def test_completed_promotion_signs_in_and_keeps_no_free_tier_fields(self, portal, capsys, tmp_path):
        ...
        state = store["providers"]["wundercorp"]
...
def free_account(monkeypatch):
    ...
    from loki_cli import models as m
    from loki_cli import models_pricing as mp
    monkeypatch.setattr(m, "check_wundercorp_free_tier", lambda **kw: True)
    monkeypatch.setattr(m, "fetch_wundercorp_recommended_models", lambda *a, **kw: {
...
class TestSignInCompletionSettlesTheModel:
    def test_config_on_the_free_tier_route_moves_to_the_account_host_and_the_recommended_free_model(
        ...
        _write_model_config({"provider": "wundercorp", "default": anon_auth.GUEST_MODEL, "base_url": WELCOME})
    ...
    def test_no_eligible_recommendation_leaves_no_default_rather_than_a_model_the_account_may_not_use(
        ...
        monkeypatch.setattr(m, "recommended_wundercorp_default_model", _portal_down)
        assert anon_auth.upgrade_guest(_args()) == 0
        model_cfg = _model_config()
        ...
        assert "run `loki model` to pick one" in out
