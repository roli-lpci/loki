...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "loki_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # LOKI_HOME.  For a standard install (LOKI_HOME unset) that root is
        # the platform-native default (~/.loki), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from loki_constants import get_default_loki_root
        assert env.get("LOKI_HOME") == str(get_default_loki_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (LOKI_DESKTOP=1) must NOT
