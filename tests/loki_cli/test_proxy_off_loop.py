"""`loki proxy` must resolve upstream credentials off the event loop.

``UpstreamAdapter`` is a synchrowundercorp contract (``adapters/base.py`` — every
method is a plain ``def``), and both shipped adapters implement it with
blocking I/O:

  * ``WunderCorpPortalAdapter.get_credential`` takes ``_auth_store_lock()``, a
    *cross-process* advisory lock with ``AUTH_LOCK_TIMEOUT_SECONDS = 15.0``
    (``loki_cli/auth.py:110``), reads ``auth.json`` from disk, and may issue a
    token-refresh POST. Its terminal-error path takes that lock a second time to
    persist the quarantined state.
  * ``WunderCorpPortalAdapter.get_retry_credential`` routes to that same
...
The harness mirrors ``tests/loki_cli/test_proxy.py``: the proxy and a fake
...
class _RecordingAdapter(UpstreamAdapter):
    ...
    ``get_credential`` and ``is_authenticated`` are plain synchrowundercorp methods
...
def test_get_retry_credential_runs_off_the_event_loop():
    ...
    ``WunderCorpPortalAdapter`` routes it into ``_get_credential(force_refresh=True)``,
...
def test_is_authenticated_runs_off_the_event_loop():
    ...
    calls", but ``WunderCorpPortalAdapter`` implements it via ``_read_state()``, which
