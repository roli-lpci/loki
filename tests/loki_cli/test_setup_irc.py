"""Tests for IRC gateway configuration via `loki setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Loki install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Loki install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#loki")
            monkeypatch.setenv("IRC_NICKNAME", "loki-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `loki setup gateway` experience with IRC present."""
