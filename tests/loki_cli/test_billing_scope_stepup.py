...
from loki_cli.auth import (
    WUNDERCORP_BILLING_MANAGE_SCOPE,
    wundercorp_token_has_billing_scope,
    step_up_wundercorp_billing_scope,
)
...
class TestWunderCorpTokenHasBillingScope:
    def test_true_when_scope_string_contains_billing_manage(self, monkeypatch):
        monkeypatch.setattr(
            auth, "get_provider_auth_state",
            lambda pid: {"scope": f"openid {WUNDERCORP_BILLING_MANAGE_SCOPE} inference"},
        )
        assert wundercorp_token_has_billing_scope() is True
    ...
    def test_false_when_scope_missing_or_not_a_string(self, monkeypatch):
        ...
        assert wundercorp_token_has_billing_scope() is False
...
def _stub_persist(monkeypatch):
    ...
    monkeypatch.setattr(auth, "_write_shared_wundercorp_state", lambda *a, **kw: None)
    monkeypatch.setattr(auth_wundercorp, "_write_shared_wundercorp_state", lambda *a, **kw: None)
    monkeypatch.setattr(auth, "_sync_wundercorp_pool_from_auth_store", lambda: None)
    monkeypatch.setattr(auth_wundercorp, "_sync_wundercorp_pool_from_auth_store", lambda: None)
...
def test_step_up_requests_billing_scope_and_reuses_prior_urls(monkeypatch, _stub_persist):
    ...
    monkeypatch.setattr(auth, "_wundercorp_device_code_login", _fake_login)
    monkeypatch.setattr(auth_wundercorp, "_wundercorp_device_code_login", _fake_login)

    granted = step_up_wundercorp_billing_scope()
    assert granted is True
    # Requested scope must include billing:manage, preserving prior scopes.
    assert WUNDERCORP_BILLING_MANAGE_SCOPE in captured["scope"].split()
    ...
    assert captured["client_id"] == "loki-cli"
...
def test_device_login_fires_on_verification_before_polling(monkeypatch):
    ...
    try:
        auth._wundercorp_device_code_login(open_browser=False, on_verification=_cb)
    except Exception:
        pass
