"""Tests for loki tools disable/enable/list command (backend)."""
...
class TestToolsDisableBuiltin:
    ...
    def test_disable_removes_toolset_from_platform(self):
        ...
        with patch("loki_cli.tools_config.load_config", return_value=config), \
             patch("loki_cli.tools_config.save_config") as mock_save:
...
def test_tools_action_accepts_deferred_plugin_without_materializing(action, capsys):
    ...
    try:
        with patch(
            "loki_cli.plugins.discover_plugins",
            side_effect=discover_deferred_platform,
        ) as discover, \
             patch("loki_cli.tools_config.load_config", return_value=config), \
             patch("loki_cli.tools_config.save_config") as save:
