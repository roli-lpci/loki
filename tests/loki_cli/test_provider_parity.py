...
the SAME provider universe as ``loki model`` (the CLI/TUI picker).
...
def test_every_loki_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `loki model` universe."""
    ...
    assert not missing, (
        "providers shown in `loki model` but not configurable in the desktop "
