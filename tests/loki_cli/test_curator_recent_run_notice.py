...
The notice prints the most recent curator run summary on `loki update`,
...
session start) so users normally never see the rename map. `loki update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    from loki_cli import update_cmd_maint
