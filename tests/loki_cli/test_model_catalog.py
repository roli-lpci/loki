"""Tests for loki_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate LOKI_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".loki"
    home.mkdir()
    ...
    import importlib
    from loki_cli import model_catalog
...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "wundercorp": {
                "metadata": {"display_name": "WunderCorp Portal"},
                "models": [
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from loki_cli.model_catalog import _validate_manifest
...
class TestFallbackChain:
    ...
    when the primary URL fails. Regression: the Doku site behind Vercel
    ...
    PRIMARY = "https://loki.computer/docs/api/model-catalog.json"
    FALLBACK = (
        "https://raw.githubusercontent.com/wundercorp/loki"
...
class TestCuratedAccessors:
    ...
    def test_wundercorp_returns_none_when_catalog_empty(self, isolated_home):
        from loki_cli import model_catalog
        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            assert model_catalog.get_curated_wundercorp_models() is None
...
class TestDefaultModelFromCache:
    ...
    def _manifest_with_default(self) -> dict:
        ...
        m["providers"]["wundercorp"]["models"][1]["default"] = True  # kimi-k2.6
        return m
    ...
    def test_reads_label_from_disk_cache(self, isolated_home):
        ...
        with patch.object(model_catalog, "_fetch_manifest") as fetch:
            ...
            assert (
                model_catalog.get_default_model_from_cache("wundercorp")
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        import loki_cli.model_catalog as model_catalog
        from loki_cli.models import PREFERRED_SILENT_DEFAULT_MODEL

        repo_root = Path(model_catalog.__file__).resolve().parent.parent
        manifest = json.loads(
            (repo_root / "website" / "static" / "api" / "model-catalog.json").read_text()
        )
        for provider in ("openrouter", "wundercorp"):
...
class TestRefreshCadence:
    def test_default_ttl_is_twenty_minutes_and_legacy_hours_honoured(self):
        ...
        with patch("loki_cli.config.load_config", return_value={"model_catalog": {"ttl_minutes": 20}}):
        ...
        with patch("loki_cli.config.load_config", return_value={"model_catalog": {"ttl_minutes": 20, "ttl_hours": 3}}):
    ...
    def test_refresh_catalogs_forces_every_source(self):
        ...
        with patch.object(model_catalog, "_load_catalog_config", return_value={
            "enabled": True, "url": "http://master", "ttl_hours": 1.0, "providers": {},
             ...
             patch("loki_cli.models.fetch_openrouter_models") as orm, \
             patch("loki_cli.models.fetch_wundercorp_recommended_models") as wundercorp:
        ...
        wundercorp.assert_called_once_with(force_refresh=True)


class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in loki_cli.models."""



    def test_picker_wundercorp_row_uses_curated_list(self, tmp_path, monkeypatch):
        """The /model picker surfaces the curated ``_PROVIDER_MODELS["wundercorp"]``
        list in curated order — matching the ``loki model`` CLI — not the live
        ...
        # ``LOKI_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` LOKI_HOME directly instead.
        import importlib
        from loki_cli import model_catalog
        from loki_cli.models import get_curated_wundercorp_model_ids
        importlib.reload(model_catalog)
        try:
            from loki_cli.model_switch_providers import list_picker_providers

            active_home = Path(os.environ["LOKI_HOME"])
            (active_home / "auth.json").write_text(
                json.dumps(
                    {
                        "providers": {"wundercorp": {"access_token": "fake"}},
                        "credential_pool": {},
                    }
                )
            )
            ...
            # (``curated["wundercorp"] = get_curated_wundercorp_model_ids()``), so the test
            # stays an invariant — it can't rot as the curated/manifest list grows.
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("loki_cli.models.check_wundercorp_free_tier", return_value=False), patch(
                "loki_cli.models.union_with_portal_free_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ), patch(
                "loki_cli.models.union_with_portal_paid_recommendations",
                side_effect=lambda ids, *a, **k: (ids, {}),
            ):
                expected = get_curated_wundercorp_model_ids()
                picker = list_picker_providers(
                    current_provider="wundercorp", max_models=99
                )
        finally:
            model_catalog.reset_cache()

        wundercorp_row = next((r for r in picker if r["slug"] == "wundercorp"), None)
        assert wundercorp_row is not None, "wundercorp row must appear when authed"
        assert wundercorp_row["models"] == expected
    ...
    def test_picker_max_models_cap_semantics(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            with patch.object(
                ...
                expected = get_curated_wundercorp_model_ids()
                full = list_picker_providers(current_provider="wundercorp", max_models=None)
                one = list_picker_providers(current_provider="wundercorp", max_models=1)
                ...
                zero = list_authenticated_providers(
                    current_provider="wundercorp", max_models=0
                )
        finally:
            model_catalog.reset_cache()

        def _wundercorp(rows):
            return next((r for r in rows if r["slug"] == "wundercorp"), None)

        # Only meaningful when the curated list actually exceeds 1 entry.
        assert len(expected) > 1, "test needs a multi-model curated wundercorp list"

        full_row = _wundercorp(full)
        assert full_row is not None and full_row["models"] == expected

        one_row = _wundercorp(one)
        assert one_row is not None and one_row["models"] == expected[:1]

        zero_row = _wundercorp(zero)
...
#
# History: qwen/qwen3.6-plus was added to _PROVIDER_MODELS["wundercorp"] in commit
...
class TestManifestMatchesInRepoLists:
    ...
    def test_in_repo_lists_match_manifest(self):
        ...
        assert self._strip_volatile(actual) == self._strip_volatile(expected), (
            ...
            "_PROVIDER_MODELS['wundercorp'] / OPENROUTER_MODELS. "
