"""Tests for normalized WunderCorp Portal account entitlement helpers."""
...
from loki_cli.wundercorp_account import (
    ...
    wundercorp_portal_topup_url,
    reset_wundercorp_portal_account_info_cache,
)
...
def test_fresh_account_payload_normalization(monkeypatch, payload, expected_paid):
    ...
    monkeypatch.setattr("loki_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("loki_cli.auth.resolve_wundercorp_access_token", lambda: "fresh-token")
    monkeypatch.setattr("loki_cli.wundercorp_account._fetch_wundercorp_account_info", lambda *a, **kw: payload)

    info = get_wundercorp_portal_account_info(force_fresh=True)

    assert isinstance(info, WunderCorpPortalAccountInfo)
...
def test_no_oauth_token_reports_inference_key_present(monkeypatch):
    monkeypatch.setattr("loki_cli.auth.get_provider_auth_state", lambda provider: {})

    class _Entry:
        label = "manual-wundercorp"
        access_token = ""
    ...
    assert info.credential_source == "pool:manual-wundercorp"
...
def test_member_spend_cap_exceeded_message(monkeypatch):
    ...
    msg = format_wundercorp_portal_entitlement_message(info, capability="WunderCorp model access")
