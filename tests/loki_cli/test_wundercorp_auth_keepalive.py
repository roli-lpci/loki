from loki_cli import wundercorp_auth_keepalive as keepalive
...
def test_refresh_always_fires_before_expiry_for_observed_lifetimes():
    ...
    for lifetime in OBSERVED_LIFETIMES_SECONDS:
        tick = keepalive._tick_seconds(
            keepalive.WUNDERCORP_AUTH_KEEPALIVE_INTERVAL_SECONDS, lifetime
        )
        horizon = keepalive._refresh_horizon_seconds(
            tick, keepalive.WUNDERCORP_INVOKE_JWT_MIN_TTL_SECONDS
        )
...
def test_interval_precedence_and_disable(monkeypatch):
    def _config(section):
        monkeypatch.setattr(keepalive, "_wundercorp_config", lambda: section)

    # An absent section leaves the module default in place.
    _config({})
    assert (
        keepalive._interval_seconds(None)
        == keepalive.WUNDERCORP_AUTH_KEEPALIVE_INTERVAL_SECONDS
    )

    _config({keepalive.WUNDERCORP_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY: 600})
    ...
    _config({keepalive.WUNDERCORP_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY: "not-a-number"})
    assert (
        keepalive._interval_seconds(None)
        == keepalive.WUNDERCORP_AUTH_KEEPALIVE_INTERVAL_SECONDS
    )
    ...
    _config({keepalive.WUNDERCORP_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY: 0})
    assert keepalive._interval_seconds(None) == 0
    assert keepalive.start_wundercorp_auth_keepalive() is None
...
def test_keepalive_refreshes_stale_pool_entry(monkeypatch):
    ...
    assert keepalive.refresh_wundercorp_auth_keepalive_once() is True
...
def test_keepalive_falls_back_to_singleton_state(monkeypatch):
    ...
    def _resolve_wundercorp_runtime_credentials(**kwargs):
        calls.append(kwargs)
        return {
            "provider": "wundercorp",
    ...
    monkeypatch.setattr(
        ...
        _resolve_wundercorp_runtime_credentials,
    )

    assert keepalive.refresh_wundercorp_auth_keepalive_once(timeout_seconds=15.0) is True
