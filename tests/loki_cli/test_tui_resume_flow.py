...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setattr(main_mod, "get_loki_home", lambda: tmp_path)
...
def test_oneshot_subprocess_exits_without_teardown_abort():
    program = textwrap.dedent(
        ...
        from loki_cli.main import _exit_after_oneshot
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """loki -z bypasses LokiCLI, but recall still needs SessionDB."""
    from loki_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "loki_state", mod("loki_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "loki_cli.config",
        mod("loki_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "loki_cli.models",
        mod("loki_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        ...
        mod("loki_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["LOKI_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="wundercorp/loki-test", provider="wundercorp", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["LOKI_MODEL"] == "wundercorp/loki-test"
    assert env["LOKI_INFERENCE_MODEL"] == "wundercorp/loki-test"
    assert env["LOKI_TUI_PROVIDER"] == "wundercorp"
    assert env["LOKI_INFERENCE_PROVIDER"] == "wundercorp"
    assert env["LOKI_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["LOKI_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("loki-tui-active-session-")
