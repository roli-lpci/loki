"""Tests for the unified provider catalog (loki_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``loki model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from loki_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
