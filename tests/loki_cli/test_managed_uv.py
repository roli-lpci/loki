"""Tests for loki_cli.managed_uv — one path, no guessing."""
...
def _runtime_info(
    ...
    from loki_cli.sqlite_runtime import SQLiteRuntimeInfo
...
def _RRR(status):
    ...
    """
    from loki_cli.managed_uv import RuntimeRepairResult
...
class TestManagedUvPath:
    ...
    def test_posix(self, tmp_path):
        with patch("loki_cli.managed_uv.get_loki_home", return_value=tmp_path):
            from loki_cli.managed_uv import managed_uv_path
...
class TestMacOSManagedPythonSigning:
    def test_signs_with_stable_identifier_and_verifies(self, tmp_path, monkeypatch):
        import loki_cli.managed_uv as managed_uv
        ...
        assert calls[0][0] == [
            ...
            "--timestamp=none",
            "--identifier",
            "com.wundercorp.lokiagent.managed-python",
            "--requirements",
            '=designated => identifier "com.wundercorp.lokiagent.managed-python"',
            str(python),
        ]
        assert calls[1][0] == [
            "/usr/bin/codesign",
...
class TestEnsureUv:
    ...
    def test_installs_if_missing(self, tmp_path):
        uv = tmp_path / "bin" / _UV_BINARY_NAME
        with patch("loki_cli.managed_uv.get_loki_home", return_value=tmp_path), \
             patch("loki_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("loki_cli.managed_uv._uv_version", return_value="uv 0.1.2"), \
             patch("loki_cli.managed_uv._install_uv") as mock_install:
    ...
    def test_install_reports_runtime_repair_to_observer(self, tmp_path):
        from loki_cli.managed_uv import (
            RuntimeRepairResult,
            ensure_uv,
        )
        ...
        observed = []
        with patch(
            "loki_cli.managed_uv.get_loki_home",
            return_value=tmp_path,
        ), patch(
            "loki_cli.managed_uv._install_uv",
            side_effect=fake_install,
        ), patch(
            "loki_cli.managed_uv._uv_version",
            return_value="uv 0.1.2",
        ), patch(
            "loki_cli.managed_uv.repair_vulnerable_runtime",
            return_value=repair,
        ):
...
class TestEnsureUvUpdateBoundary:
    ...
    ``loki update`` runs the call site from the old, already-imported
    ``loki_cli.main`` against the freshly pulled ``managed_uv``. A release
    ...
    def test_failure_unpacks_without_raising(self, tmp_path):
        with patch("loki_cli.managed_uv.get_loki_home", return_value=tmp_path), \
             patch("loki_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("loki_cli.managed_uv._install_uv", side_effect=RuntimeError("network down")):
            from loki_cli.managed_uv import ensure_uv
...
class TestEnsureUvWindowsSafe:
    ...
    def test_uvresult_would_break_windows_list2cmdline(self):
        ...
        import subprocess
        from loki_cli.managed_uv import _UvResult
        with pytest.raises(TypeError):
            subprocess.list2cmdline([_UvResult("C:\\loki\\uv.exe"), "pip"])
    ...
    def test_windows_returns_plain_str_safe_for_subprocess(self, tmp_path):
        ...
        with patch("loki_cli.managed_uv.get_loki_home", return_value=tmp_path), \
             patch("loki_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")):
            from loki_cli.managed_uv import _UvResult, ensure_uv
            uv_bin = ensure_uv()
...
class TestUpdateManagedUv:
    ...
    def test_fresh_stamp_skips_network_self_update_but_not_repair(self, tmp_path):
        ...
        from loki_cli.managed_uv import RuntimeRepairResult, update_managed_uv

        uv = tmp_path / "bin" / _UV_BINARY_NAME
        _make_executable(uv)
        # The stamp reader imports get_loki_home separately from the binary
        ...
        with patch("loki_cli.managed_uv.get_loki_home", return_value=tmp_path), \
             patch("loki_cli.managed_uv._uv_self_update_stamp", return_value=stamp), \
             patch("loki_cli.managed_uv.repair_vulnerable_runtime",
                   return_value=RuntimeRepairResult("skipped")) as mock_repair, \
             patch("loki_cli.managed_uv.subprocess.run") as mock_run:
    ...
    def test_stale_stamp_runs_self_update_and_refreshes_stamp(self, tmp_path):
        ...
        from loki_cli.managed_uv import UV_SELF_UPDATE_INTERVAL_SECONDS, update_managed_uv
        ...
        with patch("loki_cli.managed_uv.get_loki_home", return_value=tmp_path), \
             patch("loki_cli.managed_uv._uv_self_update_stamp", return_value=stamp), \
             patch("loki_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("loki_cli.managed_uv._uv_version", return_value="uv 0.2.0"), \
             patch("loki_cli.managed_uv.subprocess.run") as mock_run:
...
class TestManagedPythonStore:
    def test_store_is_checkout_scoped_across_profiles(self, tmp_path, monkeypatch):
        from loki_cli.managed_uv import managed_python_install_dir

        checkout = tmp_path / "checkout"
        monkeypatch.setenv("LOKI_HOME", str(tmp_path / "profiles" / "alpha"))
        alpha = managed_python_install_dir(checkout)
        monkeypatch.setenv("LOKI_HOME", str(tmp_path / "profiles" / "beta"))
        beta = managed_python_install_dir(checkout)

        expected = checkout / ".loki-runtime" / "python"
...
class TestRuntimeRepair:
    def test_safe_runtime_is_a_noop(self, tmp_path):
        from loki_cli.managed_uv import repair_vulnerable_runtime

        root, live, sentinel = _make_runtime_install(tmp_path)
        current = _runtime_info(live / "bin" / "python", (3, 53, 1))
        with patch(
                 "loki_cli.managed_uv.probe_sqlite_runtime",
                 return_value=current,
             ), \
             patch(
                 "loki_cli.managed_uv._install_safe_python_generation"
             ) as mock_install:
        ...
        assert not (root / ".loki-runtime").exists()
    ...
    def test_failed_candidate_preserves_live_venv(self, tmp_path):
        ...
        generation = root / ".loki-runtime" / "python" / "generation-test"
        ...
        with patch(
             ...
             patch(
                 "loki_cli.managed_uv._stage_candidate_venv",
                 return_value=None,
             ):
            result = repair_vulnerable_runtime("uv", project_root=root)
        ...
        reacquired = _acquire_repair_lock(root / ".loki-runtime")
    ...
    def test_successful_repair_removes_parked_backup(self, tmp_path):
        ...
        candidate_python.write_text("candidate interpreter", encoding="utf-8")
        fixed = _runtime_info(candidate_python, (3, 53, 1))
        candidate_venv = root / ".loki-runtime" / "venv-candidate"
        ...
        with patch(
             ...
             patch(
                 "loki_cli.managed_uv._smoke_candidate_venv",
...
class TestStageCandidateVenvCrossPlatform:
    ...
    def test_sync_keeps_uv_project_config_and_merges_stderr(self, tmp_path):
        ...
        from loki_cli.managed_uv import _stage_candidate_venv

        root = tmp_path / "checkout"
        root.mkdir()
        (root / "uv.lock").write_text("# lock\n", encoding="utf-8")
        generation = root / ".loki-runtime" / "python" / "gen"
        ...
        with patch("loki_cli.managed_uv.subprocess.run", side_effect=fake_run), \
             patch(
                 "loki_cli.managed_uv._smoke_candidate_venv",
...
class TestRuntimeCutover:
    def test_os_lock_blocks_concurrent_repair_and_releases(self, tmp_path):
        from loki_cli.managed_uv import _acquire_repair_lock, _release_repair_lock

        runtime_root = tmp_path / ".loki-runtime"
    ...
    def test_post_swap_smoke_failure_rolls_back_live_venv(self, tmp_path):
        from loki_cli.managed_uv import _cut_over_candidate

        root, live, sentinel = _make_runtime_install(tmp_path)
        runtime_root = root / ".loki-runtime"
...
class TestInstallUvInternals:
    def test_installer_uses_host_branch_and_managed_directory(self, tmp_path):
        ...
        with patch("loki_cli.managed_uv._install_uv_posix") as mock_posix, \
             patch("loki_cli.managed_uv._install_uv_windows") as mock_windows:
...
class TestRuntimeRequestMinorLine:
    ...
    def test_requests_minor_line(self):
        from loki_cli.managed_uv import _runtime_request
...
class TestMinorLineFallForward:
    ...
    `loki update` with no path to a fixed runtime.
    """
...
class TestRefreshManagedUvCatalog:
    ...
    def test_version_change_reports_true(self, tmp_path):
        ...
        with patch("loki_cli.managed_uv.get_loki_home", return_value=tmp_path), \
             patch("loki_cli.managed_uv._install_uv"), \
             patch(
                 "loki_cli.managed_uv._uv_version_string",
...
class TestRepairRetriesAfterUvRefresh:
    def _run_repair(self, tmp_path, *, refresh_result, second_attempt):
        ...
        with patch(
             ...
             patch(
                 "loki_cli.managed_uv._refresh_managed_uv_catalog",
    ...
    def test_retry_success_proceeds_to_staging(self, tmp_path):
        def second_attempt(root):
            generation = root / ".loki-runtime" / "python" / "generation-retry"
            candidate_python = generation / "bin" / "python"
            candidate_python.parent.mkdir(parents=True)
...
class TestDefaultLiveVenv:
    ...
    'not-applicable' on every loki update and stayed on journal_mode=DELETE
    ...
    def test_dot_venv_only_is_targeted(self, tmp_path):
        from loki_cli.managed_uv import _default_live_venv
...
class TestVenvPythonUpdateBoundary:
    """``_venv_python`` must survive a loki_constants predating its symbol.

    ``loki update`` imports loki_constants from the OLD checkout, ``git
    pull`` replaces that file, and the freshly-pulled managed_uv then runs its
    lazy ``from loki_constants import venv_python_path`` against the module
    ...
    plainly contains it, which is what made the error so confusing:
        ...
        cannot import name 'venv_python_path' from 'loki_constants'
        (~/.loki/loki-agent/loki_constants.py)
    ...
    def test_recovers_when_the_cached_module_predates_the_symbol(self, monkeypatch):
        ...
        from loki_cli.managed_uv import _venv_python
        ...
        monkeypatch.delattr(loki_constants, "venv_python_path", raising=False)
        ...
        expected = Path("/opt/loki/venv/Scripts/python.exe") \
            if sys.platform == "win32" else Path("/opt/loki/venv/bin/python")
        assert _venv_python(Path("/opt/loki/venv")) == expected

    def test_recovery_uses_the_shared_helper_not_a_second_copy(self, monkeypatch):
        """The reload must resolve through loki_constants, not open-code it.

        Hand-rolling `Scripts`/`bin` here is what #76105 deduped away and what
        `test_no_open_coded_venv_layout_remains_in_loki_cli` bans.
        """
        import loki_constants
        ...
        assert _venv_python(Path("/opt/loki/venv")) == sentinel
...
class TestWindowsRuntimeSelfLock:
    ...
    A CLI ``loki update`` runs from the venv's own python, and
    ...
    def test_self_lock_defers_repair_before_provisioning(
        ...
        from loki_cli import managed_uv
        from loki_cli.managed_uv import repair_vulnerable_runtime
        ...
        with patch(
                 "loki_cli.managed_uv._windows_runtime_holders",
                 return_value=(False, ""),
             ), \
             patch(
                 "loki_cli.managed_uv.probe_sqlite_runtime",
    ...
    def test_venv_launcher_ancestor_is_a_self_lock(self, tmp_path, monkeypatch):
        r"""The venv\Scripts\loki.exe shim stays mapped while it waits for
