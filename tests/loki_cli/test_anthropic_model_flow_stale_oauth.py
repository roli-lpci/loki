"""Tests for Bug #12905 fix — stale OAuth token detection in loki model flow.

Bug 3: `loki model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.loki/.env but no valid
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        monkeypatch.setattr("loki_cli.secret_prompt.masked_secret_prompt", lambda _: "")

        from loki_cli.model_setup_flows import _model_flow_anthropic
        cfg = {}
