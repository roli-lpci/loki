...
Covers:
- Defense 1: gateway stop/restart refuse when _LOKI_GATEWAY=1
...
from loki_cli.cron import cron_command
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "loki gateway stop",
        "loki gateway uninstall",
        "loki  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "LOKI GATEWAY RESTART",           # uppercase
    ])
    def test_loki_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.loki.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.loki/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l loki-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.loki.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.loki.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.loki.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.loki/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.loki.gateway-apollo:/a.plist' "
            "'ai.loki.gateway-cronus:/c.plist' 'ai.loki.gateway-plutus:/p.plist' "
            "'ai.loki.gateway:/Users/botuser/Library/LaunchAgents/ai.loki.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated loki note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.loki.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.loki.gateway',
        "launchctl kick'start' -k gui/501/ai.loki.gateway",
        "launchctl kick\\start -k gui/501/ai.loki.gateway",
        'launchctl "kickstart" -k gui/501/ai.loki.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.loki.gateway',
        "launchctl dis\\able gui/501/ai.loki.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.loki."gateway"',
        # Same class on the systemctl and loki-CLI branches.
        'systemctl re"start" loki-gateway',
        'loki gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.loki.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.loki.update-checker',
        'systemctl re"start" loki-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "loki cron list",
        "loki update",
        "loki config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `loki gateway start` is benign — starting a gateway from inside a
        ...
        "loki gateway start",
        "loki gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway loki
        # services must not be falsely blocked (the old `.*loki` matched any
        # loki token).
        "launchctl unload ai.loki.update-checker.plist",
        "launchctl restart ai.loki.daemon",
        ...
        "systemctl restart loki-meta.service",
        "systemctl restart loki-cron-helper",
        ...
        "echo after the loki gateway restarted cleanly",
        "the loki gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "loki skill view gateway-notes && echo loki gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `loki` is a path component
        # there, not a command.
        "cat '/docs/loki gateway restart-notes.md'",
        "less /home/user/notes/loki gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "loki gateway restart",
        "loki gateway restart; echo done",
        "loki gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;loki gateway restart",
        "true && loki gateway stop",
        "echo $(loki gateway restart)",
        "echo `loki gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.loki.gateway"])',
        'subprocess.run(["loki", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.loki.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["loki", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: loki gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `loki -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry LOKI_HOME/LOKI_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("LOKI_PROFILE", "zeus")
        monkeypatch.delenv("LOKI_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "loki -p zeus gateway stop",
        "loki -p zeus gateway restart",
        "loki --profile zeus gateway restart",
        "loki --profile zeus gateway stop",
        "loki --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "loki -v -p zeus gateway restart",
        "loki -p zeus -v gateway restart",
        "loki --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "loki -p venus gateway stop",
        "loki -p venus gateway restart",
        "loki --profile venus gateway restart",
        "loki --profile=venus gateway stop",
        "loki -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "loki -p zeus gateway start",
        "loki -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("loki gateway restart")
        assert _contains_gateway_lifecycle_command("loki gateway stop")

    def test_loki_home_derived_profile(self, monkeypatch):
        # Without LOKI_PROFILE the guard falls back to the LOKI_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("LOKI_PROFILE", raising=False)
        monkeypatch.delenv("LOKI_PROFILE_NAME", raising=False)
        import loki_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("loki -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("loki -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("loki -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("loki gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_loki_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade loki then run loki gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_loki_gateway_once.sh). The script must live under
        # LOKI_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("LOKI_HOME", str(tmp_path / ".loki"))
        scripts_dir = tmp_path / ".loki" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_LOKI_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _LOKI_GATEWAY=1.

    Issue #37453: systemctl --user restart loki-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart loki-gateway",
        "systemctl --user restart loki-gateway",
        "systemctl stop loki-gateway.service",
        "loki gateway restart",
        "loki gateway uninstall",
        "launchctl kickstart gui/501/ai.loki.gateway",
        "launchctl bootout gui/501/ai.loki.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.loki.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.loki/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.loki.gateway.restart-once.plist",
        "pkill -f loki.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart loki-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.loki.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-loki label: label-independent detection is the point
        # (#62891 second reproduction used `ai.loki.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.loki.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _LOKI_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _LOKI_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_LOKI_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="loki gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["loki gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.loki.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.loki.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-loki systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run loki gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("loki gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeloki gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under LOKI_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("loki gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/loki\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 loki gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_loki_home()` falls back to Path.home(), which raises
        RuntimeError when neither LOKI_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart loki-gateway' .",
        "grep -rn 'loki gateway restart' ./logs",
        "rg 'loki gateway restart' ../archive",
        "grep -c 'systemctl stop loki-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM loki_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell loki gateway restart"',
        'sqlite3 ./db ".system systemctl restart loki-gateway"',
        'psql ./x -c "\\! systemctl restart loki-gateway"',
        "grep -r 'loki gateway restart' . | sh",
        "grep -r 'loki gateway restart' ./logs | bash",
        "grep -r 'loki gateway restart' . | sudo sh",
        "grep -r 'x' . ; loki gateway restart",
        "grep -r 'x' . && systemctl restart loki-gateway",
        'grep -r "$(loki gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart loki-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell loki gateway restart"',
        'sqlite3 db ".system systemctl restart loki-gateway"',
        'psql -c "\\! systemctl restart loki-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    loki_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run loki gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LOKI_HOME", str(tmp_path / ".loki"))
        (tmp_path / ".loki").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run loki gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop loki-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart loki-gateway' /var/log/syslog",
        "rg 'loki gateway restart' /home/user/.loki/logs/",
        "journalctl -u loki-gateway --grep 'systemctl restart loki-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'loki_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell loki gateway restart"',
        'psql -c "\\! systemctl restart loki-gateway"',
        "grep 'systemctl restart loki-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart loki-gateway",
        'grep "$(systemctl restart loki-gateway)" f',
        "grep 'restart' log; systemctl restart loki-gateway",
        'sqlite3 db "SELECT 1"; loki gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "loki gateway restart",
        "sudo systemctl stop loki-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart loki-gateway%'\""
        )
