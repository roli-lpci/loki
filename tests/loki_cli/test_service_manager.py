"""Tests for loki_cli.service_manager — the abstract ServiceManager
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `loki_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import loki_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("loki_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
...
# unprivileged loki user from every lifecycle op. These tests run
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from loki_cli.service_manager import _seed_supervise_skeleton
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``loki gateway run``, an agent action, the Open WebUI helper) holds
    the per-LOKI_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root LOKI_HOME, no ``-p``) and named-profile
    render paths.
    """
    default_text = S6ServiceManager._render_run_script("default", {})
    # Root profile: bare `loki gateway run --replace` (no -p flag).
    assert "loki gateway run --replace" in default_text
    assert "loki -p default" not in default_text
...
#
# `loki gateway stop` inside a container dispatches through
...
def test_s6_log_run_creates_leaf_as_loki_without_chown(
    s6_scandir, fake_subprocess_run,
) -> None:
    """log/run must not root-chown/unlink volume paths; create leaf as loki.

    #45258 parent ownership is stage2's job (``logs/gateways`` seeded as
    loki). Restartable log/run must not pathname-chown or pathname-rm a
    loki-writable tree from root — that is a symlink TOCTOU hole.
    """
    ...
    assert not any(line.lstrip().startswith("chown ") for line in log_text.splitlines()), (
        "restartable log/run must not invoke chown on loki-writable paths; "
        f"saw: {log_text!r}"
    )
    assert 's6-setuidgid loki mkdir -p "$log_dir"' in log_text
    assert 's6-setuidgid loki rm -f "$log_dir/lock"' in log_text
    assert 'else\n  mkdir -p "$log_dir"\n  rm -f "$log_dir/lock"\nfi\n' in log_text
    ...
    mkdir_as_loki_idx = log_text.index('s6-setuidgid loki mkdir -p "$log_dir"')
    rm_as_loki_idx = log_text.index('s6-setuidgid loki rm -f "$log_dir/lock"')
    exec_idx = log_text.index("s6-log 1 ")
    assert mkdir_as_loki_idx < rm_as_loki_idx < exec_idx
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    gateways = loki_home / "logs" / "gateways"
    ...
    # Mark the drop so fake rm can refuse unlink outside LOKI_HOME the way
    # a real loki uid cannot delete a foreign root-owned lock.
    ...
    (bin_dir / "s6-setuidgid").write_text(
        "#!/bin/sh\n"
        "shift\n"
        'LOKI_TEST_DROPPED=1 exec "$@"\n',
        encoding="utf-8",
    )
    real_rm = "/bin/rm"
    (bin_dir / "rm").write_text(
        "#!/bin/sh\n"
        # Privilege-dropped: no-op. Models that loki cannot unlink a foreign
