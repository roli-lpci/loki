"""Tests for loki_cli.stderr_timestamp."""
...
from loki_cli import stderr_timestamp
...
_LAUNCHD_ENV = {"PATH": "/usr/bin", "XPC_SERVICE_NAME": "ai.loki.gateway-butler"}
...
def test_main_injects_flag_into_stale_gateway_child(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XPC_SERVICE_NAME", "ai.loki.gateway-butler")
    ...
    stale = [sys.executable, "-c", code, "-m", "loki_cli.main", "gateway", "run", "--replace"]
