"""Tests for loki_cli.web_server and related config utilities."""
...
import loki_cli.config as _cfg_mod
import loki_cli.web_routers.chat_ws as _rt_chat_ws
import loki_cli.web_routers.status as _rt_status
import loki_cli.web_server_chat as _web_server_chat
import loki_cli.web_server_config as _web_server_config
import loki_cli.web_server_dashboard as _web_server_dashboard
import loki_cli.web_server_files as _web_server_files
import loki_cli.web_server_gateway as _web_server_gateway
import loki_cli.web_server_lifecycle as _web_server_lifecycle
import loki_cli.web_server_memory as _web_server_memory
import loki_cli.web_server_messaging as _web_server_messaging
import loki_cli.web_server_sessions as _web_server_sessions
...
@pytest.fixture
def _install_example_plugin(_isolate_loki_home):
    """Drop the example-dashboard fixture into the per-test LOKI_HOME
    ...
    ``LOKI_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, loki-achievements, model providers
    ...
    from loki_cli import web_server

    user_plugins_dir = get_loki_home() / "plugins"
    ...
    # plugin in exactly as a real operator would with `loki plugins
    # enable example`.
    ...
    #      and start serving requests against a torn-down LOKI_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Loki vars not present in .env."""
        env_file = tmp_path / ".env"
...
class TestSessionTokenInjection:
    """The desktop shell mints LOKI_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import loki_cli.web_server as ws

        original_app = ws.app
        original_token = ws._SESSION_TOKEN
        monkeypatch.setenv("LOKI_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
    ...
    def test_falls_back_to_random_token(self, monkeypatch):
        ...
        monkeypatch.delenv("LOKI_DASHBOARD_SESSION_TOKEN", raising=False)
        with patch.object(
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_loki_home):
        """Create a TestClient and isolate the state DB under the test LOKI_HOME."""
        try:
            from starlette.testclient import TestClient
        except ImportError:
        ...
        import loki_state
        from loki_constants import get_loki_home
        from loki_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(loki_state, "DEFAULT_DB_PATH", get_loki_home() / "state.db")
    ...
    def test_get_status_loads_gateway_config_off_event_loop(self, monkeypatch):
        ...
        import loki_cli.web_server as web_server
    ...
    def test_profiles_sidebar_heals_stale_schema_store(self):
        ...
        array — the desktop rendered "No sessions yet" after `loki update`
    ...
    def test_startup_eager_reconcile_heals_stale_store(self):
        ...
        #79531/#80037: after `loki update` an old-schema state.db used to
    ...
    def test_startup_eager_reconcile_never_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(loki_state, "SessionDB", boom)
    ...
    def test_heal_gives_up_when_reconcile_cannot_fix_the_store(self, monkeypatch):
        ...
        original_init = loki_state.SessionDB.__init__
        ...
        # the class on loki_state covers every open the helper makes.
        monkeypatch.setattr(loki_state.SessionDB, "__init__", counting_init)
    ...
    def test_generic_corruption_does_not_trigger_writable_heal(
        ...
        monkeypatch.setattr(loki_state, "SessionDB", corrupt_open)
    ...
    def test_decode_error_triggers_writable_heal(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(loki_state, "SessionDB", scripted_open)
    ...
    def test_messaging_platforms_profile_scopes_gateway_reads(self, monkeypatch):
        ...
        LOKI_HOME contextvar override (#56986), so /api/messaging/platforms
        ...
        """
        import loki_cli.web_server as web_server
        from loki_cli import profiles as profiles_mod
    ...
    def test_declared_surface_put_writes_config_and_secret(self):
        from loki_constants import get_loki_home
        from loki_cli.config import load_env
        ...
        config_path = get_loki_home() / "hindsight" / "config.json"
    ...
    def test_put_memory_provider_config_writes_config_and_secret(self):
        from loki_constants import get_loki_home
        from loki_cli.config import load_config, load_env
    ...
    def _isolate_honcho_config(self):
        # Honcho tests write the suite-wide LOKI_HOME honcho.json; snapshot and
        # restore it so provider status/config state never leaks across tests.
        from loki_constants import get_loki_home

        path = get_loki_home() / "honcho.json"
    ...
    def test_put_honcho_writes_host_block_root_and_secret(self, monkeypatch, tmp_path):
        ...
        cfg = json.loads((get_loki_home() / "honcho.json").read_text(encoding="utf-8"))
        # baseUrl is root-scoped; the rest live in the active host block.
        assert cfg["baseUrl"] == "https://honcho.example.dev"
        assert cfg["hosts"]["loki"]["workspace"] == "myws"
        assert cfg["hosts"]["loki"]["peerName"] == "eri"
        assert cfg["hosts"]["loki"]["environment"] == "local"
        assert cfg["hosts"]["loki"]["sessionStrategy"] == "per-repo"
        # The key lands where the client reads first; GET keeps it write-only.
        assert cfg["hosts"]["loki"]["apiKey"] == "hch-test-key"
    ...
    def test_get_media_requires_auth(self):
        from loki_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_update_loki_returns_docker_guidance_without_spawning(self, monkeypatch):
        import loki_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn loki update")
        ...
        # method through loki_cli.config directly.
        monkeypatch.setattr(
            "loki_cli.config.detect_install_method", lambda *_a, **_k: "docker"
        )
        monkeypatch.setattr(_cfg_mod, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(_web_server_gateway, "_spawn_loki_action", fail_spawn)
        _web_server_gateway._ACTION_PROCS.pop("loki-update", None)
        _web_server_gateway._ACTION_RESULTS.pop("loki-update", None)

        resp = self.client.post("/api/loki/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "loki-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull wundercorp/loki-agent:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/loki-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull wundercorp/loki-agent:latest" in line for line in status_data["lines"])

    def test_update_loki_returns_apt_guidance_without_spawning(self, monkeypatch):
        import loki_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("APT-managed update guard should not spawn loki update")

        monkeypatch.setattr(_web_server_files, "_dashboard_local_update_managed_externally", lambda: False)
        # The shared admission gate (#91277 Phase 3) resolves the install
        # method through loki_cli.config directly, so patch it there (the
        # web_server module alias only feeds the /update/check endpoint).
        monkeypatch.setattr(
            "loki_cli.config.detect_install_method", lambda *_a, **_k: "apt"
        )
        ...
        assert data["update_command"] == "pkg upgrade loki-agent"
        assert spawned is False

        check = self.client.get("/api/loki/update/check")
        assert check.status_code == 200
        check_data = check.json()
        assert check_data["install_method"] == "apt"
        assert check_data["can_apply"] is False
        assert check_data["update_command"] == "pkg upgrade loki-agent"
    ...
    def test_update_status_recovers_completed_result_after_dashboard_restart(self, monkeypatch, tmp_path):
        ...
        action_id = "c" * 32
        (tmp_path / "loki-update.log").write_text(
            "=== loki-update started 2026-08-17 11:19:34 ===\n"
            "pulling updates...\n",
            encoding="utf-8",
        )
        (tmp_path / "update.log").write_text(
            "=== loki update started 2026-08-17T11:19:35 ===\n"
            "✓ Update complete!\n"
            f"=== loki-update completed {action_id} ===\n",
            encoding="utf-8",
        )
        ...
        status = self.client.get("/api/actions/loki-update/status?lines=2000")
        ...
        assert f"=== loki-update completed {action_id} ===" in data["lines"]

    def test_update_loki_spawns_with_action_id(self, monkeypatch):
        import loki_cli.web_server as web_server
        ...
        monkeypatch.setattr(_web_server_gateway, "_spawn_loki_action", fake_spawn)
        _web_server_gateway._ACTION_PROCS.pop("loki-update", None)
        _web_server_gateway._ACTION_RESULTS.pop("loki-update", None)
        ...
        assert calls == [
            (["update"], "loki-update", {"LOKI_ACTION_ID": "a" * 32})
        ]

    def test_update_loki_reuses_running_action(self, monkeypatch):
        import loki_cli.web_server as web_server
        ...
        _web_server_gateway._ACTION_PROCS["loki-update"] = Proc()
        _web_server_gateway._ACTION_IDS["loki-update"] = "b" * 32

        try:
            resp = self.client.post("/api/loki/update")
        finally:
            _web_server_gateway._ACTION_PROCS.pop("loki-update", None)
            _web_server_gateway._ACTION_IDS.pop("loki-update", None)
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        tests/loki_cli/test_setup_hidden_env.py.
        """
        from loki_cli.web_server_messaging import _MESSAGING_KEYS_PAGE_KEYS, _build_catalog_entry, _channel_managed_env_keys
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Loki provider — keep the user's aggregator instead of writing a
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from loki_cli.config import save_env_value
        from loki_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from loki_cli.config import save_env_value
        from loki_cli.web_server import _SESSION_TOKEN
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "suggested_username": "loki_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/LokiSetupBot/loki_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/LokiSetupBot/loki_pair_restart_fails_bot",
            ...
            return {
                "status": "ready",
                "bot_username": "loki_pair_restart_fails_bot",
        ...
        monkeypatch.setattr(_web_server_gateway, "_spawn_loki_action", fail_spawn_action)
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from loki_cli.web_server_profiles import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``loki model`` custom flow. Regression for the desktop loop where a
    ...
    def test_deleting_the_active_custom_endpoint_clears_its_model_mirror(self):
        ...
        """
        from loki_cli.config import custom_endpoint_key_env, get_env_value, load_config
    ...
    def test_numeric_yaml_provider_key_can_be_activated_and_deleted(self):
        ...
        """
        from loki_cli.config import get_config_path, load_config
    ...
    def test_custom_endpoint_save_scopes_to_the_requested_profile(self):
        ...
        """
        from loki_cli import profiles as profiles_mod
        from loki_cli.config import custom_endpoint_key_env
        from loki_constants import get_loki_home

        default_home = get_loki_home()
        worker_home = profiles_mod.get_profile_dir("worker")
        worker_home.mkdir(parents=True)
    ...
    def test_custom_endpoint_save_leaves_a_hand_written_env_ref_alone(self, monkeypatch):
        ...
        from loki_cli.config import custom_endpoint_key_env, get_config_path, get_env_value
    ...
    def test_two_endpoints_on_one_host_keep_separate_credentials(self):
        ...
        the first's key.
        """
        from loki_cli.config import custom_endpoint_key_env, get_env_value
...
class TestBuildSchemaFromConfig:
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from loki_cli.web_server_config import CONFIG_SCHEMA, _timezone_options
...
class TestConfigRoundTrip:
    ...
    def test_round_trip_preserves_schema_invisible_nested_keys(self):
        ...
        from loki_cli.config import load_config, read_raw_config, save_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_loki_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_profiles_create_builder_mcp_auth_is_profile_scoped(
        ...
        from loki_constants import get_loki_home
        import loki_cli.profiles as profiles_mod
    ...
    def test_toolsets_resolve_subscription_features_once(self, monkeypatch):
        ...
        from loki_cli.wundercorp_subscription import WunderCorpSubscriptionFeatures

        calls = 0
        features = WunderCorpSubscriptionFeatures(
            subscribed=False,
            wundercorp_auth_present=False,
            provider_is_wundercorp=False,
            features={},
            account_info=None,
        )
        ...
        monkeypatch.setattr(
            tools_config,
            "get_wundercorp_subscription_features",
            resolve_features,
        )
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        logged-out WunderCorp Subscription rows, xAI TTS without Grok OAuth, and
        never-installed KittenTTS/Piper. The endpoint now reports the honest
        state so keyless ≠ ready.
        """
        import loki_cli.tools_config as tools_config
        from loki_cli.wundercorp_account import WunderCorpPortalAccountInfo

        # Logged out of WunderCorp Portal → managed subscription rows need sign-in.
        monkeypatch.setattr(
            "loki_cli.wundercorp_subscription.get_wundercorp_portal_account_info",
            lambda *a, **k: WunderCorpPortalAccountInfo(
        ...
        import loki_cli.tools_config_post_setup as tools_config_post_setup
        ...
        assert by_name["WunderCorp Subscription"]["status"] == "needs_auth"
    ...
    def test_select_managed_wundercorp_provider_reports_needs_wundercorp_auth(self, monkeypatch):
        """Selecting a managed WunderCorp row while logged out flags needs_wundercorp_auth.
        ...
        (ensure_wundercorp_portal_access) — so the row never activated and nothing
        told the user to sign in. The endpoint now reports the entitlement
        gap so the client can drive the existing WunderCorp OAuth flow.
        """
        from loki_cli.wundercorp_account import WunderCorpPortalAccountInfo
        ...
        resp = self.client.put(
            ...
            json={"provider": "WunderCorp Subscription (Browser Use cloud)"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
        assert data["needs_wundercorp_auth"] is True
        assert data["feature"] == "browser"
        # The selection is still persisted — activation is what's gated.
        # Managed rows store the single 'wundercorp' provider string (the runtime
        # maps it to the Browser Use cloud through the WunderCorp Tool Gateway).
        from loki_cli.config import load_config
        cfg = load_config()
        assert cfg["browser"]["cloud_provider"] == "wundercorp"
    ...
    def test_terminal_ssh_probe_ready_when_configured(self, monkeypatch):
        ...
        assert "loki@devbox.example.com" in ssh["detail"]
...
class TestDesktopLoopbackAuthExemption:
    ...
    def test_exempt_with_desktop_env_and_session_token_on_loopback(self, monkeypatch):
        ...
        monkeypatch.setenv("LOKI_DESKTOP", "1")
        monkeypatch.setenv("LOKI_DASHBOARD_SESSION_TOKEN", "desktop-minted")
    ...
    def test_not_exempt_without_desktop_env(self, monkeypatch):
        ...
        monkeypatch.setenv("LOKI_DASHBOARD_SESSION_TOKEN", "tok")
    ...
    def test_not_exempt_without_any_credential(self, monkeypatch):
        ...
        # LOKI_DESKTOP=1 alone is not enough: a plain serve with the env var
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from loki_cli.web_server_config import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from loki_cli.web_server_config import _denormalize_config_from_web
        from loki_cli.config import save_config
...
class TestDenormalizeProviderSwitch:
    ...
    def test_vendor_slug_switches_off_non_aggregator_provider(self):
        ...
        with _patch("loki_cli.models_detect.provider_has_credentials", lambda p: p == "openrouter"):
...
class TestStatusInstallId:
    ...
    Loki home (not the profile home), survives a fresh process-cache read,
    ...
    def test_status_reports_persistent_install_id(self, monkeypatch):
        ...
        id_file = get_default_loki_root() / "install_id"
    ...
    def test_all_profiles_of_one_install_share_the_id(self, monkeypatch, tmp_path):
        """LOKI_HOME=<root> and LOKI_HOME=<root>/profiles/<name> resolve to
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from loki_cli.web_server_dashboard import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.loki/dashboard-themes/."""
    ...
    def test_ignores_transient_profile_override(self, tmp_path, monkeypatch):
        ...
        from loki_constants import (
            reset_loki_home_override,
            set_loki_home_override,
        )
        from loki_cli import web_server

        token = set_loki_home_override(str(other))
        try:
            results = _web_server_dashboard._discover_user_themes()
        finally:
...
class TestThemeBootstrapCSS:
    ...
    @staticmethod
    def _write_theme(loki_home, name="ocean"):
        themes_dir = loki_home / "dashboard-themes"
    ...
    def test_user_theme_renders_bundle_vars(self, tmp_path, monkeypatch):
        ...
        assert css.startswith('<style id="loki-theme-bootstrap">')
...
class TestDeleteSessionEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_loki_home):
        ...
        monkeypatch.setattr(
            loki_state, "DEFAULT_DB_PATH", get_loki_home() / "state.db"
        )
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``loki_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in loki_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_loki_home):
        ...
        # Pin the SessionDB to the isolated LOKI_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_loki_home, _install_example_plugin):
        ...
        installs it into the per-test ``LOKI_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into LOKI_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, loki-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (loki-achievements is loaded by default).
        resp = self.client.get("/api/plugins/loki-achievements/overview")
        assert resp.status_code == 401
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_user_plugins_ignore_profile_home_override(self, tmp_path, monkeypatch):
        ...
        LOKI_HOME override scopes a request to another profile."""
        from loki_constants import (
            reset_loki_home_override,
            set_loki_home_override,
        )
    ...
    def test_user_plugins_found_under_profile_scoped_process(self, tmp_path, monkeypatch):
        ...
        sets LOKI_HOME=<root>/profiles/<name>) must still discover user
        plugins installed in the loki root's plugins/ directory."""
        root = tmp_path / "loki-root"
...
# or ``sh -c 'printf …'``) instead of the real ``loki --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_tui_python_command_uses_child_path(self, tmp_path):
        ...
        command = f"loki-review-python{Path(sys.executable).suffix}"
        ...
        assert env["LOKI_PYTHON"] == command
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from loki_cli.pty_bridge import PtyUnavailableError
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from loki_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import loki_cli.main_tui_launch as tui_launch
    import loki_cli.web_server as ws
    ...
    gateway_url = env.get("LOKI_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_loki_home, _install_example_plugin):
        ...
        the per-test ``LOKI_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_loki_home):
        import threading
        ...
        with self._client():
            assert called.wait(3.0), "expected cron tick under LOKI_DESKTOP=1"
...
class TestServeIndexMissingIndex:
    ...
    def _client_with_dist(tmp_path, monkeypatch, *, write_index: bool):
        ...
        monkeypatch.setattr(ws, "WEB_DIST", dist)
        monkeypatch.delenv("LOKI_SERVE_HEADLESS", raising=False)
        spa_app = FastAPI()
        _web_server_dashboard.mount_spa(spa_app)
    ...
    def test_index_uses_ssh_token_applied_after_spa_mount(
        ...
        assert resp.status_code == 200
        assert 'window.__LOKI_SESSION_TOKEN__="after-mount"' in resp.text


class TestHeadlessServeTokenPage:
    """Headless `loki serve` must serve the Desktop token handshake page
    at `/` when the dashboard auth gate is off (#94227).

    The Electron renderer boots by fetching `/` and extracting
    ``window.__LOKI_SESSION_TOKEN__`` for WebSocket auth. Headless serve
    ...
    def _headless_client(monkeypatch, *, gated: bool):
        ...
        monkeypatch.setenv("LOKI_SERVE_HEADLESS", "1")
        spa_app = FastAPI()
    ...
    def test_root_serves_token_page_when_not_gated(self, monkeypatch):
        ...
        match = re.search(
            r'window\.__LOKI_SESSION_TOKEN__\s*=\s*("(?:\\.|[^"\\])*")',
            resp.text,
        )
        ...
        assert "window.__LOKI_AUTH_REQUIRED__=false" in resp.text
...
class TestHashedAssetCacheHeaders:
    ...
    def test_serve_css_is_immutable_and_keeps_prefix_rewrites(
        ...
        prefixed = client.get(
            ...
            headers={"X-Forwarded-Prefix": "/loki"},
        )
        assert prefixed.status_code == 200
        assert prefixed.headers["cache-control"] == self._IMMUTABLE
        assert "url(/loki/ds-assets/bg.png)" in prefixed.text
        assert "url(/loki/fonts-terminal/x.woff2)" in prefixed.text
