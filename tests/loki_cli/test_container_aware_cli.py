...
def container_env(tmp_path, monkeypatch):
    """Set up a fake LOKI_HOME with .container-mode file."""
    loki_home = tmp_path / ".loki"
    loki_home.mkdir()
    monkeypatch.setenv("LOKI_HOME", str(loki_home))
    monkeypatch.delenv("LOKI_DEV", raising=False)

    container_mode = loki_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=loki-agent\n"
        "exec_user=loki\n"
        "loki_bin=/data/current-package/bin/loki\n"
    )
    return loki_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("loki_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "loki-agent"
    assert info["exec_user"] == "loki"
    assert info["loki_bin"] == "/data/current-package/bin/loki"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from loki_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "loki"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "loki-agent" in cmd
    assert "/data/current-package/bin/loki" in cmd
    assert "chat" in cmd
