"""Tests for the AI_AGENT / LOKI_AGENT harness-attribution env vars.
...
Loki-specific marker, without clobbering an outer harness.

The AI_AGENT value must equal Loki' id in the public agent-harness
registry (``loki-agent`` in huggingface.js ``agent-harnesses.ts``) —
...
environment does not inherit the Loki process env, and survives the
cross-session leak guard that strips ``LOKI_SESSION_*`` from subprocess
...
class TestWrapCommandAdvertisesHarness:
    ...
    def _wrap(self, command: str) -> str:
        ...
        env._cwd_marker = "__LOKI_CWD_testsession0__"
        env._snapshot_path = "/tmp/loki-snap-testsession0.sh"
    ...
    def test_wrap_command_contains_export(self):
        ...
        assert 'LOKI_AGENT="${LOKI_AGENT:-true}"' in wrapped
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT LOKI=$LOKI_AGENT"')

        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("AI_AGENT", "LOKI_AGENT")}
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=clean_env, timeout=30,
        )
        assert f"AI={HARNESS_ID} LOKI=true" in out.stdout

        outer_env = dict(clean_env, AI_AGENT="pi", LOKI_AGENT="false")
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
            env=outer_env, timeout=30,
        )
        assert "AI=pi LOKI=false" in out.stdout
