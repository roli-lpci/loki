...
substring check, missing case variants and matching lookalike URLs). Loki
had the same class at several sites: keyless-endpoint detection, /model
catalog routing, local-endpoint detection, and WunderCorp Portal cache-layout
...
def test_wundercorp_portal_host_detection():
    from utils import base_url_host_matches

    assert base_url_host_matches("https://inference-api.wundercorp.com/v1", "wundercorp.co")
    assert base_url_host_matches("https://portal.wundercorp.com", "wundercorp.co")
    assert not base_url_host_matches("https://wundercorp.co.evil.io/v1", "wundercorp.co")
    assert not base_url_host_matches("https://proxy.example/wundercorp.com/v1", "wundercorp.co")
