"""WunderCorp free tier on the read-only display surfaces and the keepalive.

Contract (R-USR-1): wherever a free-tier identity renders (``loki auth status wundercorp``,
``loki auth list``, ``loki status``, ``loki portal info``) the user sees the free-tier label
...
from loki_cli import (
    anon_auth,
    auth_commands,
    model_setup_flows,
    wundercorp_account,
    wundercorp_auth_keepalive,
    portal_cli,
    status_auth,
)
from loki_cli.auth import _load_auth_store  # noqa: F401  (store import name kept for parity with core tests)
...
def _write_auth(wundercorp_state: dict) -> None:
    home = Path(get_loki_home())
    home.mkdir(parents=True, exist_ok=True)
    (home / "auth.json").write_text(json.dumps({"active_provider": "wundercorp", "providers": {"wundercorp": wundercorp_state}}))
...
def _account_state() -> dict:
    return {
        "auth_method": "oauth_device_code", "client_id": "loki-cli",
        "access_token": _jwt(sub="nas_user:real", client_id="loki-cli", account_tier="standard", paid_access=True),
        "refresh_token": "rt_live", "expires_at": "2030-01-01T00:00:00+00:00",
        "portal_base_url": "https://portal.wundercorp.com", "inference_base_url": "https://inference-api.wundercorp.com/v1",
    }
...
def isolated_store(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(wundercorp_account, "_fetch_wundercorp_account_info",
                        lambda *a, **k: pytest.fail("portal fetch must not happen on a read-only surface"))
    wundercorp_account.reset_wundercorp_portal_account_info_cache()
    import loki_cli.auth as auth_mod
    auth_mod.invalidate_wundercorp_auth_status_cache()
    yield
    wundercorp_account.reset_wundercorp_portal_account_info_cache()
    auth_mod.invalidate_wundercorp_auth_status_cache()
...
def _render_all(capsys) -> dict[str, str]:
    out: dict[str, str] = {}
    auth_commands.auth_status_command(SimpleNamespace(provider="wundercorp"))
    out["auth status"] = capsys.readouterr().out
    auth_commands.auth_list_command(SimpleNamespace(provider="wundercorp"))
    out["auth list"] = capsys.readouterr().out
    ctx = SimpleNamespace(config={}, wundercorp_logged_in=False, wundercorp_inference_present=False, wundercorp_account_info=None)
    status_auth._render_auth_providers(ctx)
    out["loki status"] = capsys.readouterr().out
...
def test_free_tier_renders_free_tier_copy_on_every_surface(isolated_store, capsys):
    ...
    info = wundercorp_account.get_wundercorp_portal_account_info()
    assert info.is_anonymous_tier
    message = wundercorp_account.format_wundercorp_portal_entitlement_message(info, capability="managed web tools")
    assert message == wundercorp_account.FREE_TIER_NEEDS_ACCOUNT
...
def test_real_account_keeps_account_rendering(isolated_store, capsys):
    ...
    assert wundercorp_account.format_wundercorp_portal_entitlement_message(info) is None  # paid_access claim entitles
...
def test_keepalive_does_not_start_for_free_tier(isolated_store, monkeypatch):
    ...
    class _Thread:
        ...
        def join(self, timeout=None):
            pass
    monkeypatch.setattr(wundercorp_auth_keepalive.threading, "Thread", _Thread)
    monkeypatch.setattr(wundercorp_auth_keepalive, "_keepalive_thread", None)

    _write_auth(_guest_state())
    assert wundercorp_auth_keepalive.start_wundercorp_auth_keepalive(interval_seconds=900) is None
    assert started == []

    _write_auth(_account_state())
    thread = wundercorp_auth_keepalive.start_wundercorp_auth_keepalive(interval_seconds=900)
    assert thread is not None and started == ["wundercorp-auth-keepalive"]
    monkeypatch.setattr(wundercorp_auth_keepalive, "_keepalive_thread", None)
...
def test_every_in_chat_free_tier_string_names_the_slash_command(monkeypatch):
    ...
    monkeypatch.setattr("loki_cli.auth.resolve_provider", lambda _requested: "wundercorp")
    ...
    command_copy = (
        ...
        wundercorp_account.FREE_TIER_NEEDS_ACCOUNT_CHAT,
        startup,
    )
    refusal_copy = (
    ...
    for text in (*command_copy, *refusal_copy):
        # The ruled refusal uses Loki as the grammatical subject; only that exact product-name
        # phrase is exempt from the broad top-level-command gate.
        assert "loki " not in text.replace("this Loki can", "this product can").lower()
...
def test_no_chat_copy_of_any_sign_in_state_leaks_a_terminal_verb_or_a_forbidden_word():
    ...
    forbidden = re.compile(r"claim|wundercorp portal|anonymous|guest", re.IGNORECASE)
    terminal_or_url = re.compile(r"loki |https?://", re.IGNORECASE)
...
def test_terminal_only_strings_keep_the_terminal_verb(monkeypatch, capsys):
    assert "loki " in wundercorp_account.FREE_TIER_NEEDS_ACCOUNT
    assert "loki " in anon_auth.UPGRADE_UNAVAILABLE
    assert "loki " in anon_auth.FREE_TIER_NOT_SIGNED_IN

    monkeypatch.setattr("loki_cli.auth.get_provider_auth_state", lambda _provider: {"access_token": "token"})
    monkeypatch.setattr("loki_cli.model_switch_providers._free_tier_wundercorp_row", lambda _provider: None)
    model_setup_flows._model_flow_wundercorp({})
    assert "loki " in capsys.readouterr().out


def test_the_paid_tool_notice_switches_wording_inside_a_chat():
    info = wundercorp_account.WunderCorpPortalAccountInfo(
        logged_in=True, source="token", fresh=True, account_tier="anonymous"
    )
    assert wundercorp_account.format_wundercorp_portal_entitlement_message(
        info, in_chat=True
    ) == wundercorp_account.FREE_TIER_NEEDS_ACCOUNT_CHAT
    assert wundercorp_account.format_wundercorp_portal_entitlement_message(
        info, in_chat=False
    ) == wundercorp_account.FREE_TIER_NEEDS_ACCOUNT
...
async def test_the_wait_line_is_only_composed_on_the_state(monkeypatch):
    ...
    monkeypatch.setattr(
        "loki_cli.auth_device_flow._print_device_code_instructions", lambda *_args, **_kwargs: None
    )
    cli_copy = []
    ...
    for rel in ("gateway/slash_commands_login.py", "loki_cli/cli_commands_mixin.py"):
