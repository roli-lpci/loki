...
from loki_cli.setup import setup_model_provider
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("loki_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("loki_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "loki_cli.main._model_flow_named_custom",
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "loki_cli.wundercorp_subscription.get_wundercorp_subscription_features",
        lambda config: type("Features", (), {"wundercorp_auth_present": True})(),
    )
    monkeypatch.setitem(
        sys.modules,
...
def test_vercel_setup_prefills_project_and_team_from_link_file(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("loki_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("loki_cli.setup.prompt", fake_prompt)
