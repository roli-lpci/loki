"""Redaction-safe forensic logging at the WunderCorp OAuth quarantine path.

A NAS-hosted Fly agent's WunderCorp bootstrap session can take a terminal
...
``_quarantine_wundercorp_oauth_state`` now emits a WARNING+ forensic record, and — the
...
from loki_cli.auth import AuthError, _quarantine_wundercorp_oauth_state


# A distinctive, obviously-fake refresh token so the redaction assertion is
# unambiguous if it ever leaks.
_FAKE_RT = "wundercorp_rt_LEAK_CANARY_do_not_log_raw_0123456789abcdef"
...
def _make_state(**overrides):
    state = {
        ...
        "access_token": "wundercorp_at_SECRET_access_token_material",
        "refresh_token": _FAKE_RT,
        "agent_key": "wundercorp_agent_key_SECRET_material",
...
def test_quarantine_emits_warning(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="loki_cli.auth"):
        _quarantine_wundercorp_oauth_state(state, _error(), reason="unit_test_quarantine")
...
def test_raw_refresh_token_never_logged(caplog):
    ...
    state = _make_state()
    with caplog.at_level(logging.DEBUG, logger="loki_cli.auth"):
        _quarantine_wundercorp_oauth_state(state, _error(), reason="unit_test_quarantine")
    ...
    assert "wundercorp_at_SECRET_access_token_material" not in text
    assert "wundercorp_agent_key_SECRET_material" not in text
