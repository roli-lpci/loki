...
``loki_cli.memory_setup._write_env_vars`` persists provider plugin
credentials to ``~/.loki/.env``. It previously called ``Path.write_text``
...
other subprocess-influencing or Loki-runtime-location name) could
otherwise plant a value into ``.env`` via the interactive memory-setup
wizard. The next Loki process would load it through the
...
from loki_cli.config import ensure_loki_home, get_env_path, load_env
from loki_cli.memory_setup import _write_env_vars


def _env_file_keys() -> set[str]:
    """Parse ``~/.loki/.env`` directly and return the set of keys present.
...
def _loki_home(tmp_path, monkeypatch):
...
def test_denylisted_key_does_not_block_other_writes(capsys):
    ...
    _write_env_vars({
        ...
        "LOKI_LANGFUSE_PUBLIC_KEY": "pk-test-123",
        "OPENROUTER_API_KEY": "sk-or-test-456",
    })

    assert "LD_PRELOAD" not in _env_file_keys()
    env = load_env()
    assert env["LOKI_LANGFUSE_PUBLIC_KEY"] == "pk-test-123"
    assert env["OPENROUTER_API_KEY"] == "sk-or-test-456"


def test_legitimate_loki_integration_key_still_writable():
    """``LOKI_*`` overall is NOT blocked — only the four runtime
    location names (HOME/PROFILE/CONFIG/ENV). Integration credentials
    following the ``LOKI_*`` convention (LOKI_LANGFUSE_*,
    LOKI_SPOTIFY_*, LOKI_QWEN_BASE_URL, ...) must keep working or
    the memory-setup wizard regresses for every plugin that follows
    the convention."""
    _write_env_vars({
        "LOKI_LANGFUSE_PUBLIC_KEY": "pk-lf-789",
        "LOKI_QWEN_BASE_URL": "https://example.com/v1",
    })

    env = load_env()
    assert env["LOKI_LANGFUSE_PUBLIC_KEY"] == "pk-lf-789"
    assert env["LOKI_QWEN_BASE_URL"] == "https://example.com/v1"
...
def test_explicit_loki_home_writes_to_that_env_file(tmp_path):
    """Plugin ``post_setup`` hooks (e.g. Supermemory) pass an explicit
    Loki home; keep that target while still routing through
    ``save_env_value`` validation instead of writing directly."""
    home = tmp_path / "plugin-home"

    _write_env_vars({"MEM0_API_KEY": "m0-test-key-abc"}, loki_home=home)
