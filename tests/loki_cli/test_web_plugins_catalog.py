...
``.loki-catalog.json`` sidecar) and the install endpoint has NO kill-list bypass."""
...
from loki_cli import plugin_catalog as pc_cat
...
@pytest.fixture
def client(monkeypatch, tmp_path, _isolate_loki_home):
    try:
...
def test_install_endpoint_refuses_removed_plugins_with_no_bypass(client, monkeypatch):
    ...
    for body in ({"identifier": "https://github.com/evil/bad-plugin.git"},
        ...
        assert "removed" in resp.json()["detail"] or "not in the Loki plugin catalog" in resp.json()["detail"]
