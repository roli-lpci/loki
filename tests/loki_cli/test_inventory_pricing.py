...
columns + Free/Pro badges and gate paid models on free WunderCorp accounts, the
same way the `loki model` CLI picker does.
"""

from threading import Event
from time import monotonic

import loki_cli.inventory as inv
...
def _patch_pricing(monkeypatch, *, free_tier, pricing, unavailable=None):
    ...
    monkeypatch.setattr(models_mod, "check_wundercorp_free_tier", lambda *, force_fresh=False: free_tier)
    monkeypatch.setattr(
...
def test_apply_pricing_free_models_get_flat_100_percent_sale(monkeypatch):
    ...
    rows = [{"slug": "wundercorp", "models": ["a/free", "b/natively-free"]}]
    inv._apply_pricing(rows)
...
def test_apply_pricing_omits_sale_when_original_not_cheaper(monkeypatch):
    ...
    rows = [{"slug": "wundercorp", "models": ["a/eq"]}]
    inv._apply_pricing(rows)
...
def test_cold_wundercorp_entitlement_keeps_models_unselectable(monkeypatch):
    ...
    monkeypatch.setattr(models_mod, "get_cached_wundercorp_free_tier", lambda: None)
    rows = [{"slug": "wundercorp", "models": ["free/model", "paid/model"]}]
    ...
    rows = [{"slug": "wundercorp", "models": ["m"], "warning": "paste WUNDERCORP_API_KEY to activate"}]
    inv._apply_pricing(rows, cached_only=True)
    assert rows[0]["warning"] == "paste WUNDERCORP_API_KEY to activate"
...
def test_prewarm_preserves_context_and_runs_once_per_profile(tmp_path, monkeypatch):
    ...
    def capture_context(_rows):
        ...
        observed[label] = (loki_home_key(), dict(scope))
        started[label].set()
    ...
    try:
        for label in ("a", "b"):
            home = tmp_path / label
            home_token = set_loki_home_override(str(home))
        ...
        assert observed["a"] == (
            loki_home_key(tmp_path / "a"),
            {"PROFILE_MARKER": "a"},
        )
        assert observed["b"] == (
            loki_home_key(tmp_path / "b"),
            {"PROFILE_MARKER": "b"},
        )
    finally:
        release.set()
...
def test_prewarm_endpoint_rotation_starts_a_new_worker(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        inv,
        "_apply_pricing",
        lambda _rows: models_pricing.get_pricing_for_provider("wundercorp"),
    )

    token = set_loki_home_override(str(tmp_path / "profile"))
    threads = []
    try:
        threads.append(
            inv._prewarm_pricing_async(
                [{"slug": "wundercorp", "models": ["a/model"]}],
        ...
        threads.append(
            inv._prewarm_pricing_async(
                [{"slug": "wundercorp", "models": ["b/model"]}],
                current_provider="wundercorp",
...
def test_prewarm_wundercorp_rotation_when_another_provider_is_current(tmp_path, monkeypatch):
    """WunderCorp endpoint identity must not depend on WunderCorp being selected."""
    from loki_constants import (
        reset_loki_home_override,
        set_loki_home_override,
    )
    ...
    monkeypatch.setattr(models_pricing, "_pricing_cache_retry_after", {})
    monkeypatch.setattr(models_pricing, "_pricing_provider_cache_keys", {})
    monkeypatch.setattr(models_pricing, "get_cached_wundercorp_inference_base_url",
        lambda: active_endpoint["value"],
    )
    monkeypatch.setattr(models_pricing, "_resolve_wundercorp_pricing_credentials",
        lambda: ("", active_endpoint["value"]),
    ...
    def in_profile(home, endpoint, *, cached_only):
        ...
        try:
            return models_pricing.get_pricing_for_provider(
                "wundercorp", cached_only=cached_only
            )
        finally:
            reset_loki_home_override(token)
