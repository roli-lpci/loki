...
from loki_cli.models_pricing import compute_sale_discount, fetch_models_with_pricing
...
def test_free_model_gets_flat_100_percent_discount():
    ...
    # "0.0000000000" strings (WunderCorp portal shape) count as free too.
...
def test_fetch_models_with_pricing_copies_nested_original(monkeypatch):
    ...
    # WunderCorp Portal opts in via include_sale_original=True.
...
def test_resolve_wundercorp_pricing_credentials_honors_inference_env_override(monkeypatch):
    """Staging profiles set WUNDERCORP_INFERENCE_BASE_URL — pricing must follow it.

    Without this, anonymous/failed-auth fallback hits prod and sale
    ``pricing.original`` never reaches Desktop/CLI pickers.
    """
    monkeypatch.setenv(
        "WUNDERCORP_INFERENCE_BASE_URL",
        "https://stg-inference-api.wundercorp.com/v1",
    )
    ...
    api_key, base_url = models_pricing._resolve_wundercorp_pricing_credentials()
    assert api_key == ""
    ...
    assert base_url == "https://stg-inference-api.wundercorp.com"


def test_resolve_wundercorp_pricing_credentials_normalizes_either_suffix(monkeypatch):
    """``/v1`` on the override is optional and must not change the result."""
    monkeypatch.setattr(
        "loki_cli.auth.resolve_wundercorp_runtime_credentials", lambda: None
    )
    for override in (
        "https://stg-inference-api.wundercorp.com",
        "https://stg-inference-api.wundercorp.com/",
        "https://stg-inference-api.wundercorp.com/v1",
        "https://stg-inference-api.wundercorp.com/v1/",
    ):
        monkeypatch.setenv("WUNDERCORP_INFERENCE_BASE_URL", override)
        assert models_pricing._resolve_wundercorp_pricing_credentials()[1] == (
            "https://stg-inference-api.wundercorp.com"
        )
