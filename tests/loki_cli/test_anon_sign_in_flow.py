...
Driven directly against the same fake account service ``loki auth upgrade`` is tested with, so the
...
from loki_cli.auth_constants import AuthError
from tests.loki_cli.test_anon_upgrade import (  # noqa: F401  (fixtures used by name)
...
def test_a_timeout_yields_timed_out_and_keeps_the_enriched_detail(portal, monkeypatch):
    ...
    from loki_cli import auth_device_flow
    enriched = auth_device_flow._wundercorp_device_auth_timeout_message(PORTAL)
...
def test_a_persist_failure_yields_failed_rather_than_raising(portal, free_account, monkeypatch):
    ...
    monkeypatch.setattr(
        auth_wundercorp, "persist_wundercorp_credentials", lambda *a, **kw: (_ for _ in ()).throw(OSError("read-only home")))
...
def test_a_settle_failure_yields_failed_rather_than_raising(portal, free_account, monkeypatch):
    ...
    persists = []
    from loki_cli import auth_wundercorp
    real_persist = auth_wundercorp.persist_wundercorp_credentials
    monkeypatch.setattr(
        auth_wundercorp, "persist_wundercorp_credentials",
...
def test_already_signed_in_short_circuits_before_any_network(portal):
    from loki_cli.auth import _load_auth_store, _save_auth_store, _save_provider_state
    store = _load_auth_store()
    _save_provider_state(store, "wundercorp", {"auth_method": "oauth_device_code", "access_token": "x"})
