"""Tests for loki_cli.cron command handling."""
...
from loki_cli import cron as cron_cli
...
class TestCronCommandLifecycle:
    ...
    def test_edit_persists_user_owned_inference_pins(self, tmp_cron_dir, capsys):
        ...
        assert updated["provider"] == "wundercorp"
...
class TestUnverifiedDeliveryVisibility:
    ...
    state must be visible in ``loki cron list`` and ``loki cron doctor``,
...
class TestCronListStatusRendering:
    ...
    def test_delivery_failed_is_not_green_ok(self, tmp_cron_dir, capsys, monkeypatch):
        monkeypatch.setattr("loki_cli.gateway.find_gateway_pids", lambda: [1])
        # capsys is not a tty, so force colors on to check the paint itself.
        monkeypatch.setattr("loki_cli.colors.should_use_color", lambda: True)
        create_job(prompt="Daily digest", schedule="every 1h")
        jobs = load_jobs()
...
class TestGatewayNotRunningWarning:
    ...
    def test_list_warns_when_gateway_absent(self, tmp_cron_dir, capsys, monkeypatch):
        ...
        monkeypatch.setattr("loki_cli.gateway.find_gateway_pids", lambda: [])
        cron_command(Namespace(cron_command="list", all=True))
        out = capsys.readouterr().out
...
class TestExternalCronProviderStatus:
    ...
    def test_status_reports_provider_not_ticker_for_chronos(
        ...
        monkeypatch.setattr(
            "loki_cli.cron._active_cron_provider_name", lambda: "chronos"
        )
...
class TestCronRunBackgroundDispatch:
    """`loki cron run` must not report 'failed' when the run was dispatched
    ...
    instead, and leave synchrowundercorp runs unchanged.
    """
    ...
    def test_sync_run_failure_still_reported(self, monkeypatch, capsys):
        # A genuine synchrowundercorp failure must keep reporting 'failed' — only
