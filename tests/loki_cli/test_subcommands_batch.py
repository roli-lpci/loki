...
from loki_cli.subcommands.dashboard import build_dashboard_parser
...
from loki_cli.subcommands.whatsapp import build_whatsapp_parser
...
def test_config_get_unset_subcommands_parse():
    """`loki config get/unset` parse key args (and --json for get)."""
...
# ── deprecated `loki login` fails gracefully, not with argparse error ────
#
# `loki login` is a removed command; its handler (`login_command` in
# `loki_cli/auth.py`) prints a deprecation notice pointing at `loki auth` /
# `loki model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `loki --help`.
