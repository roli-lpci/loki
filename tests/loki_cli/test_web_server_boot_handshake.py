...
Simulates a slow loki_cli.gateway import (15-30 s on a fresh Windows install
...
1. _lifespan synchrowundercorp warmup: patched _warm_gateway_module sleeps N seconds;
...
import loki_cli.web_server as web_server_mod
...
# ---------------------------------------------------------------------------
# Test 1 — _lifespan synchrowundercorp warmup blocks startup (intentional)
# ---------------------------------------------------------------------------

def test_lifespan_warmup_is_synchrowundercorp():
    """
    _warm_gateway_module runs synchrowundercorply before the lifespan yield (#73083).
    ...
    # Startup must block for at least SLOW_SECONDS (the import runs synchrowundercorply).
    ...
    assert startup_ms >= threshold_ms, (
        ...
        f"warmup is not synchrowundercorp."
    )
