...
import loki_cli.doctor as doctor
import loki_constants
from loki_cli import config as config_mod
import loki_cli.gateway as gateway_cli
from loki_cli import doctor as doctor_mod
from loki_cli.doctor_config import _has_provider_env_config
import shutil
from loki_cli import doctor_tools
from loki_cli import doctor_state
from loki_cli import doctor_platform
from loki_cli import doctor_config
...
class TestDoctorPlatformHints:
    def test_termux_package_hint(self, monkeypatch):
        ...
        assert loki_constants.is_termux() is True
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `loki doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "LOKI_HOME", loki_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_marks_kanban_available_only_when_missing_worker_env_gate(self, monkeypatch):
        ...
        monkeypatch.delenv("LOKI_KANBAN_TASK", raising=False)
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("LOKI_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_loki_home(self, tmp_path, provider="", memory_config=None):
        """Create a minimal LOKI_HOME with config.yaml."""
        home = tmp_path / ".loki"
    ...
    def _run_doctor_and_capture(
        ...
        *,
        ...
        home = self._make_loki_home(tmp_path, provider, memory_config)
        ...
        monkeypatch.setattr(doctor_mod, "LOKI_HOME", home)
        ...
        try:
            from loki_cli import auth as _auth_mod
            monkeypatch.setattr(_auth_mod, "get_wundercorp_auth_status_local", lambda: {})
...
def test_run_doctor_accepts_loki_provider_ids_that_catalog_aliases(
...
def _doctor_env_for_agent_browser(monkeypatch, tmp_path):
    ...
    branch in run_doctor (loki_cli/doctor.py ~1557-1605)."""
    home = tmp_path / ".loki"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
    project = tmp_path / "project"
    ...
    try:
        from loki_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_wundercorp_auth_status", lambda: {})
        monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
        monkeypatch.setattr(_auth_mod, "get_xai_oauth_auth_status", lambda: {})
    except Exception:
        pass
...
def test_run_doctor_fix_warms_npx_cache_when_agent_browser_resolves_via_npx(
    ...
    """`loki doctor --fix` must actually call warm_agent_browser_npx_cache()
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp LOKI_HOME.

        ``run_doctor`` reads the module-level ``LOKI_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("LOKI_HOME")`` alone
        leaves doctor probing the REAL ~/.loki. On a dev machine with a
    ...
    def test_no_token_and_not_gh_authenticated_shows_warn(self, monkeypatch, tmp_path):
        ...
        from loki_cli.doctor import run_doctor
        import io, contextlib
...
def _run_doctor_with_healthy_oauth_fallback(
    ...
    *,
    ...
    monkeypatch.setattr(_auth_mod, "get_wundercorp_auth_status_local", lambda: {"logged_in": True})
    monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from loki_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("loki_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "loki_cli.doctor", raising=False)
        from loki_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorXaiOAuthStatus:
    ...
    failure (or runtime exception) cannot silence the WunderCorp / Codex / Gemini /
    ...
    def _run(self, monkeypatch, tmp_path, *, xai_auth_fn) -> str:
        ...
        from loki_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_wundercorp_auth_status_local", lambda: {"logged_in": False})
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        """WunderCorp / Codex / Gemini / MiniMax rows must survive an xAI import failure."""
        home = tmp_path / ".loki"
        home.mkdir(parents=True, exist_ok=True)
        (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
        project = tmp_path / "project"
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        assert "WunderCorp Portal auth" in out
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale LOKI_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"LOKI_MAX_ITERATIONS={ghost}\n")
        (loki_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "LOKI_HOME", loki_home)
        monkeypatch.setattr(doctor_mod, "get_loki_home", lambda: loki_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("LOKI_MAX_ITERATIONS", str(os_environ_value))
        else:
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale LOKI_MAX_ITERATIONS" in out
        env_after = (loki_home / ".env").read_text(encoding="utf-8")
        assert "LOKI_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_loki_tool_progress_warning_says_unsupported_since_floor(self):
        """LOKI_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_config.collect_deprecated_env_vars({"LOKI_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["LOKI_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_config.collect_deprecated_env_vars({"LOKI_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["LOKI_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"

    def _run_doctor_with_config(self, monkeypatch, tmp_path, *, config_yaml: str, env_text: str = ""):
        loki_home = tmp_path / ".loki"
        loki_home.mkdir(parents=True)
        (loki_home / "config.yaml").write_text(config_yaml, encoding="utf-8")
        env_body = env_text if env_text else "OPENAI_API_KEY=sk-test\n"
        (loki_home / ".env").write_text(env_body, encoding="utf-8")
    ...
    def test_report_does_not_count_as_blocking_issue(self, monkeypatch, tmp_path, capsys):
        ...
        findings = doctor_config.report_deprecated_config_and_env(
            ...
            {"LOKI_TOOL_PROGRESS_MODE": "verbose"},
        )
        out = capsys.readouterr().out
        assert len(findings) == 2
        assert "Deprecated: delegation.max_async_children" in out
        assert "Deprecated: LOKI_TOOL_PROGRESS_MODE" in out
...
class TestMacOSTCCGrants:
    ...
    def test_warns_on_cdhash_pinned_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.wundercorp.lokiagent" and cdhash H"97e692f3890f781fa0ad5ad6cb9d769cfaf42628"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        assert "TCC grants will reset after every update" in out
        assert "cdhash-pinned" in out
        assert "loki update" in out
    ...
    def test_ok_and_repair_info_on_identifier_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.wundercorp.lokiagent"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        ...
        assert "tccutil reset ScreenCapture com.wundercorp.lokiagent" in out
        assert "toggle" in out
        assert "relaunch" in out

    def test_ok_on_certificate_anchored_dr(self, monkeypatch, capsys, tmp_path):
        """A cert-anchored DR (loki desktop --setup-tcc-identity, or a
        ...
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.wundercorp.lokiagent" and certificate root = H"aabbcc"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        assert "TCC signing identity is stable" in out
