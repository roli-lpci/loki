"""Tests for `loki curator run` CLI behavior."""
...
def test_run_defaults_to_synchrowundercorp(monkeypatch, capsys):
    import agent.curator as curator_state
    import loki_cli.curator as curator_cli
    ...
    assert calls[0]["synchrowundercorp"] is True
    assert calls[0]["dry_run"] is False
    assert "background" not in capsys.readouterr().out


def test_dry_run_default_reports_synchrowundercorp_wording(monkeypatch, capsys):
    import agent.curator as curator_state
    import loki_cli.curator as curator_cli
    ...
    assert "Read the report with `loki curator status`" in out
