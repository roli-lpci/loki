"""Tests for loki_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("loki_cli.skills_config.save_config")
...
class TestIsSkillDisabled:
    ...
    @patch("loki_cli.config.load_config")
    @patch.dict("os.environ", {"LOKI_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """LOKI_SESSION_PLATFORM should be used when LOKI_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_loki_platform_takes_precedence(self, tmp_path, monkeypatch):
        """LOKI_PLATFORM should win over LOKI_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from loki_cli.skills_config import _get_categories
        skills = [
