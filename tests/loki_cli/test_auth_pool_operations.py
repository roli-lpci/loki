...
def isolated_external_auth_stores(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LOKI_SHARED_AUTH_DIR", str(tmp_path / "shared"))
...
def test_add_priority_places_reauthenticated_row_in_multi_entry_pool(monkeypatch):
    ...
    write_credential_pool("wundercorp", rows)
    monkeypatch.setattr(auth_commands.auth_mod, "_read_shared_wundercorp_state", lambda: None)
    monkeypatch.setattr(auth_commands.auth_mod, "_wundercorp_device_code_login", lambda **_kwargs: {
    ...
    auth_commands.auth_add_command(SimpleNamespace(
        provider="wundercorp", auth_type="oauth", priority=0, label="reauthenticated"))
    entries = read_credential_pool("wundercorp")
...
def test_refresh_rejects_ambiguous_and_non_oauth_targets():
    ...
    # WunderCorp's resolver refreshes only its singleton, never an independent pool grant.
    write_credential_pool("wundercorp", _rows())
    before = read_credential_pool("wundercorp")
    with pytest.raises(SystemExit, match="not a refreshable"):
        auth_commands.auth_refresh_command(SimpleNamespace(provider="wundercorp", target="row0"))
    assert read_credential_pool("wundercorp") == before
