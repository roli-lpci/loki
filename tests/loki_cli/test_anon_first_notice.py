"""CLI one-time notice: an explicit-provider install learns once that the WunderCorp free tier exists."""
...
def test_signed_in_account_is_never_told_about_the_free_tier():
    with _auth_store_lock():
        store = _load_auth_store()
        store.setdefault("providers", {})["wundercorp"] = {
            "auth_method": "oauth", "access_token": _jwt(client_id="loki-cli", account_tier="pro"),
    ...
    assert anon_auth.GUEST_NOTICE_FLAG not in _load_auth_store()["providers"]["wundercorp"]
