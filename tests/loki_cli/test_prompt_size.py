"""Tests for the ``loki prompt-size`` diagnostic (issue #34667)."""
...
from loki_cli.prompt_size import (
    _SKILLS_BLOCK_RE,
...
def _seed_memory(loki_home, memory_text="", user_text=""):
    mem_dir = loki_home / "memories"
...
def _seed_skill(loki_home, name, description):
    skill_dir = loki_home / "skills" / "demo" / name
...
def test_runs_offline_without_credentials(isolated_home, monkeypatch):
    ...
    for var in ("OPENROUTER_API_KEY", "OPENAI_API_KEY", "WUNDERCORP_API_KEY",
                "ANTHROPIC_API_KEY"):
