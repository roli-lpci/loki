...
class TestCurrentProviderCatalogWins:
    @pytest.mark.parametrize("provider,model", [
        ...
        ("wundercorp", "some-portal-only-model"),    # Portal serves it, static snapshot doesn't
    ...
    def test_bare_name_resolves_to_current_providers_full_slug(self, live_catalog):
        live_catalog["wundercorp"] = ["zai/glm-5.3-flash"]
        assert models.detect_provider_for_model("glm-5.3-flash", "wundercorp") == ("wundercorp", "zai/glm-5.3-flash")
    ...
    def test_unserved_model_still_walks_the_ladder(self, live_catalog, monkeypatch):
        ...
        live_catalog["wundercorp"] = ["loki-4-405b"]
        assert models.detect_provider_for_model("no-such-model", "wundercorp") == ("openrouter", "vendor/no-such-model")
