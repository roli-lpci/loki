"""Regression tests for the WunderCorp Portal env-override bypassing the host
allowlist, mirroring the existing WUNDERCORP_INFERENCE_BASE_URL /
_ALLOWED_WUNDERCORP_INFERENCE_HOSTS treatment.

Real incident (2026-07): a hosted agent provisioned by wundercorp-account-service
on the `staging` Vercel environment is stamped with
``LOKI_PORTAL_BASE_URL=https://portal.staging-wundercorp.com`` in its
...
Before this fix, ``resolve_wundercorp_access_token`` / ``resolve_wundercorp_runtime_
...
``_WUNDERCORP_PORTAL_ALLOWED_HOSTS``, which only recognised the production host.
...
``_quarantine_wundercorp_oauth_state`` and wiped the entire credential pool.

The correct fix (mirroring ``_wundercorp_inference_env_override()``): the env
...
from loki_cli.auth import (
    ...
    _WUNDERCORP_PORTAL_ALLOWED_HOSTS,
    _wundercorp_portal_env_override,
)


class TestPortalEnvOverrideHelper:
    def test_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("LOKI_PORTAL_BASE_URL", raising=False)
        monkeypatch.delenv("WUNDERCORP_PORTAL_BASE_URL", raising=False)
        assert _wundercorp_portal_env_override() is None


    def test_env_override_not_gated_by_allowlist(self, monkeypatch):
        """The whole point: an env-set staging host is NOT in
        _WUNDERCORP_PORTAL_ALLOWED_HOSTS, and the helper must return it anyway —
        gating happens only for network-provenance values."""
        monkeypatch.setenv(
            "LOKI_PORTAL_BASE_URL", "https://portal.staging-wundercorp.com"
        )
        assert "portal.staging-wundercorp.com" not in _WUNDERCORP_PORTAL_ALLOWED_HOSTS
        assert (
            _wundercorp_portal_env_override() == "https://portal.staging-wundercorp.com"
        )


class TestResolveAccessTokenEnvOverrideWins:
    """End-to-end: resolve_wundercorp_access_token must use the env override for
    ...
    def _run_and_capture(self, monkeypatch, auth):
        ...
        monkeypatch.setattr(auth_wundercorp, "_refresh_access_token", _fake_refresh)
        ...
        try:
            auth.resolve_wundercorp_access_token()
        finally:
    ...
    def test_env_override_wins_even_with_staging_state_stored(
        ...
        a prior LOKI_AUTH_JSON_BOOTSTRAP seed), and the env var is set to
        ...
        staging_portal = "https://portal.staging-wundercorp.com"
        monkeypatch.setenv("LOKI_HOME", str(tmp_path))
        monkeypatch.setenv("LOKI_PORTAL_BASE_URL", staging_portal)
    ...
    def test_no_env_no_staging_state_prod_url_used_unmodified(
        ...
        monkeypatch.delenv("LOKI_PORTAL_BASE_URL", raising=False)
        monkeypatch.delenv("WUNDERCORP_PORTAL_BASE_URL", raising=False)
        self._write_auth_file(tmp_path, stored_portal_url=DEFAULT_WUNDERCORP_PORTAL_URL)

        seen_portal_urls, records = self._run_and_capture(monkeypatch, auth)

        assert seen_portal_urls == [DEFAULT_WUNDERCORP_PORTAL_URL]
