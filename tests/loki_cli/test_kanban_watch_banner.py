...
from loki_cli import kanban_ops
from loki_cli.kanban import kanban_command
from loki_cli.kanban_parser import build_parser
...
def test_watch_names_resolved_board(tmp_path, monkeypatch, capsys, environment, explicit, expected):
    ...
    monkeypatch.setenv("LOKI_KANBAN_HOME", str(tmp_path))
    ...
    if environment:
        monkeypatch.setenv("LOKI_KANBAN_BOARD", environment)
