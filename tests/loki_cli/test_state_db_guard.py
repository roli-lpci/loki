...
from loki_cli.backup import verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    LOKI_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def loki_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("loki_cli.config") or mod == "loki_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, loki_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, loki_home, capsys, monkeypatch):
