...
def test_startup_route_uses_configured_wundercorp_provider(monkeypatch):
    monkeypatch.setattr(model_switch, "DIRECT_ALIASES", {})
    route = model_switch.resolve_startup_model_route(
        "wundercorp/deepseek-v4-pro",
        user_providers={"wundercorp": {"base_url": "https://inference.example/v1"}},
    )
    assert route == model_switch.StartupModelRoute("deepseek-v4-pro", "wundercorp", "")
...
def test_startup_route_aggregator_native_slug_stays_on_aggregator(monkeypatch):
    ...
    monkeypatch.setattr(model_switch, "DIRECT_ALIASES", {})
    monkeypatch.setattr(
        "loki_cli.models._find_openrouter_slug",
