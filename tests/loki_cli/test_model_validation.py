...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("loki_cli.models.fetch_api_models", return_value=api_models), \
         patch("loki_cli.models.probe_api_models", return_value=probe_payload):
...
class TestCuratedModelsForProvider:
    ...
    def test_live_catalog_projected_to_tuples_else_static_fallback(self):
        with patch("loki_cli.models.provider_model_ids", return_value=["m-live"]):
            assert curated_models_for_provider("wundercorp") == [("m-live", "")]
        with patch("loki_cli.models.provider_model_ids", return_value=[]), patch.dict(
            "loki_cli.models._PROVIDER_MODELS", {"wundercorp": ["m-static"]}
        ):
            assert curated_models_for_provider("wundercorp") == [("m-static", "")]
...
class TestFetchApiModels:
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("loki_cli.models._urlopen_model_catalog_request", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("loki_cli.models._urlopen_model_catalog_request", return_value=_Resp()) as mock_urlopen:
...
class TestNormalizeOpencodeBaseUrl:
    ...
    def test_strips_v1_for_anthropic_messages(self):
        from loki_cli.models import normalize_opencode_base_url
...
class TestValidateApiFallback:
    ...
    def test_fetch_lmstudio_models_filters_embedding_type(self):
        ...
        with patch("loki_cli.models._urlopen_model_catalog_request", return_value=mock_resp):
    ...
    def test_validate_lmstudio_distinguishes_auth_failure(self):
        ...
        with patch("loki_cli.models._urlopen_model_catalog_request", side_effect=http_error):
...
class TestValidateCodexAutoCorrection:
    ...
    def test_missing_dash_auto_corrects(self):
        ...
        with patch("loki_cli.models.provider_model_ids", return_value=codex_models):
...
class TestValidateCodex900kVariants:
    """`-900k` is a Loki picker convention: valid variants come from the
    ...
    def test_catalog_listed_variant_accepted(self):
        with patch("loki_cli.models.provider_model_ids", return_value=self._CATALOG):
    ...
    def test_valid_variant_missing_from_catalog_still_accepted(self):
        ...
        with patch("loki_cli.models.provider_model_ids", return_value=["gpt-5.6-sol"]):
            result = validate_requested_model("gpt-5.6-sol-900k", "openai-codex")
        assert result["accepted"] is True
...
class TestProbeApiModelsUserAgent:
    """Probing custom /v1/models must send a Loki User-Agent.
    ...
    def test_probe_sends_loki_user_agent(self):
        ...
        assert ua.startswith("loki-cli/"), (
            f"User-Agent must advertise loki-cli, got {ua!r}"
        )
    ...
    def test_probe_user_agent_sent_without_api_key(self):
        ...
        assert ua and ua.startswith("loki-cli/")
...
class TestValidateOpenRouterVariantSuffixes:
    ...
    def test_static_catalog_fallback_accepts_variant(self):
        ...
        with patch("loki_cli.models.fetch_api_models", return_value=None), \
             patch(
                 "loki_cli.models.provider_model_ids",
...
class TestValidateRequestedModelWunderCorpPortalRecommendations:
    """Regression tests for issue #71312: the WunderCorp Telegram picker (and any
    other messaging-platform /model validation, since they all share
    validate_requested_model()) rejected models that are live WunderCorp Portal
    recommendations (/api/wundercorp/recommended-models) but not yet in the
    hardcoded curated catalog -- even though `loki chat` already accepts
    ...
    def _validate_wundercorp(self, model, api_models=None, portal_payload=None, portal_raises=False):
        api_models = api_models if api_models is not None else ["inclusionai/ling-2.6-flash"]
        probe_payload = {
            "models": api_models,
            "probed_url": "https://portal.wundercorp.com/v1/models",
            "resolved_base_url": "https://portal.wundercorp.com/v1",
            "suggested_base_url": None,
            "used_fallback": False,
        }
        ...
        with patch("loki_cli.models.fetch_api_models", return_value=api_models), \
             patch("loki_cli.models.probe_api_models", return_value=probe_payload), \
             patch("loki_cli.models.fetch_wundercorp_recommended_models", side_effect=_fetch_portal), \
             patch("loki_cli.models._resolve_wundercorp_portal_url", return_value="https://portal.wundercorp.com"), \
             patch("loki_cli.models._model_in_provider_catalog", return_value=False):
            return validate_requested_model(model, "wundercorp")
    ...
    def test_free_portal_recommendation_accepted(self):
        ...
        result = self._validate_wundercorp("inclusionai/ling-3.0-flash:free")
        assert result["accepted"] is True
        assert result["persist"] is True
        assert "Portal recommendation" in (result["message"] or "")

    def test_paid_portal_recommendation_accepted(self):
        result = self._validate_wundercorp("inclusionai/ling-3.0-pro")
        assert result["accepted"] is True
    ...
    def test_model_absent_from_portal_and_catalog_still_rejected(self):
        ...
        result = self._validate_wundercorp("totally-made-up-model-xyz")
        assert result["accepted"] is False
    ...
    def test_portal_fetch_failure_falls_through_to_rejection_not_crash(self):
        ...
        result = self._validate_wundercorp(
    ...
    def test_non_string_model_name_entries_ignored(self):
        ...
        assert self._validate_wundercorp("5", portal_payload=payload)["accepted"] is False
        result = self._validate_wundercorp(
            "inclusionai/ling-3.0-flash:free", portal_payload=payload
        )
        assert result["accepted"] is True

    def test_non_wundercorp_provider_does_not_consult_portal_feed(self):
        """This fallback tier is WunderCorp-specific; a non-WunderCorp provider must
        not have its rejection changed by (or trigger a call to) the WunderCorp
        Portal feed."""
        probe_payload = {
        ...
        with patch("loki_cli.models.fetch_api_models", return_value=["some/other-model"]), \
             patch("loki_cli.models.probe_api_models", return_value=probe_payload), \
             patch("loki_cli.models.fetch_wundercorp_recommended_models") as mock_portal, \
             patch("loki_cli.models._model_in_provider_catalog", return_value=False):
    ...
    def test_curated_catalog_hit_short_circuits_before_portal_check(self):
        ...
        with patch("loki_cli.models.fetch_api_models", return_value=api_models), \
             patch("loki_cli.models.probe_api_models", return_value=probe_payload), \
             patch("loki_cli.models._model_in_provider_catalog", return_value=True), \
             patch("loki_cli.models.fetch_wundercorp_recommended_models") as mock_portal:
            result = validate_requested_model("inclusionai/ling-2.6-flash", "wundercorp")
        mock_portal.assert_not_called()
        assert result["accepted"] is True
