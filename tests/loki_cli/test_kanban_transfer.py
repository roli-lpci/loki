"""Tests for kanban board export / import (``loki_cli.kanban_transfer``).
...
from loki_cli import kanban_transfer as kt
from loki_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("LOKI_KANBAN_HOME", str(root))
        for var in ("LOKI_KANBAN_DB", "LOKI_KANBAN_WORKSPACES_ROOT",
                    "LOKI_KANBAN_ATTACHMENTS_ROOT", "LOKI_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Loki"):
