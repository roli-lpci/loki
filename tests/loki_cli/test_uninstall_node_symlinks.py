"""Tests for loki_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $LOKI_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Loki-managed node dir.
"""
...
def _make_loki_node(loki_home: Path) -> Path:
    """Create a fake $LOKI_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = loki_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    loki_home = fake_home / ".loki"
    _make_loki_node(loki_home)
    ...
    removed = uninstall.remove_node_symlinks(loki_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    loki_home = fake_home / ".loki"
    node_bin = _make_loki_node(loki_home)
