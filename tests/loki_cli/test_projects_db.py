"""Tests for the per-profile Projects store (loki_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Loki Agent", folders=["/tmp/loki"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "loki-agent"
    assert proj.name == "Loki Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/loki"
    assert [f.path for f in proj.folders] == ["/tmp/loki"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "loki-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' LOKI_HOME.
