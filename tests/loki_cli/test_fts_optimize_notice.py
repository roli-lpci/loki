...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            loki_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(loki_constants, "get_loki_home", lambda: tmp_path)
    monkeypatch.setattr(loki_state, "SessionDB", FakeSessionDB)
    ...
    assert "loki sessions optimize-storage" in capsys.readouterr().out
    conn.close()
