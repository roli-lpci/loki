...
calls ``os._exit(0)`` ~5ms after ``LOKI_BACKEND_READY``, which no in-process
...
def test_live_backend_survives_timezone_drifted_parent_marker(tmp_path):
    ...
    try:
        ...
        env.update(
            ...
            PYTHONUNBUFFERED="1",
            LOKI_PARENT_PID=str(parent.pid),
            LOKI_PARENT_START_MARKER=drifted_marker,
            LOKI_PARENT_NONCE="nonce-95693",
            LOKI_SERVE_WATCHDOG_POLL_S="0.5",
        )
        env.pop("LOKI_DESKTOP", None)
        code = (
        ...
        assert _read_until(serve, "LOKI_BACKEND_READY"), "backend never announced READY"
