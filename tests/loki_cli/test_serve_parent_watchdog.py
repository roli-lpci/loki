"""Regression tests for Desktop-owned ``loki serve`` lifecycle tracking."""

from loki_cli.web_server_lifecycle import (
    _is_serve_orphaned,
...
def test_parent_watchdog_treats_empty_marker_env_as_absent(monkeypatch):
    """Blank inherited LOKI_PARENT_START_MARKER/NONCE degrade to PID-only tracking."""
    from loki_cli import web_server_lifecycle

    monkeypatch.setenv("LOKI_PARENT_PID", "4242")
    monkeypatch.setenv("LOKI_PARENT_START_MARKER", "")
    monkeypatch.setenv("LOKI_PARENT_NONCE", "")
    seen = {}
...
def test_parent_watchdog_warns_when_disarmed_by_unusable_marker(monkeypatch, caplog):
    ...
    monkeypatch.setenv("LOKI_PARENT_PID", "4242")
    monkeypatch.setenv("LOKI_PARENT_START_MARKER", "ps:Sat")
    monkeypatch.setenv("LOKI_PARENT_NONCE", "n")
    started = []
    monkeypatch.setattr(
        web_server_lifecycle.threading, "Thread", lambda *a, **k: started.append(1) or _NoThread()
    )
    with caplog.at_level(logging.WARNING, logger="loki_cli.web_server"):
        web_server_lifecycle._start_parent_death_watchdog()
