"""Tests for loki_cli configuration management."""
    ...
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            if sys.platform == "win32":
                # Windows default is %LOCALAPPDATA%\loki — see
                # loki_constants._get_platform_default_loki_home.
                ...
                assert home == base / "loki"
            else:
                assert home == Path.home() / ".loki"


class TestEnsureLokiHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from loki_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``loki logs`` surfaces it)
...
class TestSaveAndLoadRoundtrip:
    ...
    def test_atomic_config_write_refuses_unparseable_existing_config(self, tmp_path):
        ...
        from loki_cli.config import atomic_config_write
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"LOKI_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"LOKI_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"LOKI_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"LOKI_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("loki_cli.config.sanitize_env_file", return_value=2),
            patch(
                "loki_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("loki_cli.config.read_raw_config", return_value={}),
            patch("loki_cli.config.get_missing_env_vars", return_value=[]),
            patch("loki_cli.config.get_missing_config_fields", return_value=[]),
            patch("loki_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from loki_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """LOKI_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `loki setup`) lets a
        ...
        via config.yaml; LOKI_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from loki_cli.config import OPTIONAL_ENV_VARS
        assert "LOKI_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `loki setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `loki setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_loki_home(self):
        from loki_cli.config_migrations import support_floor_message
        from loki_constants import display_loki_home

        msg = support_floor_message()
        assert f"{display_loki_home()}/config.yaml" in msg
    ...
    _V20_FIXTURE = {
        "_config_version": 20,
        "model": {"default": "anthropic/claude-fable-5", "provider": "wundercorp"},
...
class TestCustomProviderCompatibility:
    ...
    def _run_ladder(current_ver: int):
        from loki_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or selected Loki runtime/security controls.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Loki
    process on next startup via the dotenv → ``os.environ`` chain in
    ``loki_cli/env_loader.py``.
    ...
    def _loki_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        "allowed_key",
        [
            "LOKI_LANGFUSE_PUBLIC_KEY",
            "LOKI_SPOTIFY_CLIENT_ID",
            "LOKI_QWEN_BASE_URL",
            "LOKI_MAX_ITERATIONS",
        ],
    )
    def test_loki_integration_keys_still_writable(self, allowed_key):
        """``LOKI_*`` overall is NOT blocked.
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "LOKI_ENV_PATH",
            "LOKI_OPTIONAL_MCPS",
            "LOKI_COPILOT_ACP_COMMAND",
            "LOKI_COPILOT_ACP_ARGS",
            "LOKI_YOLO_MODE",
            "LOKI_ACCEPT_HOOKS",
            "LOKI_REDACT_SECRETS",
            "LOKI_INTERACTIVE",
            ...
            "LOKI_CRON_SESSION",
            "LOKI_SINGLE_QUERY_SESSION",
            "LOKI_SESSION_KEY",
            "LOKI_SESSION_PLATFORM",
        ],
    )
    def test_loki_security_control_keys_are_not_writable(self, protected_key):
    ...
    def test_preexisting_optional_mcps_override_still_loads(self, tmp_path):
        ...
        from loki_cli.config import invalidate_env_cache

        catalog = tmp_path / "custom-mcp-catalog"
        (tmp_path / ".env").write_text(
            f"LOKI_OPTIONAL_MCPS={catalog}\n",
            encoding="utf-8",
        )
        invalidate_env_cache()

        assert load_env()["LOKI_OPTIONAL_MCPS"] == str(catalog)

    @pytest.mark.parametrize(
        ("key", "expected"),
        [
            ("Path", "PATH"),
            ("Loki_Yolo_Mode", "LOKI_YOLO_MODE"),
            ("Loki_Optional_Mcps", "LOKI_OPTIONAL_MCPS"),
            ("Loki_Copilot_Acp_Command", "LOKI_COPILOT_ACP_COMMAND"),
            ("Loki_Copilot_Acp_Args", "LOKI_COPILOT_ACP_ARGS"),
        ],
    )
    def test_windows_policy_names_are_case_insensitive(self, key, expected):
        from loki_cli.config import _env_var_policy_name
    ...
    def test_windows_env_assignment_matching_is_case_insensitive(self, prefix):
        from loki_cli.config import _env_line_defines_key
...
"""
    ...
    def test_persist_migration_writes_full_read_raw_config(self, tmp_path):
        from loki_cli.config import _persist_migration, read_raw_config
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"LOKI_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: loki\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "loki"
...
def test_default_config_has_no_duplicate_top_level_keys():
    ...
    import ast
    import loki_cli.config as cfg_mod
...
class TestConfigCommandFailClosedSurface:
    """`loki config set/unset` must exit cleanly (no traceback) when the
