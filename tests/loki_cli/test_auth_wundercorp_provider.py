"""Regression tests for WunderCorp OAuth refresh and inference JWT interactions."""
...
from loki_cli.auth import AuthError, get_provider_auth_state, resolve_wundercorp_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        import ssl
        from loki_cli.auth import _resolve_verify
...
def _setup_wundercorp_auth(
    ...
    *,
    ...
    (loki_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_wundercorp_runtime_credentials_prefers_invoke_jwt_and_mirrors(
    tmp_path,
    monkeypatch,
):
    import loki_cli.auth as auth_mod

    loki_home = tmp_path / "loki"
    token = _invoke_jwt(seconds=3600)
    _setup_wundercorp_auth(
        loki_home,
        access_token=token,
        scope=auth_mod.DEFAULT_WUNDERCORP_SCOPE,
        expires_at=_future_iso(3600),
        expires_in=3600,
    )
    monkeypatch.setenv("LOKI_HOME", str(loki_home))

    creds = auth_mod.resolve_wundercorp_runtime_credentials()

    assert creds["api_key"] == token
    assert creds["source"] == auth_mod.WUNDERCORP_AUTH_PATH_INVOKE_JWT
    assert creds["auth_path"] == auth_mod.WUNDERCORP_AUTH_PATH_INVOKE_JWT

    payload = json.loads((loki_home / "auth.json").read_text())
    singleton = payload["providers"]["wundercorp"]
    assert singleton["agent_key"] == token
    assert datetime.fromisoformat(singleton["agent_key_expires_at"]).timestamp() > time.time() + 300

    pool_entries = payload["credential_pool"]["wundercorp"]
    assert len(pool_entries) == 1
    assert pool_entries[0]["agent_key"] == token
    assert pool_entries[0]["source"] == auth_mod.WUNDERCORP_DEVICE_CODE_SOURCE


def test_resolve_wundercorp_runtime_credentials_invoke_jwt_is_idempotent(
    tmp_path,
    monkeypatch,
):
    import loki_cli.auth as auth_mod
    ...
    auth_store = {
        ...
        "providers": {
            "wundercorp": {
                "portal_base_url": "https://portal.wundercorp.com",
                "inference_base_url": "https://inference-api.wundercorp.com/v1",
                "client_id": "loki-cli",
                "token_type": "Bearer",
                "scope": auth_mod.DEFAULT_WUNDERCORP_SCOPE,
                "access_token": token,
    ...
    monkeypatch.setattr(auth_mod, "_write_shared_wundercorp_state", _unexpected_shared_write)
    monkeypatch.setattr(auth_wundercorp, "_write_shared_wundercorp_state", _unexpected_shared_write)
    monkeypatch.setattr(
        auth_mod,
    ...
    assert (
        payload["providers"]["wundercorp"]["agent_key_obtained_at"]
        == original_obtained_at
    )


def test_resolve_wundercorp_runtime_credentials_reauths_when_invoke_scope_missing(
    tmp_path,
    monkeypatch,
):
    import loki_cli.auth as auth_mod
    ...
    with pytest.raises(AuthError) as exc:
        auth_mod.resolve_wundercorp_runtime_credentials()

    assert exc.value.code == "missing_inference_invoke_scope"
    assert exc.value.relogin_required is True
    payload = json.loads((loki_home / "auth.json").read_text())
    assert payload["providers"]["wundercorp"]["agent_key"] is None
    assert "credential_pool" not in payload or not payload["credential_pool"].get("wundercorp")
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LOKI_HOME", str(loki_home))
    monkeypatch.setenv("LOKI_AGENT_USE_LEGACY_SESSION_KEYS", "true")

    creds = auth_mod.resolve_wundercorp_runtime_credentials()

    assert creds["api_key"] == token
    payload = json.loads((loki_home / "auth.json").read_text())
    assert payload["providers"]["wundercorp"]["agent_key"] == token
    ...
    result = auth_mod._wundercorp_device_code_login(
        portal_base_url="https://portal.example.com",
        inference_base_url="https://inference.example.com/v1",
        open_browser=False,
        timeout_seconds=1,
    ...
    assert requested_scopes == [auth_mod.DEFAULT_WUNDERCORP_SCOPE]
    assert result["agent_key"] == login_token


def test_wundercorp_inference_auth_logs_do_not_include_secret_values(
    tmp_path,
    monkeypatch,
    caplog,
):
    ...
    monkeypatch.setattr(auth_wundercorp, "_refresh_access_token", _fake_refresh_access_token)

    caplog.set_level(logging.DEBUG, logger="loki_cli.auth")
    auth_mod.resolve_wundercorp_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_wundercorp_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    """get_wundercorp_auth_status() should find WunderCorp credentials in the pool
    even when the auth store has no WunderCorp provider entry — this is the
    case when login happened via the dashboard device-code flow which
    saves to the pool only.
    """
    from loki_cli.auth import get_wundercorp_auth_status

    loki_home = tmp_path / "loki"
    loki_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store — no WunderCorp provider entry
    (loki_home / "auth.json").write_text(json.dumps({
        "version": 1, "providers": {},
    }))
    monkeypatch.setenv("LOKI_HOME", str(loki_home))

    # Seed the credential pool with a WunderCorp entry
    from agent.credential_pool import PooledCredential, load_pool
    pool = load_pool("wundercorp")
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    entry = PooledCredential.from_dict("wundercorp", {
        "access_token": token,
    ...
    status = get_wundercorp_auth_status()
    assert status["logged_in"] is True
    assert "example.com" in str(status.get("portal_base_url", ""))


def test_get_wundercorp_auth_status_empty_returns_not_logged_in(tmp_path, monkeypatch):
    """get_wundercorp_auth_status() returns logged_in=False when both pool
...
# =============================================================================
# _login_wundercorp: "Skip (keep current)" must preserve prior provider + model
# =============================================================================


class TestLoginWunderCorpSkipKeepsCurrent:
    """When a user runs `loki model` → WunderCorp Portal → Skip (keep current) after
    ...
    unconditionally after login, which flipped model.provider to "wundercorp" while
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        config_path = loki_home / "config.yaml"
        ...
        return loki_home, config_path, auth_path

    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        """Patch OAuth + model-list + prompt so _login_wundercorp doesn't hit network."""
        import loki_cli.auth as auth_mod
        import loki_cli.auth_wundercorp as auth_wundercorp
        import loki_cli.models as models_mod
        from loki_cli import models_pricing
        import loki_cli.wundercorp_subscription as ns

        fake_auth_state = {
            "access_token": "fake-wundercorp-token",
            "agent_key": "fake-agent-key",
            "inference_base_url": "https://inference-api.wundercorp.com",
            "portal_base_url": "https://portal.wundercorp.com",
            "refresh_token": "fake-refresh",
            "token_expires_at": 9999999999,
        }
        monkeypatch.setattr(
            auth_mod, "_wundercorp_device_code_login",
            lambda **kwargs: dict(fake_auth_state),
        )
        monkeypatch.setattr(
            auth_wundercorp, "_wundercorp_device_code_login",
            lambda **kwargs: dict(fake_auth_state),
        )
        monkeypatch.setattr(
        ...
        def _check_wundercorp_free_tier(**kwargs):
            free_tier_calls.append(kwargs)
            return None

        monkeypatch.setattr(models_mod, "check_wundercorp_free_tier", _check_wundercorp_free_tier)
        monkeypatch.setattr(
            models_mod, "partition_wundercorp_models_by_tier",
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        """User picks Skip → config.yaml untouched, WunderCorp creds still saved."""
        import argparse
        import yaml
        from loki_cli.auth import PROVIDER_REGISTRY, _login_wundercorp

        loki_home, config_path, auth_path = self._setup_home_with_openrouter(
        ...
        _login_wundercorp(args, PROVIDER_REGISTRY["wundercorp"])
        ...
        # auth.json: active_provider restored to openrouter, but WunderCorp creds saved
        auth_after = json.loads(auth_path.read_text())
        assert auth_after["active_provider"] == "openrouter"
        assert "wundercorp" in auth_after["providers"]
        assert auth_after["providers"]["wundercorp"]["access_token"] == "fake-wundercorp-token"
        # Existing openrouter creds still intact
        assert auth_after["providers"]["openrouter"]["api_key"] == "sk-or-fake"

    def test_picking_model_switches_to_wundercorp(self, tmp_path, monkeypatch):
        """User picks a WunderCorp model → provider flips to wundercorp with that model."""
        import argparse
        import yaml
        from loki_cli.auth import PROVIDER_REGISTRY, _login_wundercorp
    ...
    def test_skip_with_no_prior_active_provider_clears_it(self, tmp_path, monkeypatch):
        ...
        instead of leaving it as wundercorp."""
        import argparse
        import yaml
        from loki_cli.auth import PROVIDER_REGISTRY, _login_wundercorp
        ...
        auth_path = loki_home / "auth.json"
        auth_after = json.loads(auth_path.read_text())
        # active_provider should NOT be set to "wundercorp" after Skip
        assert auth_after.get("active_provider") in {None, ""}
        # But WunderCorp creds are still saved
        assert "wundercorp" in auth_after.get("providers", {})


# =============================================================================
# persist_wundercorp_credentials: shared helper for CLI + web dashboard login paths
# =============================================================================


def _full_state_fixture() -> dict:
    """Shape of the dict returned by _wundercorp_device_code_login /
    refresh_wundercorp_oauth_from_state. Used as helper input."""
    token = _invoke_jwt(seconds=3600)
    expires_at = _future_iso(3600)
    return {
        "portal_base_url": "https://portal.example.com",
...
def test_persist_wundercorp_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    """Helper must populate BOTH credential_pool.wundercorp AND providers.wundercorp.

    Regression guard: before this helper existed, `loki auth add wundercorp`
    wrote only the pool. After the WunderCorp agent_key's 24h TTL expired, the
    401-recovery path in run_agent.py called resolve_wundercorp_runtime_credentials
    which reads providers.wundercorp, found it empty, raised AuthError, and the
    agent failed with "Non-retryable client error". Both stores must stay
    in sync at write time.
    """
    from loki_cli.auth import persist_wundercorp_credentials, WUNDERCORP_DEVICE_CODE_SOURCE
    ...
    entry = persist_wundercorp_credentials(state)

    assert entry is not None
    assert entry.provider == "wundercorp"
    assert entry.source == WUNDERCORP_DEVICE_CODE_SOURCE

    payload = json.loads((loki_home / "auth.json").read_text())

    # providers.wundercorp populated with the full state (new behaviour)
    singleton = payload["providers"]["wundercorp"]
    ...
    # credential_pool.wundercorp has exactly one canonical device_code entry
    pool_entries = payload["credential_pool"]["wundercorp"]
    assert len(pool_entries) == 1, pool_entries
    pool_entry = pool_entries[0]
    assert pool_entry["source"] == WUNDERCORP_DEVICE_CODE_SOURCE
    assert pool_entry["agent_key"] == state["agent_key"]
    assert pool_entry["inference_base_url"] == "https://inference.example.com/v1"


def test_persist_wundercorp_credentials_idempotent_no_duplicate_pool_entries(tmp_path, monkeypatch):
    ...
    ``load_pool()``. The helper now writes providers.wundercorp and lets seeding
    ...
    persist_wundercorp_credentials(first)
    ...
    persist_wundercorp_credentials(second)

    payload = json.loads((loki_home / "auth.json").read_text())

    # providers.wundercorp reflects the latest write (singleton semantics)
    assert payload["providers"]["wundercorp"]["access_token"] == second_token
    assert payload["providers"]["wundercorp"]["agent_key"] == second_token

    # credential_pool.wundercorp has exactly one entry, carrying the latest agent_key
    pool_entries = payload["credential_pool"]["wundercorp"]
    assert len(pool_entries) == 1, pool_entries
    assert pool_entries[0]["source"] == WUNDERCORP_DEVICE_CODE_SOURCE
...
def test_persist_wundercorp_credentials_no_label_uses_auto_derived(tmp_path, monkeypatch):
    """When the caller doesn't pass ``label``, the auto-derived fingerprint
    is used (unchanged default behaviour — regression guard).
    """
    from loki_cli.auth import persist_wundercorp_credentials
    ...
    entry = persist_wundercorp_credentials(_full_state_fixture())
    assert entry is not None
    ...
    # No "label" key embedded in providers.wundercorp when the caller didn't supply one.
    payload = json.loads((loki_home / "auth.json").read_text())
    assert "label" not in payload["providers"]["wundercorp"]


def test_refresh_token_reuse_detection_surfaces_actionable_message():
    """Regression for #15099.

    When the WunderCorp Portal server returns ``invalid_grant`` with
    ``error_description`` containing "reuse detected", Loki must surface an
    ...
    re-authenticate" string led users to report this as a Loki persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from loki_cli.auth import _refresh_access_token
    ...
    assert "loki auth add wundercorp" in message.lower()
...
def test_refresh_token_exchange_sends_refresh_token_header():
    """WunderCorp refresh tokens must be sent in a header so sandbox proxies can
    ...
    assert client.kwargs["headers"]["x-wundercorp-refresh-token"] == "refresh-1"
...
# =============================================================================
# Shared WunderCorp token store — cross-profile persistence (Codex-style auto-import)
# =============================================================================


@pytest.fixture
def shared_store_env(tmp_path, monkeypatch):
    """Redirect LOKI_SHARED_AUTH_DIR to a tmp_path.

    Required for every test that exercises the shared WunderCorp store — the
    ...
    monkeypatch.setenv("LOKI_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without LOKI_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.loki/shared/`` across CI runs.
    """
    from loki_cli.auth import _wundercorp_shared_store_path

    monkeypatch.delenv("LOKI_SHARED_AUTH_DIR", raising=False)

    with pytest.raises(RuntimeError, match="shared WunderCorp auth store"):
        _wundercorp_shared_store_path()
...
def test_shared_store_write_and_read_roundtrip(shared_store_env):
    ...
    from loki_cli.auth import (
        _wundercorp_shared_store_path,
        _read_shared_wundercorp_state,
        _write_shared_wundercorp_state,
    )
    ...
    loaded = _read_shared_wundercorp_state()
...
def test_persist_wundercorp_credentials_mirrors_to_shared_store(
    tmp_path, monkeypatch, shared_store_env,
):
    """persist_wundercorp_credentials must populate BOTH per-profile auth.json
    AND the shared store, so a future profile's `loki auth add wundercorp
    --type oauth` can one-tap import instead of redoing device-code.
    """
    from loki_cli.auth import (
        _wundercorp_shared_store_path,
        _read_shared_wundercorp_state,
        persist_wundercorp_credentials,
    )

    loki_home = tmp_path / "loki"
    loki_home.mkdir(parents=True, exist_ok=True)
    (loki_home / "auth.json").write_text(
        json.dumps({"version": 1, "providers": {}})
    )
    monkeypatch.setenv("LOKI_HOME", str(loki_home))

    persist_wundercorp_credentials(_full_state_fixture())

    # Per-profile auth.json populated
    payload = json.loads((loki_home / "auth.json").read_text())
    assert "wundercorp" in payload.get("providers", {})
    ...
    assert str(_wundercorp_shared_store_path()).startswith(str(shared_store_env))
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    every field persist_wundercorp_credentials() needs.
    """
    from loki_cli import auth as auth_mod
    import loki_cli.auth_wundercorp as auth_wundercorp

    auth_mod._write_shared_wundercorp_state(_full_state_fixture())
    ...
    monkeypatch.setattr(auth_mod, "refresh_wundercorp_oauth_from_state", _fake_refresh)
    monkeypatch.setattr(auth_wundercorp, "refresh_wundercorp_oauth_from_state", _fake_refresh)

    result = auth_mod._try_import_shared_wundercorp_state()
    ...
    assert result["client_id"] == "loki-cli"
...
class TestStalePortalBaseUrlMigration:
    """_migrate_stale_wundercorp_portal_url auto-corrects stale portal_base_url on load."""

    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from loki_cli.auth import _load_auth_store, DEFAULT_WUNDERCORP_PORTAL_URL
        ...
        auth_file.write_text(json.dumps({
            ...
            "providers": {
                "wundercorp": {
                    "portal_base_url": "https://api.wundercorp.com",
        ...
        wundercorp = store["providers"]["wundercorp"]
        assert wundercorp["portal_base_url"] == DEFAULT_WUNDERCORP_PORTAL_URL
    ...
    def test_runtime_credentials_rejects_http_for_production_portal(
        ...
        auth_file = loki_home / "auth.json"
        store = json.loads(auth_file.read_text())
        store["providers"]["wundercorp"]["portal_base_url"] = (
            "http://portal.wundercorp.com"
        )
        ...
        monkeypatch.setattr(
            auth_wundercorp, "_refresh_access_token", _fake_refresh_access_token
        )

        auth_mod.resolve_wundercorp_runtime_credentials()
        assert refresh_calls == [auth_mod.DEFAULT_WUNDERCORP_PORTAL_URL]
...
class TestWunderCorpDeviceAuthTimeoutMessage:
    def test_timeout_message_mentions_captcha_login_and_retry(self):
        from loki_cli.auth import _wundercorp_device_auth_timeout_message

        msg = _wundercorp_device_auth_timeout_message("https://portal.wundercorp.com")
        assert "CAPTCHA" in msg
        assert "loki portal" in msg
        assert "https://portal.wundercorp.com/login" in msg
    ...
    def test_timeout_message_falls_back_to_default_portal(self):
        from loki_cli.auth import (
            DEFAULT_WUNDERCORP_PORTAL_URL,
            _wundercorp_device_auth_timeout_message,
        )

        msg = _wundercorp_device_auth_timeout_message("")
        assert f"{DEFAULT_WUNDERCORP_PORTAL_URL.rstrip('/')}/login" in msg
...
def test_poll_for_token_timeout_raises_actionable_message():
    ...
    so both the CLI login and the dashboard poller (web_server_oauth._wundercorp_poller,
...
def test_wundercorp_device_code_login_timeout_raises_actionable_message(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {
            "device_code": "device",
            "user_code": "SMCL-97YT",
            "verification_uri": "https://portal.wundercorp.com/manage-subscription",
            "verification_uri_complete": (
                "https://portal.wundercorp.com/manage-subscription"
                "?user_code=SMCL-97YT"
            ),
            "expires_in": 600,
            "interval": 1,
    ...
    def _timeout(**kwargs):
        raise TimeoutError(
            auth_mod._wundercorp_device_auth_timeout_message(
