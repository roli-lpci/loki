...
Profile-aware location: ``$LOKI_HOME/logs/dashboard-auth.log``.
...
from loki_cli.dashboard_auth.audit import audit_log, AuditEvent


@pytest.fixture
def profile_home(tmp_path, monkeypatch):
    """Redirect $LOKI_HOME and ~ to a tmp dir for the duration of the test."""
    home = tmp_path / ".loki"
    home.mkdir()
...
def test_audit_writes_jsonlines(profile_home):
    audit_log(AuditEvent.LOGIN_START, provider="wundercorp", ip="1.2.3.4")
    audit_log(
        AuditEvent.LOGIN_SUCCESS,
        provider="wundercorp", user_id="u1",
    ...
    assert second["provider"] == "wundercorp"
...
def test_audit_redacts_token_like_fields(profile_home):
    audit_log(
        AuditEvent.LOGIN_SUCCESS,
        provider="wundercorp", access_token="should-not-appear",
