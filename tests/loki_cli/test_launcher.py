"""Tests for the top-level `./loki` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./loki` should use `loki_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "loki"
    called = []

    fake_main_module = types.ModuleType("loki_cli.main")

    def fake_main():
        called.append("loki_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "loki_cli.main", fake_main_module)
