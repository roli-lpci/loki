"""Portal-URL resolution for Phase 2b billing errors (wundercorp_billing).
...
def _preview(monkeypatch):
    monkeypatch.setenv("LOKI_PORTAL_BASE_URL", "https://nas-pr-412.wundercorp.wtf")


def test_absolutize_resolves_relative(_preview):
    assert (
        _absolutize_portal_url("/billing?topup=open")
        == "https://nas-pr-412.wundercorp.wtf/billing?topup=open"
    )
