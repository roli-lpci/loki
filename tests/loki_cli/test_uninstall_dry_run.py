...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_loki_home", lambda: loki_home)
    monkeypatch.setattr(uninstall, "_is_default_loki_home", lambda home: False)
    ...
    assert str(loki_home) in output
    assert project_root.exists()
    assert loki_home.exists()
