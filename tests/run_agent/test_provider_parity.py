...
class TestBuildApiKwargsWunderCorpPortal:
    def test_includes_wundercorp_product_tags(self, monkeypatch):
        ...
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == wundercorp_portal_tags(session_id=agent.session_id)
...
# ── Chat completions response handling (OpenRouter/WunderCorp) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_wundercorp_when_no_openrouter(self, monkeypatch):
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
        from agent.auxiliary_client import _WUNDERCORP_MODEL, get_text_auxiliary_client
        wundercorp_auth = {
            "access_token": _fake_invoke_jwt(),
            "scope": "inference:invoke",
        }
        with patch("agent.auxiliary_client._read_wundercorp_auth", return_value=wundercorp_auth), \
             patch("agent.auxiliary_client.OpenAI") as mock, \
             patch("loki_cli.models.get_wundercorp_recommended_aux_model", return_value=None):
            client, model = get_text_auxiliary_client()
        assert model == _WUNDERCORP_MODEL

    def test_custom_endpoint_when_no_wundercorp(self, monkeypatch):
        """Custom endpoint is used when no OpenRouter/WunderCorp keys are available.
