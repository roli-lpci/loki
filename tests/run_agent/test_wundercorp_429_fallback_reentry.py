"""Regression guard: a genuine WunderCorp 429 must re-enter the retry loop so the
top-of-loop WunderCorp rate-limit guard can activate the fallback chain.
...
``record_wundercorp_rate_limit()`` moments earlier and either activates a fallback
...
class TestGenuineWunderCorp429ReentersLoop:
