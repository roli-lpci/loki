...
binary download are exercised manually by `loki secrets bitwarden
...
def loki_home(tmp_path, monkeypatch):
    """Point Loki at an isolated home directory."""
    home = tmp_path / ".loki"
    home.mkdir()
    monkeypatch.setenv("LOKI_HOME", str(home))
    # Some modules cache get_loki_home; clear if needed.
    import loki_constants
    if hasattr(loki_constants, "_LOKI_HOME_CACHE"):
        loki_constants._LOKI_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(loki_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from loki_cli.env_loader import _apply_external_secret_sources
