#!/usr/bin/env bash
# Drive the Loki-Setup dmg bootstrap through its first-run GUI.
#
...
# the redirect env), click "Install Loki ->" with native input, then watch
...
tell application "System Events"
  set procs to (every process whose name contains "Loki")
...
LOKI_BIN="$INSTALL_DIR/venv/bin/loki"
...
install_complete() {
  [ -d "$INSTALL_DIR/.git" ] && [ -x "$LOKI_BIN" ] && installed_app
}
...
BOOTSTRAP_LOG="$HOME/.loki/logs/bootstrap-installer.log"
bootstrap_error() {
...
while :; do
  if install_complete; then
    log "install landed: checkout + venv console script + Loki.app present"
  ...
  if ! kill -0 "$SETUP_PID" 2>/dev/null; then
    ...
    log "Loki-Setup exited (pid $SETUP_PID) before the install landed"
    exit 1
  fi
  err="$(bootstrap_error)"
  if [ -n "$err" ]; then
