// @ts-check
/**
 * Launch the Loki desktop app from a captured launch spec and click the
 * real update flow: Settings -> About -> "Update now".
 *
 * The spec is written by launch-capture/sitecustomize.py at `loki
...
async function main() {
  ...
  // an "update complete, reopen Loki to finish" overlay and never exits;
  ...
  const boundedClose = async (application, label) => {
    ...
    // (`loki serve` python + node helpers) survives and keeps writing
  ...
  phase('relaunch');
  log('relaunching the updated app (the "reopen Loki" step)');
