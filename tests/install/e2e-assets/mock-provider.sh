...
#   mock_start <workroot>      start, write config into $LOKI_HOME
#   mock_stop                  kill the background server
#
# Requires: ASSETS (e2e-assets dir), LOG_DIR, LOKI_HOME, ok/fail helpers.
...
EOF
  ...
  ok "provider 'mock' configured in $LOKI_HOME (api $url/v1)"
}
