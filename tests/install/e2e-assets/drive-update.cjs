// drive-update.cjs — launch the INSTALLED Loki.exe (real Electron desktop
...
//
//   node <this file> <path-to-Loki.exe> <proof-dir>
//
...
if (!exePath || !proofDir) {
  console.error('usage: node drive-update.cjs <Loki.exe> <proof-dir>')
  process.exit(1)
}
...
async function main() {
  ...
  const app = await _electron.launch({
    ...
    // Inherit the driver's env: LOKI_HOME (isolated install) and
  ...
  // The real backend (`loki serve`) is booting underneath; give it time.
  ...
  // buttonless boot-progress card ("Starting Loki... 86%"), then the
  ...
  if (!visible) {
    ...
    const status = await page.evaluate(() =>
      window.lokiDesktop?.updates?.check?.() ?? Promise.resolve('no updates.check bridge')
  ...
  // The "Updating Loki — this window will close" overlay should appear,
  ...
  // the staged binary), which claims LOKI_HOME/.loki-update-in-progress
  ...
  // proved the desktop had exited and `loki update` was already running).
  //
  ...
  const lokiHome = process.env.LOKI_HOME
  const markerPath = lokiHome ? path.join(lokiHome, '.loki-update-in-progress') : null
  const resultPath = lokiHome ? path.join(lokiHome, '.loki-update-result.json') : null
