...
; Drive the REAL Loki-Setup.exe (Tauri bootstrap installer) window:
; click Install, wait for the install to finish, click Launch, and wait for
; the real Loki.exe (Electron desktop) window to appear.
;
; Adapted from @ethernet8023's e2e/windows/install-loki-desktop.ahk
...
setupExe := A_Args.Length >= 2 ? A_Args[2] : "Loki-Setup.exe"
...
Log("Launch clicked; waiting for the Loki desktop app window")
...
try {
    ...
    throw Error("Loki.exe real-sized window did not appear within 300s of clicking Launch")
}
