...
# ============================================================================
# Proves, on a real Windows machine, that a user who installs Loki the way
...
# actually touches:
#
#   INSTALL   - downloads the production Loki-Setup.exe from the website,
#               launches it HEADED, and AutoHotkey clicks Install, waits,
#               then clicks Launch. The real Electron Loki.exe must appear.
...
#   installer's `git clone` and `loki update`'s `git fetch origin`
...
param(
    ...
    # open-app-update (from a desktop-installer install) and loki-update /
    ...
    # they land.
    [ValidateSet("open-app-update", "loki-desktop-app-update", "loki-update", "desktop-installer@latest", "installer-script", "installer-script+desktop")]
    [string]$Route = "open-app-update",

    # The OLD version: the ref served as `main` while the installer runs,
    # i.e. what the user starts on. The published Loki-Setup.exe carries
    ...
    [string]$WorkRoot = $(if ($env:LOKI_E2E_WORKROOT) { $env:LOKI_E2E_WORKROOT } else { Join-Path $env:TEMP "loki-desktop-gui-e2e" }),

    [string]$SetupExeUrl = "https://loki-assets.wundercorp.com/Loki-Setup.exe",
...
$LokiHome  = Join-Path $WorkRoot "loki-home"
$InstallDir  = Join-Path $LokiHome "loki-agent"
...
function Invoke-Git([string[]]$GitArgs) {
    ...
    # if loki ever adds a git command that calls something with ^ this will break, lol.
...
function Set-GitRedirect {
    # we redirect to our own repo so we can play around with what commit loki thinks we're on.
...
function Get-DesktopExe {
    foreach ($c in @(
        (Join-Path $InstallDir "apps\desktop\release\win-unpacked\Loki.exe"),
        (Join-Path $InstallDir "apps\desktop\release\win-arm64-unpacked\Loki.exe")
    )) {
...
# Install-side state snapshot, taken BEFORE Test-LokiRuns can throw: on
...
function Save-InstallSideState([string]$Label) {
    ...
    $logsDir = Join-Path $LokiHome "logs"
    if (Test-Path -LiteralPath $logsDir) {
        Copy-Item $logsDir (Join-Path $dest "loki-logs") -Recurse -Force -ErrorAction SilentlyContinue
    }
    $resultFile = Join-Path $LokiHome ".loki-update-result.json"
    ...
    Get-ChildItem -LiteralPath $LokiHome -ErrorAction SilentlyContinue |
        Select-Object Name, Length, LastWriteTime |
        Format-Table -AutoSize | Out-String |
        Set-Content (Join-Path $dest "loki-home-ls.txt")
}

function Test-LokiRuns([string]$Label) {
    Save-InstallSideState $Label
    $lokiExe = Join-Path $InstallDir "venv\Scripts\loki.exe"
    Assert-True (Test-Path -LiteralPath $lokiExe) "$Label -- venv\Scripts\loki.exe exists"
    & $lokiExe --version 2>&1 | ForEach-Object { Write-Host "    loki --version| $_" }
    Assert-True ($LASTEXITCODE -eq 0) "$Label -- loki --version exits 0"
}
...
function Invoke-RefInstaller {
    ...
    $flags = @("-SkipSetup", "-LokiHome", $LokiHome, "-InstallDir", $InstallDir)
...
function Invoke-LokiUpdate {
    ...
    $updateArgs = @("update")
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    $helpText = & $lokiExe update --help 2>&1 | Out-String
    if ($helpText -match '--yes') { $updateArgs += "--yes" }
    New-Item -ItemType Directory -Path (Join-Path $WorkRoot "logs") -Force | Out-Null
    $log = Join-Path $WorkRoot "logs\update.log"
    Push-Location $InstallDir
    try {
        & $lokiExe @updateArgs 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $updateExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
    }
    Write-LogGroup "loki update transcript" $log
    Assert-True ($updateExit -eq 0) "loki update exited $updateExit (expected 0)"
}

function Invoke-LokiDesktopAppUpdate([string]$TargetSha) {
    # The loki-desktop launch surface: `loki desktop` runs its whole
    ...
    $prevCap = $env:LOKI_E2E_CAPTURE_LAUNCH
    $env:PYTHONPATH = if ($prevPy) { "$capDir;$prevPy" } else { $capDir }
    $env:LOKI_E2E_CAPTURE_LAUNCH = $spec
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    Push-Location $InstallDir
    try {
        & $lokiExe desktop 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $capExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
        $env:PYTHONPATH = $prevPy
        $env:LOKI_E2E_CAPTURE_LAUNCH = $prevCap
    }
    Write-LogGroup "loki desktop (launch capture) transcript" $log
    Assert-True ($capExit -eq 0) "loki desktop exited 0 during launch capture"
    ...
    try {
        & $node "launch-from-spec.mjs" --spec $spec `
            --result (Join-Path $LokiHome ".loki-update-result.json") `
    ...
    Assert-True ($driveExit -eq 0) "app driven via captured loki desktop spec; update completed"
}
...
function Stop-LokiAppProcesses([string]$Label) {
    # Close the desktop app the blunt way between phases (a user quitting).
    # Only Loki.exe (Electron) -- never loki.exe (the venv CLI shim).
    $procs = @(Get-Process -Name "Loki" -ErrorAction SilentlyContinue)
    foreach ($p in $procs) {
        try { Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue } catch {}
    }
    if ($procs.Count -gt 0) {
        Write-Host "  [$Label] stopped $($procs.Count) Loki.exe process(es)"
        Start-Sleep -Seconds 3
    }
}

function Get-ManagedNode {
    # `loki update`/desktop builds use the Loki-managed Node; use the same
    # one to run the Playwright driver so no system Node is required.
    $candidates = @(
        (Join-Path $LokiHome "node\node.exe"),
        (Join-Path $LokiHome "bin\node\node.exe"),
...
# ----------------------------------------------------------------------------
# Phase: install-gui -- website Loki-Setup.exe, headed, AHK-driven
# ----------------------------------------------------------------------------
function Invoke-PhaseInstallGui {
    ...
    Write-Step "$($Mode.ToUpper()) (GUI): Loki-Setup.exe from the website, headed, AHK clicks"
    ...
    $setupExe = Join-Path $WorkRoot "Loki-Setup.exe"
    if (-not (Test-Path -LiteralPath $setupExe)) {
        Write-Host "  downloading $SetupExeUrl"
        Invoke-WebRequest -Uri $SetupExeUrl -OutFile $setupExe
    }
    Assert-True ((Get-Item $setupExe).Length -gt 1MB) "Loki-Setup.exe downloaded ($([math]::Round((Get-Item $setupExe).Length / 1MB, 1)) MB)"
    ...
    Remove-Item Env:LOKI_SETUP_DEV_REPO_ROOT -ErrorAction SilentlyContinue
    New-Item -ItemType Directory -Path $LokiHome -Force | Out-Null
    ...
    try {
        ...
        Write-Host "  Loki-Setup.exe launched (pid $($installer.Id))"

        # Drive it: Install click -> wait -> Launch click -> Loki.exe window.
        ...
        $ahk = Start-Process -FilePath $ahkExe `
            -ArgumentList (Join-Path $AhkDir "install-and-launch.ahk"), $ahkLog, "Loki-Setup.exe", (Join-Path $LokiHome "logs\bootstrap-installer.log") `
            -PassThru
        ...
        # actually be running.
        Assert-True ($null -ne (Get-Process -Name "Loki" -ErrorAction SilentlyContinue)) "Loki.exe process is running (installer Launch hand-off worked)"

        # Installer should have exited after Launch.
        if (-not $installer.HasExited) {
            Start-Sleep -Seconds 10
        }
        Assert-True $installer.HasExited "Loki-Setup.exe exited after Launch"
    }
    finally {
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        # Surface the installer's own log win or lose, full and folded.
        $bootLog = Join-Path $LokiHome "logs\bootstrap-installer.log"
    ...
    Stop-LokiAppProcesses "post-install"
    ...
    Test-LokiRuns "post-$Mode-gui"
    Assert-True ($null -ne (Get-DesktopExe)) "packaged Desktop Loki.exe exists"
...
function Invoke-GuiUpdateDesktopRoute([string]$TargetSha) {
    ...
    Assert-True ($null -ne $desktopExe) "packaged Loki.exe present before update"
    ...
    try {
        ...
        # `loki update` + desktop rebuild + relaunch. Which updater depends
        # on the installed checkout, and BOTH are production paths:
        #   * checkouts shipping scripts/desktop-update.ps1 -> that script,
        #     which writes .loki-update-result.json on every exit;
        #   * older checkouts -> the staged loki-setup.exe --update flow,
        #     which does NOT write the result JSON.
        # So: poll for COMPLETION = (result JSON) OR (checkout reached the
        # target sha AND the marker is gone). The sha/marker/loki/relaunch
        ...
        $updateLog = Join-Path $LokiHome "logs\update.log"
        $updateLogPos = 0
        ...
        Test-LokiRuns "post-update"
        Assert-True ($null -ne (Get-DesktopExe)) "Loki.exe still present after update"
        ...
        Write-Host "  waiting for the relaunched Loki.exe ..."
        $rDeadline = (Get-Date).AddMinutes(5)
        $relaunched = $null
        while ((Get-Date) -lt $rDeadline) {
            $relaunched = Get-Process -Name "Loki" -ErrorAction SilentlyContinue
            if ($relaunched) { break }
        ...
        # Foreground the relaunched Loki window so the proof screenshot
        ...
        try {
            $mainProc = Get-Process -Name "Loki" -ErrorAction SilentlyContinue |
...
'@ -ErrorAction SilentlyContinue
        ...
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        $handoffLog = Join-Path $LokiHome "logs\desktop-update-handoff.log"
        ...
        Stop-LokiAppProcesses "post-update"
    }
}

function Invoke-PhaseInstall {
    # Dispatch on the install axis. Each arm ends with the same contract:
    # checkout at OLD, loki runs, and state carries how OLD landed so any
    ...
    switch ($InstallMethod) {
        ...
        "installer-script" {
            ...
            Test-LokiRuns "post-install-script"
        }
        "installer-script+desktop" {
            ...
            Assert-True ((Get-InstalledHead) -eq $state.old) "installed checkout is at OLD"
            Test-LokiRuns "post-install-script-desktop"
...
function Invoke-PhaseUpdate {
    ...
    New-Item -ItemType File -Path (Join-Path $LokiHome ".skip_upstream_prompt") -Force | Out-Null
    ...
    switch ($Route) {
        ...
        "loki-desktop-app-update" {
            Invoke-LokiDesktopAppUpdate $state.current
        }
        "loki-update" {
            Invoke-LokiUpdate
        }
        ...
        "desktop-installer@latest" {
            # A user re-downloading Loki-Setup.exe and clicking Install over
...
function Invoke-CheckedPhaseUpdate {
    ...
    foreach ($oldLog in @((Join-Path $WorkRoot "logs\update.log"), (Join-Path $LokiHome "logs\desktop.log"))) {
