...
# (Loki-Setup.dmg from the website) can reach HEAD.
#
...
#              loki-desktop-app-update  capture `loki desktop`'s spawn,
#                                         launch the spec under Playwright,
#                                         click Update now
#              loki-update              CLI update from the installed venv
...
#     --update-method open-app-update|loki-desktop-app-update
...
UPDATE_METHOD=""
INSTALL_REF=""
DMG_URL="https://loki-assets.wundercorp.com/Loki-Setup.dmg"
...
case "$UPDATE_METHOD" in
  open-app-update|loki-desktop-app-update|loki-update|installer-script|installer-script+desktop) ;;
...
WORK_ROOT="${LOKI_E2E_WORKROOT:-${RUNNER_TEMP:-${TMPDIR:-/tmp}}/loki-macos-desktop-e2e}"
...
EOF
  ...
  export INSTALL_DIR="$LOKI_HOME/loki-agent"
}
...
phase_stage() {
  ...
  arm_redirect
  mkdir -p "$LOKI_HOME"
  touch "$LOKI_HOME/.skip_upstream_prompt"
...
find_installed_app() {
  ...
  local cand
  for cand in \
    "$INSTALL_DIR/apps/desktop/release/mac-arm64/Loki.app" \
    "$INSTALL_DIR/apps/desktop/release/mac/Loki.app" \
    "/Applications/Loki.app"; do
...
phase_install() {
  ...
  . "$STATE"
  arm_redirect
  step "installing OLD ($OLD_REF) via the published Loki-Setup.dmg"

  local dmg="$WORK_ROOT/Loki-Setup.dmg"
  ...
  # "Install Loki" button with native input.
  local rc=0
  ...
  log_group "Loki-Setup (dmg bootstrap) transcript" "$LOG_DIR/bootstrap-install.log"
  ...
  "$loki" --version 2>&1 | ts_prefix > "$LOG_DIR/version-old.log" || fail "loki --version failed after install"
  ok "loki --version works: $(head -c 120 "$LOG_DIR/version-old.log" | tr -d '\n')"
  find_installed_app >/dev/null || fail "no installed Loki.app after the dmg bootstrap"
...
phase_update() {
  ...
  case "$UPDATE_METHOD" in
    loki-update)
      ...
      # update subcommand only in later releases; ask the installed loki.
      local loki="$INSTALL_DIR/venv/bin/loki"
      local update_cmd=("$loki" update)
      ...
      [ "$rc" -eq 0 ] || fail "loki update exited $rc; transcript above"
      ;;
    installer-script)
    ...
    installer-script+desktop)
      ...
      for cand in \
        "$INSTALL_DIR/apps/desktop/release/mac-arm64/Loki.app" \
        "$INSTALL_DIR/apps/desktop/release/mac/Loki.app"; do
        [ -d "$cand" ] && { head_app="$cand"; break; }
      done
      [ -n "$head_app" ] || fail "no built Loki.app under the checkout after the +desktop update"
    ...
    open-app-update)
      ...
      # which is exactly what the app's children (git, loki update) need.
      local app app_bin
...
PYEOF
    ...
    loki-desktop-app-update)
      ...
      [ "$rc" -eq 0 ] || fail "loki desktop exited $rc during launch capture"
      [ -f "$spec.captured" ] || fail "loki desktop exited 0 but no launch was captured"
  ...
  mkdir -p "$ildest"
  cp -R "$HOME_SANDBOX/.loki/logs" "$ildest/loki-logs" 2>/dev/null || true
  local ud="$HOME_SANDBOX/Library/Application Support/Loki"
  ...
  "$INSTALL_DIR/venv/bin/loki" --version 2>&1 | ts_prefix > "$LOG_DIR/version-head.log" \
    || fail "loki --version failed after update"
  ok "loki --version works post-update"
