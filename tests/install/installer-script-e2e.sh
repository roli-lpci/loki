...
#              install landed on OLD with a working `loki`
#   update     advance served main to HEAD, apply ONE update method, assert
#              the checkout landed on HEAD with a working `loki`
#
# Usage:
#   tests/install/installer-script-e2e.sh --update-method loki-update|installer-script|installer-script+desktop
...
case "$UPDATE_METHOD" in
  loki-update|installer-script|installer-script+desktop|loki-desktop-app-update) ;;
  *) echo "error: --update-method must be loki-update, installer-script, installer-script+desktop or loki-desktop-app-update, got '$UPDATE_METHOD'" >&2; exit 1 ;;
esac
...
WORK_ROOT="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/loki-installer-script-e2e"
LOG_DIR="${LOKI_E2E_LOG_DIR:-$WORK_ROOT/logs}"
...
# Isolated HOME: the runner's real one may carry a preinstalled loki or a
# developer config, and old installer scripts hardcode $HOME/.loki (the
# LOKI_HOME env override is newer than tags we sample). GIT_CONFIG_GLOBAL
# above keeps working -- an explicit path wins over $HOME/.gitconfig.
export HOME="$WORK_ROOT/home"
mkdir -p "$HOME/.local/bin"
export PATH="$HOME/.local/bin:$PATH"
export LOKI_HOME="$HOME/.loki"
mkdir -p "$LOKI_HOME"
...
assert_desktop_artifact() {
  ...
  local cand
  for cand in \
    "$release_dir/linux-unpacked/Loki" \
    "$release_dir/linux-unpacked/loki" \
    "$release_dir/mac-arm64/Loki.app" \
    "$release_dir/mac/Loki.app"; do
...
assert_checkout() {
  ...
  ok "checkout is $2 ($1)"
  local loki="$INSTALL_DIR/venv/bin/loki"
  [ -x "$loki" ] || fail "no loki console script at $loki"
  "$loki" --version 2>&1 | ts_prefix > "$LOG_DIR/version-$2.log" \
    || fail "loki --version failed after $2; log in $LOG_DIR/version-$2.log"
  ok "loki --version works: $(head -c 120 "$LOG_DIR/version-$2.log" | tr -d '\n')"
}

smoke_desktop() {
  # $1: label (old|head). Prove the installed CLI can produce the desktop
  # app: `loki desktop --build-only` runs the full desktop pipeline
  # (workspace install, renderer build, stamp write) and stops before the
  # launch -- the same call `loki update` itself makes. Probe the
  # INSTALLED loki for the flag rather than assuming this checkout's
  # surface: sampled OLD releases may predate `loki desktop` or
  # --build-only entirely, and for them the phase skips, loudly.
  local loki="$INSTALL_DIR/venv/bin/loki"
  if ! "$loki" desktop --help 2>/dev/null | grep -qF -- --build-only; then
    ok "loki desktop --build-only not supported at $1; skipping desktop smoke"
    return 0
  fi
  local rc=0
  (cd "$INSTALL_DIR" && "$loki" desktop --build-only < /dev/null 2>&1 \
    | ts_prefix > "$LOG_DIR/desktop-smoke-$1.log") || rc=$?
  log_group "loki desktop --build-only ($1) transcript" "$LOG_DIR/desktop-smoke-$1.log"
  [ "$rc" -eq 0 ] || fail "loki desktop --build-only ($1) exited $rc; transcript above"
  ok "loki desktop --build-only works at $1"
...
case "$UPDATE_METHOD" in
  loki-update)
    ...
    # installed loki; older ones read the prompt from stdin, so close it.
    LOKI="$INSTALL_DIR/venv/bin/loki"
    if "$LOKI" update --help 2>&1 | grep -qF -- --yes; then
      update_cmd=("$LOKI" update --yes)
    else
      update_cmd=("$LOKI" update)
    fi
    rc=0
    (cd "$INSTALL_DIR" && "${update_cmd[@]}" < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/update.log") || rc=$?
    log_group "loki update transcript" "$LOG_DIR/update.log"
    [ "$rc" -eq 0 ] || fail "loki update exited $rc; transcript above, log at $LOG_DIR/update.log"
    ;;
  installer-script)
  ...
  loki-desktop-app-update)
    # The real user surface: `loki desktop` launches the app, the user
    ...
    # suite's own) is configured into LOKI_HOME exactly like the dev:mock
    ...
    step "capturing the loki desktop launch spec (build runs for real)"
    rc=0
    (cd "$INSTALL_DIR" && \
      PYTHONPATH="$ASSETS/launch-capture${PYTHONPATH:+:$PYTHONPATH}" \
      LOKI_E2E_CAPTURE_LAUNCH="$SPEC" \
      "$LOKI" desktop < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/desktop-launch-capture.log") || rc=$?
    log_group "loki desktop (launch capture) transcript" "$LOG_DIR/desktop-launch-capture.log"
    [ "$rc" -eq 0 ] || fail "loki desktop exited $rc during launch capture; transcript above"
    # Exit 0 without a capture means a version that never reached its
    # launch - that must fail loudly, not pass as a no-op.
    [ -f "$SPEC.captured" ] || fail "loki desktop exited 0 but no launch was captured at $SPEC"
...
mkdir -p "$ildest"
cp -R "$LOKI_HOME/logs" "$ildest/loki-logs" 2>/dev/null || true
if [ -n "${XDG_DATA_HOME:-}" ]; then
  cp -R "$XDG_DATA_HOME/loki/logs" "$ildest/desktop-userdata-logs" 2>/dev/null || true
fi
cp "$LOKI_HOME/.loki-update-result.json" "$ildest" 2>/dev/null || true
ls -la "$LOKI_HOME" > "$ildest/loki-home-ls.txt" 2>/dev/null || true
