...
def test_frontmatter_required_fields():
    ...
    loki = fm["metadata"]["loki"]
    assert loki["tags"]
    assert "related_skills" in loki
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Loki Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["loki"]["related_skills"]:
