...
def test_required_frontmatter_fields(p):
    ...
    loki = (fm.get("metadata") or {}).get("loki") or {}
    if not (loki.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.loki.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    loki = (fm.get("metadata") or {}).get("loki") or {}
    dangling = [
        rs for rs in (loki.get("related_skills") or []) if rs not in _all_names()
    ]
