"""Guard: Loki-owned subprocesses must not resolve managed runtimes by bare PATH.

Loki installs runtimes for itself — ``uv`` at ``$LOKI_HOME/bin/uv``, Node at
``$LOKI_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Loki's own
...
* when a system copy also exists, the one Loki does not own wins — which is
...
``iter_loki_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Loki provisions into LOKI_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Loki-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("loki_cli/update_cmd_deps.py", "uv"): (
    ...
    ("loki_cli/update_cmd_deps.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    # (loki_agent-<version>/) gets created and deleted concurrently, and
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Loki-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$LOKI_HOME/bin (uv) and $LOKI_HOME/node are not on an "
        ...
        "  node/npm -> loki_constants.find_node_executable()\n"
        "  PATH env -> loki_constants.iter_loki_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(loki_constants, helper))


def test_managed_uv_helpers_exist():
    from loki_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
