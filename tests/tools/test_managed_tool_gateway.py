...
def test_resolve_managed_tool_gateway_derives_vendor_origin_from_shared_domain():
    with patch.dict(
        os.environ,
        {
            "TOOL_GATEWAY_DOMAIN": "wundercorp.com",
        },
        clear=False,
    ), patch.object(managed_tool_gateway, "managed_wundercorp_tools_enabled", return_value=True):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "wundercorp-token",
        )

    assert result is not None
    assert result.gateway_origin == "https://firecrawl-gateway.wundercorp.com"
    assert result.wundercorp_user_token == "wundercorp-token"
...
def test_resolve_managed_tool_gateway_is_inactive_without_wundercorp_token():
    with patch.dict(
        os.environ,
        {
            "TOOL_GATEWAY_DOMAIN": "wundercorp.com",
...
def test_resolve_managed_tool_gateway_is_disabled_without_subscription():
    with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "wundercorp.com"}, clear=False), \
         patch.object(managed_tool_gateway, "managed_wundercorp_tools_enabled", return_value=False):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "wundercorp-token",
        )
...
def test_read_wundercorp_access_token_refreshes_expiring_cached_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "loki_cli.auth.resolve_wundercorp_access_token",
        lambda refresh_skew_seconds=120: "fresh-token",
    )

    assert managed_tool_gateway.read_wundercorp_access_token() == "fresh-token"
...
def test_connector_gateway_origin_pins_the_deployed_connectors_host():
    ...
    with patch.dict(
        os.environ,
        {"TOOL_GATEWAY_DOMAIN": "wundercorp.com", "TOOL_GATEWAY_SCHEME": "https"},
        clear=False,
    ):
        os.environ.pop("CONNECTOR_GATEWAY_URL", None)
        assert managed_gateway_auth.connector_gateway_origin() == (
            "https://connector-gateway.wundercorp.com"
        )
...
def test_managed_gateway_origin_honors_the_harness_override():
    ...
    with patch.dict(os.environ, {"TOOL_GATEWAY_URL": "http://127.0.0.1:3009/"}, clear=False):
        ...
        assert managed_gateway_auth.is_managed_wundercorp_gateway_url(
            "http://127.0.0.1:3009/api/vendorx/generations"
        )
        assert not managed_gateway_auth.is_managed_wundercorp_gateway_url(
            "https://tools.wundercorp.com/api/vendorx/generations"
        )
...
def test_connector_gateway_origin_honors_its_own_override():
    ...
    with patch.dict(
        ...
        assert managed_gateway_auth.managed_gateway_origin() == (
            "https://tool-gateway.wundercorp.com"
        )
        assert managed_gateway_auth.is_managed_wundercorp_gateway_url(
...
def test_default_bearer_gate_accepts_both_deployed_hosts_only():
    ...
    with patch.dict(
        ...
        os.environ.pop("CONNECTOR_GATEWAY_URL", None)
        for trusted in (
            "https://connector-gateway.wundercorp.com/v1/connectors/execute",
            "https://tool-gateway.wundercorp.com/api/vendorx/generations",
        ):
            assert managed_gateway_auth.is_managed_wundercorp_gateway_url(trusted)
        for untrusted in (
            "https://tools.wundercorp.com/v1/connectors/execute",
            "https://evil-connector-gateway.wundercorp.com.attacker.dev/v1/connectors",
            "https://connector-gateway.wundercorp.com.attacker.dev/v1/connectors",
            "http://connector-gateway.wundercorp.com/v1/connectors",
            "http://tool-gateway.wundercorp.com/api/vendorx/generations",
        ):
            assert not managed_gateway_auth.is_managed_wundercorp_gateway_url(untrusted)
