...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        self._managed_patchers = [
            patch("tools.tool_backend_helpers.managed_wundercorp_tools_enabled", return_value=True),
            patch("tools.managed_tool_gateway.managed_wundercorp_tools_enabled", return_value=True),
        ]
    ...
    def test_no_config_raises_with_helpful_message(self):
        ...
        with patch("plugins.web.firecrawl.provider.Firecrawl"):
            with patch("tools.managed_tool_gateway.read_wundercorp_access_token", return_value=None):
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "wundercorp.com"}):
            with patch("tools.managed_tool_gateway.read_wundercorp_access_token", return_value="wundercorp-token"):
                with patch("plugins.web.firecrawl.provider.Firecrawl") as mock_fc:
                    from plugins.web.firecrawl.provider import _get_firecrawl_client
                    result = _get_firecrawl_client()
                    mock_fc.assert_called_once_with(
                        api_key="wundercorp-token",
                        api_url="https://firecrawl-gateway.wundercorp.com",
                    )
...
class TestBackendSelection:
    ...
    ``loki tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def test_wundercorp_backend_maps_to_firecrawl(self):
        """The managed 'wundercorp' selection is serviced by the firecrawl
        provider (whose client resolver routes managed)."""
        from tools.web_tools import _get_backend
        with patch("tools.web_tools._load_web_config", return_value={"backend": "wundercorp"}):
            assert _get_backend() == "firecrawl"

    def test_managed_gateway_does_not_preempt_explicit_exa(self):
        """Regression: a WunderCorp OAuth token (managed gateway "ready") must NOT
        beat an explicitly configured EXA_API_KEY in the fallback path.
        Free WunderCorp tiers don't include web search, so the user's deliberate
    ...
    def test_managed_gateway_does_not_preempt_explicit_tavily(self):
        """A WunderCorp OAuth token must not beat an explicit TAVILY_API_KEY."""
        from tools.web_tools import _get_backend
        with patch("tools.web_tools._load_web_config", return_value={}), \
             patch("tools.web_tools._is_tool_gateway_ready", return_value=True), \
             patch.dict(os.environ, {"TAVILY_API_KEY": "tvly-test"}):
    ...
    def test_managed_gateway_only_falls_through_to_firecrawl(self):
        """When no explicit-credential backend is configured, a WunderCorp-managed
...
class TestCheckWebApiKey:
    ...
    def test_configured_firecrawl_backend_accepts_managed_gateway(self):
        with patch("tools.web_tools._load_web_config", return_value={"backend": "firecrawl"}):
            with patch("tools.managed_tool_gateway.peek_wundercorp_access_token", return_value="wundercorp-token"):
...
class TestNonBuiltinProviderAvailability:
    ...
    def test_check_web_api_key_returns_true_for_custom_provider(self):
        ...
        with patch("tools.web_tools._ddgs_package_importable", return_value=False), \
             patch("tools.managed_tool_gateway.peek_wundercorp_access_token", return_value=None):
            from tools.web_tools import check_web_api_key
            assert check_web_api_key() is True
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from loki_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.loki/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.loki/.env), not bare os.getenv. Parametrized over the four
    ...
    def test_keenable_search_reads_key_via_get_env_value(self, monkeypatch):
        ...
        with patch(
            ...
            assert headers["X-Keenable-Title"] == "loki-agent"
    ...
    def test_tavily_request_reads_key_via_get_env_value(self, monkeypatch):
        ...
        with patch(
            ...
            assert headers["X-Client-Name"] == "loki-agent"
