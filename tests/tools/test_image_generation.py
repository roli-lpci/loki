...
class TestGptQualityPinnedToMedium:
    ...
    and cannot be overridden via config. Pinning keeps WunderCorp Portal billing
...
class TestManagedGatewayErrorTranslation:
    """4xx from the WunderCorp managed gateway should be translated to a user-actionable message."""

    def test_4xx_translates_to_value_error_with_remediation(self, image_tool, monkeypatch):
        """403 from managed gateway → ValueError mentioning FAL_KEY + loki tools."""
        ...
        managed_gateway.wundercorp_user_token = "test-token"
    ...
    def test_billing_meter_error_is_preserved_instead_of_called_model_unavailable(
        ...
        assert "WunderCorp Portal billing" in msg
...
class TestManagedKreaRouting:
    ...
    def test_routes_native_krea_model_to_krea_plugin_in_managed_mode(
        ...
        monkeypatch.setattr(
            "loki_cli.plugins._ensure_plugins_discovered", lambda *a, **k: None
        )
