...
was gated on ``[ -f "$LOKI_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'LOKI_HOME="{home}"\n'
        # In tests we run unprivileged; as_loki is a passthrough then.
        'as_loki() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Loki loads $LOKI_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/loki/.env.example -> $LOKI_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $LOKI_HOME/.env from it on first boot"
    )
