...
def _restore_tool_modules():
    original_loki_home = os.environ.get("LOKI_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_loki_home is None:
            os.environ.pop("LOKI_HOME", None)
        else:
            os.environ["LOKI_HOME"] = original_loki_home
        _reset_modules(("tools", "loki_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    loki_cli = types.ModuleType("loki_cli")
    loki_cli.__path__ = [str(REPO_ROOT / "loki_cli")]  # type: ignore[attr-defined]
    sys.modules["loki_cli"] = loki_cli
    loki_home = tmp_path / "loki-home"
    os.environ["LOKI_HOME"] = str(loki_home)
    sys.modules["loki_cli.config"] = types.SimpleNamespace(
        get_loki_home=lambda: loki_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="loki-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": loki_home / "modal_snapshots.json",
