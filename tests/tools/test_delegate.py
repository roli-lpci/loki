...
class TestStripBlockedTools(unittest.TestCase):
    ...
    def test_mixed_composite_is_subtracted_at_child_assembly(self):
        ...
        ``loki-cli`` contains both allowed tools and every sensitive
        ...
        parent.enabled_toolsets = ["loki-cli"]
...
class TestDelegateTask(unittest.TestCase):
    ...
    def test_wundercorp_child_rederives_api_mode_from_model(self):
        ...
        Native wire selected (opt-in since 2026-09-06, ``wundercorp.anthropic_wire``)."""
        with patch("loki_cli.providers._wundercorp_anthropic_wire", return_value="native"):
            self._wundercorp_child_rederives_api_mode_from_model()

    def _wundercorp_child_rederives_api_mode_from_model(self):
        parent = _make_mock_parent(depth=0)
        parent.base_url = "https://inference-api.wundercorp.com/v1"
        parent.api_key = "portal-jwt"
        parent.provider = "wundercorp"
        ...
        with patch("run_agent.AIAgent") as MockAgent:
            ...
            _, kwargs = MockAgent.call_args
            self.assertEqual(kwargs["provider"], "wundercorp")
            self.assertEqual(kwargs["model"], "loki-4-405b")
...
class TestDelegationProviderIntegration(unittest.TestCase):
    ...
    def test_cross_provider_delegation(self, mock_creds, mock_cfg):
        """Parent on WunderCorp, subagent on OpenRouter — full credential switch."""
        mock_cfg.return_value = {
            "max_iterations": 45,
            "model": "google/gemini-3-flash-preview",
            "provider": "openrouter",
        ...
        parent.provider = "wundercorp"
        parent.base_url = "https://inference-api.wundercorp.com/v1"
        parent.api_key = "wundercorp-key-abc"
        ...
        with patch("run_agent.AIAgent") as MockAgent:
            ...
            _, kwargs = MockAgent.call_args
            # Child should use OpenRouter, NOT WunderCorp
            self.assertEqual(kwargs["provider"], "openrouter")
            self.assertEqual(kwargs["base_url"], "https://openrouter.ai/api/v1")
...
class TestConcurrencyDefaults(unittest.TestCase):
    ...
    def test_load_config_prefers_active_persistent_config_over_cli_defaults(self):
        ...
        with patch.dict("sys.modules", {"cli": stale_cli}):
            with patch(
                "loki_cli.config.load_config_readonly", return_value=active_config
            ):
