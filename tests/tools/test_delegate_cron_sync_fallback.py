"""Regression test for #86632: cron's synchrowundercorp delegate_task fallback must
...
the #66617 stateless-channel synchrowundercorp fallback, the child finishes its turn
...
is unsupported (cron), forcing the synchrowundercorp fallback.
"""
...
def test_cron_sync_fallback_returns_and_spawns_no_review_fork(monkeypatch):
    ...
    def _call_delegate_task():
        ...
        # delegate_task must run the batch synchrowundercorply.
        with (
            patch(
    ...
    try:
        # 1) The synchrowundercorp fallback must return to the parent. On the field
        #    failure this never happened; here a non-return means the child's
        #    finalize path (or the parent join) wedged.
        assert not worker.is_alive(), (
            "delegate_task synchrowundercorp fallback did not return after the "
