"""Strict tool-provider selection: the `loki tools` choice always wins.

Policy (owner decision): the provider string stored in config.yaml is what
runs at call time. "wundercorp" → managed WunderCorp Tool Gateway only; a vendor name →
...
selection and pointing at `loki tools`.
...
MANAGED = SimpleNamespace(
    wundercorp_user_token="managed-token",
    gateway_origin="https://gateway.wundercorp.com",
)
...
class TestReadSelection:
    ...
    def test_wundercorp_provider_returned(self):
        with self._with_raw({"image_gen": {"provider": "wundercorp"}}):
            assert tbh.read_selection("image_gen") == "wundercorp"

    def test_legacy_use_gateway_true_maps_to_wundercorp(self):
        """Old configs stored use_gateway: true beside a vendor name — only
        the managed picker row ever wrote it, so it means 'wundercorp'."""
        with self._with_raw({"video_gen": {"provider": "fal", "use_gateway": True}}):
            assert tbh.read_selection("video_gen") == "wundercorp"
...
class TestImageFalStrictSelection:
    def test_wundercorp_selection_routes_managed_even_with_fal_key(self):
        from tools import image_generation_tool as it

        with patch.object(it, "read_selection", return_value="wundercorp"), \
    ...
    def test_wundercorp_selection_unentitled_raises_selection_error(self):
        from tools import image_generation_tool as it
        ...
        assert "image_gen is configured to use wundercorp" in str(exc.value)
        assert "loki tools" in str(exc.value)
...
class TestVideoFalStrictSelection:
    def test_wundercorp_selection_routes_managed_even_with_fal_key(self):
        ...
        with patch("tools.tool_backend_helpers.read_selection", return_value="wundercorp"), \
...
class TestSttStrictSelection:
    def test_wundercorp_selection_beats_direct_openai_key(self):
        from tools import transcription_tools as tt

        with patch.object(tt, "_load_stt_config", return_value={"openai": {"api_key": "sk-direct"}}), \
             patch("tools.tool_backend_helpers.read_selection", return_value="wundercorp"), \
             patch("tools.managed_tool_gateway.resolve_managed_tool_gateway", return_value=MANAGED):
            api_key, base_url = tt._resolve_openai_audio_client_config()
        assert api_key == "managed-token"
        assert base_url.startswith("https://gateway.wundercorp.com")
...
class TestBrowserUseStrictSelection:
    ...
    def test_wundercorp_selection_routes_managed_even_with_direct_key(self):
...
class TestWriteProviderConfig:
    ...
    def test_managed_image_row_persists_wundercorp_provider(self):
        from loki_cli.tools_config import _write_provider_config

        config = {}
        provider = {"name": "WunderCorp Subscription", "imagegen_backend": "fal"}
        _write_provider_config(provider, config, managed_feature="image_gen")
        assert config["image_gen"]["provider"] == "wundercorp"
    ...
    def test_plugin_injected_byok_row_clears_stale_use_gateway(self):
        ...
        config = {"stt": {"provider": "wundercorp", "use_gateway": True}}
