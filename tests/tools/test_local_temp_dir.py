...
Loki exposes ``terminal.temp_dir`` (mirrored to ``TERMINAL_TEMP_DIR``) so
...
def test_default_is_loki_cache_not_tmp(tmp_path, monkeypatch):
    """With no overrides at all, the default temp root is real storage under
    LOKI_HOME (cache/terminal), NOT tmpfs /tmp."""
    import loki_constants  # noqa: F401 — resolves LOKI_HOME per call
    ...
    assert result == str(tmp_path / ".loki" / "cache" / "terminal")
...
def test_cleanup_terminal_temp_cache(tmp_path, monkeypatch):
    ...
    assert not (root / "loki_bg_dead1.log").exists()
