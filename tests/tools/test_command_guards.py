...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects LOKI_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("LOKI_INTERACTIVE", "LOKI_GATEWAY_SESSION", "LOKI_EXEC_ASK", "LOKI_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["LOKI_INTERACTIVE"] = "1"
        session_key = os.getenv("LOKI_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No LOKI_INTERACTIVE or LOKI_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
...
class TestGatewayApprovalAllowPermanent:
    ...
    def _capture_gateway_payload(self, command, session_key):
        ...
        def notify(data):
            ...
            # The notify fires synchrowundercorply before _await_gateway_decision
