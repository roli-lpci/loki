...
Tests cover:
- managed_wundercorp_tools_enabled() subscription-based gate
...
from loki_cli.wundercorp_account import WunderCorpPaidServiceAccessInfo, WunderCorpPortalAccountInfo
from tools.tool_backend_helpers import (
    coerce_modal_mode,
    has_direct_modal_credentials,
    managed_wundercorp_tools_enabled,
...
# managed_wundercorp_tools_enabled
# ---------------------------------------------------------------------------
class TestManagedWunderCorpToolsEnabled:
    """Subscription-based gate: True for paid WunderCorp subscribers."""

    def test_disabled_when_not_logged_in(self, monkeypatch):
        monkeypatch.setattr(
            "loki_cli.wundercorp_account.get_wundercorp_portal_account_info",
            lambda: WunderCorpPortalAccountInfo(logged_in=False, source="none", fresh=False),
        )
        assert managed_wundercorp_tools_enabled() is False
...
class TestWunderCorpToolGatewayUnavailableMessage:
    def test_uses_entitlement_reason_for_logged_in_user(self, monkeypatch):
        monkeypatch.setattr(
            "loki_cli.wundercorp_account.get_wundercorp_portal_account_info",
            lambda force_fresh=False: WunderCorpPortalAccountInfo(
                logged_in=True,
                source="account_api",
                fresh=True,
        ...
        message = wundercorp_tool_gateway_unavailable_message("managed image generation")
...
class TestResolveModalBackendState:
    ...
    @staticmethod
    def _resolve(monkeypatch, mode, *, has_direct, managed_ready, wundercorp_enabled=False):
        """Helper to call resolve_modal_backend_state with feature flag control."""
        monkeypatch.setattr(
            "tools.tool_backend_helpers.managed_wundercorp_tools_enabled",
            lambda: wundercorp_enabled,
        )
    ...
    def test_auto_prefers_managed_when_available(self, monkeypatch):
        result = self._resolve(monkeypatch, "auto", has_direct=True, managed_ready=True, wundercorp_enabled=True)
    ...
    def test_direct_selects_direct_when_available(self, monkeypatch):
        result = self._resolve(monkeypatch, "direct", has_direct=True, managed_ready=True, wundercorp_enabled=True)
        assert result["selected_backend"] == "direct"

    def test_direct_none_when_no_credentials(self, monkeypatch):
        result = self._resolve(monkeypatch, "direct", has_direct=False, managed_ready=True, wundercorp_enabled=True)
        assert result["selected_backend"] is None

    # --- managed mode ---


    def test_managed_blocked_when_wundercorp_disabled(self, monkeypatch):
        result = self._resolve(monkeypatch, "managed", has_direct=True, managed_ready=True, wundercorp_enabled=False)
        assert result["selected_backend"] is None
