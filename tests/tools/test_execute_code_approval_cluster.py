...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_env._scrub_child_env — broad LOKI_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: LOKI_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_loki_allowlist_and_secret_blocks():
    from tools.code_execution_env import _scrub_child_env

    env = {
        # operational allowlist → kept
        "LOKI_HOME": "/h", "LOKI_PROFILE": "p",
        "LOKI_CONFIG": "/c.yaml", "LOKI_ENV": "/e",
        "LOKI_DELEGATED_CHILD_CONTEXT": "1",
        # other LOKI_* → dropped (broad prefix removed)
        "LOKI_BASE_URL": "https://x", "LOKI_INTERACTIVE": "1",
    ...
    for kept in (
        ...
        "LOKI_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "LOKI_BASE_URL", "LOKI_INTERACTIVE", "LOKI_KANBAN_TASK",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped LOKI_* vars."""
    import logging
