...
from tools.skills_hub_search import (
    LOKI_INDEX_TTL, _load_loki_index, create_source_router, parallel_search_sources, unified_search,
)
...
class TestParseFrontmatterQuick:
    def test_valid_frontmatter_including_nested_yaml(self):
        ...
        nested = "---\nname: test\nmetadata:\n  loki:\n    tags: [a, b]\n---\n\nBody.\n"
        assert GitHubSource._parse_frontmatter_quick(nested)["metadata"]["loki"]["tags"] == ["a", "b"]
    ...
    def test_browseable_trusted_repos_have_taps(self):
        ...
        for repo in TRUSTED_REPOS:
            assert repo in tap_repos, (
                ...
                "browsable via `loki skills browse`."
            )
...
def _make_index_source(skills):
    """Build a LokiIndexSource pre-loaded with a fixed skill list."""
    from tools.skills_hub_official import LokiIndexSource
    src = LokiIndexSource(auth=GitHubAuth())
    src._index = {"skills": skills}
    src._loaded = True
    return src


class TestLokiIndexSearch:
...
class TestProviderFilter:
    ...
    def test_unified_search_provider_filter_keeps_index_source(self):
        ...
        src = MagicMock()
        src.source_id.return_value = "loki-index"
...
class TestOptionalSkillSourceMetadata:
    def test_scan_all_emits_repo_root_relative_metadata(self, tmp_path):
        ...
        assert meta is not None
        assert meta.repo == "wundercorp/loki"
...
class TestOptionalSkillSourceLiveRepoFallback:
    ...
    searchable and installable without `loki update` (live-repo fallback)."""
...
class TestInstallPathSafety:
    ...
    def test_install_from_quarantine_rejects_category_bucket_overwrite(self, tmp_path):
        ...
        Regression test for GitHub issue #75983: ``loki skills install … --name
...
# ---------------------------------------------------------------------------
# _load_loki_index — centralized index fetch (Browse-hub landing / search)
# ---------------------------------------------------------------------------


class TestLoadLokiIndex:
    ...
    def _isolate_cache(monkeypatch, tmp_path):
        ...
        cache_file = tmp_path / "loki-index.json"
        monkeypatch.setattr("tools.skills_hub_search._loki_index_cache_file", lambda: cache_file)
        return cache_file
    ...
    def test_persistent_decoding_error_falls_back_to_stale_cache(
        ...
        old = time.time() - (LOKI_INDEX_TTL + 100)
        import os
