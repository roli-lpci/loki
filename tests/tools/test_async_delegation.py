...
def _clean_state():
    ...
    # queue (worker threads push events asynchrowundercorply; a drain that races an
...
def _drain_for(delegation_id, timeout=5.0):
    ...
    Completion events are pushed asynchrowundercorply by worker threads, so a
...
def test_real_process_restart_restores_owned_completion_once(tmp_path):
    ...
    env = {**os.environ, "LOKI_HOME": str(tmp_path), "PYTHONPATH": repo}
    producer = r'''
import time
from tools import async_delegation as ad
r = ad.dispatch_async_delegation(
...
def test_delegate_task_background_routes_async_and_does_not_block(monkeypatch):
    ...
    child synchrowundercorply, and the child completes on the background thread.
