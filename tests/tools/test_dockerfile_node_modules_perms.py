...
Older images made /opt/loki/tui-ui and /opt/loki/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV LOKI_TUI_DIR=/opt/loki/tui-ui" in text
    assert "cd ../tui-ui && npm run build" in text
    assert "chown -R loki:loki /opt/loki/tui-ui" not in text
    assert "chown -R loki:loki /opt/loki/node_modules" not in text
