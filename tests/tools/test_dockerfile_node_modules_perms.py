...
Older images made /opt/loki/ui-tui and /opt/loki/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV LOKI_TUI_DIR=/opt/loki/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R loki:loki /opt/loki/ui-tui" not in text
