...
def test_output_path_rejects_loki_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    loki_home = tmp_path / "loki-home"
    loki_home.mkdir()
    monkeypatch.setattr(file_safety, "_loki_home_path", lambda: loki_home)
    monkeypatch.setattr(file_safety, "_loki_root_path", lambda: loki_home)

    target = loki_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    loki_home = tmp_path / "loki-home"
    token_dir = loki_home / "mcp-tokens"
