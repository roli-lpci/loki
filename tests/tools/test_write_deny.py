...
class TestWriteDenyExactPaths:
    ...
    def test_loki_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.loki/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "loki_root"
        ...
        # Sanity check: LOKI_HOME does point to the profile dir, not the root.
        from loki_constants import get_loki_home, get_default_loki_root
...
class TestWriteAllowed:
    ...
    def test_loki_control_files_requested_writable(self):
