...
A manual `cronjob(action='run')` used to execute the job synchrowundercorply on the
...
Sync fallbacks preserved:
  - no routable session (direct Python callers, `loki cron run`)
...
class TestSyncFallbacks:
    ...
    def test_async_delivery_unsupported_falls_back_to_sync(self):
        """One-shot runtimes (loki -z, cron child, Kanban) keep sync."""
        with _bound_session_key():
...
class TestCronjobRunToolIntegration:
    ...
    def test_run_action_sync_path_unchanged_without_session(self):
        """No session context → the legacy synchrowundercorp behavior (executed +
