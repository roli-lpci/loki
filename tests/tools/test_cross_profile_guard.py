...
def fake_loki(tmp_path, monkeypatch):
    """Build a two-profile Loki layout and point LOKI_HOME at
    the loki-security profile (matching the original-incident shape).
    """
    ...
    monkeypatch.setenv("LOKI_HOME", str(sec_home))

    import loki_constants
    monkeypatch.setattr(loki_constants, "get_default_loki_root", lambda: root)

    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_loki_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_loki):
        from tools.file_tools import write_file_tool
        target = fake_loki["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_allowed_guard_retired(self, fake_loki):
        ...
        from tools.file_tools import write_file_tool
        target = fake_loki["root"] / "skills" / "shared-skill" / "SKILL.md"
    ...
    def test_non_loki_path_unaffected(self, fake_loki, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_allowed_guard_retired(self, fake_loki):
    ...
    def test_cross_profile_patch_bypass(self, fake_loki):
        from tools.file_tools import patch_tool
        target = fake_loki["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_writes_through_guard_retired(self, fake_loki):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_loki, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_loki["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up LOKI_HOME (set in
        ...
        assert "not found in active profile 'loki-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.loki/profiles/<name>/."""
        # Don't set LOKI_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_loki_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_loki_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_loki):
        """When active profile is 'loki-security', the prompt warns
        ...
        assert "Active Loki profile" in src
        assert "cross_profile=True" not in src  # guard retired
        assert "~/.loki/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Loki profile: default" in src
        assert "Active Loki profile: {active_profile}" in src
