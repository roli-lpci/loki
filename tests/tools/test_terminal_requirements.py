...
def _clear_terminal_env(monkeypatch):
    ...
    # Default: no WunderCorp subscription — patch both the terminal_tool local
    # binding and tool_backend_helpers (used by resolve_modal_backend_state).
    monkeypatch.setattr(terminal_tool_module, "managed_wundercorp_tools_enabled", lambda: False)
    import tools.tool_backend_helpers as _tbh
    monkeypatch.setattr(_tbh, "managed_wundercorp_tools_enabled", lambda: False)
    monkeypatch.setattr("tools.terminal_tool_backends.managed_wundercorp_tools_enabled", lambda: False)


def test_local_terminal_requirements(monkeypatch, caplog):
    """Local backend uses Loki' own LocalEnvironment wrapper."""
...
def test_modal_backend_managed_mode_without_feature_flag_logs_clear_error(monkeypatch, caplog, tmp_path):
    ...
    assert ok is False
    assert any(
        "WunderCorp Tool Gateway access is not currently available" in record.getMessage()
        for record in caplog.records
    )
