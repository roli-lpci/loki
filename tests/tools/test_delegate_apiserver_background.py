...
Previously async_delivery_supported()=False forced SYNCHROWUNDERCORP execution for
...
NOT from LOKI_SESSION_ID: constructing a child agent calls
set_current_session_id(child.session_id), clobbering the LOKI_SESSION_ID
...
def _clean_queue_and_context(monkeypatch):
    ...
    os.environ.pop("LOKI_SESSION_ID", None)
    while not process_registry.completion_queue.empty():
        try:
            process_registry.completion_queue.get_nowait()
        except Exception:
...
def _patch_delegate(monkeypatch):
    ...
    def clobbering_build_child(**kw):
        ...
        # LOKI_SESSION_ID ContextVar + os.environ, clobbering the spawner's
...
def test_apiserver_session_with_id_dispatches_background(monkeypatch):
    """async_delivery=False + a raw session id (LOKI_SESSION_ID) →
    ...
    monkeypatch.setenv("LOKI_SESSION_ID", "raw-sid-7")
    set_session_vars(
        platform="api_server",
        chat_id="raw-sid-7",
    ...
    # LOKI_SESSION_ID with (see clobbering_build_child).
...
def test_apiserver_session_without_id_stays_synchrowundercorp(monkeypatch):
    ...
    assert "SYNCHROWUNDERCORPLY" in parsed.get("note", "")
