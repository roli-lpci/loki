...
Covers:
- :class:`LokiTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.LokiMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _LOKI_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# LokiTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = LokiTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("LOKI_HOME", str(tmp_path))
        storage = LokiTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (LokiMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: LokiTokenStorage, **context_attrs):
    ...
    provider = _LOKI_PROVIDER_CLS.__new__(_LOKI_PROVIDER_CLS)
    provider._loki_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LOKI_HOME", str(tmp_path))
        storage = LokiTokenStorage("mgr-srv")
        ...
        with patch.object(
            _LOKI_PROVIDER_CLS.__bases__[-1], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LOKI_HOME", str(tmp_path))
        storage = LokiTokenStorage("flow-srv")
        ...
        with patch.object(
            _LOKI_PROVIDER_CLS.__bases__[-1],
            "async_auth_flow",
