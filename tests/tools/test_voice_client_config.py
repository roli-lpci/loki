...
Real config files in a temp LOKI_HOME, real resolution chains (no mocked
...
def voice_home(tmp_path, monkeypatch):
    """Isolated LOKI_HOME + reloaded config modules; yields a config writer."""
    home = tmp_path / ".loki"
    home.mkdir()
    ...
    for var in (
        ...
        "DEEPINFRA_API_KEY", "LOKI_LOCAL_STT_LANGUAGE",
    ):
