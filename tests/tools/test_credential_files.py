...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        loki_home.mkdir()
        (loki_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"LOKI_HOME": str(loki_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(loki_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.loki/token.json"
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
        (loki_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.loki/skills"


    def test_custom_container_base(self, tmp_path):
        loki_home = tmp_path / ".loki"
        (loki_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"LOKI_HOME": str(loki_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.loki")

        assert mounts[0]["container_path"] == "/home/user/.loki/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.loki/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.loki/skills/cat/myskill/scripts/run.sh" in paths
    ...
    def test_skips_excluded_bookkeeping_dirs(self, tmp_path):
        ...
        paths = {f["container_path"] for f in files}
        assert "/root/.loki/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.loki/skills/cat/myskill/references/api.md" in paths
        for excluded in (
            ".hub",
            ".archive",
            ".curator_backups",
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape LOKI_HOME."""
        monkeypatch.setenv("LOKI_HOME", str(tmp_path / ".loki"))
        (tmp_path / ".loki").mkdir()

        # Create a sensitive file one level above loki_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside loki_home
    ...
    def test_nested_subdir_inside_loki_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of LOKI_HOME must be allowed."""
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
        subdir = loki_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside LOKI_HOME pointing outside must be rejected."""
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
        monkeypatch.setenv("LOKI_HOME", str(loki_home))

        # Create a sensitive file outside loki_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside loki_home pointing outside
        symlink = loki_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes LOKI_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, loki_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape LOKI_HOME."""
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
        monkeypatch.setenv("LOKI_HOME", str(loki_home))
        ...
        sensitive.write_text("{}")
        self._write_config(loki_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(loki_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside LOKI_HOME via config must still mount."""
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
        monkeypatch.setenv("LOKI_HOME", str(loki_home))

        (loki_home / "oauth.json").write_text("{}")
        self._write_config(loki_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
        (loki_home / "cache" / "documents").mkdir(parents=True)
        (loki_home / "cache" / "audio").mkdir(parents=True)
        (loki_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("LOKI_HOME", str(loki_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.loki/cache/documents" in paths
        assert "/root/.loki/cache/audio" in paths
        assert "/root/.loki/cache/videos" in paths
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_loki_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        loki_home = tmp_path / ".loki"
        loki_home.mkdir()
        # Use legacy dir name with content — get_loki_dir prefers
        # populated old over new.
        legacy_doc = loki_home / "document_cache"
        legacy_img = loki_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(loki_home / "document_cache") in host_paths
        assert str(loki_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.loki/cache/documents" in container_paths
        assert "/root/.loki/cache/images" in container_paths
    ...
    def test_empty_loki_home(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.loki/attachments" in container_paths
        assert "/root/.loki/images" in container_paths
        assert "/root/.loki/cache/images" in container_paths
        for mount in mounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``LOKI_HOME/images``, not
        ...
        """
        loki_home = tmp_path / ".loki"
        (loki_home / "images").mkdir(parents=True)
        monkeypatch.setenv("LOKI_HOME", str(loki_home))

        mounts = get_cache_directory_mounts()
        by_container = {m["container_path"]: m["host_path"] for m in mounts}
        assert "/root/.loki/images" in by_container
        assert by_container["/root/.loki/images"] == str(loki_home / "images")
    ...
    def test_images_upload_file_maps_into_container(self, tmp_path, monkeypatch):
        ...
        loki_home = tmp_path / ".loki"
        (loki_home / "images").mkdir(parents=True)
        upload = loki_home / "images" / "upload_20260722_181019_1.png"
        upload.write_bytes(bytes.fromhex("89504e470d0a1a0a"))
        monkeypatch.setenv("LOKI_HOME", str(loki_home))

        assert (
            map_cache_path_to_container(str(upload))
            == "/root/.loki/images/upload_20260722_181019_1.png"
        )
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        loki_home = tmp_path / ".loki"
        img_dir = loki_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.loki/cache/images/generated.png"
        )
    ...
    def test_maps_path_even_when_cache_dir_missing(self, tmp_path, monkeypatch):
        ...
        mapped = map_cache_path_to_container(str(loki_home / "cache" / "images" / "x.png"))
        assert mapped == "/root/.loki/cache/images/x.png"
        assert (loki_home / "cache" / "images").is_dir()


class TestToAgentVisiblePathPerBackend:
    """#76577 follow-up: translation covers every backend that relocates the
    Loki cache — not just docker — and skips the ones where the host path
    stays correct (local; singularity auto-binds the host home)."""

    def _staged(self, tmp_path, monkeypatch):
        loki_home = tmp_path / ".loki"
        (loki_home / "attachments").mkdir(parents=True)
        monkeypatch.setenv("LOKI_HOME", str(loki_home))
        return str(loki_home / "attachments" / "drop.zip")

    def test_docker_maps_to_root_loki(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "/root/.loki/attachments/drop.zip"

    def test_ssh_maps_to_tilde_loki(self, tmp_path, monkeypatch):
        staged = self._staged(tmp_path, monkeypatch)
        monkeypatch.setenv("TERMINAL_ENV", "ssh")
        from tools.credential_files import to_agent_visible_cache_path
        assert to_agent_visible_cache_path(staged) == "~/.loki/attachments/drop.zip"
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        loki_home = tmp_path / ".loki"
        doc_dir = loki_home / "cache" / "documents"
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — LOKI_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping LOKI_HOME, but every master
    ...
    def test_master_credential_store_is_refused(self, tmp_path, rel_path):
        ...
        with patch.dict(os.environ, {"LOKI_HOME": str(home)}):
    ...
    def test_refused_entry_does_not_block_the_rest_of_the_batch(self, tmp_path):
        ...
        paths = [m["container_path"] for m in mounts]
        assert "/root/.loki/google_token.json" in paths
        assert "/root/.loki/.env" not in paths
