...
def _enable_managed_wundercorp_tools(monkeypatch):
    """Ensure managed_wundercorp_tools_enabled() returns True even after module reloads.
    ...
    loki_cli.wundercorp_account — so the function body returns True.
    """
    monkeypatch.setattr(
...
def _install_fake_tools_package():
    ...
    # loki_cli.auth (imported transitively by wundercorp_account /
...
def test_browser_use_explicit_local_mode_stays_local_even_when_managed_gateway_is_ready(tmp_path):
    ...
    env.update({
        ...
        "TOOL_GATEWAY_USER_TOKEN": "wundercorp-token",
