...
def _exec_py(image: str, py: str) -> str:
    """Run a Python snippet inside the image as the loki user, return stdout."""
    inner = (
        "source /opt/loki/.venv/bin/activate && "
        "cd /opt/loki && "
        f"python3 -c {shlex.quote(py)}"
    )
    # Drop to the loki user (UID 10000) so we exercise the same path the
    # dashboard PTY child runs as — not root.
    cmd = [
        "docker", "run", "--rm", "--entrypoint", "su", image,
        "loki", "-s", "/bin/bash", "-c", inner,
    ]
...
def test_loki_tui_dir_env_is_set(built_image: str) -> None:
    """LOKI_TUI_DIR must point at the prebuilt bundle dir in the image."""
    r = subprocess.run(
        ["docker", "run", "--rm", "--entrypoint", "sh", built_image,
         "-c", 'printf "%s" "$LOKI_TUI_DIR"'],
