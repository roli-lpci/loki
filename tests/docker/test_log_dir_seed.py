"""Runtime smoke test for Docker $LOKI_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the loki user after container boot.
...
registered later runs its log service as the dropped loki user and
...
def test_logs_gateways_seeded_and_loki_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by loki after boot."""
    ...
    assert "logs=loki" in r.stdout, (
        f"logs/ not owned by loki: {r.stdout}"
    )
    assert "gateways=loki" in r.stdout, (
        f"logs/gateways/ not owned by loki: {r.stdout}"
    )
