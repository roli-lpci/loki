...
Override the image with ``LOKI_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("LOKI_TEST_IMAGE", "loki-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``LOKI_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("LOKI_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged loki user
# ---------------------------------------------------------------------------
#
# Background: every Loki runtime path inside the container drops to UID
# 10000 (the ``loki`` user) via ``s6-setuidgid loki``. ``docker exec``
...
# fails (PermissionError swallowed) for loki, so a test that ran as root
...
# helpers below run every probe under ``-u loki`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: loki).
