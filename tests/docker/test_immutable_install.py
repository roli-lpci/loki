...
Build the real image and verify at runtime:
  ...
  1. /opt/loki is not writable by the loki user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and LOKI_DISABLE_LAZY_INSTALLS are set
  3. /opt/loki/.install_method contains "docker" (code-scoped stamp)
  4. $LOKI_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $LOKI_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_loki(
    built_image: str, container_name: str,
) -> None:
    """The loki user must not be able to modify /opt/loki.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/loki as the loki user
        "touch /opt/loki/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"loki user can write to /opt/loki (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/loki/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"loki user can write to /opt/loki/.venv: {r.stdout}"
    )


def test_loki_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    LOKI_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $LOKI_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $LOKI_HOME was not healed on boot: "
        f"{r.stdout}"
    )
