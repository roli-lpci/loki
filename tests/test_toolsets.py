...
class TestToolsetConsistency:
    ...
    def test_loki_platforms_share_core_tools(self):
        """All loki-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on loki-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["loki-cli", "loki-telegram", "loki-discord", "loki-whatsapp", "loki-slack", "loki-signal", "loki-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_loki_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("loki-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("loki-cli")
        second = resolve_toolset("loki-cli")
        ...
        assert (
            "loki-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("loki-cli", include_registry=False)
        second = resolve_toolset("loki-cli", include_registry=False)
        assert first == second
