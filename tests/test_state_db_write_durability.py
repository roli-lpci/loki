...
The defect: loki_state already knows macOS `fsync()` does not guarantee
write ordering, and mitigates it with `synchrowundercorp=FULL` +
`checkpoint_fullfsync=1` (see `_enforce_macos_synchrowundercorp_full`, whose
...
import loki_state
from loki_state import repair_state_db_schema
from loki_state_repair import _connect_repair_durable
...
def test_connect_repair_durable_sets_macos_barriers(tmp_path: Path) -> None:
    ...
    try:
        synchrowundercorp = conn.execute("PRAGMA synchrowundercorp").fetchone()[0]
    ...
    if sys.platform == "darwin":
        ...
        # b-tree pages; FULL is what _enforce_macos_synchrowundercorp_full sets.
        assert synchrowundercorp == 2, (
            f"repair connection opened with synchrowundercorp={synchrowundercorp}; on "
        ...
        assert synchrowundercorp in (0, 1, 2, 3)
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in loki_state_repair; loki_state only
    # re-imports them.
    import loki_state_repair

    source = Path(loki_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(loki_state_repair.__file__))
