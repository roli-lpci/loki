...
def test_root_session_no_compression(db):
    ...
    assert prov["currentLokiSessionId"] == "root1"
    assert prov["rootLokiSessionId"] == "root1"
    assert prov["parentLokiSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_loki_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentLokiSessionId"] == "old"
    assert prov["rootLokiSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousLokiSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootLokiSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"loki"}
    assert "sessionProvenance" in meta["loki"]
    assert meta["loki"]["sessionProvenance"]["currentLokiSessionId"] == "root1"
