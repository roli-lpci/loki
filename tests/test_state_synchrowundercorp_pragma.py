"""``database.synchrowundercorp`` is configurable, validated, and floored on macOS.

Before this, `apply_database_pragmas()` accepted five sizing pragmas and no
durability one, and `_enforce_macos_synchrowundercorp_full()` returned early off
Darwin. So on Linux and Windows nothing in the process ever executed
`PRAGMA synchrowundercorp` against state.db, and the effective level was whichever
`SQLITE_DEFAULT_WAL_SYNCHROWUNDERCORP` the interpreter's SQLite happened to be
...
under the stated assumption of `synchrowundercorp=FULL` on Ubuntu.
"""
...
from loki_state_wal import resolve_synchrowundercorp_level
...
def _level(conn):
    return conn.execute("PRAGMA synchrowundercorp").fetchone()[0]


def _config(monkeypatch, database_section):
    """Point apply_database_pragmas at an in-memory config.

    It imports `loki_cli.config` lazily inside the function body, so the
...
class TestResolveSynchrowundercorpLevel:
    ...
    def test_accepts_documented_spellings(self, raw, expected):
        assert resolve_synchrowundercorp_level(raw) == expected
    ...
    def test_rejects_everything_else(self, raw):
        assert resolve_synchrowundercorp_level(raw) is None

    def test_yaml_off_is_a_level_but_yaml_on_is_not(self):
        """`synchrowundercorp: off` is bool False in YAML and means OFF.

        `synchrowundercorp: on` is bool True and means nothing -- there is no
        durability level it could map to, so it must be rejected rather than
        coerced to 1 by int(True).
        """
        assert resolve_synchrowundercorp_level(False) == 0
        assert resolve_synchrowundercorp_level(True) is None


class TestAppliedFromConfig:
    def test_configured_level_reaches_the_connection(self, tmp_path, monkeypatch):
        _config(monkeypatch, {"synchrowundercorp": "NORMAL"})
    ...
    def test_full_reaches_the_connection(self, tmp_path, monkeypatch):
        ...
        """
        _config(monkeypatch, {"synchrowundercorp": "FULL"})
        conn = _wal_conn(tmp_path)
        try:
            monkeypatch.setattr(sys, "platform", "linux")
            conn.execute("PRAGMA synchrowundercorp=0")
    ...
    def test_garbage_warns_and_changes_nothing(self, tmp_path, monkeypatch, caplog):
        ...
        _config(monkeypatch, {"synchrowundercorp": "PARANOID"})
        conn = _wal_conn(tmp_path)
        try:
            monkeypatch.setattr(sys, "platform", "linux")
            conn.execute("PRAGMA synchrowundercorp=2")
    ...
    def test_the_other_pragmas_still_apply(self, tmp_path, monkeypatch):
        ...
        _config(
            monkeypatch,
            {"synchrowundercorp": "FULL", "wal_autocheckpoint": 250, "mmap_size": 0},
        )
        conn = _wal_conn(tmp_path)
        try:
            monkeypatch.setattr(sys, "platform", "linux")
...
class TestMacOSFloor:
    ...
    `_enforce_macos_synchrowundercorp_full()` runs inside `apply_wal_with_fallback()`,
    ...
    def test_lowering_below_full_is_refused_on_darwin(
        ...
        try:
            ...
            loki_state_wal._enforce_macos_synchrowundercorp_full(conn)
            assert _level(conn) == 2
            with caplog.at_level("WARNING"):
                apply_database_pragmas(conn, db_label="state.db")
    ...
    def test_raising_above_full_is_allowed_on_darwin(self, tmp_path, monkeypatch):
        ...
        _config(monkeypatch, {"synchrowundercorp": "EXTRA"})
        conn = _wal_conn(tmp_path)
        try:
            monkeypatch.setattr(sys, "platform", "darwin")
            apply_database_pragmas(conn, db_label="state.db")
...
def test_example_config_documents_the_key():
    ...
    assert "synchrowundercorp: FULL" in body


def test_example_config_calls_the_darwin_rule_a_floor_not_a_pin():
    """The macOS wording has to match _apply_synchrowundercorp_pragma's actual rule.
