...
- files inside the Loki home tree are repaired with ``chmod u+rw``
...
import loki_state
from loki_state import SessionDB, preflight_db_writability
...
def loki_home(tmp_path, monkeypatch):
    """Isolated LOKI_HOME so the repair scope covers tmp DBs."""
    home = tmp_path / ".loki"
    home.mkdir()
...
class TestRepairScope:
    def test_repairs_readonly_db_inside_home(self, loki_home):
        db = loki_home / "state.db"
        _make_db(db)
        os.chmod(db, 0o444)
    ...
    def test_repairs_readonly_sidecars(self, loki_home):
        db = loki_home / "state.db"
        _make_wal_db(db)
    ...
    def test_repairs_readonly_parent_directory(self, loki_home):
        sub = loki_home / "kanban"
        sub.mkdir()
        db = sub / "kanban.db"
        _make_db(db)
        os.chmod(sub, 0o555)
...
class TestRefusalOutsideScope:
    def test_actionable_error_names_file_and_chmod(self, loki_home, tmp_path):
    ...
    def test_wal_error_warns_against_deletion(self, loki_home, tmp_path):
        outside = tmp_path / "elsewhere"
        outside.mkdir()
        db = outside / "custom.db"
        _make_wal_db(db)
...
class TestSkips:
    ...
    def test_healthy_db_untouched(self, loki_home):
        db = loki_home / "state.db"
        _make_db(db)
...
class TestSessionDBIntegration:
    def test_sessiondb_selfheals_readonly_db_in_home(self, loki_home):
        db_path = loki_home / "state.db"
