...
from loki_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(loki_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <loki_home>/plugins/<name> and opt it in."""
    plugin_dir = loki_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LOKI_HOME", str(tmp_path / "loki_empty"))
