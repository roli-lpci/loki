...
developer's REAL ~/.loki/state.db, and a pytest-spawned process flipped
...
# Must match the root the guard itself computes.  Hardcoding ``~/.loki``
# silently disarmed every assertion below on Windows, where the real root is
# ``%LOCALAPPDATA%\loki``: the paths under test were then *correctly*
...
REAL_ROOT = loki_state._real_platform_state_root()
...
class TestProductionPathRefused:
    def test_explicit_production_db_path_raises(self):
        """SessionDB pointed at the real ~/.loki/state.db must fail hard."""
    ...
    def test_default_resolution_to_production_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(
            loki_state, "DEFAULT_DB_PATH", loki_state._IMPORT_DEFAULT_DB_PATH
        )
        with pytest.raises(RuntimeError, match="live-system guard"):
            SessionDB()
...
class TestHermeticPathsAllowed:
    ...
    def test_tmp_loki_home_default_resolution_works(self, tmp_path, monkeypatch):
        """Argless SessionDB() under a hermetic LOKI_HOME must succeed."""
        monkeypatch.setenv("LOKI_HOME", str(tmp_path / "hermetic-home"))
        monkeypatch.setattr(
            loki_state, "DEFAULT_DB_PATH", loki_state._IMPORT_DEFAULT_DB_PATH
        )
        db = SessionDB()
...
class TestBypassMarker:
    ...
    def test_bypass_marker_disables_state_db_guard(self):
        ...
        """
        loki_state._ensure_test_isolation(REAL_ROOT / "state.db")
...
class TestSessionStoreLoudFailure:
    def test_guard_error_is_not_swallowed_into_jsonl_fallback(
        ...
        monkeypatch.setattr(loki_state, "SessionDB", _boom)
        with pytest.raises(RuntimeError, match="live-system guard"):
...
class TestSubprocessChildCovered:
    def test_child_without_loki_home_is_refused(self, tmp_path):
        """A subprocess child of a test (no LOKI_HOME) must be blocked.
        ...
        env = {
            ...
            if k not in ("LOKI_HOME", "PYTEST_PLUGINS", "PYTHONPATH")
        }
    ...
    def test_child_with_tmp_loki_home_succeeds(self, tmp_path):
        """Same child, hermetic LOKI_HOME: must work — no false positive."""
        env = {
            k: v
            for k, v in os.environ.items()
        ...
        env["PYTEST_CURRENT_TEST"] = "tests/fake.py::test_child (call)"
        env["LOKI_HOME"] = str(tmp_path / "child-home")
        env["PYTHONPATH"] = str(Path(__file__).resolve().parents[2])
        code = (
            "from loki_state import SessionDB\n"
            "db = SessionDB()\n"
