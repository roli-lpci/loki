...
entirely on the process environment: ``LOKI_HOME`` says *where* to write and
...
# hardcoded ~/.loki — that root is %LOCALAPPDATA%\loki on Windows) and
...
root = loki_state._real_platform_state_root()
if root is None:
    print("NO-ROOT")
else:
    try:
        loki_state._ensure_test_isolation(root / "state.db")
    except RuntimeError:
        print("REFUSED")
    else:
        print("ALLOWED")
...
def _scrubbed_env(**overrides):
    ...
    Also strips ``LOKI_TEST_ISOLATION`` — the conftest-exported marker
    ...
    env = {
        ...
        and k not in ("LOKI_HOME", "LOKI_TEST_ISOLATION")
    }
...
class TestScrubbedChildEnvironment:
    def test_child_without_pytest_env_still_refuses_production_db(self):
        """The #82770 escape: no PYTEST_* and no LOKI_HOME, yet still a test.

        This is the exact shape of the leak — the child resolves the real
        ``state.db`` because ``LOKI_HOME`` is gone, and the env-only guard
    ...
    def test_env_bypass_lets_a_deliberate_child_through(self):
        ...
        """
        env = _scrubbed_env(**{loki_state._STATE_DB_GUARD_BYPASS_ENV: "1"})
...
class TestPytestProcessRecognition:
    ...
    def test_recognises_pytest_invocations(self, cmdline):
        assert loki_state_guard._process_looks_like_pytest(self._FakeProc(cmdline))

    @pytest.mark.parametrize(
        "cmdline",
        [
            ["loki", "gateway", "start"],
            ["/usr/bin/python", "-m", "loki_cli.main", "sessions", "list"],
            # A path that merely *contains* "pytest" is not a pytest process:
            # tmp paths like /tmp/pytest-of-dev/... show up in real argv.
            ["loki", "run", "--file", "/tmp/pytest-of-dev/test0/input.txt"],
        ],
    )
    def test_ignores_non_pytest_invocations(self, cmdline):
        assert not loki_state_guard._process_looks_like_pytest(self._FakeProc(cmdline))
    ...
    def test_unreadable_process_is_not_pytest(self):
        ...
        assert not loki_state_guard._process_looks_like_pytest(_Denied())
