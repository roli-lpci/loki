...
work on a file several Loki processes share (gateway service, the Desktop
app's `loki serve` backend, CLI sessions, the TUI slash worker):

* `loki_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `loki_state_repair._cross_process_repair_lock`   — writable_schema surgery / VACUUM
