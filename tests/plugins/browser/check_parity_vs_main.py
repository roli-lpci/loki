...
Run from the PR worktree:
    ...
    cd ~/.loki/loki-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.loki/loki-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.loki/loki-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a loki-agent checkout"
)
...
# Isolated LOKI_HOME for the config write.
