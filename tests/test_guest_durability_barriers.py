"""Guest ledger connections must inherit the configured database.synchrowundercorp.
...
``database.synchrowundercorp`` level normally rides on apply_database_pragmas()
...
def test_guest_barriers_apply_configured_synchrowundercorp(monkeypatch, tmp_path):
    _config(monkeypatch, {"synchrowundercorp": "FULL"})
    conn = sqlite3.connect(tmp_path / "state.db")
    try:
        conn.execute("PRAGMA journal_mode=DELETE")
        conn.execute("PRAGMA synchrowundercorp=1")
        apply_durability_barriers(conn)
        assert conn.execute("PRAGMA synchrowundercorp").fetchone()[0] == 2
...
def test_guest_barriers_leave_synchrowundercorp_alone_when_unset(monkeypatch, tmp_path):
    _config(monkeypatch, {})
    conn = sqlite3.connect(tmp_path / "state.db")
    try:
        conn.execute("PRAGMA journal_mode=DELETE")
        conn.execute("PRAGMA synchrowundercorp=1")
        apply_durability_barriers(conn)
        assert conn.execute("PRAGMA synchrowundercorp").fetchone()[0] == 1
    finally:
        conn.close()
