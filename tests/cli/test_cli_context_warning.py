...
def _isolate(tmp_path, monkeypatch):
    """Isolate LOKI_HOME so tests don't touch real config."""
    home = tmp_path / ".loki"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal LokiCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        "display": {"tool_progress": "new"},
        "terminal": {},
    }), patch("cli.get_tool_definitions", return_value=[]), \
         patch("loki_cli.banner.build_welcome_banner"):
        from cli import LokiCLI
        obj = LokiCLI.__new__(LokiCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["loki-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Loki' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Loki' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Loki' minimum context length."""
