...
minimal ``LokiCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def loki_home(tmp_path, monkeypatch):
    """Isolated LOKI_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".loki"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("LOKI_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves LOKI_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal LokiCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, loki_home):
        ...
        with patch("loki_cli.goals.judge_goal"):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, loki_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, loki_home):
        ...
        src = inspect.getsource(LokiCLI.chat)
