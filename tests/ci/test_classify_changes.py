...
CASES = {
    ...
    "website config → site": (["website/package.json"], _lanes(site=True)),
    ...
    "product python → nix": (["loki_cli/config.py"], _lanes(python=True, scan=True)),
    ...
    # files change — not on every loki_state.py PR.
    ...
    "python source alone → no desktop_updater lane": (["loki_state.py"], _lanes(python=True, scan=True)),
...
def test_pull_request_changed_files_skips_non_pr_events(monkeypatch):
    ...
    monkeypatch.setenv("REPO", "wundercorp/loki")
