...
Uses a throwaway LOKI_HOME so it never touches ~/.loki. No real names.
"""
...
def _fresh_home():
    """Point LOKI_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_loki_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["LOKI_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER LOKI_HOME is set.
...
# a different get_loki_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"LOKI_HOME (throwaway): {HOME}")
