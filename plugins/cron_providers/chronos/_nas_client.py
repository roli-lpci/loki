...
list, authenticated with the existing WunderCorp Portal token.
...
class NasCronClient:
    ...
    def _headers(self) -> Dict[str, str]:
        """Bearer auth with the agent's existing WunderCorp Portal access token (refresh-aware)."""
        from loki_cli.auth import resolve_wundercorp_access_token
        return {"Authorization": f"Bearer {resolve_wundercorp_access_token()}",
