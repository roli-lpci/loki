"""Loki Achievements dashboard plugin backend, mounted at /api/plugins/loki-achievements/.
...
ACHIEVEMENTS: List[Dict[str, Any]] = [
    ...
    _ach("let_him_cook", "Let Him Cook", "Let Loki run a serious autonomous tool chain in one session.", "Agent Autonomy", "flame", metric="max_tool_calls_in_session", tiers=[200, 500, 1200, 3000, 8000]),
    _ach("autonomous_avalanche", "Autonomous Avalanche", "Accumulate a lifetime avalanche of Loki tool calls across sessions.", "Agent Autonomy", "avalanche", metric="total_tool_calls", tiers=[1000, 3000, 8000, 20000, 50000]),
    _ach("toolchain_maxxer", "Toolchain Maxxer", "Use a wide spread of distinct Loki tools in one session.", "Agent Autonomy", "nodes", metric="max_distinct_tools_in_session", tiers=[18, 28, 45, 70, 100]),
    ...
    # Loki Native
    _ach("skillsmith", "Skillsmith", "Work with Loki skills enough to leave fingerprints.", "Loki Native", "hammer_scroll", metric="skill_events", tiers=[5000, 15000, 40000, 100000, 250000]),
    _ach("skill_issue_skill_created", "Skill Issue? Skill Created.", "Create or patch durable procedures instead of repeating yourself.", "Loki Native", "anvil", metric="skill_manage_events", tiers=[25, 75, 200, 600, 1500]),
    _ach("memory_keeper", "Memory Keeper", "Persist durable knowledge with memory or Mnemosyne.", "Loki Native", "crystal", metric="memory_events", tiers=[100, 300, 1000, 3000, 8000]),
    _ach("memory_palace", "Memory Palace", "Build a serious durable-memory trail.", "Loki Native", "palace", metric="memory_write_events", tiers=[100, 300, 1000, 3000, 8000]),
    _ach("context_dragon", "Context Dragon", "Brush against compression, huge context, or token pressure repeatedly.", "Loki Native", "dragon", metric="context_events", tiers=[5000, 15000, 40000, 100000, 250000]),
    _ach("gateway_dweller", "Gateway Dweller", "Live through gateway-connected Loki workflows.", "Loki Native", "antenna", metric="gateway_events", tiers=[5000, 15000, 40000, 100000, 250000]),
    _ach("plugin_goblin", "Plugin Goblin", "Use or develop plugins enough that the dashboard notices.", "Loki Native", "puzzle", metric="plugin_events", tiers=[1000, 3000, 8000, 20000, 50000]),
    _ach("rollback_wizard", "Rollback Wizard", "Invoke rollback/checkpoint recovery magic.", "Loki Native", "rewind", metric="rollback_events", tiers=[500, 1500, 4000, 10000, 25000], secret=True),
    ...
    _ach("multi_model_mage", "Multi-Model Mage", "Use a real spread of distinct model names across Loki history.", "Model Lore", "prism", metric="distinct_model_count", tiers=[10, 20, 40, 80, 160]),
    ...
    _ach("provider_polyglot", "Provider Polyglot", "Use models from multiple providers across Loki history.", "Model Lore", "swap", metric="distinct_provider_count", tiers=[2, 3, 5, 8, 12]),
    ...
    _ach("open_weights_pilgrim", "Open Weights Pilgrim", "Actually chat with local/open-weight models through Loki session metadata.", "Model Lore", "terminal", metric="local_model_chat_sessions", tiers=[1, 3, 10, 30, 100]),

    # Workflow Intelligence
    _ach("toolset_cartographer", "Toolset Cartographer", "Navigate Loki toolsets deliberately instead of treating tools as a blur.", "Loki Native", "compass", metric="toolset_events", tiers=[20, 60, 200, 600, 1500]),
    _ach("config_surgeon", "Config Surgeon", "Operate on real config files, manifests, env files, and dashboard settings without flinching.", "Loki Native", "key", metric="config_events", tiers=[100, 300, 1000, 3000, 10000]),
    ...
    # Lifestyle
    _ach("marathon_operator", "Marathon Operator", "Accumulate a serious number of Loki sessions.", "Lifestyle", "marathon", metric="session_count", tiers=[75, 200, 500, 1500, 5000]),
    _ach("weekend_warrior", "Weekend Warrior", "Run Loki on weekends enough times to make it a lifestyle.", "Lifestyle", "calendar", metric="weekend_sessions", tiers=[25, 75, 200, 600, 1500]),
...
def _data_dir() -> Path:
    """Durable data root (``<loki home>/plugin-data/loki-achievements/``). State used to
    live in the install tree and died on ``loki plugins remove``/``update``; legacy files
    migrate on first read (see ``_data_file``)."""
    try:
        from plugins.plugin_storage import plugin_data_dir
        return plugin_data_dir("loki-achievements")
    except Exception:
        # Standalone dashboard import (no plugins package on sys.path): same layout, computed locally.
        root = get_loki_home() / "plugin-data" / "loki-achievements"
...
def _data_file(name: str) -> Path:
    path = _data_dir() / name
    if not path.exists():
        legacy = get_loki_home() / "plugins" / "loki-achievements" / name
        if legacy.exists():
            try:
...
_PROVIDER_MARKERS = ["openai", "anthropic", "google", "gemini", "mistral", "meta", "qwen", "deepseek", "xai", "wundercorp", "ollama", "groq", "openrouter", "codex"]
...
def analyze_messages(session_id: str, title: str, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "plugin_events": hits(r"plugin|dashboard-plugins|__LOKI_PLUGIN|manifest\.json"),
        "rollback_events": hits(r"rollback|checkpoint"),
        "docs_activity_events": hits(r"docs|documentation|doku|README"),
...
METRIC_LABELS = {
    ...
    "max_distinct_tools_in_session": "distinct Loki tools used in one session",
    ...
    "skill_events": "Loki skill mentions or tool use",
    ...
    "total_tool_calls": "lifetime Loki tool calls",
    ...
    "local_model_chat_sessions": "Loki sessions whose model metadata is local/open-weight",
...
def criteria_for(definition: Dict[str, Any]) -> str:
    if definition.get("secret") and definition.get("state") == "secret":
        return "Secret: exact requirement hidden until Loki sees the first matching signal. Keep using Loki across debugging, tools, memory, skills, plugins, and model workflows to reveal it."
    if "threshold_metric" in definition:
        tiers_list = sorted(definition.get("tiers", []), key=lambda t: t["threshold"])
        if not tiers_list:
            return "Requirement: use Loki in the matching workflow."
    ...
    return "Requirement: complete the matching Loki behavior."


def display_achievement(item: Dict[str, Any]) -> Dict[str, Any]:
    clean = dict(item)
    if clean.get("state") == "secret":
        return {**clean, "name": "???", "description": "Secret achievement: hidden until Loki detects the first relevant behavior in your session history.", "criteria": criteria_for(clean), "icon": "secret"}
...
def scan_sessions(limit: Optional[int] = None, progress_callback: Optional[Any] = None, progress_every: int = 250) -> Dict[str, Any]:
    """Scan Loki sessions and build per-session achievement stats.
...
def _run_scan_and_update_cache(publish_partial_snapshots: bool = True) -> None:
    """Execute a scan + snapshot update (synchrowundercorply or from a thread). With
    ...
    incrementally; synchrowundercorp /rescan callers pass ``False`` since they block on the result."""
    with _SCAN_LOCK:
...
def _start_background_scan() -> None:
    ...
    with _BACKGROUND_SCAN_LOCK:
        ...
        if existing is not None and existing.is_alive():
            return
        thread = threading.Thread(target=_run_scan_and_update_cache, kwargs={"publish_partial_snapshots": True}, name="loki-achievements-scan", daemon=True)
...
def evaluate_all(force: bool = False) -> Dict[str, Any]:
    ...
    while the first scan runs; ``force=True`` (manual /rescan) scans synchrowundercorply. Cold
