...
Chat REST API (synchrowundercorp googleapiclient via ``asyncio.to_thread``). The Pub/Sub
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        try:
            from loki_constants import get_loki_home as _get_loki_home
            _loki_home = _get_loki_home()
        except (ModuleNotFoundError, ImportError):
            _loki_home = _Path.home() / ".loki"
        self._thread_count_store = _ThreadCountStore(_loki_home / "google_chat_thread_counts.json")
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        from loki_constants import get_loki_home as _get_loki_home
        return _get_loki_home() / "google_chat_bot_id.json"
    ...
    async def _build_message_event(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> Optional[MessageEvent]:
        """Parse a Chat API message into a loki MessageEvent."""
    ...
    async def send_clarify(
        ...
        def _button(text: str, choice: str) -> Dict[str, Any]:
            return {"text": text, "action": "loki_clarify", "parameters": {"clarify_id": clarify_id, "choice": choice}}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Loki is thinking…' marker (Chat has no typing API); ``send()``
        ...
        body = _thread_body(getattr(self.config, "typing_status_text", None) or "Loki is thinking…", thread_id)
...
Walkthrough:
  ...
  3. Create a Pub/Sub topic (e.g. loki-chat-events) and a Pull subscription.
...
def interactive_setup() -> None:
    """``loki setup`` wizard: print GCP instructions, prompt for env vars, persist to ``~/.loki/.env``."""
    from loki_cli.cli_output import print_info, print_success, print_warning, prompt, prompt_yes_no
    ...
    print()
    print_success("Google Chat configuration saved to ~/.loki/.env")
    print_info("Restart the gateway: loki gateway restart")
...
async def _standalone_send(
    ...
    gateway runner is not in-process, e.g. ``loki cron``). Needs SA credentials and a
    ...
    # Bound the synchrowundercorp token refresh so a hung STS endpoint can't stall cron.
    try:
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `loki setup` to install Google Chat support.",
        ...
        platform_hint=(
            ...
            "confirmations instead. While you are generating a response, a 'Loki is thinking…' marker message "
