...
// `message.content.text` before Loki can see it. We rewrite the two inbound
...
const MARKER = "Loki patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // Loki to patch; keep the legacy v8 path below for older pinned installs.
    if (
