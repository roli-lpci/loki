...
Small Node helper that bridges Loki Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Loki is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Loki-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Loki can receive inbound messages
...
The Loki plugin's `loki photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Loki is Python, so replies go through this sidecar
