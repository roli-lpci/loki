...
``(id, projectSecret)``. Storage: runtime SDK creds -> ``~/.loki/.env``; management
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a loki dependency
...
# published CLI client until Loki gets its own client_id.
...
DEFAULT_PROJECT_NAME = "Loki Agent"
DEFAULT_POLL_INTERVAL = 5  # RFC 8628 polling defaults; Photon's `interval` / `expires_in` win
DEFAULT_POLL_TIMEOUT = 1800
E164_RE = re.compile(r"^\+[1-9]\d{6,14}$")


# -- auth.json helpers (shares the file with the rest of loki-agent) ------------

def _auth_json_path() -> Path:
    """``~/.loki/auth.json`` honouring the active Loki profile."""
    try:
        from loki_constants import get_loki_home
        return Path(get_loki_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.loki")) / "auth.json"
...
def _store_pool_record(key: str, record: Dict[str, Any]) -> None:
    ...
    from loki_cli.auth import _auth_store_lock
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.loki/.env`` (secret never bound to a printable local
    in a caller — CodeQL clean flow)."""
    try:
        from loki_cli.config import save_env_value
    except ImportError:
        logger.warning("photon: loki_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    rows = [
        ...
        "──────────────────────",
        "  device token        : " + (
            "✓ stored" if load_photon_token() else "✗ missing (run `loki photon setup`)"),
        "  project id          : " + (sid if sid else "✗ missing"),
        "  project secret      : " + ("✓ stored" if sec else "✗ missing"),
        "  my number           : " + (phone if phone else "✗ missing (run `loki photon setup --phone ...`)"),
        "  assigned number     : " + (assigned if assigned else "✗ missing (run `loki photon setup`)")]
    emit("\n".join(rows))
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_token() -> str:
        return (
            ...
            else "✗ missing (run `loki photon setup`)"
        )
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `loki photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `loki photon setup`)"
