...
(enabled/policy/allow_from) falling back independently. Config ~/.loki/feishu_comment_rules.json (mtime-cached,
hot-reload); pairing store ~/.loki/feishu_comment_pairing.json."""
...
# long after profile/LOKI_HOME overrides have been applied, so freezing is safe.
RULES_FILE = get_loki_home() / "feishu_comment_rules.json"
PAIRING_FILE = get_loki_home() / "feishu_comment_pairing.json"
...
def _main() -> int:
    try:
        __import__("loki_cli.env_loader", fromlist=["load_loki_dotenv"]).load_loki_dotenv()
    except Exception:
        pass
