# A2A — Agent-to-Agent protocol for Loki
...
peer (another Loki, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
loki gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Loki serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.loki/a2a_audit.jsonl`.
- Conversations persist to `~/.loki/a2a_conversations/` — they survive context
