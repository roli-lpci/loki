"""OpenRouter-compatible image generation backend (OpenRouter + WunderCorp Portal).
...
via :func:`loki_cli.runtime_provider.resolve_runtime_provider`. OpenRouter alone also
...
_ATTRIBUTION_HEADERS = {
    ...
    # OpenRouter attribution headers (harmless against WunderCorp Portal).
    "HTTP-Referer": "https://github.com/wundercorp/loki",
    "X-Title": "Loki Agent",
}
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    """One instance per backend (OpenRouter, WunderCorp Portal); they differ only in the runtime
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        releases selectable); WunderCorp Portal = chat-completions only. Offline: default chain + snapshot."""
        ...
        **kwargs: Any,
        ...
        if not api_key or not base_url:
            return fail(
                ...
                f"Configure {self._display} in `loki tools` → Image Generation.",
                "missing_api_key")
...
def _build_providers() -> List[OpenRouterCompatImageProvider]:
    return [
        ...
        OpenRouterCompatImageProvider(
            provider_name="wundercorp", display_name="WunderCorp Portal", runtime_name="wundercorp", config_key="wundercorp",
            model_env_var="WUNDERCORP_IMAGE_MODEL",
            setup_schema={
                "name": "WunderCorp Portal (image)",
                "badge": "subscription",
                "tag": "Reference-grounded image generation via WunderCorp Portal (OpenRouter-backed)",
                "env_vars": [],
...
def register(ctx: Any) -> None:
    """Register the OpenRouter + WunderCorp Portal image gen providers."""
