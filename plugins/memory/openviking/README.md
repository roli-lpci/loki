...
- OpenViking server running and reachable from Loki

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Loki can identify older servers that expose the legacy status-only health
...
connection values into Loki, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.loki/.env`; for a named profile use
...
OpenViking's server config is separate from Loki:
...
Loki-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, Loki lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Loki sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Loki also sends `User-Agent: openviking-memory-loki/<version>` on
OpenViking requests. This standard harness identifier contains the Loki
...
does not ask for a peer ID. Without a configured peer, Loki sends neither
...
order remains environment, linked OpenViking config, then Loki YAML. To use
...
on the old implicit `loki` peer now use user memory for new writes. Without
...
Set `agent: loki` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `loki-remember-<random>` OpenViking
...
asynchrowundercorply after the tool returns.
...
not advertised or used. The one-shot session is separate from the live Loki
...
Use the same OpenViking profile and credentials as Loki for manual recovery.
...
Loki built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Loki action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because Loki
...
file URIs, such as
`viking://user/default/peers/loki/memories/preferences/mem_abc123.md` (any
