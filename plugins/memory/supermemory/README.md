...
- Hosted: API key from [app.supermemory.ai/integrations?connect=loki](http://app.supermemory.ai/integrations?connect=loki)
...
Before running `loki memory setup`, add the local endpoint to
...
Then run `loki memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$LOKI_HOME/supermemory.json`
...
| `container_tag` | `loki` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `loki-{identity}` → `loki-coder`). |
...
`https://api.supermemory.ai`. Loki resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: loki`, and document writes stamp
`metadata.sm_source: loki`. This is a **functional routing key, not telemetry**:
it groups Loki-written memories into a dedicated "Loki" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Loki profile:

```json
{
  "container_tag": "loki-{identity}"
}
```

For a profile named `coder`, this resolves to `loki-coder`. The default profile resolves to `loki-default`. Without `{identity}`, all profiles share the same container.
