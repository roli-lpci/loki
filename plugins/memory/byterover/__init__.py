...
$LOKI_HOME/byterover/ (profile-scoped); ``memory.byterover.auto_extract: false`` disables curate hooks.
"""
...
def _get_brv_cwd() -> Path:
    ...
    return get_loki_home() / "byterover"
...
class ByteRoverMemoryProvider(MemoryProvider):
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        """Run brv query synchrowundercorply (blocks up to _QUERY_TIMEOUT) so context is ready before the first LLM call."""
    ...
    def queue_prefetch(self, query: str, *, session_id: str = "") -> None:
        """No-op: prefetch() runs synchrowundercorply at turn start."""
