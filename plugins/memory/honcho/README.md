...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/loki>
...
```bash
loki memory setup honcho   # configure Honcho directly (works on a fresh install)
loki memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
loki config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.loki/.env
```

> `loki honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `loki memory setup honcho`.
...
Set `"recallSync": true` in `$LOKI_HOME/honcho.json` (at the root or under
`hosts.loki`), or enable **Current-query recall** in the memory settings or
`loki honcho setup`. An explicit host-level `false` overrides root-level `true`.
...
settings continue to apply only to the default asynchrowundercorp mode.
...
cycle (not per pass). Select a fast, inexpensive model under `loki model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$LOKI_HOME/honcho.json` | Profile-local (isolated Loki instances) |
| 2 | `~/.loki/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Loki profile: `loki` (default) or `loki_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `loki honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
- **`per-session`** — Loki session ID (timestamp + hex). Every `loki` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Loki profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`loki`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Loki profile: `loki` (default) or `loki_<profile>` (e.g. `loki -p coder` -> host key `loki_coder`). Older `loki.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `LOKI_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `loki-agent`) |
...
|---------|-------------|
| `loki memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `loki honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `loki memory setup`) |
| `loki honcho status` | Show resolved config for active profile |
| `loki honcho enable` / `disable` | Toggle Honcho for active profile |
| `loki honcho mode <mode>` | Change recall or observation mode |
| `loki honcho peer --user <name>` | Update user peer name |
| `loki honcho peer --ai <name>` | Update AI peer name |
| `loki honcho tokens --context <N>` | Set context token budget |
| `loki honcho tokens --dialectic <N>` | Set dialectic max chars |
| `loki honcho map <name>` | Map current directory to a session name |
| `loki honcho sync` | Create host blocks for all Loki profiles |
