...
Behavioral settings live in `$LOKI_HOME/mem0.json` (set them via `loki memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.loki/.env`.
...
| `user_id` | `loki-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Loki session and call `mem0_search` — it connects to your server.
...
```bash
loki memory setup mem0 --mode oss \
...
```bash
loki memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$LOKI_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.loki/mem0_qdrant"}}
  }
}
```
...
```bash
loki memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$LOKI_HOME/mem0.json`).
