...
Config: $LOKI_HOME/hindsight/config.json (profile-scoped), else ~/.hindsight/
...
_RETAIN_CONTEXT_DEFAULT = "conversation between Loki Agent and the User"
...
def _context_thread(target, name: str) -> threading.Thread:
    ...
    closed without the profile's secret scope + LOKI_HOME override. (The shared
...
def _load_config() -> dict:
    """$LOKI_HOME/hindsight/config.json (profile-scoped), else ~/.hindsight/config.json
    (legacy, shared), else environment variables."""
    for path in (get_loki_home() / "hindsight" / "config.json", Path.home() / ".hindsight" / "config.json"):
        if path.exists():
    ...
    return {
        ...
        "banks": {"loki": {"bankId": os.environ.get("HINDSIGHT_BANK_ID", "loki"),
                             "budget": os.environ.get("HINDSIGHT_BUDGET", "mid"), "enabled": True}},
    }


def _event_timestamp() -> str:
    """Configured Loki event time with an explicit UTC offset."""
    event_time = _loki_now()
    # loki_time.now() is aware; guard a replacement clock emitting offset-less dates.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._bank_id, self._budget, self._bank_id_template = "loki", "mid", ""
    ...
    def save_config(self, values, loki_home):
        """Merge *values* into $LOKI_HOME/hindsight/config.json."""
        from utils import atomic_json_write
        config_path = Path(loki_home) / "hindsight" / "config.json"
    ...
    def post_setup(self, loki_home: str, config: dict) -> None:
        ...
        run_setup(self, loki_home, config)
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "loki"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: loki-{profile}", "default": ""},
            ...
            {"key": "recall_sync", "description": "Recall synchrowundercorply against the current message before each turn (higher relevance, adds recall latency to the turn). Default off: recall runs in the background and is injected on the next turn.", "default": False},
            ...
            {"key": "retain_async","description": "Process retain asynchrowundercorply on the Hindsight server", "default": True},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Loki Agent and the User"},
    ...
    def _new_embedded_client(self):
        ...
        logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                     cfg.get("profile", "loki"), llm_provider)
        ...
        kwargs = dict(profile=cfg.get("profile", "loki"), llm_provider=llm_provider,
    ...
    def _apply_connection_settings(self, cfg: dict) -> None:
        ...
        banks = cfg_get(cfg, "banks", "loki", default={})
        self._bank_id_template = cfg.get("bank_id_template", "") or ""
        self._bank_id = _resolve_bank_id_template(
            self._bank_id_template,
            fallback=cfg.get("bank_id") or banks.get("bankId", "loki"),
    ...
    def _start_embedded_daemon(self) -> None:
        ...
        if hasattr(os, "geteuid") and os.geteuid() == 0:
            msg = ("Hindsight local_embedded mode cannot run as root "
                   ...
                   "memory daemon. Run Loki as a non-root user, or switch "
                   "to cloud / local_external mode via 'loki memory setup'.")
            logger.warning(msg)
            # Also print: otherwise the user would only see Loki get sluggish.
            with contextlib.suppress(Exception):
                # Surface to the terminal too — a daemon that never starts would otherwise fail silently and
                # the user would only see Loki get sluggish. (issue #13125)
    ...
    def _daemon_start_worker(self) -> None:
        import traceback
        log_path = get_loki_home() / "logs" / "hindsight-embed.log"
        ...
        try:
            ...
            profile = self._config.get("profile", "loki")
    ...
    def _recall_disabled(self) -> bool:
        """Guards shared by the async and synchrowundercorp recall paths."""
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        # Opt-in: recall synchrowundercorply against the *current* message so the
        # injected memories match this turn's query, not the previous turn's.
        # See wundercorp/loki#5820.
