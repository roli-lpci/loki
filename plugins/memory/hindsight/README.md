...
Loki spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.loki/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p loki ui start
```
...
Config file: `~/.loki/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `loki` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `loki-{profile}` isolates memory per active Loki profile. Empty placeholders collapse cleanly (e.g. `loki-{user}` with no user becomes `loki`). |
...
| `recall_sync` | `false` | Recall synchrowundercorply against the *current* message each turn (higher relevance, adds recall latency). Default off: recall runs in the background and is injected on the next turn. |
...
| `retain_async` | `true` | Process retain asynchrowundercorply on the Hindsight server |
...
| `retain_source` | — | Opt-in `metadata.source` attached to retained memories (identifies the storing client, e.g. `loki`). Empty by default — no attribution tag ships unless you set it. |
...
The LLM API key is stored in `~/.loki/.env` as `HINDSIGHT_LLM_API_KEY`.
