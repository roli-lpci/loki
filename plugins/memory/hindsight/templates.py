...
catalog, filter to the ``loki`` integration, apply a manifest via
...
def fetch_loki_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``loki`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "loki" in (e.get("integrations") or [])]
