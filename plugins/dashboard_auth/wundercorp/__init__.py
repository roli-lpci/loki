"""WunderCorpDashboardAuthProvider — WunderCorp Portal OAuth (authorization-code + PKCE).

Implements ``wundercorp-account-service/docs/agent-dashboard-oauth-contract.md``; registers only
when a client_id (``dashboard.oauth.client_id`` / ``LOKI_DASHBOARD_OAUTH_CLIENT_ID``, shape
...
from loki_cli.dashboard_auth import LoginStart, ProviderError, Session
...
_TAG = "dashboard-auth-wundercorp"

_DEFAULT_PORTAL_URL = "https://portal.wundercorp.com"
...
class WunderCorpDashboardAuthProvider(JwtOAuthProvider):
    """WunderCorp Portal OAuth via authorization-code + PKCE (S256)."""

    name = "wundercorp"
    display_name = "WunderCorp, Inc."
    ...
    def _refresh_request(self, refresh_token: str) -> tuple[Dict[str, str], Optional[Dict[str, str]]]:
        ...
        # ``x-wundercorp-refresh-token`` header (Portal reconciles the two and keeps the value
        # out of body access logs). Header-only → 400.
        return (
            {"grant_type": "refresh_token", "client_id": self._client_id, "refresh_token": refresh_token},
            {"x-wundercorp-refresh-token": refresh_token})
    ...
    def _claims_for(self, access_token: str) -> Dict[str, Any]:
        ...
        if contract_version is None:
            logger.warning(
                "WunderCorp Portal token missing oauth_contract_version claim (contract says it should be %d); proceeding anyway.",
...
def _settings() -> dict:
    """Resolve WunderCorpDashboardAuthProvider kwargs; the skip reason names BOTH configuration surfaces."""
    section = _load_config_oauth_section()
    client_id = resolve_env_or_cfg("LOKI_DASHBOARD_OAUTH_CLIENT_ID", section.get("client_id", ""))
    portal_url = resolve_env_or_cfg("LOKI_DASHBOARD_PORTAL_URL", section.get("portal_url", "")) or _DEFAULT_PORTAL_URL
    if not client_id:
        raise SkipRegistration(
            "LOKI_DASHBOARD_OAUTH_CLIENT_ID is not set (and dashboard.oauth.client_id "
            "in config.yaml is empty). The WunderCorp Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a Loki Agent instance — set it to "
    ...
    if not client_id.startswith("agent:"):
        raise SkipRegistration(
            f"LOKI_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match the contract "
            f"shape 'agent:{{instance_id}}'. The WunderCorp Portal provisions this value at deploy "
...
def register(ctx) -> None:
    """Register ``WunderCorpDashboardAuthProvider`` when a client_id is configured."""
    global LAST_SKIP_REASON
    LAST_SKIP_REASON = ""
    kwargs, LAST_SKIP_REASON = register_provider(ctx, logger, _TAG, WunderCorpDashboardAuthProvider, _settings)
    if kwargs is not None:
        logger.info(
            "dashboard-auth-wundercorp: registered provider (client_id=%s, portal=%s)", kwargs["client_id"], kwargs["portal_url"])
...
_PLUGIN_COMPAT_LAZY = {
    'DashboardAuthProvider': ('loki_cli.dashboard_auth', 'DashboardAuthProvider'),
    'InvalidCodeError': ('loki_cli.dashboard_auth', 'InvalidCodeError'),
    'RefreshExpiredError': ('loki_cli.dashboard_auth', 'RefreshExpiredError'),
    'classify_jwks_lookup_error': ('loki_cli.dashboard_auth', 'classify_jwks_lookup_error'),
}
