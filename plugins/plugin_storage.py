"""Per-plugin persistent storage: ``<loki home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<loki home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `loki plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<loki home>/plugin-data/<name>/``; resolves ``get_loki_home()`` on
    every call so it follows the active profile — don't cache across profile switches."""
    from loki_constants import get_loki_home
    root = get_loki_home() / "plugin-data" / _validate_name(name)
