...
Let the loki agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where loki runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `loki meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `loki meet` CLI |
...
| `cli.py` | `loki meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$LOKI_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `loki meet node {run,list,approve,remove,status,ping}` |
...
loki meet install                                      # pip + Chromium
loki meet setup                                        # preflight
loki meet auth                                         # optional
loki meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
loki meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.loki/.env
loki meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
loki meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
loki meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.loki/.env
loki meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, loki will **not** switch your system audio input automatically — the
...
loki plugins enable google_meet
loki meet node run --display-name my-mac --host 0.0.0.0 --port 18789
