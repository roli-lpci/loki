...
Standalone subprocess spawned by ``process_manager.py``; configured via ``LOKI_MEET_*`` env,
status + transcript written under ``$LOKI_MEET_OUT_DIR`` (filesystem is the only IPC).
...
Debug: ``LOKI_MEET_URL=... LOKI_MEET_OUT_DIR=/tmp/x LOKI_MEET_HEADED=1 \\
...
# collects {speaker, text}; ``window.__lokiMeetDrain()`` pulls new entries.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__lokiMeetInstalled) return;
  window.__lokiMeetInstalled = true;
  window.__lokiMeetQueue = [];
  ...
  window.__lokiMeetDrain = () => {
    const out = window.__lokiMeetQueue.slice();
    window.__lokiMeetQueue = [];
    return out;
  };
})();
...
_ADMISSION_PROBE_JS = r"""
    (() => {
      ...
      if (window.__lokiMeetInstalled && document.querySelector(
...
def _start_pcm_pump(rt: dict, bridge_info: dict, pcm_path: Path, state: "_BotState") -> None:
    ...
    if platform_tag == "linux":
        cmd = ["paplay", "--raw", "--rate=24000", "--format=s16le", "--channels=1",
               f"--device={target or 'loki_meet_sink'}", str(pcm_path)]
...
def _setup_realtime(rt: dict, api_key: str, state: _BotState) -> None:
    ...
    if not api_key:
        state.set(error="realtime mode requested but no API key in LOKI_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
        rt["enabled"] = False
        return
    try:
...
_BotConfig = SimpleNamespace  # everything the bot reads from ``LOKI_MEET_*`` env vars


def _config_from_env() -> _BotConfig:
    env = os.environ.get
    out_raw = env("LOKI_MEET_OUT_DIR", "").strip()
    return _BotConfig(
        url=env("LOKI_MEET_URL", "").strip(),
        out_dir=Path(out_raw) if out_raw else None,
        headed=env("LOKI_MEET_HEADED", "").lower() in {"1", "true", "yes"},
        auth_state=env("LOKI_MEET_AUTH_STATE", "").strip(),
        guest_name=env("LOKI_MEET_GUEST_NAME", "Loki Agent"),
        duration_s=_parse_duration(env("LOKI_MEET_DURATION", "")),
        realtime=env("LOKI_MEET_MODE", "transcribe").strip().lower() == "realtime",
        # LOKI_MEET_REALTIME_KEY is resolved by process_manager.start() via the parent's
        # profile secret scope; OPENAI_API_KEY only serves standalone `python -m` runs.
        realtime_api_key=env("LOKI_MEET_REALTIME_KEY") or env("OPENAI_API_KEY", ""),
        realtime_model=env("LOKI_MEET_REALTIME_MODEL", "gpt-realtime"),
        realtime_voice=env("LOKI_MEET_REALTIME_VOICE", "alloy"),
        realtime_instructions=env("LOKI_MEET_REALTIME_INSTRUCTIONS", ""),
        lobby_timeout=float(env("LOKI_MEET_LOBBY_TIMEOUT", "300")))
    ...
    while not stop_flag["stop"]:
        ...
        if not state.in_call and (now - last_admission_check) > 3.0:
            ...
            if _probe(page, _ADMISSION_PROBE_JS):
                ...
                return
        try:
            queued = page.evaluate("window.__lokiMeetDrain && window.__lokiMeetDrain()")
...
def run_bot() -> int:
    ...
    if not _is_safe_meet_url(cfg.url):
        sys.stderr.write("google_meet bot: refusing to launch — LOKI_MEET_URL must be a "
                         "meet.google.com URL. got: %r\n" % cfg.url)
        return 2
    if cfg.out_dir is None:
        sys.stderr.write("google_meet bot: LOKI_MEET_OUT_DIR is required\n")
        return 2
