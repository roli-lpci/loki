...
One active meeting at a time, recorded in ``$LOKI_HOME/workspace/meetings/.active.json``
...
def _root() -> Path:
    return Path(get_loki_home()) / "workspace" / "meetings"
...
def start(url: str, *, out_dir: Optional[Path] = None, headed: bool = False,
          auth_state: Optional[str] = None, guest_name: str = "Loki Agent", duration: Optional[str] = None,
    ...
    for name in ("transcript.txt", "status.json"):
        with contextlib.suppress(OSError):
            (out / name).unlink()
    env = {**os.environ, "LOKI_MEET_URL": url, "LOKI_MEET_OUT_DIR": str(out),
           "LOKI_MEET_GUEST_NAME": guest_name}
    for value, var in (
        (headed and "1", "LOKI_MEET_HEADED"),
        (auth_state, "LOKI_MEET_AUTH_STATE"),
        (duration, "LOKI_MEET_DURATION"),
        (mode, "LOKI_MEET_MODE"),  # bot defaults to transcribe when unset (v1 behavior)
        (realtime_model, "LOKI_MEET_REALTIME_MODEL"),
        (realtime_voice, "LOKI_MEET_REALTIME_VOICE"),
        (realtime_instructions, "LOKI_MEET_REALTIME_INSTRUCTIONS")):
        if value:
            env[var] = value
    ...
    if not realtime_api_key:
        ...
        realtime_api_key = get_secret("LOKI_MEET_REALTIME_KEY") or get_secret("OPENAI_API_KEY")
    if realtime_api_key:
        env["LOKI_MEET_REALTIME_KEY"] = realtime_api_key
