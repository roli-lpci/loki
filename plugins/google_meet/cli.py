"""CLI commands for the google_meet plugin (``loki meet <subcommand>``).
...
def _auth_state_path() -> Path:
    return Path(get_loki_home()) / "workspace" / "meetings" / "auth.json"


# ``loki meet <sub>`` in help order.
_SUBCOMMAND_HELP = (
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    """Build the ``loki meet`` argparse tree (called at plugin load time)."""
    ...
    p["join"].add_argument("--guest-name", default="Loki Agent")
...
_DISPATCH = {
    ...
    "stop": lambda a: _print_result(pm.stop(reason="loki meet stop")),
...
def meet_command(args: argparse.Namespace) -> int:
    ...
    if not sub:
        print("usage: loki meet {setup,auth,join,status,transcript,say,stop,node}")
        return 2
...
def _cmd_setup() -> int:
    ...
    print("  google auth    : " + (f"ok ({auth_path})" if auth_path.is_file() else "not saved — run: loki meet auth"))
    print()
    all_ok = system_ok and pw_ok and chromium_ok
    print("ready. Join a meeting:  loki meet join https://meet.google.com/abc-defg-hij" if all_ok
    ...
    if not realtime:
        ...
        if system == "Linux":
            ...
            print("\n  NOTE: macOS does not auto-route audio. Open\n    System Settings → Sound → "
                  ...
                  "loki will not switch your default input for you.")
    print("\ndone. verify with: loki meet setup")
    return 0
...
def _cmd_auth() -> int:
    ...
    try:
        ...
        return 1
    print("saved. you can now run: loki meet join <url>")
    return 0
...
if __name__ == "__main__":  # pragma: no cover
    parser = argparse.ArgumentParser(prog="loki meet")
    register_cli(parser)
