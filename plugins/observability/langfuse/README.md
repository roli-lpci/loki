...
This plugin ships bundled with Loki but is **opt-in** — it only loads when
...
loki tools  # → Langfuse Observability
...
Set these in `~/.loki/.env` (or via `loki tools`):
...
loki chat -q "hello"              # then check Langfuse for a "Loki turn" trace
```

Generation observations include the Loki system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `LOKI_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
LOKI_LANGFUSE_ENV=production       # environment tag
LOKI_LANGFUSE_RELEASE=v1.0.0       # release tag
LOKI_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
LOKI_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
LOKI_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
LOKI_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`LOKI_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
loki plugins disable observability/langfuse
```
