...
Auto-tracks and cleans up ephemeral files created during Loki Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$LOKI_HOME` and `/tmp/loki-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `LOKI_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `LOKI_HOME` or `/tmp/loki-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$LOKI_HOME/disk-cleanup/` is itself excluded
- `$LOKI_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
