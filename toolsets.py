...
_LOKI_WEBHOOK_SAFE_TOOLS = ["web_search", "web_extract", "vision_analyze", "clarify"]
...
def _bundle(description, extras=()):
    """A `loki-*` platform bundle: the shared core tools plus optional platform extras."""
    return _ts(description, _LOKI_CORE_TOOLS + list(extras))


def _core_without(*excluded, kanban=True):
    """_LOKI_CORE_TOOLS minus *excluded* (and, unless kanban=True, every kanban_* tool); order preserved."""
    return [t for t in _LOKI_CORE_TOOLS if t not in excluded and (kanban or not t.startswith("kanban_"))]
...
TOOLSETS = {
    ...
    "x_search": _ts(
        ...
        "(SuperGrok OAuth or XAI_API_KEY). Off by default; enable in `loki tools` → "
        "X (Twitter) Search.",
        ["x_search"],
    ),
    ...
    "video_gen": _ts(
        ...
        "separate tools. Configure via ``loki tools`` → Video Generation.",
    ...
    "browser": _ts(
        ...
        "iframes, hold-click)",
        [t for t in _LOKI_CORE_TOOLS if t.startswith("browser_")],
    ),
    "cronjob": _ts(
    ...
    "kanban": _ts(
        ...
        "the kanban dispatcher (LOKI_KANBAN_TASK env set). The dispatcher runs "
        ...
        [t for t in _LOKI_CORE_TOOLS if t.startswith("kanban_")],
    ),
    ...
    # config (see the non-configurable-toolset recovery loop in loki_cli/tools_config.py).
    "coding": _ts(
    ...
    # Full Loki toolsets (CLI + messaging platforms). All share the core tools;
    # there is deliberately no agent-callable send_message tool. loki-acp is the
    ...
    "loki-cli": _bundle("Full interactive CLI toolset - all default tools plus cronjob management"),

    # Mirrors loki-cli; `loki tools` platform config filters it down and
    # _get_platform_tools() drops _DEFAULT_OFF_TOOLSETS unless user-enabled.
    "loki-cron": _bundle("Default cron toolset - same core tools as loki-cli; gated by `loki tools`"),
    "loki-telegram": _bundle("Telegram bot toolset - full access for personal use (terminal has safety checks)"),
    "loki-discord": _bundle(
        "Discord bot toolset - full access (terminal has safety checks via dangerous "
        "command approval)",
        ["discord", "discord_admin"],
    ),
    "loki-whatsapp": _bundle("WhatsApp bot toolset - similar to Telegram (personal messaging, more trusted)"),
    "loki-slack": _bundle("Slack bot toolset - full access for workspace use (terminal has safety checks)"),
    "loki-signal": _bundle("Signal bot toolset - encrypted messaging platform (full access)"),
    "loki-bluebubbles": _bundle("BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server"),
    "loki-homeassistant": _bundle("Home Assistant bot toolset - smart home event monitoring and control"),
    "loki-email": _bundle("Email bot toolset - interact with Loki via email (IMAP/SMTP)"),
    "loki-mattermost": _bundle("Mattermost bot toolset - self-hosted team messaging (full access)"),
    "loki-matrix": _bundle("Matrix bot toolset - decentralized encrypted messaging (full access)"),
    "loki-dingtalk": _bundle("DingTalk bot toolset - enterprise messaging platform (full access)"),
    "loki-feishu": _bundle("Feishu/Lark bot toolset - enterprise messaging via Feishu/Lark (full access)", _FEISHU_TOOLS),
    "loki-weixin": _bundle("Weixin bot toolset - personal WeChat messaging via iLink (full access)"),
    "loki-qqbot": _bundle("QQBot toolset - QQ messaging via Official Bot API v2 (full access)"),
    "loki-wecom": _bundle("WeCom bot toolset - enterprise WeChat messaging (full access)"),
    "loki-wecom-callback": _bundle("WeCom callback toolset - enterprise self-built app messaging (full access)"),
    "loki-yuanbao": {
        "description": "Yuanbao Bot 元宝消息平台工具集 - 群信息、成员查询、私聊、贴纸表情",
        "tools": _LOKI_CORE_TOOLS + _YUANBAO_TOOLS,
        "module": "tools.yuanbao_tools",
        "includes": [],
    },
    "loki-sms": _bundle("SMS bot toolset - interact with Loki via SMS (Twilio)"),
    "loki-webhook": _ts("Webhook toolset - receive and process external webhook events", _LOKI_WEBHOOK_SAFE_TOOLS),
    "loki-gateway": _ts(
        "Gateway toolset - union of all messaging platform tools",
        [],
        includes=[
            "loki-telegram", "loki-discord", "loki-whatsapp", "loki-slack",
            "loki-signal", "loki-bluebubbles", "loki-homeassistant", "loki-email",
            "loki-sms", "loki-mattermost", "loki-matrix", "loki-dingtalk",
            "loki-feishu", "loki-wecom", "loki-wecom-callback", "loki-weixin",
            "loki-qqbot", "loki-webhook", "loki-yuanbao",
        ],
    ),
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """A bundle's tools minus _LOKI_CORE_TOOLS (one level of includes).

    Disabling a `core + extras` bundle must not strip the core tools every other
    toolset shares. One `includes` pass suffices (only loki-gateway nests
    bundles). Unknown names: full resolution minus core.
    """
    core = set(_LOKI_CORE_TOOLS)
