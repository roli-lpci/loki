...
//!
//! Mirrors `loki_constants.get_loki_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\loki
//!   macOS:   ~/.loki
//!   Linux:   ~/.loki  (override via $LOKI_HOME)
//!
//! NOTE (macOS): Python's get_loki_home(), scripts/install.sh, and the
//! Electron desktop's resolveLokiHome() ALL use ~/.loki on macOS — there
...
/// Returns the canonical Loki home directory, respecting $LOKI_HOME if set.
pub fn loki_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("LOKI_HOME") {
    ...
    {
        // %LOCALAPPDATA%\loki — matches scripts/install.ps1's $LokiHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("loki");
        }
    }

    // macOS + Linux + fallback: ~/.loki (matches Python get_loki_home(),
    // install.sh, and the Electron desktop's resolveLokiHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".loki");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    loki_home().join("bootstrap-cache")
}
...
/// LOKI_HOME so it survives repo checkout deletion (unlike anything under
/// loki-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\loki\loki-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_loki`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under LOKI_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves LOKI_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    loki_home().join(".loki-update-in-progress")
}
...
pub fn copy_self_to_loki_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to LOKI_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_LOKI_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".loki-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under LOKI_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[loki-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("LOKI_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_loki_home() -> String {
    loki_home().to_string_lossy().into_owned()
}
