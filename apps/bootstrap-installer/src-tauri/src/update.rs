...
//!
//! Driven when the installer is launched as `Loki-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Loki desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `loki update`
//!      or repair bootstrap can race locked files),
//!   2. run `loki update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in loki_cli/main.py),
//!   3. run `loki desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `loki update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_loki) and
...
/// `loki update` exit code meaning "another loki process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// loki_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `loki update`'s own guard decide.
...
///
/// The marker is also the cross-process update lock: `loki update` claims
/// the same file (see `loki_cli/update_lock.py`) so a dashboard-spawned
...
/// overwriting — the pre-fix clobber is what let a dashboard `loki update`
...
/// UPDATE_MARKER_MAX_AGE_SECONDS in loki_cli/update_lock.py — all three read
...
/// a dashboard-spawned `loki update`) still blocks.
...
/// the marker name our PID — because a `loki update` child that
...
async fn run_update(app: AppHandle) -> Result<()> {
    let loki_home = crate::paths::loki_home();
    let install_root = loki_home.join("loki-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_loki`
    ...
    //
    // The same marker is the cross-process update lock (loki_cli/
    // update_lock.py claims it too), so a live foreign owner means another
    // updater — most often a dashboard-spawned `loki update` — is already
    ...
    let _update_marker = match UpdateMarkerGuard::acquire(
        ...
        Err(owner) => {
            ...
            let msg = format!(
                "Another Loki update is already running (PID {}, started {} ago). \
    ...
    let loki = resolve_loki(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the loki CLI under {}. Is Loki installed? \
    ...
    // async on Windows. If it still holds the venv shim, `loki update`
    ...
    // ---- stage 2: loki update -----------------------------------------
    // Pass --branch so `loki update` targets the branch this installer was
    ...
    // without --branch, `loki update` switches the checkout to `main` (a
    ...
    // --force skips `loki update`'s Windows running-exe guard (which would
    ...
    // time `loki update` runs there is no legitimate loki.exe to protect,
    // and the guard would only produce a false "Loki is still running" stop.
    //
    ...
    // probe only checks the loki.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Loki windows" message.
    ...
    // Retry-once for the update-boundary crash. `loki update` lazily imports
    ...
    // second `loki update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Loki" state
    // a retry can't fix.
    ...
    // checkout predating the LOKI_UPDATE_HANDOFF_PID env fix (8c76fe19f)
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Loki is still running. Close all Loki windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "loki update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::loki_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: loki desktop --build-only ----------------------------
    // `loki update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `loki update` already does above, and `loki update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `loki desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Loki manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_loki_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Loki to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            // Loki.exe, so an image-name kill would tear down the app itself.
            emit_log(
                app,
                Some(stage),
                LogStream::Stdout,
                &format!(
                    "[handoff] Loki still holding install files ({}); locating backend shims…",
                    format_locked_paths(&locked)
                ),
            );
            let shim = venv_loki(install_root);
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_loki(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Loki.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Loki.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Find processes running the exact `venv\Scripts\loki.exe` shim for this
/// installation. Windows image names are case-insensitive and the desktop is
/// also Loki.exe, so matching by image name alone is unsafe.
#[cfg(windows)]
...
/// Spawn `loki <args>` from `cwd`, stream stdout/stderr as Log events on the
...
async fn run_streamed(
    ...
    // the same rule about pipe EOF: `loki update` is precisely the shape that
...
/// Path to the venv loki shim under an install root, regardless of existence.
fn venv_loki(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("loki.exe")
    } else {
        install_root.join("venv").join("bin").join("loki")
    }
}

/// Resolve the loki CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `loki` on PATH.
fn resolve_loki(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_loki(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "loki.exe" } else { "loki" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    let mut envs = vec![(
        "LOKI_HOME".to_string(),
        loki_home.as_os_str().to_os_string(),
    )];
    // `loki update` is a Python CLI writing to a pipe here, so CPython
    ...
    // `loki update` child claims that SAME lock (loki_cli/update_lock.py).
    ...
    // refuses its parent's marker with exit 2 ("Loki is still running")
    // and no number of retries can ever succeed. Keep the variable name in
    // sync with HANDOFF_PID_ENV in loki_cli/update_lock.py.
    envs.push((
        "LOKI_UPDATE_HANDOFF_PID".to_string(),
        OsString::from(std::process::id().to_string()),
    ));
    if let Some(path) = path_with_prepended_entries(&[
        loki_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_loki_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Loki.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.loki-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.loki-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::loki_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_loki_is_under_install_root() {
        let root = Path::new("/x/loki-agent");
        let shim = venv_loki(root);
    ...
    fn update_child_env_forces_unbuffered_python() {
        let envs = update_child_env(Path::new("/x/loki-agent"));
        assert!(
            envs.iter()
    ...
    fn update_child_env_names_our_pid_for_the_lock_handoff() {
        let envs = update_child_env(Path::new("/x/loki-agent"));
        assert!(
            envs.iter().any(|(k, v)| k == "LOKI_UPDATE_HANDOFF_PID"
                && v.to_str() == Some(std::process::id().to_string().as_str())),
            "the loki update child claims the same marker we hold; without our pid \
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_loki(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/loki-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn same_windows_path_accepts_case_only_difference() {
        assert!(same_windows_path(
            Path::new(r"C:\Users\tester\.loki\loki-agent\venv\scripts\LOKI.EXE"),
            Path::new(r"c:\users\tester\.loki\loki-agent\venv\Scripts\loki.exe"),
        ));
    }
    ...
    fn same_windows_path_rejects_desktop_binary() {
        assert!(!same_windows_path(
            Path::new(r"C:\Users\tester\.loki\loki-agent\apps\desktop\Loki.exe"),
            Path::new(r"C:\Users\tester\.loki\loki-agent\venv\Scripts\loki.exe"),
        ));
    }
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".loki-update-in-progress");
    ...
    fn acquire_refuses_while_a_live_updater_owns_the_marker() {
        ...
        // dashboard `loki update` and install-mode bootstrap mutate one tree
    ...
    fn acquire_adopts_a_marker_prewritten_with_our_own_pid() {
        // #74761: desktop writeUpdateMarker(lokiHome, child.pid) races ahead
    ...
    // checkout's `loki update` reads it as a live foreign update and exits
    ...
    fn missing_or_garbage_marker_never_heals() {
        ...
        let garbage = dir.join(".loki-update-in-progress");
    ...
    fn heal_end_to_end_marker_lifecycle() {
        ...
        // own exit 2, e.g. a genuinely still-running Loki, stays terminal).
        assert!(!should_heal_self_marker_refusal(
            Some(UPDATE_EXIT_CONCURRENT),
            &marker
        ));
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Loki.app"]),
            Some(PathBuf::from("/Applications/Loki.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Loki.app");
        let tmp = base.join("Loki.app.loki-update-new");
        let old = base.join("Loki.app.loki-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Loki.app");
        let tmp = base.join("Loki.app.loki-update-new"); // intentionally absent
        let old = base.join("Loki.app.loki-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Loki.app");
        let tmp = base.join("Loki.app.loki-update-new"); // absent
        let old = base.join("Loki.app.loki-update-old");
        write_marker(&target, "OLD");
