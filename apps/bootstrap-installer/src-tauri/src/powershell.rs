...
/// read side does not see EOF until the last of them closes it. `loki update`
...
/// stranded with it.
///
/// Same bound `Invoke-LokiStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `loki_home_override` propagates to the child as $LOKI_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, loki_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = loki_home_override {
        cmd.env("LOKI_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, loki_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = loki_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_loki_home() {
    ...
    /// child. `loki update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
