...
export const $lokiHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.loki/logs/bootstrap-installer.log')
    $lokiHome.set('~/.loki')
  ...
  try {
    const [logPath, lokiHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_loki_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $lokiHome.set(lokiHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Loki" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Loki-Setup.exe --update);
...
export async function launchLokiDesktop(): Promise<void> {
  ...
  await invoke('launch_loki_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Loki repository' },
