...
async function seedBot(lokiHome: string, mockUrl: string, name: string): Promise<void> {
  const dir = path.join(lokiHome, 'profiles', name)
  fs.mkdirSync(dir, { recursive: true })
...
test.beforeAll(async () => {
  ...
  for (const name of ['alpha', 'beta', 'gamma']) {
    await seedBot(sandbox.lokiHome, mock.url, name)
  }
...
test('file bots into user sections by menu and drag; rename; delete returns them to Unassigned', async () => {
  ...
  const alphaProfile = path.join(fixture!.sandbox.lokiHome, 'profiles', 'alpha', 'profile.yaml')
