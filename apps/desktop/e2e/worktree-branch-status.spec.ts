...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Loki E2E'], { cwd: repo })
...
function configureRepoCwd(lokiHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(lokiHome, mockUrl)
  fs.appendFileSync(path.join(lokiHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(lokiHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.lokiHome, mock.url, repo)
