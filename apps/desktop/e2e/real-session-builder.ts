 ...
 * AIAgent loop, using the E2E mock provider configured in `lokiHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(lokiHome: string) {
  ...
  static async start(lokiHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(lokiHome)
