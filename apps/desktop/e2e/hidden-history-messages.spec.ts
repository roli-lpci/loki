...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.lokiHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.lokiHome)
  const builder = await RealSessionBuilder.start(sandbox.lokiHome)
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.lokiHome, mock.url)
  fs.appendFileSync(path.join(sandbox.lokiHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
