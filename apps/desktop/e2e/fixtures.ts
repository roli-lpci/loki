/**
 * Shared E2E fixtures for the Loki desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → loki serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `loki desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `loki-e2e-${prefix}-${Math.random()}`))
  const lokiHome = path.join(root, 'loki-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(lokiHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(lokiHome, 'config.yaml')
 ...
 */
export function writeEnvFile(lokiHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(lokiHome, '.env')
 ...
 */
function writeEmptyConfig(lokiHome: string): void {
  const configPath = path.join(lokiHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - LOKI_HOME → sandbox loki-home (isolated config/sessions)
 *  - LOKI_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - LOKI_DESKTOP_IGNORE_EXISTING=1 → don't pick up `loki` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - LOKI_DESKTOP_ROOT → REPO_ROOT (dev checkout resolution)
 *  - LOKI_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    LOKI_HOME: sandbox.lokiHome,
    LOKI_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    LOKI_DESKTOP_IGNORE_EXISTING: '1',
    LOKI_DESKTOP_ROOT: REPO_ROOT,
    LOKI_DESKTOP_APP_NAME: `LokiE2E-${Date.now()}`,
    ...
    LOKI_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated LOKI_HOME + userData
 * @param env      - the process environment (already has LOKI_HOME etc.)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.lokiHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via LOKI_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`loki serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.lokiHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { LOKI_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Loki backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Loki.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Loki.app', 'Contents', 'MacOS', 'Loki')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'loki')
}
 ...
 * progress without spawning a real Loki backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * LOKI_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    LOKI_DESKTOP_BOOT_FAKE: '1',
    LOKI_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + loki-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).LOKI_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).LOKI_DESKTOP_CLI
  delete (env as Record<string, string | undefined>).LOKI_DESKTOP_ROOT
