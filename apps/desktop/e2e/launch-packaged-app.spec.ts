...
/**
 * E2E smoke tests for the packaged Loki desktop app.
 *
 ...
 * (credential stripping, isolated LOKI_HOME + userData, unique app name).
 *
...
test('window opens with the Loki title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      lokiDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).lokiDesktop?.hud?.open({ sessionId: null })
  )
