 ...
 * packaged binary) with a real `loki serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → loki serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Loki title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Loki')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { lokiDesktop?: Record<string, unknown> }).lokiDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → loki serve is spawned → WS connects → config
