...
cd apps/desktop
XCURSOR_SIZE=24 LOKI_DESKTOP_DEV_SERVER=http://127.0.0.1:5174 \
...
node apps/desktop/scripts/profile-typing.mjs \
  --chars=400 --cps=30 --out=/tmp/loki-typing
# → /tmp/loki-typing.cpuprofile  (open in Chrome DevTools Performance)
# → /tmp/loki-typing.before.heapsnapshot
# → /tmp/loki-typing.after.heapsnapshot
```
...
1. **Per-keystroke `scrollHeight` read removed.** The expansion useEffect
   ...
   (forces synchrowundercorp layout). Replaced with a `draft.length > 60`
