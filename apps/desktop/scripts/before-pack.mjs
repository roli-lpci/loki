 ...
 * product name (`Loki`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Loki'
 *
 * This is a hard failure with no obvious cause for the user — `loki desktop`
 ...
 * (the mac-unpacked Loki.app bundle) and Windows (win-unpacked), so we
 ...
 * Loki.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `loki desktop --build-only` (loki_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Loki.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Loki'}.exe`
