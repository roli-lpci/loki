 ...
 *
 * Stamps the Loki icon + identity onto the packed Windows Loki.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `loki desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Loki')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Loki'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Loki.exe keeps the stock Electron icon`)
  }
}
