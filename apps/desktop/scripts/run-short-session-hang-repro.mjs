...
const UPSTREAM_URL = 'https://github.com/wundercorp/loki.git'
...
const LEGACY_STORAGE_KEY = 'loki.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'loki.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.lokiDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `loki-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const lokiHome = join(sandbox, 'loki-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(lokiHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(lokiHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        LOKI_DESKTOP_APP_NAME: `LokiShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        LOKI_DESKTOP_ROOT: target.targetRoot,
        LOKI_DESKTOP_IGNORE_EXISTING: '1',
        LOKI_DESKTOP_USER_DATA_DIR: userData,
        LOKI_HOME: lokiHome,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'loki-short-session-ab-'))
  const prepared = []
