#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Loki icon + version metadata onto the
// built Loki.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Loki.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `loki desktop`, the installer's
...
// path (which rebuilds via `loki desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Loki.exe>
//
...
// Stamp the Loki icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Loki',
      FileDescription: 'Loki',
      CompanyName: 'WunderCorp, Inc.',
      LegalCopyright: 'Copyright (c) 2026 WunderCorp, Inc.'
    }
  })

  console.log('[set-exe-identity] done — Loki icon + identity stamped')
}
