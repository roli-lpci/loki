...
// no longer bundles the Loki Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Loki.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Loki'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Loki.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Loki'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default LOKI_HOME for non-sandboxed runs -- matches main.ts's
// resolveLokiHome(). On Windows it's %LOCALAPPDATA%\loki; elsewhere
// it's ~/.loki. The fresh-install sandbox launchFresh() sets its own
// LOKI_HOME and never touches this.
const DEFAULT_LOKI_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'loki')
  }
  return path.join(os.homedir(), '.loki')
})()
const VENV_ROOT = path.join(DEFAULT_LOKI_HOME, 'loki-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'loki-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.LOKI_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Loki-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Loki-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Loki-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Loki-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.LOKI_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.LOKI_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const lokiHome = path.join(sandbox, 'loki-home')
  ...
  env.LOKI_DESKTOP_CWD = cwd
  env.LOKI_DESKTOP_IGNORE_EXISTING = '1'
  env.LOKI_DESKTOP_TEST_MODE = 'fresh-install'
  env.LOKI_DESKTOP_USER_DATA_DIR = userDataDir
  env.LOKI_HOME = lokiHome
  delete env.LOKI_DESKTOP_CLI
  delete env.LOKI_DESKTOP_ROOT
  ...
  console.log(`  LOKI_HOME: ${lokiHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(lokiHome, 'loki-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Loki Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of loki_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'loki-agent', 'loki_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Loki
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + LOKI_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  LOKI_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
