...
**The native desktop app for [Loki Agent](../../README.md) — the self-improving AI agent from [WunderCorp, Inc.](https://wundercorp.co).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Loki surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Loki and hear it back.</td></tr>
...
### Install with Loki (recommended)

Already have the Loki CLI? Just run:

```bash
loki desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Loki gateway or install Loki locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Loki Desktop website.](https://loki.computer/).
...
```bash
# throwaway LOKI_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Loki Agent runtime into `LOKI_HOME`
(`~/.loki`, or `%LOCALAPPDATA%\loki` on Windows), using the same layout as a
CLI install.
...
- **Loki Agent** runs as a headless `loki serve` process and exposes the
...
4. `LOKI_DESKTOP_CLI`, or `loki` on `PATH`
5. a system Python that can import the Loki runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Loki
...
screen offers **Connect to existing Loki** before starting the local installer.
...
terminal commands, and file operations run against the remote Loki host, not
...
transport- or Loki-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Loki-Session-Token`.
...
Boot logs land in `LOKI_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\loki\loki-agent\.loki-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\loki\loki-agent\venv"
```

> The default Loki home on Windows is `%LOCALAPPDATA%\loki`. Set the `LOKI_HOME` env var if you've relocated it.
