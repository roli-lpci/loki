...
import type { LokiReadDirResult } from '@/global'
import type * as LokiModule from '@/loki'
...
// getStatus would supply the connected backend's loki_home — a REMOTE path in
// remote mode. The disk scanner must NOT derive the plugin root from it (#66899).
const getStatus = vi.fn(async () => ({ loki_home: '/remote/box/.loki' }))

vi.mock('@/loki', async importActual => ({
  ...(await importActual<typeof LokiModule>()),
  getStatus: () => getStatus()
}))

const desktopPluginsRoot = vi.fn<() => Promise<string>>()
const readDir = vi.fn<(path: string) => Promise<LokiReadDirResult>>()
...
beforeEach(() => {
  ...
  getStatus.mockClear()
  ;(window as unknown as { lokiDesktop: unknown }).lokiDesktop = {
    desktopPluginsRoot,
...
afterEach(() => {
  delete (window as unknown as { lokiDesktop?: unknown }).lokiDesktop
})

describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend loki_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.loki/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })

    await discoverRuntimePlugins()

    expect(desktopPluginsRoot).toHaveBeenCalled()
    expect(readDir).toHaveBeenCalledWith('/local/.loki/desktop-plugins')
    ...
    expect(readDir).not.toHaveBeenCalledWith('/local/.loki/plugins')
    // The remote backend's loki_home must never feed the local plugin scan.
    expect(getStatus).not.toHaveBeenCalled()
    expect(readDir).not.toHaveBeenCalledWith('/remote/box/.loki/desktop-plugins')
  })
  ...
  it('treats a folder without plugin.js as metadata, not a throwing file read', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.loki/desktop-plugins') {
        return {
          entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.loki/desktop-plugins/my-feature' }]
        }
      }

      if (dir === '/local/.loki/desktop-plugins/my-feature') {
        return {
          entries: [
            { isDirectory: false, name: 'README.md', path: '/local/.loki/desktop-plugins/my-feature/README.md' }
          ]
        }
      }
    ...
    expect(readDir).toHaveBeenCalledWith('/local/.loki/desktop-plugins/my-feature')
  ...
  it('a DIRECTORY named plugin.js is not a plugin entry (metadata walk rejects it)', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.loki/desktop-plugins') {
        return { entries: [{ isDirectory: true, name: 'odd', path: '/local/.loki/desktop-plugins/odd' }] }
      }

      if (dir === '/local/.loki/desktop-plugins/odd') {
        // A folder literally named plugin.js — must resolve to "no entry".
        return {
          entries: [{ isDirectory: true, name: 'plugin.js', path: '/local/.loki/desktop-plugins/odd/plugin.js' }]
        }
      }
  ...
  it('loads a unified desktop half (app-root copy + package marker) OPT-IN and tags it with its package', async () => {
    ...
    const root = '/local/.loki/desktop-plugins'
    ...
    readDir.mockImplementation(async dir => {
      ...
      if (dir === `${root}/uni`) {
        return {
          entries: [
            { isDirectory: false, name: '.loki-package.json', path: `${root}/uni/.loki-package.json` },
    ...
    readFileText.mockImplementation(async file =>
      file.endsWith('.loki-package.json')
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches the Electron-resolved app root, never the backend loki_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.loki/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.loki/desktop-plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.loki/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
...
describe('plugin source reads (512 KiB preview-cap bug)', () => {
  ...
  const standaloneRootWith = (name: string) => {
    const folder = `/local/.loki/desktop-plugins/${name}`
  ...
  it('loads the full source via readPluginSource when the shell offers it', async () => {
    ;(window.lokiDesktop as unknown as { readPluginSource: unknown }).readPluginSource = readPluginSource
    desktopPluginsRoot.mockResolvedValue('/local/.loki/desktop-plugins')
    ...
    try {
      ...
      expect(readPluginSource).toHaveBeenCalledWith('/local/.loki/desktop-plugins/big/plugin.js')
      expect(register).toHaveBeenCalledTimes(1)
  ...
  it('older shell without readPluginSource: a truncated preview read fails LOUDLY, never evaluates', async () => {
    ...
    try {
      ...
      expect($pluginRecords.get().huge).toMatchObject({
        kind: 'disk',
        status: 'error',
        file: '/local/.loki/desktop-plugins/huge/plugin.js'
      })
...
describe('bundled-shadowed disk copies', () => {
  it('skips a disk copy of a bundled plugin but publishes a visible inventory row', async () => {
    ...
    publishPlugin({ id: 'loki-bots', name: 'Bot Mode', kind: 'bundled', status: 'loaded' })
    ...
    try {
      const id = await loadRuntimePlugin(
        'export default { id: "loki-bots", name: "Bot Mode", register() {} }',
        'loki-bots',
        { file: '/local/.loki/desktop-plugins/loki-bots/plugin.js' }
      )

      // Skipped — the bundled copy stays the only live registration...
      expect(id).toBeNull()
      expect($pluginRecords.get()['loki-bots']).toMatchObject({ kind: 'bundled', status: 'loaded' })
      ...
      expect($pluginRecords.get()['loki-bots:disk-shadowed']).toMatchObject({
        kind: 'disk',
        status: 'disabled',
        file: '/local/.loki/desktop-plugins/loki-bots/plugin.js'
      })
