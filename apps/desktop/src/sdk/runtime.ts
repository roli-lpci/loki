 ...
 * plugins resolve `@wundercorp/loki-plugin-sdk` through the vite alias; RUNTIME-loaded
...
// `sdk/runtime` — so a module-scope `{ __LOKI_PLUGIN_SDK__: sdk, … }` is
...
// `Object.keys(GLOBALS.__LOKI_PLUGIN_SDK__)` threw
...
function pluginNamespaces() {
  return {
    __LOKI_PLUGIN_SDK__: sdk,
    __LOKI_REACT__: React,
    __LOKI_REACT_JSX__: jsxRuntime,
    __LOKI_REACT_JSX_DEV__: jsxDevRuntime
  }
}
...
export function sdkImportMap(): Record<string, string> {
  cached ??= {
    '@wundercorp/loki-plugin-sdk': shimUrl('__LOKI_PLUGIN_SDK__'),
    'react/jsx-dev-runtime': shimUrl('__LOKI_REACT_JSX_DEV__'),
    'react/jsx-runtime': shimUrl('__LOKI_REACT_JSX__'),
    react: shimUrl('__LOKI_REACT__')
  }
