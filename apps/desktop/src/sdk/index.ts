/**
 * @wundercorp/loki-plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__LOKI_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, lokiApi, type LokiGateway } from '@/loki'
...
import type { PaginatedSessions, UsageStats } from '@/types/loki'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Loki profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.lokiDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.lokiDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and LokiGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (loki-agent#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.lokiDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Loki Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.lokiDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Loki Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Loki Desktop to open another Bot chat.' })
   ...
   *
   *  `isStaleTile` (loki-agent#90102): the caller's reconciliation probe
  ...
  profileRoutes: async () => {
    ...
    if (!getProfileRoutes) {
      throw new Error('Loki Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return lokiApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return lokiApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
 ...
 *  "LOKI AGENT" with it, a `chat.empty` contribution writes its own name. */
...
export type { LokiGateway } from '@/loki'
...
export type { LokiOpenTarget } from '@/lib/loki-open-target'
...
/** Loki' reasoning levels + their compact labels, so a plugin surfacing a
...
export type { RpcEvent, StatusResponse } from '@/types/loki'
