...
vi.mock('@/loki', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), lokiApi: vi.fn() }))
...
const { deleteProfile, lokiApi } = await import('@/loki')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its LOKI_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { lokiDesktop: unknown }).lokiDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(lokiApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(lokiApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(lokiApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
