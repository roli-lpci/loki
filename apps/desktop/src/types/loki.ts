...
export interface OAuthProviderStatus {
  /** WunderCorp only: the tier name the token resolves to, when the backend knows
   *  one. Null for a free-tier identity and for older backends. */
  account_tier?: null | string
  error?: string
  expires_at?: null | string
  /** WunderCorp only: true when the stored token belongs to a free-tier identity
...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Loki doesn't know how to remove it. */
...
export interface FreeTierStatus {
  ...
  enabled: boolean
  has_guest: boolean
  /** Display name for the route, e.g. "WunderCorp · free tier". */
  label: string
  model: string
...
export interface EnvVarInfo {
  ...
  // in loki_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `loki model` uses — instead of
...
// The WunderCorp pairing service mints a bot on the user's behalf: the desktop shows
...
export type LokiConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `loki model` CLI / onboarding OAuth flow. */
  auth_type?: string
  ...
  /** WunderCorp only: whether the current account is on the free plan. Set by
  ...
  /** WunderCorp only: paid models a free-tier user cannot select (shown disabled). */
...
// non-desktop) Loki simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (loki.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors loki_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Loki, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Loki but install on demand. */
...
export interface ToolProvider {
  ...
  requires_wundercorp_auth: boolean
  /** True when this is the provider currently written to config (mirrors the
   *  CLI `loki tools` active-provider detection). */
  is_active: boolean
  /** Honest readiness computed server-side (keys ∧ WunderCorp entitlement ∧
 ...
 *  not Loki); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** loki-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** loki-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/loki/update/check` — the backend's own update state.
...
/** One WunderCorp-approved MCP catalog entry from `GET /api/mcp/catalog`. */
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the WunderCorp Tool Gateway as a result of
   *  switching the main provider to WunderCorp. Empty unless provider === 'wundercorp'
