...
const { getProfiles } = await import('@/loki')
...
const remoteConn = (over: Partial<LokiConnection> = {}): LokiConnection =>
  ({ baseUrl: 'https://loki-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as LokiConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { lokiDesktop: { getConnection } })
