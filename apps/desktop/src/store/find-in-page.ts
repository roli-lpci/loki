 ...
 * `updateFindResults` synchrowundercorply and never wires this listener.
 */
export function initFindInPageListener(): () => void {
  listenerRefs += 1

  if (listenerRefs === 1) {
    detachListener = window.lokiDesktop?.onFoundInPage?.(result => {
...
export function initOpenFindBarListener(): () => void {
  ...
  if (openFindBarRefs === 1) {
    detachOpenFindBar = window.lokiDesktop?.onOpenFindBarRequested?.(() => {
      openFindBar()
    })
  }
