...
/**
 * Feature store for backend (agent) plugins — the native Loki plugins plus
 ...
 * primitives `loki plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Loki. */
  portable?: boolean
 ...
 *  LOKI_HOME. Always refetches (it's a cheap local disk scan on the
...
export async function installAgentPlugin(
  ...
  opts: {
    ...
    ref?: string
    /** Target profile's LOKI_HOME (null/undefined = backend launch profile). */
    profile?: string | null
  }
