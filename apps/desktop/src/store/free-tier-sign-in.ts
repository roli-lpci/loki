...
import { cancelOAuthSession, listOAuthProviders, pollOAuthSession, startOAuthLogin } from '@/loki'

import { type FreeTierRequester, WUNDERCORP_PROVIDER_ID, refreshFreeTierStatus } from './free-tier'
...
/**
 * Drive one sign-in attempt end to end: resolve what identity this Loki is
...
export async function beginFreeTierSignIn(requestGateway: FreeTierRequester) {
  ...
  // No free-tier identity AND a real WunderCorp account already connected: there is
  ...
  if (minting) {
    try {
      ...
      const wundercorp = providers.find(provider => provider.id === WUNDERCORP_PROVIDER_ID)

      if (wundercorp?.status.logged_in && wundercorp.status.free_tier !== true) {
  ...
  try {
    const start = await startOAuthLogin(WUNDERCORP_PROVIDER_ID)
...
async function pollOnce(sessionId: string, requestGateway: FreeTierRequester, mine: number) {
  ...
  try {
    const result = await pollOAuthSession(WUNDERCORP_PROVIDER_ID, sessionId)
