...
const agentConn = (over: Partial<LokiConnection> = {}): LokiConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as LokiConnection

const localConn = (over: Partial<LokiConnection> = {}): LokiConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as LokiConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<LokiConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<LokiConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { lokiDesktop: { getConnection, getConnectionFor }, localStorage })
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  it('runs the hook inside the serialized section, right before the activation; a declined hook activates nothing', async () => {
    ...
    // An accepted hook runs synchrowundercorply before the activation starts.
    await ensureGatewayAgent('homelab', 'research', {
      beforeActivate: () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<LokiConnection>(() => undefined))
