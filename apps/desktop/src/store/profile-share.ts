 ...
 *
 * The archive is the CLI's own `loki profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/loki'
...
import type { ProfileDesktopOverlay } from '@/types/loki'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Loki profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.lokiDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.lokiDesktop?.selectPaths?.({
