import type { ConnectionState } from '@wundercorp/loki-shared'
...
import type { SessionInfo, UsageStats } from '@/types/loki'
...
const COMPOSER_MODEL_KEY = 'loki.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'loki.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'loki.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'loki.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'loki.desktop.composer.fast'
...
function composerScopeForConnection(connection: LokiConnection | null): string | null {
  if (!connection) {
    return null
  }
...
const LAST_SESSION_KEY = 'loki.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'loki.desktop.lastRoute'
...
// redundant synchrowundercorp localStorage reads on every get/set call within
// the same synchrowundercorp block. The flag resets on cross-window `storage`
...
function workspaceCwdKey(connection: LokiConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.lokiDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.lokiDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<LokiConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'loki.desktop.sessionOwnerHints.v1'
 ...
 * LokiConnection lookup is unavailable. */
...
export const setConnection = (next: Updater<LokiConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useLokiConfig.
