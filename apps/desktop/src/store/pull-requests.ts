...
import type { LokiBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/loki'
...
export const $pullRequestsByBranch = atom<Record<string, LokiBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'loki.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('loki.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: LokiBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
