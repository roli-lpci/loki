...
export function hasRegistryTopology(): boolean {
  // The bridge exists before its asynchrowundercorp cache load. Treat that window
  ...
  return $connectionsRegistry.get() !== null || Boolean(window.lokiDesktop?.connections?.list)
}
