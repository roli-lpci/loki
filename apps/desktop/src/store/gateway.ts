import { type ConnectionState, type GatewayEvent, registryBackendScopeKey, resolveGatewayWsUrl } from '@wundercorp/loki-shared'
import { atom } from 'nanostores'

import type { LokiConnection } from '@/global'
import { LokiGateway, setApiRequestConnection } from '@/loki'
...
function dialProfile(
  desktop: NonNullable<typeof window.lokiDesktop>,
  profile: string,
...
const isOpen = (gateway: LokiGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: LokiConnection) => void
  /**
...
interface Secondary {
  ...
  connection: LokiConnection | null
  gateway: LokiGateway
...
interface GatewayRegistryState {
  ...
  primaryGateway: LokiGateway | null
  ...
  $gateway: ReturnType<typeof atom<LokiGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('loki.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<LokiGateway | null>(null),
...
// only writer is applyActive(), which sets it in the same synchrowundercorp step
...
export function setPrimaryGateway(gateway: LokiGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<LokiConnection, 'connectionId'> | null): void {
...
async function isAttachedSharedRemote(
  ...
  const desktop = window.lokiDesktop
...
async function requestOnPrimaryGateway<T>(
  ...
  if (!gateway || !isOpen(gateway)) {
    throw new Error('Loki gateway unavailable')
  }
...
export function activeGateway(): LokiGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the loki module (null
  ...
  // Publish the BARE profile this route serves, in the same synchrowundercorp step
...
function publishActiveConnection(connection: LokiConnection): void {
...
async function gatewayForProfile(
  ...
  leaseRequest = false,
  spawnPriority: SpawnPriority = 'background'
): Promise<{ gateway: LokiGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
  const key = normKey(profile)
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Loki gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.lokiDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Loki Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<LokiGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Loki gateway is not connected" on the Sessions `+` action (#88880).
