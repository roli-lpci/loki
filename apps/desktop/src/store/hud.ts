 ...
 * composer with the reply scrolling above it, so Loki can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.lokiDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.lokiDesktop?.hud
...
export function resetHudLayout(): void {
  void window.lokiDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.lokiDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.lokiDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
