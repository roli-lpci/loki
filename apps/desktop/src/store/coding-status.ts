...
import type { LokiGitWorktree, LokiRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: LokiGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, LokiRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, LokiGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<LokiRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<LokiGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<LokiRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<LokiGitWorktree[]>>()
const $noRepoStatus = atom<LokiRepoStatus | null>(null)
const $noWorktrees = atom<LokiGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<LokiRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<LokiGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: LokiRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: LokiGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<LokiRepoStatus | null>
  seq: number
  target: string
}
...
async function drainRepoStatusRefreshes(): Promise<void> {
  ...
  // This reset is synchrowundercorp with the final empty-queue check. A refresh
