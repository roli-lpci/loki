 ...
 *
 * Local models ship on main behind `--local` (either `loki desktop --local`
 * or the flag on Loki.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.lokiDesktop?.localModelsEnabled === true
)
