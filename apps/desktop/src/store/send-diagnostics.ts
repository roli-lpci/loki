...
// user explicitly clicks Upload → diagnostics.share_wundercorp runs backend-side
// (collect + force-redact + WunderCorp-S3 upload) → the modal shows the private
// view link plus the support handoff (GitHub Issues · WunderCorp Portal Support ·
// Discord).
//
// Consent is per-upload and explicit — no "always allow", mirroring the CLI's
// `loki debug share --wundercorp` confirmation contract. On a remote connection
...
interface ShareWunderCorpResponse {
  error?: string
  expires_at?: string
  ok: boolean
  upload_id?: string
...
async function collectLocalExtras(): Promise<Record<string, string>> {
  try {
    const logs = await window.lokiDesktop?.getRecentLogs?.()
...
export async function confirmSendDiagnostics(): Promise<void> {
  ...
  try {
    ...
    const response = await gateway.request<ShareWunderCorpResponse>(
      'diagnostics.share_wundercorp',
      {
