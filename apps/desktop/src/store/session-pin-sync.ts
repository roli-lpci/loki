...
import { setSessionPinnedRemote } from '@/loki'
...
function pullRemotePins(): void {
  ...
  for (const row of rowsByPinId(loadedSessionRows()).values()) {
    ...
    if (row.pinned && !heldLocally) {
      // Mark mirrored first: pinSession fires the pin listener synchrowundercorply,
...
// pinSession/unpinSession), which fires reconcile() again synchrowundercorply.
...
function reconcileInner(): void {
  ...
  // Push before pull. The pin listener fires synchrowundercorply on a local toggle,
