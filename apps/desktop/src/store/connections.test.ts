...
// (barrier → machine-context reset → wipe, synchrowundercorply; the barrier is
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { lokiDesktop: { connections: { list, setLastUsed } }, localStorage })
})
...
describe('selectConnection', () => {
  ...
  it('an activation that published the new source but never settled counts as committed', async () => {
    ...
    try {
      setConnectionsRegistry(registry)
      $connection.set({ connectionId: 'local', mode: 'local' })
      // The socket activates and publishes synchrowundercorply; only the trailing
    ...
    try {
      ...
      ensureGatewayAgent
        .mockImplementationOnce((connectionId, profile, options) => {
          ...
          // Low-level activation publishes synchrowundercorply. The trailing descriptor
