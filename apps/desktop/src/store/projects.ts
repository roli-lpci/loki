...
import type { LokiGitBaseBranch, LokiGitBranch } from '@/global'
import { getLokiConfig, lokiApi, type LokiGateway } from '@/loki'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/loki'
...
const PROJECT_SCOPE_KEY = 'loki.desktop.projectScope'
...
async function activeProjectsContext(profile = projectProfile()): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== normalizeProfileKey($activeGatewayProfile.get())) {
    throw new Error('Active Loki profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await lokiApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<LokiGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<LokiGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Loki
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getLokiConfig(context.profile))
...
// repo, returning where Loki should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<LokiGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<LokiGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.lokiDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.lokiDesktop?.writeClipboard?.(path)
  }
}
