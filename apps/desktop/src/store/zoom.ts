...
export function setZoomPercent(percent: number): void {
  window.lokiDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.lokiDesktop?.zoom) {
  void window.lokiDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.lokiDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
