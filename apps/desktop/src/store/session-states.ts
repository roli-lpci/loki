...
import { type GatewayEvent, LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@wundercorp/loki-shared'
...
// is published later and asynchrowundercorply: navigate → route effect →
...
const TILES_KEY = 'loki.desktop.sessionTiles.v2'
const LEGACY_TILES_KEY = 'loki.desktop.sessionTiles.v1'
...
const BOT_CHAT_SCOPE_KEY = 'loki.desktop.botChatSessions.v1'
 ...
 *  (loki-agent#90102). The tile bucket is a Local Storage cache, and a
 ...
 * restore re-dials the profile's backend, whose ensure_loki_home() re-creates
 * the profile directory the delete just removed (loki-agent#94235). Same
...
export function dropTilesForProfile(
  ...
  const ownerMatches = (owner: SessionProfileRoute | undefined): boolean => {
    ...
    // would orphan a live conversation (loki-agent#94235). Tiles persisted
...
// Dev hook for automation (mirrors __LOKI_LAYOUT_TREE__).
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__LOKI_SESSION_TILES__ = {
