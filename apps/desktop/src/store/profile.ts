import { LOCAL_CONNECTION_ID } from '@wundercorp/loki-shared'
import { atom, batch, computed } from 'nanostores'

import type { LokiConnection } from '@/global'
import { getProfiles, lokiApi, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/loki'
...
import type { ProfileInfo } from '@/types/loki'
...
// /api/profiles/active `current`). "default" is the root ~/.loki. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'loki.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'loki.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await lokiApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new LOKI_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.lokiDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<LokiConnection | null> {
  const getConnection = window.lokiDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<LokiConnection | null> {
  const getConnectionFor = window.lokiDesktop?.getConnectionFor
...
// activates it synchrowundercorply, which lets the caller sever the previous
...
// section, synchrowundercorply right before the socket is activated — i.e. after
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'loki.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.lokiDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.lokiDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.loki) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.lokiDesktop?.touchBackend?.(target).catch(() => undefined)
}
