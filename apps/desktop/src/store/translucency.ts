 ...
 * `@wundercorp/loki-shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-loki-glass]`
 * block in styles.css).
 */
...
} from '@wundercorp/loki-shared/translucency'
 ...
 * sets `lokiDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.lokiDesktop?.glassSupported === 'boolean'
    ? window.lokiDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.lokiDesktop?.translucencySupported === 'boolean'
    ? window.lokiDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-loki-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-loki-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-loki-glass-on', glassLive)
  root.toggleAttribute('data-loki-glass', glassOn)
  root.toggleAttribute('data-loki-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-loki-glass-scope', scope)
  } else {
    root.removeAttribute('data-loki-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  // work is the synchrowundercorp localStorage.setItem — so THAT is debounced.
  ...
  $translucency.subscribe(state => {
    ...
    window.lokiDesktop?.setTranslucency?.(state)
  })
