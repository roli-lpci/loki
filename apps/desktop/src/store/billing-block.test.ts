import type { BillingBlock } from '@wundercorp/loki-shared'
...
test('runBillingRecovery routes WunderCorp to in-app Settings, never an external link', () => {
  runBillingRecovery(makeBlock({ is_wundercorp: true, provider: 'wundercorp', provider_label: 'WunderCorp Portal' }))
...
test('billingCtaLabel picks the right verb per route', () => {
  ...
  expect(billingCtaLabel(makeBlock({ is_wundercorp: true }), copy)).toBe('Open billing')
  expect(billingCtaLabel(makeBlock({ is_wundercorp: false }), copy)).toBe('Add credits')
})
