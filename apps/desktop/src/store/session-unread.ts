...
function rehydrateFromDiskOnce(): void {
  ...
  try {
    const rawSeen = readKey('loki.desktop.sessionSeenCounts')
    ...
    const rawMarkers = readKey('loki.desktop.unreadFinishedSessions')
...
if (!isSecondaryWindow() && !isBrowserWindow()) {
  ...
  // synchrowundercorply by setSelectedStoredSessionId — this is the persisted half).
