 ...
 * app's bounds and stays visible while Loki is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'loki.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'loki.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.lokiDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.lokiDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.lokiDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.lokiDesktop?.petOverlay?.close()
}
