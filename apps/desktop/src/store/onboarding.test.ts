...
describe('refreshOnboarding', () => {
  ...
  it('does not downgrade configured=true on fallback-only readiness failures', async () => {
    ...
    window.localStorage.setItem('loki-desktop-onboarded-v1', '1')
    ...
    expect(window.localStorage.getItem('loki-desktop-onboarded-v1')).toBe('1')
  })
  ...
  it('enters setup when the selected OpenRouter credential is genuinely empty', async () => {
    ...
    expect($desktopOnboarding.get().reason).toContain('No usable credentials found for openrouter.')
    expect(window.localStorage.getItem('loki-desktop-onboarded-v1')).toBeNull()
  })
...
describe('OAuth onboarding', () => {
  ...
  it('clears stale readiness errors after OAuth succeeds and model confirmation is shown', async () => {
    ...
    installApiMock(async ({ body, path }: { body?: unknown; path: string }) => {
      ...
      if (path === '/api/providers/oauth/wundercorp/submit') {
      ...
      if (path.startsWith('/api/model/recommended-default?')) {
        return { provider: 'wundercorp', model, free_tier: false }
      }

      if (path === '/api/model/set') {
        return { ok: true, provider: 'wundercorp', model, gateway_tools: [] }
      }
    ...
    const requestGateway: OnboardingContext['requestGateway'] = async (method, params) => {
      ...
      if (method === 'setup.runtime_check') {
        expect(params).toEqual({ provider: 'wundercorp' })
    ...
    $desktopOnboarding.set(
      baseState({
        flow: {
          ...
          provider: makeOAuthProvider('wundercorp', 'WunderCorp Portal'),
          start: {
        ...
        reason:
          'No access token found for WunderCorp Portal login. setup.status reports configured credentials, but runtime resolution still failed.',
        requested: true
      })
    )
    ...
    if (state.flow.status === 'confirming_model') {
      expect(state.flow.label).toBe('WunderCorp Portal')
  ...
  it('does not advance when the default model assignment is not persisted', async () => {
    ...
    installApiMock(async ({ path }: { path: string }) => {
      ...
      if (path.startsWith('/api/model/options')) {
        return { providers: [{ name: 'WunderCorp Portal', slug: 'wundercorp', models: [model] }] }
      }
...
describe('device-code poll expiry', () => {
  ...
  function deviceCodeProvider() {
    ...
    return { ...makeOAuthProvider('wundercorp', 'WunderCorp Portal'), flow: 'device_code' as const }
  }
  ...
  it('lapses to an error with actionable guidance when the window expires still pending', async () => {
    vi.useFakeTimers()
    installApiMock(async ({ path }: { path: string }) => {
      if (path === '/api/providers/oauth/wundercorp/start') {
        return deviceStart(2)
      }

      if (path === '/api/providers/oauth/wundercorp/poll/device-sess-1') {
