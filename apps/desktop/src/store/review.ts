...
import type { LokiReviewFile, LokiReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Loki' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'loki.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'loki.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'loki.desktop.reviewTreeMode'
const SELECTED_KEY = 'loki.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<LokiReviewFile[]>([])
...
export const $reviewShipInfo = atom<LokiReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['lokiDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: LokiReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly LokiReviewFile[], path: string): LokiReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.lokiDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.lokiDesktop?.openExternal?.(url)
    }
