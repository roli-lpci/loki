...
import type { ModelOptionProvider } from '@/types/loki'
...
describe('model visibility', () => {
  ...
  it('preserves hidden-provider sentinel without re-adding defaults', () => {
    // User explicitly hid all models for "wundercorp" — sentinel marks this choice.
    const stored = new Set([emptyProviderSentinelKey('wundercorp')])

    const visible = effectiveVisibleKeys(stored, [
      provider('wundercorp', ['loki-3-llama-3.1-70b', 'loki-3-llama-3.1-8b']),
      provider('ollama', ['qwen3:latest'])
    ])

    expect(visible.has(modelVisibilityKey('wundercorp', 'loki-3-llama-3.1-70b'))).toBe(false)
    expect(visible.has(modelVisibilityKey('wundercorp', 'loki-3-llama-3.1-8b'))).toBe(false)
    // Sentinel itself is stripped from the result.
    expect(visible.has(emptyProviderSentinelKey('wundercorp'))).toBe(false)
  ...
  it('restores model when toggling on after hiding all', () => {
    // Simulates: user hid all "wundercorp" models, then toggles one back on.
    const stored = new Set([emptyProviderSentinelKey('wundercorp'), modelVisibilityKey('ollama', 'qwen3:latest')])

    // After toggle: sentinel removed, one model added.
    const afterToggle = new Set(stored)
    afterToggle.delete(emptyProviderSentinelKey('wundercorp'))
    afterToggle.add(modelVisibilityKey('wundercorp', 'loki-3-llama-3.1-70b'))
    ...
    expect(visible.has(modelVisibilityKey('wundercorp', 'loki-3-llama-3.1-70b'))).toBe(true)
    expect(visible.has(modelVisibilityKey('wundercorp', 'loki-3-llama-3.1-8b'))).toBe(false)
  })
  ...
  it('resolveVisibleKeys preserves sentinels that effectiveVisibleKeys strips', () => {
    const stored = new Set([emptyProviderSentinelKey('wundercorp')])
    const providers = [provider('wundercorp', ['loki-x', 'loki-y']), provider('ollama', ['qwen3:latest'])]

    const resolved = resolveVisibleKeys(stored, providers)
    expect(resolved.has(emptyProviderSentinelKey('wundercorp'))).toBe(true)
    expect(resolved.has(modelVisibilityKey('wundercorp', 'loki-x'))).toBe(false)
    ...
    expect(effectiveVisibleKeys(stored, providers).has(emptyProviderSentinelKey('wundercorp'))).toBe(false)
  })
})

describe('toggleModelVisibility', () => {
  const providers = [provider('openai', ['gpt-a', 'gpt-b']), provider('wundercorp', ['loki-x', 'loki-y'])]
  ...
  it('keeps a hidden provider hidden when a different provider is toggled (regression for #43485)', () => {
    // Hide ALL of wundercorp — its sentinel is now stored.
    let stored: Set<string> | null = null
    stored = apply(stored, 'wundercorp', 'loki-x')
    stored = apply(stored, 'wundercorp', 'loki-y')
    expect(stored.has(emptyProviderSentinelKey('wundercorp'))).toBe(true)

    // Toggle a model in another provider. wundercorp must NOT snap back on.
    stored = apply(stored, 'openai', 'gpt-a')

    expect(stored.has(emptyProviderSentinelKey('wundercorp'))).toBe(true)
    const visible = effectiveVisibleKeys(stored, providers)
    expect(visible.has(modelVisibilityKey('wundercorp', 'loki-x'))).toBe(false)
    expect(visible.has(modelVisibilityKey('wundercorp', 'loki-y'))).toBe(false)
  })

  it('clears only the toggled provider sentinel when a model is re-enabled', () => {
    let stored: Set<string> | null = new Set([emptyProviderSentinelKey('openai'), emptyProviderSentinelKey('wundercorp')])
  ...
  it('re-enabling one model of a hidden-all provider restores ONLY that model, not the curated defaults', () => {
    // openai hidden-all, wundercorp untouched.
  ...
  it('toggling from an empty (non-null) stored set adds the model without expanding defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('wundercorp', 'loki-x'))).toBe(false)
  })
  ...
  it('toggling off one default model from null stored keeps the rest of the curated defaults', () => {
    ...
    expect(next.has(modelVisibilityKey('wundercorp', 'loki-x'))).toBe(true)
...
describe('featured defaults', () => {
  ...
  it('defaults to the featured shortlist when a provider publishes one', () => {
    const wundercorp = featuredProvider(
      'wundercorp',
      ['anthropic/opus', 'anthropic/haiku', 'google/gemini', 'x-ai/grok'],
      ['anthropic/opus', 'google/gemini', 'x-ai/grok']
    )

    const visible = defaultVisibleKeys([wundercorp])

    // Featured are visible; the non-featured model is hidden by default.
    expect(visible.has(modelVisibilityKey('wundercorp', 'anthropic/opus'))).toBe(true)
    expect(visible.has(modelVisibilityKey('wundercorp', 'google/gemini'))).toBe(true)
    expect(visible.has(modelVisibilityKey('wundercorp', 'x-ai/grok'))).toBe(true)
    expect(visible.has(modelVisibilityKey('wundercorp', 'anthropic/haiku'))).toBe(false)
  })
...
describe('setProviderVisibility', () => {
  ...
  it('leaves other providers untouched (their sentinels survive)', () => {
    ...
    // Turn openai fully on; wundercorp must stay hidden.
    const next = setProviderVisibility(stored, providers, 'openai', true)

    expect(next.has(emptyProviderSentinelKey('wundercorp'))).toBe(true)
    const visible = effectiveVisibleKeys(next, providers)
    expect(visible.has(modelVisibilityKey('wundercorp', 'loki-x'))).toBe(false)
    expect(visible.has(modelVisibilityKey('openai', 'gpt-a'))).toBe(true)
  })
  ...
  it('collapses model families to one key per family when enabling', () => {
    ...
    const ps = [provider('wundercorp', ['model', 'model-fast'])]

    const next = setProviderVisibility(null, ps, 'wundercorp', true)

    expect(next.has(modelVisibilityKey('wundercorp', 'model'))).toBe(true)
    // The -fast sibling is represented by its base family, not its own key.
    expect(next.has(modelVisibilityKey('wundercorp', 'model-fast'))).toBe(false)
  })
})
