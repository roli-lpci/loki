...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    getLokiConfigRecord: vi.fn(),
    notify: vi.fn()
  }
})

vi.mock('@/loki', () => ({
  getLokiConfigRecord: mocks.getLokiConfigRecord,
  testMcpServer: vi.fn()
}))
...
afterEach(() => {
  ...
  mocks.getLokiConfigRecord.mockReset()
...
it('coalesces reconnects during a sweep into one fresh follow-up sweep', async () => {
  ...
  mocks.getLokiConfigRecord.mockReturnValueOnce(first).mockResolvedValue({ mcp_servers: {} })

  startMcpHealthChecker()
  mocks.gatewayState.set('open')
  await flush()
  expect(mocks.getLokiConfigRecord).toHaveBeenCalledTimes(1)
  ...
  await flush()
  await flush()
  expect(mocks.getLokiConfigRecord).toHaveBeenCalledTimes(2)
})
