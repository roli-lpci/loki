 ...
 *  Working in `~/www/loki-agent`, the draft says "loki-agent" constantly
 *  — a "Use skill: loki-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`loki-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/loki-agent-dev`, `plugin:skill`). */
