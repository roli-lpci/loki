...
import { addMcpServer, getMcpCatalog, listMcpServers, removeMcpServer } from '@/loki'
 ...
 *
 * Matches the draft against the WunderCorp-approved MCP catalog's `suggest`
 * metadata (`GET /api/mcp/catalog` — the same reviewed manifests behind
 * `loki mcp catalog`), by whole-word keyword and pasted-link host suffix,
