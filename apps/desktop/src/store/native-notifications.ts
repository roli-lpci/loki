...
import { type LokiOpenTarget, resolveLokiOpenPath } from '@/lib/loki-open-target'
...
const STORAGE_KEY = 'loki:native-notifications'
...
// "Backgrounded" = the user isn't on Loki. `document.hidden` only flips when
...
export interface NativeNotificationInput {
   ...
   * `sessionId` (plugins). Same vocabulary as `loki://index-network/intent/1`.
   */
  activate?: string
...
export function dispatchNativeNotification(input: NativeNotificationInput): boolean {
  ...
  void window.lokiDesktop?.notify({
...
export interface PluginNotificationAction {
  id: string
  label: string
  /** Navigate here on button press (path or `loki://index-network/intent/1`). */
  activate?: LokiOpenTarget
 ...
 *  user is away from Loki — the in-app toast (`host.notify`) covers the
 *  foreground case. */
export function dispatchPluginNativeNotification(pluginId: string, input: PluginNativeNotificationInput): void {
  const activate = resolveLokiOpenPath(input.activate) ?? undefined
  ...
  const actions: NativeNotificationAction[] | undefined = input.actions?.map(action => ({
    activate: resolveLokiOpenPath(action.activate) ?? undefined,
    id: action.id,
    text: action.label
  }))
