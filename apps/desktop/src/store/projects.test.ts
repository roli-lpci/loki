...
const loki = await import('@/loki')
const getLokiConfig = vi.mocked(loki.getLokiConfig)
...
describe('project scope', () => {
  ...
  it('enterProject scopes the sidebar to the project id', () => {
    ...
    // swallowed); the synchrowundercorp scope change is what matters here.
    enterProject('p_123')
  ...
  it('persists the scope to localStorage', () => {
    enterProject('p_abc')
    expect(window.localStorage.getItem('loki.desktop.projectScope')).toBe('p_abc')
  })
})
  ...
  it('names the stale-backend remedy when a remote gateway lacks the worktree route', async () => {
    ...
    desktopGit.mockReturnValue({
      worktreeAdd: vi.fn(async () => {
        throw new Error(
          'Expected JSON from https://vps/api/git/worktree/add but got HTML (status 404). The endpoint is likely missing on the Loki backend.'
        )
      })
    } as never)
...
describe('createProject', () => {
  ...
  it.each(['default', 'coder'])('creates in the active %s profile without leaving All profiles', async profile => {
    const created = { folders: [], id: 'p_new', name: 'Loki Agent', primary_path: '/srv/loki' }
    ...
    vi.mocked(loki.lokiApi).mockResolvedValue({ projects: [tree], active_id: created.id })
    $activeGatewayProfile.set(profile)
    setShowAllProfiles(true)

    await expect(createProject({ folders: ['/srv/loki'], name: created.name, use: true })).resolves.toEqual(created)
    ...
    expect(loki.lokiApi).toHaveBeenCalledWith(
  ...
  it('does not retarget a project create when the profile changes during reconnect', async () => {
    ...
    const pending = createProject({ folders: ['/srv/loki'], name: 'Loki Agent' })
    const rejection = expect(pending).rejects.toThrow('Active Loki profile changed while connecting')
...
describe('repository discovery policy', () => {
  ...
  it('records disabled policy without invoking the filesystem scanner', async () => {
    ...
    getLokiConfig.mockResolvedValue({
      desktop: {
  ...
  it('passes custom roots and exclusions to Electron and records on the origin gateway', async () => {
    ...
    expect(getLokiConfig).toHaveBeenCalledWith('default')
  ...
  it('does not scan the local filesystem for remote connections but still refreshes the project tree', async () => {
    ...
    expect(scanRepos).not.toHaveBeenCalled()
    expect(getLokiConfig).not.toHaveBeenCalled()
    ...
    // `scan: true`) — repos with zero Loki sessions must still surface —
