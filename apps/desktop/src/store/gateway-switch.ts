...
import { resetSidebarBatchCapability } from '@/loki'
...
// when startLoki re-emits boot progress, and tells the resume path that a
 ...
 * binding to the outgoing backend in ONE synchrowundercorp step. Both switch doors —
...
export function beginGatewaySwitch(): GatewaySwitchToken {
  ...
  try {
    ...
    // A synchrowundercorp wipe has no rollback: once it starts, some outgoing-source
