...
const LAST_PROFILE_STORAGE_KEY = 'loki.desktop.lastProfileByConnection'
...
async function rememberConnection(connectionId: string): Promise<void> {
  const setLastUsed = window.lokiDesktop?.connections?.setLastUsed
 ...
 *     wipe runs inside the activation's serialized section, synchrowundercorply
  ...
  try {
    ...
    try {
      try {
        ...
        // The socket is activated and its descriptor published synchrowundercorply;
