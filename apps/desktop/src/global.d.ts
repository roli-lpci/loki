import type { GatewayWsUrlResult } from '@wundercorp/loki-shared'
import type { TranslucencyState } from '@wundercorp/loki-shared/translucency'

import type { LokiNotification } from '../electron/notification-types'
...
declare global {
  interface Window {
    lokiDesktop: {
      ...
      // Resume this session in the user's own terminal emulator (`loki --tui
      ...
      // Loki Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new LOKI_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: LokiApiRequest) => Promise<T>
      notify: (payload: LokiNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<LokiReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<LokiReadFileTextResult>
      selectPaths: (options?: LokiSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<LokiPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<LokiPreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<LokiPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: LokiActiveWork) => void
      setTitleBarTheme?: (payload: LokiTitleBarTheme) => void
      ...
      /** Launch flag: the WunderCorp free tier is on for this launch
       *  (LOKI_GUEST_ONBOARDING=1 or --guest-onboarding). Read-only fact the
      ...
      zoom?: {
        ...
        /** Synchrowundercorp zoom factor of this window (1 = 100%). */
        factor?: () => number
      ...
      readDir: (path: string) => Promise<LokiReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<LOKI_HOME>/desktop-plugins),
      ...
      /** LOCAL `<LOKI_HOME>/logs` (profile-aware) — error card "Open Logs". */
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<LokiGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<LokiGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<LokiGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<LokiRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: LokiReviewScope, baseRef?: null | string) => Promise<LokiReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: LokiReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<LokiReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<LokiRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<LokiPrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: LokiTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<LokiTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: LokiWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: LokiPreviewFileChanged) => void) => () => void
...
export interface LokiTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface LokiTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `loki update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  lokiRoot?: string
...
export interface LokiTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface LokiActiveWork {
  count: number
  titles: string[]
}

export interface LokiWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Loki Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Loki Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Loki Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // Whether the OAuth partition holds a live WunderCorp portal (Privy) session — the
  // portal authenticates via Privy, so this reflects the privy-token cookie, NOT
  // the loki gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Loki Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface DesktopBootProgress {
  error: string | null
  fakeMode: boolean
  /** True when the boot failure is a WunderCorp Cloud agent that is down (HTTP 502/503/504). */
...
export interface LokiApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface LokiPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface LokiReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface LokiPreviewWatch {
  id: string
  path: string
}
...
export interface LokiGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface LokiGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface LokiGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface LokiRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface LokiRepoStatus {
  branch: null | string
...
// turn began.
export type LokiReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface LokiReviewFile {
  path: string
  added: number
  removed: number
...
export interface LokiReviewList {
  files: LokiReviewFile[]
...
export interface LokiBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface LokiRepoPullRequests {
  ghReady: boolean
  prs: LokiBranchPullRequest[]
}
...
export interface LokiPrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface LokiReviewShipInfo {
  ghReady: boolean
  pr: LokiReviewPr | null
}

export interface LokiReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface LokiReadDirResult {
  entries: LokiReadDirEntry[]
  error?: string
}

export interface LokiPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface LokiSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
