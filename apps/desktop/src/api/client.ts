import { JsonRpcGatewayClient } from '@wundercorp/loki-shared'

import type { LokiApiRequest } from '@/global'
...
// "Timed out connecting to Loki backend" that hangs the UI (#48504).
//
...
export class LokiGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Loki gateway connection closed',
      connectErrorMessage: 'Could not connect to Loki gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Loki gateway is not connected',
...
// process to route them to that host (loki:api's registry branch). Null
...
export function lokiApi<T>(request: LokiApiRequest): Promise<T> {
  return window.lokiDesktop.api<T>({ ...connectionScoped(), ...request })
}
