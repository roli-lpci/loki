...
export function getToolsets(profile?: ProfileScope): Promise<ToolsetInfo[]> {
  return window.lokiDesktop.api<ToolsetInfo[]>({
...
export function setToolsetEnabled(
  ...
  return window.lokiDesktop.api<{ ok: boolean; name: string; enabled: boolean }>({
    ...capabilityScoped(profile),
...
export function getToolsetConfig(name: string, profile?: ProfileScope): Promise<ToolsetConfig> {
  return window.lokiDesktop.api<ToolsetConfig>({
    ...capabilityScoped(profile),
...
export function getToolsetModels(
  ...
  return window.lokiDesktop.api<ToolsetModelsResponse>({
    ...capabilityScoped(profile),
...
export function selectToolsetModel(
  ...
  return window.lokiDesktop.api<{ ok: boolean; name: string; model: string }>({
    ...capabilityScoped(profile),
...
export interface SelectToolsetProviderResponse {
  ...
  capability?: string
  /** Present (true) when a managed WunderCorp row was selected but the Portal
   *  entitlement is missing — the row won't activate until the user signs
   *  in to WunderCorp Portal. */
  needs_wundercorp_auth?: boolean
  /** The managed feature key (e.g. "browser") when needs_wundercorp_auth is set. */
  feature?: string
}
...
export function selectToolsetProvider(
  ...
  return window.lokiDesktop.api<SelectToolsetProviderResponse>({
    ...capabilityScoped(profile),
...
export function runToolsetPostSetup(
  ...
  return window.lokiDesktop.api<ActionResponse & { key: string }>({
    ...capabilityScoped(profile),
...
export function getTerminalBackends(): Promise<TerminalBackendsResponse> {
  return lokiApi<TerminalBackendsResponse>({
    ...profileScoped(),
...
export function selectTerminalBackend(backend: string): Promise<{ ok: boolean; backend: string }> {
  return lokiApi<{ ok: boolean; backend: string }>({
    ...profileScoped(),
...
export function getComputerUseStatus(): Promise<ComputerUseStatus> {
  return lokiApi<ComputerUseStatus>({
    ...profileScoped(),
