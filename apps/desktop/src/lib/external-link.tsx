...
const SKIP_PROTO_RE = /^(?:file|data|mailto|javascript|blob|chrome|about|loki):/i
...
export function fetchLinkTitle(url: string): Promise<string> {
  ...
  const bridge = typeof window === 'undefined' ? undefined : window.lokiDesktop?.fetchLinkTitle
...
export function openExternalLink(href: string): void {
  if (href) {
    void window.lokiDesktop?.openExternal?.(href)
  }
}
 ...
 * doesn't cost a context switch out of Loki, and it is the surface the agent
