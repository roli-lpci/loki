 ...
 */
import { getApiRequestConnection, lokiApi } from '@/api/client'
 ...
 * synchrowundercorp-cheap on the no-op paths (no registry topology, scope already
...
export function maybeBackfillLegacySessionOwners(): void {
  ...
  void lokiApi<{ ok: boolean; profile: string; stamped: number }>({
