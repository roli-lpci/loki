...
//
// When someone is in a hands-free "Hey Loki" voice chat, the natural way to
...
// phrase (optionally addressed to Loki), so a real turn that merely contains
...
// Optional address prefixes so "loki stop" / "ok stop" / "hey loki, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey loki', 'hey loki,', 'loki', 'loki,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("loki") is not a stop command on its own.
      continue
    }
 ...
 * to Loki). Returns false for anything that merely contains "stop" as part of
