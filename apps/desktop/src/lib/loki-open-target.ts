 ...
 * `loki://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `loki://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `loki://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type LokiOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const LOKI_PROTOCOL = 'loki:'
...
export function normalizeLokiOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('loki://') || trimmed.startsWith(`${LOKI_PROTOCOL}//`)) {
    try {
      ...
      // loki://index-network/intent/1 → /index-network/intent/1
...
export function resolveLokiOpenPath(target: LokiOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeLokiOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeLokiOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeLokiOpenString(target.path)
 ...
 * (`loki://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromLokiDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveLokiOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveLokiOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `loki://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromLokiDeepLink('open', name, params)
}
