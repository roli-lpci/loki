...
const globalOptions = { model: 'loki-4', provider: 'wundercorp', providers: [] }
...
describe('requestModelOptions', () => {
  ...
  it('recovers an empty gateway catalog through profile-scoped REST without replacing the session selection', async () => {
    const gatewayPayload = { model: 'loki-local', provider: 'loki-local' }

    const restPayload = {
      model: 'profile-default',
      provider: 'openai-codex',
      providers: [{ models: ['loki-local'], name: 'Loki Local vLLM', slug: 'loki-local' }]
    }
  ...
  it('keeps the gateway result when both catalog paths have no selectable models', async () => {
    const gatewayPayload = { model: 'loki-local', provider: 'loki-local', providers: [] }
  ...
  it('prefers an owner-routed request over the ambient gateway socket', async () => {
    const gatewayPayload = {
      model: 'chrome-model',
      provider: 'wundercorp',
      providers: [{ models: ['chrome-model'], name: 'WunderCorp', slug: 'wundercorp' }]
    }
  ...
  it('keeps an empty owner-routed catalog instead of replacing it from ambient REST', async () => {
    const ownerPayload = { model: 'berry-local', provider: 'loki-local', providers: [] }
...
describe('manualPickRemoved', () => {
  const providers = [
    ...
    { name: 'WunderCorp', slug: 'wundercorp', models: [] } // present but unconfigured / re-auth
  ]
  ...
  it('never clobbers when the provider has an empty model list (re-auth)', () => {
    expect(manualPickRemoved(providers, 'wundercorp', 'loki-4')).toBe(false)
  })
