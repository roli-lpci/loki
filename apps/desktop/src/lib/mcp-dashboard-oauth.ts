...
const UPDATE_BACKEND = 'Update the Loki backend to support Desktop MCP OAuth callbacks.'
...
export async function completeMcpDesktopOAuth({
  ...
  const bridge = window.lokiDesktop.mcpOauth

  // A legacy null connection can resolve to a remote registry primary.
  if (!bridge && scope.connectionId !== 'local') {
    throw new Error('Update Loki Desktop to support MCP OAuth callbacks.')
  }
  ...
  try {
    ...
    await window.lokiDesktop.openExternal(authUrl)
    let pollFailures = 0
