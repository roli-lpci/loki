import { lokiApi } from '@/api/client'
import type {
  LokiConnection,
  LokiReadDirResult,
  LokiReadFileTextResult,
  LokiSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: LokiConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: LokiConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<LokiReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<LokiReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<LokiReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<LokiReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.lokiDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: LokiSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
