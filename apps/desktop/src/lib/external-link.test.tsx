...
function installDesktopBridge(partial: Partial<Window['lokiDesktop']> = {}) {
...
function installTitleBridge(title: string) {
  ...
  installDesktopBridge({ fetchLinkTitle: bridge as unknown as Window['lokiDesktop']['fetchLinkTitle'] })
...
describe('external link helpers', () => {
  ...
  it('opens a web link in the in-app browser', async () => {
    ...
    installDesktopBridge({ openExternal: openExternal as unknown as Window['lokiDesktop']['openExternal'] })
  ...
  it('prefixes a pretty link to a known host with its brand glyph', () => {
    ...
    const url = 'https://github.com/wundercorp/loki/pull/123'
