...
beforeEach(() => {
  ...
  // stub that invokes synchrowundercorply recurses until the stack gives out. The
  ...
  document.querySelector('loki-watch')?.remove()
  delete (window as unknown as Record<string, unknown>).__lokiWatch
})

/** The overlay lives in a closed shadow root, so a test can only see the host. */
const host = () => document.querySelector('loki-watch')
...
const drawn = () =>
  (window as unknown as { __lokiWatch: { parts: Record<string, HTMLElement> & { shadow: ShadowRoot } } })
    .__lokiWatch.parts
...
describe('watchInPage', () => {
  it('mounts one host on documentElement and reuses it across stages', () => {
    ...
    expect(document.querySelectorAll('loki-watch')).toHaveLength(1)
  ...
  it('rebuilds its chrome when the injected source changes underneath it', () => {
    ...
    const stamp = (value: number) => {
      ;(window as unknown as Record<string, unknown>).__lokiWatchTag = value
    }
