...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __lokiWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__lokiWatch || (win.__lokiWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('loki-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('loki-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __lokiWatchTag?: number }).__lokiWatchTag
