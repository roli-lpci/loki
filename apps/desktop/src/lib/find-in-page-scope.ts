...
//
// `applying` is the re-entrancy guard: it's set for the whole synchrowundercorp
...
export function performScopedFind(root: Element, query: string, options: ScopedFindOptions): ScopedFindResult {
  ...
  // typed query fails on the first match whose casing differs — 'Loki'
  // for 'loki', sentence-initial capitals, ALL-CAPS. Without the
