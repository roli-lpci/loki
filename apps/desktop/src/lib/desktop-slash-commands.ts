...
/** Why a known Loki command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Loki profile', surface: action('profile') },
  {
    name: '/skin',
...
function isKnownLokiSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Loki' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownLokiSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Loki and were never asked for. It is
