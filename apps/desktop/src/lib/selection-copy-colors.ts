...
// Loki is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.lokiMode
 ...
 * the only carriers of Loki' palette — are dropped so the paste target's
