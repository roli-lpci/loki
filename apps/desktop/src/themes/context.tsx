...
import { BUILTIN_THEME_LIST, DEFAULT_SKIN_NAME, DEFAULT_TYPOGRAPHY, wundercorpTheme } from './presets'
...
const SKIN_KEY = 'loki-desktop-theme-v2'
const MODE_KEY = 'loki-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'loki-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'loki-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'loki-desktop-active-profile-v1'
...
const RETIRED_SKINS = new Set(['wundercorp-light', 'default', 'gold'])
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = resolveTheme(skinName) ?? wundercorpTheme
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  const typo = { ...DEFAULT_TYPOGRAPHY, ...wundercorpTheme.typography, ...theme.typography }
  ...
  root.dataset.lokiTheme = skinName
  root.dataset.lokiMode = rendered
  ...
  window.lokiDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('loki-boot-background', chromeBg)
    window.localStorage.setItem('loki-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.lokiThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.lokiDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Loki authoring/activating a skin from a
