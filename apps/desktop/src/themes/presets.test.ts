...
// The pre-GitHub WunderCorp palette stays available as wundercorp-alt; the default name
// still means GitHub chrome + brand blue.
describe('wundercorp-alt is the retired WunderCorp, not the default', () => {
  it('is registered under its own name and leaves wundercorp as the default', () => {
    expect(DEFAULT_SKIN_NAME).toBe('wundercorp')
    expect(BUILTIN_THEMES['wundercorp-alt']).toBe(wundercorpAltTheme)
    expect(BUILTIN_THEMES.wundercorp).not.toBe(wundercorpAltTheme)
    expect(wundercorpAltTheme.darkColors?.background).toBe('#0D2F86')
    expect(BUILTIN_THEMES.wundercorp.darkColors?.background).not.toBe(wundercorpAltTheme.darkColors?.background)
  })
})
