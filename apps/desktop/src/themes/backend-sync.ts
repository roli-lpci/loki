/**
 * Live skin sync from the Loki backend.
 *
 * The backend resolves the active skin (built-in or `$LOKI_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Loki
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { LokiSkin } from '@wundercorp/loki-shared/skin'
...
// resolve a persisted skin pick synchrowundercorply, like a built-in or a user
...
const BACKEND_THEMES_KEY = 'loki-desktop-backend-themes-v1'
 ...
 */
export function ingestBackendSkin(skin: LokiSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  // (wundercorp), so we never register a converted theme under `default`. It is still a
  // valid apply TARGET though: a runtime switch back to `default` must repaint the
  // desktop to its own default (setTheme normalizes `default` → wundercorp). So we only
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as LokiSkin)
