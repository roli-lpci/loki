...
import { getLokiConfigRecord, type LokiConfigRecord, saveLokiConfig } from '@/loki'
...
export interface I18nConfigClient {
  getConfig: () => Promise<LokiConfigRecord>
  saveConfig: (config: LokiConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.lokiDesktop?.api) {
      return Promise.resolve({})
    }

    return getLokiConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.lokiDesktop?.api) {
...
export function getConfigDisplayLanguage(config: LokiConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: LokiConfigRecord, locale: Locale): LokiConfigRecord {
