...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `LokiPlugin` and
...
`$LOKI_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`loki-desktop-plugins` skill.
