 ...
 *    desktop shell fires only while the user is AWAY from Loki. This is the
...
import { host, type PluginOs, type PluginRestOptions, type PluginTranslate } from '@wundercorp/loki-plugin-sdk'
...
function notifyOne(kind: string, spec: { titleKey: string; toast: ToastKind }, ev: CompletionEvent): void {
  ...
  // is away from Loki (the toast above covers the foreground case). Isolated:
