/**
 * Routines: the Loki cron jobs scoped to the bot you're chatting with — the
...
// loki -p <bot> chat delegation wrapper so the run reaches that bot's
...
export function routinePrompt(
  ...
  return (
    ...
    `loki -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Loki-native schedule string; Advanced exposes it raw.
...
/** Compose the Loki schedule string from picker state. */
