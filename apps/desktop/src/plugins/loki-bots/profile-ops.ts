...
import { forgetSessionUnread, host, queryClient } from '@wundercorp/loki-plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['loki-bots']
...
/** Permanently delete a bot's Loki profile, then remove plugin-local state
 ...
 * directory (loki-agent#52279). That is the "can't delete a bot" error. */
