...
import { host, LruCache } from '@wundercorp/loki-plugin-sdk'
...
// to (local, remote URL, SSH, Loki Cloud, docker) must be able to find
...
// loki_cli/config_defaults.py and tools/bot_relay.py and fails if the
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['loki-bots']?.title || profile?.display_name || ''),
