...
describe('routing', () => {
  ...
  it('resolves @loki to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@loki take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @loki', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @loki,/)
    ...
    expect(peer).toMatch(/group chat with @loki/)
  })
