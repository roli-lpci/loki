...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "Loki". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `loki profile rename
  ...
  // never generic "Loki" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Loki (the agent it is)
