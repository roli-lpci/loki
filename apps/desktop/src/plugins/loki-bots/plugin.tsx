/**
 * Loki Bot Mode — a "one chat per agent" roster for the Loki desktop.
 *
 * Left pane "Bots": one row per Loki profile (a bot = an agent profile) with
 ...
 *
 * Right tile "Routines": scheduled tasks (Loki cron jobs) scoped to the
 ...
 * bot-initiated sends use `loki -p <bot> chat --in ~ -c "Bot Chat"`.
 */

import { CHAT_EMPTY_AREA, COMPOSER_AREAS, host, PALETTE_AREA, translateNow } from '@wundercorp/loki-plugin-sdk'
import type { ChatEmptyProps, PluginContext } from '@wundercorp/loki-plugin-sdk'
...
export default {
  ...
  register(ctx: PluginContext) {
    ...
    // synchrowundercorply per keystroke. Multi-source rosters contribute their
    // precomputed @name-device handles via botHandle. The active profile is
    // excluded (a bot doesn't @ itself); 'default' surfaces as @loki.
    ctx.register({
            ...
            try {
              const registry =
                typeof window !== 'undefined'
                  ? await Promise.resolve(window.lokiDesktop?.connections?.list?.()).catch(() => null)
                  : null
    ...
    try {
      if (typeof window !== 'undefined') {
        unbindConnectionsChanged =
          window.lokiDesktop?.connections?.onChanged?.(payload => {
    ...
    // A bot's chat before it has spoken: core's splash is Loki' wordmark and
      ...
      data: {
        handler: async (draft: ComposerDraftPayload): Promise<ComposerDraftPayload> => {
          ...
          const lines = mentionedBots.map(bot => {
            ...
            const title = String(
              botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['loki-bots']?.title || bot.title || ''
            ).trim()

            // message_agent only resolves canonical identities: the relay
            // matches a roster row's handle/profile (± @connection-id), the
            // local path a bare profile name or 'loki'. botHandle() prefers
