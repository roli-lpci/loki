...
import { atom, host } from '@wundercorp/loki-plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'loki-bots-groups'
...
/** The v3 envelope stored under the default profile's `loki-bots-groups`
 ...
 *  clients (Loki Go, headless backends) see rooms regardless of which
...
export function scheduleGroupChatServerSync(
  ...
  // Queue on the ACTIVE gateway synchrowundercorply (tests and older hosts have no
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Loki"
 *  (community report, Aug 21 2026: renamed default still read "Loki is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Loki (matching displayName and the
 *  @loki handle) so the main agent never loses its name in rooms. */
export function groupSpeakerLabel(name?: null | string) {
  ...
  // Core profile display_name (`loki profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'Loki' : trimmed
}
