 ...
 * the raw Cloud hostname, or as generic "Loki" when Cloud was the only
 * source.
 *
...
vi.mock('@wundercorp/loki-plugin-sdk', () => ({ host: hostMock }))
...
describe('alias identity survives the hosted handoff (#89131)', () => {
  ...
  it('renders the sole Cloud-only default as the alias, not "Loki"', () => {
  ...
  it('never leaks the alias onto a same-named default on another connection', () => {
    ...
    // Local default while the ACTIVE gateway is local: untouched "Loki".
    expect(displayName({ name: 'default' }, null)).toBe('Loki')
  })
