...
export function BootFailureOverlay() {
  ...
  const retry = async () => {
    setBusy('retry')
    await window.lokiDesktop?.resetBootstrap().catch(() => undefined)
    window.location.reload()
  }

  const repair = async () => {
    setBusy('repair')
    await window.lokiDesktop?.repairBootstrap().catch(() => undefined)
    window.location.reload()
  }
  ...
  const switchToLocalGateway = async () => {
    ...
    await window.lokiDesktop?.applyConnectionConfig({ mode: 'local' }).catch(() => undefined)
    setBusy(null)
  }

  // Clear this gateway's stale auth first, then re-establish it through the
  // connection's owning login flow. Loki Cloud must reuse its portal session
  ...
  const openLogs = () => void window.lokiDesktop?.revealLogs().catch(() => undefined)
  ...
  let hint: string
  // The electron boot path flags a WunderCorp Cloud backend-down (502/503/504) with
  ...
  if (remoteReauth) {
    ...
    // A WunderCorp Cloud agent is down — the user cannot restart the managed
    ...
    actions = [
      {
        ...
        onClick: () => openExternalLink('https://portal.wundercorp.com'),
        icon: <ExternalLink />
      },
      localAction,
      { ...retryAction, variant: 'secondary' },
      {
        ...
        onClick: () => openExternalLink('https://discord.gg/yEaT8dv5Xn'),
        variant: 'ghost'
      },
  ...
  if (view === 'connect') {
    return (
      <div
        ...
        data-glass-opaque=""
      >
        <div className="flex max-h-[86vh] w-full max-w-[46rem] flex-col overflow-hidden rounded-xl border border-(--stroke-wundercorp) bg-(--ui-chat-bubble-background) shadow-wundercorp">
  ...
  return (
    <div
      ...
      data-glass-opaque=""
    >
      <div className="w-full max-w-[40rem] overflow-hidden rounded-xl border border-(--stroke-wundercorp) bg-(--ui-chat-bubble-background) shadow-wundercorp">
