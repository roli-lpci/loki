...
//
// 20, down from 60: the first-paint commit is synchrowundercorp and uninterruptible,
...
export function subscribeToThreadForeground(shouldReanchor: () => boolean, onReanchor: () => void): () => void {
  ...
  const onForeground = () => {
    ...
    // Browser callbacks are asynchrowundercorp; the guard also keeps synchrowundercorp
...
// synchrowundercorp frame, a 100-800ms stall once a second on a streaming long
...
const ThreadMessageListInner: FC<ThreadMessageListProps> = ({
  ...
  // synchrowundercorp commit that freezes input right after the switch. Route
  ...
  // the point is a small synchrowundercorp commit; forcing 8 turns into it would put
  ...
  // the OLD session's closure, synchrowundercorply in the commit that swaps
  // transcripts.
  useLayoutEffect(() => {
    const el = scrollRef.current
    ...
    const settle = () => {
      ...
      // Most session switches are synchrowundercorp and stabilize within 2 frames;
