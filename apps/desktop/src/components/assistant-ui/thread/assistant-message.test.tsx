    ...
    metadata: {
      ...
      custom: {
        errorSurface: {
          ...
          layer: 'auth',
          provider: 'wundercorp',
          providerLabel: 'WunderCorp Portal',
          retryable: false
        }
      }
    }
...
describe('expired OAuth grant recovery', () => {
  it('explains the expiry and re-runs that provider sign-in in one click', async () => {
    ...
    expect(await screen.findByText(/WunderCorp Portal sign-in has expired/)).toBeTruthy()
    // Signing in changes the outcome, so Retry stays as the follow-up click.
    expect(screen.getByRole('button', { name: 'Retry' })).toBeTruthy()

    screen.getByRole('button', { name: 'Sign in to WunderCorp Portal again' }).click()
