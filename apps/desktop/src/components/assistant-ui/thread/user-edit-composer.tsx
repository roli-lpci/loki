...
import { lokiDirectiveFormatter } from '@/components/assistant-ui/directive-text'
import {
...
export const UserEditComposer: FC<UserEditComposerProps> = ({ cwd, gateway, sessionId }) => {
  ...
  const replaceTriggerWithChip = useCallback(
    (item: Unstable_TriggerItem) => {
      ...
      rememberInitialDraft()
      recordUndoPoint()
      const serialized = lokiDirectiveFormatter.serialize(item)
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLElement>) => {
    if (!dragHasAttachments(event.dataTransfer, LOKI_PATHS_MIME)) {
      return
    }
  ...
  const submitEdit = (editor: HTMLDivElement) => {
    ...
    try {
      ...
      // be synchrowundercorp or async, and whose promise we don't have access to).
      scheduleTimeout(() => {
        setSubmitting(false)
      }, 200)
    } catch {
