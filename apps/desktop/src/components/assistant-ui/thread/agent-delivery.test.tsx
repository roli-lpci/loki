...
describe('delivery command detection', () => {
  it('matches the canonical delivery command', () => {
    const cmd = 'loki -p turqoise chat --in ~ -c "Bot Chat" -Q -q "Message from 🤖 Loki (@loki): hi there"'

    expect(deliveryTargetFromCommand(cmd)).toBe('turqoise')
  })

  it('matches with a cd prefix and timeout wrapper', () => {
    const cmd = 'cd ~ && timeout 240 loki -p mr-tester chat --in "~" -Q -q "Message from 🤖 Loki: hello"'
  ...
  it('ignores ordinary terminal commands', () => {
    ...
    expect(deliveryTargetFromCommand('loki -p turqoise chat -q "plain question"')).toBeNull()
    expect(deliveryTargetFromCommand('loki sessions list')).toBeNull()
  })
})

describe('reply extraction', () => {
  it('strips session_id bookkeeping and keeps the reply', () => {
    const output = 'session_id: 20260813_220347_f69ac6\nHi Loki! Good to hear from you.'

    expect(replyTextFromResult({ output })).toBe('Hi Loki! Good to hear from you.')
  })
