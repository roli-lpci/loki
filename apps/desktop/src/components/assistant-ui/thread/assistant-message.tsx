...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's LOKI_HOME/logs. On a
  ...
  // An expired/revoked OAuth grant (HTTP 401 on wundercorp / openai-codex / ...):
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.lokiDesktop?.logsRoot?.()
      ...
      const result = await window.lokiDesktop?.openDir?.(root)
