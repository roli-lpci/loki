...
const LOKI_REF_TYPES = WIRE_REFERENCE_KINDS
type LokiRefType = (typeof LOKI_REF_TYPES)[number]
...
const LOKI_DIRECTIVE_RE = referenceRe()
...
export const lokiDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(LOKI_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...lokiDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Loki directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.lokiDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.lokiDesktop?.readFileDataUrl(id)
