 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.loki.send(prompt)` (or declarative
 * `data-loki-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'loki-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'loki-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `loki.send(prompt)`.
 ...
 *  wires `data-loki-send` so declarative HTML works with zero script:
 *  `<button data-loki-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.loki={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-loki-send]"):null;' +
    'if(el)send(el.getAttribute("data-loki-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `loki serve` — renders live.)
