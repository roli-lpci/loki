...
const SCRIPT: Record<string, { id: string; src: string }> = {
  instagram: { id: 'loki-ig-embed', src: 'https://www.instagram.com/embed.js' },
  tiktok: { id: 'loki-tt-embed', src: 'https://www.tiktok.com/embed.js' },
  twitter: { id: 'loki-tw-embed', src: 'https://platform.twitter.com/widgets.js' }
}
...
export default function SocialEmbedRenderer({ descriptor }: { descriptor: EmbedDescriptor }) {
  ...
  useEffect(() => {
    ...
    void loadScript(descriptor.provider).then(() => {
      // The script renders asynchrowundercorply; nudge a few times so the embed
