...
import type { ModelOptionProvider, OAuthProvider } from '@/types/loki'
...
const API_KEY_OPTIONS: ApiKeyOption[] = [
  ...
  },
  {
    ...
    docsUrl: 'https://github.com/wundercorp/loki#bring-your-own-endpoint',
    placeholder: 'http://127.0.0.1:8000/v1'
  }
]

// Build the FULL API-key provider catalog from the backend model options so the
// onboarding / Providers key form lists every `api_key` provider `loki model`
...
function useApiKeyCatalog(): ApiKeyOption[] {
  ...
  useEffect(() => {
    ...
    // Promise.resolve().then so a synchrowundercorp throw (e.g. no desktop bridge in
    ...
    >
      <div
        className={cn(
          ...
          bare
            ? ''
            : 'overflow-hidden rounded-xl border border-(--stroke-wundercorp) bg-(--ui-chat-bubble-background) shadow-wundercorp',
