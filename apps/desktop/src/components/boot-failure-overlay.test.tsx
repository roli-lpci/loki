...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Loki gateway',
    fakeMode: false,
    message: 'boot failed',
...
describe('BootFailureOverlay', () => {
  ...
  it('recovers a cloud connection through the portal cascade instead of native OAuth', async () => {
    const gatewayUrl = 'https://agent-1.agents.wundercorp.com'
    const logout = vi.fn().mockResolvedValue({ ok: true, connected: false })
    const nativeLogin = vi.fn().mockResolvedValue({ ok: true, connected: false })
    const cloudStatus = vi.fn().mockResolvedValue({ portalBaseUrl: 'https://portal.wundercorp.com', signedIn: false })

    const cloudLogin = vi.fn().mockResolvedValue({
      ok: true,
      portalBaseUrl: 'https://portal.wundercorp.com',
      signedIn: true
    })
    ...
    const restore = stubDesktop(
      ...
      },
      {
        ...
        oauthLogoutConnectionConfig: logout,
        probeConnectionConfig: vi.fn().mockResolvedValue({ providers: [{ id: 'wundercorp', type: 'oauth' }] })
      }
    )
  ...
  it('shows the WunderCorp Cloud down recovery when the backend flags isCloudBackendDown', async () => {
    const restore = stubDesktop(remoteToken)
    $desktopBoot.set({
      error: 'WunderCorp Cloud agent ares-3009.agents.wundercorp.com is down (HTTP 503: server-side fault).',
      fakeMode: false,
    ...
    try {
      ...
      // remote-failure copy.
      expect(await screen.findByText(/WunderCorp Cloud agent is down/i)).toBeTruthy()
      ...
      expect(screen.getByText(/ares-3009\.agents\.wundercorp\.com/i)).toBeTruthy()
    } finally {
      restore()
    }
  })
