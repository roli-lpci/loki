...
function sessionsBotsTree() {
  ...
  registerPane('loki-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'loki-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('loki-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('loki-bots:pane')
    expect($hiddenStripTabs.get()).toContain('loki-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('loki-bots:pane')

    expect(setStripTabHidden('loki-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('loki-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('loki-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('loki-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('loki.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('loki-bots:pane')
    ...
    revealTreePane('loki-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('loki-bots:pane')
    expect(window.localStorage.getItem('loki.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'loki-bots:pane', title: 'loki-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'loki-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
