import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@wundercorp/loki-shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as LokiGateway | null
  const gatewayRef = useRef<LokiGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.lokiDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<LokiGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as LokiGateway | null
      }),
    []
  )
