...
vi.mock('@/loki', async importOriginal => {
  const actual = await importOriginal<typeof LokiModule>()

  class FakeLokiGateway {
  ...
  return { ...actual, LokiGateway: FakeLokiGateway }
})

import type * as LokiModule from '@/loki'
import type { LokiGateway } from '@/loki'
import {
  $gateway,
...
const fakeGateway = { connectionState: 'open' } as unknown as LokiGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as LokiGateway, 'default')
  $gateway.set(primary as unknown as LokiGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.lokiDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as LokiGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { lokiDesktop: unknown }).lokiDesktop = { getConnection }
    $gateway.set(dropped)
