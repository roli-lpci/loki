...
import type { RpcEvent } from '@/types/loki'
...
export function primaryRuntimeConnectionId(connection: Pick<LokiConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.lokiDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: LokiGateway | null) => void
  refreshLokiConfig: (force?: boolean, shouldPublish?: () => boolean) => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: LokiConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Loki...". Once the
    ...
    const attemptReconnect = async (manual?: { profile: string; activationEpoch: number }) => {
      ...
      try {
        ...
        // "Starting Loki…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Loki backend'
        )
        ...
        // as an opaque "Could not connect to Loki gateway". resolveGatewayWsUrl
        ...
        if (!manual || (isActivePrimary() && gatewayActivationEpoch() === manual.activationEpoch)) {
          reconcileBusyStatesOnReconnect()
          await callbacksRef.current.refreshLokiConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        // Barrier up + machine-context reset + session wipe, in one synchrowundercorp
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshLokiConfig(false, ownsSwitch).catch(() => undefined),
        ...
        // newer switch started synchrowundercorply by error recovery/notification UI.
        if (switchToken !== null) {
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startLoki re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new LokiGateway()
    ...
    const offGatewayReconnect = registerGatewayReconnect(async () => {
      ...
      if (!isActivePrimary()) {
        ...
        if (!(await ensureActiveGatewayOpen())) {
          throw new Error('Loki gateway is not connected')
        }
    ...
    async function boot() {
      ...
      try {
        ...
        // reaches the recovery affordance instead of hanging "Starting Loki…".
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshLokiConfig(),
          ...
          // retry it), not brick boot behind the "Loki couldn't start"
