...
const LOKI_PATHS_MIME = 'application/x-loki-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(LOKI_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(LOKI_PATHS_MIME)
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.lokiDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (loki --tui, vim) grab the mouse, so a plain drag
    ...
    // Capture the buffer on a leading-edge throttle and persist synchrowundercorply via
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.lokiDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[loki-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.lokiDesktop?.terminal?.write(sessionId, `${command}\r`)
