...
import type { LokiReadDirEntry, LokiReadDirResult } from '@/global'
...
export type ProjectTreeEntry = LokiReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: LokiReadDirEntry) {
...
async function filterIgnored(entries: LokiReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<LokiReadDirResult> {
  if (!window.lokiDesktop) {
