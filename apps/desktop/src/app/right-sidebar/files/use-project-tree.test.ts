...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { lokiDesktop: { readDir: typeof readDir } }).lokiDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): LokiReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { lokiDesktop: unknown }).lokiDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: LokiReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<LokiReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: LokiReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: LokiReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: LokiReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<LokiReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.lokiDesktop is missing', async () => {
    delete (window as unknown as { lokiDesktop?: unknown }).lokiDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: LokiReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: LokiReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: LokiReadDirResult) => void) | undefined
