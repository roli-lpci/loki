...
export function singleFlightSessionResume<T>(storedSessionId: string, run: () => Promise<T>): Promise<T> {
  ...
  // Promise.resolve().then(run) tolerates run() being synchrowundercorp, returning a
  // bare value, or throwing synchrowundercorply (test doubles and legacy callers do
