...
describe('renderRpcResult', () => {
  ...
  describe('session.save', () => {
    it('echoes the saved file path', () => {
      expect(renderRpcResult({ file: '/home/user/.loki/sessions/saved/x.json' }, 'save')).toBe(
        'Saved transcript to /home/user/.loki/sessions/saved/x.json'
      )
    })
  })
  ...
  describe('session.status', () => {
    it('passes through the multi-line plain-text output verbatim', () => {
      const output = 'Loki TUI Status\n\nSession ID: s-1\nModel: wundercorp-loki-3 (unknown)'
  ...
  describe('session.usage', () => {
    ...
    it('appends credits_lines when present', () => {
      const body = renderRpcResult(
        {
          ...
          output: 20,
          total: 30,
          credits_lines: ['WunderCorp credits: 8,420 remaining', 'Resets: 2026-08-01']
        },
        'usage'
      )
      ...
      expect(body.split('\n')).toEqual([
        ...
        'WunderCorp credits: 8,420 remaining',
        'Resets: 2026-08-01'
      ])
    })
  })
