...
import type { SessionCreateResponse, SessionMessage, SessionResumeResponse, UsageStats } from '@/types/loki'
...
export function useSessionActions({
  ...
  const startFreshSessionDraft = useCallback(
    (options: boolean | FreshSessionDraftOptions = false) => {
      ...
      // Clear the durable route intent synchrowundercorply, before React Router
  ...
  const createBackendSessionForSend = useCallback(
    async (preview: string | null = null): Promise<string | null> => {
      ...
      try {
        ...
        // genuine user switch retargets selection AND route synchrowundercorply
  ...
  const resumeSession = useCallback(
    async (storedSessionId: string, replaceRoute = false, capturedOwner?: SessionProfileRoute) => {
      ...
      // also what use-route-resume's self-heal assumes ("set synchrowundercorply at
      // resume entry").
      setFreshDraftReady(false)
      clearNotifications()
      resetViewSync()
  ...
  const removeSession = useCallback(
    async (storedSessionId: string) => {
      ...
      // Selection and runtime refs are updated synchrowundercorply at routing
