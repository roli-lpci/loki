...
export function useRouteResume({
  ...
  useEffect(() => {
    ...
    if (routedSessionId) {
      ...
      // stuck empty forever. selectedStoredSessionIdRef is set synchrowundercorply at
      ...
      if ((gatewayBecameOpen || explicitlyRequested || !alreadyActive) && shouldResume && !creatingSessionRef.current) {
        ...
        // resumeSession makes synchrowundercorply at entry.
  ...
  // sets it synchrowundercorply at entry) and the pathname/gateway are unchanged, so
