...
describe('useSessionStateCache — per-session turn timer', () => {
  beforeEach(() => {
    ...
    // want it synchrowundercorp so the global mirror is observable immediately. The
    ...
    // assignment doesn't overwrite the null the synchrowundercorp callback just set —
    ...
    // test-only artifact of firing synchrowundercorply).
...
function LayoutProbeHarness({
  ...
  // useLayoutEffect fires synchrowundercorply right after the DOM commit, BEFORE
  ...
  // that reads the refs in this window — including a synchrowundercorp DOM event
