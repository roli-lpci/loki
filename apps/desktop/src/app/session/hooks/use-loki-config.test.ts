...
import { getLokiConfig } from '@/loki'
...
import { useLokiConfig } from './use-loki-config'

vi.mock('@/loki', () => ({
  getLokiConfig: vi.fn(),
  getLokiConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'loki.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getLokiConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getLokiConfig>>)

describe('useLokiConfig refreshLokiConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Loki' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useLokiConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshLokiConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useLokiConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getLokiConfig>>>()
    vi.mocked(getLokiConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useLokiConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshLokiConfig(true)
    })
    expect(getLokiConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getLokiConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getLokiConfig>>>()
    vi.mocked(getLokiConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useLokiConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshLokiConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getLokiConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getLokiConfig>>>()
    vi.mocked(getLokiConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshLokiConfig(true)
      refreshC = result.current.refreshLokiConfig(true)
    })
