import { registryBackendScopeKey } from '@wundercorp/loki-shared'
...
describe('connection-qualified session deletion', () => {
  ...
  it('tears down the selected session from synchrowundercorp refs when render state is stale', async () => {
...
describe('resumeSession failure recovery', () => {
  ...
  it('resumes via the gateway default (deferred build) — not lazy, no eager opt-out', async () => {
    ...
    // pre-warms in the background). The client must NOT force the synchrowundercorp
...
describe('resumeSession warm-cache mapping integrity', () => {
  ...
  it('pins an untagged row to the active registry connection instead of the same-named local profile', async () => {
    setConnection({ connectionId: 'loki01', mode: 'remote' } as never)
