...
import { getLokiConfig, getLokiConfigDefaults } from '@/loki'
...
export function useLokiConfig({ activeSessionIdRef }: LokiConfigOptions) {
  ...
  const refreshLokiConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getLokiConfig(), getLokiConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Loki' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshLokiConfig, sttEnabled, voiceMaxRecordingSeconds }
}
