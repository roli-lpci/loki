...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshLokiConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshLokiConfig — two REST calls (config + defaults) per event, per
