 ...
 * billing-specific toast — never the generic "Loki error" — with a smart CTA
 * (WunderCorp → in-app Settings → Billing, other providers → their billing page).
 */
function surfaceBillingBlock(sessionId: string, raw: unknown): void {
  ...
  notify({
    ...
    kind: 'warning',
    icon: 'credit-card',
    title: block.is_wundercorp
      ? translateNow('billingBlock.titleWunderCorp')
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'Loki reported an error',
