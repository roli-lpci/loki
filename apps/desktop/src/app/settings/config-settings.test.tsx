...
vi.mock('@/loki', () => ({
  ...
  saveLokiConfig: (config: unknown, profile?: string) => saveLokiConfig(config, profile),
...
beforeEach(() => {
  ...
  getLokiConfigSchema.mockResolvedValue({ fields: {} })
...
describe('ConfigSettings autosave', () => {
  it('sends a later revert instead of diffing it away against the stale page-load baseline', async () => {
    getLokiConfigRecord.mockResolvedValue({ checkpoints: { enabled: false }, other: 'untouched' })
    ...
    try {
      ...
      await vi.waitFor(() => expect(saveLokiConfig).toHaveBeenCalledTimes(1))
      expect(saveLokiConfig.mock.calls[0][0]).toEqual({ checkpoints: { enabled: true } })

      // Revert: flip it back to its original value and let autosave fire again.
      toggle.click()
      await vi.advanceTimersByTimeAsync(700)

      await vi.waitFor(() => expect(saveLokiConfig).toHaveBeenCalledTimes(2))
      ...
      expect(saveLokiConfig.mock.calls[1][0]).toEqual({ checkpoints: { enabled: false } })
    } finally {
      vi.useRealTimers()
    }
  })
