...
import { getLokiConfigDefaults, getLokiConfigRecord, saveLokiConfig } from '@/loki'
...
export function SettingsView({ onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getLokiConfigRecord()
  ...
  const resetConfig = async () => {
    ...
    try {
      await saveLokiConfig(await getLokiConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
