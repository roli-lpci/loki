...
const getLokiConfigRecord = vi.fn()
const saveLokiConfig = vi.fn()
...
vi.mock('@/loki', () => ({
  ...
  getLokiConfigRecord: () => getLokiConfigRecord(),
  saveLokiConfig: (config: unknown) => saveLokiConfig(config),
...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'wundercorp', model: 'loki-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      {
        name: 'WunderCorp',
        slug: 'wundercorp',
        models: ['loki-4', 'loki-4-mini'],
        authenticated: true,
        capabilities: { 'loki-4': { reasoning: true, fast: true } }
      }
    ]
  })
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'wundercorp', model: 'loki-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  getMoaModels.mockResolvedValue(null)
  setModelAssignment.mockResolvedValue({ ok: true, provider: 'wundercorp', model: 'loki-4', gateway_tools: [] })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'wundercorp', model: 'loki-4', free_tier: null })
  setEnvVar.mockResolvedValue({ ok: true })
  getLokiConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveLokiConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  it('loads the current main model and lists configured providers only', async () => {
    ...
    // "WunderCorp" shows in both the trigger and the open list.
    expect((await screen.findAllByText('WunderCorp')).length).toBeGreaterThan(0)
  ...
  it('replaces the selected provider and model when the active profile changes', async () => {
    getGlobalModelInfo
      ...
      .mockResolvedValueOnce({ provider: 'wundercorp', model: 'loki-4' })
    ...
    await waitFor(() => expect(screen.getAllByRole('combobox')[0].textContent).toContain('WunderCorp'))
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getLokiConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveLokiConfig).toHaveBeenCalledWith(
      ...
      providers: [
        {
          ...
          models: ['loki-4'],
          authenticated: true,
          capabilities: { 'loki-4': { reasoning: false, fast: false } }
        }
      ]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'wundercorp', model: 'loki-4' }]
    })
    ...
    expect(screen.getByText('wundercorp')).toBeTruthy()
  })
...
describe('ModelSettings MoA preset editor', () => {
  const moaConfig = () => ({
    ...
    presets: {
      default: {
        reference_models: [
          { provider: 'wundercorp', model: 'loki-4' },
  ...
  it('does not clear the model or save when the same provider is re-selected', async () => {
    ...
    try {
      ...
      fireEvent.click(slotSelects().ref1Provider)
      fireEvent.click(await screen.findByRole('option', { name: 'WunderCorp' }))
      await vi.advanceTimersByTimeAsync(700)

      // Radix treats re-picking the current value as a no-op (no
      // onValueChange), so nothing changes: no save, model still shown.
      expect(saveMoaModels).not.toHaveBeenCalled()
      expect(screen.getByText('wundercorp · loki-4')).toBeTruthy()
    } finally {
      vi.useRealTimers()
    }
  })
        ...
        expect.objectContaining({
          presets: expect.objectContaining({
            default: expect.objectContaining({
              reference_models: [
                expect.objectContaining({ provider: 'wundercorp', model: 'loki-4', enabled: false }),
...
describe('ModelSettings code-skew 503', () => {
  const skewError = new Error(
    'Error invoking remote method \'loki:api\': Error: 503: {"detail":"Restart required: This process is running code from 08b4875f4a but the checkout on disk is now 48d2528066. The model picker would risk a stale-module crash — restart the Desktop-owned backend to load the new code (use Restart backend in Loki Desktop, or quit and reopen the app)"}'
  )
  ...
  it('unwraps the stale-backend 503 instead of dumping IPC JSON', async () => {
    ...
    expect(screen.queryByText(/loki:api/)).toBeNull()
  ...
  it('recycles the Desktop-owned backend and reloads the catalog', async () => {
    ...
    ;(window as unknown as { lokiDesktop: { recycleBackend: typeof recycleBackend } }).lokiDesktop = {
      recycleBackend
    }
