...
import { disconnectOAuthProvider, listOAuthProviders } from '@/loki'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/loki'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.lokiDesktop?.terminal)
...
//      in loki_cli/provider_catalog.py) — the SAME provider identity
//      `loki model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Loki can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Loki can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Loki never deletes creds another tool owns behind a silent API call.
