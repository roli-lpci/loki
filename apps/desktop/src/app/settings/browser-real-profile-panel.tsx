...
import { type ProfileScope, saveLokiConfigRecord } from '@/loki'
 ...
 * the default browser's profile (cookies/logins) into a Loki-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config } = useLokiConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        await saveLokiConfigRecord(next, profile)
        notify({
          kind: 'info',
