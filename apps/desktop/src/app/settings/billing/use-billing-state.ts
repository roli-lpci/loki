...
export const FALLBACK_PORTAL_BILLING_URL = 'https://portal.wundercorp.com/billing'
export const FALLBACK_PORTAL_URL = 'https://portal.wundercorp.com'

// The billing endpoint is the authoritative source of truth for balance / cap /
// plan — the inference `x-wundercorp-credits-*` headers are best-effort and can drift
  ...
  if (!billing.logged_in || subscription?.logged_in === false) {
    return {
      notice: {
        ...
        message: 'Run /portal in the TUI or open the WunderCorp portal to connect your account.',
...
function freeTierView(billing: BillingStateResponse): BillingView {
  return {
    notice: {
      ...
      message: 'Sign in with a WunderCorp account to unlock more models and tools.',
      title: "You're on the WunderCorp free tier",
      tone: 'info'
    },
    plan: {
      caption:
        'Runs on wundercorp/welcome with connectors included. Signing in keeps your connectors and adds the tools that need an account and every other model.',
      tierName: 'WunderCorp · free tier'
    },
    planFootnote:
      'The free tier has no balance and nothing to pay. Payment and usage appear when you sign in with a WunderCorp account.',
    status: 'free_tier',
    summary: [
