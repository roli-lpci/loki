 ...
 * agent sources (local runtime + any number of remote gateways / Loki Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.lokiDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.lokiDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `loki update` to every eligible source; per-connection results
