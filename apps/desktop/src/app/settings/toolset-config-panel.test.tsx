...
import type { ToolsetConfig } from '@/types/loki'
...
const getLokiConfigSchema = vi.fn()
...
vi.mock('@/loki', () => ({
  ...
  getLokiConfigSchema: () => getLokiConfigSchema(),
    ...
    providers: [
      {
        ...
        env_vars: [],
        post_setup: null,
        requires_wundercorp_auth: false,
        is_active: false
      },
      {
        name: 'ElevenLabs',
...
beforeEach(() => {
  ...
  getLokiConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getLokiConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveLokiConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveLokiConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
  ...
  describe('readiness pills', () => {
    it('renders the server status instead of assuming keyless rows are Ready', async () => {
      // The false-Ready bug: a logged-out WunderCorp Subscription row and a
      ...
      getToolsetConfig.mockResolvedValue(
        config({
          providers: [
            ...
            {
              name: 'WunderCorp Subscription',
              badge: 'subscription',
              tag: 'Managed OpenAI TTS',
              env_vars: [],
              post_setup: null,
              requires_wundercorp_auth: true,
              is_active: false,
              status: 'needs_auth'
            },
            {
  ...
  describe('managed WunderCorp provider activation', () => {
    const wundercorpBrowserConfig = () =>
      config({
        name: 'browser',
        active_provider: null,
        providers: [
          {
            name: 'WunderCorp Subscription (Browser Use cloud)',
            badge: 'subscription',
    ...
    it('surfaces a sign-in notice when the PUT reports needs_wundercorp_auth', async () => {
      ...
      // reports needs_wundercorp_auth and the panel must surface a sign-in action
      // instead of the misleading "provider selected" success toast.
      const { notify } = await import('@/store/notifications')

      getToolsetConfig.mockResolvedValue(wundercorpBrowserConfig())
      selectToolsetProvider.mockResolvedValue({
        ok: true,
        name: 'browser',
        provider: 'WunderCorp Subscription (Browser Use cloud)',
        needs_wundercorp_auth: true,
        feature: 'browser'
      })

      render(<ToolsetConfigPanel onConfiguredChange={vi.fn()} toolset="browser" />)

      // The single WunderCorp row auto-expands; activate via the explicit button.
      await screen.findByRole('button', { name: /WunderCorp Subscription/ })
      fireEvent.click(await screen.findByRole('button', { name: /Use this backend/ }))

      await waitFor(() =>
        expect(selectToolsetProvider).toHaveBeenCalledWith('browser', 'WunderCorp Subscription (Browser Use cloud)')
      )
      await waitFor(() =>
    ...
    it('drives the existing WunderCorp OAuth device-code flow from the sign-in action and refetches', async () => {
      const { notify } = await import('@/store/notifications')
      ...
      startOAuthLogin.mockResolvedValue({
        ...
        verification_url: 'https://portal.wundercorp.com/device?user_code=WUNDERCORP-1234',
        poll_interval: 5,
        expires_in: 600
      })
      ...
      try {
        ...
        await waitFor(() => expect(startOAuthLogin).toHaveBeenCalledWith('wundercorp'))
        ...
        await waitFor(() => expect(pollOAuthSession).toHaveBeenCalledWith('wundercorp', 'sess-1'), { timeout: 8000 })
