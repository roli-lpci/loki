...
import { getElevenLabsVoices, getLokiConfigSchema, saveLokiConfig } from '@/loki'
...
import type { ConfigFieldSchema, LokiConfigRecord } from '@/types/loki'

import { lokiConfigCacheWriter, useLokiConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<LokiConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useLokiConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => lokiConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['loki-config-schema'] : ['loki-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getLokiConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  // never touched — possibly changed out-of-band by `loki config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<LokiConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await saveLokiConfig(patch, scopeProfile)
  ...
  const applyConfig = (next: LokiConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: LokiConfigRecord) => {
