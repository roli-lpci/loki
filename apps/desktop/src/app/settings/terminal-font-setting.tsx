...
import { setLokiConfigCache, useLokiConfigRecord } from '../hooks/use-config-record'
...
function fontFamilyFromConfig(config: LokiConfigRecord): string {
...
export function TerminalFontSetting() {
  ...
  const { data: loadedConfig } = useLokiConfigRecord()
  ...
  const [staleConfig, setStaleConfig] = useState<LokiConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    // holds — successful saves write it back via setLokiConfigCache, so
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-3">
            <Input
              ...
              className="flex-1"
              disabled={draft === null}
              list="loki-terminal-font-families"
          ...
          <datalist id="loki-terminal-font-families">
