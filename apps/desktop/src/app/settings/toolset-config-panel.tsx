 ...
 * server-computed `status` (keys ∧ WunderCorp entitlement ∧ post-setup install
 ...
 * log inline — the GUI equivalent of the install step `loki tools` runs
...
/**
 * Backend model catalog — the GUI counterpart of the model picker `loki
...
export function ToolsetConfigPanel({ toolset, onConfiguredChange, profile }: ToolsetConfigPanelProps) {
  ...
  // Guard the WunderCorp Portal sign-in poll loop against unmount/state updates.
  ...
  // panel highlighted the first keyless provider (e.g. WunderCorp Portal) even when
  ...
  async function handleSelect(provider: ToolProvider) {
    ...
    try {
      ...
      if (result.needs_wundercorp_auth) {
        // Managed WunderCorp row selected without Portal entitlement: the config
        ...
        // sign-in action). Reuses the existing WunderCorp Portal device-code flow.
        notify({
          kind: 'warning',
          title: copy.wundercorpAuthNeededTitle,
          message: copy.wundercorpAuthNeededMessage(provider.name),
          action: { label: copy.wundercorpAuthSignIn, onClick: () => void signInToWunderCorpPortal() }
        })
  ...
  // Drive the existing WunderCorp Portal OAuth device-code flow (the same session
  ...
  async function signInToWunderCorpPortal() {
    try {
      const start = await startOAuthLogin('wundercorp', profile)

      if (start.flow !== 'device_code') {
        notifyError(new Error(`unexpected flow: ${start.flow}`), copy.wundercorpAuthFailed)
      ...
      if (window.lokiDesktop?.openExternal) {
        try {
          await window.lokiDesktop.openExternal(url)
        } catch {
      ...
      for (let attempt = 0; attempt < 120 && mountedRef.current; attempt += 1) {
        ...
        const polled = await pollOAuthSession('wundercorp', start.session_id, profile)

        if (polled.status === 'approved') {
          notify({ kind: 'success', title: copy.wundercorpAuthDoneTitle, message: copy.wundercorpAuthDoneMessage })
          await refresh()
          onConfiguredChange?.()
        ...
        if (polled.status !== 'pending') {
          notifyError(new Error(polled.error_message || `Sign-in ${polled.status}`), copy.wundercorpAuthFailed)
      ...
      if (mountedRef.current) {
        notifyError(err, copy.wundercorpAuthFailed)
      }
    }
  }
          ...
          <div className="overflow-hidden rounded-xl bg-background/60" key={provider.name}>
            ...
            {isExpanded && (
              <div className="grid gap-2 bg-muted/20 p-3">
                ...
                {provider.requires_wundercorp_auth && (
                  <p className="text-[0.72rem] text-muted-foreground">{copy.wundercorpIncluded}</p>
                )}
