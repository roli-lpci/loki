...
function setWorkspaceTransfer(transferring: boolean): void {
  window.lokiDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.lokiDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.lokiDesktop?.hud?.beginMove?.()
}
 ...
 * (see loki:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.lokiDesktop?.hud?.endMove?.()
      }
