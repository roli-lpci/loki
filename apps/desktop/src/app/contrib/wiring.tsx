...
import { getLatestSessionMessages } from '@/loki'
...
import { useLokiConfigRecord } from '../hooks/use-config-record'
...
import { useLokiConfig } from '../session/hooks/use-loki-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshLokiConfig, sttEnabled, voiceMaxRecordingSeconds } = useLokiConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('loki:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('loki:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshLokiConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshLokiConfig])
  ...
  const configRecord = useLokiConfigRecord()
