...
import { pathFromLokiDeepLink, resolveLokiOpenPath } from '@/lib/loki-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    // "About Loki" — it is the OS-standard affordance for updating THIS app,
    ...
    const unsubscribe = window.lokiDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow('client'))
  ...
  useEffect(() => {
    window.lokiDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.lokiDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.lokiDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // loki://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.lokiDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future lokiDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveLokiOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.lokiDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (loki://index-network/intent/1, loki://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromLokiDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.lokiDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.lokiDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.lokiDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.lokiDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
