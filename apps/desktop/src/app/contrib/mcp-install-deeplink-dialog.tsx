...
import { getLokiConfigRecord, saveMcpServers } from '@/loki'
...
import { setLokiConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `loki://mcp/install` deep links. The payload is
 ...
 * confirming lets Loki spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getLokiConfigRecord())
      ...
      setLokiConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
