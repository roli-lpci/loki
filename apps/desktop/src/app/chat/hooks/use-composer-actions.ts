...
export const LOKI_PATHS_MIME = 'application/x-loki-paths'

/**
 * Eagerly resolve files from a drop event into [File?, path, isDirectory?]
 * triples. Internal Loki sources (e.g. the project tree) ride on a custom
 * MIME and produce path-only entries; OS drops produce File-bearing entries.
 *
 * Must be called synchrowundercorply from inside the drop handler — `DataTransfer`
...
export function extractDroppedFiles(transfer: DataTransfer): DroppedFile[] {
  ...
  const getPath = window.lokiDesktop?.getPathForFile
  ...
  try {
    const internalRaw = transfer.getData(LOKI_PATHS_MIME)
  ...
  // synchrowundercorp way to tell a dropped folder from a file, and it lives only on
...
export function useComposerActions({
  ...
  const attachImageBlob = useCallback(
    async (blob: Blob) => {
      ...
      try {
        ...
        const savedPath = await window.lokiDesktop?.saveImageBuffer(data, blobExtension(blob), name)
  ...
  const pasteClipboardImage = useCallback(
    async ({ silent = false }: { silent?: boolean } = {}) => {
      try {
        const path = await window.lokiDesktop?.saveClipboardImage()
  ...
  const attachDroppedItems = useCallback(
    async (candidates: DroppedFile[]) => {
      ...
      for (const candidate of candidates) {
        ...
        const fallbackPath =
          !knownPath && window.lokiDesktop?.getPathForFile ? window.lokiDesktop.getPathForFile(file) : ''
