...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__lokiTourEngine) {
    ${driverIife}
    w.__lokiTourHolder = {};
    w.__lokiTourCollect = (${collectTourTargets.toString()});
    w.__lokiTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__loki-tour-style')) {
    var style = document.createElement('style');
    style.id = '__loki-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__lokiTourEngine(
    w.driver.js.driver,
    w.__lokiTourHolder,
