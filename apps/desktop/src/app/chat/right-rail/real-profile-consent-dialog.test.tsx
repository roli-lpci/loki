...
vi.mock('@/loki', () => ({
  saveLokiConfigRecord: (config: Record<string, unknown>, profile?: unknown) => mocks.save(config, profile)
}))

const promptCopy = {
  title: 'Stay signed in to your sites',
  body: 'Let Loki browse with a snapshot of your default browser profile.',
...
vi.mock('../../hooks/use-config-record', () => ({
  lokiConfigCacheWriter: () => (config: Record<string, unknown>) => mocks.cache(config),
  useLokiConfigRecord: () => ({ data: mocks.loadedConfig })
}))

describe('RealProfileConsentDialog', () => {
  beforeEach(() => {
    mocks.loadedConfig = { browser: { allow_private_urls: false }, model: { provider: 'wundercorp' } }
