        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.lokiDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const annotateGuest = useCallback((): null | PreviewAnnotateGuest => {
    ...
    return {
      capture: async rect => {
        ...
        const dataUrl = await window.lokiDesktop.capturePreview?.({ rect, viewport, webContentsId })
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Loki to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.lokiDesktop?.watchPreviewFile ||
      !window.lokiDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.lokiDesktop.onPreviewFileChanged(payload => {
    ...
    void window.lokiDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.lokiDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.lokiDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:loki-preview')
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.lokiDesktop?.zoom?.factor?.() || 1
      ...
      openGuestContextMenu(
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.lokiDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.lokiDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.lokiDesktop?.openExternal(currentUrl)
                : undefined
            }
