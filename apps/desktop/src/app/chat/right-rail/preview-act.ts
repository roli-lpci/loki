...
const preamble = () => `  var w = window;
  ...
  w.__lokiActHolder = w.__lokiActHolder || {};
  w.__lokiAct = ${actEngineSource()};
  w.__lokiWatch_fn = (${watchInPage.toString()});
  w.__lokiWatchTag = ${WATCH_TAG};
  var holder = w.__lokiActHolder;
  var act = function (a) { return w.__lokiAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__lokiWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__lokiHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'LOKI-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
