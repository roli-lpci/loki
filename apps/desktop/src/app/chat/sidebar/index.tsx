...
import { searchSessions, type SessionInfo, type SessionSearchResult } from '@/loki'
...
// highlight markers (sqlite snippet() delimiters — see loki_state_search.py).
...
export function ChatSidebar({
  ...
  useEffect(() => {
    ...
    window.addEventListener('loki:new-session-shortcut', onShortcut)

    return () => {
      window.removeEventListener('loki:new-session-shortcut', onShortcut)
      clearTimeout(timeout)
    }
  }, [])
  ...
  // A turn can `git worktree add/remove` in the terminal (e.g. you ask Loki to
