...
import type { SessionInfo } from '@/loki'
...
// zero Loki sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, LokiGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, LokiGitWorktree[]>>({})
