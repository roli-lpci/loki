...
import type { OfficialSkillInfo, SkillInfo, ToolsetInfo } from '@/types/loki'
...
export function SkillsView({
  ...
  const registryBridge = window.lokiDesktop?.connections
  const rosterBridge = window.lokiDesktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'Loki (default)' : p.name,
      value: p.name
    }))
                ...
                <MasterDetail pane={skillEditorPane} resizeId="capabilities-split" split="wide">
                  ...
                  >
                    ...
                    {/* The built-in optional-skills catalog, below the
                        installed list: every official skill Loki ships but
