...
How to build Loki Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Loki surface can also
...
- A **runtime home change** (switching the underlying `LOKI_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Loki — do not assume
...
## WunderCorp free tier: state is pulled, never latched in the renderer

The free tier (a WunderCorp identity with no account, `loki_cli/anon_auth.py`) reaches the renderer
...
`POST /api/providers/oauth/wundercorp/start` + poll route, which over a free-tier identity registers the
