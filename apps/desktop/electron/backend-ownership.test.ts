...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'loki serve --port 0', ...identity(overrides) }
}
...
test('failed persistence awaits asynchrowundercorp cleanup of the exact identity', async () => {
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/loki serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m loki_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/loki --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Loki Runtime\\loki.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('loki chat --query serve'), false)
