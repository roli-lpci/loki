 ...
 *
 * The pre-update handoff kills Loki-OWNED venv daemons (the memory plugin's
 * hindsight daemon) so the updater never races a mapped shim. External
 * holders (a user terminal running `loki`, unrelated scripts) must NOT be
...
/**
 * True when a process is a Loki-owned venv daemon: its exe lives under
 ...
 * spawns DETACHED — it outlives Loki and holds venv shims mapped).
 */
export function isLokiOwnedVenvDaemon(
