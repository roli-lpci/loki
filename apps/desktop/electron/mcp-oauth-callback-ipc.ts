 ...
 *   - the browser sees only a minimal "return to Loki" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to Loki.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('loki:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('loki:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('loki:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
