...
test('desktop backend PATH adds Loki-managed bins and missing POSIX sane entries', () => {
  ...
  assert.deepEqual(entries.slice(0, 3), [
    '/Users/test/.loki/node/bin',
    '/Users/test/.loki/node',
    '/Users/test/.loki/loki-agent/venv/bin'
  ])
...
test('managed Node dirs lead with the platform-native layout but always offer both', () => {
  const posix = lokiManagedNodePathEntries('/Users/test/.loki', {
    platform: 'darwin',
    pathModule: path.posix
  })

  const windows = lokiManagedNodePathEntries('C:\\Users\\test\\AppData\\Local\\loki', {
    platform: 'win32',
  ...
  assert.deepEqual(posix, ['/Users/test/.loki/node/bin', '/Users/test/.loki/node'])
...
test('managed Node dirs are empty without a Loki home', () => {
  assert.deepEqual(lokiManagedNodePathEntries(undefined, { platform: 'darwin', pathModule: path.posix }), [])
  assert.deepEqual(lokiManagedNodePathEntries('', { platform: 'win32', pathModule: path.win32 }), [])
})
...
test('every managed Node dir outranks the inherited PATH on both platforms', () => {
  for (const [platform, pathModule, home, inherited, delimiter] of [
    ['darwin', path.posix, '/Users/test/.loki', '/usr/local/bin:/usr/bin', ':'],
    ['win32', path.win32, 'C:\\loki', 'C:\\Program Files\\nodejs;C:\\Windows\\System32', ';']
  ] as const) {
    ...
    const managed = lokiManagedNodePathEntries(home, { platform, pathModule })
...
test('buildDesktopBackendEnv extends PYTHONPATH and backend PATH together', () => {
  ...
  assert.ok(
    env.PATH.startsWith(
      '/Users/test/.loki/node/bin:/Users/test/.loki/node:/Users/test/.loki/loki-agent/venv/bin:'
    )
  )
...
test('Windows PATH casing and delimiter are preserved without POSIX sane entries', () => {
  ...
  assert.ok(
    env.Path.startsWith(
      'C:\\Users\\test\\AppData\\Local\\loki\\node;C:\\Users\\test\\AppData\\Local\\loki\\node\\bin;'
    )
  )
