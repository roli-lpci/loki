...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(LOKI_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Loki-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.loki/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'loki-desktop-ssh')
  }

  return path.join(os.homedir(), '.loki', 'desktop-ssh')
}
