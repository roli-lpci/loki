...
export const ZOOM_STORAGE_KEY = 'loki:desktop:zoomLevel'
 ...
 * but forgot to emit 'loki:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('loki:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `loki:zoom:changed`, which never fired —
