 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Loki dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Loki install on the remote (login-shell probe),
 ...
 *     `LOKI_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForLoki in.
 *
 * The minted LOKI_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const REMOTE_LOCK_DIR = '~/.loki/desktop-ssh'
...
// Resolve the remote loki executable. An EXPLICIT path is honored strictly
...
async function locateLoki(ssh, remoteLokiPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The loki binary or wrapper script
    ...
    //     the Loki version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/wundercorp/loki/issues/74411
    return candidate
  }
  ...
  if (remoteLokiPath) {
    if (await isExecutable(remoteLokiPath)) {
      return resolveLauncher(remoteLokiPath)
    }

    const err: any = new Error(
      `The Loki path you set is not an executable on the remote host: "${remoteLokiPath}". ` +
        'Check the path (it must be the full path to the `loki` binary on the remote, e.g. ' +
        '~/loki-agent/.venv/bin/loki), or clear it to auto-detect.'
    )

    err.kind = 'loki-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v loki')}`)).trim()
  ...
  candidates.push('~/.local/bin/loki')
  candidates.push('/usr/local/bin/loki')
  candidates.push('~/.loki/loki-agent/venv/bin/loki')
  ...
  const err: any = new Error(
    'Loki is not installed on the remote host (could not find a `loki` executable). ' +
      'Install it on the remote with:  curl -fsSL https://loki.computer/install.sh | sh  ' +
      '— or set the Loki path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<loki> --version`,
// e.g. "Loki Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH loki a
// connection uses, so a stale/unexpected install is visible.
async function probeLokiVersion(ssh, lokiPath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(lokiPath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Loki Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The LOKI_HOME the remote dashboard will use (explicit env wins, else
// ~/.loki). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteLokiHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${LOKI_HOME:-$HOME/.loki}"')).trim().split('\n').pop()

    return out || '~/.loki'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Loki home.')
...
marker=home/'.loki-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Loki checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, lokiHome) {
  const home = assertSafeRemoteHome(lokiHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Loki install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Loki update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Loki update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteLokiProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteLokiHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Loki profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Loki home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'lokiPath', 'lokiHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !lokiPath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(lokiPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/loki`, leaving neither
      // launcher nor LOKI_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `loki_home=os.path.expanduser(${shq(lokiHome)}) if ${shq(lokiHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if loki_home:\n' +
      ' expected_entries.add(os.path.join(loki_home,"loki-agent","venv","bin","loki"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.lokiPath)
  const expectedHome = lock.lokiHome ? expandRemotePath(lock.lokiHome) : "''"
  ...
  const executableMatch = lock.lokiHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.lokiPath === expected.lokiPath &&
    lock.lokiHome === expected.lokiHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.lokiPath)})
loki_home=os.path.expanduser(${py(lock.lokiHome)})
expected_entries={expected_path,os.path.join(loki_home,"loki-agent","venv","bin","loki")}
...
// execing Loki. mutexPath is expandRemotePath() output — a complete shell
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"loki-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(lokiPath, profile, opts: any = {}) {
  const loki = expandRemotePath(lokiPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.lokiHome || '~/.loki')}/.loki-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.lokiHome || '~/.loki')}/.loki-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env LOKI_DESKTOP=1${opts.guestOnboarding === true ? ' LOKI_GUEST_ONBOARDING=1' : ''} ${loki} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} loki-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, lokiPath) {
  const loki = expandRemotePath(lokiPath)

  const out = await ssh.exec(
    `help="$(${loki} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(
  ...
  if (!(await remoteSupportsSshOwnership(ssh, lokiPath))) {
    const err: any = new Error(
      'The remote Loki install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Loki on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(lokiPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        lokiHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForLoki, a token-gated
...
async function connect(deps) {
  ...
  const lokiHome = await probeRemoteLokiHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, lokiHome)
  const lokiPath = await locateLoki(ssh, remoteLokiPath)
  log(`located loki at ${lokiPath}`)
  const lokiVersion = await probeLokiVersion(ssh, lokiPath)

  if (lokiVersion) {
    log(`remote loki version: ${lokiVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Loki Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.lokiPath === lokiPath &&
      lock.lokiHome === lokiHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    lokiHome,
    guestOnboarding,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, lokiHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForLoki(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
