...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'loki_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'loki_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'loki_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Loki.app/Contents/MacOS/Loki', 'darwin'),
    '/Applications/Loki.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Loki.app/Contents/MacOS/Loki', 'darwin'),
    '/Users/x/Applications/Loki.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Loki\\Loki.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Loki'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\loki-desktop\\Loki.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\loki-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Loki.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_LokiXXXX/loki', 'linux', { APPIMAGE: '/home/x/Apps/Loki.AppImage' }),
    '/home/x/Apps/Loki.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/loki/linux-unpacked/loki', 'linux', {}), '/opt/loki/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/loki', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Loki.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Loki.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.loki/loki-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.loki/loki-agent',
    uninstallArgs: ['-m', 'loki_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/loki/linux-unpacked',
    lokiHome: '/home/x/.loki'
  })
  ...
  assert.match(script, /'-m' 'loki_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/loki\/linux-unpacked'/)
  assert.match(script, /export LOKI_HOME='\/home\/x\/\.loki'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.loki/loki-agent',
    agentRoot: '/home/x/.loki/loki-agent',
    uninstallArgs: ['-m', 'loki_cli.uninstall', '--mode', 'full'],
    appPath: null,
    lokiHome: '/home/x/.loki'
  })

  // System python + source on PYTHONPATH so import loki_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.loki\/loki-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'loki_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'loki_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    lokiHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'loki_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\loki',
    uninstallArgs: ['-m', 'loki_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Loki',
    lokiHome: 'C:\\Users\\x\\AppData\\Local\\loki'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import loki_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\loki;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "loki_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Loki" >nul 2>&1/)
