 ...
 *
 * So we delegate to `loki gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `loki update`'s
...
export function stopGatewayBeforeUpdate(
  ...
  return runGatewayLifecycleCommand(lokiCliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(lokiCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(lokiCliPath, ['gateway', 'start', '--all'], deps)
}

function runGatewayLifecycleCommand(lokiCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(lokiCliPath, args)
  }

  if (!existsSync(lokiCliPath)) {
    return false
  }

  try {
    exec(lokiCliPath, args, {
