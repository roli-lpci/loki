 ...
 *
 * Claiming is asynchrowundercorp so a failed persistence transaction can await child
  ...
  return {
    ...
    async reapOrphans(): Promise<number[]> {
      ...
      for (let i = 0; i < entries.length; i += 1) {
        ...
        // if it reaches reapOrphans (see main.ts startLoki + #87295).
...
export function backendCommandMatches(command: unknown): boolean {
  return /(?:^|[\s/\\"])(?:loki(?:\.exe)?|loki_cli\.main|loki_cli[/\\]main\.py)"?(?:\s+(?:--profile|-p)\s+\S+)?\s+(?:serve|dashboard)(?:\s|$)/i.test(
    String(command ?? '')
  )
}

/** Coordinates all quit paths so asynchrowundercorp backend teardown runs once. */
