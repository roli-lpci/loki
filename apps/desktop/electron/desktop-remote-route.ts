...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'LOKI_DESKTOP_REMOTE_URL is set but LOKI_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Loki backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `loki serve` the desktop never
