 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `loki update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `loki desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'Loki couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>loki desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>loki desktop --force-build</code>, then restart the app.</p>
