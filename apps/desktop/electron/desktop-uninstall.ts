 ...
 * the three user-facing uninstall modes to the `loki uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m loki_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `loki_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'loki_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Loki.app/Contents/MacOS/Loki  → …/Loki.app
 *   Windows: …\Loki\Loki.exe                 → …\Loki  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Loki.app/Contents/MacOS/Loki → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Loki.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Loki.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Loki$/i.test(dir) || /[\\/]loki-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/loki
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import loki_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, lokiHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import loki_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Loki install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "LOKI_HOME=${String(lokiHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
