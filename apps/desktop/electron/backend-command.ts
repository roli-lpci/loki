// Backend subcommand routing for the desktop-managed Loki process.
//
// The desktop app launches its own headless backend via `loki serve` — it
...
// app hasn't updated yet, or an older `loki` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Loki profile to pin via `--profile`.
 */
 ...
 * `-m loki_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `loki_cli/subcommands/dashboard.py` source registers
