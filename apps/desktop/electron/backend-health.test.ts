...
test('uses lightweight /api/health for current backends', async () => {
  ...
  await waitForLokiReady('http://127.0.0.1:9000/', {
    token: 'secret-token',
...
test('does not fall back to heavyweight /api/status for transient health failures', async () => {
  ...
  await assert.rejects(
    waitForLokiReady('http://127.0.0.1:9000', {
      fetchPublicJson: async url => {
        calls.push(['public', url])
        throw new Error('Timed out connecting to Loki backend after 15000ms')
      },
      fetchJson: async url => {
        calls.push(['token', url])
      },
...
test('recognizes missing-route shapes only', () => {
  ...
  assert.equal(
    isMissingHealthEndpointError(
      new Error('Expected JSON from /api/health but got HTML. The endpoint is likely missing on the Loki backend.')
    ),
    true
  )
  assert.equal(isMissingHealthEndpointError(new Error('Timed out connecting to Loki backend after 15000ms')), false)
...
test('anonymous gate-shaped 401 falls back to /api/status (backend predates /api/health)', async () => {
  ...
  await waitForLokiReady('http://192.168.1.132:9119', {
    token: null,
    fetchPublicJson: async url => {
      calls.push(['public', url])
...
test('a credentialed 401 fails fast for reauth instead of reporting a dead session ready', async () => {
  ...
  await assert.rejects(
    waitForLokiReady('https://gateway.example', {
      token: 'session-token',
      fetchPublicJson: async () => {
...
test('unsigned OAuth is a terminal reauth failure; needsOauthLogin alone is not', () => {
  // The unsigned-in throw must set isReauthRequired so startLoki latches.
  ...
  assert.equal(isReauthRequiredError(new Error('Could not reach the remote Loki gateway')), false)
})
...
test('a credentialed probe still uses the 404 fallback for a genuinely missing route', async () => {
  ...
  await waitForLokiReady('https://gateway.example', {
    token: 'session-token',
    fetchPublicJson: async () => {
      throw new Error('public probe must not be used when credentialed')
    },
...
test('isWunderCorpCloudAgentUrl detects cloud agent hosts', () => {
  // Positive cases
  assert.equal(isWunderCorpCloudAgentUrl('https://ares-3009.agents.wundercorp.com'), true)
  assert.equal(isWunderCorpCloudAgentUrl('https://ares-3009.agents.wundercorp.com/api/health'), true)
  assert.equal(isWunderCorpCloudAgentUrl('http://test.agents.wundercorp.com'), true)

  // Negative cases
  assert.equal(isWunderCorpCloudAgentUrl('http://127.0.0.1:9000'), false)
  assert.equal(isWunderCorpCloudAgentUrl('https://gateway.example.com'), false)
  assert.equal(isWunderCorpCloudAgentUrl('https://wundercorp.co'), false)
  assert.equal(isWunderCorpCloudAgentUrl('not-a-url'), false)
})

test('waitForLokiReady surfaces actionable error for cloud agent 503', async () => {
  let attempts = 0
  const currentTime = { value: 0 }

  try {
    await waitForLokiReady('https://ares-3009.agents.wundercorp.com', {
      fetchPublicJson: async () => {
        attempts++
        // Always return 503
    ...
    assert.ok(error.message.includes('WunderCorp Cloud agent'), `unexpected message: ${error.message}`)
    assert.ok(error.message.includes('503'), `should mention status code: ${error.message}`)
    assert.ok(error.message.includes('portal.wundercorp.com'), `should mention portal: ${error.message}`)
    assert.ok(error.message.includes('discord.gg/yEaT8dv5Xn'), `should mention Discord: ${error.message}`)
...
test('waitForLokiReady does not cloud-wrap non-cloud 503 errors', async () => {
  const currentTime = { value: 0 }
...
test('makeWunderCorpCloudBackendDownError produces the Cloud shape and preserves cause', () => {
  const err = new Error('upstream unavailable') as any
  err.statusCode = 503
  const result = makeWunderCorpCloudBackendDownError('https://ares-3009.agents.wundercorp.com', err)
  assert.ok(result)
  ...
  assert.ok(result?.message.includes('WunderCorp Cloud agent ares-3009.agents.wundercorp.com is down'))
})

test('makeWunderCorpCloudBackendDownError returns null for a Cloud 401 (routes to reauth)', () => {
  const err = new Error('Unauthorized') as any
  err.statusCode = 401
  assert.equal(makeWunderCorpCloudBackendDownError('https://ares-3009.agents.wundercorp.com', err), null)
})

test('makeWunderCorpCloudBackendDownError returns null for a non-Cloud 503 (generic remote failure)', () => {
  const err = new Error('Service Unavailable') as any
  err.statusCode = 503
  assert.equal(makeWunderCorpCloudBackendDownError('https://gateway.example.com', err), null)
  assert.equal(makeWunderCorpCloudBackendDownError('http://127.0.0.1:9000', err), null)
})

test('makeWunderCorpCloudBackendDownError preserves legacy string-prefix compatibility', () => {
  const result = makeWunderCorpCloudBackendDownError(
    'https://ares-3009.agents.wundercorp.com',
