/**
 * windows-loki-path.ts
 *
 * Pure, dependency-injected pieces of Windows `loki` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvLokiCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `loki` shim shadowed the real
 *      loki.cmd/loki.exe; the shim then failed the --version probe and
 ...
 *      and Loki launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvLokiCommand() — unwrapWindowsVenvLokiCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `loki`) must
 * not shadow `loki.cmd`/`loki.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Loki launcher that drives `loki update`.
...
export function chooseUpdaterArgs(signals: BootstrapRecoverySignals, branch: string): string[] {
  const canRunUpdater = signals.hasVenvLoki && signals.hasVenvPython
...
export interface ResolveVenvLokiCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportLokiCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `loki`/`loki.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/loki(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m loki_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportLokiCli(). A
 * venv whose update died mid-`pip install` still has python.exe + loki.exe
 ...
 *
 * Returns null when `command` is not a venv loki shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvLokiCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvLokiCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^loki(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Loki at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Loki Python at ${python}`,
    command: python,
