...
test('oauthGuardMayHardFail keeps the strict guard for oauth and mixed deployments', () => {
  assert.equal(oauthGuardMayHardFail([{ name: 'wundercorp', supportsPassword: false }]), true)
  assert.equal(
    oauthGuardMayHardFail([
...
test('oauthGuardMayHardFail keeps the strict guard for string oauth providers', () => {
  assert.equal(oauthGuardMayHardFail(['wundercorp'] as any), true)
  assert.equal(oauthGuardMayHardFail(['wundercorp', 'basic'] as any), true)
})
