...
import { makeWunderCorpCloudBackendDownError } from './backend-health'
import {
...
test('normalizeRemoteHeaders keeps safe proxy headers and drops transport/auth headers', () => {
  assert.deepEqual(
    normalizeRemoteHeaders({
      ...
      'X-Loki-Session-Token': { encoding: 'plain', value: 'token' },
...
test('remoteRequestMatchesBaseUrl treats HTTPS and WSS as the same gateway origin', () => {
  assert.equal(
    remoteRequestMatchesBaseUrl(
      'wss://loki.example.com/gateway/api/ws?ticket=abc',
      'https://loki.example.com/gateway'
    ),
    true
  )
  assert.equal(remoteRequestMatchesBaseUrl('ws://loki.example.com/api/ws', 'http://loki.example.com'), true)
  assert.equal(
    remoteRequestMatchesBaseUrl('wss://loki.example.com/other/api/ws', 'https://loki.example.com/gateway'),
    false
  )
  assert.equal(
    remoteRequestMatchesBaseUrl('wss://other.example.com/gateway/api/ws', 'https://loki.example.com/gateway'),
    false
  )
})
...
test('profileRemoteOverride returns the per-profile remote with defaulted auth mode', () => {
  const config = {
    profiles: {
      coder: { mode: 'remote', url: '  https://coder.example.com/loki  ', token: { value: 'sek' } }
    }
  }

  assert.deepEqual(profileRemoteOverride(config, 'coder'), {
    url: 'https://coder.example.com/loki',
    authMode: 'token',
    token: { value: 'sek' }
  })
})
...
test('profileRemoteOverride treats a cloud entry as a remote override', () => {
  ...
  const config = {
    profiles: {
      coder: { mode: 'cloud', url: 'https://agent-1.agents.wundercorp.com', authMode: 'oauth' }
    }
  }

  assert.deepEqual(profileRemoteOverride(config, 'coder'), {
    url: 'https://agent-1.agents.wundercorp.com',
    authMode: 'oauth',
    token: undefined
  })
})
...
test('localProfileEntry preserves inactive SSH drafts but drops Cloud state', () => {
  const ssh = { mode: 'ssh', host: 'box', user: 'alice', remoteLokiPath: '/loki' }
...
// --- translateSelfProfileQuery (registry SSH-scoped loki:api contract) ---
...
test('resolveProfileApiRequest uses exact method and path eligibility for mixed families', () => {
  ...
  assert.deepEqual(
    resolveProfileApiRequest('iris', '/api/model/recommended-default?provider=wundercorp', {
      requestMethod: 'GET'
    }),
    {
      backendProfile: 'iris',
      requestPath: '/api/model/recommended-default?provider=wundercorp'
    }
  )
})
...
test('resolveProfileApiRequest routes action-status polls with the action-spawning routes', () => {
  ...
  // install): _spawn_loki_action registers the dynamic action name only in
...
test('normalizeRemoteBaseUrl strips trailing slashes, hash, and query', () => {
  ...
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/loki/'), 'https://gw.example.com/loki')
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/loki?x=1#frag'), 'https://gw.example.com/loki')
})

test('normalizeRemoteBaseUrl preserves a path prefix', () => {
  assert.equal(normalizeRemoteBaseUrl('https://host/loki'), 'https://host/loki')
})
...
test('normalizeRemoteBaseUrl auto-prepends http:// for scheme-less host:port input', () => {
  ...
  assert.equal(normalizeRemoteBaseUrl('gw.example.com/loki/'), 'http://gw.example.com/loki')
})
...
test('buildGatewayWsUrl honors a path prefix', () => {
  assert.equal(buildGatewayWsUrl('https://host/loki', 't'), 'wss://host/loki/api/ws?token=t')
})
...
test('buildGatewayWsUrlWithTicket uses ?ticket= not ?token=', () => {
  const url = buildGatewayWsUrlWithTicket('https://gw.example.com/loki', 'tkt-9')
  assert.equal(url, 'wss://gw.example.com/loki/api/ws?ticket=tkt-9')
...
test('authModeFromStatus returns oauth when auth_required is true', () => {
  assert.equal(authModeFromStatus({ auth_required: true, auth_providers: ['wundercorp'] }), 'oauth')
})
...
test('cookiesHaveSession detects the bare access-token cookie', () => {
  assert.equal(cookiesHaveSession([{ name: 'loki_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession detects the __Host- and __Secure- prefixed variants', () => {
  assert.equal(cookiesHaveSession([{ name: '__Host-loki_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveSession([{ name: '__Secure-loki_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession is false for an empty value', () => {
  assert.equal(cookiesHaveSession([{ name: 'loki_session_at', value: '' }]), false)
})
...
test('cookiesHaveSession ignores unrelated cookies (AT-only by design)', () => {
  ...
  assert.equal(cookiesHaveSession([{ name: 'loki_session_rt', value: 'x' }]), false)
...
test('AT_COOKIE_VARIANTS covers all three deploy shapes', () => {
  assert.deepEqual(AT_COOKIE_VARIANTS, ['__Host-loki_session_at', '__Secure-loki_session_at', 'loki_session_at'])
})

test('RT_COOKIE_VARIANTS covers all three deploy shapes', () => {
  assert.deepEqual(RT_COOKIE_VARIANTS, ['__Host-loki_session_rt', '__Secure-loki_session_rt', 'loki_session_rt'])
})

// --- cookiesHaveLiveSession (AT or RT — the connectivity check) ---

test('cookiesHaveLiveSession is true for a live access-token cookie', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'loki_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-loki_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-loki_session_at', value: 'x' }]), true)
})
...
test('cookiesHaveLiveSession is true for an RT cookie even with NO access-token cookie', () => {
  ...
  assert.equal(cookiesHaveLiveSession([{ name: 'loki_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-loki_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-loki_session_rt', value: 'x' }]), true)
})

test('cookiesHaveLiveSession is true when both AT and RT are present', () => {
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'loki_session_at', value: 'a' },
      { name: 'loki_session_rt', value: 'r' }
    ]),
    true
  )
})
...
test('cookiesHaveLiveSession is false for empty values', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'loki_session_at', value: '' }]), false)
  assert.equal(cookiesHaveLiveSession([{ name: 'loki_session_rt', value: '' }]), false)
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'loki_session_at', value: '' },
      { name: 'loki_session_rt', value: '' }
    ]),
    false
  )
})
...
// --- cookiesHavePrivySession (WunderCorp portal / Privy auth, NOT gateway cookies) ---
...
test('cookiesHavePrivySession does NOT treat loki gateway cookies as a portal session', () => {
  // The whole point of Q7: a gateway session cookie is NOT a portal sign-in.
  assert.equal(cookiesHavePrivySession([{ name: 'loki_session_at', value: 'x' }]), false)
  assert.equal(cookiesHavePrivySession([{ name: '__Host-loki_session_rt', value: 'x' }]), false)
})
...
test('cookiesHavePrivyAccessToken is false for empty values, gateway cookies, and non-arrays', () => {
  ...
  assert.equal(cookiesHavePrivyAccessToken([{ name: 'loki_session_at', value: 'x' }]), false)
...
test('gateway WS URL IPC result serializes success and the auth-vs-transport matrix', async () => {
  ...
  for (const error of [
    ...
    new Error('Timed out connecting to Loki backend after 8000ms'),
...
test('gatewayTicketFailure only copies an integer statusCode, not a message prefix', () => {
  ...
  // classifier (makeWunderCorpCloudBackendDownError) handles the prefix at the mint
...
// BEFORE waitForLokiReady. This mirrors main.ts buildRemoteConnection's
// catch — classify a WunderCorp Cloud server fault via the shared factory, else
...
test('OAuth ticket-mint 503 surfaces the Cloud-down error (startup boundary)', () => {
  const baseUrl = 'https://ares-3009.agents.wundercorp.com'
  const ticketErr = new Error('upstream unavailable') as any
  ticketErr.statusCode = 503

  // The exact production sequence from main.ts.
  const cloudError = makeWunderCorpCloudBackendDownError(baseUrl, ticketErr)

  if (cloudError !== null) {
    assert.equal((cloudError as any).isCloudBackendDown, true)
    assert.equal((cloudError as any).statusCode, 503)
    assert.ok(cloudError.message.includes('WunderCorp Cloud agent ares-3009.agents.wundercorp.com is down'))
