...
// asynchrowundercorply, and Windows rejects unlink/rename on a path whose handle is
...
export function downloadTempPath(destPath: string): string {
  return path.join(path.dirname(destPath), `.loki-download-${crypto.randomBytes(4).toString('hex')}.part`)
}
...
export function pumpStreamToFile(res: ReadableLike, destPath: string, deps: PumpDeps): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    // throws synchrowundercorply still lands on the rejection path instead of escaping
