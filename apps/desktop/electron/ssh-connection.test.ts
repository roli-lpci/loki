...
test('redactSecrets scrubs the spawn-time session token env var', () => {
  const line = 'setsid env LOKI_DASHBOARD_SESSION_TOKEN=abc123deadbeef LOKI_DESKTOP=1 loki dashboard'
  const out = redactSecrets(line)
  assert.ok(!out.includes('abc123deadbeef'))
  assert.match(out, /LOKI_DASHBOARD_SESSION_TOKEN=<redacted>/)
  // non-secret env vars are preserved
  assert.match(out, /LOKI_DESKTOP=1/)
})
...
test('redactSecrets scrubs Authorization and X-Loki-Session-Token headers', () => {
  ...
  assert.match(redactSecrets('X-Loki-Session-Token: hdr_888'), /X-Loki-Session-Token: ?<redacted>/)
  assert.ok(!redactSecrets('X-Loki-Session-Token: hdr_888').includes('hdr_888'))
})
...
test('controlSocketPath default base stays under sun_path even with the temp-listener suffix', () => {
  ...
  const p = controlSocketPath('loki', 'remote-build-server', 22) // no baseDir → default
...
test('buildExecArgs ends with host then the remote command', () => {
  ...
  const args = buildExecArgs(conn, 'command -v loki', 15000)
  assert.equal(args[args.length - 1], 'command -v loki')
...
test('close() removes the control socket when -O exit fails', async () => {
  const dir = path.join(os.tmpdir(), `loki-ssh-close-${process.pid}-${Date.now()}`)
...
test('open() creates the control-socket directory if it does not exist', async () => {
  const dir = path.join(os.tmpdir(), `loki-ssh-test-${process.pid}-${Date.now()}`)
