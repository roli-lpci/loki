// Profile-delete routing logic for the `loki:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_loki_home() and
...
/** Parse a `loki:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its LOKI_HOME.
 *
...
/**
 * Route the next `loki:api` request away from the primary/window backend
 ...
 * ensure_loki_home() recreates the directory the delete just removed.
 */
