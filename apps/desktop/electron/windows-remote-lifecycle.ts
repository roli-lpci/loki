...
const READY_RE = /^LOKI_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitLokiPath = '') {
  const explicit = psLiteral(explicitLokiPath)
  ...
  const script = [
    ...
    '$lokiHome=$env:LOKI_HOME',
    'if(-not $lokiHome){$lokiHome=Join-Path $env:LOCALAPPDATA "loki"}',
    'Assert-NoReparse $lokiHome $true',
    '$candidate=[IO.Path]::Combine($lokiHome, "loki-agent\\venv\\Scripts\\loki.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "loki-agent\\.venv\\Scripts\\loki.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $lokiHome "loki-agent\\venv\\Scripts\\loki.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "loki-agent\\.venv\\Scripts\\loki.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command loki.exe -ErrorAction SilentlyContinue',
    ...
    '$loki=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$loki=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $loki){throw "Loki is not installed on the remote Windows host."}',
    'Assert-NoReparse $loki $false',
    'if($explicit -and $loki -ne $explicit){throw "The configured Loki path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($loki), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;lokiHome=$lokiHome;lokiPath=$loki;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(lokiHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class LokiMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(lokiHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".loki-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[LokiMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, lokiHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(lokiHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitLokiPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitLokiPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'loki_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'loki_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'loki_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.lokiHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".loki-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};lokiPath=${psLiteral(reservation.lokiPath)};lokiHome=${psLiteral(reservation.lokiHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.lokiPath === 'string' &&
    typeof lock.lokiHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.lokiPath === runtime.lokiPath &&
    lock.lokiHome === runtime.lokiHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteLokiPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.lokiHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.lokiPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Loki on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.lokiPath = inspection.path
  const lokiVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located loki at ${runtime.lokiPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, lokiPath: runtime.lokiPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForLoki(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Loki SSH"', 'powershell.exe -NoLogo')
