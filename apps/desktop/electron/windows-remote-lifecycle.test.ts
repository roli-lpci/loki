...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command loki.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          lokiHome: 'C:\\Users\\alice\\.loki',
          lokiPath: 'C:\\Loki\\loki.exe',
          python: 'C:\\Loki\\python.exe'
        })
      }

      if (script.includes('.loki-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('loki_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.loki\\profiles\\research')
  assert.match(script, /\.loki-update-in-progress/)
...
test('Windows probe validates Loki and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $lokiHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Loki is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Loki is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Loki's\\python.exe" }, 'inspect', [
    'C:\\x y\\loki.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'loki_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Loki''s/)
  assert.match(script, /C:\\x y\\loki\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { lokiPath: lock.lokiPath, lokiHome: lock.lokiHome }
