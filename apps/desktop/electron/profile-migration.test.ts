 ...
 *      top of startLoki() in main.ts, before primaryProfileKey() is read); the
...
function baseDeps(overrides: Record<string, unknown> = {}) {
  ...
  return {
    legacyActivePath: '/home/u/.loki/active_profile',
    lokiHome: '/home/u/.loki',
    profilesRoot: '/home/u/.loki/profiles',
...
test('findRunningGatewayProfiles returns [] when no pid files exist', () => {
  const fs = makeFs({ '/home/u/.loki/profiles/coder': { dir: true } })
  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.loki/profiles', ['coder'], {
      ...fs,
      isLokiProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles drops stale (non-loki) recycled PIDs', () => {
  // Two profiles have pid files, but only coder's pid is a live loki process.
  // The recycled PID at 5678 belongs to an unrelated process (e.g. Chrome).
  const fs = makeFs({
    '/home/u/.loki/profiles/coder/gateway.pid': { content: '{"pid":1234}' },
    '/home/u/.loki/profiles/writer/gateway.pid': { content: '{"pid":5678}' }
  })

  const deps = {
    ...fs,
    isLokiProcess: (pid: number) => pid === 1234
  }

  assert.deepEqual(findRunningGatewayProfiles('/home/u/.loki/profiles', ['coder', 'writer'], deps), ['coder'])
})

test('findRunningGatewayProfiles tolerates malformed pid files', () => {
  const fs = makeFs({
    '/home/u/.loki/profiles/coder/gateway.pid': { content: '{not json' }
  })
...
test('findRunningGatewayProfiles drops non-integer and non-positive PIDs', () => {
  const fs = makeFs({
    '/home/u/.loki/profiles/coder/gateway.pid': { content: '{"pid":-1}' },
    '/home/u/.loki/profiles/writer/gateway.pid': { content: '{"pid":1.5}' },
    '/home/u/.loki/profiles/extra/gateway.pid': { content: '{"pid":0}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.loki/profiles', ['coder', 'writer', 'extra'], {
      ...fs,
      isLokiProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles preserves order of allProfiles', () => {
  const fs = makeFs({
    '/home/u/.loki/profiles/coder/gateway.pid': { content: '{"pid":1}' },
    '/home/u/.loki/profiles/writer/gateway.pid': { content: '{"pid":2}' }
  })

  const deps = { ...fs, isLokiProcess: () => true }
  assert.deepEqual(findRunningGatewayProfiles('/home/u/.loki/profiles', ['coder', 'writer'], deps), [
    'coder',
    'writer'
  ])
})
...
test('listProfileDirs includes default and any name passing the regex', () => {
  const fs = makeFs({
    '/home/u/.loki/profiles/default': { dir: true },
    '/home/u/.loki/profiles/coder': { dir: true },
    '/home/u/.loki/profiles/writer': { dir: true }
  })
...
test('listProfileDirs skips files (not directories) and invalid names', () => {
  const fs = makeFs({
    '/home/u/.loki/profiles/default': { dir: true },
    '/home/u/.loki/profiles/some-file.txt': { dir: false },
    '/home/u/.loki/profiles/UPPERCASE': { dir: true } // regex rejects
  })
...
test('decideMigration suppresses write when best is default (single-profile fallback)', () => {
  ...
  // default-primary and we leave it alone. Default's DB is $LOKI_HOME/state.db,
  // not profiles/default/state.db.
  const deps = baseDeps()

  const d = decideMigration(null, [], ['default', 'coder'], deps, p => (p.endsWith('/.loki/state.db') ? 99 : 50))
...
test('migrateActiveProfileIfMissing is a no-op when a user-selected preference file exists', () => {
  ...
  const fs = makeFs({
    ...
    '/home/u/.loki/profiles/coder': { dir: true },
    '/home/u/.loki/profiles/writer': { dir: true },
    '/home/u/.loki/profiles/writer/state.db': { size: 400 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing writes legacy choice with no _migrated flag', () => {
  ...
  const fs = makeFs({
    '/home/u/.loki/active_profile': { content: 'coder' },
    '/home/u/.loki/profiles/coder': { dir: true },
    '/home/u/.loki/profiles/writer': { dir: true }
  })
...
test('migrateActiveProfileIfMissing writes heuristic choice with _migrated=true', () => {
  ...
  const fs = makeFs({
    '/home/u/.loki/profiles/coder/state.db': { size: 50 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.loki/profiles/writer/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing is a no-op for single-profile (default-only) installs', () => {
  ...
  // Production default DB is ~/.loki/state.db, not profiles/default/state.db.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.loki/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing prefers a single running gateway over heuristics', () => {
  ...
  const fs = makeFs({
    '/home/u/.loki/profiles/coder/gateway.pid': { content: '{"pid":42}' },
    '/home/u/.loki/profiles/writer/state.db': { size: 500 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isLokiProcess: (pid: number) => pid === 42,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
...
// ---------------------------------------------------------------------------
// Production layout: default is ~/.loki, not ~/.loki/profiles/default
// ---------------------------------------------------------------------------

test('profileStateDbPath puts default at lokiHome, named under profilesRoot', () => {
  assert.equal(profileStateDbPath('default', '/home/u/.loki', '/home/u/.loki/profiles'), '/home/u/.loki/state.db')
  assert.equal(
    profileStateDbPath('conduit', '/home/u/.loki', '/home/u/.loki/profiles'),
    '/home/u/.loki/profiles/conduit/state.db'
  )
})

test('profileGatewayPidPath puts default at lokiHome', () => {
  assert.equal(
    profileGatewayPidPath('default', '/home/u/.loki', '/home/u/.loki/profiles'),
    '/home/u/.loki/gateway.pid'
  )
  assert.equal(
    profileGatewayPidPath('coder', '/home/u/.loki', '/home/u/.loki/profiles'),
    '/home/u/.loki/profiles/coder/gateway.pid'
  )
})
...
test('findRunningGatewayProfiles sees default gateway.pid at lokiHome', () => {
  const fs = makeFs({
    '/home/u/.loki/gateway.pid': { content: '{"pid":99}' },
    '/home/u/.loki/profiles/coder/gateway.pid': { content: '{"pid":11}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.loki/profiles', ['default', 'coder'], {
      ...fs,
      lokiHome: '/home/u/.loki',
      isLokiProcess: pid => pid === 99
    }),
    ['default']
  )
})
...
test('migrateActiveProfileIfMissing does not pin a tiny named profile over a large default DB', () => {
  ...
  // ~/.loki/profiles/<name>, never scored ~/.loki/state.db, and wrote
  // { profile: named, _migrated: true }.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.loki/profiles/conduit': { dir: true },
    '/home/u/.loki/state.db': { size: 409 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.loki/profiles/conduit/state.db': { size: 2 * 1024 * 1024, mtime: NOW - 60_000 }
  })
...
test('migrateActiveProfileIfMissing still pins a named profile that actually beats default', () => {
  ...
  const fs = makeFs({
    '/home/u/.loki/profiles/work': { dir: true },
    '/home/u/.loki/state.db': { size: 5 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.loki/profiles/work/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing does not pin default when only default gateway is running', () => {
  ...
  const fs = makeFs({
    '/home/u/.loki/gateway.pid': { content: '{"pid":7}' },
    '/home/u/.loki/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isLokiProcess: (pid: number) => pid === 7,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
