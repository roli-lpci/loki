 ...
 * runtime, remote gateways, Loki Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@wundercorp/loki-shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteLokiPath || '').trim() !== String(activeSsh.remoteLokiPath || '').trim() ||
...
/** Turn `ls ~/.loki/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Loki Cloud
 * instances are platform-managed — we never run `loki update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteLokiPath: input.remoteLokiPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Loki-managed names; an empty result stores nothing.
 ...
 * `remoteLokiPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Loki Cloud' : 'Remote gateway'),
...
export function reconcileAppliedGlobalConnection(
  ...
  const hostLabel = hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Loki Cloud' : 'Remote gateway')
