 ...
 * `iter_loki_node_dirs()` in loki_constants.py, which the Electron main
 * process cannot import.
 */
function lokiManagedNodePathEntries(
  lokiHome,
  ...
  const root = pathModule.join(lokiHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const lokiNodeDirs = lokiManagedNodePathEntries(lokiHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([lokiNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeLokiHomeRoot(lokiHome, { pathModule = pathModuleForPlatform(process.platform) }: any = {}) {
  if (!lokiHome) {
    return lokiHome
  }

  const resolved = pathModule.resolve(String(lokiHome))
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). loki_bootstrap sets this inside the child too, but only
