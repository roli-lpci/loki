...
export interface MigrationDeps {
  ...
  /** Default profile home (`~/.loki`). Default's state.db and gateway.pid live here. */
  lokiHome: string
  /** Named-profile root (`~/.loki/profiles`). Does not contain `default`. */
  profilesRoot: string
  ...
  isLokiProcess: (pid: number) => boolean
  now: () => number
...
/**
 * Production layout: default IS `lokiHome`; named profiles are children of
 * `profilesRoot`. There is no `profiles/default` directory on a normal install.
 */
export function profileStateDbPath(name: string, lokiHome: string, profilesRoot: string): string {
  return name === 'default' ? `${lokiHome}/state.db` : `${profilesRoot}/${name}/state.db`
}

export function profileGatewayPidPath(name: string, lokiHome: string, profilesRoot: string): string {
  return name === 'default' ? `${lokiHome}/gateway.pid` : `${profilesRoot}/${name}/gateway.pid`
}

function resolveLokiHome(profilesRoot: string, lokiHome?: string): string {
  if (lokiHome) {
    return lokiHome
  }

  // Tests that predate lokiHome pass only profilesRoot.
...
/**
 * Return the profile names whose gateway.pid file points to a live loki process.
 ...
 *
 * `lokiHome` is optional so existing call sites that only pass `profilesRoot`
 * still work: it is derived as the parent of `…/profiles`.
 */
export function findRunningGatewayProfiles(
  profilesRoot: string,
  allProfiles: string[],
  deps: Pick<MigrationDeps, 'existsSync' | 'readFileSync' | 'isLokiProcess'> & { lokiHome?: string }
): string[] {
  const lokiHome = resolveLokiHome(profilesRoot, deps.lokiHome)
  const running: string[] = []

  for (const name of allProfiles) {
    const pidFile = profileGatewayPidPath(name, lokiHome, profilesRoot)
...
export function decideMigration(
  ...
  for (const name of candidates) {
    const s = score(profileStateDbPath(name, deps.lokiHome, deps.profilesRoot))
...
/** Default is always a candidate; it is `$LOKI_HOME`, not `$LOKI_HOME/profiles/default`. */
...
export function migrateActiveProfileIfMissing(desktopProfileConfigPath: string, deps: MigrationDeps): boolean {
  ...
  // launches `loki --profile default` and is worse than writing nothing
