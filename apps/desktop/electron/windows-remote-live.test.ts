...
//   LOKI_WIN_SSH_HOST   ssh alias/host of the Windows box
//   LOKI_WIN_SSH_USER   remote user
//   LOKI_WIN_SSH_CLI absolute path to the remote loki.exe under test
const liveHost = process.env.LOKI_WIN_SSH_HOST || ''
const liveUser = process.env.LOKI_WIN_SSH_USER || ''
const configuredLoki = process.env.LOKI_WIN_SSH_CLI || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Loki-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredLoki)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredLoki.replace('loki.exe', 'python.exe')}' -m loki_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredLoki.replace('loki.exe', 'python.exe')
        const terminate = `& '${python}' -m loki_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.lokiPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m loki_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m loki_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
