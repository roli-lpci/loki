 ...
 *
 * The staged Tauri `loki-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `loki update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `loki update`, swaps/relaunches
 * the app, and writes .loki-update-result.json. With the app gone before
 * the update starts, the LOKI_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `loki update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into LOKI_HOME on *every* platform
 * (`loki-setup.exe` on Windows, `loki-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_loki_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `loki update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(lokiHome, 'loki-setup.exe')
 ...
 *
 * `copy_self_to_loki_home` deliberately no-ops during `--update`
 ...
 * "Another Loki update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `loki update`, which
 ...
 * the installer is written to LOKI_HOME at install/repair time, making mtime
 ...
 * Node reports exec failures asynchrowundercorply) or an early `exit`. A failed
