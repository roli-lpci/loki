...
function createTempGitRepo() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'loki-update-count-'))
  ...
  try {
    ...
    git('config', 'user.email', 'loki@example.invalid')
...
test('compareApiUrl builds the GitHub compare URL for HTTPS origins', () => {
  assert.equal(
    compareApiUrl({
      currentSha: SHA_A,
      originUrl: 'https://github.com/wundercorp/loki.git',
      targetSha: SHA_B
    }),
    `https://api.github.com/repos/wundercorp/loki/compare/${SHA_A}...${SHA_B}`
  )
})
...
test('compareApiUrl handles SSH origin forms', () => {
  for (const originUrl of [
    ...
    'ssh://git@github.com/wundercorp/loki.git',
