...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('loki:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('loki:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('loki:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('loki:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('loki:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('loki:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('loki:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('loki:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('loki:pet-overlay:control', payload)
  })
}
