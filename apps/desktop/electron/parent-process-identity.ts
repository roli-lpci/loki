export type ParentWatchdogEnv = {
  LOKI_PARENT_PID: string
  LOKI_PARENT_START_MARKER?: string
  LOKI_PARENT_NONCE?: string
  /** Spawn tag consumed by loki_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { LOKI_PARENT_PID: String(pid) }
  env.LOKI_SPAWN = spawnTag(pid, startMarker)
  ...
  env.LOKI_PARENT_START_MARKER = startMarker
  env.LOKI_PARENT_NONCE = nonce

  return env
}

/** Build the `LOKI_SPAWN` tag mirrored by loki_cli/process_identity.py.
