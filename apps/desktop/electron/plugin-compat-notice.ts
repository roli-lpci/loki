 ...
 *
 * The Python side (loki_cli/plugin_compat.py) statically scans the user's enabled external plugins on
 * every CLI/gateway/TUI start and writes LOKI_HOME/.plugin-compat-report.json when any plugin imports a
...
export function readReport(lokiHome: string): PluginCompatReport | null {
  try {
    const raw = fs.readFileSync(path.join(lokiHome, REPORT_FILE), 'utf8')
...
export function pendingNotice(lokiHome: string, userData: string): PendingNotice | null {
  const report = readReport(lokiHome)
