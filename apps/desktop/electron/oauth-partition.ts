 ...
 * (`persist:loki-remote-oauth`) — the jar was keyed on the auth *mode*, not
 ...
 * setup: one box, two dashboards) fought over the same `loki_session*`
 ...
 *     cascade deliberately shares one jar with the WunderCorp Portal session.
...
export const LEGACY_OAUTH_PARTITION = 'persist:loki-remote-oauth'
