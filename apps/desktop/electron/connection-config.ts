 ...
 *     `X-Loki-Session-Token` header; WS uses `?token=`.
...
// loki_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - loki_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - loki_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / loki #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-loki_session_at', '__Secure-loki_session_at', 'loki_session_at']
const RT_COOKIE_VARIANTS = ['__Host-loki_session_rt', '__Secure-loki_session_rt', 'loki_session_rt']

// The WunderCorp portal (NAS) does NOT use Loki gateway session cookies — it is a
...
// Keep this aligned with loki_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['loki', 'test', 'tmp', 'root', 'sudo'])
...
function gatewayTicketFailure(error, authMessage, transportMessage) {
  ...
  // classification (isServerSideHttpError / makeWunderCorpCloudBackendDownError) and
...
/** Which Loki profile the remote SSH dashboard should actually run as.
...
// Loki Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteLokiPath = String(entry.remoteLokiPath || '').trim()

  if (remoteLokiPath) {
    out.remoteLokiPath = remoteLokiPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Loki installation. Preserve an explicit mapping
  // when it is a valid Loki profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_loki_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `LOKI_HOME` scopes it.
 *
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process LOKI_HOME can belong to a different
 ...
 * `loki:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://loki.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `loki:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a loki session ACCESS-token cookie
...
/**
 * True if the cookie jar holds a live WunderCorp PORTAL (Privy) session — a non-empty
 ...
 * the Loki gateway session cookies, so cloud sign-in / discovery liveness
