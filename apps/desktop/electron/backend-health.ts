...
export interface LokiReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForLokiReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote Loki gateway uses OAuth, but you are not signed in. ' +
...
/**
 * The one factory for the actionable WunderCorp Cloud agent-is-down error, shared by
 ...
 *  - readiness-probe exhaustion in waitForLokiReady().
 *
 * Returns null unless the backend is a *.agents.wundercorp.com host AND the
 ...
 */
export function makeWunderCorpCloudBackendDownError(baseUrl: string, error: unknown): Error | null {
  if (!isWunderCorpCloudAgentUrl(baseUrl)) {
    return null
  }
  ...
  try {
    ...
    // baseUrl is known to parse (isWunderCorpCloudAgentUrl already did); keep the raw
  ...
  const err = new Error(
    `WunderCorp Cloud agent ${hostname} is down ` +
      `(HTTP ${serverError.statusCode}: server-side fault). ` +
      'Check https://portal.wundercorp.com for backend status, ' +
      'or switch to Local mode in Settings → Gateway. ' +
      'You can also reach out on Discord at discord.gg/yEaT8dv5Xn ' +
...
/**
 * True when the backend URL points at a WunderCorp-managed Loki Cloud instance
 * (e.g. ares-3009.agents.wundercorp.com). These are Fly.io-hosted machines
 * the user cannot restart themselves — a 503 from one means the server is down
 * and the recovery path is Portal/Discord/wait.
 */
export function isWunderCorpCloudAgentUrl(baseUrl: string): boolean {
  try {
    const host = new URL(baseUrl).hostname

    return host.endsWith('.agents.wundercorp.com')
  } catch {
    return false
  }
}
 ...
 * `isReauthRequired` so startLoki latches; `needsOauthLogin` alone only
...
export async function waitForLokiReady(baseUrl: string, options: LokiReadyOptions): Promise<void> {
  ...
  // When a WunderCorp-managed cloud agent returns a server-side HTTP error
  ...
  const cloudError = makeWunderCorpCloudBackendDownError(baseUrl, lastError)

  if (cloudError !== null) {
    throw cloudError
  }

  throw new Error(`Loki backend did not become ready: ${detail}`)
}
