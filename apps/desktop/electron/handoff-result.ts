 ...
 * console. It writes LOKI_HOME/.loki-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Loki within 30 minutes.
...
export function handoffResultPath(lokiHome: string): string {
  return path.join(lokiHome, '.loki-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(lokiHome)
  let raw: string
