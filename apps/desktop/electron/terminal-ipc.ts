...
// registry, and the loki:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // LOKI_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.LOKI_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Loki' TUI into
    ...
    env.TERM_PROGRAM = 'Loki'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a loki/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from LOKI_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.LOKI_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `loki:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('loki:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('loki:terminal:attach', (event, id) => {
  ...
  ipcMain.handle('loki:terminal:write', (_event, id, data) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('loki:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('loki:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('loki:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
