...
export function registerHudIpc({
  ...
  ipcMain.on('loki:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('loki:hud:windowing', event => {
  ...
  ipcMain.on('loki:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('loki:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('loki:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('loki:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('loki:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('loki:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('loki:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('loki:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('loki:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('loki:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('loki:hud:close', async () => {
    closeHudWindow()
