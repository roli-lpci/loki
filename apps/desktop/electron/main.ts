...
import { buildDesktopBackendEnv, lokiManagedNodePathEntries, normalizeLokiHomeRoot } from './backend-env'
import {
  isReauthRequiredError,
  makeWunderCorpCloudBackendDownError,
...
import { isLokiOwnedVenvDaemon } from './venv-holder-select'
...
const USER_DATA_OVERRIDE = process.env.LOKI_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.LOKI_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.LOKI_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with LOKI_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[loki] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[loki] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. LOKI_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[loki] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[loki] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via LOKI_DESKTOP_PASSWORD_STORE, bridged by the
// `loki desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[loki] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[loki] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `loki desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Loki sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[loki] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[loki] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[loki] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[loki] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[loki] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('loki:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Loki burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[loki] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[loki] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[loki] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[loki] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// LOKI_HOME — the user-facing root for everything Loki-related. Mirrors
// scripts/install.ps1's $LokiHome and scripts/install.sh's $LOKI_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\loki (matches install.ps1)
//   macOS / Linux: ~/.loki (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.loki directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\loki yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// LOKI_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// LOKI_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.loki / %LOCALAPPDATA%\loki.
function resolveLokiHome() {
  if (process.env.LOKI_HOME) {
    return normalizeLokiHomeRoot(process.env.LOKI_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'loki-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a LOKI_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\loki and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('LOKI_HOME')

    if (fromRegistry) {
      return normalizeLokiHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'loki')
    const legacy = path.join(app.getPath('home'), '.loki')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.loki setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.loki')
}

const LOKI_HOME = resolveLokiHome()

function pathWithLokiManagedNode(...entries) {
  const managed = lokiManagedNodePathEntries(LOKI_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_LOKI_ROOT — the canonical mutable Loki install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_LOKI_ROOT = path.join(LOKI_HOME, 'loki-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_LOKI_ROOT, 'venv')
...
// boots, going straight to `resolveLokiBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_LOKI_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_LOKI_ROOT, '.loki-bootstrap-complete')
...
// active-profile.json records which Loki profile the desktop launches its
// local backend as. When set, startLoki() passes `loki --profile <name>
// dashboard …`, which deterministically pins LOKI_HOME (see
// _apply_profile_override in loki_cli/main.py) and bypasses the sticky
// ~/.loki/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors loki_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// lokiDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under LOKI_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by loki_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(LOKI_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (loki_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.LOKI_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.LOKI_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.LOKI_DESKTOP_SKIP_QUIT_CONFIRM === '1'
// WunderCorp free tier gate, decided ONCE here and stamped onto every backend spawn
// (desktopBackendSpawnEnv) and the renderer (loki:launch-flags).
const GUEST_ONBOARDING = guestOnboardingEnabled()

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.LOKI_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.LOKI_DESKTOP_APP_NAME || 'Loki'
...
// 'loki:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.wundercorp.lokiagent) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.wundercorp.lokiagent')
}

// Seed the native About panel with the live Loki version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `loki update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveLokiVersion(),
  copyright: 'Copyright © 2026 WunderCorp, Inc.'
})
...
// startLoki(); this pool only holds EXTRA profile
...
// The legacy LOKI_DESKTOP_POOL_* env vars remain the initial-value fallback
...
function readPersistedPoolLimits() {
  try {
    ...
    const fromEnv = clampPoolLimits({
      maxBackends: Number(process.env.LOKI_DESKTOP_POOL_MAX) || undefined,
      idleMs: Number(process.env.LOKI_DESKTOP_POOL_IDLE_MS) || undefined
    })
...
function logPoolSpawnFailure(label: string, error: unknown): void {
  if (isBackgroundSlotWaitTimeout(error)) {
    ...
    rememberLog(
      `Loki backend for profile ${label} failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
//   * WSL2 IPC stall  = the renderer's `loki:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.LOKI_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startLoki() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning loki serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Loki backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  if (lokiLog.length > 300) {
    lokiLog.splice(0, lokiLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-loki-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('loki:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the loki:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('loki:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_LOKI_ROOT
  })
}
...
// The Tauri updater writes LOKI_HOME/.loki-update-in-progress for the whole
...
// (`force_kill_other_loki`, taskkill /IM loki.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(LOKI_HOME)),
...
const BUNDLE_SWAP_RELAUNCH_FLAG = '--loki-bundle-swap-relaunched'
...
// The detached updater swaps the packaged bundle on disk AFTER `loki update`
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Loki will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(LOKI_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Loki update did not finish',
        `${result.message}\n\nDetails: ${path.join(LOKI_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
  ...
  if (outcome === 'timeout') {
    ...
    await advanceBootProgress('backend.update-restart', 'Restarting Loki to load the updated app…', 14)
...
function findOnPath(command) {
  ...
  // shell-script shim named `loki` — must not shadow `loki.cmd`/`loki.exe`.
...
function unwrapWindowsVenvLokiCommand(command, backendArgs) {
  return resolveVenvLokiCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `loki serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `loki` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'loki_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `loki --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, LOKI_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveLokiBackend step 4); execFileSync of a .cmd without
...
function isLokiSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'loki_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.LOKI_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Loki deps (notably pywinpty's Rust-built
    ...
    for (const version of SUPPORTED_VERSIONS) {
      try {
        const out = execFileSync(
          ...
          // pathologically wedged reg.exe can't hang the synchrowundercorp boot
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the loki-import
            // probes, but still python.exe under cold cache / AV scan, so
            // share the probe budget rather than running unbounded (this
            // synchrowundercorp exec previously had no timeout at all).
            timeout: PROBE_TIMEOUT_MS
          })
        )
...
// findGitBash — locate bash.exe on Windows. Resolves LOKI_GIT_BASH_PATH
...
// normal LOKI_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\loki\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'loki', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'loki', 'git', 'bin', 'git.exe'))
  }
...
function recentLokiLog() {
  return lokiLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's loki root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_LOKI_ROOT.
// LOKI_DESKTOP_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.LOKI_DESKTOP_ROOT && path.resolve(process.env.LOKI_DESKTOP_ROOT),
    !IS_PACKAGED && isLokiSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isLokiSourceRoot(ACTIVE_LOKI_ROOT) ? ACTIVE_LOKI_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_LOKI_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('loki:updates:progress', merged)
  }
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'loki-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
// Windows that binary owns ALL repo mutation — running `loki update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(LOKI_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `loki update` can write
// fresh entry points. On Windows this is the file the running backend
// `loki.exe` holds open; on POSIX it's never mandatory-locked.
function venvLokiShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'loki.exe')
    : path.join(updateRoot, 'venv', 'bin', 'loki')
}
...
// Kill only Loki-OWNED venv daemons (the memory plugin's hindsight daemon:
...
// venv files mapped. External holders (a user terminal running `loki`,
...
function killLokiOwnedVenvDaemons(updateRoot) {
  if (!IS_WINDOWS) {
    return
  }
  ...
  try {
    ...
    holders = (Array.isArray(parsed) ? parsed : [parsed]).filter(p =>
      isLokiOwnedVenvDaemon(p?.ExecutablePath, p?.CommandLine, scriptsDir)
    )
  } catch {
  ...
  for (const holder of holders) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      rememberLog(`[updates] stopping Loki-owned venv daemon (hindsight) PID ${pid} before hand-off`)
...
// only signals the direct child, so on Windows a backend `loki.exe` that
// spawned its own grandchildren (a `loki` REPL, a pty terminal session, the
// gateway) would survive and keep the venv shim locked. taskkill /T /F reaps
// the whole tree synchrowundercorply. Windows-only: this is called solely from the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Loki backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Loki backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.lokiBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Loki backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.lokiBackendIdentity
...
// `lokiProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `loki.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a loki
// REPL, a pty terminal, the gateway) keep `loki.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const lokiProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\loki.exe, but the
  // backend is `python.exe -m loki_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (lokiProcess && Number.isInteger(lokiProcess.pid)) {
    initialPids.push(lokiProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(lokiProcess, {
  ...
  // `loki gateway stop --all`: the CLI discovers every profile's gateway
  ...
  stopGatewayBeforeUpdate(venvLokiShimPath(updateRoot), LOKI_HOME)

  // Reap Loki-OWNED venv daemons the tree-kill above cannot reach: the
  ...
  killLokiOwnedVenvDaemons(updateRoot)

  const shim = venvLokiShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentLokiProcess = backendConnectionState.getProcess()

        if (currentLokiProcess && Number.isInteger(currentLokiProcess.pid)) {
          stragglers.push(currentLokiProcess.pid)
        }
...
// `loki update`). Instead we spawn the staged Loki-Setup binary with
// --update and quit, so it can run `loki update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `loki update` → relaunch),
      ...
      // in: the LOKI_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `loki update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `loki desktop`, never the Tauri installer that self-copies
      // loki-setup.exe into LOKI_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `loki` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `loki update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `loki update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, lokiRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(LOKI_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Loki — this window will close and the updater will open. Don’t reopen Loki yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(LOKI_HOME, rememberLog)
    ...
    // loki.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running loki, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Loki install open ' +
        '(a second Loki window or a terminal running loki?). Close it and retry.'
      ...
      if (IS_WINDOWS) {
        ...
        startGatewaysAfterUpdateAbort(venvLokiShimPath(updateRoot))
      }
    ...
    // Loki processes running from this venv.  The updater normally refuses
    ...
    if (IS_WINDOWS) {
      ...
      // is asynchrowundercorp on Windows: even after releaseBackendLock's PID-exit
    ...
    // `loki update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `loki update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          LOKI_HOME,
          LOKI_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithLokiManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `loki update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(LOKI_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Loki update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(LOKI_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Loki will keep running — try again, or run \`loki update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvLoki = path.join(venvBin, IS_WINDOWS ? 'loki.exe' : 'loki')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Loki launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the loki
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.loki-bootstrap-complete')),
      hasVenvLoki: fileExists(venvLoki),
...
// Python-level pre-update snapshot inside `loki update`.
function preflightStateDb(lokiHome, rememberLog) {
  const stateDbPath = path.join(lokiHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(lokiHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(lokiHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(lokiHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `loki update`, swaps/relaunches the app bundle, and writes
// .loki-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'loki update', percent: null })

    return { ok: true, manual: true, command: 'loki update', lokiRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      LOKI_HOME,
      LOKI_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithLokiManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Loki — this window will close. Don’t reopen Loki yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_LOKI_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isLokiSourceRoot(ACTIVE_LOKI_ROOT) &&
    fileExists(venvPython) &&
    canImportLokiCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_LOKI_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `loki
...
function resolveWebDist() {
  const override = process.env.LOKI_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set LOKI_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: loki desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: loki desktop --force-build`
  )
...
function resolveLokiCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Loki.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveLokiCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'loki_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_LOKI_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Loki at ${ACTIVE_LOKI_ROOT}`,
    command,
    args: ['-m', 'loki_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      lokiHome: LOKI_HOME,
      pythonPathEntries: [ACTIVE_LOKI_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_LOKI_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveLokiBackend(backendArgs) {
  // 1. Explicit override -- LOKI_DESKTOP_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.LOKI_DESKTOP_ROOT && path.resolve(process.env.LOKI_DESKTOP_ROOT)

  if (overrideRoot && isLokiSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Loki source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `loki` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isLokiSourceRoot.)
  if (!IS_PACKAGED && isLokiSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Loki source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_LOKI_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\loki\\loki-agent (Windows) or ~/.loki/loki-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Loki runtime at ${ACTIVE_LOKI_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `loki` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    LOKI_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.LOKI_DESKTOP_IGNORE_EXISTING !== '1') {
    let lokiCommand = null
    const lokiOverride = process.env.LOKI_DESKTOP_CLI

    if (lokiOverride) {
      const resolvedOverride = findOnPath(lokiOverride)

      if (resolvedOverride) {
        lokiCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(lokiOverride, { isWsl: IS_WSL })) {
        lokiCommand = lokiOverride
      } else {
        rememberLog(`Ignoring Windows Loki override under WSL: ${lokiOverride}`)
      }
    } else {
      lokiCommand = findOnPath('loki')
    }

    if (lokiCommand) {
      if (looksLikeDesktopAppBinary(lokiCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Loki CLI: ${lokiCommand}`)
        lokiCommand = null
      }
    }

    if (lokiCommand) {
      const unwrapped = unwrapWindowsVenvLokiCommand(lokiCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `loki` shim
      ...
      const shellForProbe = isCommandScript(lokiCommand)

      // LOKI_DESKTOP_CLI is an explicit deployment override (used by
      ...
      if (shouldTrustLokiOverride(lokiOverride) || verifyLokiCli(lokiCommand, { shell: shellForProbe })) {
      ...
      rememberLog(
        `Ignoring existing Loki CLI at ${lokiCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed loki_cli module via system Python.
  ...
  if (python) {
    ...
    // having loki_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\loki\loki-agent\venv.
    if (canImportLokiCli(python)) {
      return {
        kind: 'python',
        label: `installed loki_cli module via ${python}`,
        command: python,
        args: ['-m', 'loki_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: loki_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveLokiBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Loki Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveLokiBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Loki install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Loki recovery was handed off to Loki Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      lokiHome: LOKI_HOME,
      logRoot: path.join(LOKI_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Loki install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Loki bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(LOKI_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startLoki() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // loki:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveLokiBackend(backend.args))
  }
  ...
  if (!isLokiSourceRoot(ACTIVE_LOKI_ROOT)) {
    throw new Error(
      `Loki install at ${ACTIVE_LOKI_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Loki' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\loki\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `loki` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Loki on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isLokiSourceRoot()
    // plus an importable loki_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Loki venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Loki at ${ACTIVE_LOKI_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Loki runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----loki-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Loki backend URL protocol: ${parsed.protocol}`))
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the Loki backend.'
                  )
                )
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Loki backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Loki-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Loki-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('loki:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveLokiCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('loki:preview-file-changed', payload)
}
...
// waitForLokiReady how to read a 401 — rejected session vs gated route.
...
async function waitForLoki(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForLokiReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('loki:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('loki:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('loki:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('loki:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Loki...".
function sendPowerResume() {
  ...
  webContents.send('loki:power-resume')
}
...
// 'loki:power-battery' push below.
ipcMain.handle('loki:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('loki:power-battery', next)
    }
  }
}
...
async function showPluginCompatNoticeOnce() {
  ...
  try {
    notice = pendingPluginCompatNotice(LOKI_HOME, app.getPath('userData'))
  } catch (err) {
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('loki:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('loki:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('loki:context-menu-spellcheck', {
...
function installMediaPermissions() {
  ...
  // Synchrowundercorp check handler: Chromium consults this for getUserMedia on
...
//
// Hosted Loki gateways gate the dashboard behind an OAuth provider (e.g.
// WunderCorp, Inc.) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``loki_session_at``),
...
//   * WunderCorp Portal now issues a 24h ROTATING, reuse-detected refresh token
//     alongside the ~15-min access token (Portal NAS #293 / loki #37247).
//     Both are set as HttpOnly cookies (``loki_session_at`` ~15 min,
//     ``loki_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// "not signed in", which on the initial boot path (startLoki → the renderer's
// single-shot boot() with no retry) surfaced as the "Loki couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Loki
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Loki Cloud agent…' : 'Sign in to Loki gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Loki backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in loki_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Loki Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
// The "cloud" connection mode lets a user sign in to the WunderCorp portal ONCE in
...
// portal session cookie living in `persist:loki-remote-oauth`:
...
// Canonical WunderCorp portal base URL, overridable for staging/dev. Mirrors the CLI
// convention (loki_cli/auth.py DEFAULT_WUNDERCORP_PORTAL_URL + the same env names)
// so a single override flips every Loki surface to the same portal.
const DEFAULT_WUNDERCORP_PORTAL_URL = 'https://portal.wundercorp.com'

function resolvePortalBaseUrl() {
  const raw = process.env.LOKI_PORTAL_BASE_URL || process.env.WUNDERCORP_PORTAL_BASE_URL || DEFAULT_WUNDERCORP_PORTAL_URL

  return String(raw).trim().replace(/\/+$/, '')
}

// Whether the OAuth partition currently holds a live WunderCorp portal session — the
// credential that powers both discovery and the silent cascade. The portal
// authenticates via PRIVY, not the Loki gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for loki_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Loki Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Loki Cloud sign-in.'))
...
// Discover the hosted (Loki Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Loki Cloud. Open Settings → Gateway, choose Loki Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Loki Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Loki Cloud session has expired. Sign in to Loki Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Loki Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every loki:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteLokiPath`/`remoteProfile` —
...
// a valid stored value (pins the root LOKI_HOME explicitly); null means "no
...
// contains "loki", avoiding false positives from stale gateway.pid files
// whose PID was recycled by the OS to an unrelated process.
function isLokiProcess(pid) {
  try {
...
//   1. Legacy ~/.loki/active_profile (explicit CLI choice via loki profile use)
//   2. Running gateway (gateway.pid with verified liveness + loki identity)
...
function migrateActiveProfileIfMissing() {
  migrateActiveProfileIfMissingPure(DESKTOP_PROFILE_CONFIG_PATH, {
    legacyActivePath: path.join(LOKI_HOME, 'active_profile'),
    lokiHome: LOKI_HOME,
    profilesRoot: path.join(LOKI_HOME, 'profiles'),
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.LOKI_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.LOKI_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Loki Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteLokiPath: (ssh || savedSsh)?.remoteLokiPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by LOKI_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Loki Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Loki Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteLokiPath: input.sshRemoteLokiPath ?? existingBlock.remoteLokiPath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (loki #37247), and the gateway middleware transparently
    ...
    try {
      ...
      // For a WunderCorp-managed Cloud agent, a 502/503/504 from the WS-ticket mint
      ...
      // else (#85335).
      const cloudError = makeWunderCorpCloudBackendDownError(baseUrl, error)
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Loki gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Loki gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.LOKI_DESKTOP_REMOTE_TOKEN,
      url: process.env.LOKI_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      lokiPath: result.lokiPath,
      lokiHome: result.lokiHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { lokiPath: result.lokiPath, lokiHome: result.lokiHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteLokiPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      platform,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteLokiPath: sshConfig.remoteLokiPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForLoki: (baseUrl, token) => waitForLoki(baseUrl, token, lease.signal, 'token'),
      ...
      // spawn command adds LOKI_GUEST_ONBOARDING=1 only when this is on.
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        lokiVersion: result.lokiVersion || '',
        ...
        // per-profile pools also use bare keys. Only startLoki' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.lokiVersion || 'loki (version unknown)'} at ${result.lokiPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteLokiVersion: result.lokiVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (LOKI_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.LOKI_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startLoki() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Loki
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with WunderCorp, Inc." instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteLokiPath: input.sshRemoteLokiPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteLokiPath || '')
          let lokiPath
          let lokiVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            lokiPath = runtime.lokiPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.lokiPath])
            lokiVersion = inspection.version
            supported = inspection.supported
          } else {
            lokiPath = await remoteLifecycle.locateLoki(ssh, sshConfig.remoteLokiPath || '')
            lokiVersion = await remoteLifecycle.probeLokiVersion(ssh, lokiPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, lokiPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Loki on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteLokiPath: lokiPath,
            remoteLokiVersion: lokiVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startLoki())
  ...
  // connect to Loki gateway". Mirror the renderer's connect here so the test
...
function resetLokiConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const lokiProcess = backendConnectionState.invalidate()
  stopBackendChild(lokiProcess)
...
// startLoki() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetLokiConnection() invalidates it.
  const lokiProcess = backendConnectionState.getProcess()
  const dying = lokiProcess && !lokiProcess.killed ? lokiProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('loki:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('loki:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile, opts: { spawnPriority?: LocalBackendSpawnPriority } = {}) {
  ...
  if (route.backend === 'primary') {
    const connection = await startLoki()
  ...
  // spawning, so two children never share one profile's LOKI_HOME.
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteLokiPath: source.remoteLokiPath,
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. loki:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: lokiLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForLoki(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.lokiPath || !state?.lokiHome) {
  ...
  return {
    ...
    lokiPath: state.lokiPath,
    lokiHome: state.lokiHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteLokiPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.lokiPath ? platform : await probeWindowsRemote(ssh, config.remoteLokiPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          lokiPath: runtime.lokiPath,
          lokiHome: runtime.lokiHome,
          pythonPath: runtime.python
        }
      }
    }

    const lokiPath = await remoteLifecycle.locateLoki(ssh, config.remoteLokiPath || '')
    const lokiHome = await remoteLifecycle.probeRemoteLokiHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, lokiPath, lokiHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { lokiPath: state.lokiPath, lokiHome: state.lokiHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForLoki(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startLoki() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForLoki(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited LOKI_HOME env (see _apply_profile_override
  // step 3 in loki_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveLokiBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const lokiCwd = resolveLokiCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(LOKI_HOME, 'profiles', key))
  )
  rememberLog(`Starting Loki backend for profile "${profile}" via ${backend.label}`)
      ...
      env: desktopBackendSpawnEnv(
        {
          ...
          TERMINAL_CWD: lokiCwd,
          LOKI_DASHBOARD_SESSION_TOKEN: token,
          ...
          ...parentIdentityEnv,
          LOKI_WEB_DIST: webDist,
          ...(readyFile ? { LOKI_DESKTOP_READY_FILE: readyFile } : {})
        },
        GUEST_ONBOARDING
      ),
      shell: backend.shell,
  ...
  child.once('error', error => {
    rememberLog(`Loki backend for profile "${profile}" failed to start: ${error.message}`)
    void teardownFailedLocalBackend(poolKey, entry).catch(cleanupError => {
      rememberLog(
        `Loki backend for profile "${profile}" cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`
      )
    })
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Loki backend for profile "${profile}" exited (${signal || code})`)
    releaseLocalBackendSlot(entry)
    releaseBackendChild(child)
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `Loki backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForLoki(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Loki backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Loki backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_loki_home() recreates the deleted profile directory.
//
...
async function startLoki() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Loki Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startLoki() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Loki backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Loki backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote Loki backend at ${remote.baseUrl}`, 24)
      await waitForLoki(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Loki backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, lokiLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.loki/active_profile file) and
    // resolves LOKI_HOME the same way `loki -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Loki runtime', 28)

        return resolveLokiBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Loki backend via ${backend.label}`, 84)
    rememberLog(`Starting Loki backend via ${backend.label}`)
    ...
    const lokiProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: desktopBackendSpawnEnv(
          {
            ...process.env,
            // Explicitly pin LOKI_HOME for the child so Python's get_loki_home()
            // resolves to the SAME location our resolveLokiHome() picked. Without
            // this pin, Python falls back to ~/.loki on every platform — fine on
            // mac/linux (where our default matches), but on Windows our default is
            // %LOCALAPPDATA%\loki, which differs from C:\Users\<u>\.loki.
            // Mismatch would split config / sessions / .env / logs across two
            // directories. install.ps1 sets LOKI_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(lokiProcess)
    ...
    const portAnnouncement = waitForDashboardPortAnnouncement(lokiProcess, {
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, lokiProcess)

    if (!processOwner) {
      stopBackendChild(lokiProcess)
      await waitForBackendExit(lokiProcess)
      releaseBackendChild(lokiProcess)
      throw new Error('Loki backend start was superseded by a newer connection attempt.')
    }

    lokiProcess.stdout.on('data', rememberLog)
    lokiProcess.stderr.on('data', rememberLog)
    ...
    lokiProcess.once('error', error => {
      releaseBackendChild(lokiProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Loki backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Loki backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Loki backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Loki backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    lokiProcess.once('exit', (code, signal) => {
      releaseBackendChild(lokiProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Loki backend exit (${signal || code})`)
      ...
      rememberLog(`Loki backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Loki backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Loki backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentLokiLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Loki backend to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Loki backend to become ready', 90)
    await Promise.race([waitForLoki(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => lokiProcess.exitCode === null && !lokiProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Loki backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Loki backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
    ...
    // The backend's plugin discovery just ran and refreshed LOKI_HOME/.plugin-compat-report.json.
...
function wireCommonWindowHandlers(win, { zoom = true }: { zoom?: boolean } = {}) {
  ...
  // links arrive via `loki:openExternal`, not here. See window-open-policy.ts.
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('loki:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while Loki is
...
// pushes pet state over IPC (loki:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via loki:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Loki app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Loki icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see loki:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Loki in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('loki:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Loki can be driven
...
// left the user with NO Loki window after the second toggle (#88513).
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('loki:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('loki:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[loki] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('loki:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `loki:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, loki:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('loki:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('loki:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('loki:quick-entry:shown')
}
...
function createWindow() {
  ...
  // synchrowundercorply before the window is gone.
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'loki desktop --force-build',
  ...
  startLoki().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('loki:connection', async (_event, profile, extra) => {
...
ipcMain.handle('loki:connection:for', async (_event, payload) => {
  ...
  // Same single-owner claim as 'loki:connection', keyed by the composite
...
ipcMain.on('loki:connection:active-route', (event, route) => {
...
// "Starting Loki…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('loki:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetLokiConnection({ soft: true }),
...
ipcMain.handle('loki:backend:touch', async (_event, profile) => {
...
ipcMain.handle('loki:pool-limits:get', async () => ({ ...poolLimits }))
ipcMain.handle('loki:pool-limits:set', async (_event, raw) => {
...
ipcMain.handle('loki:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('loki:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('loki:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('loki:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('loki:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveLokiBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Loki is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `loki-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, LOKI_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('loki:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('loki:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('loki:zoom:get', event => {
...
ipcMain.on('loki:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('loki:backend:recycle', async (_event, profile) => {
...
ipcMain.handle('loki:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startLoki() call restarts the
...
ipcMain.handle('loki:bootstrap:repair', async () => {
  // Forceful repair: force the next startLoki() through the full installer
  ...
  // "bypass active runtime" path inside startLoki() and fall through
...
ipcMain.handle('loki:boot-progress:get', async () => bootProgressState)
ipcMain.handle('loki:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('loki:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('loki:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('loki:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('loki:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('loki:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('loki:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('loki:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('loki:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('loki:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('loki:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('loki:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('loki:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('loki:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('loki:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteLokiPath: entry.remoteLokiPath
    })
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteLokiPath: connection.remoteLokiPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteLokiProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root LOKI_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('loki:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// loki:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('loki:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('loki:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `loki update` to every eligible registered connection at once.
...
ipcMain.handle('loki:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/loki/update', {}, { timeoutMs: 15_000 })
...
// analogue of the loki:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('loki:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('loki:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startLoki() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('loki:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- Loki Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('loki:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('loki:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('loki:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('loki:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of loki:profile:set: records the profile the
...
// the gateway (#79886).
ipcMain.handle('loki:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('loki:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new LOKI_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('loki:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('loki:requestMicrophoneAccess', async () => {
...
ipcMain.handle('loki:window:readBelow', async event => {
...
async function handleLokiApiRequest(request) {
  ...
  // backend calls ensure_loki_home() which recreates the profile directory,
...
ipcMain.handle('loki:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its LOKI_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleLokiApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleLokiApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('loki:ambient:claim', (_event, key) => ownsAmbientCue(String(key ?? '')))
...
ipcMain.handle('loki:data-url-read-max:get', () => ({
...
ipcMain.handle('loki:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('loki:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('loki:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('loki:readFileText', async (_event, filePath) => {
...
// `loki:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('loki:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('loki:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('loki:writeClipboard', (_event, text) => {
...
ipcMain.handle('loki:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('loki:readClipboard', () => clipboard.readText())

ipcMain.handle('loki:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('loki:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('loki:context-menu:edit', (event, command) => {
...
ipcMain.handle('loki:context-menu:copy-image', event => {
...
ipcMain.handle('loki:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('loki:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('loki:capturePreview', async (_event, payload) => {
  const guest = electronWebContents.fromId(Number(payload?.webContentsId))

  return capturePreviewContents(guest, payload?.rect, payload?.viewport)
})

ipcMain.handle('loki:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('loki:saveClipboardImage', async () => {
...
ipcMain.handle('loki:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('loki:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('loki:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('loki:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('loki:active-work', (event, payload) => {
  const id = event.sender.id
...
ipcMain.on('loki:titlebar-theme', (_event, payload) => {
...
ipcMain.on('loki:native-theme', (_event, mode) => {
...
// synchrowundercorp writeFileSync and doing one per tick blocks the main process
...
// Answered synchrowundercorply so preload can publish the verdict before the
...
ipcMain.on('loki:translucency:support', event => {
...
ipcMain.on('loki:translucency', (_event, payload) => {
...
ipcMain.on('loki:keep-awake', (_event, on) => {
...
ipcMain.handle('loki:quick-entry:settings:get', async () => {
...
ipcMain.handle('loki:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('loki:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('loki:quick-entry:submit', {
...
ipcMain.on('loki:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('loki:quick-entry:state', payload)
  }
})

ipcMain.on('loki:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('loki:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('loki:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `loki:window:openSession`).
...
ipcMain.handle('loki:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
  ...
  // The match count still arrives asynchrowundercorply via `found-in-page`; this
  // reply only acknowledges that Chromium has begun returning this request.
  return { count: 0 }
})

ipcMain.handle('loki:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('loki:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('loki:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveLokiCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('loki:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveLokiCwd()
}))

ipcMain.handle('loki:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('loki:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('loki:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('loki:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('loki:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('loki:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('loki:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: lokiLog.slice(-200) }))
...
ipcMain.on('loki:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (loki:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('loki:updates:check', async () =>
...
ipcMain.handle('loki:updates:apply', async (_event, payload) =>
...
ipcMain.handle('loki:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('loki:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Loki version (the one `release.py` bumps in
// loki_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Loki version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveLokiVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'loki_cli', '__init__.py')
...
// Renderer-bundle skew: `loki update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Loki version and push it into the native About panel
// just before showing it, so an in-place `loki update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveLokiVersion()} — app build out of date, update the desktop app`
        : resolveLokiVersion(),
      copyright: 'Copyright © 2026 WunderCorp, Inc.'
    })
    app.showAboutPanel()
...
ipcMain.handle('loki:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveLokiVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    lokiRoot: resolveUpdateRoot(),
...
// The About page's "Restart Loki" button (shown when bundleSwapPending):
...
ipcMain.handle('loki:app:relaunch', async () => {
...
// the actual removal via `loki uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (loki_cli/uninstall.py + loki_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_LOKI_ROOT
  ...
  const fallback = () => ({
    loki_home: LOKI_HOME,
    agent_installed: isLokiSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'loki_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, LOKI_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Loki agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import loki_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_LOKI_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `loki-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `loki-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('loki:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('loki:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('loki:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('loki:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// loki:// deep links (e.g. loki://blueprint/morning-brief?time=08:00,
// loki://mcp/install?name=NAME&config=B64 — the vendor "Add to Loki"
// button, or loki://plugin/install?repo=owner/repo). Dev
// (`LOKI_DESKTOP_DEV_SERVER`) registers loki-dev:// instead — bare
// Electron or a stale OS handler often owns loki:// on dev machines.
...
// ---------------------------------------------------------------------------
const LOKI_PROTOCOL = DEV_SERVER ? 'loki-dev' : 'loki'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['loki-dev', 'loki'] : ['loki']
...
function handleDeepLink(url) {
  ...
  // loki://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('loki:deep-link', payload)
...
ipcMain.handle('loki:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${LOKI_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(LOKI_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(LOKI_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${LOKI_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `loki://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startLoki), whose
...
app.whenReady().then(() => {
  ...
  // startLoki() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching loki:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Loki never keeps another app's chord hostage.
