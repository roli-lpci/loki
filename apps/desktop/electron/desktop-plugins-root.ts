// The standalone desktop-plugin root (`<LOKI_HOME>/desktop-plugins`) and the
...
//      is COPIED out as `<root>/<name>/` with a `.loki-package.json` marker
...
export const PACKAGE_MARKER = '.loki-package.json'
...
async function packageOrigin(packageDir: string): Promise<Pick<DesktopHalfMarker, 'catalogName' | 'repo' | 'sha'>> {
  try {
    const sidecar = JSON.parse(await fs.promises.readFile(path.join(packageDir, '.loki-catalog.json'), 'utf8')) as {
      catalog_name?: string
      repo?: string
      sha?: string
    }
...
/** Every loki home the app knows about locally: the default plus each profile. */
export async function localHomes(lokiHome: string): Promise<string[]> {
  const profiles = await listDirs(path.join(lokiHome, 'profiles'))

  return [lokiHome, ...profiles.map(name => path.join(lokiHome, 'profiles', name))]
}
...
export async function migrateProfileScopedDesktopPlugins(lokiHome: string, appRoot: string): Promise<string[]> {
  const moved: string[] = []

  for (const profile of await listDirs(path.join(lokiHome, 'profiles'))) {
    const scopedRoot = path.join(lokiHome, 'profiles', profile, DESKTOP_PLUGINS_DIR)
...
export async function reconcileUnifiedDesktopHalves(lokiHome: string, appRoot: string): Promise<string[]> {
  const touched: string[] = []
  const seen = new Set<string>()

  for (const home of await localHomes(lokiHome)) {
