// Launching the Loki TUI in the user's OWN terminal emulator.
//
...
// `loki --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Loki runtime is often a venv Python invoked
//     as `python -m loki_cli.main`, not a `loki` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Loki
