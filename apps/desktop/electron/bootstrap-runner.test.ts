...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'loki-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/loki-runner-test',
    sourceRepoRoot: null,
    lokiHome: '/tmp/loki-runner-test',
    logRoot: '/tmp/loki-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'loki-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'loki-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/loki-agent',
      lokiHome: '/tmp/loki'
    }),
    ['--dir', '/tmp/loki-agent', '--loki-home', '/tmp/loki', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/loki-agent', '--loki-home', '/tmp/loki', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/loki', '--loki-home', '/tmp/home', '--branch', 'main']
  )
})
