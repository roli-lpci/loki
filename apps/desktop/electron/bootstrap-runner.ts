 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Loki Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.loki-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(lokiHome) {
  return path.join(lokiHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.loki/loki-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(lokiHome) {
  if (!lokiHome) {
    return null
  }

  const candidate = path.join(lokiHome, 'loki-agent', 'scripts', installScriptName())
...
function cachedScriptPath(lokiHome, commit) {
  return path.join(bootstrapCacheDir(lokiHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/wundercorp/loki/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(lokiHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(lokiHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, lokiHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass LOKI_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          LOKI_HOME: lokiHome || process.env.LOKI_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, lokiHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, lokiHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--loki-home', lokiHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, lokiHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, lokiHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, lokiHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(lokiHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, lokiHome, emit })
