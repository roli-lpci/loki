...
// hardening, LOKI_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('loki:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('loki:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('loki:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('loki:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<LOKI_HOME>/desktop-plugins`,
  // resolved from the main-process LOKI_HOME (see resolveLokiHome) — NOT from
  // the connected backend. A remote backend reports its own `loki_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    const base = profile && profile !== 'default' ? path.join(lokiHome, 'profiles', profile) : lokiHome
  ...
  async function desktopPluginsRoot(): Promise<string> {
    const root = await ensureDir(path.join(lokiHome, DESKTOP_PLUGINS_DIR))
    await migrateProfileScopedDesktopPlugins(lokiHome, root)
    await reconcileUnifiedDesktopHalves(lokiHome, root)

    return root
  }

  ipcMain.handle('loki:fs:desktopPluginsRoot', async () => desktopPluginsRoot())
  ...
  ipcMain.handle('loki:fs:reconcileDesktopPlugins', async () => {
    const root = await ensureDir(path.join(lokiHome, DESKTOP_PLUGINS_DIR))

    return reconcileUnifiedDesktopHalves(lokiHome, root)
  })

  // The LOCAL logs root (`<LOKI_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where LOKI_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('loki:fs:logsRoot', async () => localPluginsRoot('logs'))

  ipcMain.handle('loki:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('loki:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('loki:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('loki:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('loki:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
