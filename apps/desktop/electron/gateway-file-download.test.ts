...
class FakeWriteStream extends EventEmitter {
  ...
  // Like fs.WriteStream: the descriptor is released asynchrowundercorply and 'close'
  // fires afterwards.
  destroy() {
    this.destroyed = true
...
const TEMP_BASENAME = /^\.loki-download-[0-9a-f]{8}\.part$/
...
test('pumpStreamToFile never unlinks a temp path it did not create when the exclusive open fails', async () => {
  ...
  const eexist: any = new Error("EEXIST: file already exists, open '/tmp/.loki-download-deadbeef.part'")
