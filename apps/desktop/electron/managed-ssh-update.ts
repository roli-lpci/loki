 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `loki update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.lokiHome, 'Loki home')
  const lokiPath = validateRemoteValue(target.lokiPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(lokiPath)

  const updateCommand =
    `env LOKI_HOME=${homeWord} ` +
    `LOKI_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'LOKI_UPDATE_ORIGIN_PROFILE=default ' +
    `LOKI_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `LOKI_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:LOKI_HOME=${psLiteral(home)}`,
    `$env:LOKI_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:LOKI_UPDATE_ORIGIN_PROFILE="default"',
    `$env:LOKI_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:LOKI_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:LOKI_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:LOKI_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:LOKI_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(lokiPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.loki-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Loki updated and every managed SSH profile is ready.'
        : restoreOk
