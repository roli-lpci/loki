 ...
 *   1. LOKI_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\loki\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect LOKI_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.LOKI_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'loki', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'loki', 'git', 'usr', 'bin', 'bash.exe'))
  }
