...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'loki-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'loki-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Loki Test'], { cwd: dir })
