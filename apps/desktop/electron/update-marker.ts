...
export function markerPath(lokiHome) {
  return path.join(lokiHome, '.loki-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(lokiHome)
  let raw
 ...
 *
 * The Tauri-based loki-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(lokiHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(lokiHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Loki to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(lokiHome, opts)
