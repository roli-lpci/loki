...
2. **Borderless elevation for floating panels.** Overlays float on
   `shadow-wundercorp` + a `--stroke-wundercorp` hairline, not thick framed boxes. In-panel
...
3. **One primitive per concern.** One `Button`, one set of control variants,
   ...
   fork.
4. **Tokens, not literals.** Reference CSS vars (`--ui-*`, `--shadow-wundercorp`,
...
```
shadow-wundercorp           /* downward-weighted, layered contact→ambient falloff */
border-(--stroke-wundercorp) /* currentColor hairline, theme-adaptive */
```
...
| `--stroke-wundercorp` | the overlay hairline (pairs with `shadow-wundercorp`) |
...
- **`BrandMark`** (`src/components/brand-mark.tsx`) is the brand glyph — the
  `wundercorp-girl` mark on a white tile, softly rounded, identical in light/dark.
...
- Cancellation is synchrowundercorp in the UI even if cleanup is async: overlays,
...
- [ ] Reuse a primitive (`Button`, `SearchField`, `SegmentedControl`,
      ...
      forking one?
- [ ] Tokens (`--ui-*`, `shadow-wundercorp`, `--stroke-wundercorp`) — zero raw colors /
      one-off shadows?
...
- [ ] Overlay uses `shadow-wundercorp` + `border-(--stroke-wundercorp)`, no hard border?
