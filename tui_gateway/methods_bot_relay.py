...
def _relay_root() -> Path:
    ...
    home = Path(os.getenv("LOKI_HOME") or os.path.expanduser("~/.loki"))
...
def _(rid, params: dict, _root=_relay_root, _run=_run_delivery) -> dict:
    ...
    GATEWAY via the one-turn ``loki -p <profile> chat -c "Bot Chat"`` transport local DMs use →
    ...
    try:
        ...
        resolved = "default" if profile.lower() == "loki" else profile
        ...
        fd, tmp = tempfile.mkstemp(prefix="loki-relay-dm-", suffix=".txt", text=True)
        try:
