...
def _profile_build_scope(profile_home):
    """Bind LOKI_HOME + secret scope for an agent build (home alone leaves get_secret() on the LAUNCH .env)."""
    if not profile_home:
        yield
        return
...
def _billing_call(rid, fn, extra: dict | None = None) -> dict:
    ...
    from loki_cli.wundercorp_billing import BillingError
    try:
        return _ok(rid, fn())
...
def _persist_branch(db, new_key: str, parent_key: str, title: str, history: list, *, source, cwd, profile_name,
    ...
    try:
        ...
        from loki_state_errors import is_disk_full_error
...
def _(rid, params: dict) -> dict:
    ...
    # ``profile`` (app-global remote mode): stored so the build and every turn re-bind LOKI_HOME.
...
def _resume_guard(ctx: _Resume) -> dict | None:
    ...
    from loki_state import SessionResumeTooLargeError, resolved_max_resume_messages
...
def _resume_eager(ctx: _Resume) -> dict:
    """Synchrowundercorp build OUTSIDE _session_resume_lock (it would stall session.close), then double-checked."""
    ...
    with _session_resume_lock:
        ...
        try:
            ...
            if (session := _sessions.get(sid)) is not None:
                ...
                # Each turn re-binds LOKI_HOME (mid-turn memory/skills reads); lease claimed lazily on turn 1.
...
def _(rid, params: dict) -> dict:
    ...
    if not (raw := _str_param(params, "cwd")):
        return _err(rid, 4016, "cwd required")
    from loki_constants import translate_cwd_for_wsl_backend
...
def _(rid, params: dict) -> dict:
    ...
    # the WS-detached sentinel: detached is attachable until grace-reap, and ``loki --tui`` rides stdio.
...
def _(rid, params: dict) -> dict:
    ...
    with _profile_db(params) as db:
        ...
        try:
            home = Path(profile_home) if profile_home is not None else get_loki_home()
...
def _(rid, params: dict, session: dict) -> dict:
    ...
    # WunderCorp credits are agent-independent (portal fetch); fail-open when absent.
    with contextlib.suppress(Exception):
        from agent.account_usage import wundercorp_credits_lines
        if credits := wundercorp_credits_lines():
...
def _(rid, params: dict, slug: str) -> dict:
    ...
    from agent.pet import store
    from loki_cli.pets import _clear_active_if
...
def _pet_config_followup(what: str, fn, *args) -> None:
    """Best-effort ``loki_cli.pets`` active-slug update after a store op that already succeeded."""
    try:
        fn(*args)
    except Exception as exc:  # noqa: BLE001
...
def _(rid, params: dict, slug: str) -> dict:
    ...
    if new_slug != slug:
        from loki_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    from loki_cli.pets import _set_enabled
    _set_enabled(False)
...
def _(rid, params: dict) -> dict:
    ...
    from loki_cli.pets import set_pet_scale
...
def _(rid, params: dict) -> dict:
    """Read-only billing view (no scope required); fail-open. The WunderCorp free tier has no account to
    ...
    try:
        ...
        from loki_cli.anon_auth import guest_carries_inference
...
def _(rid, params: dict) -> dict:
    ...
    from loki_cli.wundercorp_billing import post_subscription_preview
...
def _billing_route(name: str, call, *, invalid=None, message: str = "", error: str = "invalid_request",
    ...
    """Portal write route on ``loki_cli.wundercorp_billing`` (lazy; tests patch its functions): ``invalid(params)``
...
def _(rid, params: dict) -> dict:
    ...
    def call():
        ...
        granted = step_up_wundercorp_billing_scope(
            open_browser=False,
...
def _(rid, params: dict, session: dict) -> dict:
    ...
    lines = [
        "Loki TUI Status", "", f"Session ID: {key}", f"Path: {display_loki_home()}",
...
def _(rid, params: dict, session: dict) -> dict:
    ...
    saved_dir = get_loki_home() / "sessions" / "saved"
    try:
        saved_dir.mkdir(parents=True, exist_ok=True)
    except Exception as e:
        return _err(rid, 5011, f"failed to create save directory {saved_dir}: {e}")
    path = saved_dir / f"loki_conversation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with session["history_lock"]:
        messages = list(session.get("history", []))
