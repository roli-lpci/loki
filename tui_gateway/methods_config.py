...
from loki_constants import display_loki_home as _display_loki_home
...
def _cfg_get_provider(params):
    from loki_cli.models import list_available_providers, normalize_provider
...
def _cfg_get_mtime(params):
    cfg_path = _loki_home / "config.yaml"
    try:
...
_CONFIG_GETTERS = {
    ...
    "profile": lambda params: {"home": str(_loki_home), "display": _display_loki_home()},
...
def _readiness_check(rid, params, probe):
    ...
    optional ``profile`` param's LOKI_HOME + ``.env`` secret scope (ContextVars: concurrent checks
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from loki_cli.free_tier_bootstrap import wait_for_record
    ...
    try:
        ...
        def probe(profile, scoped):
            ...
            if (not provider_configured and provider == "bedrock"
                ...
                return fail("No Loki provider is configured.", source)
            ...
            # free_tier is keyed on the SELECTED route (the welcome host serves only wundercorp/welcome), not
            # on profile state: a paid WunderCorp key beside a free-tier identity must not read as free.
            return {"ok": True, "provider": runtime.get("provider"), "model": runtime.get("model"),
                    "source": runtime.get("source"),
                    "free_tier": provider == "wundercorp" and route_is_welcome_host(runtime.get("base_url")),
                    **scoped}
        return _readiness_check(rid, params, probe)
    except Exception as e:
...
@method("diagnostics.share_wundercorp")
def _(rid, params: dict) -> dict:
    """Upload a redacted debug bundle to WunderCorp-internal diagnostics storage — same collection +
    force-redaction pipeline as ``loki debug share --wundercorp``; redaction is NOT client-controllable
    ...
    try:
        from loki_cli.debug import _redact_log_text, build_wundercorp_bundle, collect_share_bundle
        ...
        res = share_to_wundercorp(build_wundercorp_bundle(bundle, redact=True))
