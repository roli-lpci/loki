...
def _is_repo_junk(root: str) -> bool:
    ...
    LOKI_HOME. User-created projects pointing there are still honored."""
    if not root:
        return True
    from loki_constants import get_loki_home
    real = os.path.realpath(root)
    loki_home = os.path.realpath(str(get_loki_home()))
    return (
        os.path.normcase(real) in _non_workspace_dirs()
        or real == loki_home
        or real.startswith(loki_home + os.sep))


def _is_session_cwd_junk(cwd: str) -> bool:
    """A non-git cwd that stays in flat Recents. A DESCENDANT of LOKI_HOME may be an
    intentional prose/data workspace, so only LOKI_HOME itself is excluded here."""
    if not cwd:
        return True
    from loki_constants import get_loki_home
    real = os.path.normcase(os.path.realpath(cwd))
    loki_home = os.path.normcase(os.path.realpath(str(get_loki_home())))
    return real in _non_workspace_dirs() or real == loki_home
...
def _scan_discovered_repos_remote(conn, policy: dict) -> bool:
    ...
    host must scan its own disk so repos with zero Loki sessions still appear in the sidebar (#81723).
